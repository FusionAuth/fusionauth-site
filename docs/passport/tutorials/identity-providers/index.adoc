...
An Identity Provider in Passport is a configuration that represents an external identity provider. The link:../../apis/identity-providers[Identity Providers]
APIs allow you to configure and manage how these external identity providers are utilized by Passport to perform federated authentication.
...
A concrete example of this scenario is when you have implemented Passport as your primary identity
...
which can be utilized by Passport to federate login to ADFS and then reconcile the user to Passport.

For example, during Login, collect the User's email address and utilize the link:../../apis/identity-providers#lookup-an-identity-provider[Lookup] API to
...
----
include::docs/passport/src/json/identity-providers/lookup/response.json[]
----
...
to obtain the access token. This access token is assumed be a JSON Web Token which you'll then send to Passport to reconcile this token and
complete the login process.

To complete the login process by reconciling the User represented by the JWT issued by the external identity provider send this JWT to Passport
...
token payload the User will be created if they do not yet exist in Passport, and then registered for the requested Passport Application.
