...
A Passport Webhook is intended to consume JSON events emitted by Passport. Creating a Webhook allows you to tell
Passport where you would like to receive these JSON events.

Webhooks provides a publish  - subscribe style integration with Passport. Creating a Webhook is the subscribe portion
...
This API is used to create a Webhook. Specifying an Id on the URI will instruct Passport to use that Id when creating the Webhook. Otherwise, Passport will create a Id for the Webhook automatically.
...
include::docs/passport/apis/_webhook-request-body.adoc[]
...
include::docs/passport/apis/_webhook-response-body.adoc[]
...
include::docs/passport/apis/_webhooks-response-body.adoc[]
