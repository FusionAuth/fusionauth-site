...
////
TODO update screenshot - two new fields, lambda and callback read only field, etc, and shorter domain textarea
////
...
link:https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-overview[Azure Active Directory v2.0] returns inconsistent claims from the UserInfo endpoint depending on the type of Microsoft account the end-user has.  See link:https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-overview[this open issue] for reference.
====

Once you have completed this configuration you will be able to enable an OpenID Connect "Login with Azure AD" button for one or more FusionAuth Applications.  See link:https://docs.microsoft.com/en-us/azure/active-directory/develop/quickstart-register-app[Azure's Register An App Quickstart Guide] as an additional reference.
...
Here we have configured our application `Redirect URI`.  This will need to be the same as the `Redirect URL` value you specify in your FusionAuth OpenID Connect Identity Providers configuration.
...
This will take you to the `Add OpenID Connect` panel, and you'll fill out the required fields.  `Client Id`, `Client secret`, and `Redirect URL` values reference the previously noted Azure AD Application's `Application (client) ID`, client secret `VALUE`, and `Redirect URI` respectively.
