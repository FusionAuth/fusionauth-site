...
For example, when a new user attempts to log into `Piped Piper` using Google, if their user does not exist in FusionAuth it will be created dynamically, and if the `Create registration` toggle has been enabled, the user will also be registered for `Piped Piper` and assigned any default roles assigned by the application.
