...
* <<Options - JWT>>
* <<Options - Managed Domains>>
* <<Options - Verification Keys>>
* <<Options - OAuth>>
...
To enable this identity provider for an application, find your application name in the table. You will always see the `FusionAuth` application, this application represents the FusionAuth user interface. If you wish to be able to log into FusionAuth with this provide you may enable this application.

In the above screenshot you will see that we have enabled this login provider for the `Piped Piper` application and enabled `Create registration`. Enabling create registration means that a user does not need to be manually registered for the application prior to using this login provider.
