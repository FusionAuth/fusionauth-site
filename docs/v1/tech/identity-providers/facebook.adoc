...
Adding a Login with Facebook button to the FusionAuth login is simple, and this guide will walk you through the steps necessary to collect the credentials from Facebook in order to enable this Social login.
...
In the following screenshot you will see that we have enabled this login provider for the `Piped Piper` application and enabled `Create registration`. Enabling create registration means that a user does need to be manually registered for the application prior to using this login provider.
