...
You may modify this file in order to change your configuration, Passport will need to be restarted to pick up changes to this configuration.
...
This configuration value is used by the FusionAuth Search and the Passport Backend services. Even when running Passport Backend without
...
is utilized by the Passport backend webservice. If this property is not defined it will be generated based upon the value of `fusionauth-search.servers`.
