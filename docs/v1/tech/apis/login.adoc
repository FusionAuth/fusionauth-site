...
By default, this API will require authentication when called with an [field]#applicationId#.

Authentication may be disabled per Application, see [field]#application.loginConfiguration.requireAuthentication# in the link:../apis/applications[Application API] or navigate to [breadcrumb]#Applications# icon:chevron-right[role=breadcrumb] [breadcrumb]#Edit# icon:chevron-right[role=breadcrumb] [breadcrumb]#Security# in the user interface.
...
[deprecated]#Removed in version 1.5.0# This parameter is no longer required to receive a Refresh Token, if it is provided on the request it will be ignored. In version 1.5.0 and later Refresh Tokens are issued by the Login API based upon the Application configuration. See [field]#loginConfiguration.generateRefreshTokens# settings in the link:./applications[Application API].
...
on the link:users#change-a-users-password[Change Password] API.
...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red]] Searches for Login Records given search criteria
[.endpoint]
.URI
--
...
|The link:../reference/data-types#instants[instant] when the login occurred.
...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red]] Export the Login Records matching the criteria
[.endpoint]
.URI
--
...
|The link:../reference/data-types#time-zone[time zone] used to adjust the stored UTC time in the export result.
...
|The end link:../reference/data-types#instants[instant] of the date/time range to include in the export.

|[field]#criteria.start# [type]#[Long]# [optional]#Optional#
|The start link:../reference/data-types#instants[instant] of the date/time range to include in the export.
