...
|The link:../reference/data-types#instants[instant] marking the expiration RSA or EC certificate.  This field is omitted for HMAC key types.

|[field]#key.id# [type]#[UUID]#
|The Id of the Key.

|[field]#key.insertInstant# [type]#[Integer]#
|The link:../reference/data-types#instants[instant] that the key was added to the FusionAuth database.
