...
The type of the identity provider will determine the objects properties as well as the validation that is performed. You can click into any of the identity provider API docs to get a list of that identity provider's properties.
...
link:authentication#no-authentication-required[icon:unlock-alt[role=green]] Lookup an Identity Provider by domain/email
[.endpoint]
.URI
--
