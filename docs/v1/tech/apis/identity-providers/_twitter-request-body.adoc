[cols="3a,7a"]
[.api]
.Request Body
|===
|[field]#identityProvider.accessToken# [type]#[String]# [required]#Required#
|The top-level access token to use with the Twitter Identity Provider. This value is retrieved from the Twitter developer website when you setup your Twitter developer account.

|[field]#identityProvider.accessTokenSecret# [type]#[String]# [required]#Required#
|The top-level access token secret to use with the Twitter Identity Provider. This value is retrieved from the Twitter developer website when you setup your Twitter developer account.

|[field]#identityProvider.applicationConfiguration# [type]#[Map<UUID, Object>]# [optional]#Optional#
|The configuration for each Application that the identity provider is enabled for.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.accessToken# [type]#[String]# [optional]#Optional#
|This is an optional Application specific override for the top level access token.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.accessTokenSecret# [type]#[String]# [optional]#Optional#
|This is an optional Application specific override for the top level access token secret.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.accessTokenSecret# [type]#[String]# [optional]#Optional#
|This is an optional Application specific override for the top level access token secret.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.buttonText# [type]#[String]# [optional]#Optional#
|This is an optional Application specific override for the top level button text.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.consumerKey# [type]#[String]# [optional]#Optional#
|This is an optional Application specific override for the top level consumer key.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.consumerSecret# [type]#[String]# [optional]#Optional#
|This is an optional Application specific override for the top level consumer secret.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.createRegistration# [type]#[boolean]# [optional]#Optional#
|Determines if a UserRegistration is created for the User automatically or not. If a user doesn't exist in FusionAuth and logs in through an identity provider, this boolean controls whether or not FusionAuth creates a registration for the User in the Application they are logging into.

|[field]#identityProvider.applicationConfiguration``[applicationId]``.enabled# [type]#[boolean]# [optional]#Optional#
|Determines if this identity provider is enabled for the Application specified by the `applicationId` key.

|[field]#identityProvider.buttonText# [type]#[String]# [optional]#Optional#
|The top-level button text to use on the FusionAuth login page for this Identity Provider.

|[field]#identityProvider.consumerKey# [type]#[String]# [required]#Required#
|The top-level consumer key to use with the Twitter Identity Provider. This value is retrieved from the Twitter developer website when you setup your Twitter developer account.

|[field]#identityProvider.consumerSecret# [type]#[String]# [required]#Required#
|The top-level consumer secret to use with the Twitter Identity Provider. This value is retrieved from the Twitter developer website when you setup your Twitter developer account.

|[field]#identityProvider.type# [type]#[String]# [required]#Required#
|This field must be set to `Twitter`.
|===

[source,json]
.Example Twitter Request JSON
----
include::../../../../src/json/identity-providers/twitter-request.json[]
----
