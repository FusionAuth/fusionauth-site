...
* Support importing Bcrypt hashes that contain a `.`. The Bcrypt Base64 encoding uses a non standard character set which includes a `.` instead of a `+` as in the standard character set. Thank you to Diego Souza Rodrigues for letting us know about this issue.
...
** Thanks to https://github.com/FusionAuth/fusionauth-issues/issues/58[@davisusanibar] for reporting the issue. https://github.com/FusionAuth/fusionauth-issues/issues/58
