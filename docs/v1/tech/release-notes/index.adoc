...
=== Enhancement
* The OAuth2 Logout endpoint is now compliant with the https://openid.net/specs/openid-connect-frontchannel-1_0.html[OpenID Connect Front-Channel Logout 1.0 - draft 02]
...
** `/.well-known/openid-configuration` now returns the following attributes:
*** `end_session_endpoint`
*** `frontchannel-logout-supported`
*** `backchannel-logout-supported`
** TL;DR The `/oauth2/logout` endpoint can call logout URLs of multiple applications and a specific Redirect URL can be requested on the URL via `post_logout_redirect_uri`.
* A new template `OAuth logout` was added for the front channel logout.
* The following theme messages were added. Please update existing themes to include them:
** `logging-out`
** `logout-title`
** `or`
...
=== Fixed
* Send email API may fail with a 500. Was introduced in 1.9.0
