...
* A public key may also be retrieved by `kid` in addition to the `applicationId` when using the link:../apis/jwt#retrieve-public-keys[Public Key API].
...
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/205[GitHub Issue #205 : Invalid SAML v2 Name Id Format], thanks to https://github.com/mikerees[@mikerees] for reporting the issue and helping us test a fix.
...
* [Pending review] Using a redirect such as `https://myapp.com/\#/login` which contains a `#` character which is not found at the end of the URL may cause an error when building the redirect URL during the Authorization Code grant.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/215[GitHub Issue #215 : Query parm issue with SPA], thanks to https://github.com/AlvMF1[@AlvMF1] for reporting the issue.
