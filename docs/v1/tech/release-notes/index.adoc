...
=== Changes
* Deprecated `jwtConfiguration.issuer`, `jwtConfiguration.algorithm`, `jwtConfiguration.secret`, `jwtConfiguration.publicKey`, `jwtConfiguration.privateKey` in `SystemConfiguration` and `Application` domain. This is all now managed through Key Master, and existing keys have been migrated into Key Master.
