...
[WARNING]
====
**Troublingshooting MySQL root user issues**
...
configured, FusionAuth might use a different hostname such as `127.0.0.1`. Therefore, you need to ensure that your MySQL server is configured to allow the `root` user
to connect from any location. To do this, you can either create a separate `root` user with a hostname of `%` or you can update the existing `root` user. Here are the
SQL statements for each method:
