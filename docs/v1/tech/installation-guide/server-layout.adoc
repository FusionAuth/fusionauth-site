...
horizontally scaled separately. The FusionAuth Search is Elasticsearch and Passport will handle the cluster configuration when multiple
...
* link:passport-backend[Install FusionAuth App]
...
should be utilized to support stateful sessions to FusionAuth. API connections to the Passport Backend are stateless
...
This configuration uses separate servers to host Passport Backend, FusionAuth Search and the database. This is a theoretical
