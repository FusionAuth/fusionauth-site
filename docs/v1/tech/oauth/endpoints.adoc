...
|The `code_challenge` parameter as described in the https://tools.ietf.org/html/rfc7636[Proof Key for Code Exchange by OAuth Public Clients (PKCE)] specification. When this parameter is provided during the Authorization request, a corresponding `code_verifier` parameter is required on the subsequent `POST` to the `/oauth2/token` endpoint.

|[field]#code_challenge_method# [type]#[String]# [optional]#Optional# [since]#Available Since 1.8.0#
|The `code_challenge_method` parameter as described in the https://tools.ietf.org/html/rfc7636[Proof Key for Code Exchange by OAuth Public Clients (PKCE)] specification. When a `code_challenge` parameter is provided and this parameter is omitted, the code challenge method defaults to "plain".

Available code challenge methods:

* `plain` - no transformation applied to code verifier to obtain code challenge
* `S256` - `BASE64URL-ENCODE(SHA256(ASCII(code_verifier)))` transformation applied to obtain code challenge
...
|The `code_verifier` parameter as described in the https://tools.ietf.org/html/rfc7636[Proof Key for Code Exchange by OAuth Public Clients (PKCE)] specification. When a `code_challenge` parameter has been provided on the antecedent `GET /oauth2/authorize` request, this parameter is then required.
