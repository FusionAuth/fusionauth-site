...
[deprecated]#Removed in version 1.5.0# This parameter is no longer required to receive a Refresh Token, if it is provided on the request it will be ignored. In version 1.5.0 and later Refresh Tokens are issued when the `offline_access` scope is requested if the Application configuration has enabled `generateRefreshTokens`. See `oauthConfiguration` settings in the link:../apis/applications[Application API].
