...
|If you have enabled `requireClientAuthentication` on the Application the user logged into, than you must provide client authentication in the form of a Basic Authorization header.

This header is created by taking the `client_id` and `client_secret` of the Application and concatenating them together using a `:` character. Here is an example:
...
This value is then Basic-64 encoded and prepended with the string `Basic`. Here is what the resulting header looks like:
