...
docker run --rm -v "$PROJECT_DIR/complete-application:/app" -w /app mcr.microsoft.com/dotnet/sdk:10.0 dotnet build
...
echo "Starting .NET API app..."
docker run --network host --name dotnet-api --rm -v "$PROJECT_DIR/complete-application":/app -w /app mcr.microsoft.com/dotnet/sdk:10.0 dotnet run &
until docker inspect dotnet-api > /dev/null 2>&1; do
  sleep 1
done
...
dotnet_ready() {
  [ "$(curl -s -o /dev/null -w '%{http_code}' -X POST http://localhost:5001/Panic 2>/dev/null)" = "401" ]
}
