...
BINARY="$APP_DIR/target/debug/your-application"
APP_PID=""
...
cargo build --manifest-path "$APP_DIR/Cargo.toml" 2>&1
pass "cargo build succeeded"

# ── Start the app ─────────────────────────────────────────────────────
echo ""
echo "Starting app on port 9012..."
(cd "$APP_DIR" && "$BINARY") > "$APP_LOG" 2>&1 &
APP_PID=$!

echo "Waiting for app to be ready..."
for i in $(seq 1 30); do
  if curl -sf --max-time 3 http://localhost:9012/ > /dev/null 2>&1; then
    echo ""
    pass "App ready on port 9012 (attempt $i/30)"
    break
  fi
...
test_home_page() {
  local status
  status=$(status_of "http://localhost:9012/")
...
test_account_unauthenticated() {
  ...
  headers=$(headers_of "http://localhost:9012/account")
  ...
  echo "$location" | grep -qE '^/$|^http://localhost:9012/$' || { fail "expected redirect to /, got: $location"; return 1; }
  pass "302 → /"
}

# Test 3: /change redirects unauthenticated users to /
test_change_unauthenticated() {
  local headers status location
  headers=$(headers_of "http://localhost:9012/change")
  status=$(echo "$headers" | grep '^HTTP/' | awk '{print $2}')
  location=$(echo "$headers" | grep -i '^location:' | tr -d '\r' | awk '{print $2}')
  [ "$status" = "302" ] || { fail "expected 302, got $status"; return 1; }
  echo "$location" | grep -qE '^/$|^http://localhost:9012/$' || { fail "expected redirect to /, got: $location"; return 1; }
...
test_login_redirect() {
  local headers status location
  headers=$(headers_of "http://localhost:9012/login")
  status=$(echo "$headers" | grep '^HTTP/' | awk '{print $2}')
  location=$(echo "$headers" | grep -i '^location:' | tr -d '\r' | awk '{print $2}')
  [ "$status" = "302" ] || { fail "expected 302, got $status"; return 1; }
  ...
  echo "$location" | grep -q 'code_challenge' || { fail "redirect is missing PKCE code_challenge: $location"; return 1; }
  pass "302 → FusionAuth OAuth endpoint with PKCE"
}
...
test_logout() {
  local headers status location
  headers=$(headers_of "http://localhost:9012/logout")
  status=$(echo "$headers" | grep '^HTTP/' | awk '{print $2}')
  location=$(echo "$headers" | grep -i '^location:' | tr -d '\r' | awk '{print $2}')
  [ "$status" = "302" ] || { fail "expected 302, got $status"; return 1; }
  echo "$location" | grep -qE '^/$|^http://localhost:9012/$' || { fail "expected redirect to /, got: $location"; return 1; }
...
run_test "/change unauthenticated redirects to /" test_change_unauthenticated
