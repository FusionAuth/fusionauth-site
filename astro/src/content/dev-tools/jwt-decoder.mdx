...
Copy and paste the following sample JWT into the decoder to try it out:
...
<form className="gap-y-6 grid my-8">
  <Textarea className="row-span-2 self-stretch height-full" id="encoded-textarea" label="Token" name="encoded-textarea" required={false} instructions="Paste a JSON web token into the text area above"/>
