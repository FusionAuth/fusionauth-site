...
Both endpoints will be protected such that a valid JSON web token (JWT) will be required in a cookie in order for the endpoints to be accessed. Additionally, the JWT must have a `roles` claim containing the appropriate role to use the endpoint.

Create the starter app called `your-application` for this API.
