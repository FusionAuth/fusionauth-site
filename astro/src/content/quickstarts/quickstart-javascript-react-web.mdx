...
▶️ Run the commands below to install the Node packages for React and the start the app server with Vite.
...
▶️ Browse again to the app at http://localhost:3000, click the login button and log in with user `richard@example.com` and password `password`. Don't click <InlineUIElement>Keep me signed in</InlineUIElement>, or you won't be able to test logout without using an incognito browser window.
