...
This app has been tested with Node v18 and React v16.2.0. This example should work with other compatible versions of Node and React.
...
First, install the FusionAuth React SDK, and React Router, which we'll use to manage the routes in our application:
...
<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/config.js"
            lang="javascript"/>

Then use this file to configure the SDK in `src/index.js`:

<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/index.js"
            lang="jsx"/>
...
```shell
cp -r ../fusionauth-quickstart-javascript-react-web/complete-application/src/assets src && \
cp -r ../fusionauth-quickstart-javascript-react-web/complete-application/src/index.css src
```

The home page will be a simple page with a welcome message and a login link. Create a new folder `pages` under `src`, in that folder create a new file `src/pages/HomePage.js`:

<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/pages/HomePage.js"
            lang="jsx"/>
...
<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/pages/AccountPage.js"
            lang="jsx"/>
...
Depending on the user's authentication state, the login or logout button should be displayed in the header. For this create a new folder `components` in `src` and a new file `src/components/LogoHeader.js`:

<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/components/LogoHeader.js"
            lang="jsx"/>

Additionally, we want to display different menu items. For this create a new file `src/components/MenuBar.js`:

<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/components/MenuBar.js"
            lang="jsx"/>

The next step is to tie it all together. Update the `src/App.js` file to add the routes and the header:

<RemoteCode url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-quickstart-javascript-react-web/main/complete-application/src/App.js"
            lang="jsx"/>
...
You can now open up an incognito window and navigate to http://localhost:3000. You will be greeted with the home page. Login with the user account you created when setting up FusionAuth, and you'll be redirected to the account page.
