...
**Step 9:** Browse to http://localhost:3000 to see the site. You can use the button at the top right to move freely between the home and account pages.
...
The `useFusionAuth()` line provides some helper functions to your code. The user is redirected to the account page if `isLoggedIn` is true. And the login button now has authentication in a single line with `startLogin()`.
...
**Step 15:** Browse again to the app at http://localhost:3000, click the login button and log in with user `richard@example.com` and password `password`. Don't click <InlineUIElement>Keep me signed in</InlineUIElement>, or you won't be able to test logout without using an incognito browser window.
