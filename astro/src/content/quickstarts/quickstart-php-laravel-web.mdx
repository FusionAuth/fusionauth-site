...
```shell
git clone https://github.com/FusionAuth/fusionauth-quickstart-php-laravel-web
...
<Aside type="note">
  It may take a few more minutes for the host to come up on http://localhost:5001 after you run the `docker compose up -d` command.
</Aside>
...
<Aside type="caution">
Normally, you should not commit your vendor folder to Git, but the Docker container won't start without all the Compose dependencies, so they are included in this guide.
</Aside>
