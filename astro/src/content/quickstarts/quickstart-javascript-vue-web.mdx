...
```shell
npm create vue@latest -- complete-application --typescript --router && cd changebank
```
...
Next, you'll need to configure and active the FusionAuth Vue SDK. You can do this by updating the `src/main.ts` file:
...
The account page will be a simple page that displays a random balance for the logged-in user. Create a new file `src/views/AccountView.vue` - or rename the existing `src/views/AboutView.vue`:
...
You can now open up an incognito window and navigate to http://localhost:5173. You will be greeted with the home page. Login with the user account you created when setting up FusionAuth, and you'll be redirected to the account page.
