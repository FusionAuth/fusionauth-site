...
In this tutorial, you are going to learn how to integrate a Java Spring Boot resource server with FusionAuth. In this case our aim is to protect an API resource from unauthorized usage.
...
An api client wants access to a resource at `/resource`. However, it is denied access until it requires an access token from FusionAuth
...
- `panic`: Tellers may call this endpoint to call the police in case of an incident. Valid roles are `teller`.
...
This example was made with `Maven`, Spring Boot version `3.1.1`, and Java 17. If you choose different options the configuration and code may be different.

If you choose you can download and unzip the package in any directory you wish and proceed to follow along with the rest of this guide. The directions will assume you have downloaded and unzipped the files into the directory `spring-api`.

### Install the dependencies

Download and unzip the Spring Initializr zip file into `spring-api`. Then run
...
From the `spring-api` directory open `src/main/resources/application.properties`.
...
- `spring.security.oauth2.resourceserver.jwt.audiences` - This is a list of the application ids in FusionAuth allowed for this resource server. FusionAuth will populate this in the `aud` claim of the JWTs and we will use that claim to validate that the token is intended for this resource server.

If you are using the kickstart it will be `http://localhost:9011/.well-known/openid-configuration/d7d09513-a3f5-401c-9685-34ab6c552453`

### Add model objects

We will need to objects to use as response from the API. One is the PanicResponse which returns a message when a successful POST call is made to `/panic`. In the base package create a new class `PanicResponse.java`.
...
Next we need an object to show the breakdown from making change. This object will hold a BigDecimal `total` and Integers `nickels` and `pennies`. In the base package create a new class `Change.java`.
...
Next we need to add controllers to handle our api calls. In the base package add a new class `MakeChangeController.java`.
...
Next we need to add a controller for tellers to call in an emergency. In the base package add a class `PanicController.java`.
...
So far we have not done anything with auth, and our controllers are unaware of authentication at all. What we want to do is protect our endpoints based on the `roles` that encoded in the JWT we receive from FusionAuth. The decoded payload of a JWT for a `teller` might look like this:
...
We need to tell Spring how to parse the `roles` out of the claim in the JWT, and for that we need a Converter. In the base package add a class `CustomJwtAuthenticationConverter.java`;
...
Because we are using a custom converter we also need to check the audience here so we pull out the `aud` claim and validate that at least of them is in the list of configured audiences for this application.

Now we can add a security configuration. In the base package create a new class `SecurityConfiguration.java`.
...
- `OAuth2ResourceServerProperties` is injected into the configuration bean and holds the values we defined in the `application.properties` file.
- In the `SecurityFilterChain` we do the following:
  1. Get our `audiences` list from the `OAuth2ResourceServerProperties`
  2. Create a new instance of our `CustomJwtAuthenticationConverter` and pass it the `audiences` list
  3. Tell the filter chain that any requests to `/make-change` must have the `customer` or `teller` role defined in the `GrantedAuthority` list we extract from the JWT.
  4. Tell the filter chain that any requests to `/panic` must have the `teller` role defined in the `GrantedAuthority` list we extract from the JWT.
  5. Set the `oauth2ResourceServer` configuration to use the `CustomJwtAuthenticationConverter` we defined above.
- `JwtDecoder` this bean tells spring to decode the JWT in the request using the information from `issuer` we defined in our properties file.

<Aside type="note">The `NimbusJwtDecoder` will automatically check that the token is not expired based on the `exp` claim.</Aside>

### Try it out

There are [several way to acquire a token](https://fusionauth.io/docs/v1/tech/oauth/) in FusionAuth, but for this example we will use the [Login API](https://fusionauth.io/docs/v1/tech/apis/login) to keep things simple.

First lets try our requests as `teller@example.com`. Based on the configuration this user has the `teller` role and should be able to use both `/make-change` and `/panic`.
...
Use that as a `Bearer` token in the `Authorization` header to make a request to `/make-change` with a query parameter `total=5.12`.
...
Uh oh, I guess we are not allowed to do that.
