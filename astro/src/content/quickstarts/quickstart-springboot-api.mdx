...
An requester wants access to an API resource at `/resource`. However, it is denied this resource until it acquires an access token from FusionAuth.
...
There are [several way to acquire a token](/docs/v1/tech/oauth/) in FusionAuth, but for this example we will use the [Login API](/docs/v1/tech/apis/login) to keep things simple.

First lets try requests as the `teller@example.com` user. Based on the configuration this user has the `teller` role and should be able to use both `/make-change` and `/panic`.
...
Make sure you requested the two required dependencies when you used the initializer.
