...
Update `nuxt.config.ts` to include the new `@sidebase/nuxt-auth` and include the provider type of `authjs` just like below. Please note if you would like to code with any of the pages prior to enabling authentication, just set `globalAppMiddleware` to false.
...
On first load of Nuxt this file will make sure that you have all of the correct environment variables. The variables are then exported in an object called `authOptions` which can be imported on the server when you need to get our session using `getServerSession`.
...
Create a new file named `/complete-application/assets/css/global.css` and copy the below CSS for the ChangeBank application.

<RemoteCode url={frontmatter.codeRoot + "/complete-application/assets/css/global.css"}
  lang="css" />

The above styles will be imported into a new layout that you will create called `/complete-application/layouts/default.vue`. Update this file to match the code below. This will allow the rest of the pages within our application and the layout itself to use the global styles. This is the first time that you will use the [useAuth](https://sidebase.io/nuxt-auth/v0.6/application-side/session-access-and-management) Composable from Sidebase. The status can then be used to determine what links should be shown in the layout.
...
Create a new file in `/complete-application/components/LoginButton.vue` that will be used for a button component.

For this button you can use the `signIn` and `signOut` functions from the `useAuth` composable. Copy the below code for the ChangeBank application into `/complete-application/components/LoginButton.vue`.
...
Create a new file in `/complete-application/components/LoginLink.vue` that will be used for a link component. For this link you can use the the same `signIn` function from the `useAuth` composable as you did in the button above.

Copy the below code for the ChangeBank application into `/complete-application/components/LoginLink.vue`.
...
Update the file `/complete-application/pages/index.vue` which will have the Homepage details. Not much here just an image and a Login Link.
...
Create a new file `/complete-application/pages/account.vue` which will have some fake Account details.
...
Here's the contents of `/complete-application/pages/account.vue`.
<RemoteCode url={frontmatter.codeRoot + "/complete-application/pages/account.vue"}
  lang="ts" />

### Make Change Page

Finally, we'll add some business logic for logged in users to make change with the following code in `/complete-application/pages/account.vue`:

<RemoteCode url={frontmatter.codeRoot + "/complete-application/pages/makechange.vue"}
  lang="ts" />

If the user session is not present the user is redirect back to the homepage at the base route. If the user is present then the `MakeChangeForm` is presented. Create a new file located at `/complete-application/components/MakeChangeForm.vue` with the below code. This component has all of the business logic needed for taking in a dollar amount of money and returning the correct amount of each coin.
...
This will take you through the `NextAuth.js` authentication flow. Automatically presenting the FusionAuth Login page because we have `fusionauth` set as the provider in our Login Button `signIn('fusionauth', { callbackUrl: '/account' })`. If you would like to see all providers you can set `fusionauth` to `undefined`.
