...
Finally, browse to <Breadcrumb label="Appearance -> Editor -> Templates -> Blank"/>. Here you are editing the page style template for all three of your new pages. Click on the paragraph (`This is the Post Content block`) twice to select it. Open the settings menu by clicking the <Uielement label="Settings"/> icon to the right of the blue <Uielement label="Save"/> button on the top right. On the <Breadcrumb label="Block"/> tab and under the <Breadcrumb label="Layout"/> section, disable <Uielement label="Inner blocks use content width"/>. Click <Uielement label="Save"/> and confirm. This changes each page to use the full browser width.
