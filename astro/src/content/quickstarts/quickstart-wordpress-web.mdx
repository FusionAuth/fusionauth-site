...
- A home page, that anyonymous users can see.
- A change page and account page, that only logged in users can see.

Under Settings - Permalinks, change Permalink structure to Post name and click Save Changes. This sets page URLs to be more human than the default numbered URLs.

<img src="/img/quickstarts/quickstart-wordpress-web/5.jpg" alt="Permalinks"/>

Now to add these pages, do the following:
- Click Pages.
...
- Click the WP icon on the top left to return to the pages screen. Then add two more blank pages. Set their titles to Account and Change, and publish them.

<img src="/img/quickstarts/quickstart-wordpress-web/6.jpg" alt="Add page"/>

Browse to Settings - Readings. Change Your homepage displays to A static page and under Homepage select Home.

<img src="/img/quickstarts/quickstart-wordpress-web/6.jpg" alt="Homepage"/>

Finally, browse to Appearance - Editor - Templates - Blank. Click on the paragraph (`This is the Post Content block`) twice. Under Layout on the right, disable Inner blocks use content width, and Save and confirm. This changes each page to use the full browser width.

<img src="/img/quickstarts/quickstart-wordpress-web/13.jpg" alt="Full width setting"/>

## Authentication

You now have a basic {frontmatter.technology} site three blank pages. Currently, logging in is useful only for this site, with its own private PostgreSQL server. So you're going to connect FusionAuth, so that you can log in to this site with the same credentials that you might want your users to use for another forum or site.
...
For a simpler user experience, you're now going to remove the option to log in with WordPress. This is an optional step.
...
For now, add the plugin WP Coder by Wow-Company.
...
In the list tab of the WP Coder plugin you can see the shortcodes of each page, like `[WP-Coder title="home"]`.
