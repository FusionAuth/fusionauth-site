...
Under <Breadcrumb>Settings -> Permalinks</Breadcrumb>, change <Field label="Permalink structure"/> to `Post name` and click <Uielement label="Save Changes"/>. This sets page URLs to be more human-readable than the default numbered URLs.
...
- Under <Breadcrumb>Bulk Actions</Breadcrumb>, click <Uielement label="Move to Trash"/>.
...
- Under <Breadcrumb>Template</Breadcrumb> on the far right, view the <Field label="Template"/> field and modify it. Do so by clicking <Uielement label="Pages"/>, and change it to <Uielement label="Blank"/>.
...
Finally, browse to <Breadcrumb>Appearance -> Editor -> Templates -> Blank</Breadcrumb>. Here you are editing the page style template for all three of your new pages. Click on the paragraph (`This is the Post Content block`) twice to select it. Open the settings menu by clicking the <Uielement label="Settings"/> icon to the right of the blue <Uielement label="Save"/> button on the top right. On the <Breadcrumb>Block</Breadcrumb> tab and under the <Breadcrumb>Layout</Breadcrumb> section, disable <Uielement label="Inner blocks use content width"/>. Click <Uielement label="Save"/> and confirm. This changes each page to use the full browser width.
...
On the {frontmatter.technology} dashboard, click <Breadcrumb>Plugins -> Add new</Breadcrumb>. In the search box on the right, paste "OpenID Connect Generic Client". On the first plugin in the results, by daggerhart, click <Uielement label="Install Now"/>, then <Uielement label="Activate"/>.
...
Back in <Breadcrumb>Pages</Breadcrumb>, for both the account and change pages, click <Uielement label="Edit"/>, enable <Uielement label="For Logged-In Users Only"/> at the bottom right, and click <Uielement label="Update"/>. Now if you log out and browse to either page, you won't be able to view it.
...
Browse to <Breadcrumb>Settings -> Dashboard Access</Breadcrumb>. Change the value of <Field label="Redirect URL"/> to `http://localhost:3000/account`. Click <Uielement label="Save Changes"/>.
...
Browse to <Breadcrumb>Media</Breadcrumb> in the sidebar. Click <Uielement label="Add New"/> and select `money.jpg` from the `complete-application` directory.
...
Browse to <Breadcrumb>Wow Plugins -> WP Coder</Breadcrumb>. Click <Uielement label="Add New"/>. In the title box at the top, enter `home`. In the <Field label="HTML code"/> box, copy the file contents from `complete-application/home.html`.
...
Click the <Breadcrumb>CSS Code</Breadcrumb> tab, and copy the file contents from `complete-application/changebank.css`.

<RemoteCode url={frontmatter.codeRoot + "/complete-application/changebank.css"}
            lang="css"/>

Click <Uielement label="Save Changes"/> and repeat this process for the account and change pages.

Browse to <Breadcrumb>Wow Plugins -> WP Coder</Breadcrumb>. Click <Uielement label="Add New"/>. In the title box at the top, enter `account`. In the <Field label="HTML code"/> box, copy the file contents from `complete-application/account.html`.
...
Browse to <Breadcrumb>Wow Plugins -> WP Coder</Breadcrumb>. Click <Uielement label="Add New"/>. In the title box at the top, enter `change`. In the <Field label="HTML code"/> box, copy the file contents from `complete-application/change.html`.
...
Click on the <Breadcrumb>JS Code</Breadcrumb> tab and add the contents of `change.js`.
...
Now you can use these shortcodes on the three pages. Browse to <Breadcrumb>Pages</Breadcrumb> and edit the account page. Click the hamburger menu at the top right and select <Uielement label="Code editor"/>. In the text box under the page title box, paste `[WP-Coder title="account"]` and click <Uielement label="Update"/>.
