...
- On the <Uielement label="Bulk Actions"/> dropdown, select <Uielement label="Move to Trash"/> and click <Uielement label="Apply"/>.

Now create new blank pages:

- Click <Uielement label="Add New"/>.
- Under <Field label="Add Title"/> enter `Home`.
- Under <Breadcrumb>Page</Breadcrumb> on the far right, view the <Field label="Template"/> field and modify it. Do so by clicking <Uielement label="Pages"/>, and select <Uielement label="Blank"/> on the dropdown.
...
Now if you browse to http://localhost:3000/wp-login.php, you can click <Uielement label="Login with OpenID Connect"/> and log in on FusionAuth with email `richard@example.com` and password `password`.
...
Browse to <Breadcrumb>WP Coder</Breadcrumb>. Click <Uielement label="Add New"/>. In the title box at the top, enter `home`. In the <Field label="HTML Code"/> box, copy and paste the following code.
...
Browse to <Breadcrumb>WP Coder</Breadcrumb>. Click <Uielement label="Add New"/>. In the title box at the top, enter `account`. In the <Field label="HTML Code"/> box, copy and paste the following code.
...
Browse to <Breadcrumb>WP Coder</Breadcrumb>. Click <Uielement label="Add New"/>. In the title box at the top, enter `change`. In the <Field label="HTML Code"/> box, copy and paste the following code.
...
Now you can use these shortcodes on the three pages. Browse to <Breadcrumb>Pages</Breadcrumb> and edit the account page. Click the hamburger menu at the top right and select <Uielement label="Code editor"/>. In the text box under the page title box, copy and paste the shortcode associated with account page which should look like `[wp_code id="2"]` and click <Uielement label="Update"/>.

Go back to the page list and edit the home page. Paste `[wp_code id="1"]` and click <Uielement label="Update"/>.

Edit the change page, paste `[wp_code id="3"]`, and click <Uielement label="Update"/>.
