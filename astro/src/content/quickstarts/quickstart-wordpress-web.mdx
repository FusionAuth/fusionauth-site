...
color: green
cta: EmailListCTA
url: https://github.com/ritza-co/fusionauth-quickstart-wordpress-web # you also need to change this value under `Clone The Code` because Astro does not render variables in .mdx files consistently
---
...
```
git clone https://github.com/ritza-co/fusionauth-quickstart-wordpress-web.git
...
- Set "Username" and "Password" to `u`.
...
- Click "Publish", confirm, and note the URL has updated to one you can browse to.
...
Go back to the page list and edit the Home page. Paste `[WP-Coder title="home"]` and click "Update".

Edit the Change page, paste `[WP-Coder title="change"]`, and click "Update".
...
* It still doesn't work, or a plugin no longer exists

Unfortunately WordPress is a quickly changing ecosystem that also requires plugins to be configured graphically. FusionAuth cannot create a completed Docker image of this tutorial because not everything can be set up in code. If you are sure this guide is out of date and can no longer be completed, please log a GitHub issue with FusionAuth to update it.