...
The last step is to implement logout. The `/logout` route in your application will redirect to the FusionAuth server. FusionAuth will then end the OAuth session for this user and redirect back to the `logoutURL` specified in your `kickstarter.json` file.

<RemoteCode url={frontmatter.coderoot + "/complete-application/src/index.ts"}
  lang="ts"
  tags="logout"/>

 The application's logout route is `/oauth2/logout`. Within this route we can safely clear all of the cookies related to this user using the below code.
