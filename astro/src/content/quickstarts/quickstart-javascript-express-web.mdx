...
- state: Created in our homepage route using some crazy math and stored in our cookie as `stateValue`
...
In the below code we check to see if the current user session has any information about the user, if not we redirect to the FusionAuth server to initiate the logout on the server. When this returns the user session cookie will now be empty and we can therefore forward back to our homepage, if we didn't add this check we would be stuck in an infinite loop between our Express server and FusionAuth (luckily unlike other providers you won't get billed per all :D)
