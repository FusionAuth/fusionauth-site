...
In the ChangeBank application there is a <Breadcrumb>Login</InlineUIElement> button which will open the `/login` route when clicked. This route will redirect them to a login page managed by FusionAuth. Include the below code to the bottom of `index.ts`.
