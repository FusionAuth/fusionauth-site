...
Then, click on the <InlineUIElement>green magnifying glass</InlineUIElement> on the list view next to your application. Scroll down to the <InlineUIElement>(.*)</InlineUIElement> section and copy the <InlineField>Client Id</InlineField> and <InlineField>Client Secret</InlineField> values off to a text file. You'll use them later.
