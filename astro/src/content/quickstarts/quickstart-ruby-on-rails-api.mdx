...
You need to update the `Gemfile` to include two new dependencies:
...
Create a new file to hold your environment variables, directly in the `complete-application` directory called `.env.development`. This new file has two variables that are used to call your FusionAuth instance `FUSIONAUTH_LOCATION` and `CLIENT_ID`, with the values that match below.
