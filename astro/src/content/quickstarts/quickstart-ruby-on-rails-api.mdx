...
This is for compatibility with the FusionAuth best practices and <a href="/docs/v1/tech/apis/hosted-backend" target="_blank">the hosted backend</a>.

</Aside>

Replace `<your_token>` with the copied token and pass as a cookie value. Make a request to `/make-change` with a query parameter `total=5.12`.
...
You will get a `401` Unauthorized response with the message `Proper role not found for user`.
