...
* Your client secret is <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.clientSecret" /></code>.
* Your example teller username is <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.tellerEmail" /></code> and the password is <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.tellerPassword" /></code>. They will have the role of `teller`.
* Your example customer username is <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.customerEmail" /></code> and the password is <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.customerPassword" /></code>. They will have the role of `customer`.
...
To make sure your local FusionAuth instance is accessible to your Laravel app, you need to [expose it to the Internet](/docs/v1/tech/developer-guide/exposing-instance). Write down the URL ngrok gave you as you'll need it soon.
...
```shell
curl "https://laravel.build/fusionauth-quickstart-php-laravel-api?with=mariadb" | bash
cd fusionauth-quickstart-php-laravel-api
```

### Initialize the Application

Initialize the Laravel application using the following:
...
Change `routes/api.php` to add both `/make-change` and `/panic` endpoints for your ChangeBank application.
...
To do so, add a new [User Provider](https://laravel.com/docs/10.x/authentication#adding-custom-user-providers) at `app/FusionAuth/Providers/FusionAuthEloquentUserProvider.php` to create new users when receiving valid JWTs from FusionAuth.
...
<RemoteCode url={frontmatter.coderoot + "/complete-application/config/auth.php"} tag="guards" lang="php"/>

Now, change the default `provider` to `fusionauth_eloquent`:

<RemoteCode url={frontmatter.coderoot + "/complete-application/config/auth.php"} tag="providers" lang="php"/>
...
Create a new file for the `/make-change` endpoint at `app/Http/Controllers/ChangeBank/MakeChangeController.php`. This controller will verify that the authenticated user has either the `teller` or `customer` role. It then takes in the URL parameter `total` to calculate which coins will be returned in the JSON payload.
...
To finish, create a controller at `app/Http/Controllers/ChangeBank/PanicController.php` to handle the `/panic` endpoint for users with the `teller` role.
...
First let's try the requests as the <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.userEmail" /></code> user. Based on the configuration this user has the `teller` role and should be able to use both `/make-change` and `/panic`.

1. Acquire an access token for <code><RemoteValue url={frontmatter.coderoot + "/kickstart/kickstart.json"} selector="$.variables.userEmail" /></code> by making the following request

<RemoteValue
    url={frontmatter.coderoot + "/kickstart/kickstart.json"}
    selector="$.variables"
    codeLang="shell"
    codeRenderer={(variables) => `
...
Make a request to `/make-change` with a query parameter `total=5.12` and passing the `token` in an `app.at` cookie.

```shell
curl --cookie 'app.at=YOUR_TOKEN' 'http://localhost:9001/make-change?total=5.12' \
```
...
```shell
curl --cookie 'app.at=YOUR_TOKEN' --request POST 'http://localhost:9001/panic'
```
...
```shell
curl --cookie 'app.at=YOUR_TOKEN' --location 'http://localhost:9001/make-change?total=3.24'
```
...
So far so good. Now call the `/panic` endpoint with `-i` flag to see the headers of the response).

```shell
curl --cookie 'app.at=YOUR_TOKEN' -i --request POST 'http://localhost:9001/panic'
```
