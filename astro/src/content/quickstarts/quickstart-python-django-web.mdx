...
Now create your Django app. Create the environment file with these contents.

`.env`
<RemoteCode url="https://raw.githubusercontent.com/RichardJECooke/fusionauth-quickstart-python-django-web/main/complete-application/.env"
 lang="shell" />

And then create a Django project.
...
The site will still error because you need to create a view. Make a `templates` directory.
...
OIDC_RP_CLIENT_SECRET = 'super-secret-secret-that-should-be-regenerated-for-production'
...
`urls.py`
<RemoteCode url="https://raw.githubusercontent.com/RichardJECooke/fusionauth-quickstart-python-django-web/main/complete-application/mysite/app/templates/app/authentication.html"
 lang="shell" />

Run all your migrations to make sure you have any necessary database tables set up.
...
You can now browse to [the Django app](http://localhost:8000/app). Log in using `richard@example.com` and you'll see the welcome message.
