...
* [Docker](https://www.docker.com), which is the quickest way to start FusionAuth. (There are [other ways](../../v1/tech/installation-guide/)).
...
All the files you'll create in this guide already exist in the `complete-application` folder if you want to copy them. All commands in this guide can be entered in a terminal in this folder — you don't need to change directory at any time.
...
By using `venv` you isolate all Python dependencies for this project locally, so as not to affect any dependencies used by other projects on your machine.

Now create a `requirements.txt` file to list the project dependencies.
...
From this point, you'll create files in the `app` folder, not in the main project.
...
* Copy the contents of the `static` folder from `complete-application/mysite/app/static` to your `/mysite/app/static` location.
* Copy the contents of `templates` folder from `complete-application/mysite/app/templates` to your `/mysite/app/templates` location.
