...
Here you are using a bootstrapping feature of FusionAuth, called [Kickstart](https://fusionauth.io/docs/v1/tech/installation-guide/kickstart).
...
Next, create a `requirements.txt` file to list the project dependencies. (All the files in this guide already exist in the `complete-application` folder if you want to copy them.)
...
You can [visit it](http://localhost:8000/) to ensure it is running, although it will not have content yet. If you need to stop it, hit `control-C` to do so. You can leave this running while you build your application so that you can see your progress.
...
Create an app within your `mysite` project and make a `templates` directory to hold the application's HTML views and `static` directory to hold the css files.
...
You are now going to work from the top down — setting up authentication URLs in the project's `settings.py`, then in the app's `urls.py`, then the `views.py`, and finally the HTML templates for the views.
 ...
 * The `admin` site is default Django admin.
...
You'll work from this point on in the `app`, not in the main project. First, create the `app/urls.py` file and set its content to match the project's URLs.
...
* The `account` view checks that the user is logged in and returns the account page template, passing in the user's email address returned from FusionAuth. If the user isn't logged in they are redirected to the login page. This also happens in the change page.
...
The final step is to create the HTML pages and style them with CSS. Copy the following folders:
