...
The `app` folder in the `mysite` project holds all your code. The `routes` folder is a special name in Remix, and contains your application's pages. Let's make a `services` folder to keep our authentication code separately.

```shell
mkdir app/services
```

In this folder add a file called `session.server.ts` and copy in the following code (we are following the example from the [Remix Auth README](https://github.com/sergiodxa/remix-auth)).
...
Next, create an authentication file, `auth.server.ts`, that uses the session provider you just created.
...
Now overwrite the `mysite/app/views.py` file. There are three pages: the home page, the account page, and the make change page.
