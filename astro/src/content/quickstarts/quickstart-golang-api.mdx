...
While the access token is acquired via the Login API above, this is for simplicity of illustration. The token can be, and typically is, acquired through one of the [OAuth grants](/docs/oauth/).
...
There are [several ways to acquire a token](/docs/oauth/) in FusionAuth, but for this example you will use the [Login API](/docs/apis/login) to keep things simple.
...
* Rather than call the Login API, you're probably going to want to use the [Authorization Code grant](/docs/oauth/#example-authorization-code-grant), which keeps all sensitive credentials within the bounds of FusionAuth. You can customize the [hosted login pages](/docs/themes/).
* You may want to generate a token using the [Client Credentials grant](/docs/oauth/#example-client-credentials-grant) if you are calling the API from another service.

### Security
* Customize the [token expiration times and policies](/docs/oauth/#configure-application-oauth-settings) in FusionAuth
* [Make sure you know how to securely consume a token](/articles/tokens/building-a-secure-jwt#consuming-a-jwt)
* Secure your API [using an API gateway](/docs/developer-guide/api-gateways/) rather than at the framework layer.
