...
Next, you'll need to configure the SDK with your FusionAuth URL and client ID. You can do this by adding the following to your `src/app/app.module.ts` file:
...
```shell
cp -r ../fusionauth-quickstart-javascript-angular-web/complete-application/src/assets src && \
cp -r ../fusionauth-quickstart-javascript-angular-web/complete-application/src/styles.css src
```
...
Next, you'll need to add the Auth Guard to the routing configuration. Open the `src/app/app.module.ts` file and add the RouterModule with the following routes:
...
Now that you have the Auth Guard and the routing configured, you can tie it all together. Open the `src/app/app.component.html` file and add the following:
...
Finally, open the `src/app/app.component.ts` file and add the following:
