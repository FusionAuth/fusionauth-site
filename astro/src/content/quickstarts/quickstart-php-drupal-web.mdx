...
- By navigating to http://localhost/admin/config/development/performance and clicking the <Uielement label="Clear all caches"/> button.
...
Enable the module by navigating to http://localhost/admin/modules, selecting the checkbox next to Twig Tweak, and clicking the <Uielement label="Install"/> button.
...
Log in by clicking the <Uielement label="Login with FusionAuth"/> button and using the username `richard@example.com` and password `password`.
