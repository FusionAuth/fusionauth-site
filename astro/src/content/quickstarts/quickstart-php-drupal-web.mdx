...
On <Breadcrumb>Step 1</Breadcrumb>, choose your language and click <Uielement label="Save and continue"/>.

On <Breadcrumb>Step 2</Breadcrumb>, choose `Standard` profile and click <Uielement label="Save and continue"/>.
...
While on the page that lists your newly created client, click the <Breadcrumb label="Settings"/> tab. Under the section <Breadcrumb label="OpenID buttons display in user login form"/>, change the value from `Hidden` to `Below` to display the "Login with FusionAuth" button on the login page.
...
In the browser, navigate to http://localhost/user/logout to log out of the Drupal application. Navigate to http://localhost/user/login and click on the <Uielement label="Login with FusionAuth"/> button.
