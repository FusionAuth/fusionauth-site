...
Next, update your `appsettings.json` file with FusionAuth Issuer and Client Id (Audience)  specific settings.
...
Update the `appsettings.Development.json` file and add the following code to it:
...
To configure ASP.NET authentication to use JWT tokens, update the `Program.cs` file. Replace the code in the `Program.cs` file with the following code:
...
We will need response objects for the API to return. One is the `PanicResponse` which returns a message when a successful `POST` call is made to `/panic`. Create a `Models` folder.
...
ASP.NET reads the `roles` field, adds those roles to the user, and will test the user for the roles via the `Authorize` attribute.
...
First let's try the requests as the `teller@example.com` user. Based on the kickstart configuration this user has the `teller` role and should be able to call both `/make-change` and `/panic`.
