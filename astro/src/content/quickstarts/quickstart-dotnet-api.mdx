...
ASP.NET reads the <InlineField>roles</InlineField> field, adds those roles to the user, and tests the user for the roles via the <InlineField>Authorize</InlineField> attribute.
...
First, try the requests as the <InlineFieldValue>teller@example.com</InlineFieldValue> user. Based on the kickstart configuration this user has the `teller` role and should be able to call both `/make-change` and `/panic`.

Acquire an access token for <InlineFieldValue>teller@example.com</InlineFieldValue> by making the following request in a new terminal window.
