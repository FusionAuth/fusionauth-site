...
Select <Breadcrumb>File -> Add Package Dependency</Breadcrumb> from the menu bar. In the search bar, paste in `https://github.com/openid/AppAuth-ios`. In the list of packages select "appauth-ios" and click <Uielement label="Add Package"/>. You will be asked to choose the packages to install. On the "Add to Target" column select `Changebank` for both "AppAuth" and "AppAuthCore". Click <Uielement label="Add Package"/> to add the packages to your project.
...
Select <Breadcrumb>File -> New -> File</Breadcrumb> from the menu bar. Select <Uielement label="Property List"/> in the <Breadcrumb>Resource</Breadcrumb> section as the template. Click <Uielement label="Next"/>. Set the name of the file to `ChangeBank`. Click <Uielement label="Create"/>. The file will be opened in the p-list editor.

Expand the "Root" item by clicking the arrow <Uielement label=">"/> button, then click the <Uielement label="+"/> button next to the "Root" item. Name the new item `authCredentials`, and select the <Field label="Type"/> as `Dictionary`. 

Now click the <Uielement label="+"/> button next to the "authCredentials" item. Name the new item `clientId`, and select the <Field label="Type"/> as `String`. In the "Value" column, enter `e9fdb985-9173-4e01-9d73-ac2d60d1dc8e`.
...
To read the settings from the p-list file, you need to add a new class to the project. Select <Breadcrumb>File -> New -> File</Breadcrumb> from the menu bar. Select <Uielement label="Swift File"/> as the template and click <Uielement label="Next"/>. Set the name of the file to `AppSettingsReader.swift`. Click <Uielement label="Create"/>.
...
Click <Uielement label="Login"/>. You should see a system notification, and then be redirected to FusionAuth in a popup browser. Log in with the username `richard@example.com` and the password `password`. You should see the home screen displaying your name, your balance, and a logout button.
...
Navigate back to the home screen by clicking the <Breadcrumb>Home</Breadcrumb> tab. Click <Uielement label="Logout"/>. You should see another system notification and then be redirected to FusionAuth to log out. Finally, you'll be returned to the login screen.
