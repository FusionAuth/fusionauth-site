...
color: blue
cta: EmailListCTA
coderoot: https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-go-api/main
---
import Aside from '../../components/Aside.astro';
import Diagram from '../../diagrams/quickstarts/resource-server.astro';
import RemoteCode from '../../components/RemoteCode.astro';

In this tutorial, you are going to learn how to integrate a Go resource server with FusionAuth. You will protect an API resource from unauthorized usage. You'll be building it for [ChangeBank](https://www.youtube.com/watch?v=CXDxNCzUspM), a global leader in converting dollars into coins.

The docker compose file and source code for a complete application are available at https://github.com/FusionAuth/fusionauth-quickstart-go-api.

## Prerequisites

- [Go](https://go.dev/): This quickstart was built using Go 1.21.0. This example may work on different versions of Go, but it has not been tested.
...
```
git clone https://github.com/FusionAuth/fusionauth-quickstart-go-api.git
cd fusionauth-quickstart-go-api
```
...
## Create Your Go Resource Server Application

Now you are going to create a Go API application. While this section builds a simple API, you can use the same configuration to integrate an existing API with FusionAuth.
...
Initialize the Go application using the following:
...
Now you are going to write some Go code. You need to create a file called 'main.go'. Here you will add the package clause and import declarations below:

<RemoteCode url={frontmatter.coderoot + "/complete-application/main.go"} tags="package-imports"
  lang="go"/>

You will also need to declare a variable that will hold the value of the public key for the application. The value for this key will be set later in the application. Declare this variable below the import declarations.
...
You will write two functions that will return values when users access them. These functions are called by the handlers. One is the `panic` which returns a message when a successful `POST` call is made to `/panic`. This will also display a message if the request is not made through a `POST`.

<RemoteCode url={frontmatter.coderoot + "/complete-application/main.go"} tags="panic-function"
  lang="go"/>

Next you need a function to show the breakdown from making changes. This function will get the `total` value from the query string. If the `total` cannot be cast to a `float64`, the function will return a message stating that. If the `total` is a valid number, the function will return the description of the change.
...
<Aside type="note">
In this example, if the key is already set, it will not call it again. In a production environment you could store this key on the server or set it once one initialization. If your api serves several applications, you will need to get the correct public key for each application if they are different.
</Aside>

<RemoteCode url={frontmatter.coderoot + "/complete-application/main.go"} tags="set-public-key"
  lang="go"/>

Then, you will add the code for comparing the required roles and the roles the user has according to the token.

<RemoteCode url={frontmatter.coderoot + "/complete-application/main.go"} tags="contains-role"
  lang="go"/>

Finally we will add a helper function that returns the name of the function being called by the endpoint.
...
In a command shell window, navigate to the root directory of your application.
...
```
We can make change using 20 quarters 1 dimes 0 nickels 2 pennies
```
...
```
We can make change using 12 quarters 2 dimes 0 nickels 4 pennies
```
...
```
Proper role not found for user
```
