...
These changes to the existing file:
...
The `app` folder in the `mysite` project holds all your code. In Remix, the designated `routes` folder contains your application pages. Let's make a `services` folder to keep your authentication code separate.
...
Now that authentication is done, the last task is to create two example pages that a user can only access when logged in.
