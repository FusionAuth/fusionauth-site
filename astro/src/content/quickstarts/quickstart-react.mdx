...
**Step 9:** Change the code in the top of the `Account()` function to the following code:
...
Similar to the homepage, `useFusionAuth()` adds the helper functions, and `startLogout()` adds authentication to the logout button.

**Step 10:** Add the user's info to the page with the following code:
...
**Step 11:** Reopen the app at http://localhost:3000. Click the login button and log in with these user credentials:
...
**Step 1:** Insert the following code into `account.tsx` just above the `return (` line:
...
The code above calls the FusionAuth endpoint at `/app/me` to get the latest user data, that might have changed since the user logged in.

**Step 2:** Overwrite the `userInfoGrid` section at the bottom of `account.tsx` with the following code:
...
The code above uses the latest user info from FusionAuth in the HTML, instead of the old user info at the time of login.

**Step 3:** Browse to the account page and click the refresh button. The latest user info will display.
