...
Similarly to the homepage code, `useFusionAuth()` adds the helper functions, and `startLogout()` adds authentication to the logout button.
...
The code above calls the FusionAuth endpoint at `/app/me` to get the latest user data, which may have changed since the user logged in.
...
**Step 3:** Browse to the account page and click the refresh button to see the latest user info.
