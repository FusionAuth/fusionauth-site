...
Create a new file named `src/app/api/auth/[...nextauth]/route.ts` and copy the following CSS for the ChangeBank application.
...
Create a new file named `src/app/globals.css` copy the below CSS for the ChangeBank application.
...
Create a new file named `src/components/login-button.tsx` copy the below code for the ChangeBank application.
...
<Aside type="tip">
If this is your first time using Next.js's App Router, you should read through <a href="https://nextjs.org/docs/app/building-your-application/routing" target="_blank">Routing Fundamentals</a>.
</Aside>
...
Create a new file named `src/app/layout.tsx` copy the below code for the ChangeBank application.
...
<Aside type="tip">
You could add additional [NextAuth providers](https://next-auth.js.org/providers/) within `complete-application/src/app/api/auth/[...nextauth]/route.ts`. However, another great part about having FusionAuth setup is that you can have a centralized customer identity and access management [CIAM](/articles/ciam/).
</Aside>
...
* You may also want to check the [next-auth.js FusionAuth Docs](https://next-auth.js.org/providers/fusionauth) for any of the FusionAuth Provider options that you might require.
