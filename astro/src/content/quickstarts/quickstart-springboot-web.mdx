...
mv /path/to/downloads/springweb.zip .
unzip springweb.zip
mv springweb/* .
mv springweb/.* .
rm springweb.zip
...
The commands above will unzip your downloaded starter package and copy the package files into your current working directory. Remember to replace `/path/to/downloads/` in the command above with the actual path to your downloads folder.
...
This controller serves the page for unauthenticated users on the `/` route and shows the application name, which is injected from the `spring.application.name` property in the `application.properties` file. The `home` return string tells Spring to render the `home.html` template which you will create later.
