...
The template is already downloaded in the example repository, but if you wish to download it yourself, go to the Initializr site at https://start.spring.io/ and download your own starter package. 
...
This example was made with Maven, Spring Boot version 2.7.12, and Java 17. If you use different options, the configuration and code may be different.
...
Now you'll create the model object referenced in the controllers. In the project base directory `src/main/java/io/quickstart/springweb`, create a new directory called `model` and add a new `Change.java` class to it.
...
From the base directory `src/main/java/io/quickstart/springweb`, add a new directory called `config` and add the following two Java classes to it.
...
Ensure the value for `spring.security.oauth2.client.registration.fusionauth-client.client-id` in the file `src/main/resources/application.properties` matches client Id configured in FusionAuth for the "Example App" Application at [http://localhost:9011/admin/application/](http://localhost:9011/admin/application/).
