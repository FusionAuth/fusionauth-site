...
tags: saml, oauth, sso, single sign-on, authentication, authorization, security, identity
...
SAML is primarily an authentication system, while OAuth is an authorization system. [*Authentication*](/articles/authentication/common-authentication-implementation-risks) is about confirming the user's identity. [*Authorization*](/docs/get-started/core-concepts/authentication-authorization) is about deciding what services they can access.
...
![Diagram showing flow between the user apps and identity provider when using multiple applications.](/img/articles/saml-vs-oauth/diagram-showing-flow-between-the-user-apps-and-identity-provider-when-using-multiple-applications.png "Diagram showing flow between the user apps and identity provider when using multiple applications")
...
There are two versions of OAuth, and there are [differences](https://www.blackduck.com/blog/oauth-2-0-vs-oauth-1-0.html) between them. This article focuses on version 2.0, which has been around since 2012. An updated version 2.1 [has been proposed](/articles/oauth/differences-between-oauth-2-oauth-2-1) but not yet approved.
...
Our [JWT Debugger](/dev-tools/jwt-decoder) tool shows you how the token and the information it contains relate to each other.
...
This information is a core source of revenue for companies like Facebook. While some users may not be keen on the [potential lack of privacy](https://www.wired.com/story/security-risks-of-logging-in-with-facebook/), those users are in the minority. However, these concerns are part of why some companies are choosing to run their own OAuth servers.
...
![Refresh token demo from OAuth Tools.](/img/articles/saml-vs-oauth/refresh-token-demo-from-oauth-tools.png "Refresh token demo from OAuth Tools.")
...
For example, if you're [developing a React app](/blog/how-to-authenticate-your-react-app), you can rely on OAuth to grant users access.
