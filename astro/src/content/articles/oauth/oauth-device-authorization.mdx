...
tags: oauth, authorization, device flow, tokens, security, iot, authentication, streaming
...
sequenceDiagram
    autonumber
    participant Roku
    participant Smartphone Browser as Smartphone Browser
    ...
    Note over Smartphone Browser, FusionAuth: User logs in via standard OAuth v2 Authorization Code Grant
    FusionAuth->>Smartphone Browser : 302 redirect to /oauth/device with activationComplete set to true
    Roku-->>FusionAuth : GET /oauth2/token (polling every 2-5 seconds)
...
- Check out the docs which explain the [OAuth2 Device Grant](/docs/apis/oauth) parameters in detail.
