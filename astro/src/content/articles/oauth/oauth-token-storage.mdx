...
| Option       | Strengths               | Weaknesses            | Use With | Works Well When...            | Recommended            |
|--------------|-------------------------|-----------------------|---------| -----------------------|-----------------------|
| `Secure`, `HTTPOnly` cookies | Secure since JavaScript can't read the token, tokens sent automatically when `credentials` option sent, widely supported | Won't work if clients are not on same domain as the APIs, requires a browser, APIs must look for access token in cookie header, not in other places| Web browser, anything that supports cookies | you control both the client and the API, everything is on the same domain | Yes |
| BFF/Server Side Session | Secure since client never sees token, easy to revoke tokens, works with any client | Additional server side component, less scalable, single point of failure for all API access | any client | you are integrating with APIs you don't control, you need to be able to closely monitor or throttle token usage | Yes, can be combined with cookie approach |
| Local Storage | JavaScript can access token and send requests using `Authorization` or other expected header, supported by some frameworks (Amplify) | Insecure, tokens vulnerable to exfiltration | Web browser | n/a |  No |
| In Memory    | Secure since your code can access tokens but other code cannot | Tokens lost when the client reloads | any client with memory protection | you are working in a SPA and okay forcing users to reauthenticate when they reload the browser | No |
| Web workers | No JavaScript can access tokens | All fetch calls must pass through web worker, have to find library or write web worker code, malicious code may still be able to get data via the web worker, access token lost when page is reloaded | Web browser | n/a | No |
| MTLS | Secure since tokens are bound to client, IETF standard | Not widely supported, requires every client to have an X.509 certificate | Web browser | you have an MTLS capable authorization server and client | No |
| DPoP | Secure since tokens are bound to client, IETF standard | Not widely supported, requires APIs to do extra work to verify the token, client must do extra work to send the token, still must take care to avoid exfiltration of key | any client | you have a DPoP capable API, authorization server and client | No |
