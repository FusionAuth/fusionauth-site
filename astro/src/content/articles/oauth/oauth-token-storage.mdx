...
tags: oauth, tokens, security, authorization, sessions, refresh tokens, jwt, authentication
...
<details>
    <summary>Summary - Storing OAuth Tokens</summary>
...
```mermaid title="The Authorization Code grant up to the point where tokens are requested from the token endpoint."
sequenceDiagram
...
```mermaid title="Storing the tokens as secure, HttpOnly cookies."
sequenceDiagram
    participant User as User/Browser
    participant App
    participant FusionAuth as Authorization Server
...
```mermaid title="Zooming in on token validation."
sequenceDiagram
    participant User as User/Browser
    participant API1 as api.example.com
    participant API2 as help.example.com
...
```mermaid title="Storing the tokens as secure, HttpOnly cookies and using introspection to validate them."
sequenceDiagram
  participant User as User/Browser
  participant App
  participant FusionAuth as Authorization Server
...
```mermaid title="Using a refresh token."
sequenceDiagram
  participant User as User/Browser
  participant App
  participant FusionAuth as Authorization Server
...
```mermaid title="Using a proxy to access APIs on different domains."
sequenceDiagram
  participant User as User/Browser
  participant App
  participant FusionAuth as Authorization Server
...
Localstorage is an insecure option because, unless you also set a fingerprint cookie, as [recommended by OWASP](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_Cheat_Sheet.html#token-sidejacking), you are exposed to XSS attacks. Any JavaScript running on the page has access to localstorage. If you do follow the OWASP recommendations by adding a fingerprint to your token and sending a cookie down with a related value, you are limited to API requests on the domain to which the cookie is scoped, which doesn't win you much.
...
```mermaid title="Storing the tokens server-side in a session."
sequenceDiagram
    participant User as User/Browser
    participant App
    participant FusionAuth as Authorization Server
...
```mermaid title="Proxying API calls using tokens stored in a server-side session."
sequenceDiagram
    participant User as User/Browser
    participant App
...
<Aside type="tip">
Learn comprehensive OAuth token security strategies. Watch our webinar [Safeguarding OAuth Tokens: Protecting Your Digital Identity](/webinar/safeguarding-oauth-tokens) to explore token storage options, validate security approaches, and implement defense-in-depth strategies for your authentication architecture.
</Aside>
