...
When using a browser, store these as `HttpOnly`, secure cookies with a <InlineField>SameSite</InlineField> value of <InlineFieldValue>Lax</InlineFieldValue> or <InlineFieldValue>Strict</InlineFieldValue>.
...
When you initially request the <InlineFieldValue>offline_access</InlineFieldValue> scope, you will receive a refresh token as well as an access token after a user authenticates.
