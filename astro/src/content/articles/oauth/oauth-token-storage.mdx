...
| Option                | Strengths                                                                 | Weaknesses                                                                                              | Use With                             | Works Well When...                                               | Recommended                           | Security Considerations                                                                                     |
|-----------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|--------------------------------------|-----------------------------------------------------------------|---------------------------------------|-----------------------------------------------------------------------------------------------------------|
| `Secure`, `HTTPOnly` cookies | Tokens sent automatically when `credentials` option sent, widely supported | Won't work if clients are not on same domain as the APIs, requires a browser, APIs must look for access token in cookie header, not in other places | Web browser, anything that supports cookies | you control both the client and the API, everything is on the same domain | Yes | Reduces attack surface by preventing JavaScript from accessing tokens, but requires correct cookie configuration and same-domain policies. |
| BFF/Server Side Session | Easy to revoke tokens, works with any client                              | Additional server side component, less scalable, single point of failure for all API access              | any client                           | you are integrating with APIs you don't control, you need to be able to closely monitor or throttle token usage | Yes, can be combined with cookie approach | Minimizes token exposure by never sharing tokens with the client, but introduces a new critical infrastructure component that must be secured.                     |
| Local Storage         | JavaScript can access token and send requests using `Authorization` or other expected header, supported by some frameworks (Amplify) | Insecure, tokens vulnerable to exfiltration                                                             | Web browser                          | n/a                                                             | No                                    | Highly vulnerable to XSS attacks, as tokens stored in local storage can be accessed and exfiltrated by malicious scripts.                      |
| In Memory             | Your code can access tokens but other code cannot                                               | Tokens lost when the client reloads                                                                     | any client with memory protection    | you are working in a SPA and okay forcing users to reauthenticate when they reload the browser | No | Reduces persistence of tokens, limiting exposure|
| Web workers           | Isolation of web worker prevents token access | All fetch calls must pass through web worker, have to find library or write web worker code, malicious code may still be able to get data via the web worker, access token lost when page is reloaded | Web browser                          | n/a                                                             | No                                    | Provides isolation for tokens, but risks persist with malicious code targeting the web worker or intercepting network traffic.                |
| MTLS                  | Tokens are bound to client, IETF standard                                                      | Not widely supported, requires every client to have an X.509 certificate                                | Web browser                          | you have an MTLS capable authorization server and client        | No                                    | Offers strong token-binding security, but operational complexity and certificate management introduce significant overhead.                    |
| DPoP                  | Tokens are bound to client, IETF standard                                                      | Not widely supported, requires APIs to do extra work to verify the token, client must do extra work to send the token, still must take care to avoid exfiltration of key | any client                           | you have a DPoP capable API, authorization server and client    | No                                    | Enhances token security by binding it to a client, but susceptible to implementation flaws and requires stringent key management.    |
