...
In a hurry? Here's a table of options for storing tokens, including the strengths and weaknesses of each choice.

| Option                | Strengths                                                                 | Weaknesses                                                                                              | Use With                             | Use When...                                               | Security Considerations                                                                                     | Recommended                           |
|-----------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|--------------------------------------|-----------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|---------------------------------------|
| `Secure`, `HTTPOnly` cookies | Tokens sent automatically when `credentials` option sent, widely supported | Won't work if clients are not on same domain as the APIs, requires a browser, APIs must look for access token in cookie header, not in other places | Web browser, anything that supports cookies | you control both the client and the API, everything is on the same domain | Reduces attack surface by preventing JavaScript from accessing tokens, but requires correct cookie configuration and same-domain policies. | Yes                                    |
| Backend for frontend (BFF)/Server Side Session | Easy to revoke tokens, works with any client                              | Additional server side component, less scalable, single point of failure for all API access              | any client                           | you are integrating with APIs you don't control, you need to be able to closely monitor or throttle token usage | Minimizes token exposure by never sharing tokens with the client, but introduces a new critical infrastructure component that must be secured | Yes, can be combined with cookie approach to provide cross domain access if some APIs live on different domains |
| Native secure storage   | OS-provided secure storage options like iOS Keychain or Android Keystore prevent token exfiltration       | Platform-specific implementation | Native mobile applications           | you are building a native application | Prevents exfiltration by malicious apps, use TLS to prevent MITM attacks                    | Yes                                    |
| Local Storage, IndexedDB         | JavaScript can access token and send requests using `Authorization` or other expected header, supported by some frameworks (Amplify) | Tokens vulnerable to exfiltration                                                             | Web browser                          | you are using a framework that requires JavaScript can access the token and you can't use the BFF pattern                                                             | Highly vulnerable to XSS attacks, as tokens stored in local storage can be accessed and exfiltrated by malicious scripts                      | No                                    |
| In Memory             | Your code can access tokens but other code cannot                                               | Tokens lost when the client reloads                                                                     | any client, but typically a web browser | you are working in a SPA and okay forcing users to reauthenticate when they reload the browser | Reduces persistence of tokens, limiting exposure | No                                    |
| Web workers           | Isolation of web worker prevents token access | All fetch calls must pass through web worker, have to find library or write web worker code, malicious code may still be able to get data via the web worker, access token lost when page is reloaded | Web browser                          | n/a                                                             | Provides isolation for tokens, but risks persist with malicious code targeting the web worker or intercepting network traffic                | No                                    |
| MTLS                  | Tokens are bound to client, IETF standard                                                      | Not widely supported, requires every client to have an X.509 certificate                                | Web browser                          | you have an MTLS capable authorization server and client        | Offers strong token-binding security, but operational complexity and certificate management introduce significant overhead                    | No                                    |
| DPoP                  | Tokens are bound to client, IETF standard                                                      | Not widely supported, requires APIs to do extra work to verify the token, client must do extra work to send the token, still must take care to avoid exfiltration of key | any client                           | you have a DPoP capable API, authorization server and client    | Enhances token security by binding it to a client, but susceptible to implementation flaws and requires stringent key management              | No                                    |
...
Client binding measures, such as [Distributed Proof of Possession (DPoP)](https://datatracker.ietf.org/doc/html/rfc9449) and [MTLS](https://datatracker.ietf.org/doc/html/rfc8705), remove the danger of XSS. A token can't be used without the private key only the proper client possesses. However, these approaches require additional setup on the client side. DPoP is relatively new and requires the API server to take additional verification steps.
