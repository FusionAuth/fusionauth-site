...
| Option                | Strengths                                                                 | Weaknesses                                                                                              | Use With                             | Works Well When...                                               | Security Considerations                                                                                     | Recommended                           |
|-----------------------|---------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|--------------------------------------|-----------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|---------------------------------------|
...
| BFF/Server Side Session | Easy to revoke tokens, works with any client                              | Additional server side component, less scalable, single point of failure for all API access              | any client                           | you are integrating with APIs you don't control, you need to be able to closely monitor or throttle token usage | Minimizes token exposure by never sharing tokens with the client, but introduces a new critical infrastructure component that must be secured. | Yes, can be combined with cookie approach |
| Local Storage         | JavaScript can access token and send requests using `Authorization` or other expected header, supported by some frameworks (Amplify) | Insecure, tokens vulnerable to exfiltration                                                             | Web browser                          | n/a                                                             | Highly vulnerable to XSS attacks, as tokens stored in local storage can be accessed and exfiltrated by malicious scripts.                      | No                                    |
| In Memory             | Your code can access tokens but other code cannot                                               | Tokens lost when the client reloads                                                                     | any client with memory protection    | you are working in a SPA and okay forcing users to reauthenticate when they reload the browser | Reduces persistence of tokens, limiting exposure | No                                    |
