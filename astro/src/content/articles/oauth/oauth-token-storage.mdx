...
| Option       | Strengths               | Weaknesses            | Works well when...            |Recommended            |
|--------------|-------------------------|-----------------------|-----------------------|-----------------------|
| `Secure`, `HTTPOnly` Cookies | Secure since JavaScript can't read the token, tokens sent automatically when `credentials` option sent | Won't work if clients are not on same domain as the APIs, requires a browser, APIs must look for access token in cookie header, not in other places| you control both the client and the API | Yes |
| BFF/Server Side Session | Secure since client never sees token, easy to revoke tokens, works with any client | Additional server side component, less scalable, single point of failure for all API access | you are integrating with APIs you don't control, you need to be able to closely monitor or throttle  token usage | Yes |
| Local Storage | JavaScript can access token and send requests using `Authorization` or other expected header, supported by some frameworks (Amplify) | Insecure, tokens vulnerable to exfiltration | n/a |  No |
| In Memory    | Secure since your JavaScript can access tokens but other JavaScript cannot | tokens lost when the client reloads from memory | you are working in a SPA and okay forcing users to reauthenticate when they reload the browser | No |
