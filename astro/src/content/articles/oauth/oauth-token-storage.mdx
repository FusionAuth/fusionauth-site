...
| Native secure storage   | OS-provided secure storage options like iOS Keychain or Android Keystore prevent token exfiltration       | Platform-specific implementation | Prevents exfiltration by malicious apps, use TLS to prevent MITM attacks                    | <GreenCheck />                                    |
...
| Web workers           | Isolation of web worker prevents token access | All fetch calls must pass through web worker, have to find library or write web worker code, malicious code may still be able to get data via the web worker, access token lost when page is reloaded | Provides isolation for tokens, but risks persist with malicious code targeting the web worker or intercepting network traffic                | <RedX /> |
| MTLS                  | Tokens are bound to client, IETF standard                                                      | Not widely supported, requires every client to have an X.509 certificate                                | Offers strong token-binding security, but operational complexity and certificate management introduce significant overhead                    | <RedX /> |
| DPoP                  | Tokens are bound to client, IETF standard                                                      | Not widely supported, requires APIs to do extra work to verify the token, client must do extra work to send the token, still must take care to avoid exfiltration of key | Enhances token security by binding it to a client, but susceptible to implementation flaws and requires stringent key management              | <RedX /> |
