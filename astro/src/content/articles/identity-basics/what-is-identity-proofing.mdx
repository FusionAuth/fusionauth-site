...
tags: identity proofing, identity verification, authentication, security, identity basics, compliance, user management, fraud
author: Deborah Ruck
...
There are two types of KBA: static and dynamic. Static KBA asks the user questions from a set repository of questions in a company's database. Both questions and answers are stored in the database and presented to the user when identity needs to be confirmed, such as for a  password reset. Static KBA is part of [multi-factor authentication or MFA](https://en.wikipedia.org/wiki/Multi-factor_authentication) and satisfies the “something you know” factor, but the answers to these questions can be easily discovered through social engineering. 
...
<Aside type="tip">
Watch our webinar on identity verification, liveness checks, and when friction is a feature. [Liveness, Deepfakes, and the Root of Trust: Why Identity Verification Starts with a Live Human](/webinar/liveness-deepfakes-and-the-root-of-trust-why-identity-verification-starts-with-a-live-human) covers these topics and more with Cameron D'Ambrosi, an industry expert.
</Aside>
