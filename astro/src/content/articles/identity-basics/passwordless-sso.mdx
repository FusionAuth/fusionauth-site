...
description: Discover the benefits of passwordless SSO. Enhance security, streamline user experience, and simplify IT management with biometrics and tokens.
section: Identity Basics
authors: Brad McCarty
icon: /img/icons/sso-explained.png 
darkIcon: /img/icons/sso-explained-drk.png
---

**Passwordless [Single Sign-On](/articles/authentication/how-sso-works)** (SSO) offers a safe and simple way to access multiple applications without using passwords. 
...
Weak passwords are a critical vulnerability in cybersecurity. According to [Verizon](https://www.verizon.com/business/resources/reports/dbir/), they may be responsible for up to 80% of data breaches. This makes traditional password systems a risky choice for enterprises. 

[Passwordless](/features/passwordless) authentication removes the need for passwords entirely, using [biometric scans](/glossary/biometrics), tokens, or other secure methods. Imagine logging in with just your fingerprint or a hardware token instead of remembering multiple passwords. Sounds convenient, right? It also boosts security and improves user experience by making logins faster and simpler.

![Infographic on Passwordless Authentication Benefits - passwordless single sign on infographic comparison-2-items-formal](https://images.bannerbear.com/direct/4mGpW3zwpg0ZK0AxQw/requests/000/060/899/642/nE38ekNX9QnBkK8xQMamprWxZ/f96071091bec85f51f552664edaf10f1abe8f1cb.jpg)

## What is Passwordless Single Sign-On (SSO)?
### How Does Passwordless SSO Work?

Passwordless Single Sign-On (SSO) is a way for users to access multiple applications without entering a password each time. Instead, they use more secure methods like biometric scans or security tokens. Let's break down how it works.
...
- **[SAML](/docs/lifecycle/authenticate-users/identity-providers/overview-samlv2) (Security Assertion Markup Language):** Often used for web-based applications.
- **[LDAP](/docs/lifecycle/migrate-users/connectors/ldap-connector) (Lightweight Directory Access Protocol):** Commonly used for on-premises systems.
- **[OAuth](/articles/oauth/modern-guide-to-oauth):** Frequently used for cloud-based applications.
- **[WebAuthn](/articles/authentication/webauthn-explained):** A newer standard for passwordless authentication using biometric data or security keys.
...
![Passwordless SSO Workflow - passwordless single sign on](/img/docs/lifecycle/authenticate-users/passwordless/reauth-enable.png)
...
Passwordless Single Sign-On (SSO) improves security by eliminating passwords, which are a common target for cyberattacks. According to Verizon's annual DBIR report, stolen credentials account for nearly 50% of enterprise attacks. By removing passwords from the equation, passwordless SSO mitigates risks like phishing, credential-stuffing, and malware attacks.

Unified multi-factor authentication (MFA) is another critical benefit, although not limited specifically to SSO. Passwordless SSO leverages advanced cryptographic protocols to provide an extra layer of security. This makes it much harder for attackers to gain unauthorized access, even if they manage to bypass one security measure. For example, biometric scans or security tokens can be used to verify user identity, reducing the risk of phishing attacks dramatically.
...
- **FIDO2**: A standard that uses public key cryptography, but may not be supported.
- **Security Keys**: Physical devices that provide secure access.
- **Biometric Technologies**: Use features like facial [recognition or fingerprint scans](https://fusionauth.io/articles/authentication/how-sso-works).
...
- **OTP Interception:** One-Time Passwords (OTPs) sent via SMS or email can [be intercepted by attackers](https://fusionauth.io/articles/authentication/passwordless-authentication-security).
...
By choosing FusionAuth, you can improve your security posture, reduce IT support costs, and improve user satisfaction. Ready to make the switch? Learn more about our [authentication solutions](https://fusionauth.io/features/authentication) and take the first step towards a passwordless future.

The road to passwordless is paved with SSO. Start your journey today with FusionAuth!
