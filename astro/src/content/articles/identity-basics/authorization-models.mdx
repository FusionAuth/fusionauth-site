...
[Attribute-Based Access Control (ABAC)](https://www.youtube.com/watch?v=cgTa7YnGfHA) establishes policies based on a combination of attributes that define what actions can be performed. It controls access based on details of a request and contextual factors, asking who can have access to what and under what conditions. Understanding ABAC's use of attributes is crucial for distinguishing it from Policy-Based Access Control (PBAC), which we'll discuss next. Remember that ABAC relies on a **predefined list of attributes**. There are four categories of attributes used to make decisions:
