...
In simplest terms, a magic link is a URL with an embedded unique token. They are sometimes included in a [multi-factor authentication (MFA)](/feature/intelligent-mfa) approach. Clicking on the link authenticates the user, allowing them to access a particular service or application. It is similar in function to a [one-time password (OTP)](/articles/authentication/developer-benefits-single-sign-on) setup but with less risk.
