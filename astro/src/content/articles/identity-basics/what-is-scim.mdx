...
`Groups`, on the other hand, are much simpler. They have a required `displayName` and a list of members, as well as an <InlineField>id</InlineField>.
...
The first is the <InlineField>id</InlineField>, which is a required attribute and is a globally unique identifier managed by the SCIM server. It must be stable and always refer to the same resource.
