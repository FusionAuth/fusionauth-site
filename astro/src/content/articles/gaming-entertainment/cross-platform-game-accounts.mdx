...
tags: gaming, entertainment, authentication, user management, sso, identity management, game accounts, oauth
...
To create a tenant, open the <Breadcrumb>Tenants</Breadcrumb> tab on the side pane and click the plus button in the top right corner. Give the tenant a unique name, in this case `GameGuruTenant`, leaving the rest of the options as they are and saving the tenant.
...
You can leave the default settings in place. A UUID is auto-generated for you. Feel free to click through tabs like <Breadcrumb>OAuth</Breadcrumb>, <Breadcrumb>CleanSpeak</Breadcrumb>, and <Breadcrumb>Email</Breadcrumb> to customize anything you think you'll need.

You also need to add some roles for this apps use case:

* `super-user`, with the <Breadcrumb>Super Role</Breadcrumb> checkbox ticked, and is used for admin users.
...
2. [Create a user](/docs/apis/users/create) and then [register the user](/docs/apis/registrations/create-a-user-registration-for-an-existing-user)
