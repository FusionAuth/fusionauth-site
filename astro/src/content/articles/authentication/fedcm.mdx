...
There are several files that the authentication provider needs to serve. Let's assume the authentication provider is hosted at [auth.example.com](auth.example.com). Here are the files that need to be available for FedCM to work:
...
"The existence of a file at the root of the IDP's domain is enforced to ensure that the file name does not introduce fingerprints about the RP being visited." –  [https://w3c-fedid.github.io/FedCM/#manifest-fingerprinting](https://w3c-fedid.github.io/FedCM/#manifest-fingerprinting) 
