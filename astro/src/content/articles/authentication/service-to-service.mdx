...
- [Set Up A FusionAuth Machine-To-Machine OAuth Example](#set-up-a-fusionauth-machine-to-machine-oauth-example)
  ...
  - [Create Server Code To Authenticate The Access Token](#create-server-code-to-authenticate-the-access-token)
...
This code uses the Axios library to make HTTP calls simpler. It has one function, `getAccessToken()`, that calls the FA `/oauth2/token` endpoint to get an access token. Note the form parameters given in this function. Here you are saving the secret key directly in the code, which is not safe in reality. In a real application, you should save keys in a `.env` file that you don't commit to Git.

Finally, note the `scope` parameter. If you excluded this line, you have an access token that provided authentication only, with no permissions to (authorization) to do anything. FA requires all scopes to be requested in the format shown — with `target-entity` and the Id of the API yu want access to.
...
- Run the command below in a new terminal to make the JavaScript above get a token a display it. Here you run Node.js in a Docker container to isolate it from the rest of your machine for safety's sake.
  ```sh
...
- Create a file called `api.mjs`. This will access as the server that receives the file upload request and the access token from the customer. The API needs to authenticate the access token is really from FusionAuth, rather than being sent by an attacker. If the token is legitimately from FusionAuth, and contains the permissions to add a file, the operation will succeed.
...
The server has two functions. The first, exported function that the client calls, takes a token and check if it is valid. If so, the server prints that the file was saved successfully. A valid token has three properties:
- It is correctly signed by FA.
- It has not expired yet.
- It has the `Create` permission for the `FileAPI` Id.

You can see the code above checks all three. Note that the function does not need to check the customer's Id, because this is not a situation where one customer has access to another customer's files.

The other function in the code, `verifyToken()`, uses two JWT helper libraries. It gets the public key of FA from the localhost URL, and uses it to check that the access token was signed correctly. It also checks that the current time is less than the expiry date of the access token.
