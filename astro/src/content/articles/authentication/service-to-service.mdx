...
- [Set Up A FusionAuth Machine-To-Machine OAuth Example](#set-up-a-fusionauth-machine-to-machine-oauth-example)
  ...
  - [Create Customer Code](#create-customer-code)
...
You are now ready to get access tokens from FusionAuth as a Customer, and verify tokens as the API.



- create customer.js
  - create js to call api and get access token
  - see token on jwt.io and fusionauth equivalent website, check if can verify token against local host
- create server.js
  - use express to receive a single file storage call
  - create js to verify token in api
