...
Let's start by summarizing how OAuth normally works for user login, which you're more familiar with than machine OAuth. You've most likely used FusionAuth for local login or third-party login. With local login, FusionAuth stores usernames and password hashes. With third-party login, another organization, like Google, handles authentication and FusionAuth is merely an intermediary. Either way, the authentication flow works as follows:
...
Finally, note the `scope` parameter. If you excluded this line, you have an access token that provided authentication only, with no permissions to (authorization) to do anything. FusionAuth requires all scopes to be requested in the format shown — with `target-entity` and the Id of the API yu want access to.
