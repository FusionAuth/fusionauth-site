...
import SystemDesign from "src/diagrams/articles/authentication/service-to-service/system-design.astro";
...
Let's review how OAuth typically works for user login, as it's more commonly encountered.
...
- Your site then starts a separate HTTP call to the server's token endpoint directly, a more secure approach than using the browser, where the access token could be exposed. In this request, your site provides the temporary authorization code, your client Id, and your client secret to obtain a key (access token), which allows access to the user's resources in future calls. The server may return a refresh token with a long lifespan, which can be used to request short-lived access tokens. This mechanism enhances security and limits the potential damage if an access token is compromised.
...
- In the <InlineUIElement>JWT</InlineUIElement> tab, enable <InlineUIElement>Enabled</InlineUIElement> and set the <InlineUIElement>Access token signing key</InlineUIElement> to `RS256`.
...
Now add a new `API` Entity Type. You don't need to change the JWT signing algorithm here because the API doesn't create access tokens, it only verifies tokens from clients. Give the Entity Type `Create`, `Read`, `Update`, and `StoreLargeData` permissions in the <InlineUIElement>Permissions</InlineUIElement> tab and save.

![Entity types in FusionAuth](/img/articles/authentication/service-to-service/types.png)
...
Why use symmetric keys if they require more effort than asymmetric ones? Because the algorithm is much faster. If your setup involves a single primary system communicating with many clients, a symmetric key gives you a significant performance improvement. For example, the system you're building in this guide will efficiently use a symmetric key. But if you have many services that need to verify JWTs, creating and sharing symmetric keys with all of them is impractical.
</Aside>
...
![FileApi entity in FusionAuth](/img/articles/authentication/service-to-service/fileapi.png)
...
Paste the returned token from your terminal into the text box on the [JWT Decoder](/dev-tools/jwt-decoder) tool page. Since an access token is plaintext encoded as Base64 bytes, no key is needed to display the original contents.
  ...
  ![Decoded JWT](/img/articles/authentication/service-to-service/jwt.png)
...
The server has two functions. The first exported function, called by the client, takes a token and checks its validity. If the token is valid, the server prints that the file was saved successfully. A valid token has three properties:
...
Manually verifying a token by looking at JWT signatures, as the code above does, is not standard in OAuth. Instead, OAuth servers should allow anyone to verify a token by passing the token to the standard `introspect` endpoint, as in the example code below.
...
Currently, FusionAuth requires the customer's secret key to be included along with the token for verification through the `introspect` endpoint, but the verifying server typically does not have access to the customer's secret key.
...
Now that you know how to manually update permissions using a script, let's explore an alternative approach: checking whether a customer has the `premium` attribute, and setting the relevant permissions directly in FusionAuth for each access request.
