...
![Diagram showing one public API connecting three backend services to two frontends (mobile and web)](../../../../public/img/articles/authentication/bff/bff1.png)
...
![Diagram showing the BFF pattern with separate mobile and web APIs](../../../../public/img/articles/authentication/bff/bff2.png)
...
The diagram below shows the simplest client–server design used by most small web apps (without OAuth).

![Diagram of a simple client-server design](../../../../public/img/articles/authentication/bff/bff1.svg)
...
The use of a cookie as the authentication credential in the browser is crucial for security, as opposed to storing the credential in the browser's local or session storage. A cookie can be set to `HttpOnly` and `SameSite=Strict` to prevent JavaScript access and ensure the cookie is only sent to the server that created it.
...
![Illustration showing how malicious JavaScript can access tokens stored in local storage but not `HttpOnly` cookies](../../../../public/img/articles/authentication/bff/bff2.svg)
...
![Diagram of the OAuth authorization code flow](../../../../public/img/articles/authentication/bff/bff3.svg)

In this design, an OAuth server is added to the browser and server components from the previous example. 

Here's how the OAuth authorization code flow handles user login:

- The user clicks **Log in** in the web app.
- The web server receives the request and redirects the user to the OAuth server's website, where they enter their credentials.
- The OAuth server redirects the browser to the web service with an authorization code in the URL.
- The web service receives the code and sends it to the OAuth server to get refresh and access tokens. (For better security, this exchange does not involve the browser.) The web service stores the tokens, then returns a cookie to the user, marking them as logged in. (Read [this guide](https://fusionauth.io/blog/what-happens-after-oauth-authentication-event) to understand what each token is used for.)

Since the OAuth tokens are never sent to the browser, they cannot be stolen by malicious JavaScript on the client side. While the initial authorization code from the OAuth server is returned to the browser and is therefore vulnerable to client-side attacks, only the web service can exchange it for access tokens, as the OAuth server will reject requests from other URLs.

If you use an authentication gateway like FusionAuth or Keycloak for a web app, this is probably the flow currently in use.

In this design, OAuth is used to authenticate the user on the app's web service, not with an external resource server on the client's behalf. 
...
![Diagram showing the web service including an access token in a request to a resource server](../../../../public/img/articles/authentication/bff/bff4.svg)
...
![Illustration of a serverless app using the Dropbox file storage API](../../../../public/img/articles/authentication/bff/bff5.svg)
...
The PKCE (proof key for code exchange) flow is similar to the authorization code flow, with two differences:

- When the initial request for an authorization code is sent, the browser includes a secret, which the OAuth server stores.
- When the request for an access token is sent, the browser includes not only the authorization code, but also the original secret. If the secret the OAuth server stored does not match the one from the browser, the server will not return an access token. The access and refresh tokens are obtained directly from the app in the browser and are stored there.

Let's consider possible attacks on this protocol:

- If an attacker can read the browser's address bar, they can see the authorization code returned from the server. However, without the challenge secret, they can't obtain access tokens.
...
What an attacker can steal depends on how the app stores the tokens and secrets. If the app keeps this sensitive data only as variables in RAM, malicious JavaScript probably won't be able to find it, unless it knows exactly where to look. But if tokens are kept only in RAM, the user will have to log in every single time they open the app. If the secrets are persisted in the browser's session or local storage, the attacker will steal everything. Note that `HttpOnly` cookies can't be used in this situation, because the app's JavaScript has to be able to read and write the token values.
...
Ignoring the file host (since it's inconsequential), a minimal backend for secure token exchange looks like the diagram below. Notice how similar this diagram is to the earlier OAuth example, with Dropbox acting as both the OAuth server and resource server.

![Diagram showing a minimal backend for secure token exchange with Dropbox acting as both the OAuth server and resource server](../../../../public/img/articles/authentication/bff/bff6.svg)

Of course, your OAuth server and your resource server might be separate servers (for example, you might use Google Accounts for OAuth to request authorization for Gemini as the AI resource). Then the design will look as follows.

![Diagram showing separate Oauth server and resource server](../../../../public/img/articles/authentication/bff/bff7.svg)
...
If your business has its own full-featured server for business logic, or a public API, you may still want to use the BFF pattern, keeping separate servers purely for authentication, so that mobile, web, and desktop apps can each have a dedicated BFF. This design is shown in the diagram below and echoes the Soundcloud design discussed in the introduction.

![Diagram showing mobile, web, and desktop apps, each with a dedicated BFF, and a separate server for authentication](../../../../public/img/articles/authentication/bff/bff8.svg)
...
If you haven't already, clone or download the repository from https://github.com/ritza-co/fusionauth-example-hostedbackend. This is the same example used in the previous section. As before, you will need Docker installed to run the example.
...
The logging link merely redirects the user to the FusionAuth hosted backend API endpoint.
...
In summary, you can add secure authentication to your serverless app by simply running FusionAuth and writing a snippet of JavaScript. This is less work than implementing the PKCE flow, and it's a more secure approach. If you are writing a mobile or desktop app, you can also use this secure technology, as long as your HTTP library supports cookies.
...
- **Add a server for any app that uses OAuth**, even mobile and desktop apps if possible, so tokens are never sent to the browser. If your app is currently serverless, add an OAuth proxy to make it safer.
...
- **Consider [multifactor authentication](https://fusionauth.io/docs/lifecycle/authenticate-users/multi-factor-authentication)** for dangerous actions like buying, selling, or accessing private information.
