...
- They recommend PKCE, which you've seen still allows theft of refresh tokens.
- They recommend frequently changing refresh tokens, which doesn't prevent an attacker from waiting until a user closes the website and then using the latest refresh token to impersonate the user for hours or days.
- They recommend hiding credentials in service workers in the browser, as service workers are less vulnerable to XSS attacks, but service workers are started by JavaScript and so can be stopped by MJ.
