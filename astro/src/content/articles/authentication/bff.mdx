...
{/* Tatenda, you can remove this if Dan explains how to fix: https://github.com/FusionAuth/fusionauth-issues/issues/3054 */}

Browse to the Applications tab of FusionAuth at http://localhost:9011/admin/application. For the `Example App`, click <Breadcrumb>Select -> Edit</Breadcrumb>. On the <InlineUIElement>Scopes tab</InlineUIElement>, for <InlineUIElement>Scope handling policy</InlineUIElement>, change to <InlineUIElement>Compatibility</InlineUIElement>. If you leave this value on <InlineUIElement>Strict</InlineUIElement>, the user's email won't be returned.
...
In a full single page app, you'll probably be using a client-side routing framework. But this example checks for only two cases: either it is handling a callback from the authentication server and checks the challenge code matches, or it displays the login link on the page for any other route. In either case, if there is a logged-in user Id cookie present, the user's email address is displayed on the screen.

All the logging link does is redirect the user to the FusionAuth hosted backend API endpoint.

When making the request, you need to specify the `openid` and `email` [scopes](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/scopes), or the user's email address won't be returned in the `app.idt` token.
