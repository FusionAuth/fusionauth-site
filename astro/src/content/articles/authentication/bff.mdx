...
Let's look at some actual code to create a BFF for a currently serverless app. The full project is available at TODO if you want to download and run it without having to manually create files.

First, you need a frontend. For this, create a file called `index.html` with the content below.
...
Next, create the tiniest Express.js server possible. Add a file called `app.js` with the content below.
...
More complicated is the `authentication.js` file. Create it and add the content below.
...
Browse to the app at http://localhost:3000 and log in with `richard@example.com` and `password`.
