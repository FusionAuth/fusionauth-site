...
If you ran the node server in the previous section, close it by pushing <kbd>Ctrl-C</kbd>, but leave the FA server running. In a new terminal, run the command below to start the serverless web app.
...
As before, browse to http://localhost:3000 to view the app. Click the login link to authenticate with FA. Use the credentials `richard@example.com` and `password`.

TODO screenshot with idt cookie and get username from cookie in code
...
When looking for such a feature, it might be called an OAuth proxy or BFF. Curity calls it the [token handler pattern](https://curity.io/blog/token-handler-the-single-page-applications-new-bff/).
...
Below is a list of which authentication gateway brands offer OAuth proxies:
...
Though this article was long, the recommendations are simple:

- Use cookies in the browser. Never store secrets in local storage or session storage.
...
- The easiest way to add an OAuth Proxy is with FA hosted backend, the Go Auth2 proxy, or building on the Passport.js Node sample in this article.
