...
<Aside type="note">
Authentication is handled by OpenID Connect (OIDC), a layer on top of OAuth, and by not OAuth directly. OIDC specifies how an authorization server can issue Id tokens, whereas OAuth governs authorization of access to resources and does not cover user identification.

FusionAuth (and other similar providers) provide an authorization server that issues both Id and access tokens, thereby implementing both OIDC and OAuth 2.0, respectively, which supports both login and API authorization.
</Aside>
...
- When the initial request for an authorization code is sent, the browser includes a secret, which the authorization server stores.
- When the request for an access token is sent, the browser includes not only the authorization code, but also the original secret. If the secret the authorization server stored does not match the one from the browser, the server will not return an access token. The access and refresh tokens are obtained directly from the app in the browser and are stored there.
