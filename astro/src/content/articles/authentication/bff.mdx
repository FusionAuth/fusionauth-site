...
- [Serverless Apps And Public API Apps](#serverless-apps-and-public-api-apps)
...
## Serverless Apps And Public API Apps

For an example of a web app without a server, consider the typical todo or task list app. You might write a todo app to help people keep track of their chores, but not want to spend time and money having the app store user data in your own server and database hosted somewhere online. So you use the Dropbox file storage API for your app to store user data in Dropbox. Now your server can be entirely static, merely a file host that serves HTML and JavaScript files, such as a domain name that points to GitHub pages or Cloudflare. This design can be called a serverless app, and is shown below.
...
While the Dropbox supports OAuth, your design doesn't have a server, so you can't use the OAuth code flow. In this case, the OAuth specification recommends the client-only PKCE flow.
