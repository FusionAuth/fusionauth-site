...
Again, this design is identical to the previous app design that had a standard server. The reason that it is called a BFF server is that it is designed exclusively to provide authorization services to the front end. The real business service server is actually the resource server.

However, your business might have its own comprehensive server for business logic. In this case you still might want to use the BFF pattern, keeping a separate server purely for authentication, so that mobile, web, and desktop apps can all have dedicated BFF servers of their own. This design would look like the diagram below, and leads us back to design in the introduction of the article.
