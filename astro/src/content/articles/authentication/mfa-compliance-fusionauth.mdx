...
For instance, one form of [passwordless authentication](/features/passwordless) is [magic links](/docs/lifecycle/authenticate-users/one-time-passwords/). Magic links are one-time, time-limited codes delivered via email, SMS, or a push notification. FusionAuth lets you easily generate magic links as the first factor of MFA or on their own. However, it's worth noting that a second factor such as email, SMS, or TOTP would be needed in conjunction with the magic link.
