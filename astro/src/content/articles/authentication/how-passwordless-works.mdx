...
Passwordless authentication [replaces passwords](/docs/lifecycle/authenticate-users/one-time-passwords/) with newer mechanisms that are both easier to use and more secure. It enhances the user experience, protects users and your data, and provides more consistency in the level of protection applied to each account.
...
[FusionAuth](/) is a developer-oriented authentication solution that simplifies integrating authentication workflows without having to build them yourself. FusionAuth gives you instant access to [passwordless authentication options](/features/passwordless) that let users log in with a magic link or one of the [WebAuthn-compatible mechanisms](/docs/lifecycle/authenticate-users/passkeys/) available on their device, such as a biometric scan or hardware key. FusionAuth is customizable, scalable, and available in both cloud-hosted and self-hosted packages.

[Get started for free](/docs/get-started/download-and-install/docker) or [talk to an expert](/contact) to start your passwordless auth journey with FusionAuth.
