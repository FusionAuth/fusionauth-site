...
Your authentication system documentation should specify this. It's best if this is an industry-standard hash such as `Argon` or `PBK2DF`. Certain authentication systems, such as FusionAuth, allow for [custom password hashing algorithms](/docs/extend/code/password-hashes/custom-password-hashing) to be used, which can mitigate the risk of using a nonstandard hash.
