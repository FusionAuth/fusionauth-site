...
A commonly held belief is that security measures make accessing resources harder---but only for end users. Attackers excel at finding ways around security tools and measures. They take advantage of annoyed users to be more successful at [phishing campaigns](https://www.barracuda.com/support/glossary/phishing-campaign) and easily collect those long, complex, inconvenient passwords.

[Multifactor authentication (MFA)](/articles/authentication/multi-factor-authentication) stepped in to stop these criminals from effectively accessing company resources using stolen passwords. But if users are already triggered by complex password policies, adding yet another step to the authentication process makes the matter go from bad to worse---in their eyes, at least.

Adaptive MFA is the logical evolution towards a more user-friendly approach. While traditional MFA simply requests an additional authentication after the password (one extra step for the end user), adaptive MFA gives administrators many possibilities for enforcing an environment and bolstering a company's security posture while keeping things simple for the (already busy) end user.

This article explains how convenient adaptive MFA can be, how it improves authentication on products, apps, and environments, and why you need to consider implementing it.
...
## Adaptive MFA Is the Answer

Adaptive MFA emerged as the solution to these issues.
...
### Why Is Adaptive MFA a Game-Changer?

Adaptive MFA offers so many benefits because it's secure *and* simple.
