...
We've explored and [implemented an MCP for controlling FusionAuth](/docs/get-started/download-and-install/development/mcp-server). MCP is great for learning a tool, building prototypes, or debugging. But I'm not sure folks will be using it to replace [configuration management](/docs/operate/deploy/configuration-management) in production systems.
