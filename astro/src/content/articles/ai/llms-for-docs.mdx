...
Large language models (LLMs) are in use by more and more developers. They use them to write code, but also to discover and leverage tools. In this article, you will learn how to make your docs more accessible to LLMs and to developers using them, based on our real world experience.

LLMs and LLM enabled tools such as [Claude](https://claude.ai/), [ChatGPT](https://chatgpt.com/) and [Cursor](https://cursor.com/) are used by developers in their daily workflow. In 2024, [a survey from GitHub](https://github.blog/news-insights/research/survey-ai-wave-grows/) found over 97% of the 2000 developers surveyed "used AI coding tools at work at some point".
...
Helping developers be successful by using our documentation to get to what they are trying to accomplish is something we spend a lot of time on here at FusionAuth. In this article, we'll talk about ways we have modified our documentation to make it work better with LLMs, and show you how to do so.
...
The downside of building and maintaining good documentation is it takes a lot of effort to do so. But if you don't get this right, the rest of the tips in this article are not going to help you much. 
...
For discovery, ask developers how they found you. We've seen a notable uptick in users mentioning finding FusionAuth via LLMs. In fact, since we started tracking it in our [setup wizard](/docs/get-started/download-and-install/setup#complete-the-setup-wizard), it has been one of our top three sources. You can also manually look for references or unique strings from your docs that are returned by an LLM of your choice. You can also examine the traffic to your website using your access logs, and see which of your content is being retrieved by the LLM user agents. This is something you'll need access logs to ascertain, since JavaScript tools like Google Analytics don't track direct requests to files at a certain location.
...
* `llms-full.txt` is a similar list but includes the full text rather than just the URL of the pages.
...
We build these files for our docs section.
...
![A copy to markdown button on a FusionAuth documentation page.](/img/articles/ai/llms-for-docs/markdown-button.png)
...
We also used an Astro build plugin to generate the markdown file at a sibling location to the HTML, so the JavaScript above could retrieve it. That plugin has since been extracted into a standalone npm package: [`astro-gen-markdown-pages`](https://www.npmjs.com/package/astro-gen-markdown-pages).

The plugin:

* runs after the Astro build completes and processes every HTML output file
* strips navigation, scripts, hidden elements, and other noise
* converts the remaining HTML to clean markdown using Turndown
* writes a `.md` file next to each `.html` file so the copy-to-clipboard button can fetch it
* assembles an `llms.txt` index linking to all the generated markdown files
...
The [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro), or MCP, allows you to let an LLM directly interface with your developer focused product.

We've explored and [implemented an MCP for controlling FusionAuth](/docs/get-started/download-and-install/development/mcp-server). MCP is great for learning a tool, building prototypes, or debugging. But I'm not sure folks will be using it to replace [configuration management](/docs/operate/deploy) in production systems.

MCP is not covered here because it is doc-adjacent but not part of documentation. 
