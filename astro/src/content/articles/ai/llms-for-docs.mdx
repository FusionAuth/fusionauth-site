...
AFDocs was a great starting point for us: we originally got an **F**. So we made [some improvements to our documentation infrastructure](https://better-static-sites.github.io/docs/). After adding an [Astro plugin to generate Markdown from the built-time AST](https://better-static-sites.github.io/docs/build-tools/gen-markdown-pages/) and automating the production of `llms.txt` indexes, we focused on discoverability: we added links to the Markdown version of each page and the `llms.txt` index, as well as a redirect for any request that passes the `accept/markdown` header. Then we focused on accessibility, optimizing our page sizes, reducing boilerplate, lifting content towards the top of the HTML, and displaying diagrams as their mermaid declarations in our Markdown output and alt text. We removed all unnecessary JavaScript from our documentation pages, moving to a [lean mermaid rendering plugin](https://better-static-sites.github.io/docs/build-tools/mermaid/) that generates SVGs at build time and an ['On this page' widget](https://better-static-sites.github.io/docs/docs-layout/toc-smol/) also built from the site's AST.
