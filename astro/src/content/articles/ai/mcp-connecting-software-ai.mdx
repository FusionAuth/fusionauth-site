...
tags: ai, mcp, oauth, authentication, machine identity, ai agent identity, security, api
author: Simon Russell
section: AI
icon: /img/icons/articles/ai/mcp-connecting-software-ai.svg
darkIcon: /img/icons/articles/ai/mcp-connecting-software-ai-dark.svg
---
...
This solved the problem of "how should the LLM indicate it needs to call a tool", but the actual communication was still specific to a particular agent framework; each tool would need to be modified to use it with a different agent. To encourage the development of more tools, Anthropic realised a standardised API format would be needed, and created the [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro) at the end of 2024.
...
![Diagram of how large language models and agents interact with MCP.](/img/articles/ai/mcp-explainer/agent-large-language-model-diagram.png)
...
<TableGrid widths="2 2 6">
  <Row>
    <Cell rowspan={2}>**Data layer**</Cell>
    <Cell>Primitives</Cell>
    <Cell>The specification defines a set of JSON-RPC endpoints that can be called to perform [tool calls](https://spec.modelcontextprotocol.io/specification/2025-06-18/server/tools) and access other features.</Cell>
  </Row>
  <Row>
    <Cell>JSON-RPC</Cell>
    <Cell>Defines a simple remote procedure call protocol using JSON. It's a [pretty short read](https://www.jsonrpc.org/specification), and some features like batching aren't supported in MCP.</Cell>
  </Row>
  <Row>
    <Cell>**Transport layer**</Cell>
    <Cell />
    <Cell>Specifies how the JSON-RPC calls will be made</Cell>
  </Row>
</TableGrid>
...
The simplest transport layer to understand is [the "stdio" transport](https://spec.modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio). This is also called "local".
...
This is actually [the second version](https://spec.modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) of an HTTP transport in the short life  of MCP – the earlier SSE transport was a little more complicated to implement. This is also called "remote".
