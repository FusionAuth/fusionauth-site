...
Each agent needs an identity. In FusionAuth, you model agents as Entities, but in another system you might use service accounts or short lived OAuth clients. 
...
Clean up agent entities when the workflow completes successfully. For error conditions, implement a reaper process for orphaned entities from failed workflows. Doing this work ensures the agents' credentials are no longer usable.
