...
When answering a query, the LLM should never even see documents the user shouldn't have access to. You don't have to craft some clever prompt. You're not relying on the model to keep secrets. With the right authorization framekwork, you're filtering documents before they reach the model.
...
For the authorization checks, use a fine-grained authorization (FGA) system. [FusionAuth FGA by Permify](/docs/extend/fine-grained-authorization/fusionauth-fga) is one option. It provides deterministic authorization that can be deployed on-site for data safety and scales with your needs. 
...
The same gateway patterns you've been using since the REST API era can help rate-limit or monitor access for either MCP API servers.
...
Consider making an SDK using your API available as well. Again, you may need to add fine-grained authorization to the services the MCP server is accessing if you need granular control beyond what OAuth scopes provide.
...
This is called the chain of identity.
...
Each agent needs an identity. In FusionAuth, you model agents as Entities, but in another system you might use service acconts or short lived OAuth clients. 

Entities are the same primitive used for IoT devices, APIs, and machine-to-machine communication. Entities can have types, permissions, and grants, making them perfect for representing agent identities.

Here's how you'd set up the drive and coordinator agent entities for the business banking workflow; the calendar and business service API agents are omitted for brevity but are configured in the same way:
...
Requests are made using the client credentials grant to ensure the calling agent has permission to call the recipient agent. Then the JWT vend API mentioned above is used to preserve the chain of identity.

The agents can log key points in their workflow:

```javascript
import { FusionAuthClient } from '@fusionauth/typescript-client';

// Use a write-only API key scoped to audit log creation
const driveAgentClient = new FusionAuthClient(
  'DRIVE_AGENT_WRITE_ONLY_API_KEY',
  'https://your-fusionauth-instance.com'
);

// Log when the drive agent begins its work
await driveAgentClient.createAuditLog({
  auditLog: {
    insertUser: 'DriveAgent (328ae70f-20cf-4f28-8e18-d1d43bdfa919)',
    message: 'Gathered 4 documents from folder',
    reason: "{\"agentType\":\"DriveAgent\",\"action\":\"drive.gather.completed\",\"actor\":\"328ae70f-20cf-4f28-8e18-d1d43bdfa919\",\"delegatedBy\":\"35d45ce6-1088-4f71-8523-f4245ecc72dc\",\"actingOnBehalfOf\":\"d384a5cb-2dfd-40dd-871e-8ae48d343250\",\"roles\":[\"business_owner\"],\"scope\":\"drive:read\"}",
    data: {
      agentType: "DriveAgent",
      action: "drive.gather.completed",
...
This is added to both the `reason` and `data` fields for searchability in FusionAuth, but the type of data to store for auditing purposes is business logic specific.

Clean up agent entities when the workflow completes successfully. For error conditions, implement a reaper process for orphaned entities from failed workflows. Doing this work ensures the agents credentials are no longer useable.
...
The landscape is evolving fast. MCP is maturing. Agent frameworks like Agentcore and Mastra are gaining traction. Data provenance, knowing which agent did what to what data, is evolving.
