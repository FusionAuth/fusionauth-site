...
Even if an agent workflow was kicked off using a scheduled job or cron, the agent is still acting with authority granted by a human being. 

It could be the engineer who configured the cron job, or the team lead who approved the deployment. There is a human identity at the root of that chain. Their credentials should be the ones encoded in the delegation token, and when they leave the company, that scheduled agent's authority should be reviewed or revoked.

Eventually agents may be autonomous, but for now, tie them to a human identity.
