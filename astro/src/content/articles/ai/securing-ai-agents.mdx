...
But first, let's define what we are talking about. Agents are 
...
- Software on the desktop or the webl like Claude Desktop or ChatGPT that access tools and APIs to perform tasks
...
## Secure Human Identities
 ...
 The bedrock. Every agent action should starts with a verified human identity. This is what your identity provider handles.
...
Because of agent non-determinism, ensure this is one of many layers of security. In addition, make sure you include this behavior in your evals, so you can check and improve it over time. We'll talk more about evals below.

## Sub-agents

You can split agents, creating multiple agents each with a specific task in mide. You can also enforce request guardrails between agents. You can let Agent A call agent B, but not vice versa.
...
The proper way to manage this access is to mint short lived tokens from these refresh tokens that are properly scoped. At a minimum, limit the scopes needed to the agent's activities. This means that the damage an agnet can do with a credential is limited both in time and in scope.
...
Below is an example of using the FusionAuth audit log to capture part of the agent ations.
...
When performing filtering, make sure your solution is deterministic and performant.
...
- are irreversable, such as deleting something from a database
...
Your identity provider can supply the tooling to support governance, such as webhooks, audit logs, and agent identities. But governance itself is an organizational discipline, not a software feature. Your identity provider helps you build governance; it's not a governance solution on its own.
