...
When an access token is needed to make an appointment, the system makes a standard refresh token grant request. What comes back is a short-lived access token with a defined expiration window that allows for calendar access.

That fresh, narrowly scoped token is then injected into the agent's context, giving the agent just enough access to make the appointment, for just long enough to do it, without ever exposing the long-lived refresh token to the agent itself.
...
When the agent takes a meaningful action, have it post an entry. The log entry captures two critical pieces of information: who is acting, and what they did.

The "who" encodes the full chain-of-identity: for example, that it was a calender agent started by a coordinator agent, acting ultimately on behalf of a specific human user.

The "what" is a plain-language description of the action, such as scheduling a meeting for participants.

Such audit logs give you a queryable, tamper-resistant record that traces every agent action back to a real person.
...
They aren't perfect, though. Sub-agents have the benefits listed above, but they multiply the number of identities and tokens you need to manage, which makes the "limit service access" layer more complex and harder to implement well.
...
Logging and tracing, mentioned above, are in tension with sandboxing. The more thoroughly you sandbox an agent, restricting its network access and isolating the environment, the harder it becomes to get logs out of that sandbox and into a durable, queryable system beyond the agent's reach. You either need to punch holes in the sandbox's network rules to allow logging traffic, which weakens the isolation, or you need to build a out-of-band mechanism to collect logs, which adds operational complexity.
...
However, FGA isn't perfect, and can't always prevent privilege escalation through tool chaining. Suppose Alice asks the CodeAssistant agent to query the staging database for a list of table names. That's a perfectly legitimate read operation on a resource she has access to. The agent returns the results. Then Alice asks the agent to create a pull request that includes a database migration script. Also a legitimate operation, since she's allowed to create PRs. But the migration script references production table names that the agent learned from staging metadata, and the PR targets a branch with a CI pipeline that automatically runs migrations against production on merge.

No single step violated the FGA rules. The agent read from staging (allowed). It created a PR (allowed). But by chaining those two permitted actions, the agent effectively gained write access to production, something FGA rules were explicitly designed to prevent.

This is why the checks at both levels matter. The sandbox-level orchestration might catch that the agent is attempting to create a PR containing production references and flag it. The GitHub MCP server might independently verify that the target branch's deployment scope exceeds what this agent-user combination is authorized for.

And ideally, the human interaction layer, discussed below, would require approval before any PR that touches a production deployment pipeline gets merged.

Each layer has a chance to break the chain that no single authorization check would catch on its own.
...
In practice these layers can fight each other, and aren't always additive. For example:
...
Building a secure agent system is not trivial and requires judgement balancing these tradeoffs.
