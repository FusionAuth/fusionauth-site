...
- Tools Claude Desktop or ChatGPT that access tools and APIs to perform tasks
...
* **Third-Party Service Access**: When agents need to access services like Google Calendar, Office 365, or Salesforce, they shouldn't have access to long lived credentials.
...
## Secure Human Identities
 ...
 The bedrock. Every agent action chain starts with a verified human identity. This is what your identity provider handles.

## Craft the Right Prompt

Instruct agents to behave like responsible employees. Prompt engineering is a security control, not just a UX concern.
## Sub-agents

Split agents to reduce blast radius and make trust boundaries explicit.
...
## Sandboxing
 — Control network access and filesystem access using containers or VMs.

## Fine-Grained Authorization

 — Use fine-grained authorization (FGA) to control access based on agent identity, user identity, resource attributes, and context. Build your authorization schema once and apply it consistently across APIs, services, MCP servers, and agent workflows.

## Logging and Tracing

Preserve the chain of identity and capture key actions at decision points. Use RFC 8693 or native API calls like the Vend JWT API to create signed tokens that encode the delegation chain (the `act` claim). Log agent actions using an audit log API with limited API keys minted for each agent. If the chain of identity isn't captured in your preferred logging system, use webhooks or tools like Kafka to fan these log entries out to other systems for compliance and monitoring.
...
Validation This lets you test the entire agentic flow end to end and make sure that you don't introduce regressions as you evolve the system.
## Human Interaction
...
## Input/Output Filtering
 ...
 Control what goes into and comes out of your agentic system using a filtering tool. Maintain stop word lists, both standard and custom, to prevent sensitive operations from being triggered in the wrong context. For example, "pull credit" or "register new business" might be allowed in a new business registration workflow but blocked in customer service.
...
## Validation
 ...
 Validate outputs based on expected inputs. This lets you test the entire agentic flow end to end and make sure that you don't introduce regressions as you evolve the system.


## Human Interaction

Agents need to ask for human confirmation before taking consequential actions.
...
what about agents not tied to human identites
