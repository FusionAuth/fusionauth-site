...
By default, the FusionAuth CLI collects data on usage. We use this to judge effectiveness of the CLI, check performance and reliability, and help drive new features and improvements.

If you'd prefer not to send usage data, you can disable telemetry by editing the `telemetry` with the following command:

```sh
# opt out of sharing usage data
npx fusionauth telemetry:disable

# opt back into sharing usage data
