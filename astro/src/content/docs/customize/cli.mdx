...
description: Learn about the FusionAuth CLI tool.
tags: fusionauth cli, command line interface, theme management, email management, lambda management, developer tools, configuration management, version control, cli tool
---

The FusionAuth command line interface (CLI) allows you to manipulate FusionAuth from the command line. The focus of the CLI is on allowing easy management of commonly modified customization code and markup, such as emails, themes or lambdas. It is not a full featured replacement for any of the [client libraries](/docs/sdks/), which wrap all of the API.
...
* generate fake user data suitable for the [User Import API](/docs/apis/users/import)
