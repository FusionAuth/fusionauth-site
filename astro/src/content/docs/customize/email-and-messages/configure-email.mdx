...
tags: smtp configuration, email setup, transactional email, spf, dkim, dmarc, email provider, sendgrid, mailgun, email security
---
...
<Aside type="note">
The DNS records you need to add for SMTP will usually be of type `TXT`. A value of `@` for the **Host** of a record means the domain itself, and is a common default value.
</Aside>
...
As well as being a popular ESP, Gmail can be used by programs to send email through SMTP. Gmail is not recommended for production use, since it has a [number of limits](https://knowledge.workspace.google.com/admin/gmail/gmail-sending-limits-in-google-workspace). However, it can be useful to test email functionality.
...
- In your tenant's <Breadcrumb>Email</Breadcrumb> tab in the FusionAuth web interface, set the following values:
  ...
  - **Password**: Get this by pushing <Breadcrumb>Reset password</Breadcrumb> in Mailgun and copying the value you are given.
  - **Security**: `TLS`
