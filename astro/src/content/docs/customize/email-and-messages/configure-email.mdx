...
<Aside type="note">
FusionAuth doesn't come bundled with an SMTP server, but expects one to be configured to enable all email based functionality. This design choice allows you to:

* use whatever email sending service you'd like (some are listed below, but FusionAuth should work with any that have an SMTP API)
