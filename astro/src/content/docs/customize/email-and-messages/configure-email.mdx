...
- A DMARC record looks like `v=DMARC1; p=none; rua=mailto:rua@dmarc.brevo.com`. It specifies what policies the email sender supports, and where to send rejected email for analysis by the sender.
...
These scores, especially the low ones for Mailchimp and Sendgrid, are similar to the experiences we had when writing this guide.
...
<APIBlock>
  ...
  <APIField name="Change password" optional>
    When enabled, you may modify the password used to authenticate to the SMTP server, when the <InlineField>Password</InlineField> field is not displayed the current password will not be modified.
  </APIField>
...
- In your account dashboard, search for SES and start the account setup wizard by clicking the <strong>Get set up</strong> on the left side bar.
...
- Send your transactional email and your marketing email through different IP addresses, as marketing email is more likely to be marked as spam. FusionAuth entirely uses transactional email.
