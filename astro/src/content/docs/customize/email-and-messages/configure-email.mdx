...
In practice, this means your SMTP provider will ask you to add a TXT record to your DNS records for your domain. The record will have a host like `mail._domainkey.fa.dns-dynamic.net` and a public key like `k=rsa;p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDeMVIzrCa3T14JsNY0IRv5/2V1/v2itlviLQBwXsa7shBD6TrBkswsFUToPyMRWC9tbR/5ey0nRBH0ZVxp+lsmTxid2Y2z+FApQ6ra2VsXfbJP3HE6wAO0YTVEJt1TmeczhEd2Jiz/fcabIISgXEdSpTYJhb0ct0VJRxcg4c8c7wIDAQAB`. When the SMTP provider sends an email on your domain's behalf it will sign the `from` address of the email with its private key, which the receiving client can verify against your publicly available DKIM public key.
...
Account SID: AC7a63585c292e1f6fa46141eebedc8f00
