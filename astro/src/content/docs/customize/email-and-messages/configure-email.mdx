...
- [IP Pools enable you to group your dedicated IPs into customized "pools" to help manage your sending reputation for different mail sending streams.](#ip-pools-enable-you-to-group-your-dedicated-ips-into-customized-pools-to-help-manage-your-sending-reputation-for-different-mail-sending-streams)
...
- [SMTP Providers](#smtp-providers)
  ...
  - [Resend](#resend)
  - [SendGrid](#sendgrid)
- [Why Can't You Use A Self-Hosted SMTP Server?](#why-cant-you-use-a-self-hosted-smtp-server)
- [Troubleshoot](#troubleshoot)
  ...
  - [Free Accounts May Send Only To Specified Recipients](#free-accounts-may-send-only-to-specified-recipients)
  - [Email Doesn't Arrive](#email-doesnt-arrive)

## Overview

Before you can use email templates and other email features, you must first enable and configure a Simple Mail Transfer Protocol (SMTP) server.
...
A DKIM record looks like `k=rsa;p=MIGfMA0GCSqGSIb3DQEBAQ8c7wIDAQAB`. It specifies a public key. The SMTP provider will sign the from address of an email with the corresponding private key. Email receivers should reject emails where the from address signature is not valid.
...
- Ensure that you add all three security records discussed in the previous section to your DNS records.
- Do everything you can to keep sender reputation high. This includes sending as few emails to the user as possible (to avoid emails being marked as spam), and remove from your email lists any non-existent email addresses that bounce.
...
Mailgun: Dedicated IPs help isolate your sending reputation and improve your deliverability when sending a large volume of messages. We recommend sending at least 50,000 messages a week to maintain a strong reputation and minimize throttling with mailbox providers.
Mailgun recommends one dedicated IP address for every 1 million messages you are sending each month. If you have questions about your use case, please open a support ticket for additional information.
...
- Mailgun (requires phone authentication, and limited to five to addresses). The only support team that activate our account for testing.

The more difficult providers are (from easiest to most complicated):

- SendGrid/Twilio — automatically disable your account on sign-up and require you to manually contact to support to have it activated. Requires phone authentication.
- Postmark — require you to have an email address on a custom domain. You cannot use an email alias or public address like Gmail or Yahoo.
...
- TODO add authorised receiver - workaddress/fa@simplelogin.com
...
- In your tenant's email tab in the FusionAuth web interface, set the following values:
  ...
  - <InlineField>Port</InlineField>: `587`
  - <InlineField>Username</InlineField>: `postmaster@fa.dns-dynamic.net` (Replace with your login)
  ...
  - <InlineField>Security</InlineField>: `TLS`
  - <InlineField>Default from address</InlineField>: `postmaster@fa.dns-dynamic.net` (Replace with your login)
- Click <InlineField>Send test email</InlineField> to test the settings work, then click the save icon at the top right. Enter your real email address for the test email, so you can test it arrives.

### Postmark

TODO use ritza email. no public emails
...
TODO - waiting for twilio/sendgrid to reply
...
The following accounts were affected:
Account SID: a0983kn43098a

To reactivate your account, we kindly ask that you reply to this email with the following information:

  - Are you a developer creating an experience to engage with your clients and/or customers on behalf of your company?
  - What Twilio products do you plan on using? Please outline your use case or what you intend to build.
  - If you are using Twilio for your business and/or company, please provide a link to the website/app. If available, provide a public source (such as a LinkedIn profile, business registry, or other online verification such as GitHub, and any social media platform) that confirms your association with the brand/company.
```
...
## Why Can't You Use A Self-Hosted SMTP Server?

It is possible to send email through an SMTP server you host on your own server, with an application like [Postfix](https://www.postfix.org/documentation.html), [Haraka](https://haraka.github.io/getting_started), [Mailcow](https://docs.mailcow.email/), or [Mail-in-a-Box](https://mailinabox.email/guide.html).

However, this will almost certainly cost you more time and money than paying a dedicated email company to handle mail for you. Below are some things you'll need to handle if you want to self-host:
- Deliverability: ISPs block or filter emails from dynamic IP addresses and untrusted servers. You need to maintain IP reputation, implement SPF, DKIM, and DMARC properly, and handle feedback loops with ISPs (bounces and spam requests).
...
You will probably get this error only with AWS, which requires your to address to be verified when testing until your SES service leaves the testing sandbox. To fix this error, send the FusionAuth test email to the same address that you verified when creating the SES service in AWS.

### Free Accounts May Send Only To Specified Recipients
...
You will get this error with Mailgun and other providers that limits their free services. To fix it, add your reciepient email address in the allowed list of receivers in the SMTP provider's settings.
