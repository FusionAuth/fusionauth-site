...
The difficult providers automatically disable your account on sign-up and require you to manually contact to support to have it activated. Generally this involves having a business website ready for review, explaining what your purpose in sending email is, and explaining how you obtained your recipients' email addresses.

Be aware that these companies can cancel your account at any time, without explanation. This can cripple your business. Here is an example of an email received by FusionAuth from Brevo while testing:
...
Choose an SMTP that you feel is trustworthy very carefully.
...
<!-- - Click your username at the top right of the page and choose <InlineField>Account & billing</InlineField>. Click the <strong>Domains</tab> and <InlineField>Start Authentication</InlineField> of your domain.
  - Add a domain: `fa.dns-dynamic.net` (or your domain name).
  - In your website domain manager, add the DNS records from the domain authentication page.
    - If you use cPanel, alter your records at <strong>Domains -> Zone Editor</strong>
    - Be careful not to duplicate the domain name. For example, if your host is `fa.dns-dynamic.net` and the MX record value the SMTP provider wants you to add is `send.fa`, be sure set the value to `send.fa.dns-dynamic.net`. Don't use `send.fa.fa.dns-dynamic.net` by pasting the value without checking what suffix your host attaches to the field.
    - Mailchimp uses CNAME records instead of TXT records for DKIM. They are the only SMTP provider in this guide that does so.
...
- Click <InlineField>Send test email</InlineField> to test the settings work, then click the save icon at the top right. Enter your real email address for the test email, so you can test it arrives. TODO
