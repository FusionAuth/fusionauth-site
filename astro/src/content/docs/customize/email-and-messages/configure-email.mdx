...
In practice, this means your SMTP provider will ask you to add a TXT record to your DNS records for your domain. The record will have a host like `mail._domainkey.fa.dns-dynamic.net` and a public key like `k=rsa;p=MIGfMA0GCSqGSIb3DQEBAQ8c7wIDAQAB`. When the SMTP provider sends an email on your domain's behalf it will sign the `from` address of the email with its private key, which the receiving client can verify against your publicly available DKIM public key.
...
- Bounces (incorrect target email address)
- Spam complaints ("Mark Email as Spam" feature in email clients)
