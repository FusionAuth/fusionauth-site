...
Together, these three records allow an email receiver to be sure the email server and sender are valid, and what responses the sender supports if an invalid email is received that claims to be from the sender.
...
- Mailgun (requires phone authentication, and limited to five to addresses)
...
- Add your domain under `Sending` in the Mailgun dashboard at https://app.mailgun.com/mg/sending/new-domain.
- fa.dns-dynamic.net
