...
- Brevo — require phone authentication. Automatically disable your account on sign-up and require you to manually contact to support to have it activated.
...
TODO - wait for support to activate account
...
- In your website domain manager, add the records from Resend at the link above.
  ...
  - Where Resend requires you to create a subdomain, be careful not to change the host value twice. For example, if your host is `fa.dns-dynamic.net` and the MX record value Resend wants is `send.fa`, be sure set the value to `send.fa.dns-dynamic.net`. Don't use `send.fa.fa.dns-dynamic.net` by pasting the value from Resend without checking what suffix your host attaches to the field.
  ...
  - TODO <img src="cloudns_resend.png" />
