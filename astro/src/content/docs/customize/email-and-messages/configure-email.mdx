...
SMTP providers handle two types of email: marketing (e.g., newsletters and sale advertisements) and transactional (e.g., account notifications and password resets). FusionAuth is concerned only with transactional email.
...
Here are the ways ESPs implement security:
- Rate limits — Emails received too frequently, or in sudden spikes in volume, are blocked.
...
SPF, DKIM, and DMARC are TXT records you need to add to your DNS records for your domain. Your SMTP provider will tell you what records to add.

An SPF record looks like `v=spf1 ip4:129.6.100.200 ip6:2610:20:6005:100::20 -all`. It lists what IP addresses are permitted to send email on behalf of the domain. ESPs should reject emails arriving from an IP address other than these.

A DKIM record looks like `k=rsa;p=MIGfMA0GCSqGSIb3DQEBAQ8c7wIDAQAB`. It specifies a public key. The SMTP provider will sign the sender address of an email with the corresponding private key. ESPs should reject emails where the sender address signature is not valid.
...
Together, these three records allow an ESP to be sure the email server and sender are valid, and what responses the sender supports if an invalid email is received.
...
- AWS SES — require email and domain verification.

Postmark was found to have especially clear documentation and an easy setup experience.

The more difficult providers are:
- Brevo
- Mailgun (Sinch)

These providers automatically disable your account on signup and require you to contact to support to have it activated. This involves having a business website ready for review, explaining what your purpose in sending email is, and explaining how you obtained your recipients' email addresses. However, Mailgun and Brevo support were friendly, quick, and unlocked the FusionAuth test account after reasonable discussion.

The worst providers are:
- Mailchimp (Mandrill)
- SendGrid (Twilio)

Mailchimp and Sendgrid were the only SMTP providers that entirely refused to provide FusionAuth an account, without any explanation except accusations of prohibited content.

Be aware that providers can cancel your account at any time, without explanation or chance to appeal. This can cripple your business. Below is an example of an email received by FusionAuth from Sendgrid. Sendgrid claimed to have done a "thorough review" in a few seconds after account creation and refused to discuss it further:

> After a thorough review, we regret to inform you that we are unable to proceed with activating your account at this time. While we understand the importance of transparency, we are not able to provide the specifics of our vetting process.

Mailchimp support was poor — entirely automated and no human could be contacted. Sendgrid support didn't even reply.

Choose an SMTP provider that you feel is trustworthy very carefully. At the time of writing, these were the scores on https://www.trustpilot.com for each provider in this guide:
...
These scores, especially the low ones for Mailchimp and Sendgrid, are similar to the experiences FusionAuth had when writing this guide.
...
To avoid breaking the various DNS records of your application's domain, you may want point FusionAuth SMTP fields to a temporary subdomain. For example, if your application is hosted at `myapp.com`, you can create `testemail.myapp.com`, and point the SMTP service and FusionAuth there while testing.

Strict SMTP providers require you to have a trustworthy looking domain and website.
...
<Aside type="warning">
Be careful not to duplicate the domain name when entering DNS records. For example, if your host is `myapp.com` and the TXT record value the SMTP provider wants you to add is `send.myapp`, be sure the final value is `send.myapp.com`, and not `send.myapp.myapp.com`. Check what suffix your host attaches to the field.
</Aside>
...
<Aside type="note">
Note that the DNS records you need to add for SMTP will nearly always be of type `TXT`. A value of `@` for the <InlineField>Host</InlineField> of a record means the domain itself, and is a common default value.
</Aside>
...
- Click <InlineField>Send test email</InlineField> to test the settings work. You must send to an email address verified with SES. Use the address you gave when creating your SES account.
...
Gmail can be used by programs to send email through SMTP, as well as being a popular ESP. Gmail is not recommended for production use, since it has a [number of limits](https://support.google.com/a/answer/166852). However, it can be useful to test email functionality.
...
- To use your real domain, browse to [Email -> Domains](https://app.mailersend.com/domains).
  ...
  - On the next page, <strong>Domain verification</strong>, you are given DNS records to add to your domain.
  ...
  - Now you can again click <InlineField>Generate new user</InlineField> for this domain and use the SMTP details in FusionAuth.
...
- Add your domain name under `Sending` in the Mailgun dashboard at https://app.mailgun.com/mg/sending/new-domain.
...
- Browse to https://app.mailgun.com/mg/sending/domains. Click the gear icon to the right of your domain name and select <InlineField>Settings</InlineField> to browse to a page like https://app.mailgun.com/app/sending/domains/myapp.com/settings. Click <InlineField>SMTP credentials</InlineField>.
- In your tenant's email tab in the FusionAuth web interface, set the following values:
...
While in the Postmark sandbox mode before account approval, you may not email addresses outside your domain. These addresses will be shown in the <strong>Activity</strong> tab of your server on Postmark.
...
For an overview of all the records Resend asks you set on your domain, please read their [documentation](https://resend.com/docs/dashboard/domains/introduction). For more general information, read the [Resend documentation on SMTP](https://resend.com/docs/send-with-smtp).

### SendGrid (Twilio)

You can register for a SendGrid account at https://signup.sendgrid.com.

Unfortunately, not only did SendGrid refuse to activate FusionAuth's account, they also disallowed logging in. So this guide cannot provide instructions on how to use their site. If you already have a SendGrid account and want to use it with FusionAuth, follow the instructions for another provider, such as Brevo. All the providers are similar to configure, except Gmail and SES.
...
- Email an address that is authorized and verified by your SMTP provider, such as the one on your authenticated domain that you used to register your account. Also check if the provider has an authorized recipient sections and add the address there.
