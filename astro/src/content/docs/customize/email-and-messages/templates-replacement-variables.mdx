...
On Send Email API request the contents of the <InlineField>requestData</InlineField> field will be made available to you when the template is rendered.
...
<APIBlock>
  ...
  <APIField name="breachResult.match" type="String">
    ...
    * `SubAddressMatch` The User's loginId and password were matched, but the email address was a sub-address match. For example, `joe+test@example.com` is a sub-address match for <InlineFieldValue>joe@example.com</InlineFieldValue>.
