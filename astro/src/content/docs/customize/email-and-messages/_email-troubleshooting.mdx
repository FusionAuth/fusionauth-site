...
* Make sure the email template is correct, or use [a default template](/docs/customize/email-and-messages/overview).
...
. Enable debugging by navigating to <strong>Tenants -> Your Tenant -> Edit -> Advanced -> SMTP Settings -> Additional properties</strong> and add `mail.debug=true`.
. Save the tenant.
. Send an email. 
. View the system logs by navigating to <strong>System -> Logs</strong>. 
...
. Enable debugging by navigating to <strong>Tenants -> Your Tenant -> Edit -> Email -> Debug</strong>.
. Save the tenant.
. Send an email.
. View the Event Logs by navigating to <strong>System -> Event Logs</strong>.
