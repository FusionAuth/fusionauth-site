...
Create a new API key with permissions to modify the theme. You can do this by going to <Breadcrumb>Settings -> API Keys</Breadcrumb>. Then click the <InlineUIElement>Add</InlineUIElement> button and select <InlineFieldValue>GET</InlineFieldValue> and `PATCH` permissions for `/api/theme/`.
