...
To apply a theme to a Tenant, navigate to <Breadcrumb>Tenants -> Your Tenant</strong>, then select the <strong>General</Breadcrumb> tab. Select the appropriate theme and save the tenant. This will apply the theme to every application in that tenant, unless there is a theme specified for an application.
