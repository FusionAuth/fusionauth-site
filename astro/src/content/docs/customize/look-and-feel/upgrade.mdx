...
Version `1.99.9` does not *require* any theme changes. User passwords are optional in version `1.99.9` at the tenant level but each application can decide, by virtue of including or omitting a password field in its advanced registration form, whether to require a password for registering a user (new or existing) for an application. Basic registration always requires a password. If an existing user without a password registers for an application with basic registration, there is a new field, `passwordSet`, that can be used on the Complete Registration page to display the password field in the basic registration
completion process.
