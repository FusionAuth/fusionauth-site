...
To enable support for multiple identity provider configurations of these types on a single login page, you will need to make the following modifications. In some cases, such as rendering login buttons, the upgrade notes provide an example modification that needs to be applied to other identity provider types. The default theme is available on every FusionAuth instance as a point of comparison or guide to theme upgrades.
...
{/*  TODO : Link to Google IdP limitations. */}
Replace the `[#if]` block including Google's GSI client JavaScript and `Google.js`. Only one Google identity provider using the GSI client can be included on the page.
...
You read that correctly. If you'd like, update the name of the macro to `linkedInButton` along with the invocation from `alternativeLogins`. Replace the opening tag of the `<button>` element inside the macro with the following
