...
Starting in version `1.99.9`, user passwords are optional and phone identities are now supported. While theme updates are not required, there are some areas that you may want to consider making changes.
...
To improve the user experience for phone number identities, the `email` field should be replaced with a `loginId` field with will use the label `Login` instead of `Email` (see Messages above). Everything will continue to function even if this change is not made, but the user experience will be improved with the new field because of the more accurate label.
