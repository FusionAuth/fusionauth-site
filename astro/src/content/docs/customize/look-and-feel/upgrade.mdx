...
<fieldset>
  ...
  [#-- Previously, the showPasswordField was used by Managed Domains --]
...
<fieldset>
  ...
  [#-- Now we add an additional check to see if passwords are enabled at the tenant level --]
...
This will hide the password field if [Managed domains](/docs/lifecycle/authenticate-users/identity-providers/overview-oidc#managed-domains) is enabled for an identity provider in-use by the application, causing `showPasswordField` to be `false`, OR if [Password enabled](/docs/get-started/core-concepts/tenants#password) is set to `false`, due to the inclusion of `tenant.passwordEnabled` in the `if`.
