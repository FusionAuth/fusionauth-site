...
<APIField name="Messages" optional>
  Editing the "Default" locale shows a read-only view containing a list of all default messages, with comments and groups explaining their uses. Below the read-only field is a <InlineField>Custom messages</InlineField> field.
  
  To override the default messages with your own message values, edit the <InlineField>Custom messages</InlineField> field. For example, to change the page title for an OAuth2 login page, add the line `oauth2-authorize-page-title=<My Example Login Page Title>`.
