...
<APIField name="Messages" optional>
  Editing the "Default" locale will show a read-only view containing full list of the default messages. Messages are divided into groups with comments that explain what they are used for. Below the read-only field is a "Custom messages" field which can be edited to add your own message values which override the default messages. For example, to change the page title displayed in a browser tab for the main OAuth2 login page you could add the line `oauth2-authorize-page-title=My Website | Login` to the custom messages field.
