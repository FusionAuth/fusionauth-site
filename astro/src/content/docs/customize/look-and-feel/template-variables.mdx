...
<APIBlock>
  ...
  <APIField name="tenant" type="Tenant">
    ...
    Since version 1.99.9, <InlineField>tenant.passwordEnabled</InlineField> can be used on advanced themes with hosted login pages to hide the password field when the tenant does not have passwords enabled. See the [Upgrade Notes](/docs/customize/look-and-feel/upgrade) for more information.
