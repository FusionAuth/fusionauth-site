...
Create a new theme by navigating to <Breadcrumb>Customization -> Themes</Breadcrumb> and then click the <Breadcrumb>Add</Breadcrumb> button. Enter a <InlineField>Name</InlineField> for the theme and click the <Breadcrumb>Save</Breadcrumb> button.

![Create a theme](/img/docs/customize/look-and-feel/create-theme.png)

Create a new API key with permissions to modify the theme. You can do this by going to <Breadcrumb>Settings -> API Keys</Breadcrumb>. Then click the <Breadcrumb>Add</Breadcrumb> button and select <InlineFieldValue>GET</InlineFieldValue> and `PATCH` permissions for `/api/theme/`.
