...
tags: tailwind css, css framework, theme customization, login page styling, advanced theme, utility-first css, frontend customization
---
...
Create a new theme by navigating to <Breadcrumb>Customization -> Themes</Breadcrumb> and then click the <Breadcrumb>Add</Breadcrumb> button. Enter a **Name** for the theme and click the <Breadcrumb>Save</Breadcrumb> button.

![Create a theme](/img/docs/customize/look-and-feel/create-theme.png)

Create a new API key with permissions to modify the theme. You can do this by going to <Breadcrumb>Settings -> API Keys</Breadcrumb>. Then click the <Breadcrumb>Add</Breadcrumb> button and select `GET` and `PATCH` permissions for `/api/theme/`.
...
To preview the changes, open FusionAuth, navigate to <Breadcrumb>Customization -> Themes</Breadcrumb>. Choose your theme, then click <Breadcrumb>Preview</Breadcrumb>.
...
In this repository, you can change the color scheme by updating the `tailwind.config.js` file:
