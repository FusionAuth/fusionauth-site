...
**OPTIONAL - ONLY NEEDED FOR CUSTOM THEMES THAT OVERRIDE AN AFFECTED TEMPLATE AND USE CUSTOM FORM FIELDS**

Version `1.69.0` updates the default theme templates to correctly display labels for [custom form fields](/docs/lifecycle/register-users/advanced-registration-forms#custom-form-fields). If your advanced theme includes customized versions of one or more of these templates (`Account edit`, `OAuth2 register`, or `OAuth2 complete registration`) and uses custom form fields, update them as described below.
