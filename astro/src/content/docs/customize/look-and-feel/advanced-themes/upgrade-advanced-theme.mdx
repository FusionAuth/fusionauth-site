---
title: Upgrade an Advanced Theme
description: An overview of how to upgrade a custom theme from one version of FusionAuth to another.
order: 1
---
...
<Aside type="note">
For a full list of required changes for a specific version, see [Advanced Theme Upgrade Notes](//docs/customize/look-and-feel/advanced-themes/upgrade).
</Aside>
...
FusionAuth warns you about missing required keys in order to avoid an inadvertent bad user experience. The default display for keys with no valid values in theme <InlineField>Messages</InlineField> is the key text, such as <InlineFieldValue>[ExternalAuthenticationException]LinkedInToken</InlineFieldValue>, which can be confusing for end users.
