...
- 🔐 Secure. When using FusionAuth, this is the easiest part. The hosted login pages are the result of a company dedicated to a security product and are secure by default. The tricky part is deciding whether to support login with email addresses, usernames, or mobile phones. And whether to use passkeys, single sign-on (SSO) or social login, and two-factor authentication. Each of these choices has security and ease-of-use implications. You need to decide what is best for the majority of your users. Passwords should follow modern best practices, like [zxcvbn](https://github.com/zxcvbn-ts/zxcvbn), and not superstition about uppercase and special characters. Ensure only humans attempt to register and log in using CAPTCHA, and prevent brute-force password attacks by rate limiting login attempts.
...
- upgrade FA frequently to get all security patches in case vulnerabilities are discovered
- autofocus the identifier field
- must label your textbox type=email in HTML to get appropriate mobile keyboard layout for fast entry. Note: also applies to telephone numbers (type=tel), URLs (type=url), and numbers (type=num) in your signup flow
- Validate fields on blur not on submit
- Make labels clickable and focus related textbox
- Show password requirements when users are choosing a password
- Let users see their password on icon click (instead of retyping it)
- SSO is less effort. Can be confusing if users forget which service they signed up with.
- Link between sign in and sign up
- Remember all entered values on page refresh if form submission fails
- Reset password page should autofill your email from the sign in page
- Allow all legal email addresses (`+` and `/` are legal characters, as are two-letter domain names).
- Give clear complete instructions for inexperienced users. "Oh, I'm supposed to invent my own password? Why didn't you say so."
- Show if capslock is on
- Remember me on this computer (cafe or campus computers)
- Skip the login screen if logged in already. Show a log out button clearly.
- autocomplete="current-password"   autocomplete="username
- <label for="email">Email address</label><input id="email" ...>.
- Don't disable paste
- Show a spinner for feedback, not dead air on a page when loading
- ARIA live region (e.g., a <div role="alert">Incorrect password</div>
- If your login screen uses icons (like the little envelope symbol in the email field), ensure they have aria-hidden="true" if purely decorative, or proper alt text if they convey meaning.
- Make timeouts long
-
