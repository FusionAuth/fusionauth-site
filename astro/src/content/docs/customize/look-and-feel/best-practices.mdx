...
Below is a condensed checklist of all the tips in the previous section. The themes later in this guide refer back to the headings, so you can see exactly what is implemented.

[Link to Separate Pages](#sp)

Name | Description | Done in Simple Theme | Done in Advanced Theme
---|---|---|---
...
<span id='p22'>VPN safe<span> | | |
<span id='p23'>Backup host<span> | | |
...
In the next section you'll see what can be accomplished using only the simple theme editor. After that, you'll see the extra features available when creating an advanced theme. This won't look at custom pages outside of FA, but if you want to create those, you can still follow all the best practices from the previous section.

## Create A Custom Simple Theme

To create a custom simple theme, first duplicate the simple theme template in <Breadcrumb>Customizations -> Themes</Breadcrumb> then choose that theme in the dropdown of the <InlineUIElement>General</InlineUIElement> tab when editing your application.



run express quickstart with latest fa
show custom theme, registration, log in, forgot password
see which features are applied / need changes - in a table
say to check which are free with the table in hte last section
