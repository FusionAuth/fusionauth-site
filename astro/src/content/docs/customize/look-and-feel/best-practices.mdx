...
- [Create A Custom Simple Theme](#create-a-custom-simple-theme)
- [Best Practice Support In The Default Theme](#best-practice-support-in-the-default-theme)
- [Create A Custom Advanced Theme](#create-a-custom-advanced-theme)
- [How To Implement Best Practices In FA](#how-to-implement-best-practices-in-fa)
...
<span id='p5'>Clickable labels</span> | | ❎ Fields are not labelled. Clicking their icons does not focus the field. | |
<span id='p6'>Viewable password</span> | | ❎ Password is not viewable. There is also no confirmation textbox. | |
...
<span id='p12'>Autofilled reset password</span> | | | |
...
<span id='p45'>Custom password rules</span> | | | |
...
<span id='p111'>Consent checkboxes<span> | | | ❎ |
...
In the next section you'll see what can be accomplished using only the simple theme editor. After that, you'll see the extra features available when creating an advanced theme. This guide won't look at custom pages outside of FA, but if you want to create those, you can still follow all the best practices from the previous section.

## Create A Custom Simple Theme

To create a custom simple theme, first duplicate the simple theme template in <Breadcrumb>Customizations -> Themes</Breadcrumb>, then choose that theme in the dropdown of the <InlineUIElement>General</InlineUIElement> tab when editing your application. You can edit the theme in <Breadcrumb>Customizations</Breadcrumb>.

## Best Practice Support In The Default Theme

Before customizing the theme, let's see how many best practices the default FA theme follows. You can see screenshots of the default theme below. The practices it supports are checked in the third column of the [summary table](#summary-table-of-best-practices).
...
Below are listed only the items where FA fails. All other best practices are correctly followed.
