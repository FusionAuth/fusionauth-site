...
- [Create A Custom Advanced Theme](#create-a-custom-advanced-theme)
  - [Best Practices That Are Already Followed](#best-practices-that-are-already-followed)
  - [Best Practices That Are Impossible In FusionAuth](#best-practices-that-are-impossible-in-fusionauth)
  - [Best Practices To Add To The Theme](#best-practices-to-add-to-the-theme)
...
Before making changes to the existing, let's look at what practices it already follows.
...
### Best Practices That Are Impossible In FusionAuth

Below are the practices that aren't possible to implement in FA.

- Hide registered accounts. For privacy, an attacker should never be told if an email for a service is in use. This is not possible in FA and has been a [complaint since 2023](https://fusionauth.io/community/forum/topic/2484/registering-with-an-existing-email).
  ![Email exists](../../../../../public/img/docs/customize/look-and-feel/best-practices/emailExists.webp)

### Best Practices To Add To The Theme

- Minimal. At first, the page might look minimal, but interactions show it lacks attention to detail and hasn't been tested. For example, if your password at login is incorrect, the error is shown in two places and the toast moves the entire page, instead of floating above it. This clutter can be removed.
...
- Clickable labels. The username and password fields in this theme don't have labels. Instead, they use the hint text when the fields are blank. This is good for minimalism, but not good for clarity, because the hint disappears when the user starts typing. If your users are technically experienced, this is fine, but if you're aiming for maximum clarity, you need to add field labels.
- Viewable password. In this theme there is neither a password confirmation box nor a way to view the password you entered. You need at least one, especially when on mobile where it's easy to mistype.
- Show password requirements. This instruction has to be added manually to the page. Be careful doing this, because an admin might change the requirements in the FA backend, but forget to update the text in the theme frontend.
- Validate on blur. Currently, validation happens only at form submission.
- Autofilled reset password username
- Colorblind safe. You can use a website like [Pilestone](https://pilestone.com/pages/color-blindness-simulator) to see how an image appears with various forms of colorblindess.
...
- Blind safe. Testing full blindness and using a screenreader, is more complicated than testing color blindness. Screen readers are configured at the operating system level and differ between Linux, MacOS, and Windows. For a partial test, you can use the [Silktide Accessibility Checker](https://chromewebstore.google.com/detail/silktide-accessibility-ch/mpobacholfblmnpnfbiomjkecoojakah). The free Silktide plugin offers accessibility checking even more powerful than Firefox, automatically checking a variety of problems.
