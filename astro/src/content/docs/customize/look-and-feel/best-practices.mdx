...
<span id='p53'>Cookie consent banner</span> | | ❎ | |
<span id='p54'>Disable cookies if no consent</span> | | ❎ | |
...
- Consent and cookies ([1](#p53), [2](#p54)). FA does not ask for permission to store cookies in the user's browser, does not inform the user, and provides no way to remove cookies if the user could decline them.
-
