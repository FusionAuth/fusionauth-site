...
tags: uuid generator, unique identifier, random uuid, developer tools, online tool, guid generator
faIcon: fa-id-badge
color: red
---
...
<div className="flex gap-x-6 items-stretch mt-8 no-prose">
  <div class:list={[border(false, false, true), "flex items-center justify-between py-1 ps-2 pe-1"]}>
    ...
    <CopyToClipboard source="uuid-value" />
  </div>
...
In FusionAuth, UUIDs are used to identify [users](/docs/get-started/core-concepts/types/users), [applications](/docs/get-started/core-concepts/types/applications), and other objects such as [roles](/docs/get-started/core-concepts/types/roles) and [groups](/docs/get-started/core-concepts/types/groups). For example, when a new user or role is created, a UUID is generated and assigned to that user or role. This UUID can then be used to uniquely identify the user within the system, grant them appropriate resources, and it is used as a primary key in the database to store and retrieve user information. For almost every resource, FusionAuth can either generate a UUID on creation or use one supplied to the API, allowing for easier migrations of existing users, roles and other entities.
