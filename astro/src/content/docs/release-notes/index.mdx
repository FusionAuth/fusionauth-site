...
* <Issue issue="2949">
    When completing a password change, either from the forgot password flow or if a password change is required on the user, once the user completes an MFA challenge during the password change, if the user chooses to trust their device for 30 days, that trust will now carry forward to the user's next login. MFA trusts for other devices, besides the device that completed the password change, will continue to be removed. NOTE, for this to work properly for API users, the API client must capture `twoFactorTrustId` from `/api/two-factor/login` API responses and provide that value in login API requests as `twoFactorTrustId`.
  </Issue>
