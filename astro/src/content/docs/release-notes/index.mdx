...
### New
* <Issue issue="1841">
    Ability to create multiple Google identity providers with different reconcile lambdas
  </Issue>

### Fixed

* <Issue issue="3111">
    The SCIM Groups filter for `displayName` now does a correct case-insensitive equals comparison for the `eq` operator. Additionally, the `displayName` filter now supports the `co`, `ew`, and `sw` operators.
  </Issue>
* <Issue issue="2949">
    When completing a password change that required a step-up MFA challenge, that trust will now carry forward to the user's next login if the user chose to trust their device for 30 days. MFA trusts for _other_ devices will continue to be removed. API integrations must capture `twoFactorTrustId` from the [Complete Multi-Factor Authentication API](/docs/apis/login#complete-multi-factor-authentication) response and provide that value on login API requests as `twoFactorTrustId`.
  </Issue>
* <Issue issue="3140">
    Google Sign In Race Condition
  </Issue>
* <Issue issue="3150">
    `IndexUser` had a new field added, but the doc annotation was missing the version
  </Issue>
* <Issue issue="3167">
    Elasticsearch nested query for registration fails when second registration added
  </Issue>
* <Issue issue="3179">
    `/api/user/change-password` GET was not documented and did not support `loginIdTypes` correctly. Clients do not support GET by `loginId` correctly
  </Issue>
* <Issue issue="3186">
...
* <Issue issue="3208">
    Redundant log messages when starting FusionAuth
  </Issue>
* <Issue issue="3227">
    Error with Connector and Require MFA / Stronger Password Policy
  </Issue>
* <Issue issue="3225">
    Javascript error noise occurs on the maintenance mode page and /admin/application -> Manage scopes
  </Issue>
