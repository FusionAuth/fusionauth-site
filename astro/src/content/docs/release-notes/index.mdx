...
* Currently, FusionAuth's Webhook Event Log does not set a retention policy by default and may grow large in volume which can result in an impact to performance when searching Webhook Event Logs. See issue for workaround. Additionally, users upgrading from version `1.53.0` of FusionAuth may experience longer upgrade durations to account for migrations performed on larger Webhook Event Log tables.
