...
### New
* A Webhook Event Log has been added. It contains a record of triggered events and attempts to deliver them to configured receivers.
...
### Fixed
* Separate post-authentication verification steps from SSO functionality. This allows disabling SSO by setting the Tenant's session timeout to `0` without breaking FusionAuth's SSO functionality on hosted login pages.
