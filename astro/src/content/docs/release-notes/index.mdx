...
<ReleaseNoteHeading version="1.48.2" releaseDate="November 21th, 2023" />

### Fixed
* In version `1.48.1` the ability to page past 10,000 results on the Users and Entities search pages when using the Elasticsearch engine was added. See [Github Issue #494](https://github.com/FusionAuth/fusionauth-issues/issues/494). There are two sets of pagination controls on these pages: one above and one below the results. If you paged past 10,000 results and clicked the "Next" button on the bottom controls there would be an error and the page would not advance. 
  * Resolves [GitHub Issue #2544](https://github.com/FusionAuth/fusionauth-issues/issues/2544)
* In some cases when interacting with FusionAuth-hosted pages in an non-secure context (such as local development) it was possible to receive an error in the browser PublicKeyCredential is not defined. This may have disrupted some flows such as passwordless login. This has been fixed.
  * Resolves [GitHub Issue #2500](https://github.com/FusionAuth/fusionauth-issues/issues/2500)
* In version `1.48.0` a change was made to reject a link request from an IdP when and `email_verified` was supplied with a value of `false`. This rejection was not happening in the case that the email claim was returned in a Userinfo response, while the email_verified claim was present in the ID token. Retrieving the email and email_verified claims from different sources is now supported.
  * Resolves [GitHub Issue #2542](https://github.com/FusionAuth/fusionauth-issues/issues/2542)

### Security
* A issue was identified in the handling of logout redirect URIs
