...
* <Issue issue="3177">
  ...
  **OAuth Logout Endpoint and the id_token_hint**
  ...
  Previously, calls to the OAuth logout endpoint allowed an access token in conjunction with the `id_token_hint` (this was undocumented).  Now only the id token will be allowed in conjunction with the `id_token_hint` parameter.
