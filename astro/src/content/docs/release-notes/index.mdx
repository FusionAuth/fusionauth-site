...
<Aside type="caution">
    If upgrading from 1.60.0 and the workaround noted on [GitHub Issue #3183](https://github.com/FusionAuth/fusionauth-issues/issues/3183) was used, the workaround should be removed before applying this upgrade.
</Aside>

* <Issue issue="3183">
    When using FusionAuth as a SAML IDP with a SAML Populate lambda, lambdas that update the SAML assertion fail.

    SAML Populate Lambdas should now work correctly.
   </Issue>
...
* <IssueResolvedVia resolvedIn="1.60.1" viaIssue="3183">
    When using FusionAuth as a SAML IDP with a SAML Populate lambda, lambdas that update the SAML assertion fail. See issue for workaround.
  </IssueResolvedVia>
