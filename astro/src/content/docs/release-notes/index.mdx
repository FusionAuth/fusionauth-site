...
<GeneralMigrationWarning>
  ...
  If you are currently using a version of FusionAuth earlier than 1.53.0, we recommend upgrading to version 1.57.0 and not an intermediate version.
</GeneralMigrationWarning>
...
* <RelatedToIssue issue="3037">
    The process for applying webhook event log retention rules, and cleaning out old webhook event logs, was formerly turned off by default, causing all webhook event log entries to be retained. This will now be defaulted to `on` in new FusionAuth installations. Existing installations that are being upgraded will retain their former setting.
  </RelatedToIssue>

### Fixed
* <Issue issue="3026">
    The TypeScript client is missing support for patching the `Entity.data` field. Support for this endpoint has been added.
  </Issue>

* <Issue issue="3027">
    Some client libraries are missing support for certain CRUDP operations. This fix adds support for these, and also adds additional automated checks to verify that client libraries have full CRUDP support domain objects in their associated APIs.
  </Issue>
...
* <Issue issue="3037">
    ...
    * Webhook event logging can now be turned off. Previously, you were only able to reduce the retention period to a very small window, however in this case the logging and log management would still occur.
    * Webhook event log retention is now enabled by default. Previously, it was disabled by default.
  </Issue>

* <Issue issue="3038">
    Webhook event log searching was improved by setting a default 1-hour search window when an unbounded search is made. Webhook event logging can create a large number of log records, and the performance of unbounded searches scaled roughly linearly with the number of log records, leading to less performant searches as the data size increased.
  </Issue>
