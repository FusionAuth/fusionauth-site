...
* <RelatedToIssue issue="3008">Currently, FusionAuth's Webhook Event Log does not set a retention policy by default and may grow too large in volume which can result in an impact to performance when searching Webhook Event Logs. Please refer to the Github issue for a workaround. Additionally, if you are upgrading from version `1.53.0` of FusionAuth you may experience longer upgrade times due to the migration on a Webhook Event Log table if there are a significant number of entries.</RelatedToIssue>
