...
### Changed
* The Nashorn JavaScript has been removed from FusionAuth. All Lambda functions will now use the GraalJS engine. No action is required, but please note that if you had any Lambda functions still using the Nashorn engine they will now execute using the GraalJS engine.
  * Resolves [GitHub Issue #1828](https://github.com/FusionAuth/fusionauth-issues/issues/1828)
* In previous versions of FusionAuth, if a new themed page was added that you were missing in your theme, the end user may be shown a page indicate the page was missing. This was shown because it was assumed that a new page would only be shown for a new feature that had not been enabled, and this page would only ever been seen during development. In this release we are adding a new theme page that may be shown w/out any additional features being enabled. For this reason, we have removed this page, and we will always fall back to the default theme in this scenario. You will still want to upgrade your theme as part of your upgrade process, but this change will ensure that we will not break any new or existing workflows when a new page is added.
...
* The LinkedIn APIs have changed, and the LinkedIn IdP no longer worked for new LinkedIn applications. This update should allow FusionAuth to continue to work with new and legacy LinkedIn applications.
...
* When viewing a User's Consents in the FusionAuth admin UI, if one or more of the consents is has been granted by another user that is not a member of their family, an error is shown in the `Given by` column.
  * Resolves [GitHub Issue #2639](https://github.com/FusionAuth/fusionauth-issues/issues/2639)
* When you configured to sign JWTs using the `ES512` algorithm, the generated signature may be intermittently invalid. This means that JWTs may seemingly fail to validate randomly and you may think you are crazy. If you are using this signing algorithm, it is recommended you use a different algorithm until you are able to upgrade.
...
### Enhancements
* Link checkers are great. They aim to protect end users from malicious links and phishing attacks. However, they wreak havoc and pain on identity providers using email based workflows to complete passwordless login, or email verification. And FusionAuth is one of those identify providers!

  FusionAuth has employed various tactics over the years to stay ahead of the techniques in use by these 3rd party tools. Their techniques continue to evolve making it difficult or impossible to know the difference between a link checker and a real human interacting with the link.

  A new confirmation page has been added that is intended to protect our users, and make our email workflows immune.

  For example, when a user initiates a request such as passwordless login, and then completes the request in the same browser, the user will not observe any change. If the user completes the request on a different browser, or simply completes a request they did not initiate - such as clicking on an email verification link sent when a use is provisioned by an administrator, the user will be prompted to confirm they wish to complete the request.
