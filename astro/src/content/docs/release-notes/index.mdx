...
<ReleaseNoteHeading version='1.56.0' releaseDate='December 21, 2024' name='Holiday Hippo'/>

### Fixed
2252 FusionAuth returns a 500 when supplying a malformed application Id in an IdP-initiated SAML login, making it hard to troubleshoot. The error should be handled more gracefully, and a more meaningful error message should be returned.
2721 @JCelentano The Application / Multi-Factor / SML Template tooltip is incorrectly referencing an email template instead of an SMS template. Update this to reference the correct template type.
2814 The passwordless API returns a 500 error when a non-esistent application Id is provided. This should return a correct response code and meaningful error.
2826 When posting an invalid grant type to the `/oauth2/token` endpoint, the list of supported grant types in the error message is missing the `device` grant type
2838 Users are being prompted to re-submit a form when first logging into a new deployment using Firefox. The request should proceed on the first attempt.
2872 When creating an entity grant for a user using the API, some invalid payloads will yield a 500 error. A more meaningful error should be returned.
2892 Executing a manual system reset in development mode with a valid kickstart file fails. This should work as expected.
2913 Supplying a malformed license key to Reactor is producing a generic error. A specific error would be more helpful.
2931 The Daily Active Users report is missing data from the most recent day. The report should include this data.
2941 After setting an application-level email verification template, the tenant-level template is still being used. The application-level setting should be honored.
2953 The standard first name field intended for use in user edit forms has an incorrect name in MySQL installations, which is preventing it from showing up in the user edit form.
2962 The webhook event log can return fewer records per page than requested with the _Results per page_ dropdown, even when more results are available. Ensure that the number of results equals the requested number.
2970 A nondescript error is returned when trying to save system settings via the FusionAuth admin application.
2990 The simple theme editor's right-hand panel isn't constraining its content properly, allowing content to spill over the bottom border.

### Enhancements
2975 Allow the post-login bootstrapping of a FusionAuth SSO session using an access token

### Internal
2963 Remove duplicated storage of internal messages relating to SCIM group operations
