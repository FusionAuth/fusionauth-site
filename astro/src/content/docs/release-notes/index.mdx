...
### New
* The Webhook Event Log! The Webhook Event Log will contain a record of each triggered event and the corresponding attempts to deliver the event to each configured webhook. This will be useful to monitor events that have succeeded or failed to be received by your configured webhooks. The attempt log will provide you timing, the status code returned by your webhook in addition to other meta-data.
...
### Enhancements
* In the FusionAuth admin UI, long lists of icon buttons have been replaced with Action dropdowns. The number of buttons on some pages grew to the point that it was becoming difficult to differentiate between the buttons, and was also visually cluttering up the view. We hope you like it!
  * Resolves [GitHub Issue #2810](https://github.com/FusionAuth/fusionauth-issues/issues/2810)

### Internal
* Java 21 LTS. Update to the latest long term support version of Java.
...
### Known Issues
* A bug was introduced that can prevent login when an SSO session timeout value of 0 is used.
