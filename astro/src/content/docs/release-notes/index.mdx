...
* <Issue issue="3177">
  Added additional validation to ensure the provided FusionAuth JWT is suitable for the intended workflow.
  </Issue>

### Changed

* <Issue issue="3177">
  Some changes have been made to how FusionAuth produces JWTs, and how JWTs are validated for various APIs.

  Please read this in case these changes could affect your integration.
  ...
  * The `use` claim has been moved from the JWT header to the body.
  ...
  The following claims are now reserved. We do not expect this to impact any of our customers. But if you are adding these claims or modifying them in a JWT populate lambda function, you will need to plan to modify your integration.
  - `auth_time` - Reserved for all tokens
  - `gty` - Reserved for all tokens
  - `tty` - Reserved for all tokens
  - `use` - Reserved for the client credentials grant

  Note that these claims are not considered reserved for the Vend API.

  Due to some additional validation, it is possible that an existing access token or id token that has not yet expired will no longer be considered valid if you send it to FusionAuth. When utilizing a short lived token with a refresh token, we do not expect this to cause any significant issues. These changes will not hav any affect on your use of the JWTs produced by FusionAuth.
...
* <Issue issue="2887">
    A user can be inadvertently shown an error when a tenant is using Advanced Threat Detection to send rate limit emails for two-factor authentication as the rate limit is exceeded by the user's action.

    The user will now be shown the correct message when this condition is met.
  </Issue>

* <Issue issue="2936">
    When a user completes a forgot password workflow, the failed login count will be reset.

    This should reduce frustration for a user that changed their password after exceeding the configured failed login count. In this scenario, if the user enters the incorrect password again they will be required to wait for the configured time period before attempting login again.
  </Issue>

* <Issue issue="2966">
    When running in dark mode, or using a dark theme, some QR code scanners are unable to read the QR code for setting up MFA. The QR code should have a high-contrast border to allow these readers to work.
  </Issue>

* <Issue issue="3105">
    In rare cases, when FusionAuth is using an air-gapped license and running in a clustered setup, restoration of outbound network connectivity can cause a standard (not air-gapped) license to be incorrectly installed.
  </Issue>

* <Issue issue="3148">
    When previewing the `Phone verification required`, `Complete registration`, and `Passwordless` theme templates, a FreeMarker error is shown. This should be handled more gracefully.
  </Issue>

* <Issue issue="3151">
    A race condition exists, by which FusionAuth can occasionally fail to properly persist an air-gapped license, reverting back to an unlicensed state after some period of time.
  </Issue>

* <Issue issue="3160">
    When regenerating Reactor's encryption key, the _Breached Password Detection_ status may take some time to return to _Active_.
  </Issue>
...
* <Issue issue="3177">
   ...
   * Adding `tty` claim to all JWTs produced to easily identify an Id token from an Access token.
   * Moving the `gty` claim from the JWT header to the body for JWTs produced by an OAuth grant.
   * Moving the `use` claim from the JWT header to the body for JWTs produced by the Client credentials grant for a SCIM server.
...
* <Issue issue="3032">
    Performance of bulk user import using the User API was improved. If you're migrating a bunch of users in, this should go a lot faster now.
  </Issue>

* <Issue issue="3154">
    Our release process now auto-updates the documentation that describes the usage stats that are collected from FusionAuth instances. This documentation will remain up to date going forward.
  </Issue>

* <Issue issue="3159">
    Version `1.59.0` made user passwords optional. A side effect of this was that when returning from a federated login, a user could still be presented with a password field on a registration completion form. This has been improved to align better with user expectations.
  </Issue>

* <Issue issue="3165">
    The password reset workflow received some error handling improvements.
  </Issue>

* <Issue issue="3171">
    Maintenance Mode error messaging was improved. Should an error occur in maintenance mode. you'll see more descriptive errors now, which should help with troubleshooting the problem.
  </Issue>

* <Issue issue="3172">
    FusionAuth now supports SAML Authn responses with multiple assertions. Previously, FusionAuth would only provide a single assertion to a SAML reconcile lambda. Now the full assertion payload is available for inspection.
  </Issue>
...
### Known Issues
* <IssueResolvedVia resolvedIn="1.59.1" viaIssue="3153">An upgrade to FusionAuth version `1.58.2` or `1.59.0` fails if the `Default Admin Registration provided by FusionAuth` form (shipped in `1.20.0`) is renamed or deleted prior to the upgrade. If you are planning an upgrade to version `1.59.0`, it is recommended that you upgrade to version `1.59.1`. If upgrading to `1.59.1` does not resolve this, please contact support.</IssueResolvedVia>
...
<ReleaseNoteHeading version='1.58.3' releaseDate='August 20, 2025' />
...
### Known Issues
* <IssueResolvedVia resolvedIn="1.58.3" viaIssue="3153">An upgrade to FusionAuth version `1.58.2` or `1.59.0` fails if the `Default Admin Registration provided by FusionAuth` form (shipped in `1.20.0`) is renamed or deleted prior to the upgrade. If you are planning an upgrade to version `1.58.2`, it is recommended that you upgrade to version `1.58.3`. If upgrading to `1.58.3` does not resolve this, please contact support.</IssueResolvedVia>
...
### Known Issues
* <IssueResolvedVia resolvedIn="1.58.3" viaIssue="3153">An upgrade to FusionAuth version `1.58.2` or `1.59.0` fails if the `Default Admin Registration provided by FusionAuth` form (shipped in `1.20.0`) is renamed or deleted prior to the upgrade. If you are planning an upgrade to version `1.58.1`, it is recommended that you upgrade to version `1.58.3`. If upgrading to `1.58.3` does not resolve this, please contact support.</IssueResolvedVia>
...
* <IssueResolvedVia resolvedIn="1.58.3" viaIssue="3153">An upgrade to FusionAuth version `1.58.2` or `1.59.0` fails if the `Default Admin Registration provided by FusionAuth` form (shipped in `1.20.0`) is renamed or deleted prior to the upgrade. If you are planning an upgrade to version `1.58.0`, it is recommended that you upgrade to version `1.58.3`. If upgrading to `1.58.3` does not resolve this, please contact support.</IssueResolvedVia>
...
* When the <InlineUIElement>Browser preview</InlineUIElement> button was used to open a new tab for simple themes in the admin UI the page would render without any applied CSS when using the Firefox browser.  Sorry Firefox users, we ask for your forgiveness. 😔
