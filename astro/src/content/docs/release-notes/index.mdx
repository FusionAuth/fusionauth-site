...
<GeneralMigrationWarning >
  This is a patch release that addresses some additional items after the 1.50.0 release. Please read the 1.50.0 notices for some important information about changes made in 1.50.0.
</GeneralMigrationWarning>

### Fixed
* FusionAuth added a First Time Setup wizard in 1.50.0. After logging into the admin application, a first time setup panel is shown in the dashboard, and a new left-hand nav item is available for it. These assets are being show after upgrades, when the intent was to only show them for new installations that haven't been configured yet. This is now only being shown for unconfigured FusionAuth instances.
  * Resolves [GitHub Issue #2729](https://github.com/FusionAuth/fusionauth-issues/issues/2729)
* In the First Time Setup summary page, FusionAuth shows sample configuration for various [quickstarts](https://fusionauth.io/docs/quickstarts/). The configuration for the [React quickstart](https://fusionauth.io/docs/quickstarts/quickstart-javascript-react-web) corresponds to a previous version of the quickstart and is incompatible with the current version. The React quickstart configuration is now formatted for the current quickstart version.
  * Resolves [GitHub Issue #2730](https://github.com/FusionAuth/fusionauth-issues/issues/2730).

<ReleaseNoteHeading version='1.50.0' releaseDate='April 24, 2024' />
