...
<ReleaseNoteHeading version='1.53.0' releaseDate='September 5, 2024' />

### Changed
* In the FusionAuth admin UI, long lists of icon buttons have been replaced with Action dropdowns. The number of buttons on some pages grew to the point that it was becoming difficult to differentiate between the buttons, and was also visually cluttering up the view.
...
### New
* A Webhook Event Log has been added. It contains a record of triggered events and the attempts to deliver them to configured receivers, along with the event metadata, webhook payload, and attempt details.
...
### Fixed
* FusionAuth `1.49.1` introduced a bug that could cause users to enter an perpetual login loop when SSO was disabled for a tenant by setting the tenant's SSO session timeout `0`. A session timeout of `0` is properly handled now.
  * Resolves [GitHub Issue #2736](https://github.com/FusionAuth/fusionauth-issues/issues/2736)
* A bug was introduced in version `1.52.0` that caused dates used in advanced searches to be set to incorrect times on subsequent searches in the same search form. Dates in advanced searches now remain unchanged across multiple searches.
  * Resolves [GitHub Issue #2843](https://github.com/FusionAuth/fusionauth-issues/issues/2843). Thanks to [@runely](https://github.com/runely) for reporting this!

### Internal
* FusionAuth now officially supports Java 21, the latest LTS Java version, released in September of 2023.
  * Resolves [GitHub Issue #2473](https://github.com/FusionAuth/fusionauth-issues/issues/2473)
* FusionAuth's database connectivity was improved by the introduction of additional internal connection pools for things like background tasks and other non-interactive tasks. This improves the performance, scalability, and the general robustness of FusionAuth.
...
### Known Issues
* A bug was introduced that incorrectly set start and end times in advanced searches. A patch was released in FusionAuth `1.53.0`.
...
### New
* A free community license is now available, which adds WebAuthn (Passkeys) to the community edition. All those with a Community license will now find a license key in their [FusionAuth account plan](https://account.fusionauth.io/account/plan) page. And there was much rejoicing! 🥳
...
### Known Issues
* A bug was introduced that can prevent login when an SSO session timeout value of 0 is used. A patch was released in FusionAuth 1.53.0.
