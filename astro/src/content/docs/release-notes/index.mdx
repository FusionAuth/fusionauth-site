...
<ReleaseNoteHeading version='1.52.0' releaseDate='August xx, 2024' />
...
<DeprecationWarning>
  ...
  This field has never been populated on API responses, but it is included in FusionAuth's client libraries. Client library users should remove references to this field from their integration prior to its removal.

  Removal of this field is targeted for the end of 2024.
</DeprecationWarning>

<DeprecationWarning>
We are deprecating support for docker images for the Linux/ARM/v7, Linux/ppc64le, and Linux/s390x architectures.

We will stop publishing docker images for these architectures at the end of 2024.
</DeprecationWarning>

### Changed
* An unused theme template was removed from the default theme template set. There is no impact other than to make custom advanced themes slightly smaller in size.
  * Resolves [GitHub Issue #2818](https://github.com/FusionAuth/fusionauth-issues/issues/2818)

### Fixed
* Certain elements in the admin UI would show strange behavior when being clicked many times rapidly. These clicks are now being properly de-bounced.
  * Resolves [GitHub Issue #2718](https://github.com/FusionAuth/fusionauth-issues/issues/2718)

* Attempting to sort API keys by key value in Key Master by clicking the key value header would result in an error. This now performs the expected behavior of sorting the list of keys by key value.
  * Resolves [GitHub Issue #2738](https://github.com/FusionAuth/fusionauth-issues/issues/2738)

* An API call for creating a tenant-scoped API key would fail with an HTTP 500 error when attempting to specify a nonexistent tenant. This now returns a meaningful validation error.
  * Resolves [GitHub Issue #2749](https://github.com/FusionAuth/fusionauth-issues/issues/2749)

* In some situations, device trust was not being automatically revoked for all users on a device, relying on an admin to perform the revocation via the admin UI. These are now automatically revoked.
  * Resolves [GitHub Issue #2753](https://github.com/FusionAuth/fusionauth-issues/issues/2753)

* The date picker component used in simple themes for things like birthdates was not picking up simple theme styling. As a fix for this, we are now using the browser-default date picker. This date picker will be used in date fields in simple and advanced theme forms, and in search forms in the admin UI that allow filtering on dates. Existing advanced themes will continue to use the old date picker.
  * Resolves [GitHub Issue #2770](https://github.com/FusionAuth/fusionauth-issues/issues/2770)

* Saving simple themes resulted in an error. A fix was provided in FusionAuth Cloud for all cloud-hosted customers. This provides a fix for self-hosted instances of FusionAuth.
  * Resolves [GitHub Issue #2777](https://github.com/FusionAuth/fusionauth-issues/issues/2777)

* Custom message keys (message keys not part of a theme's default messages) were not being saved in simple themes. Custom messages are now properly saved. Additionally, the theme editor now has a separate area for all message overrides and custom messages, making it easier to manage message customization.
  * Resolves [GitHub Issue #2778](https://github.com/FusionAuth/fusionauth-issues/issues/2778)

* Equals signs in query parameter values were not being parsed correctly, such as when appearing as padding at the end of base64-encoded values. Equals signs embedded in query parameter values are now handled properly.
  * Resolves [GitHub Issue #2792](https://github.com/FusionAuth/fusionauth-issues/issues/2792)

* The preview window for simple themes was showing unstyled pages in Firefox. Simple theme previews in Firefox now render as expected.
  * Resolves [GitHub Issue #2794](https://github.com/FusionAuth/fusionauth-issues/issues/2794)

* The default `orderBy` parameter value for the [Group Member Search API](/docs/apis/groups#search-for-group-members) did not provide a consistent ordering of results, leading to issues with SCIM API responses. The updated default ensures the same order of results for each API call.
  * Resolves [GitHub Issue #2798](https://github.com/FusionAuth/fusionauth-issues/issues/2798)

* In the simple themes editor, color picker popovers would sometimes render too far from their respective buttons. Color pickers now always render adjacent to their buttons.
  * Resolves [GitHub Issue #2803](https://github.com/FusionAuth/fusionauth-issues/issues/2803)

* A new health check endpoint was added in `1.51.1`, but the domain ojbect to represent its response did not make it into the FusionAuth client libraries. This adds the response object to our client libs.
  * Resolves [GitHub Issue #2804](https://github.com/FusionAuth/fusionauth-issues/issues/2804)

* Newlines and tabs were not being rendered when viewing audit entries in the view dialog. These whitespace characters are now being correctly rendered.
  * Resolves [GitHub Issue #2808](https://github.com/FusionAuth/fusionauth-issues/issues/2808)

* In interactive maintenance mode, a situation existed where the progress bar would not be correctly shown as completed after a database update was actually finished, requiring a user to click the Submit button a second time to proceed. The progress bar now always reflects completion correctly.
  * Resolves [GitHub Issue #2815](https://github.com/FusionAuth/fusionauth-issues/issues/2815)


### Enhancements
* Support for passkeys is now available in all editions of FusionAuth, including the free Community edition! Passkeys provide better security than passwords, are more resistant to phishing and other attacks, and are a great option to offer your users.
  * Resolves [GitHub Issue #2663](https://github.com/FusionAuth/fusionauth-issues/issues/2663)

### New
* Version `1.52.0` introduces support for licensed Community editions of FusionAuth. After upgrading, those with a Community plan can now get even more out of FusionAuth with a free license. Log in to your account to access [your license](https://account.fusionauth.io/account/plan).
