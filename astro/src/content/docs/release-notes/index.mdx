...
<ThemeUpdateWarning>
  The new consent prompt themed page requires the `scopeConsentField` macro and `resolveScopeMessaging` function to be defined in the *Helpers* template in order to render scope consent form fields. These *must* be added to a custom theme in order for it to function. See [theme upgrade notes](/docs/customize/look-and-feel/upgrade#version-1500) for details on updating an existing theme with the new macro and function.
</ThemeUpdateWarning>
