...
<ReleaseNoteHeading version='1.99.9' releaseDate='MONTH DAY, YEAR' />
...
<Aside type="database" title="Database migration">
  ...
  For installations with 30 million or more users, downtimes of up to 15 minutes can be expected. For installations that use startup probes, like Kubernetes, it is recommended to have the startup probe tolerate this migration time without restarting. See the `fusionauth-app.startup-probe.file-path` option in [Configuration Reference](/docs/reference/configuration#options) for more details.

  { props.migration_message }
</Aside>
<ThemeUpdateWarning version="1.99.9">
...
### Changed
* <Issue issue="3132">
  Additional usage stats are collected for the use of grace periods with application refresh tokens and hashing or expiring api keys.
  </Issue>

### New
* <Issue issue="208">
  FusionAuth now supports phone numbers as a first class identity type. Link to a "what do I need to do to start using phone" page. (maybe goes with 1552).
  </Issue>
* <Issue issue="1552">
  FusionAuth now supports Passwordless Authentication through SMS.
  </Issue>
* <Issue issue="NOT_DOCUMENTED_YET_OR_UNKNOWN">
  Users no longer have to have passwords.
  </Issue>
* <Issue issue="3141">
  JWT Populate Lambdas now have access to `scope` and are allowed to modify `exp` to a shorter time than the configuration
  </Issue>
* <Issue issue="2589">
  Ability to use one-time code for passwordless using hosted login pages. Supports phone out of the box, API users can use email
  </Issue>

### Security
* Security improvement (PR 906)
* Security improvement (PR 951)
...
* <Issue issue="1778">
  Passwords longer than 255 characters will fail
  </Issue>
* <Issue issue="3073">
  During a slow migration of a user using a generic or LDAP connector, the known device cookie is not created. The new device webhook and new device email are still invoked if configured.
  </Issue>
* Immutable universal flag (PR 944)

### Enhancements
* <Issue issue="1574">
  Add debug/error event logging to generic messenger and other enhancements
  </Issue>
...
### Known Issues
* <IssueResolvedVia resolvedIn="1.58.1" viaIssue="3124">After releasing FusionAuth version `1.58.0` we identified a race condition that could prevent an upgrade to version `1.58.0` from successfully completing in a clustered environment. We removed this release from distribution. If you happened to install version `1.58.0` during the brief time that it was available, it is recommended that you upgrade to version `1.58.2`.</IssueResolvedVia>
