...
### Security
* <Issue issue="2933">
  A security vulnerability was discovered in the FusionAuth hosted pages. Under specific application configurations, and with insufficient authorization validation being performed on an access token, an attacker could bypass required steps in post-authentication workflows, allowing unauthorized access to protected resources.
...
### Fixed
* <Issue issue="2869">
  FusionAuth's SCIM Groups API does not properly perform atomic updates to groups and members in SCIM Patch operations. This can lead to consistency issues when multiple SCIM Patch requests for groups and members are simultaneously processed.
  </Issue>
