...
<ReleaseNoteHeading version='1.55.0' releaseDate='December 18, 2024' name='Holiday Hippo'/>

<DatabaseMigrationWarning />

### TBD if we need any dependency updates in 1.55.0, this is just a placeholder
* <Issue issue="2873">
  Update dependencies.
  * Upgrade `xyz` `n.n.n` -> `m.m.m`
  </Issue>

### Security
* <Issue issue="2906">
  Added additional validation checks for authorized URLs in OAuth workflows
  </Issue>
* <Issue issue="2916">
  Add additinal validation of an authorizing JWT when issuing a new JWT.
  </Issue>

### Changed
* As part of the new refresh token grace period functionality, the length of the FusionAuth refresh token is increasing by a small amount. You'll need to accommodate for this if you are storing refresh tokens using storage that is expecting tokens to always exist at the previous tokens' length.
  * Related to [GitHub Issue #2952](https://github.com/FusionAuth/fusionauth-issues/issues/2952)

### New
* <Issue issue="2741">
  Allow for the sending of usage stats. Usage stats allow us to understand how our users use our product, which helps us know where we need to invest in new features and enhancements. 
  </Issue>

### Fixed
* <Issue issue="2737">
  The confirmation page that is shown when users are completing verification and other workflows shows a FreeMarker error when some cookies are unavailable. This could happen when cookies are deleted by a user, removed by a proxy, or when running in an iframe. Note that using the FusionAuth hosted pages in iframes is unsupported, however we still like to handle errors gracefully.
  </Issue>
* <Issue issue="2793">
...
* <Issue issue="2928">
  The `POST /api/user/registration` call is documented as returning a refreshTokeId, but the id is not being returned on the response.
  </Issue>
* <Issue issue="2929">
  When a license containing Breached Password Detection is missing a key, a NullPointerException is generated. This error should be handled more gracefully.
  </Issue>
* <Issue issue="2932">
...
* <Issue issue="2536">
  You can now choose to have your API keys hashed when stored. For hashed keys, you will only be able to see the actual API key value at creation time. Existing API keys cannot be converted to hashed keys.
  </Issue>
* <Issue issue="2537">
  API keys can now be optionally set to expire at a given date/time. An expired key will remain in the system but will cause a `401` response to be returned if it is used in an API call. Expiration can be edited, so a key's expiration can be extended, and a key's expiration can be set to a past date/time to force it int an expired and unusable state.
  </Issue>
* <Issue issue="2742">
...
* <Issue issue="2859">
  The First-time Setup wizard was improved with more descriptive and consistent text around licensing a Community edition of FusionAuth.
  </Issue>
* <Issue issue="2864">
  In the new Webhook Event Log there were numerous small UX and copy improvments added.
  </Issue>
* <Issue issue="2878">
...
### Internal
* <Issue issue="2147">
  Added tests to verify correct handing of wildcards in URLs in various places in the application.
  </Issue>
* <Issue issue="2880">
  Cleaned up our default theme templates by removing some internal `TODO` comments that weren't benefitting users..
  </Issue>
