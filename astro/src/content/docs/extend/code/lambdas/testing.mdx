...
tags: lambda testing, unit test lambda, integration test lambda, fusionauth lambda, test lambda, node.js, lambda debugging
order: 20
---
...
- Click the <IconButton icon="add"/> button at the top right to add a new lambda.
...
- Enable <Breadcrumb>Debug Enabled</Breadcrumb> so that you can see messages in the event log.
...
![Creating a JWT populate lambda](/img/docs/extend/code/lambdas/populate-lambda.png)

Save the lambda.

Now activate the lambda for the example app.

- Navigate to <Breadcrumb>Applications</Breadcrumb> and click the <IconButton icon="edit"/> button on the "Example app".
- Click on the <Breadcrumb>JWT</Breadcrumb> tab.
- Toggle <Breadcrumb>Enabled</Breadcrumb> to on.
- Under <Breadcrumb>Lambda Settings</Breadcrumb>, select the lambda you created, called `ATest`, for the **Access Token populate lambda**.
- Click the <IconButton icon="save" /> button to save the changes.

![Enabling the lambda in an application](/img/docs/extend/code/lambdas/edit-application.png)

<Aside type="note">
  You could also create the lambda above using the FusionAuth API instead of the UI:

  ```bash
  curl -X POST "http://localhost:9011/api/lambda/f3b3b547-7754-452d-8729-21b50d111505" \
    -H "Authorization: lambda_testing_key" \
  ...
  curl -X PATCH "http://localhost:9011/api/application/E9FDB985-9173-4E01-9D73-AC2D60D1DC8E" \
    -H "Authorization: lambda_testing_key" \
    -H "Content-Type: application/json" \
    -d '{
      "application": {
        "jwtConfiguration": { "enabled": true },
        "lambdaConfiguration": { "accessTokenPopulateId": "f3b3b547-7754-452d-8729-21b50d111505" }
      }
    }'
  ```

  Using the API is explained in a later section.
</Aside>
...
Locate the redirect route handler (which is the function that starts with `router.get('/oauth-redirect'`) and add a line to write the JWT to the console at login. The line should be added in the `then()` callback to FusionAuth, as shown below:

```js
.then((response) => {
  console.log(response.response.access_token); // Insert this line
...
In the FusionAuth admin UI, navigate to <Breadcrumb>System -> Event Log</Breadcrumb> and you will see a log entry of the invocation of your "Hello World" lambda.

The entire HTTP response is logged in the test app console (not in the event log). The JWT is a long alphanumeric line at the end of the response. It should look something like below.

```
eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Im8yNUlhRFRFTjkxR3NoVW9iWVhDSGdCMG1FUSJ9..gaeqzVEzGsz8yuC0oF0TLnSUJVWbY8ZSVTdxC6SKery4lIm2icpDMqhpXyHACZMWha9PxDJepVrOiWGukLBFr-5TciNsBO_DrBNnbScyx3FoYKzAXbWov_BRDROnXGr63GlHgh_O0nWKZlgGD9TZemGuzicPSuRuQCKKgPQVbOgIk9SxA1lCa8Js_SAho8MQCIu69_YtjoHUwq1Y1o2lmoZTBz2m5VjhF-6IkNzoC_ls8dZW-XnLT65MWeLDKuQINmYG8VxrADkUvRPvAamJpJj5fBtOgiOwUoa8rMOqCTifGPu00Oh6-ppdhj5gShLApBLxwqeAiifzKAKDkGmjMQ
```

Copy this token from your terminal and paste it into the **Token** text box on the [FusionAuth Online JWT Decoder](/dev-tools/jwt-decoder). You'll see `"message": "Hello World!"` in the <Breadcrumb>Payload</Breadcrumb> box, showing you that your new lambda ran correctly.
...
![Update lambda](/img/docs/extend/code/lambdas/updated-lambda.png)
...
```shell-session
npm install --save-dev     tape@^5.6.6     faucet@^0.0.4     fetch-mock@^9.11.0     jsonwebtoken@^9.0.1     uuid@^9.0.0
```
...
<ExtractedCode src="example-testing-lambdas/complete-application/userCreator.js" lang="javascript" />
...
<ExtractedCode src="example-testing-lambdas/complete-application/userLogin.js" lang="javascript" />
...
<ExtractedCode src="example-testing-lambdas/complete-application/test_1.js" lang="javascript" />
...
<ExtractedCode src="example-testing-lambdas/complete-application/test_2.js" lang="javascript" />
...
<ExtractedCode src="example-testing-lambdas/complete-application/test_3.js" lang="javascript" />
...
Note that running code downloaded from a database is a security risk. Any administrator with access to your FusionAuth admin UI can put malicious code into your lambdas that could use Node.js to access your local disk or send passwords over the internet. To keep safe, run your tests only in a Docker container with no disk access to your physical machine, and having no passwords stored in the container.
