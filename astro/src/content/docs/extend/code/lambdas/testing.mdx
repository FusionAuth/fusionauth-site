...
* Your example username is <InlineFieldValue>richard@example.com</InlineFieldValue> and the password is <InlineFieldValue>password</InlineFieldValue>.
* Your admin username is <InlineFieldValue>admin@example.com</InlineFieldValue> and the password is <InlineFieldValue>password</InlineFieldValue>.
...
- Enter the <InlineField>Id</InlineField> <InlineFieldValue>f3b3b547-7754-452d-8729-21b50d111505</InlineFieldValue>.
- Enter the <InlineField>Name</InlineField> <InlineFieldValue>[ATest]</InlineFieldValue> (to put it at the top of the list of lambdas alphabetically).
- Select the <InlineField>Type</InlineField> <InlineFieldValue>JWT Populate</InlineFieldValue>.
- Leave the <InlineField>Engine</InlineField> as <InlineFieldValue>GraalJS</InlineFieldValue>.
...
- Under <Breadcrumb>Lambda Settings</Breadcrumb>, select the lambda you created, called <InlineFieldValue>ATest</InlineFieldValue>, for the <InlineField>Access Token populate lambda</InlineField>.
...
Log in to the app at `http://localhost:3000` with user <InlineFieldValue>richard@example.com</InlineFieldValue> and password <InlineFieldValue>password</InlineFieldValue>.
...
Copy this token from your terminal and paste it into the <InlineField>Token</InlineField> text box on the [FusionAuth Online JWT Decoder](/dev-tools/jwt-decoder). You'll see `"message": "Hello World!"` in the <Breadcrumb>Payload</Breadcrumb> box, showing you that your new lambda ran correctly.
...
Now you can retrieve the <InlineFieldValue>ATest</InlineFieldValue> lambda you created earlier. This is a useful way to check that a lambda you've created has been successfully uploaded for your app.
