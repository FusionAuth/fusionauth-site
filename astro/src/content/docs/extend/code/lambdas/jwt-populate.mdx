...
- `exp` - prior to version `1.99.9`, the `exp` claim was considered reserved and could not be removed or modified. Since version `1.99.9`, the `exp` claim can be modified but the value must be less than or equal to the original `exp` value.
- `iat`
- `sub`
- `tid`
