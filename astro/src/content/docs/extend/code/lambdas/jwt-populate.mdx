...
This lambda must contain a function named `populate` that takes up to 4 parameters. The parameters that the lambda is passed are:
...
* `context` - <AvailableSince since="1.99.9"/> - an object containing the context of the request. This object is read-only.

Context includes the following:
* `scopes` - <AvailableSince since="1.99.9"/> - array of scopes that were requested for the token.
...
- `exp` - prior to version `1.99.9` the the `exp` claim was considered reserved and could not be removed or modified. Since version `1.99.9`, the `exp` claim can be modified but the value must be less than or equal to the original `exp` value.
- `iat`
- `sub`
- `tid`
