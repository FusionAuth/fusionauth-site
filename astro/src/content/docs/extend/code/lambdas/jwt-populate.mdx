...
tags: jwt populate lambda, custom claims, fusionauth lambda, javascript, token customization, jwt hook, add jwt claims, openid connect
---
...
* `context` - <AvailableSince since="1.59.0"/> - an object containing the context of the request, including access to [secrets](/docs/extend/code/lambdas#secrets). This object is read-only.

### `jwt` object

You may add or modify anything in the `jwt` object. However, you may not modify the header or certain reserved claims. When adding a custom claim to the JWT, always use a namespace to avoid clashes with future standard or FusionAuth-specific claims. The following claims are considered reserved and modifications or removal will not be reflected in the final JWT payload:
