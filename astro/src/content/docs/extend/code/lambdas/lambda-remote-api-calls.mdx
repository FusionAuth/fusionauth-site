...
Lambda HTTP Connect allows you to make HTTP requests from within a lambda. Any lambda can make a request to any network accessible URL which returns JSON.

This features allows you to access data from external systems to configure certain token claims or to add data to a user profile. It also allows you to push profile or other data from FusionAuth to an external system as needed.
