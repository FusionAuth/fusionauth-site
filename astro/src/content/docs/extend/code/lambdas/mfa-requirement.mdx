...
`result` is of type `MFARequiredLambdaResult` and the object will look like this:
```javascript
class MFARequiredLambdaResult {
    /**
...
`policies` is an immutable argument of type `MFAPolicies` and the object will look like this:
```javascript
class MFAPolicies {
    /**
...
`context` is an immutable argument of type `MFAContext` and the object will look like this:
```javascript
class MFAContext {
    /**
