...
This lambda must contain a function named `checkRequired` that takes six parameters. The parameters that the lambda is passed are:
...
class Context {
    ...
    /**
     * When available, the user's registration. For SSO cases where an existing user is signing
     * in to a new application, at the time the lambda would be called, the user will not have
     * a registration yet. That doesn’t happen until later (in complete registration).
...
class StartInstant {
     ...
     * see https://fusionauth.io/docs/reference/data-types#instants for instant documentation
...
class Trust {
    ...
    /**
     * When the MFA trust expires, see https://fusionauth.io/docs/reference/data-types#instants
     * @type {Instant}
     */
    expirationInstant;
    ...
    /**
     * When the MFA trust was created, see https://fusionauth.io/docs/reference/data-types#instants
     * @type {Instant}
     */
    insertInstant;
...
See [Registration API](/docs/apis/registrations) documentation for the `context.registration` object details.
