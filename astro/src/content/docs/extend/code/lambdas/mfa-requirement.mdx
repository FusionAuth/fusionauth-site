...
tags: mfa requirement lambda, multi-factor authentication, fusionauth lambda, step-up auth, conditional mfa, enterprise feature, mfa policy
---
...
class Context {
    ...
    */
    ...
    /**
     * The client risk score for the user calculated by a combination of Intelligent MFA signals if Intelligent MFA is available.
     * @type {ClientRisk} - See below for details
     * @since 1.68.0
     */
    clientRisk;
    ...
    /**
      * Provides access to services available to the lambda, including secrets.
      * Use `context.services.secrets.get('<key>')` to retrieve a secret value.
      * @type {Services}
      * @since 1.64.0
      */
    services;
...
class ClientRisk {
    /**
     * The client risk score for the user calculated by a combination of Intelligent MFA signals if Intelligent MFA is available.
     * @type {String} - Possible score values are [LOW], [MEDIUM], or [HIGH]. Or [NOT_AVAILABLE] if Intelligent MFA is not available.
     * @since 1.68.0
     */
    status;
...
The possible values for `authenticationType` are:
...
Once a lambda is created, you must assign it to a Tenant. Using the FusionAuth admin UI, find the **MFA requirement lambda** field found on the <Breadcrumb>Multi-Factor</Breadcrumb> tab in the Tenant configuration.

Or if you are using the Tenant API, assign the lambda Id to the `tenant.lambdaConfiguration.multiFactorRequirementId` field.

A lambda can also be configured at the application level and, if an application is provided on the request (see action above), the application's lambda will take precedence over the tenant's lambda. To assign an application-specific lambda, using the FusionAuth admin UI, find the **MFA requirement lambda** field found on the <Breadcrumb>Multi-Factor</Breadcrumb> tab in the Application configuration.
