...
When an OpenID Connect identity provider is used to complete a federated login request FusionAuth will use well known OpenID claims and the configured linking strategy to reconcile the user; that is to match the user to an existing account or create a new one. You may optionally utilize a lambda to customize the user and user registration during this authentication event.
