...
This lambda must contain a function named `reconcile` that takes at least three parameters, the fourth, fifth, and sixth parameters are optional. The parameters that the lambda is passed are:
