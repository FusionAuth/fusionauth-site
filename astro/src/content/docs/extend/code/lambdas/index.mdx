...
Don't use exceptions should for flow control; reserve them for _exceptional_ situations.
...
* Lambdas do not have full access to JavaScript modules and libraries. They also cannot import, require or load other libraries.

* Unlike other [Identity Provider linking strategies](/docs/lifecycle/authenticate-users/identity-providers/#linking-strategies), `Link Anonymously` does not call a reconcile lambda.
...
<Aside type="caution">
  Version 1.49 removed the Nashorn engine. Upgrading to version `1.49.0` or beyond will migrate existing lambdas to GraalJS.
</Aside>
