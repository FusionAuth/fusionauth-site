...
tags: login validation lambda, custom login validation, fusionauth lambda, javascript, authentication hook, user validation, credential check
---
...
Once a lambda is created, you must assign it to a Tenant. Using the FusionAuth admin UI, find the **Login validation lambda** field found on the <Breadcrumb>Security</Breadcrumb> tab in the Tenant configuration.

For the Tenant API, assign the lambda Id to the **tenant.lambdaConfiguration.loginValidationId** field.
