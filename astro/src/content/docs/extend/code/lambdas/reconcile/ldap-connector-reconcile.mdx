...
* `user` - the FusionAuth User object. You can modify this object. However you cannot modify the `username` or <InlineField>email</InlineField> attributes once the account is linked.
...
When using this connector with Microsoft Active Directory, the <InlineField>objectGUID</InlineField> attribute will need to be configured to be returned as a byte array. This can be accomplished by appending the suffix `;binary` as an LDAP attribute option to the `objectGUID` in the requested attributes configuration.
...
Navigate to <Breadcrumb>Settings -> Connectors</Breadcrumb> and click <Breadcrumb>Add</Breadcrumb> and select LDAP when prompted to select a connector type.
...
Active Directory does not have a <InlineField>uid</InlineField> attribute, and delivers the GUID as a binary value.
...
<Aside type="note">
`objectGUID;binary` must be specified in your <InlineField>Requested Attributes</InlineField> of the LDAP connector.
