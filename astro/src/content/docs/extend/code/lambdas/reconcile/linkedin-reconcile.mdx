...
Navigate to <Breadcrumb>Settings -> Identity Providers</Breadcrumb> and select your existing LinkedIn configuration or click <Breadcrumb>Add provider</Breadcrumb> and select LinkedIn if it has not yet been configured.
...
The best way to know which format your user's data is being return in is to use the `console.log` statement and print the <InlineField>linkedInUser</InlineField> field to the event log.
