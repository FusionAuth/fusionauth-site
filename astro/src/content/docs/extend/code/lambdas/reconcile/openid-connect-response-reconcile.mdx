...
* `context` - an object containing the context of the request, including access to [secrets](/docs/extend/code/lambdas#secrets). This object is read-only. &nbsp;<AvailableSince since="1.64.0"/>
...
The `id_token` will be provided to the Lambda only when it was returned by the IdP and the signature can be verified. The `id_token` will be returned by the Identity Provider when the <InlineFieldValue>openid</InlineFieldValue> scope was requested.
...
Navigate to <Breadcrumb>Settings -> Identity Providers</Breadcrumb> and select your existing an OpenID Connect configuration or click <Breadcrumb>Add provider</Breadcrumb> and select OpenID Connect if it has not yet been configured.
