...
tags: custom password hashing, password encryptor, fusionauth plugin, import passwords, hash migration, password algorithm, custom hash
---
...
To discover how many users use each password hashing scheme in your instance, create a script that does the following:

1. Call the [Search for Users API](/docs/apis/users/search) to retrieve the full list of User Ids you want to sample; use a wildcard (e.g. `*`) to retrieve Ids for all Users. 
1. For each User, call the [Retrieve a User API](/docs/apis/users/retrieve) to find the `encryptionScheme`, aggregating the total for each hashing scheme as you go.

To stop your script from overwhelming your FusionAuth instance, add a delay of a few seconds between calls. Alternatively, if you have a large amount of users, you could run the script during non-peak hours at a faster rate.

<Aside type="note">
Once installed, leave all password hashing plugins in place.
</Aside>
