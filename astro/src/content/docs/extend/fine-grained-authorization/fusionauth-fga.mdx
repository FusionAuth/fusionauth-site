...
* add permission checks to your application
...
For local development you can install Permify using Docker compose in the same file as FusionAuth. The following configuration creates a Permify instance for this banking example:
...
The following schema defines the relationships between entities in the banking scenario:
...
* `relation` defines a relationship between two entities, such as the `vp` role for a user
* `attribute` is a value associated with an entity, such as the `open_hour` for a bank
* `action` is the name of a permission that can be checked using the Permify API or SDK, such as viewing the `account` page
* 

You can also write a `rule` like `is_in_time_range` which uses attributes from an entity to codify more complicated permissions. Rules can be global or associated with a particular entity definition.
...
For this tutorial, run the following node script to load the schema:
...
Once you have a schema, use authorization data to define relationships and attributes.


The following relationships example assigns each user a role at a bank:
...
The following attributes example assigns each bank an opening hour and a closing hour:
...
Finally, add permission checks to your application code. Each check will return either `true` or `false`, based on whether the action is allowed.

Multiple pieces of information can be provided to Permify to determine if a given permission is granted or not. These include:

* the requested action or permission, which corresponds to the action defined in the schema, such as `account` or `admin`
* the entity ids and types involved
* the subject id which indicates who is asking for this permission
* any context needed, such as the time or the requester's IP address

Here's an example which has the following information:
...
* the current hour provided for context
...
[Loading the authorization](#loading-authorization-data) is a continuous process, not a one-time thing. You must regularly reload the following categories of entities:
