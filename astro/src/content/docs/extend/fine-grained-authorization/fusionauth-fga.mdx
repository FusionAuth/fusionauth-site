...
<Aside type="note">
  Permify uses the term entities to refer to anything that can have a relation in a Permify authorization schema. This is distinct and different from [FusionAuth entities](/docs/get-started/core-concepts/entity-management), which has no formal schema and is instead managed by code.
