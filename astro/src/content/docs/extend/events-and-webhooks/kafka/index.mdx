...
Specify a topic that you've already created in your Kafka cluster and press <InlineUIElement>Send test event</InlineUIElement> to make sure that the connection is working as expected. After seeing that it succeeded, don't forget to press <InlineUIElement>Save</InlineUIElement> in the top right to turn on the Kafka integration.
