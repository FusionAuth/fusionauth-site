...
tags: kafka integration, kafka webhook, fusionauth kafka, event streaming, message queue, kafka topic, event pipeline
---
...
FusionAuth sends system events to Kafka with no further configuration. However, you must enable Tenant-based events individually for each Tenant, as noted in [the webhook documentation](/docs/extend/events-and-webhooks/).
