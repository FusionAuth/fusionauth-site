---
title: New Device
htmlTitle: User Login New Device Event
description: Learn about the User Login New Device event.
tags: new device login event, device login webhook, fusionauth authentication, webhook event, new device detection
---
...
<Event description="This event is generated when a user logs in with a new or unrecognized device, as indicated by the `newDevice` [login API parameter](/docs/apis/login)."
       plan="enterprise"
       eventType={eventType}
       name="User Login New Device"
       scope="tenant"
       transactional="true"
       version="1.30.0"/>
...
<EventBody eventType={eventType}
  ...
  <APIField slot="leading-fields" name="event.tenantId" type="UUID">
