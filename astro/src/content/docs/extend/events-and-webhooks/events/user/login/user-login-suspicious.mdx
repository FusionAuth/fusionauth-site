---
title: Suspicious
htmlTitle: User Login Suspicious Event
description: Learn about the User Login Suspicious event.
tags: suspicious login event, threat detection webhook, fusionauth authentication, webhook event, advanced threat detection
---
...
<EventBody eventType={eventType}
  ...
  <APIField slot="trailing-fields" name="event.threatsDetected" type="Array<String>">
    The types of potential threats that have been flagged for this event. For a full list of potential threats, see [risk signals](/docs/lifecycle/authenticate-users/risk-signals); FusionAuth fires this event whenever any enabled risk signal returns a `HIGH` value.
  </APIField>
