...
Building the `/set-address` page is very similar to the `/account` page. We wll build the UI first, and then add a route to render the page and another one to handle the form submission. But before that, 
we need to ensure that we can save the user's address in the the fusionauth database.

In the dashboard at <Breadcrumb>Home > Customizations > Form Fields > Add Field</Breadcrumb>, you can create a new field called `address`. This will be registered under the `user.data` object.
...
In the code above, we collect the address entered by the user and send a request to update the user information. If the request is successful, we redirect the user to the existing `/make-change` page.

Great! You can test it by registering a user. You should be redirected to a similar page to the one below.
