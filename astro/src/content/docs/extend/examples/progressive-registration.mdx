...
The steps outlined below extend the [example ChangeBank application](https://github.com/fusionauth/fusionauth-example-progressive-registration) to include progressive registration but can be tailored to your application's specific requirements. The application repo contains a Kickstart file with the configuration you need to set up FusionAuth to allow authentication from the Express app. Follow the steps in the companion application README to get up and running.
...
Now rerun the ChangeBank application using the command below.
