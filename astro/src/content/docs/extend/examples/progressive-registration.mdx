...
FusionAuth provides SDKs for many languages to make the API usage easy. In this section, you will extend the Changebank application to have API routes that allow registration to be completed by utilizing API calls.

- `api/register`: This endpoint will register a user using the FusionAuth API.
- `api/register/:userId`: This endpoint will update the user's data using the FusionAuth API.

Great! Let's add the logic for our endpoints in the `index.ts` file. You will start first by importing the necessary dependencies and adding the necessary middleware to parse JSON in API requests.

Add the following import to the top of the `complete-application/src/index.ts` file.

```ts
import bodyParser from "body-parser";
```

Beneath the line of code `app.use(express.urlencoded());` in  the `complete-application/src/index.ts` file, add the following middleware to parse requests with JSON data.

```ts
app.use(bodyParser.json());
```

After that, you can add the registration API endpoints to your application in the `complete-application/src/index.ts` file. The first endpoint will be called `/api/register` and accept a JSON body with only the `email` and `password` fields.
...
With this endpoint added, you can add the logic to update the user's data using the FusionAuth API. The second endpoint will update the user's data and will be called `/api/register/:userId` and accept only a JSON body with the `address` field.
...
The `updateRegistration` method is also available in the Typescript SDK. However, this method sends a `PUT` request to the FusionAuth `/api/user/registration` endpoint, which means you can have data inconsistencies if you are not careful. For this reason, we recommend using the `patchRegistration` method and only use the `updateRegistration` method if you are sure that you want to update the entire registration object, which might clear out other fields that you might have set.


You can now rerun the Changebank application using the following command:

```sh
npm run dev
```

You can now test the application by registering a user using the `/api/register` endpoint. You can do this by sending a `POST` request to the route `http://localhost:8080/api/register`:
...
You will receive a response with the `registrationId` and `user` fields.
...
Now, you can use the `user.id` value to call the `/api/register/:userId` endpoint to update the user's data. You can do this by sending a `PATCH` request to `http://localhost:8080/api/register/<userId>`:
...
You will receive response similar to the one below with the updated user data.
...
- If you are using the FusionAuth admin UI, you can create a registration form that will collect the user's information. This form can be customized with as many fields and steps as you need to fit your application's requirements.
- If you are using the FusionAuth API, you can use the available client libraries or call the FusionAuth API directly to create user registrations. This is useful if you have a custom UI or data collection process.
