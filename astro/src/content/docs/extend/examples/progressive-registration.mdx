...
To implement progressive registration, you need a [FusionAuth license](/docs/get-started/core-concepts/licensing) to access advanced registration features. This guide is based on the [ChangeBank](https://github.com/FusionAuth/fusionauth-quickstart-javascript-express-web) application, which will be extended to 
...
- The user will be given a form to enter their basic information such as first name, last name, and email.
- The user will receive a form to enter their password and complete the registration.

The challenge here is managing the routing to the home page even more when you need the user to enter specific information on the home page
after a successful registration. This can be, for example, entering a credit card number or being prompted to create an entity such as a project or anything else that might be required to use the application. 
...
<Breadcrumb>Home > Customizations > Forms</Breadcrumb>. Click the <InlineUIElement>Add</InlineUIElement> button on this page. You will be presented with a form to create a new form with fields such as
`name` that can be arbitrary, the `type` that should be **Registration**, and the **Form steps** section where you will customize the registration form.
...
and a field for the email for the first step. The second step will collect the password and complete the registration.
...
Great! You can now move on integrating this registration form into our ChangeBank application.
...
The ChangeBank application does not have a registration form or logic to handle the registration. You will need to:
- Add a button to the navigation bar to redirect the user to the registration form.
- Add a route to handle the registration and logic blocks to the handler of the `/oauth-redirect` route to ensure the user is redirected to the correct page after login or registration.
...
You can also notice a registration link on the login page:
...
URL for redirection to the FusionAuth registration page. You can add this route in the `complete-application/src/index.ts` file.
...
The user will be redirected to the home page if all these steps are successful. You must add logic blocks to this route handler to ensure the user is redirected to the correct page after a login or registration. In the case of a registration, you want to redirect the user to a page where
...
The above first checks that the user is authenticated and then retrieves the user's details from the cookie, such as the user's email and user ID. You need these details to update the user's address.
...
Replace the `noapikeyneeded` with an API key.

You can now add the code for the form to the `set-address.html` file. The form will collect the user's address and save it in the <InlineField>user.data</InlineField> object.
...
The code above collects the user's address and sends a request to update the user information. If the request is successful, the user is redirected to the existing `/make-change` page. You can test it by registering a user. You should be redirected to a similar page to the one below.

<img src="/img/docs/extend/progressive-registration/set-address.png" alt="Set address"/>

On the `/make-change` page, you will modify the existing UI and backend logic to save the amount in the FusionAuth database, but also a reason in case the amount is significant.

### Tweaking the Make Change Page

You will add a new field labeled `Reason for change` that will be displayed if the user's entry is greater than or equal to 500. You will also modify the form submission route to ensure the data is saved in the FusionAuth database.
The first step is to add a new field to the form. In the dashboard at <Breadcrumb>Home > Customizations > Form Fields > Add Field</Breadcrumb>, you will create new fields called `amountChangedReason` and `amount`. 
...
You add a listener to the `changeInput` field in the code above. If the amount the user enters is superior or equal to 500, the UI displays the reason input field, with the tag `<div id="reason-container">`. 
By default, the reason is that the input field is hidden.

Now that you have the logic to save the data in the FusionAuth database, you need to add the code to the `complete-application/src/index.ts` file.

You will add the following code to the `make-change` route handler for POST requests:
...
You have explored how to add a progressive registration form using the FusionAuth admin dashboard. However, you can use your own UI to collect the information. In the next section, you will explore how to add a progressive registration form using the FusionAuth API.

## Using FusionAuth API

In some cases, you can use your UI to collect the information. In this case, you should understand that implementing a progressive registration process in your application will be
...
To register a user using the API, you must send a `POST` request to the `/api/user/registration` endpoint. The request requires the following fields and headers:
...
The API supports more fields that can be included based on your requirements. The [FusionAuth API documentation](/docs/APIs/registrations#request) contains a complete list of supported fields.

An interesting field is the <InlineField>user.data</InlineField> field. According to your application requirements, this field can store any data you want to associate with the user. For example, you can use this field to store the user's address or any other data you want to collect during registration.
To ensure this field can be filled, create a custom field in the FusionAuth admin UI or use the API.

In the dashboard at <Breadcrumb>Home > Customizations > Form Fields > Add Field</Breadcrumb>, you can create a new field.
In the form, specify the field's name and set the field type to **Custom user data (user.data.*)**. The image below shows how to add a new field called `address` under the `user.data` object.
...
The `updateRegistration` method is also available in the Typescript SDK. However, this method sent a `PUT` request to the `/api/user/registration` endpoint, which means you can have
