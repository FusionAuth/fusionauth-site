...
The steps outlined below extend the [example ChangeBank application](https://github.com/FusionAuth/fusionauth-quickstart-javascript-express-web) to include progressive registration but can be tailored to your application's specific requirements. The application repo contains a Kickstart file with the configuration you need to set up FusionAuth to allow authentication from the Express app. Follow the steps in the companion application README to get up and running. 

You can start the project with `npm run dev` but we recommend using [Docker](https://www.docker.com/) instead. If you choose Docker, follow these steps:
...
In the `complete-application/.env` file, add the following environment variable:

```text
fusionAuthURLBackend="http://fusionauth:9011"
```

This variable specifies the internal URL for communication between the web application and the `fusionauth` service within Docker.

3. Update the `getKey` Function

Modify the `getKey` function in `complete-application/src/routes/index.ts` to use the `fusionAuthURLBackend` environment variable for the `jwksUri` parameter and the `client` declaration.
...
Once these changes are made, you can start the application with Docker by running the following command:
...
This starts the ChangeBank application on port 8080 inside a Docker container, and you can access it at http://localhost:8080.
...
Now go to <Breadcrumb>Customizations -> Forms</Breadcrumb> and click the <InlineUIElement>Add</InlineUIElement> button.
...
Now you'll add a `/register` to handle registration. The `/register` route constructs the URL that redirects the user to the FusionAuth registration page. 

In the `complete-application/src/index.ts` file, add the following code.
...
Save the changes and navigate to the registration page to view the two-step registration in action. The first step collects the user's first name, last name, and email.
...
In the FusionAuth admin UI, navigate to <Breadcrumb>Customizations -> Form Fields -> Add</Breadcrumb>, and create a new field called `Address` . For <InlineField>Field</InlineField>, select `Custom user data (user.data.*)`. The `Address` field will be registered under the <InlineField>user.data</InlineField> object.
...
Now configure the client with an API key to ensure it can execute requests.

In `complete-application/src/index.ts`, find the declaration of the `client` variable, which looks something like below.

```ts
const client = new FusionAuthClient('noapikeyneeded', fusionAuthURLBackend);
```

Replace the `noapikeyneeded` with an API key. If you are following along with the sample application, the API key set with the kickstart configuration is `33052c8a-c283-4e96-9d2a-eb1215c69f8f-not-for-prod`.
...
Now run the ChangeBank application using the command below if your application is not already running.
