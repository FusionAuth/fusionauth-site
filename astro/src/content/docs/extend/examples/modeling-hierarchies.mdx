...
**TODO: This needs to be rewritten with an example app that uses Git submodules to include the `fusionauth-containers` and the `fusionauth-example-kickstart` repositories.**

Use `git clone` to clone the repository at https://github.com/FusionAuth/fusionauth-containers, or download and unzip it.

Also, use `git clone` to clone the repository at https://github.com/FusionAuth/fusionauth-example-kickstart, or download and unzip it.

Open a terminal in the directory containing the repository files from the `fusionauth-containers` repository.
