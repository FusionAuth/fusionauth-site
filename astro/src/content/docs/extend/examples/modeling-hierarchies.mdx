...
Fine-grained authentication (FGA) allows for more granular permissions than Entities. This includes:

* attribute based access control (ABAC) where you can limit access based on attributes of the request or objects being accessed, like limiting access to a system based on time of day
* relationship based access control (ReBAC) where you can assign build relationships and have authorization decisions be based on those, like granting access to a folder and all subfolders and files contained therein

To use FGA, you use [FusionAuth FGA by Permify](/docs/extend/fine-grained-authorization/fusionauth-fga), a standalone authorization server. 

With FGA, you can model the corporate, department, and document relationships using this schema:
...
* can answer "which resources can this user access?" and "which users can access this resource?" queries efficiently
* permissions can be checked in bulk

Use one of the [supported authorization SDKs](https://docs.permify.co/api-reference/introduction) to check permissions within your application:
...
When a new company or other object is added, update the relationships to ensure access to the documents for that company. You can use the authorization SDKs to create and update the relationships between the corporations, companies, departments, documents and users.
