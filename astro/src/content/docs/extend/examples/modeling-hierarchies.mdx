...
In a new terminal, run the commands below to install Axios and run the script to check what permissions Richard has to both documents. Here you use Docker again, with the Deno 2 image, which can run TypeScript without any compile step. In reality, you could use the TypeScript compiler, and Node, Bun, or any other JavaScript environment you like.
