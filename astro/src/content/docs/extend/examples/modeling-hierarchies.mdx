...
- [Example Node Application](#example-node-application)
  ...
  - [Run Your Website](#run-your-website)
- [Clean Up](#clean-up)
- [TODO](#todo)
...
Use `git clone` to clone the repository at https://github.com/fusionauth/fusionauth-example-multiapp-dashboard, or download and unzip it. TODO
...
<Aside type="note">
  ...
  ```sh
  docker rm fa faDb; docker compose down -v
  ```
</Aside>
...
- Lastly, manage the Password document, and give it an entity grant to the Change Bank Operations department with permission IsMember.
  - Notice here that when you search for `Operations`, the search dropdown list provides you only the names `Change Bank Operations` and `Change Insurance Operations`, with no Ids or hierarchical links displayed. So if you had instead called both departments just `Operations`, without indicating the company they belonged to in their name, you wouldn't know which one to choose when trying to add the document to the department. Though it would be more elegant to call the entity by the same name as the department, just `Operations`, you need to work with the limitations of FusionAuth here to create a makeshift hierarchy. Alternatively, you could create all entities programmatically using the API an entity Ids. That would allow you to use any names you wanted.

You haven't used set any read or write permissions yet, because those are linked only to users, or flow implicitly downwards through the company hierarchy set by `IsMember`.
