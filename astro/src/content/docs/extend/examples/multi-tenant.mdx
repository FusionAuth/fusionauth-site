...
This guide covers logical isolation of tenants. If you need physical isolation with FusionAuth, run different instances. You can also [read this article for more information](/learn/expert-advice/identity-basics/multi-tenancy-vs-single-tenant-idaas-solutions).
...
Instead of doing all of these manually, you may also download [a kickstart file](https://github.com/FusionAuth/fusionauth-example-kickstart/blob/master/example-apps/multi-tenant-control-plane-setup.json), customize it, and run it using [Kickstart](/docs/get-started/download-and-install/development/kickstart). 
...
<Aside type="note">
You can use [any of the client libraries](/docs/v1/tech/client-libraries/) to create FusionAuth configuration. This document happens to use PHP, but feel free to use whatever language makes the most sense in the context of your development.

You can manipulate FusionAuth with client libraries by creating JSON objects as documented by the [APIs](/docs/apis/overview) and then calling the corresponding operation.
</Aside>
