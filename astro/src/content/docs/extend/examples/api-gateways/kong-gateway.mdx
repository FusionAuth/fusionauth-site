...
tags: kong gateway, api gateway, jwt, fusionauth kong, api management, kong plugin
---
...
** Note: If the terminal seems to hang after outputting `Debugging info logged to 'kong-quickstart.log'`, it may be having trouble connecting to Docker. Try resetting Docker to factory defaults to solve this issue.
...
First, you need to make sure the JWT issuer setting is correct. Navigate to <Breadcrumb>Tenants -> Your Tenant</Breadcrumb> and change the **Issuer** field to the URL of your FusionAuth instance, for example, `https://local.fusionauth.io`. Record this value, because you will use it later when generating the JWT.

![Tenant issuer](/img/docs/extend/examples/api-gateways/kong-gateway-tenant-issuer.png)

Next, you need to configure an application that will issue tokens to access the Kong Gateway service.

Navigate to <Breadcrumb>Applications</Breadcrumb> and create a new application. Fill out the **Name** field, then click the <Breadcrumb>JWT</Breadcrumb> tab and toggle the <Breadcrumb>Enable application-level JWT override</Breadcrumb> switch. Select `Auto generate a new key on save...` for the **Access Token signing key** field. This will generate an RS256 asymmetric key pair specifically for this application.

![FusionAuth application JWT settings](/img/docs/extend/examples/api-gateways/kong-gateway-fusionauth-jwt-settings.png)

Click the <Breadcrumb>Save</Breadcrumb> button.

After saving the application, find the application in the list of applications and click <Breadcrumb>View</Breadcrumb> next to the application you just created. Here you will find the application **Id**. Note this value, as we will refer to it later as `APPLICATION_ID`.

![FusionAuth Application Id](/img/docs/extend/examples/api-gateways/kong-gateway-application-id.png)

Now, navigate to <Breadcrumb>Settings -> Key Master</Breadcrumb>. You will see the access key that you've just created in this list. By default, it has the name `Access token signing key generated for application <NAME_OF_APPLICATION>`. View the key and copy the entire contents of the **PEM encoded** field under the <Breadcrumb>Public Key</Breadcrumb> section into a file in your current working directory. Name the file something like `public.pem`.

![FusionAuth Public Key for Access Token](/img/docs/extend/examples/api-gateways/kong-gateway-public-key.png)
...
To make this example simpler, you'll use the Login API to get the JWT. To do this, you need an API key with POST access to the `/api/login` endpoint. Go to <Breadcrumb>Settings -> API Keys</Breadcrumb> to add the key. Make sure the appropriate tenant is selected in the **Tenant** field.

![FusionAuth API Key setup](/img/docs/extend/examples/api-gateways/kong-gateway-api-key-setup.png)
...
![FusionAuth API Key setup](/img/docs/extend/examples/api-gateways/kong-gateway-api-key.png)

Finally, make sure there is at least one user registered to your application so that you can test with a JWT issued for that user. You can create a new user by navigating to <Breadcrumb>Users -> Add user</Breadcrumb>. Toggle the **Send email to set up password** switch to off and manually enter a password in the **Password** field.

![FusionAuth User Creation](/img/docs/extend/examples/api-gateways/kong-gateway-create-user.png)
...
![FusionAuth User Registration](/img/docs/extend/examples/api-gateways/kong-gateway-user-registration.png)
...
Here, `<YOUR_FUSIONAUTH_URL>` should exactly match the **Issuer** field from <Breadcrumb>Tenants -> Your Tenant</Breadcrumb>, `<CONSUMER_USERNAME>` should match the **username** that you supplied to the consumer in the prior step, and `<PUBLIC_KEY_FILENAME>` should match the name of the file that you saved the public key to, from the <Breadcrumb>Key Master</Breadcrumb> page.
...
Here, `<YOUR_FUSIONAUTH_URL>` is the **Issuer** name, and `<API_KEY>` is the key you noted when setting it up on the <Breadcrumb>Settings -> API Keys</Breadcrumb> page.
...
* The issuer URL set in the **Issuer** field from <Breadcrumb>Tenants -> Your Tenant</Breadcrumb> must exactly match the `key` value in the curl command when creating the consumer credentials on Kong. If the values do not match exactly, including any white space and slashes, the JWT token will not be accepted by Kong.
