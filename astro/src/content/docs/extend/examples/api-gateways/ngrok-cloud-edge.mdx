...
tags: ngrok, cloud edge, oidc, oauth2, fusionauth ngrok, tunnel authentication
---
...
You can view the [ngrok guide](https://ngrok.com/docs/integrations/endpoint-sso/fusionauth-sso-oidc) as well.
...
Navigate to <Breadcrumb>Applications</Breadcrumb> and then create a new Application. Fill out the **Name** field, then click the <Breadcrumb>OAuth</Breadcrumb> tab.
...
Log into an account with ngrok Cloud Edge enabled. Navigate to [the ngrok dashboard](https://dashboard.ngrok.com) and then to <Breadcrumb>Cloud Edge -> Edges</Breadcrumb>.

![The ngrok Cloud Edges configuration screen](/img/docs/extend/examples/api-gateways/ngrok-edges.png)

Click <Breadcrumb>Create Edge</Breadcrumb> and select an `HTTPS Edge`. Click <Breadcrumb>Create HTTPS Edge</Breadcrumb>.
...
Click on <Breadcrumb>Start a Tunnel</Breadcrumb>. This will give you an `ngrok` command to run.
...
Click on <Breadcrumb>Begin setup</Breadcrumb>.
