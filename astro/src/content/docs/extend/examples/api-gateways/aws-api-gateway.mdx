...
tags: aws api gateway, amazon api gateway, jwt authorizer, lambda authorizer, fusionauth aws, api protection
---
...
Navigate to <Breadcrumb>Applications</Breadcrumb> and create a new application. Fill out the **Name** field, then click the <Breadcrumb>OAuth</Breadcrumb> tab.
...
On the <Breadcrumb>JWT</Breadcrumb> tab, toggle the <Breadcrumb>Enable application-level JWT override</Breadcrumb> field and set the **Access Token signing key** and **Id Token signing key** to `Auto generate a new key on save...`
...
Under **Trigger Configuration**, choose `API Gateway` as the source. select <Breadcrumb>Use an existing API</Breadcrumb> and then select the API gateway set up earlier. Select the `$default` deployment stage.
...
* Ensure that your FusionAuth application's **Access Token signing key** and **Id Token signing key** on the application's <Breadcrumb>JWT</Breadcrumb> tab are asymmetric (RS256).
