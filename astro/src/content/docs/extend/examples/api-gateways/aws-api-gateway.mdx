...
Under <InlineField>Trigger Configuration</InlineField>, choose <InlineFieldValue>API Gateway</InlineFieldValue> as the source. select <InlineUIElement>Use an existing API</InlineUIElement> and then select the API gateway set up earlier. Select the `$default` deployment stage.
