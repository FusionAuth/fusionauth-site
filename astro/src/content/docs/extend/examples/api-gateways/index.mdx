...
You typically need to ensure that FusionAuth is signing the JWT with an asymmetric key. Do that by navigating to <Breadcrumb>Settings -> Key Master</strong> to create or import the key pair. Then configure the Application to use the key by navigating to <strong>Applications -> Your Application -> JWT</Breadcrumb>.
