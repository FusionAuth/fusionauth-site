...
While FusionAuth does not directly support limiting device logins, you can implement a solution using FusionAuth APIs and custom logic in your application. 
...
This guide will extend the [express based](https://expressjs.com) [ChangeBank application](/docs/quickstarts/quickstart-javascript-express-web) to limit concurrent logins. The application will be extended to limit the user to 2 concurrent logins.

The completed application code is available on [GitHub)[https://github.com/FusionAuth/fusionauth-device-limit-guide]
