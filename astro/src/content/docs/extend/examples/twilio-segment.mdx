...
tags: twilio segment, user analytics, webhook integration, user data, segment integration, fusionauth segment
---
...
- Enable webhooks for the events:
  - **user.create.complete** `Checked`
  - **user.email.update** `Checked`
  - **user.login.success** `Checked`
  - **user.update.complete** `Checked`
- Click <IconButton icon="save"/> at the top right of the page.
- Browse to <Breadcrumb>Settings -> Webhooks</Breadcrumb>.
- Click <IconButton icon="add"/> at the top right.
- Enter the URL `http://fa_seg`.
- Click <IconButton icon="save"/> at the top right. All events are enabled by default. You don't need to worry about security because the receiving service will run on the same Docker network as FusionAuth.
