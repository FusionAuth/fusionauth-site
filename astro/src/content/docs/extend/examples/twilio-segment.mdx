...
- `callSegmentIdentify()`, which calls the Segment `identify` method to update user information. The call is made only FusionAuth update completed webhooks. User data goes in the `traits` property.
- `callSegmentTrack()`, which calls the Segment `track` method to associate an event with user. In this script you're tracking only logins, but you could add any event you want. Data about the event goes in the `properties` property. There isn't any to add for a successful login.
...
- Copy the `app.mjs` script from earlier into this directory too.
...
- Create the file `Dockerfile` with the content below.
  ...
  RUN apk add --no-cache curl node npm bash
...
- Click Save at the top right. All events are enabled by default. You don't need to worry about security because the receiving service will run on the same Docker network as FusionAuth.
...
- In Segment you should see your event arriving. You may have to refresh the page. If you cannot see your event, consult the troubleshooting section at the end of this guide.
  ![Segment event debugger](segmentEventReceived.webp)
...
- Browse to http://localhost:9011/admin/user and create a new user with any details you want.
  ![New FusionAuth user](segmentNewUser.webp)
- In the terminal you should see a notification from the filtering script:
  ```sh
...
- Install Node.js.
- Change the webhook URL in FusionAuth settings from `http://fa_seg:3000` to `http://host.docker.internal`.
