...
You can run Permify the open-source project on your own for free, forever. For more information, see the [Permify documentation](https://docs.permify.co/).
...
For user data, you can use the [User Create](/docs/extend/events-and-webhooks/events/user/user-create) and [User Update](/docs/extend/events-and-webhooks/events/user/user-update) webhooks to propagate user data. Make these webhooks transactional, but fast, so use something like a durable queue to receive the events. Or if you have Kafka in your system already, [publish them to Kafka](/docs/extend/events-and-webhooks/kafka). Then, use one of the [Permify SDKs](https://docs.permify.co/getting-started/enforcement#sdks) and whenever changes occur, update Permify.
