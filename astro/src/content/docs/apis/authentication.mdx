---
title: Authenticate with our APIs
description: Learn how the FusionAuth APIs authenticate requests.
order: 0
tags: api authentication, api key authentication, bearer token, basic authentication, client credentials, jwt authentication, fusionauth api, rest api, http api, api security, tenant authentication, global api key
---
import APIAuthenticationIcon from "src/components/api/APIAuthenticationIcon.astro";
import APIKeyCrossTenantNote from 'src/content/docs/apis/_shared/_api-key-cross-tenant-note.mdx';
...
import TenantAuthentication from 'src/content/docs/apis/_shared/_tenant-authentication.mdx';
...
* <APIAuthenticationIcon classList={["mr-1"]} type="global-api-key"/> [Global API Key Authentication](#global-api-key-authentication)
...
When an API is marked with a yellow lock icon such as <APIAuthenticationIcon classList={["px-1"]} type="api-key"/> it means you can use any type of API Key (global or tenant-scoped).
...
See the [API Endpoints reference](/docs/reference/api-endpoints) for a complete list of endpoints that use this authentication type.
...
When an API is marked with a red lock icon such as <APIAuthenticationIcon classList={["px-1"]} type="global-api-key"/>, use a global API key. Attempts with a tenant-scoped API key fail.

In addition if an `X-FusionAuth-TenantId` header is provided to this endpoint, the request will fail with a `401`.

## Basic Authentication using an API Key

<AvailableSince since="1.27.0"/>
...
The requesting and target entities, as well as permissions, are all managed using [Entities](/docs/get-started/core-concepts/types/entity-management).
...
<APIBlock>
  ...
  <APIField name="Tenant">
    ...
    Once a tenant is assigned to an API Key it will no longer be considered global and the set of endpoints it can call will be limited.
  ...
  <APIField name="Access Control List">
    ...
    <PlanBlurbApi plan="enterprise" feature="IP access control lists" />
  </APIField>
...
<Aside type="caution">
If you do not grant any endpoint permissions to an API key, the key has access to *all endpoints* and is essentially a 'super user' key. However, if you associate a tenant with a key, this is not the case. Tenant-scoped keys are, by default, only granted a subset of permissions.
</Aside>
...
When you assign an API key to a tenant, any requests made with this key will only be able to operate on users, applications, groups, and other entities in the selected tenant. Tenant-scoped keys will not be able to call global endpoints.
