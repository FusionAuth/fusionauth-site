...
Some APIs may be authenticated by the source IP address of the request. For example, if `fusionauth-app.local-metrics.enabled` is set to <InlineFieldValue>true</InlineFieldValue>,  `/api/prometheus/metrics` and `/api/status` will accept requests from `localhost` without any other form of authentication.
...
<APIBlock>
  ...
  <APIField name="Key Manager">
    ...
    Any attempt to call the API Keys API with a non-manager key (`keyManager` set to <InlineFieldValue>false</InlineFieldValue>) will return a HTTP response status code `401`.
  </APIField>
