...
* <APIAuthenticationIcon classList={["mr-1"]} type="api-key"/> <ScrollRef target="API Key Authentication" />
* <APIAuthenticationIcon classList={["mr-1"]} type="basic"/> <ScrollRef target="Basic Authentication using an API Key" />
* <APIAuthenticationIcon classList={["mr-1"]} type="client-credentials"/> <ScrollRef target="Client Credentials" />
* <APIAuthenticationIcon classList={["mr-1"]} type="jwt"/> <ScrollRef target="JWT Authentication" />
* <APIAuthenticationIcon classList={["mr-1"]} type="none"/> <ScrollRef target="No Authentication Required" />
* <APIAuthenticationIcon classList={["mr-1"]} type="local-bypass"/> <ScrollRef target="Localhost Authentication Bypass" />
...
When an API is marked with a red passport icon such as <APIAuthenticationIcon classList={["px-1"]} type="client-credentials"/>, the authorization becomes a two step process. To complete the process and generate a token you must:
...
When an API is marked with a blue identity icon such as <APIAuthenticationIcon classList={["px-1"]} type="jwt"/> it means you may call this API without
