...
* <Icon name="lock" /> <ScrollRef target="API Key Authentication" />
* <Icon name="shield" /> <ScrollRef target="Basic Authentication using an API Key" />
* <Icon name="passport-red" /> <ScrollRef target="Client Credentials" />
* <Icon name="badge" /> <ScrollRef target="JWT Authentication" />
* <Icon name="unlock" /> <ScrollRef target="No Authentication Required" />
* <Icon name="server" /> <ScrollRef target="Localhost Authentication Bypass" />
...
When an API is marked with a red locked icon such as <span class="px-2"><Icon name="lock" title="Supports API Keys"/></span> it means you are required to provide an API key.

To enable access to a secured API, create one or more API keys. The API key is then supplied in the HTTP request using the Authorization header. See <ScrollRef target="Managing API Keys" /> for more information on adding additional keys.
...
<iframe allowfullscreen="" frameborder="0" width="560" height="315" src="//www.youtube.com/embed/eJWRKjd4DV4?rel=0" enablejsapi="true" data-gtm-yt-inspected-8="true"></iframe>
...
When an API endpoint is marked with a shield such as <span class="px-2"><Icon name="shield" title="Supports Basic Auth"/></span> it means you call this API and authenticate using HTTP basic authentication. HTTP basic authentication is a simple, standards based, authentication method. A username and password are supplied, separated by a `:`. It must be prefaced by the string `Basic` and a space. The `username:password` string is base64 encoded.
...
When an API is marked with a red passport icon such as <span class="px-2"><Icon name="passport-red" title="Supports Client Credentials"/></span>, the authorization becomes a two step process. To complete the process and generate a token you must:
...
Here is an example [client credentials grant using Entities](/docs/lifecycle/authenticate-users/oauth/overview#example-client-credentials-grant).
...
When an API is marked with a blue identity icon such as <span class="px-2"><Icon name="badge" title="Supports JWT" /></span> it means you may call this API without
...
When an API that is marked with a green unlocked icon such as <span class="px-2"><Icon name="unlock" title="No authentication required" /></span> it means that you are not required to provide an `Authorization` header as part of the request. The API is either designed to be publicly accessible or the request may take a parameter that is in itself secure.
...
<img src="/img/docs/create-api-key.png" alt="Create an API Key" width="1200" />
{/* /img/docs/apis/create-api-key.png */}
...
These permissions are managed via the [API Key API] or in the administrative user interface under the [breadcrumb](/docs/apis/api-keys)#Endpoints# section.
...
<img src="/img/docs/apis/api-key-endpoint-permissions.png" alt="API Key endpoint permissions." width="1200" role="top-cropped" />
