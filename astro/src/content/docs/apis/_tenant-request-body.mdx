...
<APIBlock>
  ...
  <APIField name="tenant.jwtConfiguration.accessTokenVerificationKeyIds" type="Array<UUID>" optional since="1.69.0">
      The list of access token verification key Ids that are trusted by this FusionAuth tenant. **tenant.jwtConfiguration.accessTokenKeyId** is implicitly included in this list and does not need to be explicitly specified.

      If **tenant.jwtConfiguration.accessTokenKeyId** is changed to a new key and the old key is supplied in this field, then this facilitates key rotation because FusionAuth will trust JWTs signed by both keys, while only signing JWTs with the new key.
  </APIField>
  ...
  <APIField name="tenant.jwtConfiguration.idTokenVerificationKeyIds" type="Array<UUID>" optional since="1.69.0">
    The list of Id token verification key Ids that are trusted by this FusionAuth tenant. **tenant.jwtConfiguration.idTokenKeyId** is implicitly included in this list and does not need to be explicitly specified.

    If **tenant.jwtConfiguration.idTokenKeyId** is changed to a new key and the old key is supplied in this field, then this facilitates key rotation because FusionAuth will trust JWTs signed by both keys, while only signing JWTs with the new key.
  </APIField>
