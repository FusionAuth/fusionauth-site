...
<APIBlock>
  ...
  <APIField name="user.legacyIdentifier" type="String" optional since="1.67" immutable>
    A legacy identifier used to override the `sub` claim in tokens issued by the Legacy Identity Provider Adapter. This value is immutable after the user is created and must be unique within the tenant when non-null.
  </APIField>
