...
<APIBlock>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    This field is required unless `tenant.passwordEnabled` is `false`, `sendSetPasswordEmail` is `true`, or `sendSetPasswordIdentityType` is set to something besides `doNotSend`. Note that by default, `sendSetPasswordIdentityType` is `doNotSend`.
  </APIField>
