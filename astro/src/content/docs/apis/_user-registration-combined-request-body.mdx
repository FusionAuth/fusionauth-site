...
<APIBlock>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    If `tenant.passwordEnabled` is true, this field is required unless `sendSetPasswordEmail` or `sendSetPasswordPhone` is set to `true`. By default, `sendSetPasswordEmail` and `sendSetPasswordPhone` are `false`.
  </APIField>
