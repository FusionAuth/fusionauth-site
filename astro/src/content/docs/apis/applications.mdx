...
<PatchSupport api_endpoint="pass:normal[/api/application/``\{applicationId\}``/role/``\{roleId\}``]" />

<XFusionauthTenantIdHeaderScopedOperation api_endpoint="pass:normal[/api/application/``\{applicationId\}``/role/``\{roleId\}``]" />
