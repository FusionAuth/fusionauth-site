...
<APIBlock>
  <APIField name={ props.parameter_prefix + "exact" } type="Boolean" optional defaults="false" since="1.61.0">
    Whether to perform a case-insensitive exact match on the Group name. If `true`, the search criteria will be interpreted as `value`. Wildcards or special characters in the search criteria will search for the character literals in the name.
  </APIField>
  <APIField name={ props.parameter_prefix + "name" } type="String" optional>
    The case-insensitive string to search for in the Group name. This can contain wildcards using the asterisk character (`*`). If no wildcards are present, the search criteria will be interpreted as `*value*` unless <InlineField>exact</InlineField> is set to `true`.
  </APIField>
