...
import DeprecatedSince from 'src/components/api/DeprecatedSince.astro';
  ...
  <APIField name="sendSetPasswordEmail" type="Boolean" optional defaults="false" deprecated>
    If `true`, FusionAuth will send the User an email asking them to set their password. The Email Template that is used is the one specified in the **setPasswordEmailTemplateId** {props.set_password_field_config_blurb}.
