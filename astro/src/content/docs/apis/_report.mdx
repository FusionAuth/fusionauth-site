...
<API method="GET" uri={`/api/report/${props.reportURI}?start={start}&end={end}&applicationId={applicationId}`} authentication={["global-api-key"]} title={`Retrieve the ${props.reportType} report`}/>
<API method="GET" uri={`/api/report/${props.reportURI}?start={start}&end={end}&applicationId={applicationId}&loginId={loginId}`} authentication={["global-api-key"]} title={`Retrieve the ${props.reportType} report for an individual user by username or email address`}/>
<API method="GET" uri={`/api/report/${props.reportURI}?start={start}&end={end}&applicationId={applicationId}&userId={userId}`} authentication={["global-api-key"]} title={`Retrieve the ${props.reportType} report for an individual user by unique Id`}/>
