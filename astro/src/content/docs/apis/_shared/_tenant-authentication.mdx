...
Tenant-scoped keys have a smaller set of endpoints available to them. When using a tenant-scoped key with the `X-FusionAuth-TenantId` header, the tenant Id provided in the header must be the same as the Id of the tenant the key is associated with.
