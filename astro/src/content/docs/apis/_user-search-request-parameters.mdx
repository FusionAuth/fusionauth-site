...
<APIBlock>
  ...
  { props.query_string_request &&
    <APIField name={props.parameter_prefix + 'ids'} type="UUID" optional>
      ...
      { props.database_search_engine_type &&
        <p>Using this parameter is mutually exclusive with the **queryString** field and is not paginated or sorted using the **sortFields** field.</p>
      }

      { props.elasticsearch_search_engine_type && <>
        <p>Using this parameter is mutually exclusive with the **query** and **queryString** fields and is not paginated or sorted using **sortFields** field.</p>
  ...
  { !props.query_string_request &&
    <APIField name={props.parameter_prefix + 'ids'} type="Array<UUID>" optional>
      <p>A list of User Ids to retrieve. When using this parameter, the results are not paginated or sorted using the **sortFields**. </p>
