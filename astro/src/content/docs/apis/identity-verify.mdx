...
When using these APIs, the general workflow is to call the start API, optionally call the send API to have FusionAuth send the verification message to the User, then call the complete API based on what the User has provided to complete verification. You will not often need to call these APIs directly but the main use cases for calling these APIs are:
