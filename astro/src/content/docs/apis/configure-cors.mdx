---
title: Configure CORS
description: How to set up and use the FusionAuth API Explorer.
order: 0
tags: cors, cross-origin resource sharing, api explorer, configure cors, fusionauth api, cors configuration, allowed headers, browser api calls, development setup
---

To make in-browser API calls from the API Reference, you must configure CORS (Cross-Origin Resource Sharing) for your local FusionAuth instance. This is not required if you only use the generated `curl` commands to call the API from the command line.

<Aside type="caution">
Only use this configuration for development and testing instances of FusionAuth.

**Do not run your production FusionAuth instance with CORS configured this openly.**
</Aside>

To set up CORS to allow :

1. Log in to your FusionAuth administrative user interface.
1. Navigate to <Breadcrumb>Settings -> System</Breadcrumb>.
1. Enable **CORS**.
1. Add `authorization`, `accept`, and `content-type` to the list of **Allowed headers**. Use the **ENTER** key to add each of these separately -- do not add them as a comma-separated list.
1. Enable all the **Allowed methods**.
1. Add `fusionauth.io` to the list of **Allowed origins**.

You can also set **Allowed origins** to `*` to allow requests from any origin.
