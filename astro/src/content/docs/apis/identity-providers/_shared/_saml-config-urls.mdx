...
**Issuer:**
`<base_url>/samlv2/sp/<identityProviderId>` by default.

To override this value, set `identityProvider.issuer` on the SAML v2 identity provider.

FusionAuth expects the Audience value in the SAML response assertion to match this same value.
