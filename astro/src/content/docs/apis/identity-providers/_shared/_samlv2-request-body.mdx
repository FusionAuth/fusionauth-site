...
<APIBlock>
  ...
  <APIField name="identityProvider.issuer" type="String" optional renderif={!props.samlv2_idp_initiated}>
    The EntityId (unique identifier) FusionAuth uses as the SAML service provider.

    When not provided, FusionAuth uses the default service provider EntityId of `{'<base_url>/samlv2/sp/<identityProviderId>'}`.

    When provided, this value must not be an empty string.
  </APIField>
  ...
  <APIField name="identityProvider.keyId" type="UUID" required deprecated>
  ...
  <APIField name="identityProvider.usernameClaim" type="String" optional>
    The name of the claim in the SAML response that FusionAuth uses to identity the username. If this is not set, the **NameId** value will be used to link a user. This property is required when <b>identityProvider.linkingStrategy</b> is set to `LinkByUsername` or `LinkByUsernameForExistingUser`
  </APIField>
  <APIField name="identityProvider.verificationKeyIds" type="Array<UUID>" required since="1.69.0">
...
<If condition={!props.samlv2_idp_initiated}>
  <JSON title="Example SAML v2 Request JSON" src="identity-providers/samlv2-request.json" />
</If>

<If condition={!props.samlv2_idp_initiated}>
  <JSON title="Example SAML v2 Request JSON (with issuer override)" src="identity-providers/samlv2-request-with-issuer.json" />
</If>

<If condition={props.samlv2_idp_initiated}>
  <JSON title="Example SAML v2 IdP Initiated Request JSON" src="identity-providers/samlv2-idp-initiated-request.json" />
</If>
