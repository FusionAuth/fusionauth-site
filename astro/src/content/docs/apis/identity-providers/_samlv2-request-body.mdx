...
<APIBlock>
  ...
  <APIField name="identityProvider.issuer" type="String" optional renderif={!props.samlv2_idp_initiated}>
    ...
    When not provided, FusionAuth uses the default service provider EntityId of <code>{'<base_url>/samlv2/sp/<identityProviderId>'}</code>.

    When provided, this value must not be blank.
  </APIField>
