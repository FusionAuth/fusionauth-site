...
<APIBlock>
  ...
  <APIField name="identityProvider.useNameIdForEmail" type="Boolean" optional>
    Whether or not FusionAuth will use the `NameID` element value as the email address of the user for reconciliation processing. If this is set to <InlineFieldValue>false</InlineFieldValue>, then the `emailClaim` property must be set.
  </APIField>
