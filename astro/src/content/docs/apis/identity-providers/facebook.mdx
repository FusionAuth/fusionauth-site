...
When the <InlineField>picture</InlineField> field is not requested FusionAuth will also call Facebook's `/me/picture` API to load the user's profile image and store it as the <InlineField>imageUrl</InlineField> in FusionAuth. When the <InlineField>picture</InlineField> field is requested, the user's profile image will be returned by the `/me` API and a second request to the `/me/picture` endpoint will not be required.
