...
<API method="PUT" uri="/api/identity-provider?type=Facebook" authentication={["api-key"]} title="Update the Facebook Identity Provider by type"/>

<PatchSupport api_endpoint="/api/identity-provider`?type=Facebook`" />

<API method="PUT" uri="/api/identity-provider/56abdcc7-8bd9-4321-9621-4e9bbebae494" authentication={["api-key"]} title="Update the Facebook Identity Provider by Id"/>
