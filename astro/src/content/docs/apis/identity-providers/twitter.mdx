---
title: Twitter
description: Learn about the APIs for creating, retrieving, updating and disabling a Twitter identity provider.
section: apis
...
<API method="POST" uri="/api/identity-provider" authentication={["api-key"]} title="Create a Twitter Identity Provider (server generated Id)"/>
...
The <InlineField>type</InlineField> property in the request JSON indicates this is a Twitter Identity Provider.
...
## Retrieve Twitter Identity Provider
...
<API method="GET" uri="/api/identity-provider/{identityProviderId}" authentication={["api-key"]} title="Retrieve the Twitter Identity Provider by Id"/>
...
<API method="PUT" uri="/api/identity-provider?type=Twitter" authentication={["api-key"]} showPatch={true} title="Update the Twitter Identity Provider by type (deprecated)"/>
...
<API method="PUT" uri="/api/identity-provider/{identityProviderId}" authentication={["api-key"]} showPatch={true} title="Update the Twitter Identity Provider by Id"/>
...
Delete by type or Id. Deleting by type attempts to delete the Identity Provider with the legacy fixed Id `45bb233c-0901-4236-b5ca-ac46e2e0a5a5`.

### Request

<API method="DELETE" uri="/api/identity-provider?type=Twitter" authentication={["api-key"]} title="Delete the Twitter Identity Provider by type (deprecated)"/>
...
<API method="DELETE" uri="/api/identity-provider/{identityProviderId}" authentication={["api-key"]} title="Delete the Twitter Identity Provider by Id"/>
