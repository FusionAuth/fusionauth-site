...
<APIBlock>
  ...
  <APIField name={ props.parameter_prefix + "tenantId" } type="UUID" optional since="1.62.0">
    Restricts the results to Identity Providers belonging to the given Tenant. This parameter will be overridden if the request contains an `X-FusionAuth-TenantId` header, or if the supplied API key is scoped to a specific Tenant.
  </APIField>
