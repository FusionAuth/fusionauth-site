...
<APIBlock>
  ...
  <APIField name="identityProvider.defaultKeyId" required deprecated>
    ...
    <DeprecatedSince since="1.69.0">
        In version 1.69.0 and above, use the **identityProvider.verificationKeyIds** field. **identityProvider.defaultKeyId** will continue to be populated with the first entry from that list for backward compatibility.
    </DeprecatedSince>
  </APIField>
