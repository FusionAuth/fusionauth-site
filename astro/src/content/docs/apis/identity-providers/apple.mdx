...
<API method="PUT" uri="/api/identity-provider?type=Apple" authentication={["api-key"]} title="Update the Apple Identity Provider by type"/>

<PatchSupport api_endpoint="/api/identity-provider`?type=Apple`" />

<br/>
<API method="PUT" uri="/api/identity-provider/13d2a5db-7ef9-4d62-b909-0df58612e775" authentication={["api-key"]} title="Update the Apple Identity Provider by Id"/>
