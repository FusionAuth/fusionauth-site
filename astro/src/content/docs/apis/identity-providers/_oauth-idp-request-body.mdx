...
<APIBlock>
  ...
  <APIField name="identityProvider.scope" type="String" optional renderif={props.idp_type !== 'Steam'}>
    ...
    { props.idp_type === 'Google' && <>
      <Aside type="version" title="Since 1.44.0">
        The <InlineField>identityProvider.scope</InlineField> value may only be used when the <InlineField>identityProvider.loginMethod</InlineField> is set to <code>UseRedirect</code>. The <InlineFieldValue>email</InlineFieldValue>, <code>profile</code>, and <code>openid</code> scopes will always be requested for all other configurations.
      </Aside>
    </>}
  </APIField>
