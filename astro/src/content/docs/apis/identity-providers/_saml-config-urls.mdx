...
Note: To receive a refresh token when completing the OAuth2 workflow when using an IdP initiated login, ensure you request the <InlineFieldValue>offline_access</InlineFieldValue> scope. To request a scope, add the following query parameter to your configured ACS in your IdP, `?scope=offline_access`. When logging into the FusionAuth admin UI, this step is optional as the <InlineFieldValue>offline_access</InlineFieldValue> scope will be implicitly added.
