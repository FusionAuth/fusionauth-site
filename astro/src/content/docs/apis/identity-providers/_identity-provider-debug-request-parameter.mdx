...
<APIField name="identityProvider.debug" type="Boolean" optional defaults="false" since="1.7.3" renderif={props.idp_since < 10703}>
...
<APIField name="identityProvider.debug" type="Boolean" optional defaults="false" renderif={props.idp_since >= 10703}>
  Determines if debug is enabled for this provider. When enabled, an Event Log is created each time this provider is invoked to reconcile a login.
