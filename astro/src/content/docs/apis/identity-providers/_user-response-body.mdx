...
import ModerationStatusResponse from 'src/content/docs/apis/identity-providers/_moderation_status_response.mdx';
...
<APIBlock>
  ...
  { !!props.two_factor_login_response && <>

  <APIField name="trustToken" type="String" since="1.33.0">
  ...
  <APIField name="tokenExpirationInstant" type="Long" since="1.33.0">
    The [instant] the <InlineField>token</InlineField> will expire. If the response does not contain a [field](/docs/reference/data-types#instants)#token#, this field will also be omitted from the response.
  </APIField>
