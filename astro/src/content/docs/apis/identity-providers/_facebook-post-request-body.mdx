...
<APIBlock>
  ...
  <APIField name="identityProvider.permissions" type="String" optional defaults="email">
    ...
    <Aside type="version" title="Available since 1.11.0">
      The default value of <InlineFieldValue>email</InlineFieldValue> is now provided and stored in the database when this field is not specified.  This is a required `permissions` value for the [Facebook Login API](https://developers.facebook.com/docs/facebook-login/) to complete login. Prior to this version, the value not defaulted.
    </Aside>
  </APIField>
