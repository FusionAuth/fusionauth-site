...
<APIBlock>
  <APIField name={ props.parameter_prefix + "name" } type="String" optional>
    The case-insensitive string to search for in the Consent name. This can contain wildcards using the asterisk character (`*`). If no wildcards are present, the search criteria will be interpreted as `pass:[*value*]`.
  </APIField>
