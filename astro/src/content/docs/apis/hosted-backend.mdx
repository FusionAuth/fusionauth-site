...
These endpoints will set the following cookies which are `Secure` and have a `SameSite` value of `Lax`. This follows our [expert advice on client-side storage](/learn/expert-advice/oauth/oauth-token-storage#client-side-storage).
...
This API will start an OAuth2 Authorization Code grant by building a valid request and then redirecting the browser to our `/oauth2/authorize` endpoint. If the user is not logged in the user will be presented with the login page and prompted for credentials before being redirected back to the <ScrollRef target="Callback" /> endpoint.

To use this API, redirect the browser to this route. This is not meant to be called by non-browser clients.

### Request

<APIAuthentication icon="unlock">Start the login flow</APIAuthentication>

<APIURI method="GET">/app/login/`{clientId}?redirect_uri={redirectUri}&state={state}&scope={scope}`</APIURI>
...
<APIBlock>
  ...
  <APIField name="redirect_uri" type="String" optional>
    The URL encoded URL that the browser will be redirected to at the end of the login flow. If provided, this URL must be included in the <InlineField>Authorized Redirect URLs</InlineField> array for your application. If not provided, the default will be the first value in the <InlineField>Authorized Redirect URLs</InlineField> array configured for your application. This parameter is validated the same as if it were being passed to `/oauth/authorize`, however when using this endpoint FusionAuth will pass <ScrollRef target="Callback" /> as the <InlineField>redirect_uri</InlineField> to `/oauth2/authorize` as that route will handle the token exchange.
  </APIField>
...
| 403  | A forbidden response typically means that the Origin of this request did not pass the FusionAuth CORS filter. Add your app origin to your [CORS Configuration] as an [field](/docs/operate/secure-and-monitor/cors)#Allowed Origin#. |
| 500  | There was a FusionAuth internal error. A stack trace is provided and logged in the FusionAuth log files.                                                                                                                             |
|      |                                                                                                                                                                                                                                      |

## Register

This API will start a registration flow by building a valid request and then redirecting the browser to our `/oauth2/register` endpoint. This endpoint is nearly identical to the <ScrollRef target="Login" /> endpoint; however the end result is user registration instead of a login. If the user is not logged in the user will be presented with the registration page and prompted for credentials before being redirected back to the <ScrollRef target="Callback" /> endpoint. If the user is logged in they will be redirected to `/oauth2/authorize` and subsequently to the <ScrollRef target="Callback" /> endpoint.

[Self-service Registration](/docs/get-started/core-concepts/applications#registration) will need to be enabled otherwise this endpoint will redirect to <ScrollRef target="Login" />.

To use this API, redirect the browser to this route. This is not meant to be called by non-browser clients.

### Request

<APIAuthentication icon="unlock">Start the registration flow</APIAuthentication>

<APIURI method="GET">/app/register/`{clientId}?redirect_uri={redirectUri}&state={state}&scope={scope}`</APIURI>
...
<APIBlock>
  ...
  <APIField name="redirect_uri" type="String" optional>
    The URL encoded URL that the browser will be redirected to at the end of the registration. If provided, this URL must be included in the <InlineField>Authorized Redirect URLs</InlineField> array for your application. If not provided, the default will be the first value in the <InlineField>Authorized Redirect URLs</InlineField> array configured for your application. This parameter is validated the same as if it were being passed to `/oauth/register`, however when using this endpoint FusionAuth will pass <ScrollRef target="Callback" /> as the <InlineField>redirect_uri</InlineField> to `/oauth2/register` as that route will handle the token exchange.
  </APIField>
  <APIField name="state" type="String" optional>
...
<APIAuthentication icon="unlock">Receive the callback from the authorization flow</APIAuthentication>

<APIURI method="GET">/app/callback/`?code={code}&locale={locale}&state={state}&userState={userState}&client_id={client_id}&tenantId={tenantId}`</APIURI>
...
<APIAuthentication icon="unlock">Refresh the access token</APIAuthentication>

<APIURI method="POST">/app/refresh/`{clientId}`</APIURI>
...
<APIAuthentication icon="badge">Get info about the user</APIAuthentication>

<APIURI method="GET">/app/me/</APIURI>
...
This API will start a logout. The cookies set on <ScrollRef target="Callback" /> or <ScrollRef target="Refresh" /> will be removed. If an SSO session was started, it will be ended.
...
<APIAuthentication icon="unlock">Start the logout flow</APIAuthentication>

<APIURI method="GET">/app/logout/`{clientId}`?`redirect_uri={redirectUri}`</APIURI>
