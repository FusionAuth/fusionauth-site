...
tags: fusionauth api, rest api, http api, hosted backend, sdk backend, client credentials, backend api, oauth2, authorization code flow, token refresh
---
...
Be sure to review the [Applications](/docs/get-started/core-concepts/types/applications#oauth) section of the FusionAuth user guide to ensure proper configuration before using the hosted endpoints.
...
[Self-service Registration](/docs/get-started/core-concepts/types/applications#registration) will need to be enabled otherwise this endpoint will redirect to [Login](#login).
