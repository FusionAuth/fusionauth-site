...
<Aside type="caution">
Documentation for this API has been updated for version 1.30 and above. For previous versions, you can view the [Pre 1.30 System documentation](/docs/v1/tech/archive/apis/system) if needed.
</Aside>
...
<APIURI method="GET">/api/system/log/export`?dateTimeSecondsFormat={dateTimeSecondsFormat}&includeArchived=true&zoneId={zoneId}`</APIURI>
...
<br/>
<APIAuthentication icon="lock">Export the System Logs</APIAuthentication>
<APIURI method="POST">/api/system/log/export</APIURI>
...
<APIAuthentication icon="undefined">link:/docs/v1/tech/apis/authentication#basic-authentication-using-an-api-key[icon:shield-alt[type=fas]] link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Retrieve FusionAuth application metrics to use with Prometheus.</APIAuthentication>
...
```text title="Example Prometheus Response"
