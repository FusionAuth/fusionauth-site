...
<ApplicationOauthconfigurationClientauthenticationpolicy />
  ...
  <APIField name="oauthConfiguration.consentMode" type="String" since="1.50.0">
    Controls the policy for prompting a user to consent to requested OAuth scopes. This configuration only takes effect when <InlineField>oauthConfiguration.relationship</InlineField> is <InlineFieldValue>ThirdParty</InlineFieldValue>.
