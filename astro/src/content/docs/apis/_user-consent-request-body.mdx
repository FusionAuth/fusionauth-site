...
<APIBlock>
  ...
  <APIField name="userConsent.status" type="String" required renderif={props.update}>
    ...
    Setting the status to <InlineFieldValue>Revoked</InlineFieldValue> when the current status is `Active` will revoke the User Consent. Setting the status to <InlineFieldValue>Active</InlineFieldValue> when the current status is `Revoked` will Un-revoke the User Consent and make active again.
  </APIField>
