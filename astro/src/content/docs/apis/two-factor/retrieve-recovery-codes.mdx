---
title: Retrieve Recovery Codes
htmlTitle: Retrieve Recovery Codes API
description: API documentation for the FusionAuth Retrieve Recovery Codes API.
tags: fusionauth api, rest api, http api, multi-factor authentication, mfa, two-factor authentication, recovery codes, backup codes
order: 1000
---
import StandardPostResponseCodes from 'src/content/docs/apis/_shared/_standard-post-response-codes.astro';

This API is used to retrieve Recovery Codes for a User.


## Request

<API method="GET" uri="/api/user/two-factor/recovery-code/{userId}" authentication={["api-key"]} title="Retrieve Recovery Codes" deprecated/>

<DeprecatedSince since="1.64.0">This endpoint is deprecated. To provide access to a user's recovery codes, [generate new recovery codes](/docs/apis/two-factor/generate-recovery-codes) and deliver those</DeprecatedSince>

<Aside type="caution" title="Always returns empty list since 1.68.0">
  As of `1.68.0`, this endpoint always returns an empty list. Recovery codes are now hashed at rest and cannot be retrieved in plaintext. To provide a user with recovery codes, generate a new set.
</Aside>

### Request Parameters

<APIBlock>
  <APIField name="userId" type="UUID" required>
    The Id of the User to retrieve Recovery Codes for.
  </APIField>
</APIBlock>

## Response

The response for this API contains the remaining Recovery Codes that are assigned to the User. Each time one is used it is removed, so this response will contain between 0 and 10 codes. Prior to `1.68.0`, this returned remaining codes. As of `1.68.0`, this always returns an empty list.

<StandardPostResponseCodes never_search_error />

### Response Body

<APIBlock>
  <APIField name="recoveryCodes" type="Array<String>">
    The array of Recovery Codes.
  </APIField>
</APIBlock>

<JSON title="Example Response JSON" src="two-factor/response-with-recovery-codes.json" />
