...
<APIBlock>
  ...
  <APIField name={props.parameter_prefix + "ids"} type="UUID" optional renderif={!!props.query_string_request}>
    ...
    {props.database_search_engine_type &&
        <> Using this parameter is mutually exclusive with the <b>queryString</b> field and is not paginated or sorted using the <b>sortFields</b> field. </>
    }
    {props.elasticsearch_search_engine_type &&
        <>
          Using this parameter is mutually exclusive with the <b>query</b> and <b>queryString</b> fields and is not paginated or sorted using <b>sortFields</b> field.
  ...
  <APIField name={props.parameter_prefix + "ids"} type="Boolean" optional defaults="false" renderif={!props.query_string_request}>
    A list of Entity Ids to retrieve. When using this parameter, the results are not paginated or sorted using the **sortFields**.
  ...
  <APIField name={props.parameter_prefix + "query"} type="String" optional renderif={!!props.elasticsearch_search_engine_type}>
    ...
    It is necessary to use the **{props.parameter_prefix}query** parameter when querying against **grants** in order to achieve expected results, as this field is defined as a [nested datatype](https://www.elastic.co/guide/en/elasticsearch/reference/6.3/nested.html) in the Elasticsearch mapping.
  </APIField>
