...
<DeviceTypeList />
  ...
  <APIField name="validateDbConstraints" type="Boolean" optional defaults="false">
    Set this value to <InlineFieldValue>true</InlineFieldValue> in order to perform additional validation of the request. This validation includes, among other things, confirming that a user exists and is registered for the application which this `refreshToken` was created for.
