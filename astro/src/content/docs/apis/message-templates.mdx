...
FusionAuth provides sample values for the `${code}`, `${verificationId}`, and `${verificationOneTimeCode}` parameters when previewing. The values for these parameters will always be `123456`, `987654`, and `abcdef` respectively. The `${code}` parameter is used to deliver a two factor code to the user while `${verificationId}` and `${verificationOneTimeCode}` are used for phone number verification messages.
