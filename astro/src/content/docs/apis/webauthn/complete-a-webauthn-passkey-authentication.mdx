---
title: Complete a WebAuthn Passkey Authentication
htmlTitle: Complete a WebAuthn Passkey Authentication API
description: API documentation for the FusionAuth Complete a WebAuthn Passkey Authentication API.
tags: fusionauth api, rest api, http api, webauthn, passkey, fido2, passkey authentication, biometric authentication
order: 700
---
import LoginResponseCodes from 'src/content/docs/apis/_shared/_login-response-codes.mdx';
import UserResponseBody from 'src/content/docs/apis/_shared/_user-response-body.mdx';
import WebauthnAuthenticateCompleteRequestBody from 'src/content/docs/apis/webauthn/_webauthn-authenticate-complete-request-body.mdx';
import XFusionauthTenantIdHeaderScopedOperation from 'src/content/docs/apis/_shared/_x-fusionauth-tenant-id-header-scoped-operation.mdx';

This API is used to complete a WebAuthn authentication ceremony by providing the values returned from the [WebAuthn JavaScript API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API) call. The API will validate the request against configured passkey requirements for the workflow and the one-time challenge generated and returned by [Start a WebAuthn Passkey Assertion or Authentication](/docs/apis/webauthn/start-a-webauthn-passkey-assertion-or-authentication).

## Request

<API method="POST" uri="/api/webauthn/login" authentication={["none"]} title="Complete a WebAuthn Authentication Ceremony"/>

### Request Headers

<XFusionauthTenantIdHeaderScopedOperation />

### Request Cookies

<APIBlock>
  <APIField name="fusionauth.trust" type="String" optional>
    The Multi-Factor Trust identifier returned by the Multi-Factor Login API response. This value may be provided to bypass the Multi-Factor challenge when a User has Multi-Factor enabled. When this cookie exists on the request it will take precedence over the **twoFactorTrustId** if provided in the request body.
  </APIField>
</APIBlock>

### Request Body

<WebauthnAuthenticateCompleteRequestBody />

## Response

The response for this API contains the User object.

<LoginResponseCodes />

### Response Body

<UserResponseBody login_response />
