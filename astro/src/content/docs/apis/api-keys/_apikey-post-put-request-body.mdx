...
<APIBlock>
  <APIField name="apiKey.expirationInstant" type="Long" optional since="1.55.0">
    ...
    <If condition={props.apikey_update_request}>
...
<If condition={props.apikey_create_request}>
  <JSON title="Example API Key request JSON" src="api-keys/post-request-body.json" />
</If>

<If condition={props.apikey_update_request}>
  <JSON title="Example API Key request JSON" src="api-keys/put-request-body.json" />
</If>
