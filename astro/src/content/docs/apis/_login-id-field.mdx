...
<APIField name="loginId" type="String" required={!props.optional}>
  The login identifier of the {props.user_label || 'user'}. The login identifier can be one of the following (see <InlineField>{props.login_id_type_field}</InlineField> for information on which of these identifiers will be used):
  * Email address
  * Phone number (if `phoneNumber` is included in <InlineField>{props.login_id_type_field}</InlineField>)
  * Username

  <Aside type="note" renderif={!props.hide_details}>
