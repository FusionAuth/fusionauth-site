...
<APIBlock>
  ...
  <APIField name={ props.base_field_name + ".jwtConfiguration.accessTokenVerificationKeyIds" } type="UUID" since="1.69.0">
      The list of access token verification key Ids that are trusted by this application. **application.jwtConfiguration.accessTokenKeyId** is implicitly included in this list and is not explicitly specified.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".jwtConfiguration.idTokenVerificationKeyIds" } type="UUID" since="1.69.0">
    The list of Id token verification key Ids that are trusted by this application. **application.jwtConfiguration.idTokenKeyId** is implicitly included in this list and is not explicitly specified.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".oauthConfiguration.authorizedURLValidationPolicy" } type="String" since="1.43.0">
    Controls the validation policy for **{props.base_field_name}.oauthConfiguration.authorizedOriginURLs** and **{props.base_field_name}.oauthConfiguration.authorizedRedirectURLs**.
  ...
  <APIField name={ props.base_field_name + ".oauthConfiguration.consentMode" } type="String" since="1.50.0">
    Controls the policy for prompting a user to consent to requested OAuth scopes. This configuration only takes effect when **{props.base_field_name + ".oauthConfiguration.relationship"}** is `ThirdParty`.
  ...
  <APIField name={ props.base_field_name + ".oauthConfiguration.relationship" } type="String" since="1.50.0">
    ...
    * `ThirdParty` - The application is external to the authorization server. Users will be prompted to consent to requested OAuth scopes based on **{props.base_field_name + ".oauthConfiguration.consentMode"}**.
  </APIField>
  <APIField name={ props.base_field_name + ".oauthConfiguration.requireClientAuthentication" } type="Boolean" since="1.3.0" deprecated>
    ...
    In version 1.28.0 and beyond, client authentication can be managed via **{props.base_field_name}.oauthConfiguration.clientAuthenticationPolicy**.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".registrationConfiguration.completeRegistration" } type="Boolean" since="1.65.0">
  ...
  <APIField name={ props.base_field_name + ".registrationDeletePolicy.unverified.enabled" } type="Boolean" since="1.13.0">
    Indicates that users without a verified registration for this application will have their registration permanently deleted after **{props.base_field_name}.registrationDeletePolicy.unverified.numberOfDaysToRetain** days.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".samlv2Configuration.defaultVerificationKeyId" } type="UUID" deprecated since="1.20.0">
    The verification key used to verify a signature when the SAML v2 Service Provider is using HTTP Redirect Bindings.

    When HTTP POST Bindings are used, this is the default verification key used if a `[KeyInfo]` element is not found in the SAML AuthNRequest. If a `[KeyInfo]` element is found, Key Master will be used to resolve the key but the key must still match this value.

    <DeprecatedSince since="1.69.0">
      In version 1.69.0 and beyond, use the **application.samlv2Configuration.verificationKeyIds** field. **application.samlv2Configuration.defaultVerificationKeyId** will continue to be populated with the first entry from that list for backward compatibility.
    </DeprecatedSince>
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".samlv2Configuration.logout.defaultVerificationKeyId" } type="UUID" deprecated since="1.25.0">
    ...
    <DeprecatedSince since="1.69.0">
      In version 1.69.0 and beyond, use the **application.samlv2Configuration.logout.verificationKeyIds** field. **application.samlv2Configuration.logout.defaultVerificationKeyId** will continue to be populated with the first entry from that list for backward compatibility.
    </DeprecatedSince>
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".samlv2Configuration.logout.verificationKeyIds" } type="Array<UUID>" since="1.69.0">
    The unique Ids of the Keys used to verify the signature if the public key cannot be determined by the `KeyInfo` element when using POST bindings, or the keys used to verify the signature when using HTTP Redirect bindings.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".samlv2Configuration.verificationKeyIds" } type="Array<UUID>" since="1.69.0">
      The unique Ids of the Keys used to verify the signature if the public key cannot be determined by the `KeyInfo` element when using POST bindings, or the keys used to verify the signature when using HTTP Redirect bindings.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".webAuthnConfiguration.bootstrapWorkflow.enabled" } type="Boolean" since="1.41.0">
    Whether the WebAuthn bootstrap workflow is enabled for this application. This overrides the tenant configuration. Has no effect if **{props.base_field_name}.webAuthnConfiguration.enabled** is `false`.
  </APIField>
  ...
  <APIField name={ props.base_field_name + ".webAuthnConfiguration.reauthenticationWorkflow.enabled" } type="Boolean" since="1.41.0">
    Whether the WebAuthn reauthentication workflow is enabled for this application. This overrides the tenant configuration. Has no effect if **{props.base_field_name}.webAuthnConfiguration.enabled** is `false`.
  </APIField>
  <APIField name="expandable" type="Array<String>" since="1.50.0" renderif={!!props.search_result}>
    ...
    For example, if you set the **expand** request parameter to `[roles]` then the value of this parameter in the response will be `[scopes]` indicating that the **scopes** property was not expanded.
  </APIField>
