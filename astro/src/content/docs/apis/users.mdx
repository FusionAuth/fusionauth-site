...
This API is used to validate whether a [change the user's password](#change-a-users-password) request to change a user's password will require MFA (via a `trustToken`). There are 3 ways to call the endpoint:
...
The first case will also verify that a `changePasswordId` is valid. This usage is generally intended to be part of an email or SMS workflow and does not require authentication. The `changePasswordId` used on this API request will have been previously generated by the [Start Forgot Password API](#start-forgot-password-workflow) or by using the Forgot Password workflow on the FusionAuth login page. Use this API to validate the `changePasswordId` before submitting a POST request with the `changePasswordId`.
...
The "Validate user password change using a `loginId`" or JWT cases are used to verify whether MFA is required to change a user's password based solely on the user's `loginId` or JWT. It does not validate a specific `changePasswordId` and is instead meant to validate whether MFA is required before submitting a [change the user's password](#change-a-users-password) request with the user's `loginId` or JWT.
