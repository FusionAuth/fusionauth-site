...
<APIBlock>
  ...
  <APIField name="sendForgotPasswordEmail" type="Boolean" optional defaults="true">
    Whether or not calling this API should attempt to send the user an email using the configured Forgot Password email template. Setting this to <InlineFieldValue>false</InlineFieldValue> will begin the Forgot Password workflow without sending an email and only create the `changePasswordId`, this may be useful if you want to send an email outside of FusionAuth, or complete this workflow without the use of email.
