...
This API is used to validate whether a POST request to change a user's password will require MFA (via a `trustToken`). There are 3 ways to call the endpoint:
1. Validate a `changePasswordId` without authentication
2. Validate a `changePasswordId` using a JWT
3. Validate user password change using a `loginId`

The first two cases will also verify that a `changePasswordId` is valid. This usage is generally intended to be part of an email or SMS workflow and does not require authentication. The `changePasswordId` used on this API request will have been previously generated by the Start Forgot Password API or by using the Forgot Password workflow on the FusionAuth login page. Use this API to validate the `changePasswordId` before submitting a POST request with the `changePasswordId`.

<API method="GET" uri="/api/user/change-password/{changePasswordId}" authentication={["none", "jwt"]} title="Validate a changePasswordId" />
...
The "Validate user password change using a `loginId`" case is used to verify whether MFA is required to change a user's password based solely on the user's `loginId`. It does not validate a specific `changePasswordId` and is instead meant to validate whether MFA is required before submitting a POST request with the user's `loginId`.
