...
This API is used to bulk import multiple Users into FusionAuth. Each User must have at least an **email** or a **username**, and a **password**.

The **password** should be Base64 bytes of the password hash from the system you are importing from. If you don't have access to your user hashes, set **password** to a random plaintext string, like a UUID. This will force users to complete the Forgot Password flow when trying to log in.

This request is useful for migrating data from an existing database into FusionAuth. Additionally, you can provide an Id for each User inside the JSON User object of the request body.
