---
title: Users
htmlTitle: FusionAuth User API Documentation
...
<APIBlock>
  ...
  <LoginIdTypeField plural optional since="1.59.0" />
...
### changePasswordId Request

This API is used to verify that a `changePasswordId` is valid before making the actual password change with the POST API. This usage is generally intended to be part of an email or SMS workflow and does not require authentication. The `changePasswordId` used on this API request will have been previously generated by the Start Forgot Password API or by using the Forgot Password workflow on the FusionAuth login page. Use this API to validate the `changePasswordId` before submitting a POST request with the `changePasswordId`.

<API method="GET" uri="/api/user/change-password/{changePasswordId}" authentication={["none"]} title="Validate a changePasswordId" />
<API method="GET" uri="/api/user/change-password/{changePasswordId}" authentication={["jwt"]} title="Validate a changePasswordId using a JWT"/>
...
<ChangePassGetResponseCodes requestParameter="<code>changePasswordId</code>" notFoundText="is incorrect or expired"/>

### loginId Request

This API is used to verify whether MFA is required to change a user's password based solely on the user's `loginId`. It does not validate a specific `changePasswordId` and is instead meant to validate whether MFA is required before submitting a POST request with the user's `loginId`.

<API method="GET" uri="/api/user/change-password?loginId={loginId}" authentication={["api-key"]} title="Validate user password change status using a loginId" />

#### Request Parameters

<APIBlock>
    <APIField name="loginId" type="String" required>
        The login identifier of the user. The login identifier can be one of the following:
        * Email address
        * Username
    </APIField>
</APIBlock>
...
<ChangePassGetResponseCodes requestParameter="user"/>
...
This usage is generally intended to be part of an email or SMS workflow and does not require authentication. The `changePasswordId` used on this API request will have been previously generated by the Start Forgot Password API or by using the Forgot Password workflow on the FusionAuth login page.
