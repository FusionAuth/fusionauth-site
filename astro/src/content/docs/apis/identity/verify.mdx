...
When using these APIs, the general workflow is to call the start API, optionally call the send API to have FusionAuth send the verification message to the user, then call the complete API based on what the user has provided to complete verification. You will not often need to call these APIs directly but the main use cases for these APIs are:

| Use Case                                              | Start API                | Send API                | Complete API                                                                                                   |
|-------------------------------------------------------|--------------------------|-------------------------|----------------------------------------------------------------------------------------------------------------|
| User creation                                         | (handled by FusionAuth)  | (handled by FusionAuth) | N/A                                                                                                            |
| User update                                           | (handled by FusionAuth)  | (handled by FusionAuth) | N/A                                                                                                            |
| Hosted Pages - login                                  | N/A                      | N/A                     | (handled by FusionAuth)                                                                                        |
| Hosted Pages - resend verification                    | (handled by FusionAuth)  | (handled by FusionAuth) | N/A                                                                                                            |
| Login API                                             | N/A                      | N/A                     | Yes, if you want to use `FormField` verification or if you don't want to use FusionAuth's `ClickableLink` page |
| Login API - resending verification emails or SMS msgs | Yes                      | Yes                     | N/A                                                                                                            |

## Start Identity Verification

This API allows you to generate a verification code for a user's identity. This code can be sent to the user via email or SMS using the [Send Identity Verification API](#send-identity-verification). The user will then use this code to complete the verification process.
...
<APIBlock>
  ...
  <APIField name="verificationStrategy" type="String" optional defaults="tenant value">
    ...
    * `FormField` - send the user a short code intended to be manually entered into a form field
    ...
    The default, when `loginId` is an email address, is <InlineField>tenant.emailConfiguration.verificationStrategy</InlineField>. When `loginId` is a phone number, the default is <InlineField>tenant.phoneConfiguration.verificationStrategy</InlineField>.
  </APIField>
</APIBlock>
...
* an email identity - the message will be sent via email using the tenant's SMTP settings and <InlineField>tenant.emailConfiguration.verificationEmailTemplateId</InlineField>
* a phone number identity - the message will be sent via SMS using <InlineField>tenant.phoneConfiguration.verificationTemplateId</InlineField> and <InlineField>tenant.phoneConfiguration.messengerId</InlineField>
...
<APIBlock>
  <APIField name="verificationId" type="String" required>
    The verification Id generated by the [Start Identity Verification API](#start-identity-verification), used to verify the User's account is valid by ensuring they have access to the provided email address or phone number.
