...
<APIBlock>
  ...
  <APIField name="messageType" type="String" optional since="1.65.0" defaults="SMS">
    ...
    This value is only used when <InlineField>method</InlineField> is equal to `sms`.
...
<APIBlock>
  ...
  <APIField name="messageType" type="String" optional since="1.65.0" defaults="SMS">
    ...
    This value is only used when the method identified by <InlineField>methodId</InlineField> is an `sms` method.
...
The response for this API contains an array of Recovery Codes that were created. In standard mode, 10 codes are returned. In FIPS mode, 14 codes are returned.

<Aside type="note">
  The returned codes are the only time they are available in plaintext — they are hashed at rest and cannot be retrieved afterward. Store them securely and provide them to the user immediately. Since `1.67.0`.
</Aside>
...
<Aside type="caution" title="Always returns empty list since 1.67.0">
  As of `1.67.0`, this endpoint always returns an empty list. Recovery codes are now hashed at rest and cannot be retrieved in plaintext. To provide a user with recovery codes, generate a new set.
</Aside>
...
The response for this API contains the remaining Recovery Codes that are assigned to the User. Each time one is used it is removed, so this response will contain between 0 and 10 codes. Prior to `1.67.0`, this returned remaining codes. As of `1.67.0`, this always returns an empty list.
