...
<Aside>
    Since version 1.62.0, using the POST method of this endpoint is preferable because it provides more information to any MFA requirement lambda functions that may be configured.
</Aside>
...
<APIBlock>
    ...
    <APIField name="token" type="String" optional>
        An access token (not an Id token) returned by FusionAuth as the result of a successful authentication. The claims from this token will be available in `context.jwt` in an MFA requirement lambda function. See the [MFA requirement lambda](/docs/extend/code/lambdas/mfa-requirement) documentation. This must be a valid token that was issued and signed by FusionAuth.
    </APIField>
    <APIField name="twoFactorTrustId" type="String" optional>
        The existing multi-factor trust obtained by completing a multi-factor login. This is the value that allows you to bypass multi-factor during the next login attempt.
    </APIField>
