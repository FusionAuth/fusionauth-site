...
When using the <code>consent</code> type, the field <b>consentId</b> is required and must be included in the request.

The data type can only be set for custom fields, and if not provided the default value will be <code>string</code>. For all managed fields, such as <code>user.firstName`</code>, the data type will be set by FusionAuth and it may not be modified.
</>}
