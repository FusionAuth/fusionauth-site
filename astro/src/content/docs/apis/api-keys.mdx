...
This API is used to create a new API Key. An API key with key manager permission set to <InlineFieldValue>true</InlineFieldValue> can create keys. An API key that is tenant scoped can create another key for the same tenant.
