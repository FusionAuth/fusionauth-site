...
<APIBlock>
  ...
  <If condition={props.elasticsearch_search_engine_type}>
    <APIField name={props.parameter_prefix + 'query'} type="String" optional since="1.13.0">
      ...
      It is necessary to use the **{props.parameter_prefix}query** parameter when querying against `registrations` in order to achieve expected results, as this field is defined as a [nested datatype](https://www.elastic.co/guide/en/elasticsearch/reference/6.3/nested.html) in the Elasticsearch mapping.
    </APIField>
  </If>
  ...
  <If condition={props.elasticsearch_search_engine_type}>
    <APIField name={props.parameter_prefix + 'queryString'} type="String" optional>
      ...
      You may search against specific fields like so: `email:*fusionauth.io`. This will match only users with a `fusionauth.io` email address. Here are [the available fields for matching](/docs/reference/user-fields).
