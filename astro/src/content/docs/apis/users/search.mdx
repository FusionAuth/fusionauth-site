...
htmlTitle: Search for Users API
description: API documentation for the FusionAuth Search for Users API.
tags: fusionauth api, rest api, http api, user management, user api, user search, search users, elasticsearch
order: 1000
---
import StandardGetResponseCodes from 'src/content/docs/apis/_shared/_standard-get-response-codes.astro';
import UserSearchRequestBodyDatabaseExamples from 'src/content/docs/apis/users/_user-search-request-body-database-examples.mdx';
import UserSearchRequestBodyElasticsearchExamples from 'src/content/docs/apis/users/_user-search-request-body-elasticsearch-examples.mdx';
import UserSearchRequestParameters from 'src/content/docs/apis/users/_user-search-request-parameters.mdx';
import UsersResponseBody from 'src/content/docs/apis/users/_users-response-body.mdx';
...
Which search query parameters are available and how they behave depends on the search engine type. Read more about [the different types of search engines](/docs/get-started/core-concepts/types/users#user-search).

## Database Search Engine

This is a good choice for [smaller installs, embedded scenarios, or other places where the additional capability of Elasticsearch is not required](/docs/get-started/core-concepts/types/users#database-search-engine).

<AvailableSince since="1.16.0"/>
...
Use a `POST` call to instead send search parameters in the request body.
...
The Elasticsearch engine has [advanced querying capabilities and better performance](/docs/get-started/core-concepts/types/users#elasticsearch-search-engine). You can also review the [Elasticsearch search guide](/docs/lifecycle/manage-users/search/user-search-with-elasticsearch) for more examples.
