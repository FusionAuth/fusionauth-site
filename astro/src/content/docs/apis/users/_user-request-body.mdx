...
import UserSendSetPasswordEmail from 'src/content/docs/apis/_shared/_user-send-set-password-email.mdx';
import SendSetPasswordIdentityType from 'src/content/docs/apis/_shared/_user-send-set-password-identity-type.mdx';
import UserDataEmailFieldRequest from 'src/content/docs/apis/_shared/_user-data-email-field-request.mdx';
import ModerationStatusRequest from 'src/content/docs/apis/_shared/_moderation_status_request.mdx';
import VerificationIds from 'src/content/docs/apis/_shared/_verificationIds.mdx';
...
<APIBlock>
  ...
  <If condition={props.http_method === 'PUT'}>
  ...
  <If condition={props.http_method === 'PUT'}>
    <APIField name="passwordFieldType" type="String" since="1.69.0" optional defaults="PLAINTEXT">
      ...
      <ul>
        <li>`HASHED` - **user.password** is an already hashed value. When this value is supplied, **user.encryptionScheme**, **user.factor**, **user.password**, and **user.salt** are required.</li>
        <li>`PLAINTEXT` - **user.password** is a plain text/unhashed value. FusionAuth will hash this value according to **user.encryptionScheme** and **user.factor**.</li>
      </ul>
    </APIField>
  </If>

  <If condition={props.http_method === 'POST'}>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    <If condition={props.http_method === 'PUT'}>
      In version `1.69.0` and above, the `passwordFieldType` field indicates whether this is a plain text password or a hashed password. Providing `user.password` indicates that you wish to change the User's password. If you do not want to change the User's password, omit this field from the request.
    </If>
  </APIField>
