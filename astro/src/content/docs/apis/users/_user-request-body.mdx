...
<APIBlock>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    { props.http_method === 'PUT' &&
        <p>
          In version <code>1.69.0</code> and above, the <code>passwordFieldType</code> field indicates whether this is a plain text password or a hashed password. Providing <code>user.password</code> indicates that you wish to change the User's password. If you do not want to change the User's password, omit this field from the request.
        </p>
      }
  </APIField>
