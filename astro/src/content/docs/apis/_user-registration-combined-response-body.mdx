...
<APIBlock>
  <APIField name="refreshToken" type="String" since="1.17.0" renderif={!!props.registration_create_response}>
    The refresh token that can be used to obtain a new access token once the provided one has expired. A refresh token will be generated and returned if <InlineField>application.loginConfiguration.generateRefreshTokens</InlineField> is <InlineFieldValue>true</InlineFieldValue> for the corresponding application.
  </APIField>
