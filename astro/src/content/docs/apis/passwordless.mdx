...
<APIAuthentication icon="lock">Generate a passwordless login code</APIAuthentication>
<APIURI method="POST">/api/passwordless/start</APIURI>
...
<APIAuthentication icon="unlock">Send the provided code via email</APIAuthentication>

<Aside type="version">
Available Since Version 1.11.0.
</Aside>

<APIURI method="POST">/api/passwordless/send</APIURI>
...
<APIBlock>
  <APIField name="code" type="String" required since="1.11.0">
    The unique code to send via email, used to complete the login request.  This value can be generated with a call to the <ScrollRef target="Start Passwordless Login" /> API.
  </APIField>
</APIBlock>

<JSON title="Example Request JSON" src="passwordless/send-request-code.json" />

<APIAuthentication icon="unlock">Generate a code and send a login link via email</APIAuthentication>
...
<APIAuthentication icon="unlock">Login using the code sent via email</APIAuthentication>
<APIURI method="POST">/api/passwordless/login</APIURI>

#### Request Headers

<APIBlock>
  <APIField type="String" optional>
