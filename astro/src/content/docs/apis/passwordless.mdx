...
<APIBlock>
  ...
  <APIField name="loginStrategy" type="String" optional since="1.59.0">
    ...
    * `FormField` - send the user a short code intended to be manually entered into a form field.
    ...
    <Aside>
      If not specified, prior to version 1.64.0, the default is based on the `loginIdType` of `loginId`:
      * email address or username - `ClickableLink`
      * phone number - `FormField`
    </Aside>
    <Aside>
      If not specified, in version 1.64.0 or greater, the default is based on the `loginIdType` of `loginId` and:
