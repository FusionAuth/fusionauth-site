...
import UserSendSetPasswordPhone from 'src/content/docs/apis/_user-send-set-password-phone.mdx';
...
<APIBlock>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    { props.http_method === 'POST' &&
        <p>
          If <code>tenant.passwordEnabled</code> is true, this field is required unless <code>sendSetPasswordEmail</code> or  <code>sendSetPasswordPhone</code> is set to <code>true</code>. By default, <code>sendSetPasswordEmail</code> and <code>sendSetPasswordPhone</code> are <code>false</code>.
        </p>
      }
