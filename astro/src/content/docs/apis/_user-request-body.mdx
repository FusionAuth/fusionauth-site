...
<APIBlock>
  ...
  { props.http_method === 'PUT' && <>
        <APIField name="currentPassword" type="String" optional>
            The User's current password. When this parameter is provided, the parameter will be checked against the User's actual current password. Useful for password updates.
        </APIField>
    </>
  }
  ...
  { props.http_method === 'PUT' &&
    <APIField name="passwordFieldType" type="String" since="1.69.0" optional defaults="PLAINTEXT">
        Describes what **user.password** in the request is.
        ...
        <table>
            <thead>
                <tr>
                    <th><code>passwordFieldType</code> value</th>
                    <th><code>user.password</code> interpreted as</th>
                    <th>When set to this value, these other fields are required</th>
                </tr>
            </thead>
            <tbody>
            <tr>
                <td><code>HASHED</code></td>
                <td>an already hashed value. FusionAuth will store exactly this value.</td>
                <td><code>user.encryptionScheme</code>, <code>user.factor</code>, <code>user.password</code>, and <code>user.salt</code></td>
            </tr>
            <tr>
                <td><code>PLAINTEXT</code></td>
                <td>a plain text/unhashed value. FusionAuth will hash and store this value according to <code>user.encryptionScheme</code> and <code>user.factor</code>.</td>
                <td>N/A</td>
            </tr>
            </tbody>
        </table>
        ...
        Option B:
        The possible values will cause **user.password** to be interpreted as:
        <ul>
            <li>`HASHED` - an already hashed value. When this value is supplied, </li>
            <li>`PLAINTEXT` - a plain text/unhashed value. FusionAuth will hash this value according to **user.encryptionScheme** and **user.factor**.</li>
        </ul>
    </APIField>
  }
  ...
  <APIField name="skipVerification" type="Boolean" optional defaults="false">
    ...
    * Set **user.identities\[x].verified** field to `false`
    * Set **user.identities\[x].verifiedReason** field to `Skipped`, which will exempt this identity from any verification gating requirements configured at the tenant level
    * Set **user.verified** field to `true` (if an email identity was created or added)
  </APIField>
  ...
  <APIField name="user.legacyIdentifier" type="String" optional since="1.67" immutable>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    { props.http_method === 'PUT' &&
        <p>
          In version 1.69.0 and above, the <code>passwordFieldType</code> field is used to indicate whether this is a plain text password or a hashed password. Regardless, if <code>user.password</code> is provided it indicates you wish to change the User's password. If you do not want to change the User's password omit this field from the request.
        </p>
      }
  </APIField>
