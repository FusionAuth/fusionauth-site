...
<APIBlock>
  ...
  <APIField name="user.password" type="String" optional>
    ...
    { props.http_method === 'POST' &&
        <p>
          This field is required unless `sendSetPasswordEmail` is `true`, or `sendSetPasswordIdentityType` is set to something besides `doNotSend`. Note that by default, `sendSetPasswordIdentityType` is `doNotSend`.
        </p>
      }
