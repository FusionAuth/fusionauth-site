...
<APIBlock>
  ...
  <APIField name="userAction.preventLogin" type="Boolean" optional defaults="false">
    Whether or not this User Action will prevent user login. When this value is set to <InlineFieldValue>true</InlineFieldValue> the action must also be marked as a time based action. See <InlineField>temporal</InlineField>.
  </APIField>
