...
<UserDataEmailFieldImportRequest />
  ...
  <APIField name="users[x].password" type="String" required>
    ...
    Please see the <ScrollRef target="Password Hashes" /> section for more information about importing particular password hashes.
  </APIField>
  ...
  <APIField name="users[x].twoFactorEnabled" type="Boolean" optional defaults="false">
    ...
    See the [Enable Two Factor](/docs/apis/messengers/two-factor#enable-multi-factor) and [Disable Two Factor](/docs/apis/messengers/two-factor#disable-multi-factor) APIs as an alternative to performing this action using the User API.
