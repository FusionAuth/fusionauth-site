...
<APIField name="users[x].twoFactor.recoveryCodes" type="Array<String>" optional>
  ...
  Since `1.67.0`, plaintext recovery codes provided here will be hashed during import. For large imports with many users having recovery codes, this will increase import time. To avoid this, pre-hash recovery codes and provide `users[x].twoFactor.recoveryCodeEncryptionScheme` and `users[x].twoFactor.recoveryCodeWorkFactor`.
</APIField>

<APIField name="users[x].twoFactor.recoveryCodeEncryptionScheme" type="String" optional since="1.67.0">
...
<APIField name="users[x].twoFactor.recoveryCodeWorkFactor" type="Integer" optional since="1.67.0">
