...
<LoginMetadataDevice />
  ...
  <APIField name="noJWT" type="Boolean" optional defaults="false">
    When this value is set to <InlineFieldValue>true</InlineFieldValue> a JWT will not be issued as part of this request. The response body will not contain the `token` or <InlineField>refreshToken</InlineField> fields, and the `access_token` and `refresh_token` cookies will not be written to the HTTP response.
...
<APIBlock>
  ...
  <APIField name="fusionauth.trust" type="String">
    The Multi-Factor Trust identifier. This value is returned when `trustComputer` was set to <InlineFieldValue>true</InlineFieldValue> on the request. This value can be used by
