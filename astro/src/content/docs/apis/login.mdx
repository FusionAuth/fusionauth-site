...
<APIBlock>
  ...
  <APIField name="botDetectionScore" type="Double" optional defaults="0.0" since="1.68.0">
    Indicates the risk of the user not being human. Acceptable values range from 0.0 (human) to 1.0 (bot). The value feeds into the Intelligent MFA signals.
  </APIField>
...
<LoginMetadataDevice />
  ...
  <APIField name="newDevice" type="Boolean" optional defaults="false">
    Indicates the user is logging in with a new device. This value is used to determine if an event should be triggered and feeds into the Intelligent MFA signals.
  </APIField>
