...
<APIBlock>
  ...
  <APIField name="loginIdTypes" type="Array<String>" optional defaults="[email, username]" since="1.99.9">
    List of types that FusionAuth will attempt to resolve a user identity with, using <InlineField>user.identities\[x].type</InlineField> and <InlineField>loginId</InlineField>. Can be one or more of the following:
