...
<APIBlock>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    ...
    To provide multiple URIs, pass each URI as a separate parameter with the same name.

    All provided values must be present in the application's **oauthConfiguration.authorizedResourceUris**; any invalid value returns an `invalid_target` error.

    When **oauthConfiguration.authorizedResourceUris** is empty, this parameter is silently ignored. Otherwise, the validated resource URIs are bound to the authorization code and carried forward to the token endpoint.
  </APIField>
