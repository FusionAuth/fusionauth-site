...
Create a SCIM Group by sending the Group attributes in the request body.
...
Retrieve a SCIM Group by its Id.
...
Retrieve a paginated list of SCIM Groups, optionally filtered.
...
Update a SCIM Group by sending the full set of attributes to replace the existing Group.
