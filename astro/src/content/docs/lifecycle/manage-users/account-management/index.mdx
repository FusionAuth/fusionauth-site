...
description: Setting up multi-factor authentication as an admin and user, as well as configuring the self-service account management pages.
tags: self-service account management, user account, profile management, multi-factor authentication setup, account settings
order: 0
---
...
Self-service provides a **Return to application** link at the top of the page. By default, this links to the `Referer` header of the request that brought the user to self-service. To specify a return link to your application, pass the `redirect_uri` query parameter in the self-service URL:

```uri
https://example.com/account/?client_id=<client_id>&redirect_uri=https://your-app.example.com
```

Self-service pages do not display a return link for users from the FusionAuth Admin UI.
...
From click on <Breadcrumb>Manage Two-Factor</Breadcrumb>
