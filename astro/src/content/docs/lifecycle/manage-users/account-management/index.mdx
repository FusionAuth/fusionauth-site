...
Add a `redirect_uri` query parameter to the account URL (for example, `/account/?client_id=<client_id>&redirect_uri=https://your-app.example.com`) to give users a way back to your application. FusionAuth carries this value through the account pages and renders it as a <Breadcrumb>Return to application</Breadcrumb> link. If you don't provide a `redirect_uri`, FusionAuth falls back to the `Referer` header of the request that brought the user to the account pages, as long as it isn't FusionAuth itself; otherwise the link is omitted.
