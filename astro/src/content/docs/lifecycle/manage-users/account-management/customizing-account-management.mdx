...
You can only use `${helpers.display(user, "birthDate")}` on top level user attributes such as `birthDate`. If you are trying to display attributes stored in the `user.data` field, you can use Freemarker to display it directly, like so: `${user.data.favColor}`.

You can use [Freemarker built-in functions](https://freemarker.apache.org/docs/ref_builtins.html) to handle defaults or format the data.
