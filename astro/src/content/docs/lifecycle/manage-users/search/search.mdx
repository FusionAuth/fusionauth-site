...
Starting in version 1.48.0 of FusionAuth every search using the Elasticsearch engine will return a [field]#nextResults# token in the response. You may use this token on subsequent calls to continue paging forward in the result set and go past the 10,000 results limit with Elasticsearch. Every search request made using a <InlineField>nextResults</InlineField> token will return a page of results and another [field]#nextResults# token that can be used to retrieve the next page. These calls can be made indefinitely until the number of returned results is 0.
