...
Below, only the `phoneNumber` field is searched for phone numbers that start with the `303` area code, and any country code, because `phoneNumber:` is specified.
