...
To do so, in the administrative user interface, navigate to <Breadcrumb>System -> About</Breadcrumb> and scroll to the <InlineUIElement>System</InlineUIElement> section and you will see the configured search engine, as well as the version of Elasticsearch, if available. Below, it is `Elasticsearch 7.17.0`.
...
Finally, below, the <InlineField>email</InlineField> field matches the string <InlineFieldValue>dinesh</InlineFieldValue> and the <InlineField>verified</InlineField> field is <InlineFieldValue>false</InlineFieldValue>. These are joined together with an <InlineFieldValue>OR</InlineFieldValue> clause which returns the user if either clause is true, but you can also use <InlineFieldValue>AND</InlineFieldValue> for the intersection.
...
* For that particular application, having a <InlineField>paid</InlineField> attribute of <InlineFieldValue>false</InlineFieldValue>.
