...
Below, only the <InlineField>email</InlineField> field is searched for the string `dinesh`, because `email:` is specified.
...
Finally, below, the <InlineField>email</InlineField> field matches the string `dinesh` and the <InlineField>verified</InlineField> field is <InlineFieldValue>false</InlineFieldValue>. These are joined together with an `OR` clause which returns the user if either clause is true, but you can also use `AND` for the intersection.
