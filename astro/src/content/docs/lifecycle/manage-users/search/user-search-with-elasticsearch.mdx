...
To do so, in the administrative user interface, navigate to <Breadcrumb>System -> About</strong> and scroll to the <strong>System</Breadcrumb> section and you will see the configured search engine, as well as the version of Elasticsearch, if available. Below, it is `Elasticsearch 7.17.0`.
