...
**Question**: Will all my existing users retroactively get emails or SMS messeges for either gate once enabled?

**Answer**:  No. More specifically, this is an accurate statement if you have **never** enabled email/phone verification and/or application registration verification and imported your users with `verified: true` with `verifiedReason: "Completed"` values (as applicable).
