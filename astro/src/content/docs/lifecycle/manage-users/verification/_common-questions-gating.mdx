...
**Answer**:  No. The only edge case if existing users have unverified identities (meaing `verified: false` with `verifiedReason: "Pending"`), and gating is enabled, then those users will be required to verify those identities at their next login.
