...
tags: phone verification, gate accounts, verify phone, sms verification, user verification gating
---
...
- Update the **Verification strategy**. This can be either `ClickableLink` or `FormField`. The former sends an SMS message with a link to verify the user's phone number. The latter sends a code which the user will provide to FusionAuth. For this tutorial, keep the default value of `FormField`.
