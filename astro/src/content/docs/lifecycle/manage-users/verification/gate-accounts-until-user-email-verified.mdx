...
This tutorial also assumes you have a paid edition and valid license. If you need to buy an edition, please visit the [pricing page](/pricing). If you need to activate your instance, please visit the [Reactor page](/docs/get-started/core-concepts/licensing).
...
Navigate to <Breadcrumb>Tenants -> Your Tenant</strong> and then select the <strong>Email</strong> tab. Scroll to the <strong>Email verification settings</Breadcrumb> section.
...
Navigate to <Breadcrumb>Customizations -> Email Templates</Breadcrumb>. Edit the `Email Verification` template by clicking on the blue "Edit" button.
