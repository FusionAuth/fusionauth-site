...
If a user is created and email verification is disabled, the verified attribute of the user object will be <InlineFieldValue>true</InlineFieldValue>. This is because FusionAuth treats all users as having verified emails when email verification is disabled.
...
If a user is created while email verification is enabled, then the <InlineField>verified</InlineField> attribute will have the value <InlineFieldValue>false</InlineFieldValue>:
