...
With paid FusionAuth plans, you can elect to have FusionAuth "gate" your users based on these verification attributes. Gating stops the user from receiving a JWT or essentially completing the authentication workflow. FusionAuth is enforcing a "gate" until they verify their phone or email. Gating happens *after* user creation.
...
<Aside type="note">
    If you kickstarted your FusionAuth instance, the message templates will be missing. You can copy the default messages templates including the phone verification with following the [Retrieving Default Templates](/docs/customize/email-and-messages/message-templates-replacement-variables#retrieving-default-templates) and add them manually in <Breadcrumb>Customizations -> Message Templates</Breadcrumb>.
</Aside>
...
<Aside type="caution">
    There is a user.mobilePhone form field, but phone number verification only works with user.phoneNumber form fields because user.mobilePhone is not an identity type.
</Aside>
