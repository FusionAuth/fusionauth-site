...
At a glance, the identity pre-verification process follows these high-level steps.
...
Use these Postman examples to exercise each step of the API-only flow in a local or test environment.
...
Now that you've successfully set up identity pre-verification via API. Explore related configuration and customization options to refine your verification experience taking the next steps:
