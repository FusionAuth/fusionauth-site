...
tags: identity pre-verification, email verification, pre-verify email, registration verification, verify before registration
---
...
* Select the `Email Verification` template in the **Verification template** dropdown.
...
Since there are other scenarios where gating can be applied for existing users, and you need FusionAuth instead of your Application to handle [Gate Accounts Until User Email Verified](/docs/lifecycle/manage-users/verification/gate-accounts-until-user-email-verified) ensure that the **Unverified behavior** value is `Gated`.
...
With the **Email address** form field available, let's [Create a Form](/docs/lifecycle/register-users/advanced-registration-forms#create-a-form) for email identity verification during registration.

Navigate to <Breadcrumb>Customizations -> Forms</Breadcrumb> and click <Breadcrumb>Add Form</Breadcrumb> in the upper right. Give the form a **Name** of `Email Verification`, then add the following components to the form:

* Add the first step by clicking <Breadcrumb>Add Step -> Form Fields</Breadcrumb>.
* Add the field by clicking <Breadcrumb>Add field</Breadcrumb>, selecting the **Field** `Email address` (the email address field you created earlier), and clicking <Breadcrumb>Submit</Breadcrumb>.
* Add the second step: <Breadcrumb>Add Step -> Verification</Breadcrumb>, selecting **Email Verification**.
...
* Select the earlier created `Email Verification` form in the **Advanced Registration Form** dropdown.
...
2. If the **user.email** field is optional and the user chooses not to enter a value, then verification steps for those fields will be skipped.
...
* Verification step is missing on the form
  * In <Breadcrumb>Tenants -> Identities</Breadcrumb>, confirm **Verify identity** is enabled for <Breadcrumb>Email</Breadcrumb>.
  * In <Breadcrumb>Customizations -> Form Fields</Breadcrumb>, confirm the field key is exactly **user.email** and it is marked <Breadcrumb>Required</Breadcrumb>.
