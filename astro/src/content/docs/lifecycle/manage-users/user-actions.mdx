...
* Use a webhook at the end of the Action to call your code to create another temporal Action in FusionAuth with an indefinite end date and <InlineField>preventLogin</InlineField> set to <InlineFieldValue>true</InlineFieldValue>.
...
* <InlineField>preventLogin</InlineField> — User may not log in if <InlineFieldValue>true</InlineFieldValue> until the Action expires.
...
* Create a new environment variable in the `.env` file called <InlineField>API_KEY</InlineField> and set the value to the value of the API key you created earlier <InlineFieldValue>FTQkSoanK7ObbNjOoU69WDVclfTx8L_zfEJbdR8M0xu-jKotV0iQZiQh</InlineFieldValue>.
