...
* Modify the FusionAuth destination assertion policy. You can do this via the API or the administrative user interface. In the administrative user interface, navigate to <Breadcrumb>Identity Providers -> Your SAMLv2 Identity Provider</strong>. Then navigate to the <strong>Options</Breadcrumb> tab, and find the <InlineField>Destination assertion policy</InlineField> field.
