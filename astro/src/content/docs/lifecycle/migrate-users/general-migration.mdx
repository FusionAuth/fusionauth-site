...
Testing loading 100k or 1M users will help you estimate the time needed for all your users.
...
* Have batches of files on the filesystem for the load (vs pulling from another database via a query). This may not be possible if you are joining two different identity stores together.
* Make sure the JSON is minified.
* Load password hashes, not plaintext passwords.
* Load in a separate tenant so you can easily test and drop users in batches.
* Load in batches of 10k.

TODO break this out to be included in FA cloud docs
For FusionAuth Cloud, the following additional recommendations apply:
* You can't run the import API code in the same network due to security concerns, but you can stand up an EC2 instance in the same AWS region.
* Make sure you provide a static IP address to the FusionAuth team so they can allow list your instance. This prevents it from being rate limited.
