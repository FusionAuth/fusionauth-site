...
* Stage your data by exporting current user data into separate JSON files. This will make debugging easier, since you can load one file at a time, and you can repeat a data load if there are issues. It will also be more performant than loading data across a network or from a database. This may not be possible if you are loading accounts two or more different identity stores together.
...
In FusionAuth testing, this approach resulted in loading 100,000 users in 62 seconds to an HA deployment. Performance is, of course, dependent on user data complexity, hardware, network and database constraints, but this should give you a good idea of possible import speed.
</Aside>
