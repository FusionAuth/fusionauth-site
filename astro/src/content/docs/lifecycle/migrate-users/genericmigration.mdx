...
The first step is to set your OAuth callback URL in the [FusionAuth admin UI](http://localhost:9011/admin). Under <Breadcrumb>Applications</Breadcrumb> edit your <InlineField>Test</InlineField> application and set the <InlineField>Authorized redirect URLs</InlineField> to <InlineFieldValue>http://localhost:7771/callback</InlineFieldValue>.
