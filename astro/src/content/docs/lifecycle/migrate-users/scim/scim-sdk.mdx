...
First, add the SCIM server. Navigate to <Breadcrumb>Entity Management -> Entity Types</strong>. Select the <strong>Add SCIM Server</Breadcrumb> option.
...
To add the entities, navigate to <Breadcrumb>Entity Management -> Entities</Breadcrumb> and create a new entity using the `+` button.
...
To do so, navigate to <Breadcrumb>Tenants -> Default Tenant</strong>. Edit the tenant and then navigate to the <strong>SCIM</Breadcrumb> tab. Once you've checked the <InlineField>Enabled</InlineField> field, you'll see all the SCIM settings.
