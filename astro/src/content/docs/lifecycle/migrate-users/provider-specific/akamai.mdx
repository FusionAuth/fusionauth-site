...
- Click <InlineUIElement>Go to builder</InlineUIElement> on the sidebar in {frontmatter.technology} dashboard.

<img src={`/img/docs/lifecycle/migrate-users/provider-specific/${migration_source_dir}/goto-builder.png`} alt="Go to builder in {frontmatter.technology}" width="1200" />
...
<img src={`/img/docs/lifecycle/migrate-users/provider-specific/${migration_source_dir}/signup.png`} alt="Customize signup in {frontmatter.technology}" width="1200" />

Confirm the changes when asked and then publish the changes to your environment.

Get the signup URL for your environment:

- Log in to your account at https://portal.{frontmatter.technology}.com.
...
Now visit the signup URL by appending `/account/sign-up` to the login URL to get a URL such as `https://app-xyhd7853hsngq.{frontmatter.technology}.com/oauth/account/sign-up`.
...
Export the new user by running the `{frontmatter.technology}/src/1_exportUsers.mjs` script.

```sh
node 1_exportUsers.mjs yourclientid yourapikey
```

If not already created, create roles in FusionAuth by running the `{frontmatter.technology}/src/2_addRoles.mjs` script.

```sh
node 2_addRoles.mjs
```

In the `{frontmatter.technology}/src/3_convertFeUserToFaUser.mjs` script, comment out the following code lines that assign a random password and require a password change.
...
- <InlineField>Name:</InlineField> `{frontmatter.technology}Connector`
...
- Select `{frontmatter.technology}Connector`.
...
Move the `{frontmatter.technology}Connector` policy to the top of the list. Use the arrow buttons to move the policy if necessary. With this configuration, all users are checked against this Connector the first time they are seen. If they log in, they’ll be migrated to the FusionAuth user database.
...
An example web service is in `{frontmatter.technology}/src/5_connectorService.mjs`.
