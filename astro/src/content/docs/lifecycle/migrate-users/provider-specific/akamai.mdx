...
tags: migrate from akamai, akamai identity cloud migration, user migration, import users to fusionauth
---
...
* In Akamai, a role is a string that can be assigned to a user. FusionAuth has [roles](/docs/get-started/core-concepts/types/roles) that are configured on an application-by-application basis and made available in a token after successful authentication.
* With Akamai, you can use schemas to separate users to manage access to clients. FusionAuth has [Groups](/docs/get-started/core-concepts/types/groups).
...
- Stores all Akamai user details that don't map to FusionAuth in the JSON **data** field.
...
- Set **Name** to `AkamaiConnector`
- Set **Authentication URL** to `http://app`. (This URL matches the Docker container you'll create later.)
...
- Enable <Breadcrumb>Migrate to FusionAuth</Breadcrumb> so that FusionAuth will no longer use Akamai for subsequent logins of the same user.
...
- [FusionAuth user import API](/docs/apis/users/import#request)
