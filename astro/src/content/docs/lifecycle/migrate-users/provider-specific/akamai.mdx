...
- Applications (for example, a website or mobile app) are called clients in FusionAuth and properties in {frontmatter.technology}. Properties are also used as API keys in {frontmatter.technology} (called client Ids and secrets).
...
{frontmatter.technology} supports images, but FusionAuth does not, so images will not be imported in this guide.
...
Permissions work slightly differently in the two systems. FusionAuth groups permissions into roles, and assigns roles to users. A user then has all the permissions associated with that user's roles. However, {frontmatter.technology} has only roles, which are strings.
...
Note that both FusionAuth and the JavaScript files in this guide must use the same network, so that the JavaScript code has access to the FusionAuth container. In the Docker Compose file, that network is currently called `faNetwork`.
...
- Extracts the bcrypt password strength (10), the salt, and hash from the {frontmatter.technology} password field. bcrypt stores these all in one field, separated by `$`. However, FusionAuth requires these values to be provided as separate fields during import.
...
If you did not complete the bulk migration tutorial earlier, you need to complete the first steps, which download the Git repository with the export scripts, export your {frontmatter.technology} users, and import their roles to FusionAuth. Follow the instructions from the [Exporting User Data From Akamai](#exporting-user-data-from-akamai) section to the [Create Roles For Your Users](#create-roles-for-your-users) section.
...
Browse to http://localhost:9011/admin/user, select the user Jane, and delete her.
</Aside>
...
- Create a `login_client` property (as you did an `owner` property in the [Exporting User Data From Akamai](#exporting-user-data-from-akamai) section) by browsing to <Breadcrumb>MANAGE PROPERTIES</Breadcrumb>. Alternatively, use the default login property. Set the property's client Id in the command above.
