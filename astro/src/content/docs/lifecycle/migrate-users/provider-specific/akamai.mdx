...
- In {frontmatter.technology}, a registration flow is called a form. Every application has a flow called `standard` to start with. Flows will be used to create example users for export testing.

### Obtaining User Data

{frontmatter.technology} and FusionAuth both support images (e.g. user profile photos) as URLs to files hosted on cloud storage. After migration, check that your photo storage host is still usable.
...
Enter the field values below. Note that the URL
