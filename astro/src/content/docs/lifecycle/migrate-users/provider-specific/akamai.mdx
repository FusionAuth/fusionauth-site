...
- Applications (e.g. a website or mobile app) are called clients in FusionAuth and properties in Akamai. Properties are also used as API keys in Akamai.
- User records are users in FA and profiles or entities in Akamai, stored in an entity type. The default entity type is called user.
- In Akamai, a registration flow is called a form. Every application has one called `standard` to start with.

### Obtaining User Data

Akamai supports images, but FA does not support them, so they will not be imported in this guide.
...
This repository has a directory called Akamai, which holds JavaScript files to import users, and sample data.
...
![Add an Akamai user](../../../../../../public/img/docs/lifecycle/migrate-users/provider-specific/akamai/addUser.webp)
...
If you want to add a lot of users, you can send each user with the API call below. You need to replace the values with your own URL, client Id, and flow version. You can find these in the Akamai console under <Breadcrumb>MANAGE PROPERTIES</Breadcrumb>. API keys are called properties in Akamai.
...
Open a terminal and run the code below to extract all your Akamai users to the file `users.json`. Replace the start of the URL with your own, and use your client Id and secret from your property.
