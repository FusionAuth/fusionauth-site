...
Create an [OIDC application](https://docs.pingidentity.com/r/en-us/pingfederate-pingone-mfa-ik/pingfederate_pingone_mfa_ik_creating_a_web_or_native_application_in_pingone) in your Ping Environment to call from the Flask app. Under <Breadcrumb>Applications -> Applications</Breadcrumb> click the <Breadcrumb>+</Breadcrumb> button to create a new application. Set the **Application Name** and select the `OIDC Web App` application type. Save the application. After saving, enable the application by clicking the toggle button (top right) next to the application name.
...
If the user was not migrated or the login was unsuccessful, enable **Debug enabled** in the Connector configuration to troubleshoot. Navigate to <Breadcrumb>Settings -> Connectors</Breadcrumb> and edit the Connector.
