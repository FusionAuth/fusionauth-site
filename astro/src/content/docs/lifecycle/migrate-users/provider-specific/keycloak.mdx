...
<Aside type="tip">
You might complete the import of all users with passwords, but afterwards cannot log in to FusionAuth with a username and password that you are sure are correct. This most likely cause is the hashing algorithm for the user is incorrect. Check the algorithm in Keycloak is the same one specified in FusionAuth. If you find `salted-pbkdf2-hmac-sha256-512` doesn't work, try `salted-pbkdf2-hmac-sha256` instead.
</Aside>
