...
The FusionAuth team has written a Keycloak compatible password hashing plugin. The <InlineField>encryptionScheme</InlineField> for this plugin is <InlineFieldValue>salted-pbkdf2-hmac-sha256-512</InlineFieldValue>. This plugin ships with versions of FusionAuth 1.34 and above. It is called the <InlineFieldValue>Salted PBKDF2 with SHA-256 with 512-bit derived key</InlineFieldValue> plugin.
...
The migrated user will have the Keycloak tenant Id and original user Id stored on the <InlineField>user.data</InlineField> object. If you have additional user attributes to migrate, review and modify the `map_user` method.
