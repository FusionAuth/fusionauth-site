...
To preserve your users' passwords, you need to perform a gradual migration. Users log in to FusionAuth with their Azure AD B2C credentials, and FusionAuth transparently migrates their data. Gradual migrations in FusionAuth use Connectors, a paid feature.
...
<Aside type="note">
Importing Azure AD B2C users stored in an external identity provider is more straightforward because you can use the Import API; you don't have to perform a gradual migration. Many of the steps in <ScrollRef target="Bulk Migration" /> will apply.
</Aside>
...
As mentioned above, a bulk migration of Azure AD B2C users requires all imported users to reset their passwords, since Azure AD B2C password hashes are inaccessible. This is typically a poor choice as it negatively affects users. However, if a gradual migration won't work because of timing or other reasons, you can move all user information other than passwords from Azure AD B2C  to FusionAuth, and then send password reset emails.
