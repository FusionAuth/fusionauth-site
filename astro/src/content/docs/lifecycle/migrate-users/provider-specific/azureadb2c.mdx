...
Now you can set up the App Registration for the Microsoft Graph API access. Follow [this guide](https://docs.microsoft.com/en-us/azure/active-directory-b2c/microsoft-graph-get-started?tabs=app-reg-ga), including the <Breadcrumb>Create Client Secret</Breadcrumb> section. Record both the Client Secret, and the Application (client) ID as above.
...
The migrated user will have the Azure AD B2C original user identities stored on the <InlineField>user.data</InlineField> object. If you have additional user attributes to migrate, review and modify the `map_user` method.
