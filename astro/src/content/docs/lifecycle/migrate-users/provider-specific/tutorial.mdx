...
description: Learn how to migrate users from an existing relational, homegrown user database into FusionAuth.
section: lifecycle
...
In addition to the example code provided here, you may find example code relevant to your migration in our [Import Scripts](https://github.com/FusionAuth/fusionauth-import-scripts) GitHub repository. 

You also may be interested in the [general migration guide](/docs/lifecycle/migrate-users/provider-specific/general) which covers the topic in more depth.
...
* [Plugins](/docs/extend/code/password-hashes/writing-a-plugin)
* [Custom Password Hashing](/docs/extend/code/password-hashes/custom-password-hashing)
...
FusionAuth has a number of properties on the base User object. You can read about those properties in the [Create a User](/docs/apis/users#create-a-user) API section. Your existing database might have additional user properties that you need to migrate. These can be stored in the `data` object inside the `User` object. This object is also described in [Create a User](/docs/apis/users#create-a-user) API section. The `data` object allows completely free-form data to be stored.
...
```groovy title="Migration Script Example"
def existingUser
def importUsers = []
...
This example uses a FusionAuthClient to perform the import. You may also call the [Import Users](/docs/apis/users#import-users) API directly if you prefer (or if a FusionAuthClient is not available for your language of choice).
