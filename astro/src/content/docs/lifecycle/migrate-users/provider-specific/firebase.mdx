...
If you have external dependencies on an Id stored in Firebase, you can add a new attribute under <InlineField>user.data</InlineField> (such as `user.data.originalId`) with the value of the Firebase Id. Because everything under <InlineField>user.data</InlineField> is indexed and available via search, you'll be able to find your users using either the new FusionAuth Id, or the original Firebase one.
...
stored on the <InlineField>user.data</InlineField> object. If you have additional user attributes
