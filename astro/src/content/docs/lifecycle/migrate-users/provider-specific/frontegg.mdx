...
tags: migrate from frontegg, frontegg migration, user migration, import users to fusionauth
---
...
- [FusionAuth](/docs/get-started/download-and-install/docker). The easiest way to run FusionAuth locally is to use [Docker](https://docs.docker.com/get-docker/) with the configuration file provided later in this tutorial.
...
<MappingUserAttributes migration_source_name="Frontegg" />

### Social Logins

<SocialLoginNote migration_source_name="Frontegg" script_supports_social_logins={false} />

<SocialLoginMigration />

### Other Entities

<OtherEntitiesIntro migration_source_name="Frontegg" other_migrated_entities="connections or roles" />

* In Frontegg, a [role](https://developers.frontegg.com/ciam/guides/authorization/rbac/roles) is a collection of permissions that can be assigned to a user or application. FusionAuth has [roles](/docs/get-started/core-concepts/types/roles) that are configured on an application-by-application basis and made available in a token after successful authentication.
* With Frontegg, you can use [groups](https://developers.frontegg.com/ciam/guides/admin-portal/workspace-modules) to organize a collection of user identities to manage access to applications. FusionAuth also has [Groups](/docs/get-started/core-concepts/types/groups).
* Frontegg uses [tenants](https://developers.frontegg.com/ciam/api/tenants) to define sets of users and help you manage users. You can also manage a set of users via a [Tenant](/docs/get-started/core-concepts/types/tenants) in FusionAuth.
...
- **Name**: `FronteggConnector`
- **Authentication URL**: `http://host.docker.internal:6252`. If you are running FusionAuth directly on your machine and not in Docker you can use `http://localhost:6252`.
- Adjust the timeout values to higher values to cater for network latency when the API requests are being made
- Enable <Breadcrumb>Enable debug logging</Breadcrumb>
...
![Connector details in FusionAuth](/img/docs/lifecycle/migrate-users/provider-specific/frontegg/connector.png)
...
- Enable <Breadcrumb>Migrate to FusionAuth</Breadcrumb> so that FusionAuth will no longer use Frontegg for subsequent logins of the same user.
- Click <Breadcrumb>Submit</Breadcrumb>.

![Connector policy details in FusionAuth](/img/docs/lifecycle/migrate-users/provider-specific/frontegg/connector-policy.png)
...
![Connector policy list in FusionAuth](/img/docs/lifecycle/migrate-users/provider-specific/frontegg/connector-added.png)
...
![Test application login URL](/img/docs/lifecycle/migrate-users/provider-specific/frontegg/login-url.png)
