...
- Note down the <InlineUIElement>Login URL</InlineUIElement>.
...
In order to use this script, you need an instance of FusionAuth running. To start a local instance FusionAuth running in Docker, run the commands below in a new terminal.
...
This Application was created using a bootstrapping feature of FusionAuth called [Kickstart](/docs/get-started/download-and-install/development/kickstart), defined in `fusionauth-import-scripts/frontegg/fusionAuthDockerFiles/kickstart/kickstart.json`.
