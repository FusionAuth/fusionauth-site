...
- Navigate to **Environments -> Development -> Env settings** and note your <InlineUIElement>Client ID</InlineUIElement>, <InlineUIElement>API Key</InlineUIElement>, and <InlineUIElement>Login URL</InlineUIElement>.
...
- Go to **Environments -> Development -> Env settings** and note down the <InlineUIElement>Login URL</InlineUIElement>.
...
- <InlineField>Authentication URL:</InlineField> `http://host.docker.internal:6252`. If you are running FusionAuth in directly on your machine and not in Docker you can use `http://localhost:6252`.
