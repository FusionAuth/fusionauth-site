...
If you are willing to perform a gradual migration, also known as a gradual migration, you can allow your users to log in to {frontmatter.technology} via FusionAuth, and have FusionAuth transparently migrate their data. Gradual migrations in FusionAuth use [Connectors](/docs/lifecycle/migrate-users/connectors), a paid feature.

A gradual migration would look like this:
1. Export all user data from the {frontmatter.technology} API, except password hashes.
...
This tutorial will cover all the bulk and gradual migration steps.
...
To test the gradual migration, you need a user with a password you know. To create a user with a password that you can use to test authentication, alter your login form in the {frontmatter.technology} web portal as follows:
...
<Aside type="note">
  If you did not go through the steps mentioned earlier in the bulk migration section, get export scripts by going through the steps in the <ScrollRef target="Export The Users With The API" /> and <ScrollRef target="Create Roles For Your Users" /> sections. The steps are summarized below.
</Aside>
