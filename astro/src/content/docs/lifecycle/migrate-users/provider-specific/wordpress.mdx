...
- Open a new terminal in the `fusionauth-import-scripts` directory and run the code below.
  ...
  docker compose up
  ```
- FusionAuth will now be running and browsable at `http://localhost:9011`. You can log in to the [FusionAuth admin UI](http://localhost:9011/admin) with <InlineFieldValue>admin@example.com</InlineFieldValue> and <InlineFieldValue>password</InlineFieldValue>. The container is called <InlineField>fa</InlineField>.
