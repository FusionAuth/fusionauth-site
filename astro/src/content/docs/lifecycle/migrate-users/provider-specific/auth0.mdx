...
If a user uses email or SMS verification, you can port the data over to FusionAuth using [the user update API](/docs/lifecycle/authenticate-users/multi-factor-authentication#directly-enable-mfa-for-another-user) or by adding the <InlineField>twoFactor</InlineField> fields to your import JSON.
...
While you can't generate a fixed set of recovery codes, once MFA has been enabled for a user, you can [generate a new set of recovery codes](/docs/apis/two-factor#generate-recovery-codes) and deliver them to the user. As of `1.67.0`, recovery codes are hashed at rest and cannot be retrieved after generation.
