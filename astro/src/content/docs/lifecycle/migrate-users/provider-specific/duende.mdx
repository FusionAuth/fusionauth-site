...
After installing the plugin and restarting FusionAuth, you can verify that the plugin is installed by navigating to <Breadcrumb>Tenants -> Your Tenant</strong>, then the <strong>Password</strong> tab. Under the <strong>Cryptographic hash settings</Breadcrumb> section, you should see the new hash scheme available in the <InlineUIElement>Scheme</InlineUIElement> dropdown. Only check that the plugin is listed, don't select it or any new user will be created with that hash scheme. Rather than selecting the plugin here, it is specified per user in the `users.json` file.
