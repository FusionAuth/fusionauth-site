...
<Aside type="note">
Apple returns a user's full name only the first time the user logs in. Since you have users who have already logged in with Apple to your previous authentication gateway, Apple will not provide the user's name when logging in to FusionAuth now. The only solution is to manually import the user's name from your previous database into FusionAuth.
</Aside>
