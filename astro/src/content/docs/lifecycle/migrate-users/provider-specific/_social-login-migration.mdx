...
Import users with the [Import API](/docs/apis/users/import), assigning each user with a social login a random password such as a UUID.
...
* The Identity Provider Id is found on the corresponding [Identity Provider API documentation](/docs/apis/identity-providers/). Look for **identityProvider.id**.
