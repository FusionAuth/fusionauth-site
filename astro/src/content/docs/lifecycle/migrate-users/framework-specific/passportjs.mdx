...
Set up an identity provider by navigating to <Breadcrumb>Settings -> Identity Providers</Breadcrumb>. In the dropdown, choose <Breadcrumb>Add Google</Breadcrumb>, and then enter your Google <InlineField>Client Id</InlineField> and <InlineField>Client secret</InlineField> (find these values in your Google Cloud console). Save the identity provider.
...
```console title="Installing required packages"
...
* Set <InlineField>Name</InlineField> to <InlineFieldValue>My App Connector</InlineFieldValue> or anything you like.
...
* Enable <Breadcrumb>Migrate user</Breadcrumb> so that FusionAuth will no longer use Passport for subsequent logins of the same user.
