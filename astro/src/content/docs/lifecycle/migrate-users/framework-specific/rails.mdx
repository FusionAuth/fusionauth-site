...
importCodeRoot: https://raw.githubusercontent.com/fusionauth/fusionauth-import-scripts/feat/add-ruby-on-rails-script
---
...
This guide provides general migration strategies with working example {frontmatter.technology} apps available [here](https://github.com/FusionAuth/fusionauth-example-migrating-rails). The example contains three sample Rails applications, each using a different authentication tool:
...
- [Ruby – version 3.3.0 or higher](https://www.ruby-lang.org/en/downloads/) to run the migration scripts, and gems.
...
- [ngrok](https://ngrok.com/) (optional) to expose your local Rails application to the internet for testing purposes.
- A basic understanding of Ruby on Rails and its authentication systems.
...
Before running the provided export scripts, update the application Id values to match your FusionAuth application Ids. All scripts generate JSON files ready for import into FusionAuth. Modify the User model field references to match your application's schema before running.

### Devise

Most Rails applications using Devise can use this export script:

<RemoteCode url={frontmatter.importCodeRoot + "/rails/export-scripts/devise/export_users_for_fusionauth.rb"} lang="ruby"  />

The script handles bcrypt password hashes directly. bcrypt includes the salt in the hash.

A sample user export file exported from the script will look like:
...
To test the slow migration, you need a user with a password you know. The provided Rails example apps in this [repository](https://github.com/FusionAuth/fusionauth-example-migrating-rails) include setup scripts `db/seeds.rb` that create test user accounts.
...
- **Use ngrok** - Use a tool like [ngrok](https://ngrok.com/) to expose your local Rails application to the internet. Make sure to add your ngrok URL to the config.hosts whitelist in `config/environments/<environment>.rb` to allow external connections.
...
- Set <InlineField>Authentication URL</InlineField> to the URL provided by ngrok or `http://<IP_ADDRESS_OR_DOMAIN>:PORT/fusionauth/connector`, replacing `IP_ADDRESS_OR_DOMAIN` with the IP address or domain name of your host machine and `PORT` with the port number of your Rails application.
...
An example controller can be found [here](https://github.com/fusionauth/fusionauth-example-migrating-rails/raw/refs/heads/main/ruby-users-devise/app/controllers/fusion_auth_connector_controller.rb) for Devise and [here](https://github.com/fusionauth/fusionauth-example-migrating-rails/raw/refs/heads/main/ruby-users-rails-auth/app/controllers/fusion_auth_connector_controller.rb) for Rails built-in authentication.

You can see working implementations of this connector in our example Rails applications, which demonstrate how to handle authentication for different Rails auth systems.
...
Log in with the username and password for a known user as defined in the seed scripts of the sample applications `db/seeds.rb`, such as `jennifer.adams@techstart.com` and `StartupLife2024!` for the devise example and `sarah.johnson@techcorp.com` and `SecurePass123!` for the Rails authentication. Your connector service should log the login request succeeding in the terminal.
