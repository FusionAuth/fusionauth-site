...
- Set <InlineField>Authentication URL</InlineField> to the URL provided by ngrok or `http://<IP_ADDRESS_OR_DOMAIN>:PORT/fusionauth/connector`. Replace `IP_ADDRESS_OR_DOMAIN` with the IP address or domain name of your host machine, and `PORT` with the port number of your Rails application.
...
- Enable <Breadcrumb>Migrate user</Breadcrumb> so that FusionAuth will no longer use your Rails backend for subsequent logins of the same user.
