...
The guides in this section and its subsections will help you understand and prepare the migration process.

There are three ways to import users into FusionAuth.
...
If you are importing only a few users, you can use the [Create User API](/docs/apis/users) to import users individually. This is typically done using [a client library](/docs/sdks). With this method, you cannot migrate the user's password.
...
- If your existing user store supports SAML or OIDC, you should be able to use an [identity provider](/docs/get-started/core-concepts/identity-providers). You would need to modify the theme and probably want to use a hint.
- If your existing user store supports LDAP or a JSON API, you can use [connectors](docs/lifecycle/migrate-users/connectors/) without migrating (a paid feature).
...
Every migration involves transferring data from the old system to the new system, called backfill, followed by a cutover for a user, where they authenticate with the new system and not with the old one.

There are three approaches to user migration.  While each approach differs in implementation details, a good way to consider which is right for you is to look at how many cutovers you want to handle.

You can:

* Migrate everyone at once, known as a **bulk** migration, also called an **offline**, or **big bang** migration. With this approach, you have one cutover.
* Segment your users and migrate **segment by segment**. With this method, you have multiple cutovers, each with a natural chunk of users.
* Migrate each user when they authenticate, known as a **gradual** migration, also called a **rolling**, **slow**, **online**, or **phased** migration. With this choice, there are two cutover points. The first is the application cutover, which happens when you direct users to FusionAuth for authentication. Then, at each user's login, the data is migrated and the user's system of record changes. Therefore there are many data cutover events.
...
With a bulk migration, you are moving all your users at one time. The exact duration varies, but there is a single cutover period. The basic steps are:
...
* The bulk requires you to write code which you'll test intensely, use once and then throw away.
...
### User By User (Gradual) Migration

This approach is a logical extension of segment by segment migration. Here, each segment is a single user. With a gradual migration, you move users one at a time, when they log in. You keep the other system running and set up a connection between them. This means you don't have to have access to the underlying password hashes or other user information. With FusionAuth, you implement a gradual migration using [Connectors](/docs/lifecycle/migrate-users/connectors/).

<PremiumPlanBlurb />

Connectors allow you to migrate users one by one transparently. Learn how [how to use Connectors with FusionAuth](/docs/lifecycle/migrate-users/connectors).

With a gradual migration:
...
To implement a gradual migration, FusionAuth needs to pass the user's auth credentials to the old system and expects the user information which is being migrated. You also need to modify applications to point to FusionAuth before any migration starts. A gradual migration has the following benefits.
...
However, a gradual migration isn't the right solution for every application. Issues to be aware of:
...
A gradual migration is a good choice when you:
...
A gradual migration is, in short, a lower risk, long duration choice.

Learn more about gradual migrations in this [blog post](/articles/identity-basics/slow-migration).
...
* Which migration approach fits your needs: bulk, segment by segment or gradual migration
...
Next, consider a user's password, and related fields such as a salt or hashing scheme. Dealing with this data depends on your migration approach. For a bulk or segment by segment migration, ensure FusionAuth understands the original system's hashing algorithm [by writing a plugin](/docs/extend/code/password-hashes/custom-password-hashing) if the password was not hashed in one of [FusionAuth's supported algorithms](/docs/reference/password-hashes). For a gradual migration, the password will be available for FusionAuth to hash or re-hash. If you have the user's passwords in plain text, FusionAuth can also hash them on import.
...
Create one or more tenants. Multiple tenants are useful for allowing someone to have the same email but different passwords, or allowing different settings such as the theme or password rules. Add these via the API or navigating to <strong>Tenants</strong> and clicking the green plus sign.
...
Now that FusionAuth is up and running, proceed to either the <ScrollRef target="Bulk Migration Implementation" />, the <ScrollRef target="Segment By Segment Implementation" /> or the <ScrollRef target="Gradual Migration Implementation" /> section.
...
<img src="/img/docs/lifecycle/migrate-users/provider-specific/api-key-creation.png" alt="Adding an application." width="1200" />
...
With FusionAuth, gradual migrations happen using [Connectors](/docs/lifecycle/migrate-users/connectors/).

<PremiumPlanBlurb />

#### Determine Your Finish Line

Unlike a bulk migration, with a gradual migration you need to set a migration completion goal. Gradual migrations move accounts one user at a time, so it is unlikely you'll migrate one hundred percent of your users through this approach. Some people log in to your application rarely while others may have abandoned their accounts. No matter how long a migration period you allow, some of your users will not log in during that time frame, and therefore won't be migrated.
...
* You will have some users who have not migrated when the gradual migration period is over. How will you handle those accounts?
...
With a gradual migration, FusionAuth is connecting to your previous datastore every time a user not currently in FusionAuth authenticates. This connection requires either an HTTP API request or an LDAP call.
...
Navigate to <strong>Settings -> Connectors</strong> and add a Connector. You can also configure Connectors by using the API; consult the [Connector API documentation](/docs/apis/connectors/) for more details.

<img src="/img/docs/lifecycle/migrate-users/provider-specific/add-connector.png" alt="Adding a Connector." width="1200" />
...
However you retrieve the number of users migrated, regularly compare it to the number of accounts in the original system before the migration began. This ration will determine if it is time to end the gradual migration.

#### Remove The Proxy

When it is time to stop the gradual migration, remove the Connector policy. All future logins will take place against FusionAuth.

<img src="/img/docs/lifecycle/migrate-users/provider-specific/delete-connector-policy.png" alt="Removing a Connector policy." width="1200" />

You may also optionally remove the Connector.

#### Handle Unmigrated Users

At this point, decide how to handle users who haven't been migrated. You may be able to find these users by subtracting the set of migrated users from the users in the original system. You may also be able to query the original system directly and note who has not signed in since the gradual migration started.
...
* Extend the time running both systems; that is, set a new goal and continue the gradual migration.
...
What happens when a user forgets their password and attempts to reset it, but you are in the middle of a gradual migration? There are two scenarios:
...
<img src="/img/docs/lifecycle/migrate-users/provider-specific/reset-user-password.png" alt="Resetting the password of a user." width="1200" role="bottom-cropped" />
...
What happens when a user attempts to register for an application and you are in the middle of a gradual migration? There are two scenarios:
...
#### Gradual Migration Alternatives

In some cases, you may be able to implement a gradual migration without using a Connector.
If profile data in your original system of record doesn't change, or you can safely lose any changes, you can do a forked gradual migration.
...
Once this proxy is up and running, you can choose when to do a bulk migration.
...
- Drop the database. Create a new database with [Kickstart](/docs/get-started/download-and-install/development/kickstart) or [Terraform](/docs/operate/deploy/terraform) to create default applications, accounts, and other items.
- Import the users into a temporary tenant different to your default application tenant. Deleting the tenant will remove all users associated with that tenant. This option maintains all the other non-tenant settings (identity providers, email templates, themes, etc.).
- Use the [bulk delete API](/docs/apis/users#bulk-delete-users). You can start deleting blocks of 5-10k users and increase the number deleted with each API call. This is slower than the other options, but has the benefit of leaving the rest of the system untouched.
...
This option is only applicable for bulk migrations, since a gradual migration can re-issue a refresh token for each user at login time.
...
Users should be able to use their JWTs from the previous system, especially if the JWTs are used only in APIs. You need to ensure the JWT producers and consumers have the correct signing secrets. Share the secrets between the old system and FusionAuth by using the [Keymaster](https://fusionauth.io/docs/apis/keys) to import existing keys or allowing clients to look up the keys from a [JWKS endpoint](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/endpoints#json-web-key-set-jwks), from both the old and the new system.
...
* Modify the FusionAuth destination assertion policy. You can do this via the API or the administrative user interface. In the administrative user interface, navigate to <strong>Identity Providers -> Your SAMLv2 Identity Provider</strong>. Then navigate to the <strong>Options</strong> tab, and find the <InlineField>Destination assertion policy</InlineField> field.
...
If you are moving to FusionAuth from another provider, there may be specific tasks to perform or concepts to understand. FusionAuth has complete tutorials to import from many providers, listed in the [Provider Specific](/docs/lifecycle/migrate-users/provider-specific) section below.

If you have a single users table and want to walk through how a migration would work with that, review the [generic migration tutorial](/docs/lifecycle/genericmigration).

If you are working with an identity provider for which there is no guide, please [open an issue in our GitHub repository](https://github.com/FusionAuth/fusionauth-issues/issues) with details.
