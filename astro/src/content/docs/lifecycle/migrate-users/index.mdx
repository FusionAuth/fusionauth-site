...
tags: migrate users, import users, user migration, password rehashing, slow migration, bulk user import
---
...
When importing only a few users, you can use the [Create User API](/docs/apis/users/create) to import users individually. This is typically done using [a client library](/docs/sdks). With this method, you cannot migrate users' passwords.
...
Finally, import the JSON using the `importUsers` method of a FusionAuth client library or by calling the REST API directly. This is [fully documented](/docs/apis/users/import).
...
Consult the [Import Users API documentation](/docs/apis/users/import) for more information.
...
You can assign them a high entropy password such as a UUID for the import and then use the [Forgot Password API](/docs/apis/users/forgot-password). This will send them an email to reset their password.
...
Make sure to check the **Migrate to FusionAuth** checkbox. Then, each user will be authenticated against the original user datastore the first time they are seen. Their data will then be migrated to FusionAuth. On subsequent logins, they'll authenticate with FusionAuth. You can learn more about configuring Connector policies in the [Connector documentation](/docs/lifecycle/migrate-users/connectors/).
...
Alternatively, you can fire off an event when a user visits the forgot password page.

* Write JavaScript code that runs on the forgot password page. Install it via a custom theme. The JavaScript can fire an event with the user's email to an API endpoint written by you. Fire this event whenever a user submits a 'forgot password' form, hitting an API endpoint of your choice.
* When an event hits the endpoint, look up the user in the legacy system. If the user exists in the legacy system, query to see whether they exist in FusionAuth.
* If they do not exist in FusionAuth but do in the legacy system, migrate the user data via the [User API](/docs/apis/users/create), setting **sendSetPasswordEmail** to `true`.
* After they are migrated, trigger the [reset password email via FusionAuth API](/docs/apis/users/forgot-password).
...
- Use the [bulk delete API](/docs/apis/users/bulk-delete) to remove users. You can start deleting blocks of 5-10k users and increase the number deleted with each API call. This is slower than the other options but has the benefit of leaving the rest of the system untouched.
...
* Modify the FusionAuth destination assertion policy. You can do this via the API or the administrative user interface. In the administrative user interface, navigate to <Breadcrumb>Identity Providers -> Your SAMLv2 Identity Provider</Breadcrumb>. Then navigate to the <Breadcrumb>Options</Breadcrumb> tab, and find the **Destination assertion policy** field.
...
At a later time, request each SAML IdP update their configuration to point to the FusionAuth ACS URL. After all upstream providers have been migrated to the latest configuration, set the **Destination assertion policy** field to <Breadcrumb>Enabled</Breadcrumb>, the most secure setting.
...
If you need further assistance migrating to FusionAuth, please ask a question in the [FusionAuth forum](/community/forum/).

If you have a paid plan, you may open a [support request](https://account.fusionauth.io) from your account for more migration assistance.
