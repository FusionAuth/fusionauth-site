...
- [Example SQL](#example-sql)
	- [Direct Database Import Through SQL Statements](#direct-database-import-through-sql-statements)
	- [API or Plugins](#api-or-plugins)
- [How To Handle Differing Password Hashing Algorithms](#how-to-handle-differing-password-hashing-algorithms)
- [Social Logins Like Facebook And Google](#social-logins-like-facebook-and-google)
...
If you need to map FusionAuth columns to columns in the new database, use SQL syntax like: `SELECT u.first_name as name`. Or you could do the mapping in the Python or Node.js code itself.

## How To Handle Differing Password Hashing Algorithms

The major difficulty migrating between any authentication services is how each service handles password hashing.
...
[Social logins](https://fusionauth.io/docs/lifecycle/authenticate-users/identity-providers) are stored in the `identity_provider_links` table, which links `identity_providers` (like Google or Apple) with `users`. Generally, a user's email address is retrieved from the identity providers, and the address is stored in the `identities` table.
...
If you cannot do a bulk migration and take your application offline, you need to know how to migrate individual users. FusionAuth provides two useful tools for this: [webhooks](https://fusionauth.io/docs/extend/events-and-webhooks) and [APIs](https://fusionauth.io/docs/apis).

(You could even do a bulk migration with the [users](https://fusionauth.io/docs/apis/users#search-for-users) API instead of a database script, but it would slower unless your database is small).

Online migration generally involves choosing relevant FusionAuth [events](https://fusionauth.io/docs/extend/events-and-webhooks/events), and using those to trigger a webhook that calls a web service you've written. The web service can then call the relevant API to get user data, and call the API of the authentication service you are migrating to, to update user data.
