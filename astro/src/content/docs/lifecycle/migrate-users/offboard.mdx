...
Some data, like users, applications, and roles, will probably be available in most authentication services. But some data is so specific to FusionAuth there is no point in trying to migrate it with a script. This includes settings for webhooks, connectors, lambdas, and user actions. There is no also point in exporting logs like daily login counts and FusionAuth instance settings like themes, because your new authentication service won't use them.

To understand the user-related data tables in the database, you need to be familiar with [FusionAuth core concepts](/docs/get-started/core-concepts). Below is a visual summary of the object hierarchy.
...
In addition to the objects above, you also want to migrate identity providers, like Google OAuth, as well as user consents to receiving various emails.

Below is the full database diagram for the tables you need to export. Though it has more fields than the diagram above, it is the same design.

![Database diagram](../../../../../public/img/docs/lifecycle/migrate-users/offboard/databaseDiagram.png)
...
<Aside type="note">
If you want to browse the diagram above in DBeaver, you can download the diagram file [here](../../../../../public/img/docs/lifecycle/migrate-users/offboard/databaseDiagram.erd), or you can view the diagram in .GRAPHML format by downloading it [here](../../../../../public/img/docs/lifecycle/migrate-users/offboard/databaseDiagram.graphml) and viewing in a GraphML editor, such as the free online one, [yEd](https://www.yworks.com/yed-live).
</Aside>
