...
You can follow this guide to try some examples of exporting data. This guide assumes you have a working copy of FusionAuth to test against, which might be a development instance or even a production instance. We recommend testing against a development instance first before trying against production.
...
The screenshot below shows DBeaver connected to the FusionAuth PostgreSQL database from the example repository using a connection string with port `5432`, database `fusionauth`, username `fusionauth`, and the password for this user (hidden). To connect to FusionAuth on a remote server, change the <InlineField>Host</InlineField> from `localhost` to your server name. The image shows the main user data table, `identities`.
