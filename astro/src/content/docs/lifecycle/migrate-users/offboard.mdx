...
<Aside type='note'>
The offboarding documentation and processes described here are tied to a specific version of FusionAuth's database schema. If you need to query the database directly, you must [download the database schema](https://fusionauth.io/direct-download) zip for your specific FusionAuth version. The schema structure may differ between versions, so you'll need to adapt these examples to match your version's database structure.
</Aside>
...
<img src="/img/docs/lifecycle/migrate-users/offboard/lifecycle-migrate-users-offboard.png" alt="Diagram showing user-related database tables including Users, Registrations, Groups, Roles, Applications and Tenants." />
