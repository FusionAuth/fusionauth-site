...
tags: export users, offboard from fusionauth, user data export, migrate away, authentication data export
---
...
If you use [FusionAuth Cloud](/docs/cloud/reference/user-data#access-user-data) (the paid, cloud-hosted version of FusionAuth), please contact support to request a backup file of your database. Import the backup file you receive from FusionAuth into a PostgreSQL instance on your computer to explore the data and schema.
...
It is possible to do a [slow migration](/docs/lifecycle/migrate-users/#user-by-user-slow-migration) using the [users](/docs/apis/users/search) API instead of a database script, but it would be slower unless your database is small.
