...
If you use the Connector as a source of authentication, the source of truth remains the LDAP server. If you migrate your users, FusionAuth becomes the source of truth after the first login of each user, which triggers the migration of that user.
