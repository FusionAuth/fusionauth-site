...
<img src="/img/docs/lifecycle/migrate-users/connectors/generic-connector.png" alt="The Generic Connector creation page." width="1200" role="shadowed bottom-cropped" />
...
<img src="/img/docs/lifecycle/migrate-users/connectors/generic-connector-security.png" alt="The Generic Connector security section." width="1200" role="shadowed bottom-cropped top-cropped" />
...
<img src="/img/docs/lifecycle/migrate-users/connectors/generic-connector-header.png" alt="The Generic Connector headers section." width="1200" role="shadowed top-cropped bottom-cropped" />
...
* Configure the Connector in <strong>Settings -> Connectors</strong>, including securing the endpoint.
...
<Aside type="caution">
A valid user object must be returned with a FusionAuth compatible UUID in the <InlineField>user.id</InlineField>. This is a requirement both both [not migrating a user](/docs/lifecycle/migrate-users/connectors/overview#required-fields) and when [migrating a user](/docs/lifecycle/migrate-users/connectors/overview#required-fields-2).

When you are not migrating a user,the <InlineField>user.id</InlineField> must be the same across subsequent logins.
</Aside>
...
You can load test your Connector using [FusionAuth's load testing framework](/docs/operate/secure-and-monitor/monitor#load-testing) or any other HTTP based load testing framework.
...
Debugging writes to the <strong>System -> Event Log</strong> and this causes additional load on the database.
...
[Here's more about load testing FusionAuth](/docs/operate/secure-and-monitor/monitor#load-testing).
