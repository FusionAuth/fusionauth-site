...
<Aside type="caution">
A valid user object must be returned with a FusionAuth compatible UUID in the <InlineField>user.id</InlineField>. This is a requirement both [not migrating a user](/docs/lifecycle/migrate-users/connectors/#required-fields) and when [migrating a user](/docs/lifecycle/migrate-users/connectors/#required-fields-1).
