...
tags: generic connector, http connector, user migration, external authentication, data source connector
---
...
<Aside type="caution">
A valid user object must be returned with a FusionAuth compatible UUID in the **user.id**. This is a requirement whether you are [authenticating but not migrating a user](/docs/lifecycle/migrate-users/connectors/#required-fields) or [migrating a user](/docs/lifecycle/migrate-users/connectors/#required-fields-1).
...
The first two have configurable timeouts, the **Connect timeout** and the **Read timeout** fields.
For **Connect timeout**, if the timeout expires before the connection can be established, the login fails.
...
You can load test your Connector using [FusionAuth's load testing framework](/docs/operate/monitor) or any other HTTP based load testing framework.
...
[Here's more about load testing FusionAuth](/docs/operate/monitor).
