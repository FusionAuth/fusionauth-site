...
<img src="/img/docs/lifecycle/migrate-users/bulk/keycloak-tenant-screen.png" alt="The keycloak realm screen." width="1200" role="bottom-cropped" />
...
<OtherEntitiesIntro other_migrated_entities="identity providers, clients, or keys" />

* In Keycloak, Identity Providers and User Federation allow user data to remain in external systems of record. FusionAuth has similar concepts of [Identity Providers](/docs/v1/tech/identity-providers/) and [Connectors](/docs/lifecycle/migrate-users/connectors/overview).
* Mappers are ways for you to customize authentication or authorization workflows. FusionAuth has a similar concept called [Lambdas](/docs/extend/code/lambdas/overview).
* Keycloak has the ability to add custom scopes. FusionAuth does not, though there is an [open feature request](https://github.com/FusionAuth/fusionauth-issues/issues/275).
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-import-scripts/master/keycloak/keycloak-export.sql"
            lang="sql"
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-import-scripts/master/keycloak/keycloak-export-postgres.sql"
            lang="sql"
...
If you have a version of FusionAuth below 1.34, the code is [available for download](https://github.com/FusionAuth/fusionauth-contrib/blob/master/Password%20Hashing%20Plugins/src/main/java/com/mycompany/fusionauth/plugins/ExamplePBDKF2HMACSHA256KeyLength512PasswordHasher.java). If you use this, follow the [plugin installation steps](/docs/extend/code/password-hashes/writing-a-plugin#install-the-plugin).
