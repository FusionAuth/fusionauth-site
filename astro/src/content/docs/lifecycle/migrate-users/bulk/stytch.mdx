...
All tests should and the plugin file should be available in `Password Hashing Plugins/target/fusionauth-example-password-encryptor-0.1.0.jar`.
...
This import script needs only FusionAuth to be running locally on port 9011, and the `users.json` file to exist in the same directory. The Fusion Kickstart file already set a sample application (user registration) to match the Ids in `4_import.js`. If your FusionAuth installation is different, please edit the Ids in `4_import.js`.

You might also want to set the user's registration and tenant for production migration.
```js
...
- FusionAuth allows only one email and phone number. Stytch has an array. The import script takes the first verified contact it can as the primary FusionAuth contact, otherwise it just takes the first one. Extra contacts are saved in the generic `data` JSON field.
...
Finally, this script imports users individually. If this is too slow when running the production migration, wrap the `importUsers()` FusionAuth SDK call in a loop that bundles users in batches of 100.