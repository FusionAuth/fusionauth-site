...
importCodeRoot: https://raw.githubusercontent.com/FusionAuth/fusionauth-import-scripts/stytch
javaCodeRoot: https://raw.githubusercontent.com/FusionAuth/fusionauth-contrib/scrypt
...
- In the `fusionauth-contrib` terminal where you built the plugin, run the commands below. This will copy the JAR file into the FusionAuth container `plugins` directory.
  ```bash
  ...
  ```
- Finally, you need to restart FusionAuth for it to detect the plugin. In the terminal where FusionAuth is running in Docker press `Ctrl+C` to stop it, wait, and run `docker compose up` again.
