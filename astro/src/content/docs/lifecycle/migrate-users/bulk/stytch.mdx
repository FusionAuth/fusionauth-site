...
- [ ] prerequisites: node, Java or Docker
- [ ] see the strategy article, this is a technical tutorial
...
processUsers().catch(error => console.error(error));

async function processUsers() {
  const stytchUsers = new Chain([
...
async function importUsers(faUser) {
  const importRequest = {
    users: [faUser],
