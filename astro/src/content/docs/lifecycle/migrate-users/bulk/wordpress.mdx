...
- start wp
- allow subscriber public account creation
- create sql query to export user, and all details, and hashes
- write article, use existing bcrypt if possible

Use this docker compose file

```yaml
# Source: https://hub.docker.com/r/bitnami/wordpress-nginx
# mkdir db && mkdir wp && sudo chmod -R 777 ./db ./wp     # don't use    sudo chown -R 1001:1001 ./db ./wp;
# docker compose up
# sudo chmod -R 777 ./db ./wp
# Browse to http://localhost/wp-login.php with user,bitnami
...
Browse to http://localhost/wp-login.php.
Login with user and bitnami.
Settings - General - Membership Anyone can register - Enable - Save Changes.
Log out.
http://localhost/wp-login.php
Register
Enter any username and email a@example.com.
...
Create a new MariaDB connection to localhost, port 3306, database wp, username wp, no password needed. Open the connection and expand the database tables.

WP has to two tables related to users: `wp_users` and `wp_usermeta`.

Below is the data you should have in `wp_users`.
...
- user_pass: The hashed password for the user. WordPress uses the wp_hash_password function to create a hash of the user's password.
...
The wp_usermeta table is designed to be highly extensible, allowing for the addition of custom metadata fields related to users, which themes or plugins can utilize without needing to modify the core wp_users table structure. This design allows WordPress to be highly customizable and extendable.
...
- nickname: The user's nickname, which can be used publicly in place of their username for display purposes.
- first_name and last_name: Store the user's first and last name, respectively.
...
- [TODO](#todo)
- [Contents to delete](#contents-to-delete)
