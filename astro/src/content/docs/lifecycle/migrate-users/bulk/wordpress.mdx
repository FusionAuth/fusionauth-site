...
Let's begin the tutorial. In this section you create a sample WordPress database in Docker, understand where the user details are kept in the database, and export them with a SQL script.
...
The two users you exported have roles Adminstrator and Subscriber, as seen in the `wp_capabilities` meta field. Create them in FusionAuth. In a terminal in the import scripts directory, run the code below.
...
There are a few helper functions at the bottom of the file to extract keys and values from the {frontmatter.techonology} `wp_usermeta` object and deserialize them from PHP. These are used when working with `wp_capabilities`.
