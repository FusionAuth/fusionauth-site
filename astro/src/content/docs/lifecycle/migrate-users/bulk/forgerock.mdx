...
* {migration_source_name} has the ability to add custom scopes. FusionAuth does not, though there is an [open feature request](https://github.com/FusionAuth/fusionauth-issues/issues/275).
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-import-scripts/master/forgerock/forgerock-export.rb"
            lang="ruby"
...
Usage: import.rb [options]
    ...
    -p USER_PASSWORD,                The password for the api user.
        --user-password
    -c, --client-id CLIENT_ID        The client id of the applicaiton to use for the authenticaton.
    -s CLIENT_SECRET,                The client secret for the application to use for authentication.
        --client-secret
    -h, --help                       Prints this help.
```

```shell title="Generate JSON Export with {migration_source_name} Rest API"
...
Usage: import.rb [options]
    -r APPLICATION_IDS,              A comma separated list of existing applications Ids. All users will be registered for these applications.
        --register-users
    -u, --users-file USERS_FILE      The exported json user data file from Forgerock. Defaults to out.csv.
