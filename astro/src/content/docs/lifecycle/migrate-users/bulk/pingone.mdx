...
export const codeRoot = 'https://raw.githubusercontent.com/FusionAuth/fusionauth-import-scripts/master'
...
- Using Microsoft Azure, you can create an [HTTP trigger](https://learn.microsoft.com/en-us/azure/azure-functions/functions-bindings-http-webhook-trigger?tabs=python-v2%2Cisolated-process%2Cnodejs-v4%2Cfunctionsv2) on the function app.
...
This guide will show you how to create an ngrok endpoint that will use the Flask API to validate user credentials from FusionAuth login requests and retrieve the user's Ping Identity profile from the PingOne API.
...
Create an OIDC application in your Ping Environment to call from the Flask app by following this [Ping One guide](https://docs.pingidentity.com/r/en-us/pingfederate-pingone-mfa-ik/pingfederate_pingone_mfa_ik_creating_a_web_or_native_application_in_pingone). 

- Select "Code" as the "Response Type".
- Take note of the Client ID of the application to use in the Flask app.
...
Create a `pingIdentityAuth.py` file and add the Flask authenticator code below to it.

<RemoteCode lang="python" url={codeRoot + "/pingidentity/export/slowmigration/pingIdentityAuth.py"} />

Modify the example function code provided as needed and remember to update the domains (for example, `apiPath`) to match the region the Ping Identity data is hosted.
...
Make sure that all requirements (like `requests` and `flask`) are installed with pip before you run the script.
...
You need to modify the `transform_json_user` function to match the user properties you want to import. This function converts the JSON returned from Ping Identity into the JSON format FusionAuth requires.

The exact implementation depends on your Ping Identity custom attributes and business logic. You could, for example, give users certain FusionAuth roles, register them for more than one application, or add them to a previously created FusionAuth group.
...
Be aware that the steps outlined for bulk migrating users will not export password hashes and users will need to change their passwords manually on FusionAuth.
...
To connect to the Ping Identity API and export users, create a worker application in your Ping Identity environment. Follow the [steps for adding a worker application](https://docs.pingidentity.com/r/en-us/pingone/p1az_adding_worker_app) in the Ping Identity docs.

Give the worker application sufficient permissions to access your user data by assigning the necessary roles, for example, the Environment, Identity Data, and Organization Admin roles. 

Generate an access token in the Configuration tab and make note of it to use in the export application.
...
Clone the code below and make modifications appropriate to your environment and needs. In addition to adding the `envID` and `access_token`, change teh `apiPath` to point to the domain of the region where your Ping Identity data is hosted.

<RemoteCode lang="python" url={codeRoot + "/pingidentity/export/bulk/exportPingUsers.py"} />

Install the `requests` requirement for the Python script with `pip install requests`.

Add the code to a file named `exportPingUsers.py` and then run the script with the following command.
...
The script transforms this data to a structure similar to the example below, and saves it to a `users.json` file in the same folder as the script.
...
<Aside type="caution">
  This script will send an email to every imported user. Ensure your SMTP server is able to handle the volume. Configure your SMTP server in <strong>Tenants -> Your Tenant -> Email</strong>.
</Aside>
...
<Aside type="note">
  This section is only relevant if you choose to do the slow migration and can be ignored if the users changed their passwords as in the previous section
</Aside>
