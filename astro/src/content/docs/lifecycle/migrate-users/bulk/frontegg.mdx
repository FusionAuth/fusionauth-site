...
- In the hamburger menu on the right for the account, click <InlineField>View account</InlineField>. Click <InlineField>Customize login box</InlineField> — <InlineField>Start customizing</InlineField>. In the sign in types, disable all options except for email and password (disable magic code). Use the buttons at the top right to review and publish this change. (This allows users to create passwords that you can import if using Connectors).
...
- login with myemailaddress+frontegguserc@simplelogin.com
- save your login url in `Env settings`, e.g. https://app-dgo95bersinj.frontegg.com/oauth
- change app url to http://localhost:6251
- with activiation email, click it, then click back to sign in. here is the sign in url: https://app-dgo95bersinj.frontegg.com/oauth/account/login?userId=e217b086-11bc-4f7a-a9da-43ea69a4428f&token=340d3ab4-d24b-4cfb-8aef-e3ef70bbd19d.
- in builder, allow users to sign up themselves
- create user at https://app-dgo95bersinj.frontegg.com/oauth/account/sign-up, myemailaddress+frontegguserc@simplelogin.com, name ccc, company ccc. password `password`.
