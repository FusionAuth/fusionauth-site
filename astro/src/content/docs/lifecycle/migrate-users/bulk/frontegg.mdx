...
Your FusionAuth instance now points to `http://host.docker.internal:6252/login` for logins of the `test` application. The next step is to write a small service that forwards the user's name and password to {frontmatter.technology} and returns if the login was successful. If it is, then you know the password is correct and can save it, hashed, to FusionAuth.
