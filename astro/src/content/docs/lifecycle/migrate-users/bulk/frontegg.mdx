...
You now need to configure FusionAuth to point to an intermediary web service between it and {frontmatter.technology} by using a Connector. For a general overview of Connectors, please see this [video](https://www.youtube.com/watch?v=ks-ppMH6WXw).
...
### Build An Authentication Intermediary Web Service
 ...
 The next step is to write the small service available on port 6252 that forwards the user's name and password to {frontmatter.technology} and returns if the login was successful. If it is, then you know the password is correct and can save it, hashed, to FusionAuth.
...
- run locally
-  curl -X POST http://localhost:6252 -H "Content-Type: application/json" -d '{"password":"p", "loginId":"test@example.com"}'
-  curl -X POST http://localhost:6252 -H "Content-Type: application/json" -d '{"password":"password", "loginId":"myemailaddress+frontegguserc@simplelogin.com"}'
