...
<iframe allowfullscreen="" frameborder="0" width="560" height="315" src="//www.youtube.com/embed/wQklLZZP_3s?rel=0" enablejsapi="true" data-gtm-yt-inspected-8="true"></iframe>
...
* [Rules](https://auth0.com/docs/rules), [Hooks](https://auth0.com/docs/hooks) and [Actions](https://auth0.com/docs/actions) are ways for you to customize authentication or authorization workflows. FusionAuth has a similar concept called [Lambdas](/docs/extend/code/lambdas/overview).
...
The <InlineField>method</InlineField> field of the Two-Factor API [lists the MFA methods supported by FusionAuth](/docs/apis/messengers/two-factor#request-body).
...
If a user uses email or SMS verification, you can port the data over to FusionAuth using [the user update API] or by adding the [field](/docs/lifecycle/authenticate-users/multi-factor-authentication#directly-enabling-mfa-for-a-user)#twoFactor# fields to your import JSON.
...
While you can't generate a fixed set of recovery codes, once MFA has been enabled for a user, you can [retrieve the recovery codes](/docs/apis/messengers/two-factor#retrieve-recovery-codes).
