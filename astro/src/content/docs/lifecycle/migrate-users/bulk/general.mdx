...
A POC is helpful in determining which [login method](/docs/get-started/core-concepts/integration-points#login-options) you should use and how to [theme the hosted login pages](/docs/customize/look-and-feel/overview) to maintain your application's look and feel. 
...
<img src="/img/docs/lifecycle/migrate-users/bulk/add-tenant.png" alt="Adding a tenant." width="1200" role="bottom-cropped" />
...
<img src="/img/docs/lifecycle/migrate-users/bulk/add-application.png" alt="Adding an application." width="1200" role="bottom-cropped" />
...
Now that FusionAuth is up and running, proceed to either the <ScrollRef target="Big Bang Implementation" />, the <ScrollRef target="Segment By Segment Implementation" /> or the <ScrollRef target="Slow Migration Implementation" /> section.
...
With FusionAuth, slow migrations happen using [Connectors](/docs/lifecycle/migrate-users/connectors/overview).
...
Navigate to <strong>Settings -> Connectors</strong> and add a Connector. You can also configure Connectors by using the API; consult the [Connector API documentation](/docs/apis/connectors/overview) for more details.

<img src="/img/docs/lifecycle/migrate-users/bulk/add-connector.png" alt="Adding a Connector." width="1200" />
...
Map each attribute from the LDAP directory into the FusionAuth user object. You do this with an [LDAP Connector Reconcile Lambda](/docs/v1/tech/lambdas/ldap-connector-reconcile). 
...
<img src="/img/docs/lifecycle/migrate-users/bulk/add-connector-policy.png" alt="Adding a Connector policy." width="1200" />
...
<img src="/img/docs/lifecycle/migrate-users/bulk/delete-connector-policy.png" alt="Removing a Connector policy." width="1200" />
...
<img src="/img/docs/lifecycle/migrate-users/bulk/reset-user-password.png" alt="Resetting the password of a user." width="1200" role="bottom-cropped" />
...
* If the user exists, migrate the user data via the [User API], setting [field](/docs/apis/users#create-a-user)#sendSetPasswordEmail# to `true`.
...
In this scenario, you also pre-load the required certificates and notify upstream IdP administrators they'll need to update their ACS URLs eventually. You'll also configure a [FusionAuth SAMLv2 Identity Provider](/docs/lifecycle/authenticate-users/identity-providers/overview) with the same values as your existing SAML SP.
