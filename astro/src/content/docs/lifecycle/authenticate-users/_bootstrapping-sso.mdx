...
* Build a normal Authorize request that redirects to the [OAuth Authorize](/docs/api/oauth#authorize) endpoint. The host and path may very based upon your proxy configuration, but the query parameters are the same as a normal request to this endpoint.
