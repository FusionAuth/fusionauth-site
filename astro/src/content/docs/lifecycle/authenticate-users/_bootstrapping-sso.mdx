...
* Enable the access token bootstrap setting on the tenant. There are two ways to do this. Use the [Admin UI](/docs/get-started/core-concepts/types/tenants#oauth) or set the **tenant.ssoConfiguration.allowAccessTokenBootstrap** field to `true` in the [Tenant API](/docs/apis/tenants).
...
* Build a normal Authorize request that redirects to the [OAuth Authorize](/docs/apis/oauth/authorize) endpoint. The host and path may very based upon your proxy configuration, but the query parameters are the same as a normal request to this endpoint.
