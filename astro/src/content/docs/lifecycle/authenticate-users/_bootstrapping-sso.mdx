...
The way to set a reverse proxy varies depending on your setup. One common scenario is an existing HTTP only secure cookie that holds the user's access token. In this example, the reverse proxy copies the value of that cookie into the `Authorization` header as it passes through.
