...
* Enable the access token bootstrap setting on the tenant. There are two ways to do this. Use the [Admin UI](/docs/get-started/core-concepts/tenants#oauth) or set the <InlineField>tenant.ssoConfiguration.allowAccessTokenBootstrap</InlineField> field to <InlineFieldValue>true</InlineFieldValue> in the [Tenant API](/docs/apis/tenants).
