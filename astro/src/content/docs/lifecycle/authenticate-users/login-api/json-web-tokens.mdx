...
tags: json web tokens, jwt, access token, refresh token, id token, jwt claims, token management
---
...
FusionAuth provides the ability to configure a couple of aspects of its JWT handling. By navigating to <Breadcrumb>Settings -> Tenants</Breadcrumb> in FusionAuth and selecting the <Breadcrumb>JWT</Breadcrumb> tab, you will see the tenant JWT configuration settings.
...
* with [Entity Management](/docs/get-started/core-concepts/types/entity-management), each entity type can have a unique key
...
To receive a refresh token from the Login API, enable Refresh Tokens for the application. Using the FusionAuth UI, navigate to <Breadcrumb>Settings -> Applications -> Security Tab</Breadcrumb>. Here you will find the Login API settings. Ensure that both **Generate Refresh Tokens** and **Enable JWT refresh** fields are enabled.
