...
import AccessTokenClaims from 'src/content/docs/lifecycle/authenticate-users/login-api/_access-token-claims.mdx';
...
For more on the OAuth grants and the tokens returned when using them, visit [the OAuth documentation](/docs/lifecycle/authenticate-users/oauth/overview).
</Aside>

After a User is authenticated via the [Login API](/docs/apis/login) or [OAuth](/docs/lifecycle/authenticate-users/oauth/overview), FusionAuth creates a JWT and returns
...
FusionAuth provides the ability to configure a couple of aspects of its JWT handling. By navigating to <strong>Settings -> Tenants</strong> in FusionAuth and selecting the JWT tab, you will see the tenant JWT configuration settings.
...
If you navigate to <strong>Applications</strong> from the main menu, you can also configure the JWT parameters, including the signing algorithm, on a per application basis. If you don't select to enable Application specific JWT configuration, the tenant configuration will be used.
...
Learn more about the [JWT Populate Lambda](/docs/v1/tech/lambdas/jwt-populate).
...
Keys are managed in [Key Master](/docs/operate/secure-and-monitor/key-master) and can be generated or imported there.
...
```text title="Example HTTP Cookie Header"
...
To receive a refresh token from the Login API, enable Refresh Tokens for the application. Using the FusionAuth UI, navigate to <strong>Settings -> Applications -> Security Tab</strong>. Here you will find the Login API settings. Ensure that both <InlineField>Generate Refresh Tokens</InlineField> and <InlineField>Enable JWT refresh</InlineField> fields are enabled.
