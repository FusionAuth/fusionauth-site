...
The single sign-on session duration can be configured at the Tenant level. Navigate to <Breadcrumb>Tenant -> Your Tenant -> OAuth</Breadcrumb> and edit the **Session timeout** value. Because this value is shared between applications, it can't be overridden in application configuration.

![Configuring the single sign-on session length.](/img/docs/lifecycle/authenticate-users/tenant-single-sign-on-session-timeout.png)
