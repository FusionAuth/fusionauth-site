...
tags: application authentication tokens, fast authentication, api token login, token-based authentication, bcrypt alternative, authentication speed, fusionauth auth tokens
---

In most cases, Users authenticate using a login Id (email or username) plus a password. FusionAuth hashes passwords with a strong cryptographic hash such as BCrypt. The process of hashing is intentionally slow by design to limit the risk of brute force attacks. In some cases, you might need a way to speed up authentication without reducing the hashing strength for the normal login process.
...
To enable Authentication Tokens, open the FusionAuth web interface and navigate to <Breadcrumb>Applications</Breadcrumb> from the main menu. Edit the Application you want to use Authentication Tokens for and click the <Breadcrumb>Security</Breadcrumb> tab. You'll see an option like this:

![Authentication Tokens](/img/docs/lifecycle/authenticate-users/authentication-token.png)
...
Note that you must provide a valid API key unless you've also unchecked the **Require an API key** setting in the <Breadcrumb>Login API Settings</Breadcrumb>.
