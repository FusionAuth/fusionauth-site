...
**How MFA works in FusionAuth:**
1. User signs in
2. FusionAuth checks tenant & user settings
3. If policy or context requires, prompt for MFA code
4. Validate code (TOTP/SMS/Email)
5. Continue login or step-up flow

[Start a free trial and evaluate in your environment.](/get-started)

## FusionAuth MFA Methods: TOTP, SMS, Email
...
[Not sure which path fits? Talk to an expert.](/contact)
...
2. Add MFA method to `twoFactor.methods` array
3. Update user via PATCH request
...
## Integrating Other MFA Methods (Generic messenger, webhooks)
