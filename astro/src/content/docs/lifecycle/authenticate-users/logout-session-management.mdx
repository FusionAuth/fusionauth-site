...
If you don't want FusionAuth SSO to be enabled, set the `rememberDevice` parameter to <InlineFieldValue>false</InlineFieldValue> in the login page. In this case, there will be no SSO session. You can also set the <InlineUIElement>Session Timeout</InlineUIElement> to zero.
