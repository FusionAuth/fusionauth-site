...
centralizedCodeRoot: https://raw.githubusercontent.com/FusionAuth/fusionauth-example-node-centralized-sessions/main
---
...
* An **application session**. This is created after FusionAuth has authenticated a user and the application has completed the token exchange. How this session works (cookie, database entry) is specific to the web or mobile application.
* A **FusionAuth SSO session**. This is created when a user checks <InlineUIElement>Keep me signed in </InlineUIElement>, or that element is automatically checked. This session is only available when using the [hosted login pages](/docs/get-started/core-concepts/integration-points#hosted-login-pages).
* A **centralized session**. This is representation of an application session registered with FusionAuth. This is done with a refresh token and works with either an API or the hosted login pages.
...
The alternative is to use application managed sessions using the front channel logout. In this situation, FusionAuth doesn't know about the state of the application sessions and relies on the applications to manage them. No sessions will appear on the <Breadcrumb>Sessions</Breadcrumb> tab and you won't be able to revoke sessions via API, though you will still be able to request session revocation, as you'll see later. With this option session timeouts are managed by each application.
...
* Make sure to set up the ChangeBank and ChangeBank Forum applications correctly. Make sure the Application has the <InlineUIElement>Refresh Grant</InlineUIElement> toggled. The <InlineUIElement>Generate refresh tokens</InlineUIElement> setting must be enabled as well.
TODO Screenshot
* Each time you build the authorization URL, request these scopes:
  * `offline_access` which creates the refresh token
  * `openid` which asks for an Id token
* In the ChangeBank and ChangeBank Forum applications, create an **application session** using whatever session solution your web or mobile app framework suggests. Find and store the refresh token Id in this session. This is available in the `sid` parameter in the Id token. It is also available in the access token, but you should prefer the Id token because the Id token should be consumed by the client, whereas the access token is presented for resource access. You should prefer the refresh token Id instead of the refresh token value because the Id will not change even when using one-time use refresh tokens.
* Whenever a request comes in to either application, make a call to the [Retrieve Refresh Tokens API](/docs/apis/jwt#retrieve-refresh-tokens) to check for the status of the refresh token. This can be done either against the REST API or using one of [the FusionAuth client libraries](/docs/sdks). You'll pass the Id of the refresh token.
* When a user logs out from ChangeBank, revoke the refresh token. You can use the [Revoke Refresh Tokens API](/docs/apis/jwt#revoke-refresh-tokens).
...
<RemoteCode url={frontmatter.centralizedCodeRoot + "/changebankforum/src/index.ts"}
  lang="ts"
...
<RemoteCode url={frontmatter.centralizedCodeRoot + "/changebankforum/src/redirectMiddleware.js"}
  lang="ts"
  />
...
<RemoteCode url={frontmatter.centralizedCodeRoot + "/changebank/src/index.ts"}
  lang="ts"
  tags="endsession"/>
...
There are other possibilities too, beyond complex logout. You can also revoke refresh tokens from other systems using the SDKs and APIs. For example, if you are building or augumenting a customer service application, you could add a button to 'log the user out' to the user details screen. Or if you encountered a [suspicious login based on a FusionAuth webhook](/docs/extend/events-and-webhooks/events/user-login-suspicious), you could revoke the refresh tokens for the user and force them to re-authenticate to ChangeBank but not to the ChangeBank Forum.

### Use With The Login API

You can also create refresh tokens and use centralized sessions with the [Login API](/docs/apis/login). This is useful if you don't want to use the hosted login pages. To use this, configure the Application to allow refresh tokens using the Login API.
...
You can pass additional information about the FusionAuth SSO session by using the `metaData.device.description` attribute. This can be added as a hidden form field. 
...
This metadata is only available to the SSO session and is not stored in any other refresh tokens. The values can be read by retrieving the SSO refresh token, which is a refresh token with no associated application Id. 
...
TODO update https://fusionauth.io/docs/get-started/core-concepts/users and maybe point folks here
