...
This guide documents logout and session management features. Logout, or sign out, processes revoke users' access to applications and functionality. Session management manages and control the access to application functionality.
...
When this guide refers to sessions without any of these prefixes, the relevant statement applies to all these types of session.s

## Types Of Session Management

When considering session management with FusionAuth, first consider whether you need a centralized session store. Do you want your FusionAuth instnace to keep track of sessions for each application? 
...
Suppose a user has logged out of ChangeBank. They have had their refresh tokens revoked and thus their centralized sessions invalidated. But ChangeBank Forum still has a valid application session. This flow shows what happens when they visit ChangeBank Forum.
...
All of these are possible because you have build all your applications to check in with FusionAuth, and FusionAuth provides programmatic control of the centralized sessions.
...
JWTs represent a session to other services, but the refresh token is still represents the session within FusionAuth.
...
<ApplicationManagedSessionInvalid alt="Application managed sessions when application session is invalid, but the SSO session is still valid." />
...
After a user has logged out, you need to send them to a different page based on their tier. Create these URLs, make sure they display different messeages, and register them as authorized redirect URLs.
...
In [Types of Sessions](#types-of-sessions), you learned about the FusionAuth SSO Session. This section will discuss it in more depth. The FusionAuth SSO session is managed by FusionAuth and is only available if you are using the hosted login pages.
...
You can write JavaScript to run on the login page [only when using advanced themes](/docs/customize/look-and-feel/advanced-theme-editor), so the hidden field value could be dynamically set. For example, you could pull a value from the URL parameters and place it in that hidden field.
...
The value of this field can be read by retrieving the FusionAuth SSO refresh token, which has no associated application Id. Here's an example of what that looks like:
