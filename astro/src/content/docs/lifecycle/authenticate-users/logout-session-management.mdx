...
Since you are setting up code at the logout URL endpoint for each application, you can revoke any refresh tokens associated with the application or user. This is not required, however. Doing so lets you combine this method with the centralized session store scenario.
...
You can use the [`/api/logout` endpoint](/docs/apis/login#logout-a-user). This is designed for situations where you have stored the refresh token in a cookie and want to revoke it from the client without an API key.
