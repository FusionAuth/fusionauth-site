...
Backchannel logout lets a server end a user's session via a direct server-to-server call, without requiring the user's browser.

There is an [OIDC backchannel logout specification](https://openid.net/specs/openid-connect-backchannel-1_0.html). FusionAuth does not implement this, though there is [an open GitHub issue for it](https://github.com/FusionAuth/fusionauth-issues/issues/465).

However, if you are using [centralized sessions](/docs/lifecycle/authenticate-users/logout-session-management#centralized-sessions), you can implement non-standard, server-to-server session revocation by revoking all the refresh tokens for a user.

If you have a refresh token for a user, you can use the [`/api/logout` endpoint](/docs/apis/login#logout-a-user) with the `global` parameter. If you have the user Id, use the [`/api/jwt/refresh`](/docs/apis/jwt#revoke-refresh-tokens) endpoint.
...
You may also use it with the `global` parameter to remove all refresh tokens for a user.
