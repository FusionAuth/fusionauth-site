...
If you would like to request OAuth scopes on the access token from FusionAuth, those should be provided in the `scope` parameter on the initial authorization request. The `scope` parameter on the request to the identity provider is configured via that IdP's <InlineField>Scope</InlineField> value, if available.

After returning from the external IdP, the `scope` parameter from the original request will be restored to complete the OAuth workflow in FusionAuth.
