...
<Aside type="version">
Identity provider source requires version `1.65.0` or later.
</Aside>

On the Identity Providers list page, the `Source` column indicates where each configuration was created. Use the `Source` filter to show providers created in the admin UI (`System`) or in Tenant Manager (`Tenant Manager`).

On the Add Identity Provider page, the `Source` field indicates where the identity provider configuration originated. If not provided during creation, it defaults to `System`.
