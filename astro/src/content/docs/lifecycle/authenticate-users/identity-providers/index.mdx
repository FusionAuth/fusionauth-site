...
All identity providers can use reconciliation lambdas except <InlineUIElement>Anonymous Link</InlineUIElement>, which has no information to pass to a lambda.
...
This is a useful option if you don't want to create a full user account in FusionAuth. But see the [Anonymous Account Limitations](#anonymous-account-limitations) below. This uses the linking strategy <InlineUIElement>Anonymous Link</InlineUIElement>.
...
Because this linking strategy provides no information about the user, <InlineUIElement>Anonymous Link</InlineUIElement> cannot call a reconciliation lambda.
