...
Omitting the `identityProvider.tenantId` parameter from the API request to create an identity provider or selecting `Global` from the dropdown on the Add Identity Provider view in the admin app will create a global identity provider.
