...
## Multiple Identity Providers Of The Same Type

<Aside type="version">
Available Since Version 1.61.0. Previously only select Identity Providers could have multiple configurations.
</Aside>

You can have different identity provider configurations for different applications. Suppose you have two different applications both using the Apple Identity Provider.

But for one, you want to request the `email name` scope and for the other you want to request the `email` scope only. To make this work, create an Apple Identity Provider with the value `email name` in the <InlineField>Scope</InlineField> field and enable it for the first application. 

Then, create another Apple Identity Provider and use the value `email` in the <InlineField>Scope</InlineField> field. Enable it for the second application.

If you are trying to assign multiple identity providers to the same application, that is not currently possible and you should use the [Overrides](#overrides) ffunctionality described below. Review the [open GitHub issue](https://github.com/FusionAuth/fusionauth-issues/issues/3241) for more details.

### Overrides

<Aside type="note">
Prefer [Multiple Identity Providers Of The Same Type](#multiple-identity-providers-of-the-same-type) if you are on version `1.61.0` or later.
</Aside>
...
For certain Identity Providers, there are no override settings because you should create more than one configuration:

* External JWT
* SAMLv2
* OpenID Connect/OIDC

Prior to version `1.61.0`. you cannot have different Identity Providers of the same type other than those listed above; use overrides as documented above for different configuration. In some cases, you need to use two different Applications to achieve your desired configuration. For example, if you need two sets of attributes for an Identity Provider, but the attributes don't exist in the <InlineUIElement>Overrides</InlineUIElement> options. An example of such an attribute is <InlineField>Linking Strategy</InlineField>.
