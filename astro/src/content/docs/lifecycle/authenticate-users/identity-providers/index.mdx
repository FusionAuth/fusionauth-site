...
Attribute mappings allow you to map claims from an Identity Provider response directly to FusionAuth user and registration fields. They are a simpler no-code alternative to reconciliation lambdas.

Attribute mappings are key value pairs that can be configured via the Identity Provider API or the Tenant Manager. The keys are fields on a FusionAuth user or registration and must begin with `user.` or `registration.`. Some example attribute mapping keys are:
...
The values are fields or claims on the Identity Provider result parameter available in the corresponding reconcile lambda type. **Note:** The OpenID Connect reconcile lambda has two parameters, `jwt` and `id_token`, attribute mapping values must include one of these as a prefix in order to indicate which parameter to use.

The values can be specified using a simple dot notation or JSON pointer notation. JSON pointer notation is necessary to refer to fields containing `.` or `/` characters. A literal `/` in a JSON pointer is escaped as `~1`. Some example attribute mapping values are:
...
During reconciliation, attribute mapping values are used to extract a claim value from the Identity Provider response. The corresponding key is used to set that value on the FusionAuth user or registration. Some other notes on the reconciliation process:
* Failures locating a value on the Identity Provider response are ignored
* Failures setting a value on the user or registration object are ignored
* All attribute mappings are considered optional

Reconcile lambdas and attribute mappings are mutually exclusive. Attempting to configure both on an Identity Provider is a validation error.
