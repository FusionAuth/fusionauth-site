...
## Add an External JWT Identity Provider

<IdpApplicationConfiguration identity_provider="External JWT" details_link="/docs/lifecycle/authenticate-users/identity-providers/external-jwt/#form-fields" image="/img/docs/lifecycle/authenticate-users/identity-providers/external-jwt/identity-provider-external-jwt-add.png"/>
...
<APIBlock>
  ...
  <APIField name="Verification keys" required since="1.69.0">
    The keys stored in Key Master that are used to verify the SAML response sent back to FusionAuth from the identity provider. These keys must be verification only keys or certificates (meaning that they only have a public key component).
  </APIField>
