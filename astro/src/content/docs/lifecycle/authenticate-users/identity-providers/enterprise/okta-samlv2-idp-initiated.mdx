...
You can tell FusionAuth how to find and use this persistent `userId` by modifying your newly created SAML IdP Initiated Provider in FusionAuth under the `Options` tab. Here you will add the value <InlineFieldValue>userId</InlineFieldValue> to the <InlineField>Unique Id claim</InlineField> field. Additionally, on the same tab, you can instruct FusionAuth where to find the email claim by filling in the <InlineField>Email claim</InlineField> with the value <InlineFieldValue>email</InlineFieldValue>. Depending on your Okta configuration, you can optionally indicate <InlineField>Use NameID for email</InlineField> instead.
