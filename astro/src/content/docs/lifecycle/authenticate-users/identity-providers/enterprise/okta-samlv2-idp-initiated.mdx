---
title: Add a SAML v2 IdP-Initiated with Okta IdP
description: Configure SAML v2 IdP-Initiated SSO With Okta.
tags: okta saml, identity provider-initiated sso, saml v2, okta identity provider, enterprise sso, idp initiated login
---
...
Navigate to the Okta admin screen. Using the left "hamburger" menu, click on <Breadcrumb>Applications Header -> Applications</Breadcrumb>. From here, click <Breadcrumb>Create App Integration</Breadcrumb>. Select the SAML option and hit next.

![Setting up a SAMLv2 application in Okta](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/okta-new-app-initial-screen.png)
...
![General settings tab when creating a SAMLv2 application in Okta](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/okta-general-settings.png)

Click <Breadcrumb>Next</Breadcrumb>. You will arrive at the <Breadcrumb>Configure SAML</Breadcrumb> section.

![Settings tab when creating a SAMLv2 application in Okta](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/configure-saml.png)
...
The final screen is related to marketing/support. This screen may or may not be present. Click the <Breadcrumb>Finish</Breadcrumb> button.

![Feedback Marketing Step](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/feedback-marketing-step.png)

Now, under the <Breadcrumb>Sign On</Breadcrumb> tab, scroll down until you see a button called "View SAML setup instructions". This will open a new window/tab. You will need the values displayed to complete the rest of this tutorial.

![View SAML Setup Instructions](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/view-saml-setup-instructions.png)
...
![Details around certificate and SSO URL and issuer](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/detail-saml-setup-instructions.png)
...
![Adding the Okta certificate to Key Master](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/add-okta-public-certificate-to-keymaster.png)
...
* Add the public certificate you just imported to **Verification keys**.
...
![Adding the IdP Initiated SSO Identity Provider](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/fusionauth-add-okta-identity-provider.png)
...
![View the IdP Initiated SSO Identity Provider](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/fusionauth-view-identity-provider.png)
...
![Configure CORS](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/fusionauth-cors-settings.png)

## Configure the FusionAuth Application Redirect URL

Navigate to <Breadcrumb>Applications -> Your Application -> OAuth</Breadcrumb>. Update the **Authorized redirect URLs** field with one or more URLs. A redirect URL is where a user will end up after authentication and may be any valid URL.
...
![Configure the FusionAuth Pied Piper application](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/fusionauth-set-up-application.png)
...
![Showing Setting the Relay and ACS values](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/saml-acs-and-relay-state.png)
...
At times it may be helpful to receive a unique and immutable id from Okta and tie this to the new FusionAuth user (this is the value of a persistent SAML NameID). To enable this, on the <Breadcrumb>Edit SAML Integration</Breadcrumb> Okta (current) screen please click <Breadcrumb>Add Another</Breadcrumb> under the <Breadcrumb>Attribute Statements</Breadcrumb> section. Under the **Name** field enter `userId`, for **Name format** leave as `Unspecified`, and finally, for **Value** enter `user.id`. With this attribute added, Okta will be sending over a persistent Id for each user.
...
![Add Persistent Id and email attribute statements](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/okta-add-attribute-statements.png)
...
You can tell FusionAuth how to find and use this persistent `userId` by modifying your newly created SAML IdP Initiated Provider in FusionAuth under the <Breadcrumb>Options</Breadcrumb> tab. Here you will add the value `userId` to the **Unique Id claim** field. Additionally, on the same tab, you can instruct FusionAuth where to find the email claim by filling in the **Email claim** with the value `email`. Depending on your Okta configuration, you can optionally indicate **Use NameID for email** instead.

![Configure FusionAuth with Email and Id attributes defined](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/fusionauth-idp-init-options.png)

## Adding a User to the Okta Application
Navigate to the <Breadcrumb>General</Breadcrumb> tab and scroll to the <Breadcrumb>App Embed Link</Breadcrumb> section and note the **Embed Link** value. This is the link a user needs to visit to begin the IdP initiated SSO, so you could place it in your application's navigation, launchpad, or elsewhere.

Finally, click on the <Breadcrumb>Assignments</Breadcrumb> tab and assign the user to the application. First, select the user by clicking the <Breadcrumb>Assign</Breadcrumb> dropdown. Then by clicking <Breadcrumb>Assign to People</Breadcrumb>, and then finally selecting the <Breadcrumb>Assign</Breadcrumb> button for the user you would like to use.

![Pick a user to assign to application step one](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/okta-assign-application-to-user-step-one.png)

Then confirm the user should have access.

![Assigning a user to the SAML SP application step two](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/okta-assign-application-to-user-step-two.png)

## Test It Out

Open an incognito browser window and visit the **Embed Link** value. This value can be found at the bottom of the <Breadcrumb>General</Breadcrumb> tab of your Okta Application configuration, in the <Breadcrumb>App Embed Link</Breadcrumb> section. Log in with your Okta IdP credentials.

![Logging in with Okta](/img/docs/lifecycle/authenticate-users/identity-providers/enterprise/okta-login-page.png)
