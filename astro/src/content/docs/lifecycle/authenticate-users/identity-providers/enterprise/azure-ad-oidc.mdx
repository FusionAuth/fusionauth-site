...
You may set a <InlineField>Reconcile lambda</InlineField> to map attributes from the JWT provided by Azure AD to the FusionAuth user or registration. This is optional. [Learn more about lambdas]. You may also modify the <InlineField>Button text</InlineField> or [field](/docs/v1/tech/lambdas/openid-connect-response-reconcile)#Button logo# if desired.
