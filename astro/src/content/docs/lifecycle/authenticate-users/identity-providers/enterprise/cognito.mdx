...
Once you have completed this configuration, you can have an OpenID Connect <Breadcrumb>Login with Cognito</InlineUIElement> button for one or more FusionAuth Applications.
...
- In the <InlineUIElement>Hosted UI settings</InlineUIElement> section, set the <InlineField>Callback URL</InlineField> to `<YOUR_FUSIONAUTH_URL>/oauth2/callback` under <strong>Allowed callback URLs</strong>; for example `https://auth.piedpiper.com/oauth2/callback`.
