...
Under the <InlineUIElement>(.*)</InlineUIElement> section, select <InlineField>Skip group assignment for now</InlineField>. Hit <InlineUIElement>Save</InlineUIElement>.
...
- Navigate to the <InlineUIElement>(.*)</InlineUIElement> section and ensure that the application you are adding Okta authentication to has the <InlineField>Enabled</InlineField> and <InlineField>Create Registration</InlineField> switches toggled. Hit <InlineUIElement>Save</InlineUIElement>.
...
If it is all set up correctly, you should be redirected back to your app, successfully logged in. The user will be added to FusionAuth, and you can examine the <InlineUIElement>(.*)</InlineUIElement> section of the user details screen to see that the Okta OIDC link was created.
