...
Additionally, you will need to update the <InlineField>Unique Id Claim</InlineField> field to `id` (from the standard claim of <InlineFieldValue>sub</InlineFieldValue>) under the <Breadcrumb>Options</Breadcrumb> tab.
