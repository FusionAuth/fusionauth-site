...
Discord does not implement OpenID Connect to spec. In order to pull in some additional data for populating the user profile (Discord username, Avatar URL, etc.) you can enable a <InlineField>Reconcile lambda</InlineField>.  Review [the lambda documentation](/docs/v1/tech/lambdas/openid-connect-response-reconcile) to learn about how to create and assign your own lambda.  This is optional.
