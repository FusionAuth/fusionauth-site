...
<APIBlock>
  ...
  <APIField name="Linking strategy" optional defaults="Link on email. Create the user if they do not exist">
    The linking strategy for LinkedIn. [See Linking Strategies for more](/docs/v1/tech/identity-providers/#linking-strategies).
  </APIField>
  <APIField name="Reconcile lambda" optional>
    A lambda may be utilized to map custom claims returned from LinkedIn. See the [LinkedIn Reconcile lambda documentation](/docs/v1/tech/lambdas/linkedin-reconcile) for more details.
