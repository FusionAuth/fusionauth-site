...
To create a GitHub Identity Provider return to FusionAuth and navigate to <Breadcrumb>Settings -> Identity Providers</Breadcrumb> and click `Add provider` and select `OpenID Connect` from the dialog.
