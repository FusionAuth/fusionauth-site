...
Once this is complete, you can add or update scopes by clicking on the `Add or remove scopes` button. It is typical to select `https://www.googleapis.com/auth/userinfo.email` and `https://www.googleapis.com/auth/userinfo.profile.profile` as well as `openid`, but feel free to enable any you would like.
