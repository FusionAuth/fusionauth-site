...
You will be prompted to define a consent type, typically <InlineField>External</InlineField>. Click <Breadcrumb>Create</Breadcrumb>.
...
Select <Breadcrumb>Web application</Breadcrumb> from the <InlineField>Application Type</InlineField> field.
...
If using a popup, select `Web application`, specify the name of the credential and fill out the <InlineField>Authorized JavaScript origins</InlineField> field. This value should be the origin of your application login page.
...
<APIBlock>
  ...
  <APIField name="Scope" optional>
    ...
    The <InlineField>Scope</InlineField> value may only be used when the <InlineField>Login method</InlineField> is set to `Use redirect for login`. The `email`, `profile`, and <InlineFieldValue>openid</InlineFieldValue> scopes will always be requested for all other configurations.
  </APIField>
...
* Set <InlineField>Discover endpoints</InlineField> to be <InlineFieldValue>false</InlineFieldValue>.
...
* Add the desired space-separated [Google OAuth2 scopes](https://developers.google.com/identity/protocols/oauth2/scopes) to <InlineField>Scope</InlineField>. The most commonly requested scopes are `openid`, `profile` and `email`. You must include `email` if you're using the [linking on email](/docs/lifecycle/authenticate-users/identity-providers/#linking-strategies) strategy.
...
Google identity providers configured with a <InlineField>Login Method</InlineField> of `Use popup for login` or `Use vendor JavaScript` use the GSI client. This provides a more modern and dynamic UX and support for Google One Tap.
...
2. Providing alternate Google IdP configurations with the `Use redirect for login` <InlineField>Login Method</InlineField>
