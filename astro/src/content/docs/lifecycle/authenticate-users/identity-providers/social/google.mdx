...
Now that you have selected or created your first project, navigate to <Breadcrumb>APIs & Services -> Credentials</strong> and then click on <strong>Create credentials</Breadcrumb> and select <InlineField>OAuth client ID</InlineField>.
...
Navigate to <Breadcrumb>APIs & Services -> Credentials</strong> and then click on <strong>Create credentials</Breadcrumb> and select <InlineField>OAuth client ID</InlineField>.
...
The last step will be to create a Google Identity Provider in FusionAuth. To create an Identity Provider navigate to <Breadcrumb>Settings -> Identity Providers</Breadcrumb> and click the menu in the upper right corner and select `Add Google`.
