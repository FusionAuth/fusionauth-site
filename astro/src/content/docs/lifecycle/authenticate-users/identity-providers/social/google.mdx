...
* Add your needed scopes to <InlineField>Scope</InlineField>. The most common requested scopes are `openid`, `profile` and `email`, but you can [use any of the Google OAuth2 scopes](https://developers.google.com/identity/protocols/oauth2/scopes). If you are [linking on email](/docs/lifecycle/authenticate-users/identity-providers/#linking-strategies), you must include the `email` scope.
