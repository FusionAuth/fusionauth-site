---
title: Add an External SAML v2 IdP
description: Learn how to set up user log in using the SAML v2 Identity Provider.
order: 1
tags: saml, set up saml v2, saml v2, saml v2 idp, idp
---
...
This guide explains how to add a login button that enables users to authenticate through an external SAML v2 Identity Provider (IdP).

<Aside type="note">
This page explains how to add an external SAML v2 Identity Provider to your FusionAuth instance: this enables users to log in to FusionAuth applications via a third-party SAML v2 Identity Provider.

To instead use FusionAuth as a SAML v2 Identity Provider for other applications (where FusionAuth provides the system of record for users), see [Host a SAML v2 Identity Provider](/docs/lifecycle/authenticate-users/saml).
</Aside>

We also provide examples for SAML providers whose implementation requires unique configuration:
...
Once configured, you can enable the SAML v2 login button for applications. The following login page provides a way to log in via the PiedPiper SAML v2 Identity Provider:

![SAML v2 Login](/img/docs/lifecycle/authenticate-users/identity-providers/login-samlv2.png)


## Add a SAML v2 Identity Provider

<IdpApplicationConfiguration identity_provider="SAML v2" details_link="/docs/lifecycle/authenticate-users/identity-providers/overview-samlv2#form-fields" image="/img/docs/lifecycle/authenticate-users/identity-providers/identity-provider-samlv2-add-top.png"/>
...
<APIBlock>
  ...
  <APIField name="Source" optional since="1.65.0" defaults="System">
    The source of this identity provider.

    This value is set only when creating the identity provider. If provided on update, it is ignored and the existing value is preserved.
  ...
  <APIField name="Verification keys" required since="1.69.0">
    The keys stored in Key Master that are used to verify the SAML response sent back to FusionAuth from the identity provider. These are the public key(s) provided to you by the identity provider.
  </APIField>
  ...
  <APIField name="Issuer" optional>
    The service provider EntityId used by FusionAuth for this SAML v2 identity provider.

    If not provided, FusionAuth uses <code>&lt;base_url&gt;/samlv2/sp/&lt;identityProviderId&gt;</code>.
  </APIField>
  <APIField name="Expected Issuer" optional since="1.43.0">
    The expected EntityId (unique identifier) of the external SAML v2 identity provider.

    Required when **Enable IdP-initiated login** is enabled.
  </APIField>
...
![Add SAML v2 <Breadcrumb>Options</Breadcrumb> section](/img/docs/lifecycle/authenticate-users/identity-providers/identity-provider-samlv2-add-options.png)
...
<APIBlock>
  ...
  <APIField name="Request signing key" optional>
    The key used to sign the SAML request. Required when **Sign request** is enabled. To create, manage, or import a key, navigate to <Breadcrumb>Settings -> Key Master</Breadcrumb>.
  </APIField>
  ...
  <APIField name="Login hint parameter name" optional since="1.43.0">
    The name of the parameter used to pass the username or email as login hint to the IDP when HTTP redirect bindings are used to initiate the AuthN request. Required when **Enable login hint** is enabled.
  </APIField>
  ...
  <APIField name="Assertion decryption key" optional since="1.55.1">
    The key used to decrypt SAML responses. Required when **Require encrypted assertions** is enabled. To create, manage or import a key, navigate to <Breadcrumb>Settings -> Key Master</Breadcrumb>.
...
![Add SAML v2 <Breadcrumb>Managed Domains</Breadcrumb> section](/img/docs/lifecycle/authenticate-users/identity-providers/identity-provider-samlv2-add-managed-domains.png)
...
After configuring the Identity Provider, FusionAuth will display values likely required by your SAML v2 Identity Provider to trust FusionAuth as a relying party. Do so by navigating to <Breadcrumb>Settings -> Identity Providers</Breadcrumb>, click the action menu next to your SAML provider, then select the <Breadcrumb>View</Breadcrumb> button.
...
When viewing the details, scroll to the <Breadcrumb>SAML v2 Integration details</Breadcrumb> section. There you will find the necessary values to configure an integration with a SAMLv2 IdP.

FusionAuth resolves the service provider EntityId from the `Issuer` value in this section. If you do not configure `identityProvider.issuer`, FusionAuth uses `<base_url>/samlv2/sp/<identityProviderId>`. If you do configure `identityProvider.issuer`, FusionAuth uses that value instead.

FusionAuth expects the Audience value in the SAML assertion to match this same value.
...
To complete the login request, the SAML v2 Identity Provider will make an HTTP `POST` request to the callback URL in FusionAuth. In order for this request to be allowed through the CORS filter you will need to navigate to <Breadcrumb>Settings -> System -> CORS</Breadcrumb> and add the SAML IdP origin as an **Allowed Origin** the CORS configuration.
...
* Make sure that `https://example.com/welcome` is added to the **Authorized Redirect URLs** field in the Application configuration.
...
See [SAML v2 Integration Details](/docs/lifecycle/authenticate-users/identity-providers/overview-samlv2#integration-details) for additional details.
...
To troubleshoot, turn on the **Debug** option and then navigate to <Breadcrumb>System -> Event Log</Breadcrumb>.
