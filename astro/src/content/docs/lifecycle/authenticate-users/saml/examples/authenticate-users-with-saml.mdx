...
If Corpo were to act as the identity provider instead of Changebank, then the steps in this guide would not work. You can read more about [using FusionAuth as a service provider to connect to another identity provider in the documentation](/docs/lifecycle/authenticate-users/identity-providers/overview-samlv2).
...
You've just logged in using basic OAuth2 authentication. Now you need to configure FusionAuth to use SAML.
...
- Set <InlineField>Issuer</InlineField> to `passport-saml`. (This matches the value sent by Passport from the website you configure in the next section.)
- Set <InlineField>Authorized redirect URLs</InlineField> to `http://localhost:3000/saml/callback`. (Be sure to click the dropdown text that appears to confirm the entry. It should read, "Add URL http://localhost:3000/saml/callback".)
- Toggle the <InlineUIElement>Debug enabled</InlineUIElement> option on. (This allows you to [see any errors](/docs/operate/troubleshooting/troubleshooting#enabling-debugging) that may occur.)
...
- Add the `Login URL` from the SAML metadata `SingleSignOnService` element's `Location` attribute to `app/.env`. Again, your value will look similar, but the UUID will be different.
  ```
...
- Now you want to let the Changebank application interact with SAML. You'll be using `passport.js`, a common Node.js library for authentication. The steps will be similar for any SAML library, though the terms and specific tasks might be slightly different. Add the following line to the top of the `dependencies` section of `package.json`.
  ...
  ```
- Add the lines below to the top of `app/services/authentication.js`, after the last `require` statement.
  <RemoteCode url={frontmatter.codeRoot + "/services/authentication.js"} lang="javascript" tags="a" />
- In the same file, add the following line to the end of the `setupPassport` function.
  <RemoteCode url={frontmatter.codeRoot + "/services/authentication.js"} lang="javascript" tags="b" />
- At the end of the same file, add the function below.
  <RemoteCode url={frontmatter.codeRoot + "/services/authentication.js"} lang="javascript" tags="c" />
...
- In the middle of the `app/routes/index.js` file, replace the current OAuth login line with a SAML login and add a callback function.
  <RemoteCode url={frontmatter.codeRoot + "/routes/index.js"} lang="javascript" tags="a" />

The code changes to the `routes` file are minimal, barely changing what is in the Passport documentation. You replace the OAuth login route (because, in this example, the customer wanted to switch entirely to SAML), add a SAML login route to direct the user to FusionAuth, and use a callback handler to direct the logged in user to their account.
...
You can now browse to http://localhost:3000 and log in exactly the same way as before, with `richard@example.com` and `password`, except you're now using SAML. If you see an error that your account isn't registered, it's because you logged in previously with the administration account. Try to log in using an incognito browser window instead.
...
If you log out and log in again without seeing the password form, it's because you enabled <InlineUIElement>Keep me signed in</InlineUIElement> on the form last time.

In the FusionAuth web interface, you can browse to <Breadcrumb>System -> Login Records</Breadcrumb> and <Breadcrumb>System -> Event Log</Breadcrumb> to see the SAML logs for your login.
