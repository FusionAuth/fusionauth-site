...
<JSON title="Recovery Codes Response JSON" src="two-factor/response-with-recovery-codes.json" />
...
As a developer, you can manage them via the [Recovery Code API](/docs/apis/two-factor#generate-recovery-codes). As of `1.67.0`, recovery codes are hashed at rest — they are only available in plaintext at the moment of generation and cannot be retrieved afterward.
