...
Go back to your [Aiven console](https://console.aiven.io/) and click <strong>Admin</strong> and then on <strong>Authentication</strong> in the sidebar to list the authentication methods again. Click on <InlineUIElement>FusionAuth</InlineUIElement>, which will still have a `PENDING CONFIGURATION` state until you finish setting it up.
