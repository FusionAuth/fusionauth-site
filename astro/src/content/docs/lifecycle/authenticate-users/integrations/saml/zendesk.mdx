...
<iframe allowfullscreen="" frameborder="0" width="560" height="315" src="//www.youtube.com/embed/QYuTOD8wjZU?rel=0" enablejsapi="true" data-gtm-yt-inspected-8="true"></iframe>
...
First, we want to create a new application named, `Zendesk`, in FusionAuth. Navigate to <strong>Applications</strong> and create an application with the following two roles:
...
Next, we'll create a lambda to take FusionAuth role information and populate the SAML assertion that Zendesk will receive. Navigate to <strong>Settings -> Lambdas</strong> and click the add button.
...
Click save, and then return to the `Zendesk` FusionAuth application by navigating to <strong>Applications</strong>
...
Next, navigate to <strong>Users</strong> to edit the user you have previously set up or create a new one. Create a registration to add that user to the `Zendesk` application and give them the appropriate role. If you don't give them an `admin` or `agent` role, they'll default to a `user` Zendesk role.

Finally, go to <strong>Settings -> Key Master</strong> and view the `Signing Key` you created or chose. Copy the `Fingerprint (SHA-256)` value. This will be something like `FF:74:12:A5:40:67:E9:90:24:FC:95:07:FC:B7:E6:36:9B:26:75:6B:24:9D:3E:49:0A:43:4D:BC:03:00:DD:AA`.
