...
tags: zendesk, saml v2, zendesk sso, saml integration, fusionauth saml, customer support sso
---
...
![Creating FusionAuth application with Zendesk roles.](/img/docs/lifecycle/authenticate-users/integrations/saml/create-application-and-roles.png)
...
![Creating a lambda to populate roles.](/img/docs/lifecycle/authenticate-users/integrations/saml/create-role-lambda.png)

Click save, and then return to the `Zendesk` FusionAuth application by navigating to <Breadcrumb>Applications</Breadcrumb>

* Edit the application.
* Go to the <Breadcrumb>SAML</Breadcrumb> tab.
* Enable SAML.
...
![Application SAML configuration.](/img/docs/lifecycle/authenticate-users/integrations/saml/saml-config.png)

Click <Breadcrumb>Save</Breadcrumb>. Now view the application configuration and scroll down to the <Breadcrumb>SAML v2 Integration details</Breadcrumb> section. Copy the following information:

* `Login URL`
* `Logout URL`

![The SAML Integration Details.](/img/docs/lifecycle/authenticate-users/integrations/saml/saml-details-for-application.png)
...
The [general Zendesk SSO instructions](https://support.zendesk.com/hc/en-us/articles/4408887505690-Enabling-SAML-single-sign-on) are worth reading.

To configure Zendesk to use FusionAuth to manage your users, head to your application's security center, then to the <Breadcrumb>Single sign-on</Breadcrumb> section. This is a direct URL to that section: `https://fusionauth-example.zendesk.com/admin/security/sso`
...
![The Zendesk SSO configuration screen.](/img/docs/lifecycle/authenticate-users/integrations/saml/zendesk-saml-config.png)
...
Proceed to the <Breadcrumb>Staff members</Breadcrumb> section. Check <Breadcrumb>External authentication</Breadcrumb> and select <Breadcrumb>Single sign-on</Breadcrumb>. You should see that SAML is enabled. Save the configuration.

Navigate to the <Breadcrumb>End users</Breadcrumb> section. Check <Breadcrumb>External authentication</Breadcrumb>. You should see that SAML is enabled. Save the configuration.
