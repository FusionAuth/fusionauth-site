...
The secure communication channel could be an email, text message, Slack message, or any other service you connect using a [Messenger](/docs/customize/email-and-messages/messengers).
...
* [Passkeys](/docs/authenticate-users/lifecycle/passkeys/): frequently called **Passwordless** authentication, distinct from Magic Links and Codes because Passkeys are built upon the WebAuthn standard for public/private key exchange in lieu of a password.
* [Multi-Factor Authentication (MFA, sometimes referred to as 2FA)](/docs/lifecycle/authenticate-users/multi-factor-authentication): often uses the term **TOTP** (Time-based One-Time Password), easily confused with OTP (One-Time Password). While TOTP and OTP are quite similar (in fact, OTP is also time-constrained), MFA TOTPs are a secondary form of authentication, not a primary form like Magic Links and Codes. You can authenticate with _only_ a Magic Link or Code, but you cannot authenticate with only a MFA TOTP.

## Strategies

When you use One-Time Passwords for authentication, you must choose a **strategy**:

* **Clickable link**: a **magic link** that the user clicks to visit and authenticate with your application; a popular choice for email authentication flows when users access their email account on the same device where they wish to access your application
* **Form field**: a **magic code** that the user copies and pastes (or memorizes and types); the preferred choice for phone-based authentication flows, since the user might prefer to use your application on a device other than their phone
