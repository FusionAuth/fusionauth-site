...
The OpenID Connect prompt parameters provides additional flexibility to control the login workflow.
...
To do this, add `prompt=none` to the request. Make this request programatically and if the `Location` header on the response contains `error=login_required` the user requires authentication. Additionally, FusionAuth will return `error_reason` and `error_description` as parameters to provide additional information to the developer. The `error_reason` is a specific a reason code that corresponds to `login_required` tell you why login is required. This will most likely be `authentication_required`, and `error_description` will contain a human readable version of the `error_reason` such as `The user is required to complete authentication.`.
