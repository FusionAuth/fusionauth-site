...
The reason for this decision is because `prompt` is an optional parameter in the OpenID specification and we cannot know if every OpenID Connect identity provider will honor this parameter. Additionally FusionAuth supports many identity providers, many of which are actually OAuth2, or partial implementations of OAuth2 and OpenID Connect. If we consider all supported identity providers, it is unlikely that all will honor this parameter, and more importantly sending unknown or unexpected parameters to some identity providers will cause a failure.
...
If you are using the `idp_hint` request parameter and also include `prompt=login` FusionAuth will still redirect to the identity provider resolved by request parameter.

The reason for this decision is that when the `idp_hint` is provided, you have provided explicit instruction to FusionAuth that this user must be authenticated by a third party and often the caller does not want the user to see the FusionAuth login page. In this context, it would not be useful to then show the user the FusionAuth login page knowing FusionAuth will not be able to authenticate the user. It is possible this is not the desired behavior for all customers.

If you encounter this edge case, you should remove the `prompt=login` from the request. Using a `login_hint` request parameter with managed domains will result in the same behavior.  This may be improved in the future by allowing for additional identity provider configuration for prompt handling, see the above section on third party identity providers for additional details.
