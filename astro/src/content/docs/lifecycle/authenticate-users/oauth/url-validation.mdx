...
In order to validate allowed authorized redirect and origin URLs containing wildcards, the <InlineField>URL validation</InlineField> setting must be configured to <InlineFieldValue>Allow wildcards</InlineFieldValue> under <Breadcrumb>Applications -> Edit Application -> OAuth</Breadcrumb>.
