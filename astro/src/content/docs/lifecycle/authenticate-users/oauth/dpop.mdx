...
When you use DPoP, the APIs receiving the access token will need to take additional steps to validate that the access token was sent by the correct client. The [FusionAuth JavaScript SDK](https://github.com/FusionAuth/fusionauth-javascript-sdk) provides built-in support for DPoP on the client side. If you are implementing DPoP validation yourself, you can implement the checks [outlined in the RFC](https://datatracker.ietf.org/doc/html/rfc9449#name-the-dpop-authentication-sch):
