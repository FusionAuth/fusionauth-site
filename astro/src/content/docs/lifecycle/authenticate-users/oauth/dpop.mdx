...
When you use DPoP, the APIs receiving the access token will need to take additional steps to validate that the access token was sent by the correct client. The React, Angular, and Vue packages in the [FusionAuth JavaScript SDK](https://github.com/FusionAuth/fusionauth-javascript-sdk) provide built-in support for generating DPoP proofs on the client side, but you are still responsible for validating DPoP proofs in your own APIs (the Resource Server).  You can implement these checks as [outlined in the RFC](https://datatracker.ietf.org/doc/html/rfc9449#name-the-dpop-authentication-sch):
