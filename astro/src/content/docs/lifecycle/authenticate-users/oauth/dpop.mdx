...
If your Resource Server requires a nonce for temporal replay protection, they will respond with a `401` or `400` error and a `WWW-Authenticate: DPoP error="use_dpop_nonce"` header containing a `DPoP-Nonce`.
