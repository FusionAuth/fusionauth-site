...
* `phone` - Populates the `phone_number` claim based on the user's <InlineField>user.phoneNumber</InlineField> and <InlineField>user.mobilePhone</InlineField> value in order of precedence. If <InlineField>user.phoneNumber</InlineField> is available, the `phone_number_verified` claim will also be included.
