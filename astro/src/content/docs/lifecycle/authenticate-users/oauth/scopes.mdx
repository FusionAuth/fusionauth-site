...
In version `1.50.0` FusionAuth introduced new features around OAuth scope handling to provide more control over requested scopes and allowing you to tailor your application's behavior to match your security requirements.

This page details these new features and explains how you can use them to customize FusionAuth behavior.
...
The `Compatibility` option is meant to maintain backwards compatibility on upgrade or when the integration does not support the requirements of `Strict` mode.
...
These scope names are reserved and cannot be used to define [custom scopes](/docs/get-started/core-concepts/scopes).
Each of the scopes can be individually `Enabled` and `Required`.
...
Provided scopes configured as `Required` must be granted user consent during the OAuth [consent prompt](#consent-prompt) in order to successfully complete the OAuth workflow.

## Consent mode

<AdvancedEditionBlurb />

An application whose <InlineField>Relationship</InlineField> has been configured as `Third-party` (requires license) has the option to prompt users to consent to the requested OAuth scopes in order to complete the OAuth workflow.
...
Applications can also define [custom scopes](/docs/get-started/core-concepts/scopes) to be requested by third-parties for more fine-grained access control to APIs or actions.

The <InlineField>Consent mode</InlineField> controls when users will be prompted to grant consent to the requested scopes.

* `Always prompt` - The user will be prompted during each OAuth workflow. This is the preferred option because it provides maximum transparency and gives users the chance to adjust the granted scopes each time they authenticate.
* `Remember decision` - The user's consent grants will be persisted. See below for detail on when users will be prompted again.
* `Never prompt` - The user will never be prompted to grant consent to requested scopes. This effectively makes the third-party application behave as if it was first-party when it comes to granting consent to requested scopes. This option is meant for testing purposes.
...
* An optional scope that was previously declined is requested and now required
