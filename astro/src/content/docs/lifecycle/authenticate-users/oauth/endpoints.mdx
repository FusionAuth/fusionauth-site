...
<APIBlock>
  ...
  <APIField name="max_age" type="Integer" optional since="1.60.0">
    ...
    If the user does not have a SSO session, this parameter will have no effect. If the user does have a SSO session and it is older than specified by this parameter, the user will be required to authenticate again.

    A value of `0` is equal to `prompt=login`.

    Note that this parameter by itself does not offer any guarantee that the user has been reauthenticated. See [OIDC Prompt Security Considerations](/docs/lifecycle/authenticate-users/oauth/prompt#security-considerations) for implementation details.
  </APIField>
  ...
  <APIField name="prompt" type="String" optional since="1.60.0">
    ...
    * `consent` - The user will be prompted for consent regardless of a previous consent decision to approve or reject a scope.
    ...
    Note that `prompt=login` by itself does not offer any guarantee that the user has been reauthenticated. See [OIDC Prompt](/docs/lifecycle/authenticate-users/oauth/prompt#security-considerations) for implementation details, security considerations, limitations and example usage.
  </APIField>
...
<APIBlock>
  ...
  <APIField name="auth_time" type="Long">
    The time of the initial authentication request, expressed as [unix time](/docs/reference/data-types#unix-time). This value of this claim will not change even if the token is re-issued through the use of a Refresh Token.
  </APIField>
