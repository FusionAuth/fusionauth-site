...
htmlTitle: "OAuth2 Endpoints | FusionAuth"
...
<APIBlock>
  <APIField name="Authorization" type="String" optional>
    An optional request header allowing the the SSO session to be bootstrapped using an existing access token (JWT).
...
<APIBlock>
  ...
  <APIField name="max_age" type="Integer" optional since="1.60.0">
    An optional parameter indicating the maximum age in seconds of the authenticated SSO session. The OpenID specification calls this the Maximum Authentication Age.

    If the user does not have a SSO session, this parameter will have no effect. If the user does have a SSO session and it is older than specified by this parameter, the user will be required to authenticate again. When this parameter is used, and the SSO session is older than the value specified, this is equivalent to requesting `prompt=login`.

    The age of the SSO session is calculated from the start of the authentication event.

    A value of `0` is equal to `prompt=login`.

    Note that this parameter by itself does not offer any guarantee that the user has been reauthenticated. See [OIDC Prompt Security Considerations](docs/lifecycle/authenticate-users/oauth/prompt#security-considerations) for additional and implementation details.
  </APIField>
  ...
  <APIField name="prompt" type="String" optional since="1.60.0">
    An optional space delimited, case sensitive parameter that allow for additional control for silent authentication, forced reauthentication and consent prompts.
    ...
    * `consent` - The user will be prompted for consent regardless of a previous consent decision to approve or reject a scope.
    ...
    See [OIDC Prompt](docs/lifecycle/authenticate-users/oauth/prompt) for additional information, security considerations, limitations and example usage.
  </APIField>
...
<APIBlock>
  ...
  <APIField name="pendingIdPLink.identityProviderUserId" type="String">
    ...
    This is the value that FusionAuth will use to uniquely link a FusionAuth User to a user in the third party identity provider.
  </APIField>
...
<APIBlock>
  ...
  <APIField name="auth_time" type="Long">
    The time of the initial authentication request, expressed as [unix time](/docs/reference/data-types#unix-time). This claim will remain the same even when the token has been re-issued through the use of a Refresh Token.
  </APIField>
