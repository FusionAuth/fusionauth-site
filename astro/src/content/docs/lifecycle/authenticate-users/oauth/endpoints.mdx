...
<APIBlock>
  ...
  <APIField name="scope" type="String" optional>
    ...
    One scope modifies the behavior of the endpoint:
...
<APIBlock>
  ...
  <APIField name="scope" type="String" since="1.50.0">
    ...
    The value contains the same scopes as the Device Authorize request with the following scopes excluded:
    * `openid`
    * `offline_access`
...
<APIBlock>
  <APIField name="Authorization" type="String" required>
    ...
    <Aside type="caution">
      In version `1.50.0` and later, the `aud` claim is required for all tokens. The `aud` claim value, or the first value in the case the claim contains a list, must correspond to an active application.
    ...
    <Aside type="note">
      In version `1.50.0` and later when the application's <InlineField>oauthConfiguration.scopeHandlingPolicy</InlineField> is set to `Strict`, the provided token must:

      * Be an access token rather than an id token
      * Contain the `openid` scope in the `scope` claim

      in order to be used with this endpoint.
    </Aside>
  </APIField>
</APIBlock>
...
<Aside type="note">
  ...
  * In `Strict` mode the response claims are populated based on the `scope` values of the provided token and available information on the User object. This behavior more closely aligns with specification recommendations.
  * In `Compatibility` mode the response claims are populated based on available information on the User object only and include some claims from the provided token. This is legacy behavior maintained for backwards compatibility.
</Aside>
