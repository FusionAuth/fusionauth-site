...
<APIBlock>
  ...
  <APIField name="idp_hint" type="UUID" optional since="1.19.0">
    ...
    Adding this request parameter will cause the FusionAuth login page to be skipped, and instead a `302` will be returned with a `Location` header of the IdP login URL. The end result is functionally equivalent to the end user clicking on the <InlineUIElement>Login with Pied Piper</InlineUIElement> button, or entering their email address when using the IdP configuration with managed domains.
  </APIField>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    ...
    This parameter may be provided multiple times to indicate multiple resource servers: `?resource=https://api.example.com&resource=https://other.example.com`

    Each value must be an absolute URI and may not contain a fragment (`#`). All provided values must be present in the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>; any invalid value returns an `invalid_target` error.

    When <InlineField>oauthConfiguration.authorizedResourceUris</InlineField> is empty, this parameter is silently ignored. The resource URIs are bound to the authorization code and carried forward to the token endpoint.
  </APIField>
...
<APIBlock>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    The URI of the target resource server, per RFC 8707 Resource Indicators for OAuth 2.0.

    This parameter may be provided multiple times. The following validation rules are applied in order; any failure returns `invalid_target` and the authorization code is immediately invalidated:

    1. If at least one resource URI from the authorize request is still present in the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>, then at least one `resource` value is required here. (If all authorize-time resources have since been removed from the application config, this requirement is lifted.)
    2. Each provided value must be present in the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>.
...
<APIBlock>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    ...
    This parameter may be provided multiple times. Behavior depends on the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>:

    * When <InlineField>oauthConfiguration.authorizedResourceUris</InlineField> is empty, this parameter is silently ignored.
    * When omitted and <InlineField>oauthConfiguration.authorizedResourceUris</InlineField> is non-empty, the resource URIs stored in the refresh token's `metaData.resources` from the original authorization are automatically applied to the new access token.
    * When provided, each value must be present in both the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField> and the refresh token's `metaData.resources` (i.e. must be a subset of what was authorized originally). Any invalid value returns `invalid_target`.
...
<APIBlock>
  ...
  <APIField name="aud" type="String">
    ...
    <Aside type="version" title="Available since 1.67.0">
      When the application has a non-empty <InlineField>oauthConfiguration.authorizedResourceUris</InlineField> and a `resource` parameter was resolved during the OAuth flow, this value changes from a plain string to an array. The first element is always the `client_id` followed by each resolved resource URI: `["client_id", "https://resource1.example.com", ...]`.
    </Aside>
  </APIField>
