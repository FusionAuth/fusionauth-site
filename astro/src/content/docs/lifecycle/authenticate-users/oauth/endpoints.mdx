...
<APIBlock>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    ...
    1. If at least one resource URI from the authorize request is still present in the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>, then at least one `resource` value is required here.
...
<APIBlock>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    ...
    To provide multiple URIs, pass each URI as a separate parameter with the same name. Behavior depends on the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>:
