...
<APIBlock>
  ...
  <APIField name="resource" type="String" optional since="1.67.0">
    ...
    All provided values must be present in the application's <InlineField>oauthConfiguration.authorizedResourceUris</InlineField>; any invalid value returns an `invalid_target` error.

    When <InlineField>oauthConfiguration.authorizedResourceUris</InlineField> is empty, this parameter is silently ignored. Otherwise, the validated resource URIs are bound to the authorization code and carried forward to the token endpoint.
  </APIField>
