...
<APIBlock>
  ...
  <APIField name="email_verified" type="Boolean">
    The OpenID Connect claim that's `true` if the user's email is verified.
  ...
  <APIField name="phone_number_verified" type="Boolean" since="1.59.0">
    The OpenID Connect claim that's `true` if the user's phone number is verified. This claim is based only on the <InlineField>user.phoneNumber</InlineField> verification status.
