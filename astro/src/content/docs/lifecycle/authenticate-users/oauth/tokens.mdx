...
A refresh token is associated with the same set of validated and consented OAuth scopes from the initial authentication request. The [Refresh Token Grant Request](/docs/api/oauth#refresh-token-grant-request) can include a subset of the original scopes to request a new token with a narrower scope.
