...
Navigate to the Application configuration from the main menu, <strong>Applications</strong>.
...
See the <ScrollRef target="Configure Application OAuth Settings" /> section above.
...
This request may be made with the optional <InlineField>scope</InlineField> field with a value of `offline_access` if you would like a refresh token provided on the eventual [`/oauth2/token` endpoint](/docs/lifecycle/authenticate-users/oauth/endpoints#complete-the-device-authorization-grant-request) return. This request will return a JSON response with values necessary to fulfill the remainder of the grant flow.
