...
Review the [Authorization](/docs/api/oauth#authorize) and [Token](/docs/api/oauth#token) endpoint documentation for additional detail on these necessary request parameters.
</Aside>
...
Review the [Authorization](/docs/api/oauth#authorize) endpoint documentation for more detail.
...
Review the [Token](/docs/api/oauth#token) endpoint documentation for more detail. The following is an example redirect URI containing the authorization code.
...
You may also choose to use the [Introspect](/docs/api/oauth#introspect) or [Userinfo](/docs/api/oauth#userinfo) endpoints to validate the access token returned from the Token endpoint and to provide you decoded claims as a JSON object.
...
In order to initiate the Device Authorization Grant flow, make a request from the device to the [Device Authorize endpoint](/docs/api/oauth#device), which is also discoverable via the [OpenID Configuration](/docs/api/oauth#openid-configuration).
...
This request may be made with the optional <InlineField>scope</InlineField> field in order to request OAuth scopes on tokens from the eventual [`/oauth2/token` endpoint](/docs/api/oauth#complete-the-device-authorization-grant-request) return. A refresh token can be requested by including `offline_access` in the <InlineField>scope</InlineField> request parameter.
...
### Poll Token endpoint
Upon receiving a response from the Device Authorize endpoint the device may begin polling the [Token endpoint](/docs/api/oauth#complete-the-device-authorization-grant-request) with the <InlineField>device_code</InlineField> at the requested <InlineField>interval</InlineField> in seconds returned in the response.
Requests to the [Token endpoint](/docs/api/oauth#complete-the-device-authorization-grant-request) will return an error stating that authorization is pending, until the end-user approves the request, at which point an access token will be returned.
...
Once the `user_code` has been received from the end-user, it may be validated by making a request to the [Device Validate endpoint](/docs/api/oauth#device-validate).
...
See the [Authorize endpoint](/docs/api/oauth#authorize) and [Token endpoint](/docs/api/oauth#token) documentation for more information.

![OAuth Device Example - Success](/img/docs/lifecycle/authenticate-users/oauth/oauth-device-success.png)

### Receive `access_token`
Once the user has provided a valid `user_code` and successfully authenticated, the request from the device to the [Token endpoint](/docs/api/oauth#complete-the-device-authorization-grant-request) will return successfully with an access token.
