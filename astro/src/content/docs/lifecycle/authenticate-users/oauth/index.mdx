...
order: 0
tags: machine to machine, oauth, oauth2, oauth 2.0, oauth 2
---
...
Navigate to the Application configuration from the main menu, <Breadcrumb>Applications</Breadcrumb>.

If you have already created a FusionAuth Application for the purpose of your application, you do not need to create another, however you will still need to complete the OAuth configuration. If an application has not yet been created, click <Breadcrumb>Add</Breadcrumb> and name your application accordingly and fill out the OAuth configuration.

In this example you will see we have created a new Application named `Pied Piper` and have filled out the fields in the <Breadcrumb>OAuth Configuration</Breadcrumb> tab. A FusionAuth application represents an authenticated resource that you will be using with FusionAuth.

Additional OAuth controls can be managed through [Tenant Configuration](/docs/get-started/core-concepts/types/tenants#tenant-configuration).

![Application OAuth Configuration](/img/docs/lifecycle/authenticate-users/oauth/oauth-application.png)
...
The Client Credentials grant takes place between two Entities. You can learn more about [Entities](/docs/get-started/core-concepts/types/entity-management) in the <Breadcrumb>Core Concepts</Breadcrumb> section.
...
![Setting up an Entity](/img/docs/lifecycle/authenticate-users/oauth/add-entity-client-credentials.png)

Below is the management screen for an Entity where you'd add or remove Grants:

![Adding a Grant to an Entity](/img/docs/lifecycle/authenticate-users/oauth/add-grant.png)
...
Review the [Authorization](/docs/apis/oauth/authorize) and [Token](/docs/apis/oauth/token) endpoint documentation for additional detail on these necessary request parameters.
</Aside>
...
See the [Retrieve a User](/docs/apis/users/retrieve) API for examples.

You may also choose to use the [Introspect](/docs/apis/oauth/introspect) or [Userinfo](/docs/apis/oauth/userinfo) endpoints to validate the access token returned from the Token endpoint and to provide you decoded claims as a JSON object.
...
This will redirect to the configured OAuth **redirect_uri** per the typical Authorization Grant flow.
The Device Authorization Grant will be considered approved when the Authorization Grant **code** has been exchanged for a token.
...
These values may be adjusted through the <Breadcrumb>Advanced</Breadcrumb> tab of [Tenant Configuration](/docs/get-started/core-concepts/types/tenants#advanced).

### Initiate the Device Authorization Grant flow

In order to initiate the Device Authorization Grant flow, make a request from the device to the [Device Authorize endpoint](/docs/apis/oauth/device), which is also discoverable via the [OpenID Configuration](/docs/apis/oauth/openid-configuration).

{/*  If I have this paragraph be part of the other one, I think the # in the links causes the **scope** text to be misinterpreted */}

This request may be made with the optional **scope** field in order to request OAuth scopes on tokens from the eventual [`/oauth2/token` endpoint](/docs/apis/oauth/token#complete-the-device-authorization-grant-request) return. A refresh token can be requested by including `offline_access` in the **scope** request parameter.
The **scope** value here will be used for the interactive portion of the workflow when the [user_code is passed to FusionAuth](/docs/lifecycle/authenticate-users/oauth#pass-user_code-to-fusionauth). See [Scopes](/docs/lifecycle/authenticate-users/oauth/scopes) for more information on configuring the application's OAuth scope policies, managing custom OAuth scopes, and prompting for user consent to granting scopes to third-party applications.

This request will return a JSON response with values necessary to fulfill the remainder of the grant flow.

![OAuth Device Example - Connect](/img/docs/lifecycle/authenticate-users/oauth/oauth-device-connect.png)
...
### Poll Token endpoint
Upon receiving a response from the Device Authorize endpoint the device may begin polling the [Token endpoint](/docs/apis/oauth/token#complete-the-device-authorization-grant-request) with the **device_code** at the requested **interval** in seconds returned in the response.
Requests to the [Token endpoint](/docs/apis/oauth/token#complete-the-device-authorization-grant-request) will return an error stating that authorization is pending, until the end-user approves the request, at which point an access token will be returned.
...
### User-interaction
Upon receiving a response from the Device Authorize endpoint the device may display to the end-user the **user_code** and a prompt to navigate to the **verification_uri**.
The **verification_uri_complete** is provided as a convenience so that the device may display a QR code used to navigate the end-user to the user-interaction page with a pre-populated **user_code** in the form.

![OAuth Device Example - Display Code](/img/docs/lifecycle/authenticate-users/oauth/oauth-device-display-code.png)

The user should then navigate to the displayed URL, and enter the activation code.

![OAuth Device Example - User Interaction](/img/docs/lifecycle/authenticate-users/oauth/oauth-device-user-interaction.png)

### Pass `user_code` to FusionAuth
Once the `user_code` has been received from the end-user, it may be validated by making a request to the [Device Validate endpoint](/docs/apis/oauth/device#device-validate).
...
See the [Authorize endpoint](/docs/apis/oauth/authorize) and [Token endpoint](/docs/apis/oauth/token) documentation for more information.

![OAuth Device Example - Success](/img/docs/lifecycle/authenticate-users/oauth/oauth-device-success.png)

### Receive `access_token`
Once the user has provided a valid `user_code` and successfully authenticated, the request from the device to the [Token endpoint](/docs/apis/oauth/token#complete-the-device-authorization-grant-request) will return successfully with an access token.
