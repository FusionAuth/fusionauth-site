...
<APIField name="Authorization" type="String" optional>
  This header is optional if you have provided the <InlineField>client_id</InlineField> in the request body.

  The <InlineField>client_secret</InlineField> may be provided in the request body as well, but may be optional depending on the OAuth grant and configuration settings. If provided, the <InlineField>client_secret</InlineField> must be valid.

  Please see [the <Breadcrumb>Client Secret</Breadcrumb> section](/docs/lifecycle/authenticate-users/oauth/endpoints#client-secret) for more details.
