...
Next, configure your tenant, under <Breadcrumb>Tenants -> Default</Breadcrumb>. Then navigate to the <Breadcrumb>Password</Breadcrumb> tab. Under the "Failed authentication settings" section, change the <InlineField>User action</InlineField> to your newly created user action, `Account Lock`.
