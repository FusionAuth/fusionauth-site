...
FusionAuth applies multi-factor authentication based on a number of configuration and situational behaviors. In this document you'll learn about the way FusionAuth automatically applies MFA during the login process.

If you are looking for guidance on implementing MFA, please [review the MFA guide](/docs/lifecycle/authenticate-users/multi-factor-authentication).
...
* What is the MFA policy for the tenant or application? It can be either `Disabled`, `Enabled`, or `Required`?
...
The duration of trust of the device can be configured with the `twoFactorTrustIdTimeToLiveInSeconds` tenant configuration value.

The goal of this contextual check is to challenge the user for another factor of authentication whenever FusionAuth determines the risk of incorrect access is higher than the user hassle.
...
<TenantMfaDecisionTree alt="Diagram of tenant MFA logic."/>

### Application Level MFA Configuration

Here's a diagram of MFA challenge logic when the application has a policy for MFA. Application policies always supersede the tenant policy.

<ApplicationMfaDecisionTree alt="Diagram of application MFA logic."/>

## License Limitations

Not all plans support all MFA features. The plan you are on affects the MFA options available to you and your users.
...
If a user logs in with an Identity Provider such as Google or OIDC, FusionAuth assumes that MFA is taken care of at the remote identity source. [Learn more about this scenario.](/docs/lifecycle/authenticate-users/identity-providers/#account-security)

There's an [open issue to add a policy to allow for more control in this scenario](https://github.com/FusionAuth/fusionauth-issues/issues/2005).
...
There are a couple of ways to do this. Let's use the example of making sure everyone in the `mfa_required` group is challenged whenever they log in. Options include:

* Set a tenant or application policy to `Enabled` and then use the API to ensure that everyone in the `mfa_required` group has an MFA method. Use a [transactional `user.update` webhook](/docs/extend/events-and-webhooks/events/user-update) to ensure that the MFA method can't be removed while the user is in that group.
* Create an MFA check application with an MFA policy of `Required` and a trust policy of `None`. After a user logs in, examine their profile and, if they are in the `mfa_required` group, redirect them to this application. They will be prompted for MFA and then redirected to the application. Users that are not in the `mfa_required` group can be sent directly through to the application. Make sure the MFA check application uses a lambda to set the `aud` and `applicationId` claims to values expected by any access token consumers.
