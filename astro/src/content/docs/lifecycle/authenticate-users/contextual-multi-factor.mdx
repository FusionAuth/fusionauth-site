...
```mermaid title="High level diagram of MFA logic."
graph TD
...
You can override this by using an [MFA requirement lambda](/docs/extend/code/lambdas/mfa-requirement) and examining the `authenticationType` parameter.
...
TODO, this is the description 

### Rolling Out Intelligent MFA

Roll out intelligent MFA in stages. Every user population is different and the goal with intelligent MFA is to reduce challenges on legitimate users and challenge when our proprietary algorithm indicates a possibility of an attacker or account takeover. 

You want to verify the behavior with your users before you enforce anything while continuing to challenge risky logins. 

The rollout plan differs by plan, because only the Enterprise plan can observe risk without acting on it.

### With an Enterprise Plan

On the Enterprise plan, any configured MFA requirement lambda logs risk and riskSignals to the event log when debug is enabled. You can use this behavior to measure first, enforce second.

1. Collect a baseline. Enable the `user.two-factor.challenge`, `user.two-factor.success`, and `user.two-factor.failed_attempt` webhooks and collect MFA metrics. You can also read the numbers on the <Breadcrumb>Reactor</Breadcrumb> tab. Calculate your current challenge rate, which is the number of MFA challenges per DAU or logins. You can pull the denominator using the [reporting API](/docs/apis/reports) or the [login API, respectively](/docs/apis/login#export-login-records). You can also collect the number of MFA related customer support requests.
1. Gather the risk scores but do not apply. Enable an [MFA requirement lambda](/docs/extend/code/lambdas/mfa-requirement), but mimics your existing policy. Behavior does not change for your users. Enable the **debug** toggle on the tenant <Breadcrumb>Multi-Factor</Breadcrumb> tab to log the user Id, composite risk, and riskSignals. If you do not already use an MFA requirement lambda, adding one means the MFA requirement decision to the lambda. You can use the sample MFA requirement lambdas below.
1. Set up an [event log create webhook](/docs/extend/events-and-webhooks/events/event-log-create) to capture the logs. Discard any that are not MFA composite risk log events. Store the values, including the composite score and the user id in a database to analyze.
1. Analyze the logs. From the log data, compare the challenge volume you would issue under `Challenge On High Risk` and `Challenge On Medium+ Risk` against your baseline. Review HIGH-scoring legitimate users and any LOW-scoring known-bad logins. The two risk policies ignore "trust this device", so users currently skipped by a trusted device are re-evaluated on risk and may be challenged.
1. Pilot risk based MFA. Use the application-level risk based policy to deliver the risk score to the lambda, and modify the lambda to enforce on `HIGH` risk for one non-critical application. Keep debug logging on and confirm challenge volume matches the shadow estimate. Collect the number of MFA related customer support requests and compare it with the baseline, corrected for number of application logins.
1. Turn off MFA debugging as your roll this out further and have more confidence in how your userbase is interacting with the MFA challenge.
1. Widen risk based MFA. Expand the configuration to more applications. Choose whether to challenge on high or medium/high per application. Use the MFA requirement lambda for any per-application or per-signal exceptions.
1. Monitor the rollout. Continue to calculate your challenge and challenge success rates. You can also watch the suspicious login webhook (the `threatsDetected` value) for trends in which threats your users detect.
...
1. Pilot risk based MFA. Set "Challenge On High Risk" on a dedicated low-traffic or internal tenant.
1. Measure the impact. Compare the Reactor counts for this tenant to your baseline.
1. Widen the rollout. Enable the policy on additional tenants, one at a time.

To roll back, set the policy to your previous MFA policy.

### Sample MFA Requirement Lambdas

Here is a sample MFA requirement lambda which mimics the `Disabled` tenant policy.
...
Here is a sample MFA requirement lambda which mimics the `Enabled` tenant policy.
...
Here is a sample MFA requirement lambda which mimics the `Required` tenant policy.
...
For more information, see [the MFA requirement lambda documentation](/docs/extend/code/lambdas/mfa-requirement).
...
1. Use a [transactional `user.update` webhook](/docs/extend/events-and-webhooks/events/user/user-update) to ensure that the MFA method can't be removed while the user is in that group.
