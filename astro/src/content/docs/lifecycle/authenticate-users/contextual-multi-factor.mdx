...
1. Gather the risk scores but do not apply the policy (optional).
    * Enable an [MFA requirement lambda](/docs/extend/code/lambdas/mfa-requirement), but mimic your existing policy so that behavior does not change for your users; see [below for sample lambdas](#sample-mfa-requirement-lambdas). 
