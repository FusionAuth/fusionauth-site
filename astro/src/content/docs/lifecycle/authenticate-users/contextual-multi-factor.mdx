...
TODO, this is the overview/description for Nathan to document. Maybe this gets its own page or changes the page title.

### Rolling Out Intelligent MFA

Every user population is different and the goal with intelligent MFA is to reduce challenges on legitimate users and challenge when our proprietary algorithm indicates a possibility of an attacker or account takeover.
...
1. Gather the risk scores but do not apply the policy.
    ...
    * Enable the **debug** toggle on the tenant <Breadcrumb>Multi-Factor</Breadcrumb> tab to log the user Id, composite risk, and riskSignals. If you do not already use an MFA requirement lambda, adding one means the MFA requirement decision to the lambda. You can use the sample MFA requirement lambdas below.
1. Capture the logs.
...
1. Widen risk based MFA rollout.
    ...
    * Turn off MFA debugging as you roll risk scoring MFA out further and have more confidence in how your userbase is interacting with the MFA challenge.
...
Here is a sample MFA requirement lambda which mimics an application policy.
...
function checkRequired(result, user, registration, context) {
  ...
  // 1. Effective login policy: the application overrides the tenant when set.
  var loginPolicy = policies.applicationLoginPolicy || policies.tenantLoginPolicy;

  // 2. Does the user have a configured MFA method?
  var methods = (user.twoFactor && user.twoFactor.methods) || [];
  var hasMethod = methods.length > 0;

  // 3. Base requirement from the login policy.
  var required;
  ...
  // 4. Honor the application trust policy. An existing trust only matters
