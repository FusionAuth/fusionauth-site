...
Not all plans support all MFA features. The plan you are on affects the MFA options available to you and your users. For more information, see [plans and features](/docs/get-started/core-concepts/plans-features).
...
<YouTube id="WwVH_ODLIJU" />

Intelligent MFA uses [risk signals](/docs/lifecycle/authenticate-users/risk-signals) integrated into FusionAuth to determine the risk level of a login attempt.
...
1. Collect a baseline (optional).
    * Enable the `user.two-factor.challenge`, `user.two-factor.success`, and `user.two-factor.failed_attempt` webhooks and collect MFA metrics. You can also read the numbers on the <Breadcrumb>Reactor</Breadcrumb> tab.
    * Calculate your current challenge rate, which is the number of MFA challenges per DAU or logins. You can pull the denominator using the [reporting API](/docs/apis/reports) or the [login API, respectively](/docs/apis/login#export-login-records).
    * You can also collect the number of MFA-related customer support requests.
1. Gather the risk scores but do not apply the policy (optional).
    * Enable an [MFA requirement lambda](/docs/extend/code/lambdas/mfa-requirement), but mimic your existing policy so that behavior does not change for your users; see [below for sample lambdas](#mfa-requirement-lambda). 
    * Enable the **debug** toggle on the tenant <Breadcrumb>Multi-Factor</Breadcrumb> tab to log the user Id, composite risk, and riskSignals. If you do not already use an MFA requirement lambda, adding one means the MFA requirement decision is delegated to the lambda. You can use the sample MFA requirement lambdas below.
1. Capture the logs (optional).
    * Set up an [event log create webhook](/docs/extend/events-and-webhooks/events/event-log-create) to capture the logs. Discard any that are not MFA composite risk log events. Store the values, including the composite score and the user Id in a database to analyze.
1. Analyze the logs (optional).
...
1. Pilot risk-based MFA. 
    * Apply an application-level risk-based policy to one non-critical application. This will **require users who don't have MFA to enable it**.
...
1. Widen risk based MFA rollout.
...
To roll back, revert the policy or the lambda to the previous value. This is a configuration change only.
...
When a signal is disabled, it will not be included in any intelligent MFA risk calculations.

![The client risk configuration menu allows you to enable and disable individual risk signals.](/img/docs/lifecycle/authenticate-users/client-risk-configuration.png)
...
When you disable an MFA method, any users who previously configured that method can continue to use the method. However, users cannot register a new instance of the disabled MFA method. To remove existing registrations for an MFA method, you must [search for users](/docs/apis/users/search) who use that MFA method (e.g. a `queryString` of `twoFactor.methods.method:email` to detect every user registered for email MFA), then script a removal of the MFA method from each User object using the [update a user API](/docs/apis/users/update).
