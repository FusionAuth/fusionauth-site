---
title: Host a SAML v2 Identity Provider
description: An overview of the SAML Identity Provider capabilities of FusionAuth.
tags: saml identity provider, saml v2, single sign-on, federated identity, saml idp, fusionauth saml, sso
---
...
To instead add an external SAML v2 Identity Provider to your FusionAuth instance, see the [Add an External SAML v2 Identity Provider](/docs/lifecycle/authenticate-users/identity-providers/overview-samlv2) documentation.
</Aside>
...
Navigate to <Breadcrumb>Applications -> Your application -> SAML</Breadcrumb> to configure this functionality. The screenshot below shows SAML configuration for the Pied Piper application:
...
<APIBlock>
  ...
  <APIField name="Logout URL" optional>
    The URL used to perform the `302` redirect as the response from the `/samlv2/logout` endpoint. If this value is omitted, the tenant configured logout URL will be used. See the **Logout URL** under the <Breadcrumb>Tenant -> Your Tenant -> OAuth</Breadcrumb> tab.
...
<APIBlock>
  ...
  <APIField name="Default verification key" required>
    ...
    If a **Require signed authentication requests** is not enabled, this field is optional.
  </APIField>
...
<APIBlock>
  <APIField name="Require signed logout requests" optional>
  ...
  <APIField name="Default verification key" required>
    ...
    If a **Require signed logout requests** is not enabled, this field is optional.
  </APIField>
...
- If the `RelayState` value does not match a configured **Authorized redirect URLs** value, the **RelayState** parameter will be treated as an opaque value and included unmodified on the request to the ACS URL. ACS URL resolution continues as follows:
  - If a `redirect_uri` parameter was provided on the IdP-initiated login request, it _must_ match a configured **Authorized redirect URLs** value, or the request will result in an error. Otherwise, the matched value will be used as the ACS URL for the workflow.
...
In FusionAuth versions _before_ `1.55.1` the `RelayState` parameter is only used for ACS URL resolution. If provided on the request, it must match a configured **Authorized redirect URLs** value, or the request will result in an error. No **RelayState** value is passed to the ACS URL.
