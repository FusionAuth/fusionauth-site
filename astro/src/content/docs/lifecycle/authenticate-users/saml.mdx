...
<APIBlock>
  ...
  <APIField name="Logout behavior" optional>
    When set to `All session participants`, each session participant that has <InlineField>Enable single logout</InlineField> set to <InlineFieldValue>true</InlineFieldValue> will be sent a Logout Request.
...
* <InlineFieldValue>id</InlineFieldValue> pulled from <InlineField>user.id</InlineField>
