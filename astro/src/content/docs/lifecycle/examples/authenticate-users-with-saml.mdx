...
Next, consider using FusionAuth as the SAML identity provider for login. Instead of letting the user log in on your site, you'll redirect them to FusionAuth to log in, where an identifying cookie will be stored in their browser, and they'll be sent back to your site. Using FA as a SAML identity provider is documented [here](https://fusionauth.io/docs/lifecycle/authenticate-users/saml). The flow below will look familiar to anyone who has used OAuth before.
...
In this section, you'll configure FusionAuth as the SAML identity provider, where users do not have to log in with Corpo.com. This is simpler than using FA as a service provider, which is done in the next section by extending the example in this section.
...
You can now browse to http://localhost:3000 and log in in exactly the same way as before, except you're now using SAML. If you see `oauth` in the browser address bar, don't worry, FA translates SAML to OAuth calls internally before returning.
...
In this section you'll implement the customer's original request — using their Corpo.com database as the SAML identity provider instead of FusionAuth. In this section you'll learn to configure FA as a service provider instead of an identity provider. Because Corpo is fictional, you'll use a free account on Okta.com act as your SAML identity provider.

<Aside type="warning">
You might want to use a second instance of FusionAuth here as your identity provider instead of Okta, but a FusionAuth SAML service provider connection to a FA SAML identity provider doesn't work. There is an issue logged for the problem [here](https://github.com/FusionAuth/fusionauth-issues/issues/2979).
</Aside>
