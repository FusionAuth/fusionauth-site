...
- [Example 1: Use FusionAuth As A SAML Service Provider](#example-1-use-fusionauth-as-a-saml-service-provider)
  ...
  - [Remove The OAuth Login Option](#remove-the-oauth-login-option)
  - [Set Up SAML Using The FusionAuth API](#set-up-saml-using-the-fusionauth-api)
- [Example 2: Use FusionAuth As A SAML Identity Provider](#example-2-use-fusionauth-as-a-saml-identity-provider)
...
You are contacted by a new corporate client, called Corpo.com, which wants all its employees to use your service. However, Corpo requires authentication to be done with SAML, using Corpo as the identity provider (the party that holds the database of users). An identity provider is often shortened to IdP. You want to add SAML authentication to your website without coding it yourself, so you are considering using FusionAuth as your gateway. In this case, FusionAuth will be used only as a service provider, often shortened to SP, not an identity provider.
...
Start by running a new instance of FusionAuth. You'll use [Docker](https://www.docker.com/get-started/) to run FusionAuth and [Node version 22](https://nodejs.org/en/download) to run the example application.
...
- Run the command below in a terminal to clone the [FusionAuth Example Repository](https://github.com/FusionAuth/fusionauth-example-saml), or download and unzip the repository.
  ```sh
...
If you do this, you will need to update `cert/cert.pem` and the SAML URL in `.env`. The instructions immediately below this explain how.
</Aside>
