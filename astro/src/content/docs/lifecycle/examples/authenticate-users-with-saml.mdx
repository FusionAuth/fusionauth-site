...
This guide will show you how to enable SAML authentication for your website and how to authenticate users with SAML via FusionAuth. To learn more about SAML, please see the [FusionAuth SAML overview](https://fusionauth.io/docs/lifecycle/authenticate-users/saml).

idp sp

Consider the following example. You provide a financial service on the Internet to users who log in to your website, called Changebank.com. Until now, the website has managed its own authentication with a username and password. You have a new corporate client, called Corpo.com, who wants all employees to use your service, but authentication will be done with SAML, using the client as the identity provider.

Let's also consider the possibility that the client is happy to let your website act as the identity provider, as long as SAML is used. In either case, you'll use FusionAuth to manage SAML, hiding the complexity of the protocol from your website.

## Technical Design Of The Example
...
Login to website with SAML using FA as IdP. documented here https://fusionauth.io/docs/lifecycle/authenticate-users/saml.
...
Login to website with SAML using Corpo.com as IdP. documented here https://fusionauth.io/docs/lifecycle/authenticate-users/identity-providers/overview-samlv2.
...
you'll use FA to represent corpo too, as FA can be used as SP or Idp.

if user already has authentication cookie from idp login will be bypassed.
...
OLD ARTICLE BELOW TO CHANGE TODO

  -  arrow, choose <InlineUIElement>Add SAML Server</InlineUIElement>, and save.
  - Click the <InlineUIElement>Add</InlineUIElement> arrow, choose <InlineUIElement>Add SAML Client</InlineUIElement>, and save.
- Browse to <Breadcrumb>Entity Management -> Entity</Breadcrumb>.
  - Click the <InlineUIElement>Add</InlineUIElement> button, choose <InlineUIElement>SAML Server</InlineUIElement>, for <InlineField>Name</InlineField> enter `BankTenant`, for <InlineField>Id</InlineField> and <InlineField>Client Id</InlineField> enter `1a3fd007-aafb-45db-b3dc-3298bcf71c98`, and for <InlineField>Secret</InlineField> enter `FIjjqyyPgBRbUMKhZIE7aQ8OVQ_Mp6DipxDAHsVF2fg`, and save.
  - Click the <InlineUIElement>Add</InlineUIElement> button, choose <InlineUIElement>SAML Client</InlineUIElement>, for <InlineField>Name</InlineField> enter `BankClient`, for <InlineField>Id</InlineField> and <InlineField>Client Id</InlineField> enter `bd09d77f-73e5-4d6c-ae0c-5436bf3e26d6`, and for <InlineField>Secret</InlineField> enter `UirdqdyvC5_1SxiDHdwlLyz7JGNxwCFJM0AL3MjUp28`, and save.
...
You have now configured a SAML server (representing your FusionAuth tenant) and a SAML client (representing the app you want to allow to manage users in FusionAuth).

![SAML entities](../../../../../public/img/docs/lifecycle/examples/SAML/entities.webp)

The next step is to enable SAML operations in your tenant.

- Browse to <Breadcrumb>Tenants</Breadcrumb>, edit the Default tenant, and in the <InlineUIElement>SAML tab</InlineUIElement> enable <InlineUIElement>Enabled</InlineUIElement>.
  - Choose the SAML server and client.
  - Choose the default lambdas.
  - Save.

![Tenant with SAML](../../../../../public/img/docs/lifecycle/examples/SAML/tenant.webp)

<Aside type="note">
You're linking the Tenant to the Entity **Type**, not the Entity, which is surprising. In FusionAuth, entities handle [machine authentication](/content/articles/authentication/service-to-service) but entities [cannot connect to users or applications](/docs/get-started/core-concepts/types-and-relationships). FusionAuth allows connecting the tenant to the entity type as a workaround to provide security for SAML. Permissions are controlled by the SAML Client entity. Whichever permissions are enabled in the entity grant from the client to the server entity are the permissions the tenant will allow.
</Aside>

## Create A FusionAuth User With SAML

FusionAuth should now be ready to receive SAML commands. To check that the SAML client is correctly configured to create a user on the SAML server, and that your Docker networking works correctly, run the curl command below to check the client permissions. If FusionAuth returns an access token and not an error then everything works.

```sh
curl -u bd09d77f-73e5-4d6c-ae0c-5436bf3e26d6:UirdqdyvC5_1SxiDHdwlLyz7JGNxwCFJM0AL3MjUp28 http://localhost:9011/oauth2/token --data-urlencode "grant_type=client_credentials" --data-urlencode "scope=target-entity:1a3fd007-aafb-45db-b3dc-3298bcf71c98:SAML:user:create"
```
```sh
# Expected output:

{"access_token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImd0eSI6WyJjbGllbnRfY3JlZGVudGlhbHMiXSwia2lkIjoiMDllOGQyMTkxIiwidXNlIjoic2NpbV9zZXJ2ZXIifQ..6SusxG40kaRboSSKMhmNH9gEYynfnsqyr7czvFtw3Yg","expires_in":3599,"scope":"target-entity:1a3fd007-aafb-45db-b3dc-3298bcf71c98:SAML:user:create","token_type":"Bearer"}
```

If you want to see what the token looks like decoded, paste it into https://fusionauth.io/dev-tools/jwt-decoder.

At this point, if you have a SAML app, you can point it to FusionAuth and see if you can create a user. The curl command above gave the URL on which to get an access token for the `SAML:user:create` [permission](/docs/lifecycle/migrate-users/SAML/#permissions). The user creation SAML endpoint is http://localhost:9011/api/SAML/resource/v2/Users.

To mimic a SAML app, you can use a JavaScript script to create a user.

- Create the file `SAML.js` in the `light` directory.
...
const user = {
  ...
  "schemas": ["urn:ietf:params:SAML:schemas:core:2.0:Users"],
  "userName": "alice"
}
...
async function getAccessToken() {
  try {
    const permissions = 'target-entity:1a3fd007-aafb-45db-b3dc-3298bcf71c98:SAML:user:create,SAML:user:read,SAML:user:update,SAML:user:delete,SAML:enterprise:user:create,SAML:enterprise:user:read,SAML:enterprise:user:update,SAML:enterprise:user:delete,SAML:group:create,SAML:group:read,SAML:group:update,SAML:group:delete';
...
async function makeUser(token, user) {
  try{
    ...
    const result = await api.post('/SAML/resource/v2/Users/', user);                    // CREATE
    console.dir(result.data);
    const userId = result.data.id;
    await api.put('/SAML/resource/v2/Users/' + userId, { ...user, "userName": "eve" }); // UPDATE
    await api.delete('/SAML/resource/v2/Users/' + userId);                              // DELETE
  }
  catch (e) {console.dir(e, { depth: null });}
}
```

This code starts with JSON holding a sample user to create, then calls two functions: `getAccessToken`, which is similar to the curl call earlier, and `makeUser`, which passes the token and the user JSON to the FusionAuth SAML endpoint. `makeUser` creates a user, stores the user UUID that FusionAuth returns, updates the user with the UUID, then deletes the user.
...
```sh
docker run --platform=linux/amd64 --rm -v ".:/app" -w "/app"  --network faNetwork denoland/deno:alpine-2.1.3 sh -c  "deno run --allow-net --allow-env ./SAML.js"
```
```sh
# Expected output:

{
  ...
  meta: {
    ...
    location: "http://fa:9011/api/SAML/resource/v2/Users/672963d1-142c-4587-8956-2ecbc2e96c62",
    resourceType: "User"
  },
  name: { familyName: "Doe", formatted: "Alice Doe", givenName: "Alice" },
  phoneNumbers: [ { primary: true, type: "mobile", value: "303-555-1234" } ],
  schemas: [
    "urn:ietf:params:SAML:schemas:core:2.0:User",
    "urn:ietf:params:SAML:schemas:core:2.0:Users"
  ],
  userName: "alice",
  "urn:ietf:params:SAML:schemas:core:2.0:Users": {}
}
...
<Aside type="tip">
Note that FusionAuth generated a new UUID for the user Id. Even if you specify an Id value in JSON, and update the FusionAuth lambda that received SAML requests to include `user.id = SAMLUser.id;`, FusionAuth still won't use your Id when creating a SAML user. This works differently to the [user API](/docs/apis/users#request), which allows you to specify the new user's Id in the POST URL.
...
- To see the new user in the FusionAuth users page, comment out the `api.put` and `api.delete` lines in `SAML.js` and run the command above again.

You should now see the user Alice in the users page, as shown below.

![User created with SAML](../../../../../public/img/docs/lifecycle/examples/SAML/user.webp)

## Create Registrations And Custom Lambdas

At this point, you have seen how to create, update, and delete FusionAuth users with SAML. You should be able to use this knowledge to correctly set up and test your own SAML app.

However, if you browse to the Registrations tab for the user, you'll see that Alice is not a member of any applications. SAML in FusionAuth can create users, but [cannot add users to applications](/docs/lifecycle/migrate-users/SAML/#adding-registrations). Your SAML app also might have some custom user data that you'd like to insert into FusionAuth.

Both these problems can be handled by customizing the SAML lambdas in FusionAuth.

### Create Custom Data Mappings

- Browse to <Breadcrumb>Customizations -> Lambdas -> SAML User Request Converter -> Edit</Breadcrumb>
- Set a custom user data mapping by changing the top of the function to be like the code below.
  ```js
  function convert(user, options, SAMLUser) {
   if (!user.data) user.data = {};
     user.data.id = SAMLUser.externalId;

    user.active = SAMLUser.active;
  ```
- Delete the user from the list of users in FusionAuth and rerun `SAML.js`.
- Now when you browse the user data tab for the new user, you'll see that she has more data stored.

![Custom user data](../../../../../public/img/docs/lifecycle/examples/SAML/data.webp)

<Aside type="caution">
FusionAuth lambdas are JavaScript run in Java using [Graal](https://www.graalvm.org/javascript). Graal supports modern JavaScript syntax, like ` `` ` strings, `...` object notation, `await`, and `=>` functions. However, at the time of writing, the FusionAuth lambda code editor does not recognize this syntax and will show errors. There is a [GitHub issue](https://github.com/FusionAuth/fusionauth-issues/issues/1889) open to fix this. Until that issue is fixed, the only way to check if your code is actually correct is to run it and review the FusionAuth <Breadcrumb>System -> Event Log</Breadcrumb> page to see if any `org.graalvm.polyglot.PolyglotException` errors occurred.
</Aside>

<Aside type="caution">
It may look like you can create a registration for the user by setting `options.applicationId`, but that won't work. The user creation parameters are documented [here](/docs/apis/users#request) and [here](/docs/extend/code/lambdas/SAML-user-request-converter#lambda-structure). The code below **won't** make the user a member of the application, and also won't throw an error.
```js
function convert(user, options, SAMLUser) {
...
To register the new user with an application, you need to make a [user API call from within the SAML user response lambda](/docs/extend/code/lambdas/lambda-remote-api-calls), after the FusionAuth user has been created (which means you can't make the call in the SAML user **request** lambda).

- Delete the user Alice from the list of users again.
- Browse to <Breadcrumb>Customizations -> Lambdas -> SAML User Response Converter -> Edit</Breadcrumb>
...
```
- Run `SAML.js` in the terminal again.
- If you view user Alice in the Users page, you can see she now is a member of the sample bank application in the registrations tab.
  ![User has registration](../../../../../public/img/docs/lifecycle/examples/SAML/registration.webp)
...
Note that this lambda is called whenever you return a user through SAML. So if you delete the user's registration to the application manually, then call SAML read for the user, the registration will be re-added, even though you didn't call SAML create. Review your user management protocols carefully to ensure this process doesn't mistakenly give old users access they shouldn't have.
...
The alternative to creating registrations from inside the lambda, is to listen for the FusionAuth [webhook](/docs/extend/events-and-webhooks) after the user is [created](/docs/extend/events-and-webhooks/events/user-create-complete), and call the FusionAuth user API from your own script. To this, you need to create a webserver that listens for the webhook.

- In the FusionAuth web interface, browse to <Breadcrumb>Tenants -> Default -> Edit -> Webhooks</Breadcrumb>
- Enable `user.create.complete` and save.
  ![Set webhook in tenant](../../../../../public/img/docs/lifecycle/examples/SAML/webhook.webp)
- Browse to <Breadcrumb>Settings -> Webhooks</Breadcrumb> and add a new one.
  - Enter an <InlineField>URL</InlineField> value of `http://register`.
  - Disable all webhooks except `user.create.complete` and save.
  ![Create webook](../../../../../public/img/docs/lifecycle/examples/SAML/webhook2.webp)
- FusionAuth is now configured to send webhooks after a user is created to a web server listening at `http://register`.
- Delete the user Alice in the FusionAuth web interface.
- Create a file called `register.ts`.
- Insert the code below.
  ```js
...
- Run the script with the command below. Note that you are specifying the network and container name, so that FusionAuth will be able to connect to the container at `http://register`.
  ...
  docker run --platform=linux/amd64 --rm -v ".:/app" -w "/app"  --network faNetwork --name register -p 7777:80 denoland/deno:alpine-2.1.3 sh -c  "deno run --allow-net --allow-env ./register.ts"
    ```
- Check the JavaScript web server is running by browsing to http://localhost:7777. You should see a blank page with no errors.
- Run `SAML.js` again.
- The terminal output of the web server should be as below. This is what a FusionAuth webhook looks like. You can use these properties to customize your application registration if you want.
  ```js
  {
    event: {
      ...
      user: {
        ...
        data: {
          extensions: { "urn:ietf:params:SAML:schemas:core:2.0:Users": {} },
...
- Replace the code in `register.ts` to be as below. Now HTTP handler will take the user Id from the webhook JSON and use it to send a POST to the user registration API.
  ```js
  Deno.serve({'hostname': '0.0.0.0', 'port': 80 }, handler);
  ...
  ```
- Run `SAML.js` one last time.
- The terminal output of the web server should be as below.
  ```js
  ...
  ```
- If you look at Alice's registrations in the user page you'll see that she is registered for the bank application again.
