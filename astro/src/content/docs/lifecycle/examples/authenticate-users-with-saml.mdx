...
This guide will show you how to enable SAML for your application and how to authenticate a user on your website with SAML via FusionAuth..

This guide is a concrete example that integrates the information from the following FusionAuth documentation:
- [Overview of SAML in FusionAuth](https://fusionauth.io/docs/lifecycle/authenticate-users/saml)
...
<Aside type="note" title="When to use SAML?">
FusionAuth has an alternative to SAML, [the user API](/docs/apis/users), which allows you to provision users with simple HTTP calls. If you're looking for an easy way to automatically create users in FusionAuth, the API is a better choice. If you're looking for a way to automatically create a set of users when installing FusionAuth, then you can use [Kickstart](/docs/get-started/download-and-install/development/kickstart), which is easier than both the API and SAML.

Use SAML in FusionAuth only when you have an existing user-management system in your organization that is primarily designed to work with SAML. SAML is a secure and full-featured standard, but does require more work to configure than the user API.
</Aside>
...
- Browse to <Breadcrumb>Applications -> Bank -> Edit -> SAML tab</Breadcrumb>.
  ...
  - Set <InlineField>Issuer</InlineField> to `changebank`.
  - Set <InlineField>Authorized redirect URLs</InlineField> to `http://saml/redirect` (be sure to click the dropdown text that appears to confirm the entry).
...
docker run --platform=linux/amd64 --rm -v ".:/app" -w "/app"  --network faNetwork  -p 3000:3000 denoland/deno:alpine-2.1.3 sh -c  "deno run --allow-net --allow-read --allow-env --unstable-detect-cjs ./bin/www"
```
