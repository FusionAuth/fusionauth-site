...
9012 acts as SP and 9011 as IdP. use separate browser windows
...
- In the terminal where you ran FusionAuth in the last example, press <kbd>Ctrl</kbd>+<kbd>c</kbd> to stop Docker, and run the command below to start two instances FusionAuth.
  ...
  docker compose up
  ```
- Log in to your new FusionAuth web interface at http://localhost:9012/admin (*not* `9011`) with credentials `admin@example.com` and `password`.
- Browse to <Breadcrumb>Reactor</Breadcrumb>.
- Enter your license key to activate Reactor and refresh the page.
- Settings -> Key Master -> Import certificate (top right)
- Name: fa
- Paste X509 XML value: ``
...
- Set <InlineField>Issuer</InlineField> to `http://localhost:9012/samlv2/sp/bf79d4ba-d37e-46d2-9a13-5e1b5bd3178e`. From the view of the SAML IdP in 9012.
- Set <InlineField>Authorized redirect URLs</InlineField> to `http://localhost:9012/samlv2/acs` (be sure to click the dropdown text that appears to confirm the entry).
- Save.

log in at http://localhost:9012/oauth2/authorize?client_id=e9fdb985-9173-4e01-9d73-ac2d60d1dc8e&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fauth%2Fcallback
