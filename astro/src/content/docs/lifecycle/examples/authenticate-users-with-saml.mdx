...
- Browse to <Breadcrumb>Applications -> Start Here App -> Select -> Edit -> OAuth tab</Breadcrumb>.
...
- Save and logout of the admin interface.

Run the command below in a terminal to clone the [FusionAuth Example SAML repository](https://github.com/fusionauth/fusionauth-example-saml.git), or download and unzip the repository.
```sh
...
- Log in to your FusionAuth web interface at http://localhost:9011/admin with the credentials `admin@example.com` and `password`.
- Browse to <Breadcrumb>Applications -> Start Here App -> Select -> Edit -> SAML tab</Breadcrumb>.
