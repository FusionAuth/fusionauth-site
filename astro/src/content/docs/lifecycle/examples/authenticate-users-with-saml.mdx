...
In this section, you'll configure FusionAuth as the SAML identity provider, where users do not have to log in with Corpo.com.
...
docker run --platform=linux/amd64 --rm -v ".:/app" -w "/app" --name app --network faNetwork  -p 3000:3000 node:23-alpine3.19 sh -c  "node ./bin/www"
```
