...
- Browse to the <Breadcrumb>Options</Breadcrumb> tab and enter `userId` for the <InlineField>Unique Id claim</InlineField> and `email` for the <InlineField>Email claim</InlineField> fields.
...
- Create a `app/cert/cert.pem` file. Copy the text in between the tags labelled `<X509Certificate>` and paste it into the file `app/cert/cert.pem`. Add the line `-----BEGIN CERTIFICATE-----` above the text and the line `-----END CERTIFICATE-----` below it, and save the file.
