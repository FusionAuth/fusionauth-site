...
The code changes to the routes file are minimal, barely changing what is in the Passport documentation. You replace the OAuth login route (because, in this example, the customer wanted to switch entirely to SAML), you add a SAML login route to direct the user to FusionAuth, and you use a callback handler to direct the logged-in user to their account.
