...
todo update limitations of lambdas article. graalvm link. https://www.graalvm.org/javascript does support ECMAScript 2024, but FA does not
...
function convert(scimUser, user) {
  ...
  try {
    var response = HttpClient.post("http://fa:9011/api/user/registration/" + user.id,
      {
...
function isUserRegistered(userId, applicationId, apiKey) {
  try {
    var response = HttpClient.get("http://fa:9011/api/user/registration/" + userId + "/" + applicationId, {headers: {'Authorization': apiKey}});
