...
The alternative to creating registrations from inside the lambda, is to listen for the FA [webhook](https://fusionauth.io/docs/extend/events-and-webhooks) after the user is [created](https://fusionauth.io/docs/extend/events-and-webhooks/events/user-create-complete), and call the FA user API from your own script. To this, you need to create a webserver that listens for the webhook.

- In the FA web interface, browse to <Breadcrumb>Tenants -> Default -> Edit -> Webhooks</Breadcrumb>
...
- FA is now configured to send webhooks after a user is created to a web server listening at `http://register`.
- Delete the user Alice in the FA web interface.
...
- Insert the code below.
  ...
  ```
- Run the script with the command below. Note that you are specifying the network and container name, so that FA will be able to connect to the container at `http://register`.
  ```sh
...
- Run `scim.js` again.
- The terminal output of the web server should be as below. This is what a FA webhook looks like. You can use these properties to customize your application registration if you want.
  ```js
  {
    event: {
  ...
  ```
- Delete the user Alice in the FA web interface one last time.
