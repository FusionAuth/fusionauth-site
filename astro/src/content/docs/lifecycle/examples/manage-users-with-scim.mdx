...
This guide will show you how to enable SCIM for your tenant (multiple FusionAuth applications), how to create, update, and delete users, and how to map user details using a lambda. Since SCIM works at the tenant level, not the application level, you can use SCIM for any new or existing applications in the tenant.
...
However, if you browse to the Registrations tab for the user, you'll see that Alice is not a member of any applications. SCIM in FusionAuth can create users, but [cannot add users to applications](/docs/lifecycle/migrate-users/scim/#adding-registrations). Your SCIM app also might have some custom user data that you'd like to insert into FusionAuth.
...
- Enable `user.create.complete` and save.
  ![Set webhook in tenant](../../../../../public/img/docs/lifecycle/examples/scim/webhook.webp)
- Browse to <Breadcrumb>Settings -> Webhooks</Breadcrumb> and add a new one.
  ...
  ![Create webook](../../../../../public/img/docs/lifecycle/examples/scim/webhook2.webp)
