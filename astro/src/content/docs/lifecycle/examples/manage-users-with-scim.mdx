...
- Browse to <Breadcrumb>Tenants</Breadcrumb>, edit the Default tenant, and in the <InlineUIElement>SCIM tab</InlineUIElement> enable <InlineUIElement>Enabled<InlineUIElement>.
  - Choose the SCIM server and client and save.
...
![Entity Types in FusionAuth](../../../../../public/img/docs/lifecycle/examples/scim/entities.webp)

## Create A FA User With SCIM

FA is now ready to receive SCIM commands.
...
try {
  ...
  console.log(token);
  await makeUser(token);
} catch (e) {console.dir(e, { depth: null });}

async function makeUser(token) {
  const data = {
    "active": true,
    "emails": [
      {
  ...
  const result = await api.post(`/scim/resource/v2/Users`, data)
...
async function getAccessToken() {
  try {
    ...
    formData.append("scope","target-entity:1a3fd007-aafb-45db-b3dc-3298bcf71c98:scim:user:create");
    // scim:user:read scim:user:update scim:user:delete scim:enterprise:user:create scim:enterprise:user:read scim:enterprise:user:update scim:enterprise:user:delete scim:group:create scim:group:read scim:group:update scim:group:delete

    const response = await axios({
      method: "post",
      url: "http://fa:9011/oauth2/token",
