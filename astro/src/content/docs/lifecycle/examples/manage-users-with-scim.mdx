...
- Set a custom user data mapping by changing the top of the function to be like the code below. Note that lambdas don't support all JavaScript syntax, so you can't use ` `` ` strings or `...` object notation.
```js
...
function convert(scimUser, user) {
  ...
  // Make API call to register user to an application
  var applicationId = "your-application-id";
  var apiKey = "your-api-key";

  // Check if the user is already registered to avoid duplicate registrations
  if (!userIsRegistered(user.id, applicationId)) {
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "/api/user/registration/" + user.id, false);
...
function userIsRegistered(userId, applicationId) {
  // Implement logic to check if user is already registered
  // This would typically involve making another API call
