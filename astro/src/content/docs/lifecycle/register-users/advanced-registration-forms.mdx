...
* A password field in one of the steps (see note below)
* At least one field on each step

<Aside type="note" title="Passwordless Registration">
  Because forms can be used with multiple tenants, they must include a password field. If a tenant has <InlineField>tenant.passwordEnabled</InlineField> set to `false`, then the password field is not displayed to users during registration.
</Aside>
