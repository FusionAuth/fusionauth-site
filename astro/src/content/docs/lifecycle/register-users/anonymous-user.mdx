...
This examines the incoming event to see if it is a login success. It then checks if the user is a newly converted user, as indicated by a value of <InlineField>user.data.anonymousUser</InlineField>. If these are all true, then the user is updated to set <InlineField>anonymousUser</InlineField> to <InlineFieldValue>false</InlineFieldValue>. For this guide, that is indication that the user has been converted to a regular user.
