...
tags: register user api, login api, api registration, user signup api, register and login
---
import ListHostedLoginPagesUseCases from 'src/content/docs/_shared/_list-hosted-login-pages-use-cases.mdx';

This page explains how to register a user and authenticate them to use an Application using the FusionAuth APIs or the hosted login pages.

## Register a User

In order to register a User, you must have first created an Application.  A tutorial for creating an application is provided in the [Application overview](/docs/get-started/core-concepts/types/applications). Once the Application has been created, you are ready to call the API to register a User.
...
![Example of login form.](/img/docs/lifecycle/register-users/login-page.png)
