...
Set up a custom domain name pointing to a [durable CNAME provided by FusionAuth](/docs/cloud/cloud#what-is-a-durable-fusionauth-cname) for your deployment. Having a custom domain name is not enough; it must be a CNAME which points to the durable DNS name for proper failover.
