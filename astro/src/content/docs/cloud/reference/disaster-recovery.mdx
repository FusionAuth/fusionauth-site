...
Set up a custom domain name pointing to a [durable CNAME provided by FusionAuth](/docs/cloud/operate/configure-custom-domain#durable-fusionauth-cname) for your deployment. Having a custom domain name is not enough; it must be a CNAME which points to the durable DNS name for proper failover.
