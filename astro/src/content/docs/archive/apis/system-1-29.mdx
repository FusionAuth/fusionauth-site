...
<API method="PUT" uri="/api/system-configuration" authentication={["api-key"]} title="Update the System Configuration"/>
...
<API method="GET" uri="/api/system/log/export?dateTimeSecondsFormat={dateTimeSecondsFormat}&lastNBytes={lastNBytes}&zoneId={zoneId}" authentication={["api-key"]} title="Export the System Logs"/>
...
<API method="POST" uri="/api/system/system-log/export" authentication={["api-key"]} title="Export the System Logs"/>
...
<API method="GET">/api/prometheus/metrics</API>
