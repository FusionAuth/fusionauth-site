...
This is the inverse of the authorization hub use case](/docs/get-started/use-cases/authorization-hub). Instead of managing access and refresh tokens for external APIs, here you are issuing tokens for your APIsafter users offer consent.
...
| How are they accepted | By logging in |  By explicitly acception        |
...
Once you've provisioned the application in FusionAuth, provide integration infomration for the MoneyScope developers. Provide these values:
...
* Use the FusionAuth user Id to [find all the user's refresh tokens](https://fusionauth.io/docs/apis/jwt#retrieve-refresh-tokens).
...
* Any plaform that wants to allow a [model context protocol (MCP)](https://modelcontextprotocol.io) client to access to APIs guarded by an MCP server
