...
You have a site that leverages social providers, using their APIs to provide functionality to your users.
...
You can implement this use case with FusionAuth APIs, specifically the `Complete the Login` [Identity Provider APIs](/docs/apis/identity-providers/). That implementation is beyond the scope of this document.
...
In each of these cases there's a dependency on a third party platform which is accessed via API to provide core app functionality.
