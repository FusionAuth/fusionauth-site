...
All of these use FusionAuth for their login. Each is represented by a different application and client Id, but your users have to log in each time they switch between applications.
...
* Have applications forward users to FusionAuth when users are not logged in to the application.

### Force SSO Session Creation

The easiest way to force the SSO session to be created for each user authentication is to modify the login page of the theme. Normally a user can check or uncheck the `rememberDevice` checkbox, labelled by default "Keep me signed in". To force the SSO session, update the theme to set the `rememberDevice` parameter to `true`.

To do so, update the `OAuth Authorize` template [using the advanced theme editor](/docs/customize/look-and-feel/advanced-theme-editor). This is the login page. 

Here's a sample FreeMarker block you can use to replace the default setup.
...
=======
Whenever a user accesses an application and is not logged in, forward them to the login URL for FusionAuth. As your applications are already delegating authentication to FusionAuth, use the same logic or library functions to create the login URL. Here's an [explanation of how to create the URL and what each component means](/docs/lifecycle/authenticate-users/oauth/endpoints#authorization-code-grant-request).
>>>>>>> Stashed changes
...
This use case only works with FusionAuth hosted login pages. If you use the Login API, you need to build your SSO implementation.
