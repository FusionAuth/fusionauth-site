...
Here's a high sequence diagram of the application login process before you integrate with FusionAuth.

<LoginBefore alt="Your application login flow before FusionAuth." />

Here's a high sequence diagram of the application login process after FusionAuth integration.
...
Now that your application delegates authentication to FusionAuth, additional features can be enabled via configuration. The specific configuration steps depends on the exact feature. You ca manage [FusionAuth configuration using IaC or other means](/docs/operate/deploy/configuration-management).
...
The hosted login pages give you maximal flexability around look and feel customization, but constrain your login flow options. If you want to build login workflows with unique requirements, the Login API. This is the data store use case. An example custom workflow would be:
