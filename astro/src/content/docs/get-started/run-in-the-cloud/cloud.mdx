...
* The IP addresses of a FusionAuth Cloud deployment are not fixed. Whenever possible, use the domain name, which is fixed. If you need IP addresses of the FusionAuth service nodes, follow the instructions in the [Deployment IP Addresses](#deployment-ip-addresses) section found above. Be aware that even after determining the assigned IP addresses, they are subject to change. Please upvote or comment on this [open issue about static IP addresses in FusionAuth Cloud](https://github.com/FusionAuth/fusionauth-issues/issues/1393).
