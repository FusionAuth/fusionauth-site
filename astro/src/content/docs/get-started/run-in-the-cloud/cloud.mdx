...
If you have needs for more requests per second, please see <ScrollRef target="Custom FusionAuth Cloud Features" /> for other options.
...
Consult the [relevant release notes](/docs/v1/tech/release-notes) for functional changes.
...
If you are looking to modify other attributes of your deployment, such as the region it is running in, the deployment size, or the service tier, review the <ScrollRef target="Modifying Deployments" /> section.
</Aside>
...
See <ScrollRef target="Accessing User Data" /> for more about retrieving user data and passwords.
...
If you are interested in migrating to FusionAuth from an auth system other than FusionAuth, check out the [Migration Guide](/docs/v1/tech/migration-guide/).

If you want to retrieve your user data from FusionAuth Cloud in order to migrate to a different auth provider, see <ScrollRef target="Accessing User Data" />.
...
* You must be running PostgreSQL. If you are running MySQL, please migrate to a PostgreSQL database locally. Here's a [forum post on how to do this](https://fusionauth.io/community/forum/topic/985/migrating-from-mysql-to-postgresql). This has been provided by the community and is not supported nor endorsed by FusionAuth.
...
* Purchase a FusionAuth Cloud deployment that meets your needs. Select the version of FusionAuth that is the same as your local installation. You will be able to upgrade FusionAuth later; see <ScrollRef target="Upgrading a Deployment" /> for more.
...
After your data is uploaded, upgrade your deployment to any subsequent FusionAuth version, if desired. See <ScrollRef target="Upgrading a Deployment" /> for more.
...
| [ ]   | Created a new deployment. Instructions: <ScrollRef target="Creating a Deployment" />.                      |
...
To migrate from FusionAuth Cloud to a self-hosted instance, please request a database export, as documented in <ScrollRef target="Accessing User Data" />.
...
Once you've tested your self-hosted installation, destroy your FusionAuth Cloud deployment as outlined in <ScrollRef target="Destroying a Deployment" />.
...
This is in contrast with upgrading the version of FusionAuth your deployment is running, which is a self-serve operation described in <ScrollRef target="Upgrading a Deployment" />.
...
<Aside type="note">
If you are an Enterprise customer with a High Availability cloud deployment and require more than your current allotment of custom domains, you are eligible for an upgrade to <ScrollRef target="Unlimited Custom Domains" />.
</Aside>
...
<EnterpriseEditionBlurb enterprise_feature="Unlimited Custom Domains" />
...
Existing custom domain DNS records pointing to the deployment's `*.fusionauth.io` domain name need to be updated to refer to the <ScrollRef target="What is a Durable FusionAuth CNAME?, durable URL" />. Once DNS records have been updated, you can complete the cutover to unlimited custom domains from the account management portal by selecting <InlineField>Cutover Custom URL(s)</InlineField> from <InlineField>Action</InlineField> menu at the top of the custom domains listing page and completing the form on the next page. If a deployment does not have any existing custom domains when unlimited domains are enabled, it can be cut over immediately.
...
If you have out of scope questions and have not purchased a support contract, you can find community support in the [forums](/community/forum/) and [documentation](/docs/v1/tech/).
...
<img src="/img/docs/get-started/run-in-the-cloud/aws-captcha-challenge-puzzle.png" alt="Puzzle Captcha AWS" width="1200" role="bottom-cropped"  top-cropped="undefined" />
<img src="/img/docs/get-started/run-in-the-cloud/aws-captcha-challenge-roadmap.png" alt="Roadmap Captcha AWS" width="1200" role="bottom-cropped"  top-cropped="undefined" />
...
<Aside type="note">
The CAPTCHA described here is separate from the CAPTCHA configured using [Advanced Threat Detection](/docs/v1/tech/advanced-threat-detection/) and can only be managed by the FusionAuth team.
</Aside>
...
There is an open issue to [allow FusionAuth deployments to have static IP addresses](https://github.com/FusionAuth/fusionauth-issues/issues/1393).
...
* If you have an edition with support, please [open a support ticket](https://account.fusionauth.io/account/support/) if you'd like additional support or guidance on sizing or testing.
...
FusionAuth Cloud has the same [limitations](/docs/get-started/core-concepts/limitations) as self hosted FusionAuth. Since it is a managed service, there are additional limitations as well:

* No access is provided to the server on which your deployment is running. This includes access to the database, Elasticsearch, or ssh. You can access your data via FusionAuth API or the administrative console. To install a plugin, please [open a support ticket](https://account.fusionauth.io/account/support).
* There is no API to manage FusionAuth Cloud deployments.
* You cannot change any of the [FusionAuth configuration options](/docs/reference/configuration).
...
* The IP addresses of a FusionAuth Cloud deployment are not fixed. Whenever possible, use the domain name, which is fixed. If you need IP addresses of the FusionAuth service nodes, follow the instructions in the <ScrollRef target="Deployment IP Addresses" /> section found above. Be aware that even after determining the assigned IP addresses, they are subject to change. Please upvote or comment on this [open issue about static IP addresses in FusionAuth Cloud](https://github.com/FusionAuth/fusionauth-issues/issues/1393).
