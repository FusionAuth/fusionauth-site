...
Choose a deployment configuration by clicking <Breadcrumb>Add</Breadcrumb> on the <Breadcrumb>Deployments</Breadcrumb> section to get an estimated cost.
...
* Navigate to the <Breadcrumb>Hosting</Breadcrumb> tab. Manage the deployment, then choose the <Breadcrumb>Upgrade</Breadcrumb> option. Select `1.x` from the <InlineField>Version</InlineField> dropdown.
...
Refresh the DNS validation status of a custom domain by selecting <Breadcrumb>Refresh Status</Breadcrumb> from the <Breadcrumb>Action</Breadcrumb> menu for the domain and clicking the <InlineField>Refresh</InlineField> button on the next page.
...
Once the proper DNS record is in place, refreshing the status of a custom domain updates the DNS validation statuses and provisions the certificate.
