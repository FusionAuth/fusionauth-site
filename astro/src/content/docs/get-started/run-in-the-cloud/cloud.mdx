...
* There is no support for proxy customization to add, for example, [tenant routing](/docs/operate/deploy/proxy-setup#proxies-and-tenants). To accomplish such goals, add your own proxy layer such as CloudFlare, with FusionAuth Cloud as an origin. Make sure you configure the `systemConfiguration.trustedProxyConfiguration.trustPolicy` and `systemConfiguration.trustedProxyConfiguration.trusted` list using the [System API](/docs/apis/system).
