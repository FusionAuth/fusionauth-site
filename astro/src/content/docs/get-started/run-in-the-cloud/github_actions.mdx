...
In GitHub, click [uielement]#fork# and add the repository to your own account. Then run `git clone ...` in a terminal to download it.
...
We'll discuss the secrets in the next section.
...
1. Compile the executable in the action and download it on the server.
1. Build your app into a Docker image, upload that to DockerHub, and download and run it on the server.
...
To make your Go binary (and all the HTML templates it serves) available for other steps in the workflow, use the [upload-artifcation action](https://github.com/actions/upload-artifact). To build and push a DockerHub image, use the [Docker action.](https://github.com/marketplace/actions/build-and-push-docker-images).
...
Add a final step to your script, after the deploy step, that runs the Playwright login test. But this time, run a test file that uses the production server URL, instead of your localhost, in the line `await page.goto('http://localhost:3000');`.
