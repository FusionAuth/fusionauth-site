...
url: https://github.com/FusionAuth/fusionauth_github_actions
codeRoot: https://raw.githubusercontent.com/FusionAuth/fusionauth_github_actions/main
---
...
```bash
 ...
 │                               │           │                                 │
 │  ┌─────────────────────────┐  │           │  ┌───────────────────────────┐  │
 │  │     App website         │  │           │  │  FusionAuth website       │  │
 │  └────────────┬────────────┘  │           │  └────────────┬──────────────┘  │
 │               │               │ ◄───────► │               │                 │
 │               │               │           │               │                 │
 │               ▼               │           │               ▼                 │
...
```
{/*
TODO  uncomment block when Mermaid works
...
```bash
 ...
 └────────────────────────────────────────────────────────┘
                            ...
                            ▼
 ┌────────────────────────────────────────────────────────┐
 │  ⬆️ Push to main branch on GitHub                      │
 └────────────────────────────────────────────────────────┘
                            │
                            ▼
 ┌────────────────────────────────────────────────────────┐
 ...
 └────────────────────────────────────────────────────────┘
                            ...
                            ▼
 ┌───────────────────────────────────────────────────────────────────────────────┐
 │                                                        Inside GitHub action   │
 │  ┌──────────────────────────────────────────────────┐                         │
 │  │  🛠️ Compile app in a virtual machine             │                         │
 │  └──────────────────────────────────────────────────┘                         │
 │                           │                                                   │
 │                           ▼                                                   │
 │  ┌─────────────────────────────────────────────────────────────────┐          │
 │  │  🔬 Test app can log in with FusionAuth                         │          │
 │  └─────────────────────────────────────────────────────────────────┘          │
 │              │                     │                          │               │
 │              │                     │                          │               │
 ...
 │              ▼                     ▼                          ▼               │
 │  ┌─────────────────────┐  ┌─────────────────────┐  ┌───────────────────────┐  │
 │  │  ☁️ AWS             │  │  ☁️ Azure           │  │  ☁️ Virtual server    │  │
 │  └─────────────────────┘  └─────────────────────┘  └───────────────────────┘  │
 └───────────────────────────────────────────────────────────────────────────────┘
```
...
There is a minimal but complete CI/CD example in this <a href={frontmatter.url}>repository</a>.

In GitHub, click <InlineUIElement>Fork</InlineUIElement> and add the repository to your GitHub account. Then run `git clone https://github.com/<your_username>/fusionauth_github_actions.git` in a terminal to download it.
...
Stop Docker in the terminal. In the `docker-compose.yml` file, change line 51 to use a new FusionAuth version, `1.48.3`. Run the startup command again.
...
Open the file `.github/workflows/test.yaml`. Line 5 currently reads as follows.
...
By default, GitHub prevents action workflows from running on forked repositories. To grant permission for the workflow to execute, navigate to the Actions tab and click on the <InlineUIElement>I understand my workflows, go ahead and enable them</InlineUIElement> button on the repository. Save, commit in Git, and push to GitHub, and the test will execute.

You can browse to the Actions tab in your GitHub repository to see the test run.
...
<Aside type="note">
Reusable actions in a workflow are different from the workflow itself, but they are both called actions in GitHub. Workflows are declared in YAML files, and reusable actions are public JavaScript or Docker repositories.
</Aside>
...
To deploy your Go application, you can do one of the following:
...
Instead, each environment should store its secrets in environment variables or a configuration file that is not overwritten on deployment. These values should only be readable by the system administrator and the application's user account.
