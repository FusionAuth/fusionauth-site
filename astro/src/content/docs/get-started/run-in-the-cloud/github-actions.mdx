...
In GitHub, click <InlineUIElement>Fork</InlineUIElement> and add the repository to your own account. Then run `git clone https://github.com/<your_username>/fusionauth_github_actions.git` in a terminal to download it.
...
Open the `docker-compose.yml` file and on line 51 set the FusionAuth image to
```yaml
...
Stop Docker in the terminal. Change the `docker-compose.yml` line 51 to use a new FusionAuth version, `1.48.3`. Again run
```bash
docker compose up
```
...
The configuration file `playwright.config.js` allows you to set which browsers to use (we used `--project=chromium` earlier), whether to start your app before running the tests, and in which directory to look for test files.
...
By default, GitHub prevents Action workflows from running on forked repositories. Therefore, you must grant permission for the workflow to execute by navigating to the Actions tab and clicking on the <InlineUIElement>I understand my workflows, go ahead and enable them</InlineUIElement> button on the repository. Save, commit in Git, and push to GitHub. This will cause the test to execute.
...
<Aside type="note">
Reusable actions in a workflow are different to the workflow itself, but they are both called Actions in GitHub. Workflows are declared in YAML files and reusable Actions are public JavaScript or Docker repositories.
</Aside>
...
- [Azure Action](https://github.com/Azure/actions)
