...
FusionAuth will start, and you will be able to log in at `http://localhost:9011/admin` with username <InlineFieldValue>admin@example.com</InlineFieldValue> and password <InlineFieldValue>password</InlineFieldValue>. In <Breadcrumb>Users</Breadcrumb>, [add a new user](/docs/get-started/core-concepts/users) by clicking the green <Breadcrumb>+</Breadcrumb> button.
...
<Aside type="note">
  By default, GitHub prevents action workflows from running on forked repositories. To grant permission for the workflow to execute, navigate to the Actions tab on your forked repository and click on the <Breadcrumb>I understand my workflows, go ahead and enable them</Breadcrumb> button on the repository.
</Aside>
...
You can browse to the Actions tab in your GitHub repository and then select <Breadcrumb>Test FusionAuth login</Breadcrumb> on the left sidebar to see the workflow run. The most recent workflow run will appear at the top of the list and you can see further details by clicking on the title of the workflow run items. Clicking on the <Breadcrumb>run-tests</Breadcrumb> job item will reveal further details of the workflow execution steps.
