...
When using FusionAuth as an OAuth identity provider, user's are logged in using what is known as the Authorization Code Grant. This leverages FusionAuth's OAuth frontend to authenticate users and provide an access token to the application.
