...
Roles are the way that FusionAuth manages user authorization to applications. When a user logs in, the roles that the user has been granted to the application are included in their access token. Since FusionAuth access tokens are JWTs, it is simple to decode the JWT and analyze the roles a user has been granted.
