...
Roles are the way that FusionAuth manages user authorization to applications. When a user logs in, the roles that the user has been granted to the application they are logged into are included in their access token. Since FusionAuth access tokens are JWTs, it is simple to decode the JWT and analyze the roles a user has been granted.
...
You can write a similar function to verifies roles and in some languages you can leverage security frameworks and libraries as well. The general rule is to ensure that every protected route has a access control check before it renders the response.
