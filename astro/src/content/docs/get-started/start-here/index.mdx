...
navOrder: 0
codeRoot: https://raw.githubusercontent.com/FusionAuth/fusionauth-first-steps-launch/main/
...
* node (this was tested with `v22.12.0`)
...
You launched a FusionAuth database and server either using Docker or in GitPod, using a kickstart to pre-populate the application details in the FusionAuth database.
...
In the FusionAuth application you can view and change your authentication URLs and how tokens are generated/exchanged.

 HERE IS WHERE THE SCREENSHOTS WILL GO


<h3>Token Exchange Details</h3>

To understand how authentication is being done, take a quick look at some of the code from the repository.  

During login, the system checks for a valid token before letting the user in.
...
If the user needs a new token, the oauth path is followed to get them a fresh valid token.
...
For the account page, which requires authorization, the user's identity is checked and the browser is redirected to the main page if there is no valid token.
...
Similarly, the make-change function requires a logged-in user so that is checked at the beginning of the function.
<RemoteCode url="https://raw.githubusercontent.com/synedra/fusionauth-example-express-start-here/main/app/src/index.ts" lang="javascript" tags="make-change" />
...
GitPod:
<Code code="gp preview `gp url 8080`" />

Local:
<Code code="open http://localhost:8080" />
