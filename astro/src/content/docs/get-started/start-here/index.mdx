...
We craft a URL that includes the `clientId` from the configuration you reviewed above, as well as some other parameters needed to offer a secure authentication experience.
...
Once you have the token, what should the app do with it? One option is to store it as a secure cookie, which the above code also does using the `res.cookie` function.
...
Now, log in to the application in your browser:
...
You looked at some FusionAuth configuration, started a JavaScript application and checked out some code, and logged in to the application using FusionAuth.
...
Enabling your users to log in using magic links can reduce friction and user frustration. Let's enable this on the "Start Here" application.
