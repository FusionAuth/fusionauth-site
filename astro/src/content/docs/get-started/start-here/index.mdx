...
This guide will walk you through standing up a sample, pre-configured Node.JS application that uses FusionAuth for login.
...
<Aside type="note">
While most of this tutorial is the same whether you are using Docker or GitPod, there are slight differences commands used.
</Aside>
...
Once you have FusionAuth up and running, you need to set up your application or applications to delegate to FusionAuth. For this guide, there's an example `Start Here App` application which is already set up to do so. Let's look at that configuration and then start the application up so you can log in. Below is a diagram showing this flow.

<AfterDelegateDiagram />

### The Application Configuration In The FusionAuth Admin UI
...
Let's look at the configured `Start Here App`. You'll look at a couple of relevent settings.
...
FusionAuth. FusionAuth examines the request parameters and, if they all are correct, sends a token to the Node.JS application. When the app receives these, **the user is authenticated**. The application can then let the user access data or functionality.  You can retrieve additional user details from FusionAuth using the token. This code does that, and stores the additional user data in the `userResponse` variable.
...
Once you have the token and any other data, what should the app do? One great option is to store it as a `Secure`, `HTTPOnly` cookie that the browser can then send back to other services. The above code does this using the `res.cookie` function.
...
You can check out how all the code and templates fit together by looking at the code in the `app` directory.
