...
By delegating to FusionAuth, your application gets access to authentication and authorization functionality, such as multi-factor authentication, social sign-on, self-service registration and more. This is where a lot of the value comes from.
...
To add a `Login With Google` button, you usually have to configure Google and then FusionAuth. But for this guide, we've already set up Google.

Here's a diagram with the new login flow.

<OAuthAndFederationDiagram alt={alt} identity_provider_name="Google" />

TODO set up sample google project
TODO publish google sampel project
...
If you want to set up your own Google project, check out [our step by step guide for configuring the Google Identity Provider](/docs/lifecycle/authenticate-users/identity-providers/social/google)
