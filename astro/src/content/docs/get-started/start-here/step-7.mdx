...
You'll see that this test makes a request to `http://localhost:9011/admin` and ensures that HTML is returned with a title tag of `Login`. This test actually verifies that FusionAuth redirects to its login page when the admin UI is accessed.

A more complex test in this test suite is called `log in`. This test actually logs a user into FusionAuth and then ensures that the user can access the application. Here's the code of this test:
...
2. Clicks the login button and verifies that the browser is redirected to FusionAuth's OAuth login page
3. Fills out the login form with the ordinary user's credentials (`richard@fusionauth.io` - `password`)
4. Submits the login form
5. Ensures that the browser is redirected back to the application
6. Ensure the user is logged in

This is a complete test that logs a real user into the application without the need to mock or simulate the actual login flow. Rather, this uses a real running version of FusionAuth to log the user in.
...
This leverages a Github Marketplace action called [fusionauth/fusionauth-github-action](https://github.com/FusionAuth/fusionauth-github-action). This action will run FusionAuth within Github Actions such that our tests can leverage it.
...
[\< Go back to step 6 - Logout](step-6) <span class="inline-block mx-6 font-bold">Congrats you're finished!</span> [Step 8 - Finished \>](step-7)
