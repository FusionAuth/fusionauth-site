...
If you are interested in air-gapping, this document is for you! Air-gapped licensing is only available with an Enterprise plan. You may also find the documentation about [non-air gapped licensing](/docs/get-started/core-concepts/licensing) useful as well.
...
Normally, an instance of FusionAuth communicates with a licensing server. An air-gapped instance uses additional license text since there's no network access.

However, instances of FusionAuth in either configuration attempt to communicate with external servers. The recommendations is to block the following hostnames:
