...
tags: docker, kubernetes, k8s, container, aws
---
...
database:
  ...
  # To use an existing secret, set `existingSecret` to the name of the secret. We expect at most two keys: <InlineFieldValue>password</InlineFieldValue> is required. `rootpassword` is only required if `database.root.user` is set.
...
Navigate to `http://localhost:9011` and you will land on the FusionAuth Setup Wizard. For a complete tutorial walking you through the initial FusionAuth configuration, see [Setup Wizard & First Login](/docs/get-started/download-and-install/setup/).
