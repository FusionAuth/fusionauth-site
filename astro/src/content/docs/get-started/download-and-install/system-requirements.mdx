...
have any questions about platform support, please ask a question on <a href="https://fusionauth.io/community/forum/" target="_blank">the forum</a> or open an issue on <a href="https://github.com/FusionAuth/fusionauth-issues/issues/new/choose" target="_blank">Github</a>. If you have a licensed edition you may open a <a href="https://account.fusionauth.io" target="_blank">support request from your account</a>.
...
FusionAuth offers instructions on downloading the MySQL JDBC driver at install time. Due to the drivers' licensing, it cannot be bundled into the application. Prior to 1.16.0, the drivers were downloaded no matter which database you used. Prior to 1.40.0, the drivers were downloaded if you are using MySQL.
...
If you are using the community plan, you can block all outbound network connectivity. Lack of connectivity will not affect the product. However, once you are using a paid plan, you must be able to call home or your license will not function properly. You must also allow access to enable advanced features provided by FusionAuth Reactor cloud services.

If you are in a firewalled or restricted environment and have a paid plan, add the following hostnames to your network allow list:
