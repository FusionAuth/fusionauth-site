...
On a paid plan, you must enable network access to FusionAuth, or your features and license will not function properly.
...
Currently FusionAuth sends a payload to the metrics endpoint for all instances. Collected metrics include:
...
Customers on a paid plan must allow a metrics connection per the license agreement, unless they have an air-gapped license agreement.
