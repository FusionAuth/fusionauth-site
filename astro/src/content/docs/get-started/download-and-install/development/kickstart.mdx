...
FusionAuth Kickstart&#8482; is a declarative tool allowing you to build a template to quickly replicate various environments. Anything that can be built with the API can be set up using Kickstart allowing for you to share multiple environments among containers, test cases, and developers.
...
To add an [air-gapped license](/docs/get-started/download-and-install/air-gapping), use the `license` field. For example:
