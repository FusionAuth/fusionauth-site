...
This is typically only an issue when FusionAuth is accessing resources outside of the Docker network to, for example, send email or request a webhook. For example, if a Rails application is running locally on a mac and you want FusionAuth, running in Docker, to send a webhook payload to it, configuring FusionAuth to send it to `localhost` won't work, since `localhost` in the Docker container refers to the Docker container, not the host machine.
