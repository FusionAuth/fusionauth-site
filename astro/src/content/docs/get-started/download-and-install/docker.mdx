...
To generally clean up your images on your system `docker image prune` is useful too as it will remove all dangling images.
...
* For macOS and Windows hosts, you can use `host.docker.internal` as the hostname instead of `localhost`.
* For Linux hosts, add `extra_hosts: - "host.docker.internal:host-gateway"` to your `docker-compose.yml`.
