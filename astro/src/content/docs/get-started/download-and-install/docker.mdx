...
<RemoteCode title="Example docker-compose.yml with mailcatcher" url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-example-docker-compose/main/mailcatcher/docker-compose.yml" lang="yaml" />
...
* Remove the old image first: `docker rmi <old image id>`. This command may prompt you to remove containers using that image. Since all state is stored in the database, you can safely remove the containers. Then, when you recreate Docker Compose, the latest image will be retrieved.
* Pull the latest image with this command: `docker pull fusionauth/fusionauth-app:latest` or `docker compose pull` and recreate your deployment.
...
<RemoteCode title="Example docker-compose.yml for building fusionauth-app including a plugin" url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-example-docker-compose/main/build/docker-compose.yml" lang="yaml" />

<RemoteCode title="Example Dockerfile for building fusionauth-app including a plugin" url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-example-docker-compose/main/build/fusionauth-app/Dockerfile" lang="yaml" />

With this example can use `docker compose build` to run the steps in the referenced Dockerfile. This will create you a custom Docker image which is consequentially used in the creation of the container in Docker Compose when running `docker compose up -d`.
...
<RemoteCode title="Example docker-compose.yml for running Kickstart" url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-example-docker-compose/main/kickstart/docker-compose.yml" lang="yaml" />

After running `docker compose up -d` you should see a line like this in the logs `docker compose logs fusionauth`:
...
You may also want to check out [the Isolated Docker Setups](https://github.com/FusionAuth/fusionauth-example-scripts/tree/master/isolated-docker-setup) if you want the ability to rapidly stand up different versions and configurations of FusionAuth.
...
<RemoteCode title="Example docker-compose.yml for installing a plugin" url="https://raw.githubusercontent.com/sonderformat-llc/fusionauth-example-docker-compose/main/plugin/docker-compose.yml" lang="yaml" />
...
* Use `host.docker.internal` as the hostname instead of `localhost`; this works on macOS and Windows hosts, for linux you have to add `--add-host=host.docker.internal:host-gateway` to the docker engine.
...
The productive runtime requirements and configuration for OpenSearch will drastically differ from the docker compose examples as they're configured without any security and redundancy.
