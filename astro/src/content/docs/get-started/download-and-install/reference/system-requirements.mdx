...
You can also run FusionAuth with MySQL 8.0 or newer using MySQL Connector. MariaDB and Percona may also work. However, FusionAuth only tests with PostgreSQL, so functionality may suffer due to [known issues](https://github.com/FusionAuth/fusionauth-issues/issues/327). Non-PostgreSQL instances of FusionAuth cannot migrate to FusionAuth Cloud.
