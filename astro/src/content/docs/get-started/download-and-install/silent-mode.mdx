...
Silent mode can be used to automatically create the database and schema that FusionAuth uses. It can also grant the correct privileges to the correct database users so that FusionAuth is not connecting to the database as a super user. When enabled, silent mode will perform the following steps:
...
Because `database.url`, `database.username`, and `database.root.username` are all set, FusionAuth enables silent mode automatically.
