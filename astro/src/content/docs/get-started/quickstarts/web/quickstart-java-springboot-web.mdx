...
This controller serves the page for authenticated users on the `/account` route and adds the claims from the `OidcUser` returned from FusionAuth to the view model on the <InlineField>profile</InlineField> attribute. The `account` return string tells Spring to render the `account.html` template which you will create later.
...
Log in using <InlineFieldValue>richard@example.com</InlineFieldValue> and <InlineFieldValue>password</InlineFieldValue>.
