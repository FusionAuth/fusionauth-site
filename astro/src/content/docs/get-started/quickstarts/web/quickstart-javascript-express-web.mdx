...
In the ChangeBank application there is a <Breadcrumb>Login</Breadcrumb> button which will open the `/login` route when clicked. This route will redirect them to a login page managed by FusionAuth. Include the below code to the bottom of `index.ts`.
...
You can now open up an incognito window and visit the Express app at http://localhost:8080/ . Log in with the user account mentioned above, <InlineFieldValue>richard@example.com</InlineFieldValue>.
