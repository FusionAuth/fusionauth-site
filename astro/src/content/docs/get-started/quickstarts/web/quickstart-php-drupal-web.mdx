...
You can enable the modules by visiting the extensions page of your Drupal application at http://localhost/admin/modules. Search for "OpenID Connect" and "Admin Toolbar" using the <InlineField>Filter</InlineField> field. Select the module checkboxes and click the <InlineUIElement>Install</InlineUIElement> button.
...
Set <InlineField>Title</InlineField> to `Changebank Menu` and click <InlineUIElement>Save</InlineUIElement>.
...
Log in by clicking the <InlineUIElement>Login with FusionAuth</InlineUIElement> button and using the username <InlineFieldValue>richard@example.com</InlineFieldValue> and password <InlineFieldValue>password</InlineFieldValue>.
