...
tags: golang web quickstart, go web authentication, go login, golang oauth, go web app
htmlTitle: FusionAuth QuickStart for Golang
...
<DockerSpinup kickstartUri="https://raw.githubusercontent.com/FusionAuth/fusionauth-quickstart-golang-web/main/kickstart/kickstart.json" />
...
<ExtractedCode src="quickstart-golang-web/complete-application/go.mod" lang="go" title="go.mod" />
...
<ExtractedCode src="quickstart-golang-web/base-app.snippet.baseApplication.go" lang="go" title="main.go" />
...
Copy the `templates` and `static` directories from the `complete-application` directory in the cloned
QuickStart repo into your `changebank` project directory.

Here's what the home page template looks like:

<ExtractedCode src="quickstart-golang-web/complete-application/templates/home.html" lang="html" title="templates/home.html" />

And the stylesheet for the application:

<ExtractedCode src="quickstart-golang-web/complete-application/static/css/changebank.css" lang="css" title="static/css/changebank.css" />

<Aside type="tip">
With the home page template in place, you can view the home page in your browser at [http://localhost:8080](http://localhost:8080).
</Aside>
...
<ExtractedCode src="quickstart-golang-web/main.snippet.imports.go" lang="go" />

Next, create some constants after your `import` block and before `func main()`:

<ExtractedCode src="quickstart-golang-web/main.snippet.constants.go" lang="go" />

<Aside type="note">
If you want to see where the FusionAuth values came from, they can be found in the
FusionAuth app ([http://localhost:9011/admin](http://localhost:9011/admin)). The tenant ID is found on the Tenants
page. To see the client ID and client secret, go to the Applications page and
click the <Breadcrumb>View</Breadcrumb> icon under the actions for the Changebank application. You'll
find the client id and client secret values in the <Breadcrumb>OAuth configuration</Breadcrumb> section.
</Aside>
...
<ExtractedCode src="quickstart-golang-web/main.snippet.oidcClient.go" lang="go" />

### Modify the `/login` Route
When the user clicks the <Breadcrumb>Login</Breadcrumb> button in your Changebank app, they'll be
...
<ExtractedCode src="quickstart-golang-web/main.snippet.loginRoute.go" lang="go" />

The <Breadcrumb>Login</Breadcrumb> button on your home page will now take you to FusionAuth to log in.
...
<ExtractedCode src="quickstart-golang-web/main.snippet.callbackRoute.go" lang="go" />
...
<ExtractedCode src="quickstart-golang-web/main.snippet.accountRoute.go" lang="go" />

Here's the account page template:

`templates/account.html`
<ExtractedCode src="quickstart-golang-web/complete-application/templates/account.html" lang="html" />
...
<ExtractedCode src="quickstart-golang-web/main.snippet.homeRoute.go" lang="go" />
...
<ExtractedCode src="quickstart-golang-web/main.snippet.logoutRoute.go" lang="go" />
