...
* Your client Id is <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.applicationId" />.
* Your client secret is `super-secret-secret-that-should-be-regenerated-for-production`.
* Your example username is <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.userEmail" /> and the password is <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.userPassword" />.
* Your admin username is <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.adminEmail" /> and the password is <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.adminPassword" />.
...
1. Install the dependencies for the web server with the code below.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/src/main.rs" lang="rust" />
...
1. Create the file `.env` in the `your-application` directory and insert the following lines.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/.env" />
...
1. Authentication is handled by `src/auth.rs`. Create that file now and insert the following code.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/src/auth.rs" lang="rust" />
...
1. Create a stylesheet file `static/changebank.css` and add the following code to it.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/static/changebank.css" lang="css" />
...
1. Create a `templates` directory within `your-application`. Create three pages inside the `templates` directory. First create the home page, `index.html`, and paste the following code into it.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/templates/index.html" lang="html" />
...
1. Create an `account.html` page and paste the following code into it.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/templates/account.html" lang="html" />
...
1. Create a `change.html` page and paste the following code into it.
   ...
   <LocalCode src="fusionauth-quickstart-rust-actix-web/complete-application/templates/change.html" lang="html" />
...
1. Log in using <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.userEmail" /> and <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.userPassword" />. The change page allows you to enter a number and see the result of the calculation. Log out and verify that you can't browse to the account or change pages.
...
* I get "This site can't be reached localhost refused to connect" when I click the login button.
  ...
  Ensure FusionAuth is running in the Docker container. You should be able to log in as the admin user <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.adminEmail" /> with the password <LocalValue path="fusionauth-quickstart-rust-actix-web/kickstart/kickstart.json" selector="$.variables.adminPassword" /> at [http://localhost:9011/admin](http://localhost:9011/admin).
