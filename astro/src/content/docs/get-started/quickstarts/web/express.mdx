...
1. Import the `jsonwebtoken` dependency and upgrade `passport.use` to fetch user data:
   ...
   ```javascript
    ...
    passport.use('fusionauth', new OAuth2Strategy({
        ...
        // get the user uuid to name the note
        const user = {
            id: decoded.sub, 
            email: decoded.email,
    ...
    app.get('/', ensureAuthenticated, (req, res) => {
        ...
        // Ensure the file exists so we don't crash
        if (!fs.existsSync(TARGET_FILE)) {
            fs.writeFileSync(TARGET_FILE, `Hello ${req.user.id}!`, 'utf8');
        }
