...
If you do this, make sure to update the `Authorized Redirect URL` to reflect the `https` protocol. Also note that the project will probably run on a different port when using SSL, so you must update that as well. To do so, log into the administrative user interface, navigate to <Breadcrumb>Applications</Breadcrumb>, then click the <InlineUIElement>Edit</InlineUIElement> button on your application and navigate to the <Breadcrumb>OAuth</Breadcrumb> tab. You can have more than one URL.
