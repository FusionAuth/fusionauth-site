...
tags: next.js quickstart, nextjs authentication, next.js login, react server authentication, node.js authentication
...
```shell-session
rm -rf ./src/app
mkdir -p './src/app/api/auth/[...nextauth]' ./src/app/account ./src/app/makechange ./src/components
```

## Authentication

Next.js 13.2 introduced [Route Handlers](https://nextjs.org/docs/app/api-reference/file-conventions/route), which are the preferred way to handle REST-like requests. In the `Changebank` application you can configure NextAuth.js FusionAuth's provider in a new route handler by creating a file within `src/app/api/auth/[...nextauth]/route.ts`.
...
<Aside type="tip">
If this is your first time using the Next.js App Router, you should read through <a href="https://nextjs.org/docs/app/getting-started/layouts-and-pages" target="_blank">Routing Fundamentals</a>.
</Aside>

Below you will find the full code for the [Root Layout](https://nextjs.org/docs/app/getting-started/layouts-and-pages).
