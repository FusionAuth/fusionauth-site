...
tags: rails quickstart, ruby on rails authentication, rails login, rails oauth, ruby web authentication
...
<RemoteValue
  ...
  {(value) => <Prism lang="shell-session" code={`OP_SECRET_KEY="${value}" bundle exec rails s`} />}
</RemoteValue>
...
<RemoteValue
  ...
  selector="$.requests.[?(@.url === '/api/application/#{applicationId}')].body.application.oauthConfiguration.clientSecret">
  {(value) => (
    <Prism lang="shell-session" code={`git clone https://github.com/FusionAuth/fusionauth-quickstart-ruby-on-rails-web.git
cd fusionauth-quickstart-ruby-on-rails-web
docker compose up -d
cd complete-app
bundle install
