...
Next, update the layout so the user has login or logout buttons on every page. Replace the existing `<%= yield %>` in `app/views/layouts/application.html.erb` with the code below.
