...
tags: .net api, dotnet quickstart, c# api, asp.net, api authentication, .net authentication, bearer token
...
First off, grab the code from the repository:
...
```shell-session
cd complete-application && dotnet run
```
...
```shell-session
dotnet add package Microsoft.AspNetCore.Authentication.JwtBearer -v 10.0.10
```

```shell-session
dotnet add package Microsoft.AspNetCore.OpenApi -v 10.0.10
```

```shell-session
dotnet add package System.IdentityModel.Tokens.Jwt -v 8.22.0
```

```shell-session
dotnet add package Microsoft.IdentityModel.Protocols.OpenIdConnect -v 8.22.0
```

Next, replace your `appsettings.json` file with settings specific to the FusionAuth Issuer and Client Id (Audience).

<ExtractedCode src="quickstart-dotnet-api/complete-application/appsettings.json" lang="json" />

Replace the code in the `appsettings.Development.json` file with the following.

<ExtractedCode src="quickstart-dotnet-api/complete-application/appsettings.Development.json" lang="json" />
...
<ExtractedCode src="quickstart-dotnet-api/complete-application/Program.cs" lang="csharp" />
...
<ExtractedCode src="quickstart-dotnet-api/complete-application/Models/PanicResponse.cs" lang="csharp" />
...
<ExtractedCode src="quickstart-dotnet-api/complete-application/Models/Change.cs" lang="csharp" />
...
<ExtractedCode src="quickstart-dotnet-api/complete-application/Controllers/MakeChangeController.cs" lang="csharp" />
...
<ExtractedCode src="quickstart-dotnet-api/complete-application/Controllers/PanicController.cs" lang="csharp" />
...
```shell-session
git clone https://github.com/FusionAuth/fusionauth-quickstart-dotnet-api.git && cd fusionauth-quickstart-dotnet-api
```
