...
First let's try the requests as the <InlineFieldValue>teller@example.com</InlineFieldValue> user. Based on the configuration this user has the `teller` role and should be able to use both `/make-change` and `/panic`.

1. Acquire an access token for <InlineFieldValue>teller@example.com</InlineFieldValue> by making the following request
...
Now let's try as <InlineFieldValue>customer@example.com</InlineFieldValue> who has the role `customer`. Acquire a token for <InlineFieldValue>customer@example.com</InlineFieldValue>.
...
Ensure FusionAuth is running in the Docker container.  You should be able to login as the admin user, <InlineFieldValue>admin@example.com</InlineFieldValue> with a password of <InlineFieldValue>password</InlineFieldValue> at [http://localhost:9011/admin](http://localhost:9011/admin).
