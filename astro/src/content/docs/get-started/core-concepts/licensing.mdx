...
An air-gapped license is designed to work in an air-gapped environment, but you must implement network controls blocking outbound requests from FusionAuth.

Here's the list of hostnames FusionAuth will contact that you can block with an air-gapped license.
