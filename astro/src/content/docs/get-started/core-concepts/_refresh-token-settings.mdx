...
<APIBlock>
  ...
  <APIField name="Refresh token revocation" optional renderif={props.page === "tenant"}>
    ...
    * **On multi-factor enable**: revokes all of a user's refresh tokens when a user enables [multi-factor authentication](/docs/lifecycle/authenticate-users/multi-factor-authentication).
    * **On password change**: revokes all of a user's refresh tokens when a user changes their password.
    * **On reuse of a one-time use token**: revokes the _entire refresh token chain_ on submission of any one-time use token that has already been rotated outside of the configured grace period. This includes the current refresh token. Does not affect refresh tokens from the user's other sessions.
  </APIField>
</APIBlock>
