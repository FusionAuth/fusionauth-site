...
You can search for users using the search field. Learn more about <ScrollRef target="User Search" />.
...
To edit a user, navigate to <Breadcrumb>Users</Breadcrumb> and click the <InlineUIElement>Manage</InlineUIElement> button next to the user you wish to edit.
...
One option is to set up an administrative identity server, whether that is a second instance of FusionAuth, Google GSuite, or Azure AD/Microsoft Entra ID.
...
User search requests may be made through the [User Search API](/docs/apis/users#search-for-users) or within the FusionAuth admin UI under <strong>Users</strong>.
...
You can add arbitrary JSON to the `user.data` field, such as `org_type` and `org_id` fields. You can do this using the [User API](/docs/apis/users) on user creation.
