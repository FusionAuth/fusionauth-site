...
<APIBlock>
  ...
  <APIField name="Skip verification" optional>
    If this is checked, FusionAuth will not send a message asking the user to verify their email address or phone number. This is only available if [Email verification](/docs/get-started/core-concepts/tenants#email) or [Phone verification](/docs/get-started/core-concepts/tenants#phone) in the tenant is set up.
  ...
  <APIField name="Send password setup to" optional>
    If this is set to `Email` or `Phone`, FusionAuth will send an email or SMS message asking the user to set their password.
    If you also have verification enabled and do not skip verification with <InlineField>Skip verification</InlineField> setting, only the setup password message will be sent to the user. Setting up the password using the message sent during this user create operation will verify the user’s email address or phone number if it is not already verified.
