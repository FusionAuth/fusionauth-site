...
<APIBlock>
  ...
  <APIField name="Send password setup to" optional>
    ...
    <Aside>
      ...
      * Implicit verification is enabled on the tenant
      ...
      Then only the setup password message will be sent to the user. Setting up the password using the message sent during this user create operation will verify the user’s email address or phone number if it is not already verified.
    </Aside>
    <Aside>
      <InlineField>Send password setup to</InlineField> is only available if the following are configured for the tenant:
      * [Setup password option](/docs/get-started/core-concepts/tenants#template-settings) - template settings must be set up
  ...
  <APIField name="Password" required>
    ...
    <Aside>
      ...
      * <InlineField>Send password setup to</InlineField> is not enabled or is set to `Do not send`
      * [Password enabled](/docs/get-started/core-concepts/tenants#password) - set to `true`
    </Aside>
  </APIField>
