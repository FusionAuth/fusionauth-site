...
For example, suppose you have a system with agents A, B, and C. With entities, you can build a permission graph such that agent A can call agent B, agent B can call agent C, but agent A cannot call agent C, agent B cannot call agent A, and agent C cannot call either A or B.

Whenever an agent wants to call another agent, they:

* present their id, secret, and desired target to FusionAuth
* FusionAuth considers if the agent has permission to call the other agent
* if denied, an error is returned
* if allowed, an access token is returned
* the agent calling the other agent presents the access token
* the called agent can verify that access token
* the called agent can then accept the request
