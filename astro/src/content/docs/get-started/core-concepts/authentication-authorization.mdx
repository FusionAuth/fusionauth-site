...
tags: authentication, authorization, authn, authz, fusionauth concepts, authentication vs authorization, identity, access control
order: 100
---

Authentication and authorization are two fundamental concepts in FusionAuth. Traditionally, we define them as:
...
Authentication means that a user has provided credentials which the system has accepted. This is often a username and password, but could be a one-time password, a token from a social auth provider, or a JWT from an external identity provider.
...
You grant access to a particular application with a [User Registration](/docs/get-started/core-concepts/types/registrations). Once registered, a user can have 0 or more [roles](/docs/get-started/core-concepts/types/roles) as defined by the [Application](/docs/get-started/core-concepts/types/applications).
