...
* Upgrade the database schema by running the migrations, bringing the database to version `1.x`. If you are running in `production` and `silent` mode, FusionAuth will apply the changes, otherwise you must apply them out of band (by running the SQL scripts). Read more about [silent mode](/docs/get-started/download-and-install/reference/silent-mode).
