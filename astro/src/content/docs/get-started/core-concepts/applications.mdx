...
FusionAuth Applications have both an **applicationId** and a **client_id**, but both use the same `Id` value. You'll provide these values for Login API requests or authorization grants to specify the application that the user would like to access.
...
1. Select a Tenant for your Application.

1. In the **OAuth** tab, configure a login method. Generally, you should at least configure **Authorized redirect URLs** and a **Logout URL** with the values provided by your IdP.
...
<APIBlock>
  <APIField name="Id" optional>
    An optional UUID. When this value is omitted a unique Id will be generated automatically. This will also be used as the **Client Id** in the <Breadcrumb>OAuth</Breadcrumb> configuration, so if you require a specific value for that, set it here. Application Ids are unique across all tenants.
  </APIField>
...
<APIBlock>
  <APIField name="Enable application-level JWT override" optional>
...
<APIBlock>
  ...
  <APIField name="Voice template" optional since="1.65.0">
...
<APIBlock>
  <APIField name="Enable application-level WebAuthn override" optional>
...
<APIBlock>
  <APIField name="Enable re-authentication workflow" optional>
...
<APIBlock>
  ...
  <APIField name="Enable auto-deletion of unverified registrations" optional>
...
Use <Breadcrumb>Registration mode</Breadcrumb> to select registration behavior, then choose <Breadcrumb>Basic</Breadcrumb> or <Breadcrumb>Advanced</Breadcrumb> to define the registration form experience.
...
<APIBlock>
  <APIField name="Registration mode" optional since="1.65.0">
...
<APIBlock>
  <APIField name="Enable SAML v2" optional>
...
<APIBlock>
  <APIField name="Require signed authentication requests" optional>
    When enabled, all unsigned requests will be rejected.
  </APIField>
  <APIField name="Authentication request verification keys" optional since="1.69.0">
...
<APIBlock>
  <APIField name="Require signed logout requests" optional since="1.25.0">
...
<APIBlock>
  <APIField name="Enable assertion encryption" optional defaults="false" since="1.47.0">
...
<APIBlock>
  <APIField name="Enable passwordless login" optional>
...
<APIBlock>
  <APIField name="Enable authentication tokens" optional>
...
You can only set an application as universal when it is created. Do this by setting **application.universalConfiguration.universal** to `true` when using the API, or select the <Breadcrumb>Universal</Breadcrumb> toggle on the Add Application view in the admin app.
