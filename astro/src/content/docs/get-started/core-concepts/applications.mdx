...
<APIBlock>
  <APIField name="Enabled" optional>
    When enabled, allow users to request login using a link sent via email (or code sent via an SMS message). Enabling this feature will cause a button to be displayed on the FusionAuth login form and allow you to utilize the Passwordless Login API.
  </APIField>
</APIBlock>
...
### Unversal Applications and the Self-serve Account application
