...
A FusionAuth Application is simply something a user can log into. When you use the Login API, you will provide an <InlineField>applicationId</InlineField> to indicate what resource you're attempting to obtain authorization. When you use one of the OAuth2 / OpenID Connect authorization grants you will provide a <InlineField>client_id</InlineField> in some fashion. This client identifier will be unique to a single FusionAuth Application which allows FusionAuth to verify the User is registered for the requested Application and subsequently return the correct roles. The <InlineField>applicationId</InlineField> and <InlineField>client_id</InlineField> can be considered synonymous, both concepts use the same <InlineFieldValue>Id</InlineFieldValue> value.
...
You can only set an application as universal when it is created. Do this by setting <InlineField>application.universalConfiguration.universal</InlineField> to <InlineFieldValue>true</InlineFieldValue> when using the API, or select the <Breadcrumb>Universal</Breadcrumb> toggle on the Add Application view in the admin app.
