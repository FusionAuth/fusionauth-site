...
<APIBlock>
  <APIField name="On login policy" optional>
    When set to `Enabled`, a two-factor challenge will be required during login when a user has configured one or more two-factor methods. When set to `Disabled`, even when a user has one or more two-factor methods configured, a two-factor challenge will not be required during login. When set to `Required`, a two-factor challenge will be required during login. If a user does not have configured two-factor methods, they will be required to configure a method to log in.

    The risk based policies use FusionAuth's Intelligent MFA capabilities, which combines multiple signals to determine whether a user should be challenged with MFA. If a user does not have configured two-factor methods, they will be required to configure a method to log in.
...
Use <InlineUIElement>Registration mode</InlineUIElement> to select registration behavior, then choose <InlineUIElement>Basic</InlineUIElement> or <InlineUIElement>Advanced</InlineUIElement> to define the registration form experience.
