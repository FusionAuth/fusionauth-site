...
|---------------------------------------------------------------------------------------------------------|--------------------------------------------------|--------------------------------|--------------------------------|
| Allows for easy horizontal scaling                                                                      | Yes                                              | No                             | Yes |
| Revocation possible                                                                                     | [Yes](/learn/expert-advice/tokens/revoking-jwts) | Yes                            | Yes |
| Revocation easy                                                                                         | No                                               | Yes                            | No  |
| Sticky sessions or session datastore required                                                           | No                                               | Yes                            | No  |
| Token transparently sent on requests (to APIs, etc)                                                     | Yes                                              | N/A                            | No  |
| Token can be presented to APIs on domains other than that in which the cookie setting component resides | No                                               | Yes (via server-side requests) | Yes |
| Works in a web browser                                                                                  | No                                               | Yes (via server-side requests) | No  |

The proper JWT storage choice is based on your threat modeling. You can also configure your JWTs to be short lived, which minimizes the amount of time a stolen JWT can be used.
