...
You can find details on adding a connector [below](#add-connector-policy-dialog). Full documentation on Connectors and Connector Policies can be found [here](/docs/lifecycle/migrate-users/connectors/).
...
<APIBlock>
  <APIField name="Action">
    ...
    *Note:* For the `Failed login` action, rate limiting can be used in combination with <ScrollRef target="Failed authentication settings" />. When enabled, and rate limiting conditions have been exceeded, login requests will be denied and bypass the login flow until conditions are no longer exceeded. <InlineField>Failed attempts</InlineField> will only be incremented when requests are not being rate limited.
  </APIField>
