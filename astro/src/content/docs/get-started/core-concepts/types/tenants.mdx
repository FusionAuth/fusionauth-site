...
tags: fusionauth tenants, multi-tenancy, tenant configuration, tenant isolation, saas tenancy
order: 1
---
...
<APIBlock>
  ...
  <APIField name="Base URL" optional since="1.68.0">
...
<APIBlock>
  <APIField name="Admin user form" optional since="1.20.0">
    ...
    <PlanBlurbApi plan="starter" />
  </APIField>
</APIBlock>

#### Username settings

Enable **Append unique suffix to usernames** to allow multiple users suggest the same username. If there are any collisions, FusionAuth will transparently create a unique username by appending a suffix. The user will continue to use the username without the suffix.

<APIBlock>
  <APIField name="Append unique suffix to usernames" optional defaults="false" since="1.27.0">
...
<APIBlock>
  ...
  <APIField name="Migrate to FusionAuth" optional>
    When selected, copy the user from the external Connector into FusionAuth so future logins authenticate against FusionAuth directly.
  </APIField>
</APIBlock>
...
<APIBlock>
  <APIField name="Messenger" required since="1.59.0">
    ...
    Required when the **Verify identity** toggle is enabled for phone numbers or when any phone templates are configured in <Breadcrumb>Template Settings</Breadcrumb>.
  </APIField>
</APIBlock>
...
<APIBlock>
  ...
  <APIField name="Allow email change while verification is pending" optional since="1.27.0">
    When enabled, the user is allowed to change their email address when they are gated because they haven't verified their email.
  </APIField>
  <APIField name="Allow phone number change while verification is pending" optional since="1.59.0">
    When enabled, the user is allowed to change their phone number when they are gated because they haven't verified their phone number.
  </APIField>
  <APIField name="Enable auto-deletion of unverified users" optional>
  ...
  <APIField name="Delete after" required>
    ...
    Required when the **Enable auto-deletion of unverified users** toggle is enabled.
  </APIField>
</APIBlock>
...
<APIBlock>
  ...
  <APIField name="Set SMTP password" optional>
  ...
  <APIField name="Enable debug logging" optional since="1.37.0">
    Creates event log entries to help debug SMTP and email delivery errors. Check <Breadcrumb>System -> Event Log</Breadcrumb>.
  </APIField>
</APIBlock>
...
<APIBlock>
  <APIField name="Enable family settings" optional>
  ...
  <APIField name="Maximum child age" required>
    ...
    Required when the **Enable family settings** toggle is `enabled`.
  </APIField>
  ...
  <APIField name="Enable auto-deletion of unverified children" optional>
  ...
  <APIField name="Delete after" required>
    ...
    Required when the **Enable auto-deletion of unverified children** toggle is enabled.
  </APIField>
</APIBlock>
...
The Authenticator/TOTP implementation is not a premium feature, and is included in the Community version. Email, SMS, and Voice require the Advanced Multi-Factor feature available in the **Starter** plan.
...
<APIBlock>
  <APIField name="On login policy" optional>
    ...
    * Challenge On Medium Risk: Only challenge on medium or high login risk &nbsp; <AvailableSince since="1.68.0"/>
    * Challenge On High Risk: Only challenge on high login risk &nbsp; <AvailableSince since="1.68.0"/>
    * Required: Always challenge during login &nbsp; <AvailableSince since="1.42.0"/>
...
<APIBlock>
  <APIField name="Enable authenticator (TOTP) MFA" optional>
...
<APIBlock>
  <APIField name="Enable email MFA" optional>
  ...
  <APIField name="Template" required>
    ...
    Required when the **Enable email MFA** toggle is `enabled`.
  </APIField>
</APIBlock>
...
Enabling either or both of the **Enable SMS MFA** or **Enable voice MFA** toggles in this section allows users to use a mobile phone number to complete a multi-factor authentication request.

<APIBlock>
  <APIField name="Enable SMS MFA" optional>
    When enabled, users can receive two-factor codes on their mobile phone via SMS.
  </APIField>
  <APIField name="SMS messenger" required>
    The Messenger used to send codes via SMS.

    Required when the **Enable SMS MFA** toggle is `enabled`.
  </APIField>
  <APIField name="SMS template" required>
    The SMS template to use.

    Required when the **Enable SMS MFA** toggle is `enabled`.
  </APIField>
  <APIField name="Enable voice MFA" optional>
    When enabled, users can receive two-factor codes on their mobile phone via voice message.
  </APIField>
  <APIField name="Voice messenger" required>
    The Messenger used to send codes via voice.

    Required when the **Enable voice MFA** toggle is `enabled`.
  </APIField>
  <APIField name="Voice template" required>
...
<APIBlock>
  ...
  <APIField name="Relying party Id" optional>
    ...
    It is important that this is configured according to the URL that a user would see in their browser while on the FusionAuth login page. See the [WebAuthn Admin Guide](/docs/lifecycle/authenticate-users/passkeys/) for more details on how to select the appropriate value.
  ...
  <APIField name="Enable debug logging" optional>
    Creates event log entries to help debug WebAuthn integration errors. Check <Breadcrumb>System -> Event Log</Breadcrumb>.
  </APIField>
</APIBlock>
...
<APIBlock>
  <APIField name="Enable passkey authentication" optional>
    When enabled, the WebAuthn bootstrap workflow can be used to authenticate users via WebAuthn from new or repeat devices.

    The availability of the bootstrap workflow can be overridden at the application level on the [WebAuthn Tab of the Application settings](/docs/get-started/core-concepts/types/applications#webauthn).
  </APIField>
...
<APIBlock>
  <APIField name="Enabled" optional>
    ...
    The availability of the re-authentication workflow can be overridden at the application level on the [WebAuthn Tab of the Application settings](/docs/get-started/core-concepts/types/applications#webauthn).
  </APIField>
  <APIField name="Authenticator attachment" optional>
...
<APIBlock>
  ...
  <APIField name="Allow access token bootstrap to create SSO session" since="1.56.0">
...
<APIBlock>
  ...
  <APIField name="Email user when action is taken" since="1.42.0">
...
<APIBlock>
  <APIField name="Enable breached password detection" optional>
...
<APIBlock>
  <APIField name="Enable SCIM server" optional>
...
<APIBlock>
  <APIField name="Enable CAPTCHA" optional>
...
![Tenant Configuration - Security: Blocked domains](/img/docs/get-started/core-concepts/tenant-configuration-security-blocked-domains.png)
...
![Tenant Configuration - Security: Client risk configuration and rate limiting](/img/docs/get-started/core-concepts/tenant-configuration-security-client-risk-rate-limit.png)
...
<APIBlock>
  ...
  <APIField name="Blocklisted IP" type="Boolean" optional defaults="true">
    Checks whether the client's IP address appears on a blocklist.
  </APIField>
  <APIField name="Bot detected" type="Boolean" optional defaults="true">
    Detects bot interactions with the browser window.
  </APIField>
  <APIField name="Dormant account" type="Boolean" optional defaults="true">
    Checks if the user has not logged in for a long period of time.
  </APIField>
  <APIField name="Dormant password" type="Boolean" optional defaults="true">
    Checks if the user's password has not been changed for a long period of time.
  </APIField>
  <APIField name="Impossible travel" type="Boolean" optional defaults="true">
  ...
  <APIField name="Recent identity change" type="Boolean" optional defaults="true">
    Checks if the user's login ID has been changed recently.
  </APIField>
  <APIField name="Recent password change" type="Boolean" optional defaults="true">
    Checks if the user's password has been changed recently.
  </APIField>
  <APIField name="Suspicious user agent" type="Boolean" optional defaults="true">
    Checks whether the client's user agent has been flagged as suspicious.
  </APIField>
  <APIField name="Unrecognized device" type="Boolean" optional defaults="true">
    Checks whether the request originates from an unrecognized device.
  </APIField>
  <APIField name="Untrusted device" type="Boolean" optional defaults="true">
