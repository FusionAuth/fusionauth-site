...
tags: fusionauth roles, user roles, rbac, role-based access control, role assignment, permissions
order: 5
---
import RoleAttributes from 'src/content/docs/get-started/core-concepts/_role_attributes.mdx';

Roles in FusionAuth are associated with an [application](/docs/get-started/core-concepts/types/applications). You can define as many roles as you want in an application. There are no limits on the number of roles a user or a group can have.
...
You can associate roles with [users](/docs/get-started/core-concepts/types/users) directly via their [registration](/docs/get-started/core-concepts/types/registrations). Or you can assign an application role to a [group](/docs/get-started/core-concepts/types/groups), and then any users in that group who have access to that application will have that role.
...
Below you can see a user's registration in the FusionAuth application, with their assigned admin role.

<Screenshot id="user-admin-role" url="/admin/user/manage/00000000-0000-0000-0000-100000000001" alt="FusionAuth application roles" fullPage={true}>
  <Highlight selector="table tbody tr:first-child td:nth-child(3)" />
</Screenshot>
...
Below are all the roles available in the FusionAuth Admin UI. There is [additional documentation for the `user_support_manager` role](#the-user_support_manager-role) and the [`mfa_deleter` role](#the-mfa_deleter-role).
...
| `mfa_deleter`              | `631ecd9d-8d40-4c13-8277-80cedb823715` | Can remove a user's multi-factor methods in the admin UI with additional safety constraints. See below. <AvailableSince since="1.68.0"/>                                      |
...
### The mfa_deleter Role

The `mfa_deleter` role allows an administrator to remove MFA methods from users in the Admin UI without requiring a one-time code from the target user. To reduce abuse risk, this role has additional constraints:

* The target user cannot be registered to the FusionAuth admin application.
* The target user must be notifiable through at least one verified primary identity:
...
* Users with this role removing their own MFA methods still use the standard verification code flow.
