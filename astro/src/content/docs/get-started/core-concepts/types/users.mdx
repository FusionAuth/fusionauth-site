...
tags: fusionauth users, user management, user data, user profile, user attributes, identity
order: 3
---
...
* <Icon name="lock-button"/> Lock the user
* <Icon name="unlock-button" /> Unlock the user
...
<Aside>
  ...
  * A [Setup password](/docs/get-started/core-concepts/types/tenants#template-settings) template has been set on the user's tenant
  * [Password enabled](/docs/get-started/core-concepts/types/tenants#password) has been set to `true`
</Aside>
...
<APIBlock>
  ...
  <APIField name="Skip verification on email or phone number changes" optional>
    If this is checked, FusionAuth will not send a message asking the user to verify their email address or phone number. This is only available if [Email verification](/docs/get-started/core-concepts/types/tenants#identities) or [Phone verification](/docs/get-started/core-concepts/types/tenants#identities) in the tenant is set up.

    <Aside>
      If both an email address and phone number are supplied and **Skip verification on email or phone number changes** is checked, then verification will be skipped for both the email address and the phone number.
    </Aside>
  </APIField>
  <APIField name="Email" optional>
    The email address of the user. This is a required field if **Send password setup to** is set to `Email`. It must be unique across all users in the tenant.
  </APIField>
  <APIField name="Phone number" optional>
    The phone number of the user. This is a required field if **Send password setup to** is set to `Phone`.
  </APIField>
  <APIField name="Username" optional>
    The username of the user. The username is stored and returned as a case-sensitive value, however, a username is considered unique regardless of the case. This is a required field if no **Email** or **Phone number** is provided.
  </APIField>
  <APIField name="Send password setup to" optional>
    ...
    * **Skip verification on email or phone number changes** is not checked
  ...
  <APIField name="Confirm password" required>
    Password confirmation field. You must enter the same value in both **Password** and **Confirm password**.
  </APIField>
</APIBlock>
...
Learn more about [Applications](/docs/get-started/core-concepts/types/applications).
...
Learn more about [WebAuthn and passkeys](/docs/lifecycle/authenticate-users/passkeys/configure).
...
Learn more about [Groups](/docs/get-started/core-concepts/types/groups).
...
Learn more about [Entities and Grants](/docs/get-started/core-concepts/types/entity-management).
...
Learn more about [Tenants](/docs/get-started/core-concepts/types/tenants).
...
* A user is created.
* A user logs in.
* The [Login Ping API](/docs/apis/login/update-instant) is used.
...
* Users imported with the [User Import API](/docs/apis/users/import)
* Users whose profile data is updated via the [User Update API](/docs/apis/users/update)
* Identities created using the [Start Identity Verification API](/docs/apis/identity-verify#start-identity-verification), with an **existingUserStrategy** of `mustNotExist`, that have not been verified and used to create a user
</Aside>
...
By selecting the appropriate [installation guide](/docs/get-started/download-and-install/fusionauth-app), one can easily create a configuration with Elasticsearch pre-enabled.
...
User search requests may be made through the [User Search API](/docs/apis/users/search) or within the FusionAuth admin UI under <Breadcrumb>Users</Breadcrumb>.
...
FusionAuth provides an advanced user search interface that reveals how you may construct **queryString** and **query** parameters for the [User Search API](/docs/apis/users/search) and [User Bulk Delete API](/docs/apis/users/bulk-delete) with desired results. Navigate to <Breadcrumb>Users</Breadcrumb> from the left navigation and click on the <Breadcrumb>Advanced</Breadcrumb> link below the **Search** input field to begin. The [<Breadcrumb>Advanced</Breadcrumb>](/docs/apis/users/search) portion of this UI is available when the search engine type is configured to `elasticsearch`.
...
You can search for users across tenants using the [User Search API](/docs/apis/users/search).
