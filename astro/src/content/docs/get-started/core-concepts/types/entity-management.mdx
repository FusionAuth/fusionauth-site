...
tags: entity management, machine to machine, non-user entities, service accounts, entity grants, entity types
order: 7
---
...
<APIBlock>
  ...
  <APIField name="Access token verification keys" optional since="1.69.0">
    These are the keys that are trusted for this entity type when entity JWTs are presented to /oauth2/introspect or in SCIM use cases. If a verification key is expiring, add its replacement here so either key can be used to verify until the old one expires or is removed. The *Access token signing key* value does not need to be explicitly added here.
  </APIField>
</APIBlock>
