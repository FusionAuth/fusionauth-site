---
title: Registrations
description: An overview of FusionAuth Registrations.
tags: user registration, application registration, user roles, registration data, application membership
order: 4
---
import RegistrationAttributes from 'src/content/docs/get-started/core-concepts/_registration_attributes.mdx';
import RegistrationsSelfService from 'src/content/docs/_shared/_registrations-self-service.mdx';

Registrations in FusionAuth are the link between [Users](/docs/get-started/core-concepts/types/users) and [Applications](/docs/get-started/core-concepts/types/applications).

A User can have zero or more registrations. Each registration can have zero or more [roles](/docs/get-started/core-concepts/types/roles) associated with it. 


The [registrations API](/docs/apis/registrations) documents the allowed attributes of a User registration.

If a User exists in a [tenant](/docs/get-started/core-concepts/types/roles) and attempts to authenticate against an [Application](/docs/get-started/core-concepts/types/applications), but are not registered, the [authentication will succeed but they will not be authorized](/docs/get-started/core-concepts/authentication-authorization).

## Core Concepts Relationships

Below is a visual reminder of the relationships between FusionAuth's primary core concepts.

![Diagram showing Registrations used within FusionAuth](/img/docs/get-started/core-concepts/core-concepts-relationships-registrations.png)

### Attributes

With [advanced registration forms](/docs/lifecycle/register-users/advanced-registration-forms), you can customize the attributes of a registration. By default, registrations have the following attributes:

<RegistrationAttributes />

### Registrations and Self-Service Registration

<RegistrationsSelfService />
