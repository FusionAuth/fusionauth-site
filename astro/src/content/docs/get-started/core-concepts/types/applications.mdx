...
tags: fusionauth application, application configuration, oauth application, client id, client secret, application setup
order: 2
---
...
<APIBlock>
  ...
  <APIField name="MFA requirement lambda" since="1.62.0" optional>
    ...
    * Trigger a suspicious login event based on your criteria. &nbsp; <AvailableSince since="1.62.0"/>
    <PlanBlurbApi plan="enterprise" feature="MFA requirement lambdas" />
  </APIField>
</APIBlock>
...
<APIBlock>
  ...
  <APIField name="Logout request verification keys" optional since="1.69.0">
    The verification keys used to verify a signature when the SAML v2 Service Provider is using HTTP Redirect Bindings OR HTTP POST Bindings.
