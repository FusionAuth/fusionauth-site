...
To create a new project from a template, first create or open a Solution. Then select <Breadcrumb>File -> New Project</Breadcrumb>. In the <Breadcrumb>New Project</Breadcrumb> dialog, select <InlineFieldValue>FusionAuth</InlineFieldValue> from the <InlineField>All project types</InlineField> dropdown. Select the FusionAuth template you want to use. Set the project name and click <Breadcrumb>Next</Breadcrumb>. Fill in the required information, including the fully qualified URL to your FusionAuth server and your FusionAuth Application Client Id.
