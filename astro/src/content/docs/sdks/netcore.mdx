...
To create a new project from a template, first create or open a Solution. Then select <Breadcrumb>File -> New Project</strong>. In the "New Project" dialog, select <strong>Custom</Breadcrumb> from the left-hand menu. Select the FusionAuth template you want to use and click <InlineUIElement>Continue</InlineUIElement>. Fill in the required information, including the fully qualified URL to your FusionAuth server and your FusionAuth Application Client Id. Click <InlineUIElement>Continue</InlineUIElement>.
