...
<APIBlock>
  ...
  <APIField name="Authorized resource URIs" optional since="1.67.0">
    An optional list of allowed resource server URIs for this Application, per RFC 8707 Resource Indicators for OAuth 2.0. Defaults to an empty list.
    ...
    When this list is empty (the default), any `resource` parameter sent by the client is silently ignored and existing application behavior is unchanged.
