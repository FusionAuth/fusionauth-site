...
<APIBlock>
  ...
  <APIField name="Client secret" readonly>
    The client secret as defined by [RFC 6749 Section 2.3.1](https://tools.ietf.org/html/rfc6749#section-2.3.1). When <InlineField>Client Authentication</InlineField> is <InlineFieldValue>Required</InlineFieldValue>, this client secret will be required to obtain an access token from the Token endpoint.
  ...
  <APIField name="Generate refresh tokens" since="1.3.0">
    When enabled, FusionAuth will return a refresh token when the <InlineFieldValue>offline_access</InlineFieldValue> scope has been requested. When this setting is disabled refresh tokens will not be generated even if the <InlineFieldValue>offline_access</InlineFieldValue> scope is requested.
  ...
  <APIField name="Logout URL" optional>
    ...
    See the [Logout endpoint](/docs/lifecycle/authenticate-users/oauth/endpoints#logout) or the [Logout And Session Management guide](/docs/lifecycle/authenticate-users/logout-session-management) for more information.
  </APIField>
