...
* User collisions are not handled by FusionAuth. If the Okta SCIM client created an account for a user with the email address <InlineFieldValue>richard@example.com</InlineFieldValue> in the tenant, then later the Azure AD SCIM client tried to create an account for a user with the email address <InlineFieldValue>richard@example.com</InlineFieldValue>, the second account creation would fail. One workaround would be for the lambda to resolve the collision, for example by adding a suffix or prefix to make the email address unique.
