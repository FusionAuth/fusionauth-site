...
You can also monitor FusionAuth usage with webhooks, use [IP ACLs](/docs/advanced-threat-detection/#ip-acls) to limit where the API key can be used from, or place an [HTTP proxy](/docs/operate/deploy/proxy-setup) in front of FusionAuth to further limit access.
