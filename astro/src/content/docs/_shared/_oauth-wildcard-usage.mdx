...
Configured URLs containing wildcards are considered during validation when **{props.fieldName}** is set to <code>{props.wildcard}</code>. Wildcards are allowed in the following positions:{/* eslint-disable-line */}
