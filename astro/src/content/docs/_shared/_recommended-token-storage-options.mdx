...
If you need to lock down JWTs further, implement [token sidejacking protection](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_Cheat_Sheet_for_Java.html#token-sidejacking) using cookies and a nonce. An alternative is [DPoP](/docs/lifecycle/authenticate-users/oauth/dpop), which uses sender-constrained (proof-of-possession) tokens to further minimize the risk of token theft and replay.
