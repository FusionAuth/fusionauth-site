...
tags: prometheus, grafana, loki, monitoring, metrics, alerts, observability, dashboards, log aggregation
---

This guide explains how to monitor FusionAuth logs and metrics with the open-source tools [Prometheus](https://prometheus.io/docs/introduction/overview), [Loki](https://grafana.com/docs/loki), and [Grafana](https://grafana.com/products/cloud/grafana/), and receive alerts when problems occur.
...
- Enter the value `prime_mvc_____errors_total` in the **Metric** browser field at the bottom. Click <Breadcrumb>Run queries</Breadcrumb>, change the panel **Title**, and then click <Breadcrumb>Apply</Breadcrumb> to save the visualization.
...
The `prometheusPromtailConfig.yml` file controls which containers Promtail will get logs from. It is documented [here](https://grafana.com/docs/loki/latest/send-data/promtail/). Create the `prometheusPromtailConfig.yml` file and add the content below.
...
- `http://faLoki:3100` in the **URL** field - this is the only setting to change.
...
- [Grafana](https://grafana.com/products/cloud/grafana/)
