...
## Install and Configure OpenTelemetry and Prometheus

There are a couple of ways to install [OpenTelemetry](https://opentelemetry.io) and [Prometheus](https://prometheus.io). We'll walk through using our various Docker Compose files, but you can also download everything by-hand. If you download and install everything yourself, you'll need to configure it by-hand as well.

First, let's create a sub-directory for all of our monitoring items:
...
This is a subdirectory off of the main project. We're using a subdirectory because if you are using the main FusionAuth Docker Compose installation method, it references the monitoring configuration files in a subdirectory named `monitoring`.

### Configuration Files

Next, you'll need to create a handful of configuration files for these services. We've created example configuration files that you can download and use with only minor modifications. To download these files, use these URLs:

* https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/monitoring/alertmanager-config.yml
* https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/monitoring/opentelemetry-collector-config.yml
* https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/monitoring/prometheus-opentelemetry-config.yml
* https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/monitoring/prometheus-rules.yml

Once you've downloaded each of these files, you'll need to rename the Prometheus file from `prometheus-opentelemetry-config.yml` to `prometheus-config.yml`. You can run a command like this to rename this file (this command should work on Windows and *nix systems):

```shell
mv prometheus-opentelemetry-config.yml prometheus-config.yml
```

The OpenTelemetry Collector is configured to pull metrics from the apps it monitors (`receivers`) and allow other apps (`exporters`) to pull metrics from it. Let's look at how this is configured. Open the file `opentelemetry-collector-config.yml`. It should look something like this:

<RemoteCode lang="yaml" url="https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/opentelemetry-collector-config.yml"/>
...
<RemoteCode lang="yaml" url="https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/prometheus-opentelemetry-config.yml"/>

This configures Prometheus to pull metrics from the OpenTelemetry Collector every fifteen seconds.

### Docker Compose Files

Finally, you'll need to download our monitoring Docker Compose files or tell the main FusionAuth Docker Compose to start up the monitoring services. We'll cover both of these options below. As we mentioned above, if you installed these services by-hand, you'll need to configure them by-hand as well. That process is out-of-scope of this document, so review the documentation for OpenTelemetry and Prometheus to determine how to configure them.

#### Main Docker Compose

If you are using the FusionAuth Docker Compose file from our download page, this file contains optional profiles that start OpenTelemetry and Prometheus in their own containers. To start Docker Compose with these profiles, you will execute this command:

```shell
docker compose --profile opentelemetry --profile prometheus up
```

#### Monitoring Docker Compose

If you prefer to run the monitoring services separately, we've built a Docker Compose that runs just those services and doesn't include FusionAuth itself. To download these Docker Compose files, use these URLs:

* https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/monitoring/docker-compose.yml
* https://raw.githubusercontent.com/FusionAuth/fusionauth-containers/main/docker/fusionauth/monitoring/.env

Make sure you download these files into the `monitoring` directory we created above. These files reference the configuration files in the same directory as the Docker Compose files, so make sure everything is in one place. This Docker Compose can be run using this command:
