...
- Under "Use case", choose "EC2", and then click <Breadcrumb>Next</Breadcrumb>.
...
- Select "Application running outside AWS" and click <Breadcrumb>Next</Breadcrumb>.
...
In your AWS console, navigate to <Breadcrumb>CloudWatch -> Dashboards</Breadcrumb>. Add a new dashboard, name it `CloudWatchLogger`, and click <Breadcrumb>Create dashboard</Breadcrumb>.
...
To create a virtual server, click <Breadcrumb>Launch new instance</Breadcrumb>. Give the server a <Breadcrumb>Name</Breadcrumb> and select "Amazon Linux" as the operating system. You can use the default "Architecture". For the instance type, select `t3.medium`, which is the lowest memory allocation that FusionAuth Docker will successfully start on.

![Create EC2 Instance-1](/img/docs/operate/secure-and-monitor/cloudwatch/create-ecs-instance-settings-1.png)

Next, generate a key pair to access the server via SSH. Click <Breadcrumb>Create new key pair</Breadcrumb>. Enter a name for the keypair, choose "RSA" as the keypair type, and `.pem` as the file format. Click <Breadcrumb>Create key pair</Breadcrumb>.
...
Click <Breadcrumb>Edit inbound rules</Breadcrumb>. Select `Custom TCP` as the <Breadcrumb>Type</Breadcrumb>. Set `9011` for the <Breadcrumb>Port range</Breadcrumb> and select `My IP` as the source for testing purposes. Click <Breadcrumb>Save rules</Breadcrumb>.
