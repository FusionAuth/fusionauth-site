...
tags: aws cloudwatch, monitoring, metrics, aws monitoring, docker, observability, alerting, amazon web services
---
...
- On the <Breadcrumb>Select trusted entity</Breadcrumb> step 1, choose `AWS service` on the **Trusted entity type**.
...
- Enter `CloudWatchAgentUser` as the name and click <Breadcrumb>Next</Breadcrumb>.
...
Click <Breadcrumb>Create dashboard</Breadcrumb>. Give the dashboard a **Name**, for example, `FusionAuthDashboard`, and click <Breadcrumb>Create dashboard</Breadcrumb>.
...
Click the `FusionAuth` namespace, then click **CPU**. Select all the CPU options and click <Breadcrumb>Create widget</Breadcrumb>.
...
- On the top right of the page, click the <IconButton icon="add"/> button to add a new API key.
...
- After saving the API key, click the red lock <IconButton icon="lock"/> next to the key to reveal and copy the value of the key. Store this key, as you will need it later.
...
All the FusionAuth APIs providing event data are documented [here](/docs/apis), and the login records API is documented [here](/docs/apis/login/export#request).
...
Select `Logs` for **Data type** and keep the defaults for the remaining options. Click <Breadcrumb>Next</Breadcrumb>.

The code for the custom logging service specified a `log_group="FusionAuth-CustomLogExporter` log group. On the next screen, click <Breadcrumb>Browse log groups</Breadcrumb> and select `FusionAuth-CustomLogExporter`. In the query box, delete the contents and enter `stats count() by bin(30s)`.

Click <Breadcrumb>Run query</Breadcrumb>. For **Visualization**, choose `Graph type:Bar`.
...
Click <Breadcrumb>Edit inbound rules</Breadcrumb>. Select `Custom TCP` as the **Type**. Set `9011` for the **Port range** and select `My IP` as the source for testing purposes. Click <Breadcrumb>Save rules</Breadcrumb>.
