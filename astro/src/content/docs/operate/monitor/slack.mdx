...
- Right-click the `fa-alert` channel and click <Breadcrumb>View channel details</Breadcrumb>. Record the <InlineField>Channel ID</InlineField> value displayed at the bottom of the <Breadcrumb>About</Breadcrumb> tab for later use.
...
- Change the <InlineField>name</InlineField> and <InlineField>display_name</InlineField> to <InlineFieldValue>fabot</InlineFieldValue>.
...
- Check that your bot has permissions for <InlineFieldValue>chat:write</InlineFieldValue> and <InlineFieldValue>chat:write.public</InlineFieldValue> in the <InlineField>Scopes</InlineField> section.
