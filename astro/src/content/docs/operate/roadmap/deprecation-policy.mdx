...
* `1.x` to `2.x` is a major release.
* `1.49.0` to `1.50.0` is a minor/point release.
...
* The deprecation is noted in the release notes. The feature is also marked as deprecated in the documentation.
* In any major release after the behavior is deprecated, the feature and corresponding documentation are removed. When the feature is removed, it is noted in the release notes. The documentation for the feature is also removed.
...
Regardless of when a feature was added, if the team finds a large risk to our customers, we may delete it without warning. In this case, the team will notify the [security announcement email list](/docs/operate/secure).
