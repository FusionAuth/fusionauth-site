...
tags: cors, cross-origin resource sharing, same-origin policy, spa, single page application, http headers, security, cors configuration
---
...
<APIBlock>
  ...
  <APIField name="Include credentials in CORS requests">
  ...
  <APIField name="Log blocked CORS requests" since="1.25.0">
    Creates event log entries to help debug requests blocked by the CORS filter. Check <Breadcrumb>System -> Event Log</Breadcrumb>. When enabled, a Debug event log will be created when the FusionAuth CORS filter returns a `403` detailing the reason for the response to assist you in configuration.
  </APIField>
</APIBlock>
