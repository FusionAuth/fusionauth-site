...
In the FusionAuth admin UI, edit a tenant and navigate to the <Breadcrumb>Security</Breadcrumb> tab. Go to <InlineField>Captcha Settings</InlineField> and toggle on <InlineField>Enabled</InlineField>. Enter your chosen reCAPTCHA version and the keys from Google. Leave the threat score at half and adjust it later if necessary. Save the changes.
