...
tags: advanced threat detection, threat detection, security, ip blocking, suspicious activity, bot detection, security monitoring, enterprise
---
...
The Advanced Threat Detection feature requires increased RAM on servers running the FusionAuth application, as documented in the [System Requirements](/docs/get-started/download-and-install/reference/system-requirements#compute-ram).
</Aside>
...
- Passwordless or one-time password login.
- Send or resend registration verification.
- Send two-factor code.
- Send or resend phone number verification.

![The rate limiting configuration screen.](/img/docs/operate/secure-and-monitor/rate-limit-settings.png)
...
Rate limiting is also utilized for cases before a user is created, like pre-verification. In this case, rate limiting occurs by identity value (email address or phone number).
...
![How rate limiting appears to the user.](/img/docs/operate/secure-and-monitor/user_login_failed.png)
...
For example, you can browse to <Breadcrumb>Customizations -> Themes</Breadcrumb> and edit any of the available Themes, such as the default FusionAuth theme. In the <Breadcrumb>Messages</Breadcrumb> tab, you can change messages shown to users. You could change the following message.
...
- Click <Breadcrumb>Switch to create a classic key</Breadcrumb> to ensure that you are not creating an Enterprise key.
- Enter your domain name, or `localhost` if you are testing locally.
- Choose v2 or v3.

The form should look like the image below.

![Create a CAPTCHA in Google](/img/docs/operate/secure-and-monitor/create_captcha.png)
...
In the FusionAuth admin UI, edit a tenant and navigate to the <Breadcrumb>Security</Breadcrumb> tab. Go to <Breadcrumb>CAPTCHA</Breadcrumb> and toggle on <Breadcrumb>Enable CAPTCHA</Breadcrumb>. Enter your chosen reCAPTCHA version and the keys from Google. Leave the threat score at half and adjust it later if necessary. Save the changes.

![Enter CAPTCHA details in FusionAuth](/img/docs/operate/secure-and-monitor/enter_captcha_details.png)
...
![Login with reCAPTCHA](/img/docs/operate/secure-and-monitor/captcha_login1.png)

![Login with reCAPTCHA](/img/docs/operate/secure-and-monitor/captcha_login2.png)
...
![hCaptcha create new site](/img/docs/operate/secure-and-monitor/hcaptcha_create_site.png)
...
Now browse to FusionAuth at http://check.example.com:9011/admin. Edit a tenant, and under <Breadcrumb>CAPTCHA</Breadcrumb>, toggle <Breadcrumb>Enable CAPTCHA</Breadcrumb> to on. Choose hCaptcha and enter your keys from the site. Leave the threat score at half and adjust it later if necessary. Save the changes.
...
![Login with hCaptcha](/img/docs/operate/secure-and-monitor/hcaptcha_login1.png)

![Login with hCaptcha](/img/docs/operate/secure-and-monitor/hcaptcha_login2.png)

## Location Aware Security

FusionAuth collects location information for requests. We use this information to secure user accounts in the following configurable ways:

* When a suspicious login events occurs, FusionAuth can emit a webhook event or send a message to the user directly.
...
![The IP access control list configuration screen.](/img/docs/operate/secure-and-monitor/ip-acl-list.png)
...
To apply the ACL to an application, browse to <Breadcrumb>Applications</Breadcrumb> and edit the application. On the <Breadcrumb>Security</Breadcrumb> tab under **Access control lists settings**, select the ACL.
...
You can restrict access on a tenant instead of an application in the same way: Edit the <Breadcrumb>Security</Breadcrumb> tab of the tenant. The ACL restriction will be applied to every application and user belonging to that tenant.

![Applying an IP ACL to an application.](/img/docs/operate/secure-and-monitor/ip-acl-application.png)
...
To only allow administrators in your company to make API calls to your application or restrict an API key to IP addresses in your building, edit the API key security in <Breadcrumb>Settings -> API Keys</Breadcrumb>.

![Applying an IP ACL to an API key.](/img/docs/operate/secure-and-monitor/ip-acl-api-key.png)
...
![Blocking consumer facing domains from registering.](/img/docs/operate/secure-and-monitor/block-domains.png)
...
- Email update — When a user's email address changes. This email is sent to the new and old addresses.
...
- Suspicious login — When a login is flagged by FusionAuth as suspicious. FusionAuth fires this event whenever any enabled [risk signals](/docs/lifecycle/authenticate-users/risk-signals) return a `HIGH` value.
...
![List of email templates.](/img/docs/operate/secure-and-monitor/security-emails-list.png)
...
![What getting a security email looks like](/img/docs/operate/secure-and-monitor/security_email.png)
...
If you want to see an example webhook, the easiest way is to browse to https://public.requestbin.com/r and paste the random link they give you, such as `https://enxy8i2pfzcy9.x.pipedream.net`, into the **URL** of a new webhook you create on the <Breadcrumb>Settings -> Webhooks</Breadcrumb> page.
