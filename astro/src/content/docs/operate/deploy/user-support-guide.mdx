...
<Aside type="caution">
Do not combine `mfa_deleter` with roles that can modify tenant configuration (for example, `tenant_manager` or `admin`). A user who can both remove MFA methods and change tenant notification templates can weaken these safeguards.
</Aside>
