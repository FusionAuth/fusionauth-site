...
You can script your changes using the API and apply those scripts to different environments. You can either use the API directly or a [client library](/docs/v1/tech/client-libraries/) in one of the supported programming languages.
...
You can manually configure FusionAuth via the administrative user interface. Changes made in this manner are recorded in the Audit Log, which you may view by navigating the <strong>System -> Audit Log</strong>.
