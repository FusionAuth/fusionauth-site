...
tags: proxy configuration, reverse proxy, nginx, apache, http proxy, load balancer, network configuration, fusionauth proxy
---
...
```mermaid title="A typical proxy configuration."
sequenceDiagram
...
```mermaid title="A proxy configuration with multiple proxies."
sequenceDiagram
...
If you are running [multiple tenants](/docs/get-started/core-concepts/types/tenants) in FusionAuth, a proxy can be useful to add the tenant Id to all requests for a given domain or path. Clients use the domain without needing to know or care about the tenant they are interacting with.
...
If you are using a proxy with a FusionAuth Cloud instance, see our [Using a Proxy with FusionAuth Cloud](/docs/cloud/operate/configure-proxy) documentation for more Cloud-specific requirements.
