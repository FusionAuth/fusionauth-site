...
* The [FusionAuth 5 minute setup guide](/docs/v1/tech/5-minute-setup-guide) - get a FusionAuth instance running and integrated with an Express application. If you want to use a different technology, please check out [the quickstarts](/docs/quickstarts).
...
First, create an API key as described in [Managing API Keys](/docs/apis/authentication#managing-api-keys). Create a superuser key byt not selecting any endpoint methods for the key. A super user API key has access to all API endpoints. If you want to limit the operations which can be performed by Terraform, limit the API key to specific endpoints. For this document, a super user API key is assumed.
...
Above, the `productionApiKey` variable is added to the `apiKeys` section of the Kickstart. [Learn more](/docs/get-started/download-and-install/development/kickstart#the-api-key).
...
* Set the `fusionauth_host` variable to the the hostname of the FusionAuth instance you want to manage. Make sure this value is the full URL including the protocol of your FusionAuth instance. When using docker, this will typically be `http://localhost:9011`.
...
Once you initialize the Terraform working directory, review different initial strategies to handle the import of default FusionAuth configuration.
...
Before you create, update, and remove resourdces with Terraform, let's examine the [Import](https://developer.hashicorp.com/terraform/language/import) and [Data Source](https://developer.hashicorp.com/terraform/language/data-sources) functionality.

There are [FusionAuth default configuration elements](https://fusionauth.io/docs/get-started/core-concepts/limitations#default-configuration) present in every FusionAuth instance. If you want to manage changes to these elements via Terraform, you must tell Terraform about them. Or, if you have existing FusionAuth configuration you want to manage via Terraform, you will need to import it.

<Aside type="tip">
The most consistent method for managing FusionAuth is to create all resources through Terraform. However, you must use either `Import` or `Data Source` to handle default or exsting FusionAuth configuration.
</Aside>

The FusionAuth default tenant and default Application are two configuration elements that you will almost certainly want to manage via Terraform. There are others outlined in the [FusionAuth default configuration elements](https://fusionauth.io/docs/get-started/core-concepts/limitations#default-configuration) that can be managed using the same methods described below.
...
<Aside type="caution">
Once imported, Terraform tracks the tenant in your state file. You can then manage the imported resource in the same way as others.
...
Instead of importing a resource, you can a `Data Source`. The list of supported FusionAuth data sources is in the [Terraform Provider documentation](https://registry.terraform.io/providers/fusionauth/fusionauth/latest/docs).

Data sources are useful if you choose to manage the default tenant and FusionAuth application outside of Terraform, either manually or via a script using the [client libraries](/docs/v1/tech/client-libraries/), but you still need to reference the tenant or application. Examples of this include:
...
* An application living in that new tenant, also called `forum`.
...
* Set the `fusionauth_api_key` variable to the value of the previously created API key.
* Set the `fusionauth_host` variable to the the hostname of the FusionAuth instance you want to manage. Make sure this value is the full URL including the protocol of your FusionAuth instance. When using docker, this will typically be `http://localhost:9011`.
* The `fusionauth_default_tenant_id` variable should be the the UUID of the default tenant.

### The Terraform File

Next, update the the `main.tf` file.
...
Then, configure the `Forum` tenant; this tenant will hold your users and applications.
...
Terraform is great for configuring and tracking changes of relatively slow-changing aspects of systems. With FusionAuth, users typically self-register, so using Terraform to manage them doesn't make much sense. Instead, turn on [self-service registration](/docs/lifecycle/register-users/basic-registration-forms) or [migrate your users](/docs/lifecycle/migrate-users/).

## Terraform Update

Once a resource is managed by Terraform, you can change `main.tf` according to the [Terraform Provider Documentation](https://registry.terraform.io/providers/fusionauth/fusionauth/latest/docs/). You might decide you want to change the `time_to_live_in_seconds` value for your JWT configuration from `3600` to `1800` seconds.

You'd update the configuration element to look something like this:
...
After you're done with editing your Terraform file, run `terraform plan` to check the planned changes. Review and fix any errors. Terraform will show you what it will change.
...
After review, run `terraform apply`. If you want to know what already has been defined by Terraform but is not specified in your `.tf` files you can run `terraform show`.

The configuration file can get very large. If you want to show specific resources, list all resources with `terraform state list`. Then examine the resource state with `terraform state show <resource-name>`.
...
Here is an example with FusionAuth resources. Imagine a situation business application adds user driven data to an application. Suppose you update your FusionAuth application with a curl command and update the `data` attribute.
...
If you're only interested in creating and destroying resources you can also ignore all changes.
...
A good example of an element to apply `prevent_destroy` to would be an in-use tenant. Deleting a Tenant removes all the associated Applications, Groups, and Users. Below, notice that the `Forum` tenant can't be destroyed.
...
Update config management index page TODO

get provider infrom from CLI TODO
screenshot of provider info TODO
