...
tags: cluster setup, high availability, multi-node, horizontal scaling, load balancing, deployment, performance tuning, clustering
---
...
FusionAuth can be run in multiple architectures; see the [Server Layout](/docs/get-started/download-and-install/reference/server-layout) documentation for more.
...
* An Elasticsearch cluster, if using the [Elasticsearch search engine](/docs/get-started/core-concepts/types/users#user-search). 
...
<AvailableSince since="1.27">
  You can use a [Prometheus endpoint](/docs/operate/monitor/prometheus) to monitor your instances.
</AvailableSince>
...
See the [Monitoring documentation](/docs/operate/monitor) for more information.

### Log Files

<AvailableSince since="1.16.0-RC1"/>
...
See [the Troubleshooting documentation](/docs/operate/troubleshooting) for more information about logs.
