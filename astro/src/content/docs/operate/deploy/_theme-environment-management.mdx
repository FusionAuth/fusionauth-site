When moving themes from one environment to another, the theme logic and look and feel may be the same, but the assets may be different. FusionAuth does not resolve environment variables in the themes, or anything other than [what is documented](/docs/customize/look-and-feel/template-variables).
...
Another option is to use custom fields in the `tenant.data` field, since the `tenant` object will always be available. Then you can use the same theme templates but reference these variables for different hostnames.
