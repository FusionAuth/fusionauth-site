...
tags: upgrade fusionauth, version upgrade, migration, system update, deployment, rpm, deb package, docker upgrade
---
...
Older FusionAuth client libraries should work properly with newer FusionAuth versions upon upgrading. However, once you use new features on that instance, FusionAuth might return enumeration values that older clients do not understand. This can cause client operations to fail.
...
If you are running a FusionAuth Cloud deployment, you can find instructions for rolling back an upgrade here: [Rolling Back From a Problematic Upgrade](/docs/cloud/upgrade-a-deployment#roll-back-to-a-backup).
...
To upgrade your FusionAuth Cloud instance, see the [Cloud Installation Guide](/docs/cloud/upgrade-a-deployment).
...
FusionAuth is available in a ZIP package for macOS, Linux, and Windows. If you are using the ZIP package, please use this guide to update an existing instance of FusionAuth. Find the ZIP packages at the FusionAuth [Downloads](/docs/get-started/download-and-install/docker) page.
