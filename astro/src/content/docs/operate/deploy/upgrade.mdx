...
import BlueGreenDeploymentDiagram from 'src/diagrams/docs/operate/deploy/_blue-green-deployment.astro';
import ClientLibraryVersioning from 'src/content/docs/operate/deploy/_client-library-versioning.mdx';
import RollingUpgradeDiagram from 'src/diagrams/docs/operate/deploy/_rolling-upgrade.astro';
...
* Monitoring the [release notes](/docs/v1/tech/release-notes).
* Joining the [FusionAuth community](https://fusionauth.io/community) forum and/or Slack.
...
Read the [release notes](/docs/v1/tech/release-notes). Release notes help you understand how a version update affects your systems. 
...
For each of the options below, FusionAuth is running any database migrations. However, you can also stop all nodes and run the database migration SQL statements yourself, rather than relying on FusionAuth to run them. See <ScrollRef target="Downtime and Database Migrations" /> for more.
...
**A:** We publish release notes for every release. You can find them here: [Release Notes](/docs/v1/tech/release-notes). There are a variety of ways to be [notified of releases](/docs/v1/tech/admin-guide#release-notifications/releases), including an email list and RSS feed.
...
FusionAuth is available in a ZIP package for macOS, Linux, and Windows. If you are using the ZIP package, please use this guide to update an existing instance of FusionAuth. Find the ZIP packages at the FusionAuth [Downloads](https://fusionauth.io/download) page.
