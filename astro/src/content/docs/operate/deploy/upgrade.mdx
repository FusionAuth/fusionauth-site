...
Older FusionAuth client libraries should work properly with newer FusionAuth versions upon upgrading. However, once new features are used on that instance, some enumeration values may be returned from FusionAuth that older clients do not understand, which will cause that client operation to fail.

Note that newer FusionAuth client libraries are *not* compatible with older FusionAuth versions. If a release violates this rule, the [release notes](/docs/release-notes) will provide more information.
