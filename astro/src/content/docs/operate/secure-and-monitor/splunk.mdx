...
- [Create A Splunk Account](#create-a-splunk-account)
- [Test Your Splunk Access Token](#test-your-splunk-access-token)
...
Let's consider the number of user logins every 10 seconds as an example. If you have only a few users, you could monitor the number every hour or even every day instead.
...
You could send the number of logins as a counter. In this case, it would function the same as a gauge.
...
TODO fix json in script. use -v with curl to see why it doesn't work
then figure out how to readd otel dashboard, mention + at top right. try rerun otel in docker compse again
discuss metrics vs detailed data in splunk
