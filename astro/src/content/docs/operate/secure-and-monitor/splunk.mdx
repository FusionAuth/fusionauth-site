...
SEE TODO SECTION AT BOTTOM OF THIS ARTICLE
...
@Tatenda, change the links below and clarify to readers if you don't actually need the first one, I think I took unnecessary steps.
...
Splunk has two sets of documentation, the [primary](https://docs.splunk.com/observability/en) and the [developer](https://dev.splunk.com/observability/docs). You need to read only the former to use Splunk with FusionAuth.
...
TODO
Write a node.js service or cronjob with curl or something that gets metrics from fusionauth container and sends them to splunk
...
[Configuration](https://docs.splunk.com/observability/en/gdi/opentelemetry/components/otlp-receiver.html#sample-configurations) file for OpenTelemetry container to prepare receivers to receive.

[Here](https://github.com/signalfx/splunk-otel-collector/blob/main/cmd/otelcol/config/collector/gateway_config.yaml) is a sample gateway configuration file which your Docker compose file configuration section is based on.

Unfortunately the config file says that the OpenTelemetry container supports only tracing, meaning that it can't send metrics or spans. Thus Splunk's support of OpenTelemetry is basically useless.

This is added as an [environment variable](https://docs.splunk.com/observability/en/gdi/opentelemetry/environment-variables.html#collector-env-var) into your Docker compose file.

You are using the Splunk Collector in [gateway mode (data forwarding), not agent mode (host monitoring)](https://docs.splunk.com/observability/en/gdi/opentelemetry/opentelemetry.html#collector-intro-deploy).
...
TODO try connect to splunk collector from fusionauth docker container manually.


### OpenTelemetry Dashboard

https://app.us1.signalfx.com/#/dashboards -> OpenTelemetry Collector -> Time to past day
