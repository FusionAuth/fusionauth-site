...
import Diagram1 from 'splunk_diagram1.astro';
import Diagram2 from 'splunk_diagram2.astro';
import Diagram3 from 'splunk_diagram3.astro';
...
This script first updates Ubuntu to reinstall basic software that FusionAuth removed to save space. The script then downloads the OpenTelemetry Java app, then edits `start.sh`, which is the command run when the container starts, to start FusionAuth using OpenTelemetry. The edit command takes the head of the `start.sh` file (all lines except the last), and writes the new command to the end of the file.

By default, the OpenTelemetry Java agent sends data to the OpenTelemetry collector at http://localhost:4317. The script changed it to send data to the Splunk container, at http://otel:4318. (Splunk uses 4317 for RPC, not HTTP).
...
If no data is sent, correct your secret key and realm in the compose file.
...
Unfortunately FusionAuth has too [many metrics](https://fusionauth.io/docs/operate/secure-and-monitor/monitor#metrics) to discuss in this article. You will need to decide which are important for you to monitor by reading the documentation. You can also create a FusionAuth webhook to notify a local Docker container to send the event to Splunk.
...
Let's get the login records every ten seconds and them to Splunk. All the FusionAuth APIs that give you event data are documented [here](https://fusionauth.io/docs/apis). The login API is documented [here](https://fusionauth.io/docs/apis/login#request-6). Note that the documentation says the date format is the standard Java type, but it actually doesn't support the constants like `ISO_LOCAL_DATE_TIME`. You have to manually enter the format string you want.
...
You can other FusionAuth API calls to `app.sh` to get other metrics to send to Splunk in the same way.

You now how to send Java runtime information to Splunk through OpenTelemetry, how to send any custom data to Splunk with a custom script, and how to see both metric types on a dashboard in the Splunk web interface.
