...
Splunk has two sets of documentation, the [primary](https://docs.splunk.com/observability/en) and the [developer](https://dev.splunk.com/observability/docs). You need to read only the primary to use Splunk with FusionAuth.
...
Create a file called `metricDockerfile.yaml`. Insert the content below, replacing your access token and realm.
...
mkdir /app
echo "#!/bin/sh"                                              >  /app/app.sh
echo "curl --request POST \\"                                 >> /app/app.sh
echo "--header \"Content-Type: application/json\" \\"         >> /app/app.sh
echo "--header \"X-SF-TOKEN: YOURTOKEN\" \\"                  >> /app/app.sh
echo "--data \\"                                              >> /app/app.sh
echo "'{"                                                     >> /app/app.sh
echo "    \"gauge\": ["                                       >> /app/app.sh
echo "        {"                                              >> /app/app.sh
echo "            \"metric\": \"memory.docker\","             >> /app/app.sh
echo "            \"dimensions\": { \"host\": \"server2\" }," >> /app/app.sh
echo "            \"value\": 42"                              >> /app/app.sh
echo "        }"                                              >> /app/app.sh
echo "    ]"                                                  >> /app/app.sh
echo "}' \\"                                                  >> /app/app.sh
echo "https://ingest.YOURREALM.signalfx.com/v2/datapoint"     >> /app/app.sh
chmod +x /app/app.sh
...
docker run --name metricbox metricimage
docker run -it --name javabox -v javaimage
```
