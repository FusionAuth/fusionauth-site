...
FusionAuth has too [many metrics](/docs/operate/secure-and-monitor/monitor#metrics) to discuss in this article. You will need to decide which are important for you to monitor by reading the documentation. You can also create a FusionAuth webhook to notify a local Docker container to send the event to Splunk.
