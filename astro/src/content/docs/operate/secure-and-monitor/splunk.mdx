...
After starting the FusionAuth container with `docker compose up`, open a terminal and attach to the container as the `root` use (the default user is `fusionauth`). Replace `fa` with your container name.

```sh
docker exec -u root -it fa bash
```
