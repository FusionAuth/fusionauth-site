...
Browse to https://app.us1.signalfx.com/#/dashboards. Choose the OpenTelemetry Collector dashboard from the <strong>Built-in dashboard groups</strong>. (This dashboard will appear as an option only if you have set up the Docker collector above). At the top, set time to past day to ensure that you see all possible values while testing.
