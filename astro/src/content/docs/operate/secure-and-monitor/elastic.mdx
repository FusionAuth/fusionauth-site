...
- TODO NO Give it a name and enable "Instance access". Select the `Admin` role for the `fa` instance.
- Click "Create API Key".
- Save the key securely for later use. Never to commit the key to git, which can expose it on the Internet.

To test that you have the correct details, open a terminal and run the following command, after setting your API key and URL. The API key is the value of the `encoded` field from the JSON in Elastic, not the `api_key` field.

```sh
export elasticKey="WGdLQ=="
export elasticUrl="https://c301.us-central1.gcp.cloud.es.io:443"
curl "${elasticUrl}/index_name" -H "Authorization: ApiKey "${elasticKey}"" -H "Content-Type: application/json"
```

The result should be `{"error":{"root_cause":[{"type":"index_not_found_exception","reason":"no such index [index_name]` because no ElasticSearch index exists yet. You should not receive an authentication or 404 error. If you do, please check your URL and API key settings.
