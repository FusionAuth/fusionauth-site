...
[Elastic Observability Cloud](https://docs.splunk.com/observability/en/get-started/service-description.html) is a Software as a Service (SaaS) solution for infrastructure monitoring (Elastic IM), application performance monitoring (Elastic APM), real user monitoring (Elastic RUM), and synthetic monitoring (Elastic Synthetic Monitoring). Elastic Observability Cloud also provides a direct integration with logs ingested in Elastic Cloud Platform and Elastic Enterprise through Log Observer Connect.

This guide explains how to connect FusionAuth to Elastic through Docker with the OpenTelemetry Collector using the Elastic API, as well as which FusionAuth metrics are useful in Elastic and how to create a simple dashboard to show them.
...
When reading Elastic documentation, such as their [getting started guide](https://www.elastic.co/guide/en/observability/current/logs-metrics-get-started.html), which tells you "you need an Elastic Stack deployment", remember that that does not mean you need to deploy the stack alongside your application or FusionAuth instance. Their guide means that you need to have the Elastic Stack installed *somewhere*, which is probably going to be on their Elastic Cloud.

## Elastic Options

- use agent in same container as app
- use agent in separate container and allow access to docker socket
- use opentelemetry
...
(From https://www.elastic.co/guide/en/starting-with-the-elasticsearch-platform-and-its-solutions/8.14/getting-started-observability.html#observability-gs-prereqs)

[Elastic Agent](https://www.elastic.co/guide/en/fleet/current/fleet-overview.html) is an application that you install on your server that monitors it and send information to Elastic Cloud to display on the dashboard. Fleet is todo. ElasticSearch is todo. Kibana is todo.

[Component diagram](https://www.elastic.co/guide/en/fleet/current/images/agent-architecture.png)

- Browse to home page of your Elastic Cloud web interface and click "Add integrations".
<img src='../../../../../public/img/docs/operate/secure-and-monitor/elastic/elastic1.png' alt='' />
- Search for and select `system`.
<img src='../../../../../public/img/docs/operate/secure-and-monitor/elastic/elastic2.png' alt='' />
- Click "Add system".
- At the bottom of the page, click "Install Elastic Agent".
- Copy the text to install the "Linux Tar" version of the agent, which will be similar to the commands below, but with different parameters. Remove `sudo` from the last line.
  ```sh
  ...
  sudo ./elastic-agent install --url=https://905e427463b64.fleet.us-central1.gcp.cloud.es.io:443 --enrollment-token=YOURTOKEN
  ```
- Enrollment Token: This token is a security measure provided by the Fleet server when you create an agent policy in the Fleet UI of Kibana. It contains all the necessary information encoded within it to facilitate the secure registration of an Elastic Agent with the Fleet server. This process avoids the need for exposing an API key directly in the command or configuration files.Purpose of the Token: The enrollment token simplifies the process of setting up and securing communications between the Elastic Agent and the Fleet server. It includes permissions and configuration details required by the agent to operate correctly and securely.
...
- <img src='../../../../../public/img/docs/operate/secure-and-monitor/elastic/elastic3.png' alt='' />
