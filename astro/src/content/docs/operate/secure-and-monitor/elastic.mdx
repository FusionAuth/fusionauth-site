...
- [Monitor the FusionAuth Application With Elastic](#monitor-the-fusionauth-application-with-elastic)
- [Send custom metrics through api](#send-custom-metrics-through-api)
...
- Also create a `.env` file with the default content below.
  ...
  ```
- Now run the FusionAuth with `docker compose up`.
...
Well done, you have successfully installed a monitoring agent for your FusionAuth machine and can now monitor it online.
...
If you run EA in a separate container you avoid this problem, but you need to have full control over the machine running Docker in order to give EA access to restricted Docker data. This may not be possible on some cloud hosts.
...
## Monitor the FusionAuth Application With Elastic

You've learnt how to monitor the FusionAuth container now in two different ways. Since a Docker container will exit if the process it runs exits, this monitoring might be enough for you. But if you want to monitor details about the FA app itself, there are various ways to do it:
...
## Send custom metrics through api
...
Previously, this guide showed you how to use a new Docker container to run an OpenTelemetry Collector to receive data from FusionAuth.
