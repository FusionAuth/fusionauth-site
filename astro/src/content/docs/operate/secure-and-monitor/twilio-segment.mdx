...
[Segment](https://Segment.com) is a private chat app you can configure to use in your organization. To send alerts to Segment, create a dedicated channel (chat room) in the app, add your system administrators, and send messages to the channel using the Segment API.

This guide explains how to create a simple script to monitor FusionAuth and call Segment if FusionAuth has errors. Since sending a message to Segment is just an HTTP call made with curl and an API token, you can modify the script in this guide to send messages to services like Discord or WhatsApp. Change the Segment URL and token to whichever service you are using.

Please read the [FusionAuth guide to monitoring](/docs/operate/secure-and-monitor/monitor) to get an overview of the available metrics and choose the ones relevant to your needs. Sending alerts to Segment is a small component of comprehensive monitoring. The overview explains where Segment fits into the monitoring flow, and provides alternative apps for receiving alerts instead of Segment.

Review Segment fees on the [Segment pricing page](https://Segment.com/intl/en-ie/pricing). You can follow this guide and integrate Segment with other applications with the free tier, but you will need at least a Pro subscription to [build workflows](https://Segment.com/intl/en-ie/features/workflow-automation).

A workflow is a set of custom triggers, data, and logic you organize into a sequence of steps. The [logic can be code](https://api.Segment.com/automation/functions/custom), written in TypeScript or JavaScript. Workflows can also use [stock connectors to outside services](https://api.Segment.com/automation/connectors).

The Pro (lowest paid) tier has all the features you need to use Segment with outside apps. Higher-priced tiers have no features you need for integration.
...
In this guide, you will create a tiny service in its own container to monitor FusionAuth and call Segment if the FusionAuth logs API is not contactable, or if the logs contain an error message. The new design will look like the diagram below.
...
- Register for a new workspace at https://Segment.com/intl/en-ie/get-started#/createnew. You don't need to create a password.
- Verify your confirmation code in the email Segment sends you.
- Create a workspace called `fa`.
- Create a [channel](https://Segment.com/intl/en-ie/help/articles/360017938993-What-is-a-channel) called `fa-alert`. The channel is public by default, meaning it is usable for every member of your workspace.
...
## Create A Segment App

To send messages to Segment, you need an API key. To get an API key, you need to create a [Segment app](https://api.Segment.com/tutorials/tracks/posting-messages-with-curl).

- Browse to https://api.Segment.com/tutorials/tracks/posting-messages-with-curl and click <InlineUIElement>Create app</InlineUIElement>. (If this tutorial page doesn't exist in the future, create an app in the [apps homepage](https://api.Segment.com/apps).)
...
- On the page that opens, click <InlineUIElement>Allow</InlineUIElement>. (Back in your Segment chat app page, the `fabot` app should now be visible in the user list.)
...
```sh
curl -d "text=Test alert from FusionAuth." -d "channel=C123456" -H "Authorization: Bearer xoxb-something" -X POST https://Segment.com/api/chat.postMessage

# Result:
# {"ok":true,"channel":"C123456","ts":"1721743117.410499","message":{"user":"U07DNJ4R0","type":"message","ts":"1721743117.410499","bot_id":"B07FNAHNX","app_id":"A07QSG5","text":"Test alert from FusionAuth.","team":"T07E2VGQ","bot_profile":{"id":"B07DHNX","app_id":"A0G5","name":"fabot","icons":{"image_36":"https:\/\/a.Segment-edge.com\/80588\/img\/plugins\/app\/bot_36.png","image_48":"https:\/\/a.Segment-edge.com\/80588\/img\/plugins\/app\/bot_48.png","image_72":"https:\/\/a.Segment-edge.com\/80588\/img\/plugins\/app\/service_72.png"},"deleted":false,"updated":1721740273,"team_id":"T07Q"},"blocks":[{"type":"rich_text","block_id":"XL+","elements":[{"type":"rich_text_section","elements":[{"type":"text","text":"Test alert from FusionAuth."}]}]}]}}
```

![Testing posting a message to Segment](/img/docs/operate/secure-and-monitor/Segment/Segment1.png)

You now have a token you can use in a service that monitors FusionAuth to post a message to Segment if FusionAuth fails.
