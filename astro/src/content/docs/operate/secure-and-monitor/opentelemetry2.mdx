...
First you need to update your `collectorConfig.yml` file to allow receiving `otlp` (OT Protocol) metrics on port `4318` at the top, and add OTLP to the list of receivers at the bottom. The update file content is shown below.
...
`SECTION 2` opens the unzipped CSV file with `tail`, loops through it with `while`, and converts every line to a JSON object of the time the user logged in, and their user Id. Still in the loop, the script uses `curl` to post the JSON data off to the OT Collector on the `otelUrl`.
...
If the OT Collector and custom script worked correctly, when you browse to Prometheus you should now be able to see the metric `login_event` in the graph.
...
OT is a tool that doesn't stand alone. Read the FusionAuth guides to [Prometheus](./prometheus.mdx) and [Elastic](./elastic.mdx) to choose a complete system that provides a dashboard with alerts to use in conjunction with OT.
