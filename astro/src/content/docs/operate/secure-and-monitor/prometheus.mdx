...
This guide explains how to monitor FusionAuth logs and metrics with the open-source tools [Prometheus](https://prometheus.io/docs/introduction/overview), [Loki](https://grafana.com/docs/loki), and [Grafana](https://grafana.com/grafana), and send you alerts when problems occur.
...
The final monitoring component you might want use is storing logs. The Grafana tool for this is [Loki](https://grafana.com/docs/loki). Loki indexes only the metadata of a log line (its time and attributes such as the server that sent it) and not its content. This is unlike Elasticsearch or OpenSearch, which indexes the log content too. Loki therefore uses far less disk space than OpenSearch but is not quickly searchable. The no-indexing choice Loki made is better for most applications, where you need only to monitor logs for errors and to store logs for auditing purposes.
...
Loki is only a log store, and will not fetch logs itself. Tools to send Loki logs include: Promtail (the original sending tool), OpenTelemetry, and Alloy (a new OpenTelemetry-compliant tool from Grafana). For more options, see the [documentation](https://grafana.com/docs/loki/latest/send-data). In this guide, you use Promtail.
...
To start using Loki, add the services below to your `docker-compose.yml` file. You are now using Grafana images because Ubuntu has none for Promtail.
...
The `faLoki` port 3100 is open so that Grafana can query it in dashboards. The `prometheusLoki` volume persists log storage across container restarts. The `prometheusLokiConfig.yml` volume allow you to adjust Loki settings. Unlike the Ubuntu images, Grafana images don't use the root user. This means that the user in the container won't have permissions to create files on the Docker host machine. In production, you can inspect the running container to see what user it has, then create the `prometheusLoki` directory and assign its owner as the container user Id. But for this prototype it's faster to set the container user to `user: root` instead, so it can directly write to the shared volume. `target=all` runs the Loki container in monolithic mode.

The `faPromtail` service waits for Loki to start, with `depends_on: faLoki`. It has volumes for a configuration file, and to have access to the log files saved by Docker, and the Docker socket file.

You should also change the `fa` service to make FusionAuth wait for `Promtail` to run before FusionAuth starts. OtherUse the code below.
...
You can leave the `prometheusLokiConfig.yml` volume in the configuration commented out. The default values are fine. But if you want to use Loki with Alertmanager, you should create the file with the contents below (where only the last line is different from the default. Below, the Alertmanager URL now points to the Docker service for the `ruler` ([rules manager](https://grafana.com/docs/loki/latest/alert)).
...
The `prometheusPromtailConfig.yml` file important. It controls which containers Promtail will get logs for. Create the file and add the content below.
...
[`docker_sd_configs`](https://grafana.com/docs/loki/latest/send-data/promtail/configuration/#docker_sd_configs) is one way for Promtail to get logs (along with local file logs and Kubernetes). It follows the Prometheus [configuration format](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#docker_sd_config), which uses the Docker [container reference format](https://docs.docker.com/reference/api/engine/version/v1.40/#operation/ContainerList). Note that while your container and service name in the Docker process list might be `fa`, the name exposed in the Docker API is actually `/fa`. You can see the `/` used in the settings above. To see this is true in Docker, run `docker inspect fa`. You'll the container name is actually `"Name": "/fa"`.


Everything is ready. Run `docker compose up` to start all monitoring components. Browse to http://localhost:3100/ready to check Loki is up.
...
If you combine the Prometheus, AlertManager, Grafana, Loki, and Ntfy infrastructure shown in this guide, your architecture will look as follows.
