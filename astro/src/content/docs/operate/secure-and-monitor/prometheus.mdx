...
sudo cat /var/lib/docker/containers/7789a4bdb6a4a40b380f347830a7bc3106f86734578150dd843004a0ec276d2f/7789a4bdb6a4a40b380f347830a7bc3106f86734578150dd843004a0ec276d2f-json.log
...
```yml
  fa:
    image: fusionauth/fusionauth-app:1.51.2 # fusionauth/fusionauth-app:latest
    container_name: fa
    depends_on:
      faPromtail:
...
Logs are monitored for errors to trigger alerts with a component called [ruler](https://grafana.com/docs/loki/latest/alert).

`-target=all` for monolithic

loki doesn't come with promtail. need that or alloy.

grafana docker images have no docs. prefer ubuntu. ubuntu image doesn't run, prefer grafana image.
...
```yml
    ...
    user: root
```
is used because otherwise default user loki won't create folders on local machine in the new volume.
...
https://docs.docker.com/reference/api/engine/version/v1.40/#operation/ContainerList


docker inspect fa | grep fa
        "Name": "/fa",


browse to http://localhost:3100/ready to check loki is up

http://localhost:9091/connections/datasources
Add data source
Loki
URL - http://faLoki:3100
Save and test

Explore
{container="fa"}
{container="fa"} |~ "(ERROR|WARN)"
