...
Each feature is configurable on a tenant by tenant basis. You can do so via [the Tenant API](/docs/apis/tenants) or by browsing to <Breadcrumb>Tenants -> My Tenant -> Security</Breadcrumb>.
...
<RateLimitDiagram></RateLimitDiagram>
...
Note that even if you enable rate limiting, FusionAuth will still receive every request sent to it. An overload of requests will cause a denial of service attack. You need to prevent this attack at the network level of your system, not in FusionAuth itself.
...
You can configure emails for Tenants by browsing to <strong>Tenant -> Your Tenant -> Email -> Template Settings</strong> or for Applications by browsing to <strong>Applications -> Your Application -> Email -> Template Settings</strong>.
...
A webhook is an outbound HTTP request bearing a message to an endpoint. A webhook is used to inform an external system of some event in FusionAuth. The webhook/API terminology can be confusing. Note that most web applications, including FusionAuth, call a trigger to send data a "webhook", but when they receive data they call it an "API". So if you're looking for a destination for a FusionAuth webhook in an external system, you won't find it under the webhook documentation; you'll find it under [API documentation](/docs/apis/webhooks). This is why webhooks are sometimes known as "reverse APIs". However, some companies, like Slack in their documentation, also call incoming requests "incoming webhooks".
...
If you want to see an example webhook, the easiest way is to browse to https://webhook.site and paste the random link they give you, such as https://webhook.site/70a036f9-b2b2-4646-bf58-c64798eea20e, into the <InlineField>URL</InlineField> of a new webhook you create on the <Breadcrumb>Settings -> Webhooks</Breadcrumb> page.
...
You will see JSON like the following arriving in the logs of webhook.site.
