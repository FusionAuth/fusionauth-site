...
```mermaid
flowchart LR
    A(User attempts to log in) --> B{{Has user exceeded rate limit?}}
    B -- No --> C(User may log in)
    B -- Yes --> D(User may not log in)
```
...
You can configure emails for Tenants or Applications, by navigating to <strong>Tenant -> Your Tenant -> Email -> Template Settings</strong> or <strong>Applications -> Your Application -> Email -> Template Settings</strong>.
...
For a comprehensive explanation of using webhooks in FusionAuth, please read [the guide](/docs/extend/events-and-webhooks). If you want to call webhooks programmatically, you can do so using the [FusionAuth API](/docs/apis/webhooks).
...
In your tenant's webhooks tab, enable <InlineField>user.login.new-device</InlineField>. Open a new incognito browser window and log in to FusionAuth.
