...
You may want to restrict an application or API key to a certain IP address or range of IP addresses. With Advanced Threat Detection, you can do this. Create an IP address access control list (IP ACL):
...
In addition to restricting the website users to an IP range, you can also restrict an API key's use. This is useful if you are managing your FusionAuth configuration via a CI/CD system. You can create an API key to modify configurations, but limit that key to the IP address range of your CI/CD system, increasing security and lowering the risk of the API key being used incorrectly.

You might want only administrators in your company to make API calls to your application, and restrict an API key to IP addresses in your building.
