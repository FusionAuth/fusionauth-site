...
For example, you can browse to **Customizations -> Themes**. Edit any of the available Themes, such as the default FusionAuth. In the <InlineField>Messages</InlineField> tab you can change messages shown to users. You could change the following message.
...
You can also use CSS and JavaScript to provide more information to users. For more information, please see the [theme customisation documentation](/docs/customize/look-and-feel/).
...
In the FusionAuth admin UI, edit a tenant, and on the **Security** tab go to <InlineField>Captcha Settings</InlineField>, enable <InlineField>Enabled</InlineField>. Enter your chosen reCAPTCHA version and your keys from Google. Leave the threat score at half and adjust it later if necessary. Save the tenant.
...
Now browse to FusionAuth at http://check.example.com:9011/admin. Edit a tenant, and under the heading <InlineField>Captcha Settings</InlineField>, enable <InlineField>Enabled</InlineField>. Choose hCaptcha and enter your keys from the site. Leave the threat score at half and adjust it later if necessary. Save the tenant.
