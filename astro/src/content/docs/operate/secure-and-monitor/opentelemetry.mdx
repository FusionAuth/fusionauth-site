...
The tool you'll use in this guide is the OpenTelemetry Collector. It consists of two components:
- A **receiver** to monitor a container or application and produce metrics about it directly, or to be pushed metrics by another tool.
- An **exporter** to expose these metrics to other tools.

The Collector can also process the values it receives in some way before exporting them.

Since you don't have access to the source code of FusionAuth, there are four ways you can use OpenTelemetry to export metrics about FusionAuth:
...
- Run an OpenTelemetry exporter in its own Docker container on your host. Usually, the OpenTelemetry container needs to have administrative permissions to the Docker engine to get metrics about the status of the FusionAuth container. This will allow you to know if FusionAuth is up, and basic metrics about the health of the container, like CPU and RAM use. However, OpenTelemetry can read metrics in the older Prometheus format, which FusionAuth provides, so in this case you don't actually need to administrative permissions.
...
<Aside type="note">
If you're wondering how Prometheus can poll the OpenTelemetry Collector for metrics if the Collector doesn't store metrics, it's because the Collector does keep recent metrics it received in RAM. The Collector doesn't store them for long however, and does not persist metrics to disk.
</Aside>
...
This service says that the [Prometheus image](https://hub.docker.com/r/ubuntu/prometheus) will start after OpenTelemetry Collector, that you can browse to Prometheus on port `9090`, and that it will save its database and configuration file in persistent directories on your machine.

The OpenTelemetry image comes from https://hub.docker.com/r/otel/opentelemetry-collector. There is also a [contrib](https://hub.docker.com/r/otel/opentelemetry-collector-contrib) image you can use instead. The image is bigger, and contains more components that interoperate with other OpenTelemetry tools you might want to use.

The `ports` exposed in the configuration are `8889`, which is where Prometheus pulls metrics from OpenTelemetry Collector, and `4138`, which will be used in the next section for the Collector to receive custom metrics.
...
FusionAuth provides only some information on its Prometheus endpoint. If there is information you are looking for that you cannot find in Prometheus, such as details about user logins, you need to extract it yourself in a custom script. You'll do write a custom script in this section.

First you need to update your `collectorConfig.yml` file to allow receiving `otlp` (OpenTelemetry Protocol) metrics on port `4318` at the top, and add OTLP to the list of receivers at the bottom. The update file content is shown below.
...
All of `SECTION 1` above is just to download a file of user login records from FusionAuth. The URL `api/system/login-record/export` is called the timespan from sixty seconds ago until now. The records are downloaded as a ZIP file and extracted.

`SECTION 2` opens the unzipped CSV file with `tail`, loops through it with `while`, and converts every line to a JSON object of the time the user logged in, and their user Id. Still in the loop, the script uses `curl` to post the JSON data off to the OpenTelemetry Collector on the `otelUrl`.
...
Run everything with `docker compose up`, or just `docker compose up script` if the other containers are already running. Check in the terminal that there are no errors in the Docker logs. Log out of FusionAuth and back in, so that the custom script will retrieve a login metric for last sixty seconds.

If the OpenTelemetry Collector and custom script worked correctly, when you browse to Prometheus you should now be able to see the metric `login_event` in the graph.

Since the metrics are separated by user Id, you will see to many lines on the screen to be useful for monitoring. Use Prometheus queries like the following three to aggregate the metrics over time and group them across users.
...
If you cannot see the custom metrics, it's faster to debug the script locally on your machine without Docker. To do so, you need to make the script executable by running `chmod +x app.sh`. You also have to change the two container URLs to `localhost` at the top of the script:
