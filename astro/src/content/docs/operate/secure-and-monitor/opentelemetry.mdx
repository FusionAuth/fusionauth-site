...
- Run an OpenTelemetry exporter in its own Docker container on your host. Usually, the OpenTelemetry container needs to have administrative permissions to the Docker engine to get metrics about the  FusionAuth container status. This will allow you to know whether FusionAuth is up and to receive basic metrics about the health of the container, like CPU and RAM use. However, OpenTelemetry can read metrics in the older Prometheus format provided by FusionAuth, so in this case, you don't actually require administrative permissions.
