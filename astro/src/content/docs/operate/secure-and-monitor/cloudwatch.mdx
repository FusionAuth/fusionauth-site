...
- Under "use cases", choose "EC2",and then click <InlineUIElement>Next</InlineUIElement>.
...
- In the list of policies, select the check box next to `CloudWatchAgentServerPolicy`, `CloudWatchFullAccess`, and `CloudWatchFullAccessV2`. If necessary, use the search box to find the policies. 
- To use Systems Manager to install or configure the CloudWatch agent, select the box next to `AmazonSSMManagedInstanceCore`. This AWS-managed policy enables an instance to use the Systems Manager service core functionality. If necessary, use the search box to find the policy. This policy isn't required if you start and configure the agent only through the command line. 
- Click <InlineUIElement>Next</InlineUIElement> and then <InlineUIElement>Create user</InlineUIElement>.

Add the `PutRetentionPolicy` to the CloudWatch agent user.
...
- Click <InlineUIElement>AddPermission</InlineUIElement> and choose `inline policy`. 
- On the next screen, choose `JSON` and replace the JSON with the code below.
...
- Click on <InlineUIElement>Create access key</InlineUIElement> on the User overview page.
...
```sh
    access key :AKIAAMTW3505274M4W76
    secret access UK7YaBBK68PXdGQJRk2UXXso4p6gEeEQxQz855D2
```
...
<Aside type='note'>
In this section, you will need the region your AWS account uses. Get your region from the address bar when logged in to AWS. For example, this guide uses the `eu-north-` region, as found in the address: `https://eu-north-1.console.aws.amazon.com/console/home?region=eu-north-1`. 
...
Edit the Dockerfile and insert the following lines above the comment "###### Start FusionAuth App". Replace `eu-north-1` with the region your AWS account uses. If the incorrect region is used here, it may take a long time to download when you build the Docker image.
...
Next, save the [`docker-compose.yaml`](https://github.com/FusionAuth/fusionauth-containers/blob/main/docker/fusionauth/docker-compose.yml) and sample [`.env`](https://github.com/FusionAuth/fusionauth-containers/blob/main/docker/fusionauth/.env) files from the FusionAuth containers repo.
...
In the `fusionauth` service, replace the `volumes:` section with the code configuration below.
...
In the AWS UI, navigate to <Breadcrumb>Services -> CloudWatch -> Dashboard</Breadcrumb>. 
...
- For <InlineField>Widget type</InlineField>, choose `Line</InlineField>. 
...
Click on the `FusionAuth` namespace, then click on <InlineField>CPU</InlineField>. Select all the CPU options and click <InlineUIElement>Create widget</InlineUIElement>.
...
Click <InlineUIElement>Browse log groups</InlineUIElement> and select a log group or stream configured in the CloudWatch agent configuration, like `cloudwatch-logs`. Click <InlineUIElement>Run query</InlineUIElement>. If data has been received on the selected stream, it will show in the "Logs" section. Click <InlineUIElement>Create Widget</InlineUIElement> to add the new log table to the dashboard. 
...
Since FusionAuth API access is needed, see the section on [setting up FusionAuth API access for the collector](#set-up-fusionauth-api-access-for-collector).
...
In the script below, be sure to set your AWS region, AWS access and secret access keys, and your FusionAuth app Id.
...
Save the Python application in a new folder and give it the name `cloudwatch_logger.py`.
...
This Dockerfile will create a new container that will run the `cloudwatcher.py` application and export and upload the login data from FusionAuth to CloudWatch. The application requires some packages, Watchtower, and Boto3, which are used for communicating with AWS CloudWatch. 
...
AWS_ACCESS_KEY_ID=AKIA4MTWJ505Z74W4W76
AWS_SECRET_ACCESS_KEY=UK7YaBBK68PXdGQJRk2UXXso4p6gEeEQxQ2855D2
```
...
Finally, add the changes to the FusionAuth `docker-compose` file. You will need to change the region, access key, secret access key, and FusionAuth app Id to your values.
...
Next, generate a key pair to access the server via SSH. Click <InlineUIElement>Create new key pair</InlineUIElement>. Enter a name for the keypair, choose "RSA" as the keypair type, and `.pem` as the file format. Click on create key pair. 
...
Select the security tab in the middle and click the <InlineUIElement>launch-wizard</InlineUIElement>. 
...
Now, find the public IP on the Instance overview screen, then log in to the console by typing the following in the terminal, replacing `ec2-user@16.16.204.161` with your public IP:
...
[default]
aws_access_key_id=AKIA4MTWJ505Z74W4W76
aws_secret_access_key=UK7YaBBK68PXGQJRk2UXXso4p6gEeEQxQz855D2
```
