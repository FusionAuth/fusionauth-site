...
- You now have the Accesskey and the secret Access key and must save it because it will be used later in the config.
...
Once the CloudWatch Agent is installed, the config `cloudwatch-config.json` is copied into the image to handle the Cloudwatch integration.
...
In the above config, you set up the region for the CloudWatchAgent, some logs to collect and display as well as the metrics we are interested in. 
...
In the `fusionauth` service also replace the  `volumes:` section with the code config below:
...
Now, in the the same folder as the Dockerfile create a .aws folder with a file called credentials with the command below:
...
The config above is for this guide only, and you must replace it with your own.
...
Add the following region info in the config file:
...
Since there are not any changes to the config files, we use the same CloudWatch-config.json, Dockerfile and docker-compose.yml file from  our on-prem docker example [Set Up A Collector To Receive Data From FusionAuth](#set-up-a-collector-to-receive-data-from-fusionauth). 
