...
Check that your bot works so far by opening a terminal and running the command below, using your bot token and channel ID. The response should start with `"ok"`.
...
You now have a token you can use in a web service that monitors FusionAuth to post a message to Slack if FusionAuth fails. Let's write that web service next.
