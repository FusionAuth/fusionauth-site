...
Prometheus is an open-source dashboard app, that can be self-hosted, or cloud-hosted with a fee. Prometheus does not aggregate or process data itself, but can call data providers to get your metrics with aggregation, if the provider supports a query language that does aggregation.
