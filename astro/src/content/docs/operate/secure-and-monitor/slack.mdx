...
https://fusionauth.io/docs/apis/event-logs
not that useful. lamda, smtp, email template, webhook errors. all important, but will miss basic errors like cannot connect to database that will show in the system log.

cannot get system logs when fa runs in docker https://fusionauth.io/docs/apis/system#export-system-logs
...
# get event logs with errors in the last 31 seconds - https://fusionauth.io/docs/apis/event-logs#search-event-logs
...
- write service in your fav lang
- create a channel the service writes to every time it runs so you know it's up
- monitor posgtres too
