...
This guide explains the process in detail, and how to create a simple shell script to monitor FusionAuth and call Slack if FusionAuth has errors.
...
Create a file called `app.bash`. Insert the content below, replacing your FusionAuth API key, and your Slack API key and Slack URL.
...
If you have trouble calling the FusionAuth API, review the [troubleshooting tips](/docs/apis/#troubleshooting). To debug the script more quickly, you can run it on your physical machine instead of inside Docker. Change the FusionAuth URL at the top from `fa` to `localhost` when doing so.
...
- Create a `monitor-up` channel the service writes to every time it runs, so you know it's up. Currently, if the service dies, you'll never know. If you are on a paid Slack subscription, to avoid becoming desensitized to a spam channel that tells you the service is running every 30 seconds, create a workflow that writes to the alert channel if the `monitor-up` hasn't received a message in the last minute.
- If you want visibility into the performance of FusionAuth, not just errors, you'll need a comprehensive monitoring service. Please read out guide to Elastic or Prometheus to mmake one.
