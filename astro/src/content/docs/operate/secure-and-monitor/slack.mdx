...
- Check that your bot has permissions for `chat:write` and `chat:write.public` on the "Scopes" section.
...
- Event logs. These contain more complicated errors relating to lambda functions, SMTP, email templates, and webhooks. Event logs can be called through [their API](/docs/apis/event-logs#search-event-logs).
...
Create a file called `app.sh`. Insert the content below, replacing your FusionAuth API key, and your Slack API key and Slack channel Id at the top.
...
To force an error to check if the service works, run `docker compose down fa`. You should see a message in Slack that the monitor could not reach FusionAuth. If not, debug the script by running it on your physical machine instead of inside Docker with `./app.sh`. Change the FusionAuth URL at the top from `fa` to `localhost` when doing so. If you have trouble calling the FusionAuth API, review the [troubleshooting tips](/docs/apis/#troubleshooting).
