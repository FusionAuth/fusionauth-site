...
<Item category="known-issue" resolvedIn="1.65.0" viaIssue="3396">
Reactor contains a broken link that should lead users to find their license key in their FusionAuth account.
</Item>

<Item category="known-issue" resolvedIn="1.65.0" viaIssue="3386">
A race condition exists: concurrent `PATCH` calls to `/api/identity-provider/{id}` can _all_ return status code `200`, but only one modification persists.
</Item>

<Item category="known-issue" viaIssue="3384" resolvedIn="1.69.0">
The default Enterprise User SCIM schema `meta.location` field contains a duplicate path segment, making it invalid.
</Item>

<Item category="known-issue" issue="3385" resolvedIn="1.69.0">
The SCIM Group schema does not mark `displayName` as required, even though the specification requires it. 
</Item>
