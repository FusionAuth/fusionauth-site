...
Introduced [Lambda Secrets](/docs/extend/code/lambdas#using-secrets), secure storage for highly sensitive values like app passwords and API keys that can be accessed from lambda functions.
</Item>

<Item category="enhancement" issue="1029" bonusIssue="1177">
Implemented OAuth2 metadata ([RFC 8414](https://tools.ietf.org/html/rfc8414)) so clients can dynamically find FusionAuth OAuth endpoints.
</Item>
...
Added an index for the [User](/docs/apis/users) `passwordChangeRequired` field to enable Elasticsearch queries. This makes it easier to count and view users who need to perform a password reset, but have not yet reset their password.
</Item>
