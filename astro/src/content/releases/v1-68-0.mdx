...
Introduced **Intelligent MFA**, which uses a collection of signals to trigger MFA challenges for users based on the risk level of a specific login attempt.
</Item>
...
Created the MFA Deleter role, a granular permission that enables a user to remove an MFA method from a user account without full FusionAuth administrator permissions.
</Item>
...
[MFA recovery codes](/docs/lifecycle/authenticate-users/multi-factor-authentication#recovery-codes) are now hashed at rest using `salted-pbkdf2-hmac-sha256`, the same algorithm used by default for password hashing. Existing recovery codes will continue to work without interruption. Their codes will be migrated to the hashed format automatically when they next regenerate their codes.
</Item>
