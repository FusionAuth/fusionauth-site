...
Applications now offer an `Unknown Scope Policy`. This can be used to enhance security by rejected or removing unrecognized scopes during an OAuth workflow. See the application [Scopes tab](/docs/get-started/core-concepts/types/applications#scopes) for more detail.
</Item>
...
<Item category="enhancement" issue="275">
The `/oauth2/device/user-code` endpoint now returns the `scope` parameter value that should be used in the interactive portion of the Device Code Grant workflow. See [Device User Code](/docs/apis/oauth/device#device-user-code-via-api-key) for more detail.
</Item>
