...
The `/oauth2/userinfo` endpoint now requires the `aud` claim to be present on the provided access token, allowing for tighter compliance with the OIDC spec. See the [UserInfo endpoint](/docs/api/oauth#userinfo) for more detail.
...
The [Refresh Token Grant](/docs/api/oauth#refresh-token-grant-request) request now supports requesting a subset of the original scopes. Previously, the request responded with an `invalid_scope` OAuth error.
</Item>
...
<Item category="enhancement" issue="275">
The `/oauth2/device/user-code` endpoint now returns the `scope` parameter value that should be used in the interactive portion of the Device Code Grant workflow. See [Device User Code](/docs/api/oauth#device-user-code) for more detail.
</Item>
