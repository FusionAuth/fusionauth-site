...
description: This update includes bugfixes, security improvements and more
authors: Dan Moore
...
FusionAuth version 1.49.0 shipped in early March, 2024. This version includes bugfixes, security improvements, and more.

{/* more */}

The focus of this update was bugfixes. We have some other bigger features planned soon, but wanted to get this release in the hands of the community.

Therefore, I hereby dub 1.49 the "Bugfix Beluga" release.

![Bugfixes are awesome.](/img/blogs/release-1-49/beluga-meme.png)

All in all there are 27 issues, enhancements, and bug fixes included in the 1.49.0 release. As always, please see the [release notes](/docs/release-notes/#version-1-49-0) for a full breakdown of the changes between 1.48.3 and 1.49.0, including any schema changes.
...
We're happy LinkedIn updated their authentication workflow to support OIDC and are excited to support that integration going forward.
...
But if you use FusionAuth and [reindex your Elasticsearch (or Opensearch) index](/docs/lifecycle/manage-users/search/search#reindexing-elasticsearch), before this release FusionAuth didn't do a great job of reporting progress. It only reported back every 250k objects, which depending on the data in the index, could give the impression that progress has halted.

In this release and going forward, FusionAuth will report reindexing progress more regularly in order to spare everyone some frustration.
...
Link checkers, which are common in corporate email settings, defeat this in the name of preventing phishing. They retrieve all the links in the email to make sure they are safe, invalidating the one-time code.
...
In this release, FusionAuth offers an optional configuration to require the user to submit a form after they click on the emailed link. While this increases friction, it also guarantees the one-time code won't be used up by any known link checker.

## The Rest Of It

As mentioned above, there were 27 issues, enhancements, and bug fixes included in this release. A selection of the changes not covered above includes:
...
Read more about all the changes in the [release notes](/docs/release-notes/#version-1-49-0).

## Upgrade At Will

The [release notes](/docs/release-notes/#version-1-49-0) are a guide to the changes, fixes, and new features. Please read them carefully to see if any features you use have been modified or enhanced.
