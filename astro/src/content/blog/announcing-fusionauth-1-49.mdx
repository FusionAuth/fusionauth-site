...
But if you use FusionAuth and [reindex your Elasticsearch (or OpenSearch) index](/docs/lifecycle/manage-users/search#reindexing-elasticsearch), before this release FusionAuth didn't do a great job of reporting progress. It only reported after processing each batch of 250,000 objects. Depending on the data in the index, this could give the impression that progress halted.
...
Speaking of frustration, FusionAuth has dealt with a lot of issues around [our magic links functionality](/docs/lifecycle/authenticate-users/magic-links-and-codes/). This seems pretty straightforward:
