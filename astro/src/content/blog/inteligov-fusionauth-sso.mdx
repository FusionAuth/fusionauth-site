...
description: Inteligov uses FusionAuth for all things authentication, including custom SSO which opened up a new revenue stream.
authors: Dan Moore
...
**Raphael:** Cost, functionality and the presence of an active community were the main drivers. Even before we adopted Fusionauth, we submitted the [localization for Brazilian Portuguese](https://github.com/FusionAuth/fusionauth-localization/blob/main/theme/messages_pt_BR.properties) and the PR was readily review and accepted which was really nice - since we had previous experience with projects that are simply stalled regarding user contribution.
...
* Make it easier to use secure headers out of the box. Would be great for FusionAuth to set be able to set headers (CSP, X-Frame-Options, etc).
* In a pentest report the exposure of the `client_id` parameter in the URL was pointed out as a vulnerability: information Leakage via URL query strings. Any way to handle that would be great.
