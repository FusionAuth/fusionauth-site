...
authors: Dan Moore
image: /img/blogs/release-1-26/product-update-fusionauth-1-26.png
categories: Release Updates, Product
tags: release notes, entities, client credentials grant, api keys, mfa, multifactor authentication
...
This release contained a number of enhancements and bug fixes. Please see the [release notes](/docs/release-notes/archive#version-1-26-0) for a full breakdown of the changes between 1.25 and 1.26. I wanted to highlight a few of the biggest ones.
...
The FusionAuth team wants every user to be able to use multi-factor authentication to secure their accounts, whether developers buy a premium plan or not. Therefore, support for time based one-time password MFA solutions such as Google Authenticator and Authy continues to be included in the Community plan. (Take that, hackers!)

<Aside nodark="true" type="caution">
Unfortunately, these changes are not backwards compatible with the previous Two Factor implementation, so if you have built on top of that, please [review the changes carefully](/docs/customize/email-and-messages/deprecated/). There is also a [1.25 -> 1.26 MFA migration guide](/docs/lifecycle/authenticate-users/multi-factor-authentication#migration-from-version-125-and-earlier).
</Aside>

Here's a customized SMS template:

![A customized SMS MFA template..](/img/blogs/release-1-26/custom-sms-template.png)

Here's the tenant edit view, where I am configuring available MFA methods:

![Configuring allowed MFA methods..](/img/blogs/release-1-26/setting-mfa-options.png)
...
Also, there's a new themeable FusionAuth index page so when your users accidentally arrive at `auth.example.com`, they are not forwarded to the FusionAuth admin UI login page. (To proceed to that login page when using the default theme, click the <InlineUIElement>lock</InlineUIElement> in the upper right.)

![The themeable root page. No more users ending up on the login page..](/img/blogs/release-1-26/themable-index-page.png)
...
We're in the process of documenting all of this, but the [release notes](/docs/release-notes/archive#version-1-26-0) serve as a good guide of the changes, fixes, and new features.
