...
image: /img/blogs/fusionauth-and-terraform/fusionauth-and-terraform.png
categories: Tutorial
...
In this post, you will learn why you would want to use the Terraform provider for FusionAuth. You can ditch the manual clicks and start defining your identity resources as code.

<Aside type="tip">TLDR; I just want the guide. You're in luck. FusionAuth just released v1.0.0.0 of the Terraform provider for FusionAuth. You can find the guide on how to use it [here](https://fusionauth.io/docs/operate/deploy/terraform).</Aside>

## Wait... What's Terraform?
Terraform is a powerful open-source Infrastructure as Code (IaC) tool from HashiCorp. It lets you manage and provision infrastructure and services through human-readable configuration files. Think of it as writing down what your infrastructure should look like and letting Terraform handle the how.

Whether you're managing a single application or running an identity empire with a dozen tenants, Terraform lets you bring order to the chaos.

Let's get into it — minimal clicking, maximum control.
...
While FusionAuth has a clean and intuitive interface, there can be a lot of clicks to manage a large number of objects. If you've ever found yourself meticulously clicking through FusionAuth's UI to configure applications, tenants, or identity providers—only to realize you forgot a setting and need to start over—you know the pain of manual identity configuration. FusionAuth makes authentication and user management easy, but managing a growing number of configurations manually can quickly become overwhelming. That's where Terraform comes in.

With Terraform, you can:
* **Automate Repetitive Work**. Managing FusionAuth through the UI is great at first. However, as your usage evolves, you'll likely need to do things like duplicate applications that are clones of others, create tenants, configure identity providers and other things on a regular basis. Doing all that manually, especially in multiple environments, is a recipe for mistakes and inconsistency. With Terraform, you define these resources once—in code—and can apply the same configuration across dev, staging, and production with confidence.
* **Version Control Your Identity Configuration.** Terraform configurations are just text files. That means you can put them in Git. You can review changes through pull requests. You can do other things with your infrastructure that you can do with code as well like rolling back to a previous state or tracking the history of changes. Instead of relying on memory or screenshots in an outdated log file, you'll have the full history clearly documented. While many of the things in this list can be done directly through the [FusionAuth APIs](https://fusionauth.io/docs/apis/), this is one list item the apis don't cover.
...
## How does it work?
As mentioned in the above tip, there is a detailed [guide](https://fusionauth.io/docs/operate/deploy/terraform) on using FusionAuth with Terraform. Here is a high level overview of how the setup works.
