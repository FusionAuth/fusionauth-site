...
FusionAuth includes an [`introspect` endpoint](/docs/api/oauth#introspect) that decodes the access token (represented by a JWT) and returns details about the current user. You will call this endpoint through your Node application by attaching the authorization token stored in the session.
...
The last endpoint in your Node app allows users to log out. To fully log out of the application, you need to both destroy the Node application's session and redirect users to the FusionAuth [logout endpoint](/docs/api/oauth#logout).
