...
```mermaid title="Diagram of the OAuth Authorization Code flow with PKCE extension using FusionAuth and Gatsby."
sequenceDiagram
...
Once you have FusionAuth running, log into the admin panel and create a new Application. This process [is outlined here](/docs/get-started/start-here/step-1), but you'll need to add your application's URLs to the OAuth configuration:
...
Before you get started, you need to create a new subdirectory and initialize an Express app. Use a [similar structure to the one outlined here](/blog/securely-implement-oauth-in-react):
