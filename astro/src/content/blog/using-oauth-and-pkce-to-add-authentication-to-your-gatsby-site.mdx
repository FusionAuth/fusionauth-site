...
htmlTitle: "How to Add OAuth Login to Gatsby With PKCE"
description: Add secure login to your Gatsby site using OAuth and PKCE. This tutorial shows how to use FusionAuth and the Authorization Code Flow.
authors: Karl Hughes
image: /img/blogs/oauth-gatsby/fusionauth-tutorial-using-oauth-and-pkce-to-add-authentication-to-your-gatsby-site.png
categories: Tutorial
tags: javascript, gatsby, oauth, pkce
excerpt_separator: "{/* more */}"
---
import Breadcrumb from 'src/components/Breadcrumb.astro';
import Diagram from "../../diagrams/blog/oauth-gatsby/oauth-gatsby.astro";
...
In this blog post, you'll learn how to create a Gatsby site that uses FusionAuth to allow users to log in and access their profile securely. This application will use an [OAuth Authorization Code workflow and the PKCE extension](/articles/oauth/modern-guide-to-oauth#authorization-code-grant) to log users in and a Node application to store your access token securely. PKCE stands for Proof Key for Code Exchange, and is often pronounced "pixie".

At a high level, the authorization process looks like this:

<Diagram alt="Diagram of the OAuth Authorization Code flow with PKCE extension using FusionAuth and Gatsby." />
...
Before you start writing any code, download FusionAuth and get it running on your local machine. FusionAuth is available [for all major operating systems](/download) or it can be [run in Docker](/docs/get-started/download-and-install/docker).

Once you have FusionAuth running, log into the admin panel and create a new Application. This process [is outlined here](/docs/quickstarts/5-minute-setup-guide), but you'll need to add your application's URLs to the OAuth configuration:

- Add <InlineField>http://localhost:9000/oauth-callback</InlineField> to the <InlineField>Authorized redirect URLs</InlineField>.
- Add <InlineField>http://localhost:9000</InlineField> to the <InlineField>Authorized request origin URLs</InlineField>.
- Enter <InlineField>http://localhost:8000</InlineField> in the <InlineField>Logout URL</InlineField> field.

You'll also want to save the "Client Id" and "Client secret" values as you'll need them later.

![FusionAuth configuration options for a Gatsby static site.](/img/blogs/oauth-gatsby/application-setup.png)
...
## Creating a new user
To test your Gatsby-based login, you'll need to add a new user and register them for your application in FusionAuth. From the Users page in FusionAuth, click <InlineUIElement>+</InlineUIElement> to add a user. Enter an email address and password for your new user and click the save button.

![Creating a new user in FusionAuth.](/img/blogs/oauth-gatsby/create-user.png)

Next, click <InlineUIElement>Add registration</InlineUIElement> to link this user to the application you created in Step 1. Click the save button when you're finished.

![Registering a user in FusionAuth.](/img/blogs/oauth-gatsby/register-user.png)
...
FusionAuth includes an [`introspect` endpoint](/docs/lifecycle/authenticate-users/oauth/endpoints#introspect) that decodes the access token (represented by a JWT) and returns details about the current user. You will call this endpoint through your Node application by attaching the authorization token stored in the session.
...
The last endpoint in your Node app allows users to log out. To fully log out of the application, you need to both destroy the Node application's session and redirect users to the FusionAuth [logout endpoint](/docs/lifecycle/authenticate-users/oauth/endpoints#logout).
...
![A login link in Gatsby.](/img/blogs/oauth-gatsby/gatsby-login-page.png)
...
- Successfully log out by clicking the <InlineUIElement>Logout</InlineUIElement> button on your profile page

![The profile page in Gatsby.](/img/blogs/oauth-gatsby/gatsby-profile-page.png)
