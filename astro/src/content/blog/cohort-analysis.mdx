...
In this guide you learned how to use basic FusionAuth login data to analyze user behavior. Visualizing user behavior is foundational to determining which parts of your business offer the most value to customers and understanding how to keep them engaged. If you see any patterns you want to investigate further, use the FusionAuth admin UI or API to look up users by their real IDs. The extracted users.json file uses anonymized IDs, so tracing individual users requires working directly with FusionAuth rather than the exported data.
