...
<Aside type="note">
FusionAuth [creates a login record at the moment of application registration](/docs/get-started/core-concepts/types/users#what-makes-a-user-active), so the extraction code skips the first login date for each user. Without this, every user would appear to have zero friction, since their first login would always match their registration date.
</Aside>
