...
The first abandonment chart is the opposite of user activity. Each user is counted in exactly one bucket based on how long it's been since their last login: 1-2 months, 2-6 months, 6-12 months, or 12+ months. The majority of users (over 500 of the 1,000 users) fall in the 12+ bucket — they haven't logged in for at least a year.
...
<Aside type="note">
FusionAuth creates a login record at the moment of registration, so the extraction code skips the first login date for each user. Without this, every user would appear to have zero friction, since their first login would always match their registration date.
</Aside>
...
```
Charts created
charts-output.html written
Server listening at http://0.0.0.0:7777
```

To view your charts, visit [http://localhost:7777](http://localhost:7777). A standalone copy is also written to `charts-output.html` so you can open or share the charts without the server running.
...
Edit the `.env` file. Adjust the URL, authorization key, and application Id to match your FusionAuth instance. Then run the file with the following command:
