...
- [A customer statistics app with charts you can customize](#a-customer-statistics-app-with-charts-you-can-customize)
...
You are going to run code to extract user and login data and save it in a JSON file with the structure below.
...
```js
{
  ...
  isVerified: bool
  registeredDate: date
  loginDates: date[] // where [0] is oldest date
}
```

Create a file called `extract.mjs` and insert the code below.
...
## A customer statistics app with charts you can customize
...
```sh
docker run --init  -it --rm --name "app" -v ".:/app" -w "/app" node:25-alpine --network faNetwork sh -c  "node fake.mjs";
```
