...
The friction chart groups users by how long it takes them to log in the first time after registering. It shows both how easy the registration to usage flow is (low friction), and how much the registration process interests the users in actually using your offering. If they realise while registering that your service isn't what they're really looking for, the time until they login will be high. This example chart shows that nearly all users do login, which is good and indicates high interest, but only after a month, which means the business should make it easier for users to explore the service after registering.
