...
Adjust the URL, authorization key, and application Id to match your FA instance. Then run the file with the command below.
...
Once the users are created, you need randomize their registration dates and create thousand of login dates. This can't be done with the FA API, so you need to run SQL directly on the PostgreSQL FA database.
