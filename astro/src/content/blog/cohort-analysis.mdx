...
Use Git to clone the [accompanying repository](https://github.com/ritza-co/fusionauth-user-charts): TODO
...
The extraction creates two files: `faUsers.json`, which contains the raw data from FusionAuth, and `users.json`, which contains the processed, denormalized set of users with all login dates. The latter is used to create the charts efficiently.
...
<Aside type="danger">
Ensure that exporting, analyzing, and storing your users' IDs, email addresses, and login dates does not violate any laws regarding personal data in your country. The easiest way to anonymize the data would be to alter the extraction code to replace email address and user IDs with a random UUID before saving to JSON.
</Aside>
TODO please do so
...
If you see any patterns you want to investigate further, edit the code to log the user Ids in a group that interests you, and then use those Ids to investigate which features in your app affect those users.
