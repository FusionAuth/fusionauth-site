...
Replace `<your_db_password>`, `<your_db_host>`, and `<your_db_user>` with your database credentials (check your `.env` file or `docker-compose.yml`). Replace `<your_application_id>` with your FusionAuth application Id, which you can find in your `.env` file. For example, if you're using the Docker setup from the guide, the command would look like this:

```console
PGPASSWORD=postgres psql -h faDb -U postgres -d fusionauth -v fusionauth_app_id=9b6e2e8a-7a1f-4b3d-8c2e-1f5a3d7e9b2c -f createMockData.sql
```
