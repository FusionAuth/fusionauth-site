...
To create fake users and login data for a FusionAuth application, you need to run a Go file and SQL script. There is no way to create fake dates using the FusionAuth API, so you need direct database access to run the SQL. This works great when you are [running locally](/docs/get-started/download-and-install/docker) or when you [self-host](/docs/get-started/download-and-install/docker), but instances running in [FusionAuth Cloud](/docs/get-started/run-in-the-cloud/cloud) don't allow direct database access.
