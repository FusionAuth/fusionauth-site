...
---
import APIRequest from "../../diagrams/blog/api-authentication/api-request.astro";
import FailedAPIRequest from "../../diagrams/blog/api-authentication/failed-api-request.astro";
import APIRequestWithAPIKey from "../../diagrams/blog/api-authentication/api-request-with-api-key.astro";
import GetAPIKey from "../../diagrams/blog/api-authentication/get-api-key.astro";
import GetHashedAPIKey from "../../diagrams/blog/api-authentication/get-hashed-api-key.astro";
import APIRequestWithAPIKeyAndCentralAuthority from "../../diagrams/blog/api-authentication/api-request-with-api-key-and-central-authority.astro";
import APIRequestWithAPIKeyHashed from "../../diagrams/blog/api-authentication/api-request-with-api-key-hashed.astro";
import APIRequestWithTimeboundAPIKey from "../../diagrams/blog/api-authentication/api-request-with-timebound-api-key.astro";
import APIRequestWithTimeboundAPIKeySecondRequest from "../../diagrams/blog/api-authentication/api-request-with-timebound-api-key-second-request.astro";
import APIRequestWithDataAPIKey from "../../diagrams/blog/api-authentication/api-request-with-data-api-key.astro";
import APIRequestWithRefreshAPIKey from "../../diagrams/blog/api-authentication/api-request-with-refresh-api-key.astro";
...
This is often managed by an API portal for a long lived key. There is a central authority is creating API keys and associating them with users or other pieces of software. After they are minted, the client must then store the API key securely.
...
Congrats, you've re-implemented a chunk of OAuth2  (TODO client registration?) as well as successfully decentralizing API authentication.

## Decentralized Authentication

As you've evolved centralized API authentication for your todo application, there were certain architectural components. I used certain names above, but they map to well-known and understood terms. Here's a table:

TODO table
client: client
central authority: authorization server
API server: resource server
getting the API key: client registration (kinda)
...
The token is signed and optionally comes with a refresh token. If you are using OIDC on top, it comes with an id token as well (learn more about OIDC TODO link).

The server needs to process the access token, which means:

* validating the signature
* examining the claims, including the expiration claim and other standard claims (learn more TODO)
* extract claim information and use it ot provide the service

Of course, nothing is free. There are some additional concerns when you are using centralized authentication.

### Additional Concerns

Because there's no communication with the authorization server (central authority) during validation of the access token, revocation becomes difficult. You can work around this in several ways:
...
However, there are some standards that can bind a token to a particular client. One is MTLS, which uses client certificates. The other is DPOP, which was recently promoted to a standard.

## In Conclusion

Centralized and decentralized protection of APIs offer different tradeoffs. When using API keys, you must keep communicating with whatever issued those API keys.
