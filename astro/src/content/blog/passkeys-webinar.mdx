...
## What Are Passkeys
Passkeys are a form of authentication that can augment or completely replace passwords. Since passkeys require both a public and private key, they are by nature multi-factor. As only one of the two keys lives on a server, they're inherently more secure than passwords. Often times the private key (held by the user) can only be accessed by a fingerprint or facial recognition. In essence, this rule creates a third factor of authentication. 
