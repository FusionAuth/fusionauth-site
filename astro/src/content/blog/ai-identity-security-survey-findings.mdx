...
[IMAGE - Two side-by-side horizontal bar charts. Left: confidence level (extremely confident → not so confident) vs. confirmed breach rate. Right: confident + breached rate by revenue band (under $10M → $1B+). Caption: "Confidence predicts breach rates. Organizational size does not."]
...
[IMAGE - Side-by-side bar chart: multi-tenant SaaS identity 83% breach rate vs. self-hosted/on-premises 38% breach rate. Caption: "Deployment model predicts AI identity breach outcomes more accurately than governance maturity, policy coverage, or confidence level."]
...
[IMAGE - Venn diagram or stacked illustration showing three overlapping risk factors: AI in production, AI widespread across workforce, and multi-tenant SaaS identity — converging at 90% confirmed incident rate and 96% shadow AI exposure.]
