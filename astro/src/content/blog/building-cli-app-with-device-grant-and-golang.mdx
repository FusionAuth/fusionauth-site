...
1. Your **Tenant ID** -- You can find this easily by clicking the <Breadcrumb>Tenants</Breadcrumb> tab on the left sidebar and then copying the **Id** value for the <Breadcrumb>Default</Breadcrumb> tenant.
...
1. Our `Run` func starts off by creating a `FusionAuthClient` and assigning it to `faClient`. This is using FusionAuth's [go-client](https://github.com/FusionAuth/go-client/tree/main/pkg/fusionauth) library, which **will be our primary interface to our FusionAuth server going forward** and is going to come in handy throughout our Device Code grant.
