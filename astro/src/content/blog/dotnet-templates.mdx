...
title: Create .NET web application with FusionAuth templates
...
categories: Product, Education, Developer Tools, .NET, Visual Studio
...
## Introduction
*Templates for nothin' and your security for free.* 

Literally. The [FusionAuth Templates for Microsoft .NET](https://marketplace.visualstudio.com/items?itemName=FusionAuth.FusionAuthVSTemplates) are free to use and so is the self-hosted [Community Plan](https://fusionauth.io/pricing?step=plan&hosting=self-hosting) of FusionAuth.

Being a Developer Relations Engineer at FusionAuth, one of my roles is to help developers develop faster. With that in mind, I couldn't be more excited to share with you the new .NET templates FusionAuth has made available. They are available through [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=FusionAuth.FusionAuthVSTemplates) as well as a [NuGet](https://www.nuget.org/packages/FusionAuth.Templates) package. 

## Getting Started

### Download The Templates

Let's start off by installing the FusionAuth templates using Visual Studio. For those that develop .NET applications outside of Visual Studio, don't worry I'll cover that too. Inside of Visual Studio, select `Extensions` from the menu bar and then select `Manage Extensions`.
...
For those of you that develop outside of Visual Studio, you can simply issue the following command from the terminal or command line.
...
If things do not go according to plan, you can find more detail in the [Readme.md](https://github.com/FusionAuth/fusionauth-example-client-libraries/blob/main/README.md) file in the repository. You could also check out one of our quickstarts, such as [.NET Web](https://fusionauth.io/docs/quickstarts/quickstart-dotnet-web) for more details on how to get the FusionAuth server up and running.
...
It really is just that simple. You now have a fully functioning and secured application. All you need to do is hit the green Start button in Visual Studio. 
...
Ok, for some, you have seen all you need to see to start using the new Visual Studio FusionAuth .NET Templates. Please enjoy and feel free to head to the Conclusion section.

For those of you that like to peek under the hood, I have reserved this section for you. Let's dig in.
...
<Aside type="danger">
The client secret is only stored in the `appsettings.Development.json` file for convenient use with the templates. For production environments you will want to store this in an environmental variable or in some other secure manner.
</Aside>
...
The `Profile` link will take to user to the `AccountController` and attempt to return the `Profile` view. Notice the `Authorize` attribute above the function name. This attribute let's the application know not to issue this view unless the user is authenticated.
...
* [FusionAuth - Getting Started](https://fusionauth.io/docs/v1/tech/getting-started/)
