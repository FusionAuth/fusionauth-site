...
Even though we've come a long way since Cross-Origin Resource Sharing (CORS) saved us from the security nightmare of OAuth [implicit grant](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/#example-implicit-grant) (`response_type=token`), it opened the door to browser apps storing tokens directly in the browser. Not just access tokens, but refresh tokens too. The most common storage options are:
...
The [OAuth 2.0 for Browser-Based Applications draft](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-browser-based-apps#section-5.1) is crystal clear: **malicious JavaScript has the same privileges as legitimate application code**. When npm packages get compromised, your SPA becomes a vector for attack, as the attacker's code runs with full access to everything your app can access. Let's break down why each storage option fails:

#### localStorage and sessionStorage

```javascript
// What 2 billion weekly downloads of compromised npm packages can do:
const stealTokens = () => {
  const tokens = {
...
This isn't theoretical. The September 2025 npm attacks included nearly identical code, just targeting different values. The draft calls this "Single-Execution Token Theft" (Section 5.1.1) when run once, or "Persistent Token Theft" (Section 5.1.2) when run repeatedly.
...
The only solution is to never allow tokens to enter the browser context. That's where Backend-for-Frontend comes in.
...
{/* TODO: Image comparing traditional SPA authentication and BFF architecture */}
...
const tokenResponse = await fetch(tokenEndpoint, {
  ...
  body: new URLSearchParams({
    ...
    client_secret: process.env.CLIENT_SECRET, // ← Browser can't do this
...
Users never experience token expiration. The backend silently refreshes tokens before they expire, maintaining seamless user experience without security compromises.
...
BFF trades convenience for security. But given the reality of modern JavaScript supply chains, where a single compromised dependency can affect millions of applications, this tradeoff is no longer optional for applications handling sensitive data.
...
This should download docker images and start FusionAuth on `http://localhost:9011`. You can log in with the default admin credentials provided in the README: `admin@example.com` and `password`.
...
Edit the `.env` file to set your `CLIENT_ID`, `CLIENT_SECRET` - you can find these in the FusionAuth admin UI under Applications > Your App > Action > View.
...
When the app loads, the frontend checks for an existing session by calling the backend. Heres the simplified code from the React app (without error handling and surrounding code):

```javascript
// Frontend: /bff/frontend/src/services/authService.js
async checkSession() {
...
If you aren't logged into FusionAuth yet, you'll see the login screen. Register a new user.
...
Each of the "Protected", "Profile", and "Call API" pages makes requests to the backend, which adds the access token from the session before forwarding to the resource server.

The browser never sees any access tokens, nor does it interact directly with the resource server.

If everything worked, you should see a random recipie on the "Call API" page, fetched securely through the BFF.
...
Update the `.env` file to set your `VITE_CLIENT_ID` to the client ID of your application in FusionAuth.

<Aside type="note">
If you've been following along with the self-hosted BFF demo, you can skip setting up the resource server and FusionAuth again. Just make sure to end the Backend and Frontend servers from the previous demo before starting this one.
</Aside>
...
{/* TODO: Hosted backend diagram */}

Instead of your own backend server, the React app communicates directly with FusionAuth's hosted backend. FusionAuth manages the OAuth flow, token storage, and session management for you, while securely proxying API requests to your resource server. Each request from the frontend includes an httpOnly cookie with the session identifier, ensuring that tokens are never exposed to JavaScript.

## The Decision Framework: When BFF Is Essential

If you're still storing tokens where JavaScript can access them, consider migrating to a BFF architecture as soon as possible. The security benefits are too significant to ignore, especially in light of recent supply chain attacks.
