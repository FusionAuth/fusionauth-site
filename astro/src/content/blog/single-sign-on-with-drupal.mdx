...
Log in to your Drupal admin dashboard, click the <Breadcrumb>Extend</Breadcrumb> menu, click <Breadcrumb>Add new module</Breadcrumb>, and paste the link.

![Adding the OIDC module to Drupal.](/img/blogs/single-sign-on-drupal/add-oidc-module-to-drupal.png)

Click <Breadcrumb>Continue</Breadcrumb>. It will take some time to fetch and add the module. Once the installation is complete, click <Breadcrumb>Enable newly added modules</Breadcrumb>.

![Enabling the OIDC module.](/img/blogs/single-sign-on-drupal/enabling-modules.png)

Scroll down to the <Breadcrumb>Security</Breadcrumb> section, check the checkbox next to "OpenID Connect".

![The OIDC module in the list.](/img/blogs/single-sign-on-drupal/oidc-module-in-list.png)

Then click <Breadcrumb>Install</Breadcrumb>.
...
In the <Breadcrumb>OAuth</Breadcrumb> tab, enter `<YOUR_DRUPAL_SITE_BASE_URL>/openid-connect/generic` in the <InlineField>Authorized redirect URL</InlineField> field. Replace `<YOUR_DRUPAL_SITE_BASE_URL>` with the URL of your Drupal site. Below it is <InlineFieldValue>http://35.227.12.233</InlineFieldValue> but that's specific to my setup.
...
Save the application by clicking the blue <Breadcrumb>Save</Breadcrumb> button in the top right corner of the page. This redirects you back to the applications list, where you can click the tiny view button next to the newly created application.

![The application list.](/img/blogs/single-sign-on-drupal/view-application-list.png)

The popup window will show you a list of information about the application. Copy the "Client Id" and "Client Secret" under the <Breadcrumb>OAuth configuration</Breadcrumb> section. You'll need these when configuring Drupal.
...
Enable the <InlineFieldValue>Generic</InlineFieldValue> option under <InlineField>Enabled OpenID Connect clients</InlineField>.
...
1. `<YOUR_FUSIONAUTH_URL>/oauth2/authorize` in <InlineField>Authorization endpoint</InlineField>.
2. `<YOUR_FUSIONAUTH_URL>/oauth2/token` in <InlineField>Token endpoint</InlineField>.
3. `<YOUR_FUSIONAUTH_URL>/oauth2/userinfo` in <InlineField>Userinfo endpoint</InlineField>.

Make sure to replace `<YOUR_FUSIONAUTH_URL>` with the URL of your FusionAuth instance.

Also, enable the <InlineField>Automatically connect existing users</InlineField> setting. This allows existing Drupal users (as determined by email) to sign in with FusionAuth.

![Configure Drupal OIDC module.](/img/blogs/single-sign-on-drupal/configure-drupal-oidc.png)

By default the OpenID login buttons are not visible. You can enable them in the <Breadcrumb>OpenID buttons display in user login form</Breadcrumb> section. Select "Above" or "Below" to show the OpenID buttons along with the default Drupal login form.

![Configure the position of the login buttons.](/img/blogs/single-sign-on-drupal/login-button-position.png)

Click on <Breadcrumb>Save configuration</Breadcrumb> at the bottom of the page.
...
Visit your Drupal site and click <Breadcrumb>Log in</Breadcrumb>. If the setup was successful, you should see a <Breadcrumb>Log in with Generic</Breadcrumb> button.
