...
In the <Breadcrumb>OAuth</Breadcrumb> tab, enter `<YOUR_DRUPAL_SITE_BASE_URL>/openid-connect/generic` in the **Authorized redirect URL** field. Replace `<YOUR_DRUPAL_SITE_BASE_URL>` with the URL of your Drupal site. Below it is `http://35.227.12.233` but that's specific to my setup.
...
Navigate to <Breadcrumb>Configuration -> People</Breadcrumb>. Under **Who can register accounts**, select "Visitors". This ensures new users can register accounts.
