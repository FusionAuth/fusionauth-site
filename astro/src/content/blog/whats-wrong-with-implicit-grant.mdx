...
seoTitle: "OAuth Implicit Grant Problems | Why OAuth2 Implicit Flow Is Deprecated"
description: Learn why the OAuth2 implicit flow is deprecated. This guide explains the problems with the OAuth implicit grant and what secure alternatives you should use instead.
...
![Just a rabbit?](/img/blogs/avoid-implicit-grant/Rabbitattack.jpg)
...
Unlike the [Authorization Code grant](/docs/lifecycle/authenticate-users/oauth/#example-authorization-code-grant), the Implicit grant does not redirect the browser back to your application backend with an Authorization Code. Instead, it puts the access token directly on the URL as part of the redirect. These URLs look like this:
...
Want to learn more about the different OAuth grants? Check out [The Modern Guide to OAuth](/articles/oauth/modern-guide-to-oauth) for an in-depth look at all the different OAuth grants and how you might use them in the real world.

Happy coding!

_[Schedule a demo](/contact) to see how FusionAuth supports secure OAuth flows, including Authorization Code with PKCE, for your single-page applications._
