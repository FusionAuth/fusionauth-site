...
Navigate to <Breadcrumb>Tenants</Breadcrumb> and then edit the <Breadcrumb>Default</Breadcrumb> tenant. Go to the <Breadcrumb>General</Breadcrumb> tab and update the **Login theme** setting to the new theme. Save the configuration when the screen looks like:
