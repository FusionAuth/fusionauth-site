...
Navigate to <Breadcrumb>Tenants</Breadcrumb> and then edit the <InlineUIElement>Default</InlineUIElement> tenant. Go to the <Breadcrumb>General</Breadcrumb> tab and update the <InlineField>Login theme</InlineField> setting to the new theme. Save the configuration when the screen looks like:
