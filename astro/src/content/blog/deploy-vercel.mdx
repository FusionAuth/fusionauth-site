...
## Next Steps
This is a basic tutorial on how to deploy an application to Vercel using FusionAuth as the authentication provider. For production applications, you may want to do things like set up [custom environments](https://vercel.com/docs/deployments/environments) and GitHub actions based on each. For example, you may want each branch to have its own environment or set up a development environment to deploy to before you deploy to production.
And just like that, with a push to GitHub, your humble little app is live on Vercel—fully authenticated and dangerously close to being mistaken for a production app.

But before you start sending out press releases and quitting your day job, a word of caution: deploying straight to production is like skydiving without a backup parachute—it's exhilarating until it isn't. If you're planning on adding new features (or “breaking things” as we call it in tech), you might want to set up [custom environments](https://vercel.com/docs/deployments/environments) in Vercel. That way, you can deploy to development or preview environments first, make sure everything still works.
