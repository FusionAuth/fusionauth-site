...
description: "One engineer reverse-engineered his DJI vacuum's API for fun and accidentally accessed 7,000 devices worldwide. Here's the authorization failure that made it possible — and how to make sure it doesn't happen in yours."
...
Sammy Azdoufal spent $2,000 on a DJI Romo robot vacuum. It's a sleek, impressive machine — DJI's first foray into the consumer robotics market. And Sammy, being a normal, well-adjusted engineer, immediately decided the official app wasn't good enough and he needed to drive it with a PlayStation 5 controller instead.
...
But he reported it responsibly, DJI patched it in two days, and now we get to talk about what actually went wrong.
...
In auth terms: there was authentication, but there was no resource ownership scoping. The token was a skeleton key when it should have been a room-specific keycard.
...
This is sometimes called **Broken Object Level Authorization (BOLA)** — [OWASP's #1 API security risk](https://owasp.org/API-Security/editions/2023/en/0xa1-broken-object-level-authorization/) for good reason. It's the class of vulnerability where an API correctly authenticates a user but then fails to verify that the authenticated user is actually entitled to that specific resource.

The pattern shows up constantly in APIs: you can see your invoice at /invoices/1042, and you just try /invoices/1043 and... that works too. Same concept, much more consequential outcome when the resources are live home camera feeds.
...
If you're building anything that issues tokens to control physical devices — or honestly, anything where one user's data should be invisible to another user — this story is worth reading slowly. (This is an [issue for AI agents](/blog/ai-authorization) as well.)
...
**What's the blast radius of a misconfigured broker or middleware layer?** MQTT and similar pub/sub systems are powerful and flexible. They're also easy to misconfigure in ways that make every subscriber a potential eavesdropper. Default-deny on topic subscriptions is not the norm — it needs to be explicitly configured.
