...
Here, `<YOUR_FUSIONAUTH_URL>` is your fully-qualified domain name, including the protocol, for example, `https://local.fusionauth.io`. `<YOUR_API_KEY>` is the key that you created in the <Breadcrumb>Setting up a FusionAuth API key</Breadcrumb> section, and `<PARENT_USER_ID>` is the "User Id" of the parent user that you created earlier. Record the <InlineFieldValue>Id</InlineFieldValue> value in the response JSON that is returned upon executing the request. This is the Id of the newly created family in FusionAuth. You will need the Id to add the child user to this family.
