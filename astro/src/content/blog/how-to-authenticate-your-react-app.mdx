...
Inside of that config file, you are going to copy the OAuth info from the FusionAuth admin panel of your application, `React Auth`. To do so, on `localhost:9011`, navigate to <Breadcrumb>Applications</Breadcrumb>, then the <Breadcrumb>React Auth</Breadcrumb> configuration, then click the <InlineUIElement>View</InlineUIElement> button.
