...
* Under the same <Breadcrumb>OAuth</Breadcrumb> tab, configure the <InlineField>Logout URL</InlineField> to where FusionAuth should send the user after they (surprise!) log out. Enter in <InlineFieldValue>http://localhost:3000/logout</InlineFieldValue>.  This will ultimately send the user to your React frontend, which will be on port 3000, when they are logged out.
