...
htmlTitle: "How to Migrate from Auth0 to FusionAuth"
description: "Learn how to export users, password hashes, connections, and configs for a smooth transition from Auth0 to FusionAuth. No downtime, no password resets."
authors: Dan Moore
...
* Unlimited tenants
* A truly free option with no limits on the number of users when you self host the Community plan
...
Joe Stech, in ["Why outsource your auth system?"](/blog/why-outsource-auth), wrote:
...
* Learn about different tenancy models: ["Multi-Tenant vs. Single-Tenant IDaaS Solutions"](/articles/identity-basics/multi-tenancy-vs-single-tenant-idaas-solutions)
* Due diligence to perform on any auth vendor: ["Performing Due Diligence on Authentication Vendors"](/articles/identity-basics/due-diligence-authentication-vendors)
* A list of the challenges of customer identity and access management: ["Challenges of CIAM"](/articles/ciam/challenges-of-ciam)
* Don't forget perspectives outside of engineering: ["Outsourcing auth: how to get buy-in from your team"](/blog/outsourced-auth-team-buy-in)
* Auth system scalability challenges: ["Making Sure Your Auth System Can Scale"](/articles/ciam/making-sure-your-auth-system-scales)
...
For both of these reasons, it's best to do a proof of concept. When leaving Auth0 for, say, FusionAuth, you need to map the Auth0 functionality used to the new provider. You can [download FusionAuth](/docs/get-started/download-and-install/docker) and kick the tires without talking to a sales person. 
...
* In Auth0, [Connections](https://auth0.com/docs/authenticate/identity-providers) are a source of data for users. FusionAuth calls these [Identity Providers](/docs/lifecycle/authenticate-users/identity-providers/).
* [Rules](https://auth0.com/docs/customize/rules), [Hooks](https://auth0.com/docs/customize/hooks) and [Actions](https://auth0.com/docs/customize/actions/actions-overview) are ways for you to customize authentication or authorization workflows. FusionAuth has a similar concept called [Lambdas](/docs/extend/code/lambdas/).
* With Auth0, [APIs, Applications and SSO Integrations](https://auth0.com/docs/get-started/auth0-overview/create-applications) are what your users can log in to. They are also called Clients in the Auth0 documentation. FusionAuth refers to these as [Applications](/docs/get-started/core-concepts/types/applications).
* [Tenants](https://auth0.com/docs/get-started/auth0-overview/create-tenants) are a high level construct which groups other entities such as Users and Applications together. FusionAuth calls these [Tenants](/docs/get-started/core-concepts/types/tenants) as well.
* For Auth0, [Roles and Permissions](https://auth0.com/docs/manage-users/access-control/configure-core-rbac/roles) provide information about what your users can do in your custom or off the shelf applications. FusionAuth has [Roles](/docs/get-started/core-concepts/types/roles) as well; they are defined on an Application by Application basis.

One example of a detailed difference that might trip you up, illustrating the importance of a prototype, is that in FusionAuth, users are explicitly mapped to applications with a [Registration](/docs/get-started/core-concepts/types/registrations). Auth0, on the other hand, allows users access to all Auth0 applications within a tenant by default. Such details are why there's no substitute for a proof of concept trialling alternatives you are seriously considering.
...
For much more on moving user data and other migration topics, [please check out the Auth0 migration guide](/docs/lifecycle/migrate-users/provider-specific/auth0).
