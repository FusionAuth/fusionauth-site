...
Each application in FusionAuth can have an [unlimited number of roles](/docs/get-started/core-concepts/types/roles). Each role is a string. Roles can either be marked as default and attached to every user registered to an application, or designated a super-user role, indicating administrative privileges. If you need a lot of roles, or fine-grained permissions in your application, you can create as many roles in FusionAuth as you'd like, perhaps using the API to automate the process.
...
> [Schedule a demo](/contact) to explore how FusionAuth and Cerbos deliver context-aware, dynamic access control.

## Enter an authorization server

Luckily, there are authorization servers such as [Cerbos](https://www.cerbos.dev/) that can help. Cerbos adds a layer of permissions on top of roles provided by FusionAuth. 
