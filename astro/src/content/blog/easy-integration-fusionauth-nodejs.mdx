...
htmlTitle: "Integrate FusionAuth with Your Node.js Application"
description: "Integrate FusionAuth into your Node.js application using Express. This guide covers setup, API key creation, and user authentication steps."
authors: Tom Vilot
image: /img/blogs/easy-integration-fusionauth-nodejs/node-and-fusionauth-example-tiny.jpg
tags: javascript, nodejs, integration, fusionauth
categories: Tutorial
...
To begin, download and Install FusionAuth. See the **Install FusionAuth** section on the [home page](/) for the instructions for each platform. Next, follow the [Setup Wizard](/docs/get-started/download-and-install/setup) tutorial until you get to the **Complete Setup** section, and then return here.
...
![FusionAuth Dashboard](/img/blogs/easy-integration-fusionauth-nodejs/complete-setup.png)

Once you click <InlineUIElement>Setup</InlineUIElement> on the <InlineUIElement>Missing Application</InlineUIElement> task on the dashboard, you'll be taken to the **Add Application** panel.

![Create Application](/img/blogs/easy-integration-fusionauth-nodejs/add-application.png)

Enter your application name, leaving the <InlineField>Id</InlineField> field blank as it will be auto-generated. Add at least one role named <InlineFieldValue>user</InlineFieldValue> by clicking on the <InlineUIElement>Add Role</InlineUIElement> button while on the <Breadcrumb>Roles</Breadcrumb> tab. Click the blue save icon in the top right to add the application. To complete the next task select <Breadcrumb>API Keys</Breadcrumb> from the left navigation sidebar.

![API Key Page](/img/blogs/easy-integration-fusionauth-nodejs/api-keys.png)
...
![Create API Key](/img/blogs/easy-integration-fusionauth-nodejs/add-api-key.png)
...
![Complete Setup Only Email](/img/blogs/easy-integration-fusionauth-nodejs/complete-setup-only-email.png)
...
Note the <InlineField>secret</InlineField> field. This is used to verify the JSON Web Token (JWT). We will talk a little more about that when we look at the console output for the `/login handler`. If we were to implement JWT, we would use a real secret. The secret can be defined either in your Application configuration or the system-wide settings. By default, when you create a new application, no secret is defined and the system-wide configuration is used which is defined in the <Breadcrumb>Settings -> System</Breadcrumb> panel, as shown here:

![System Settings](/img/blogs/easy-integration-fusionauth-nodejs/complete-setup.png)
...
That's all there is to it! Integrating FusionAuth with Node.js is easy and introduces minimal overhead in your code base. If you have any questions about this blog post, please post in the [forum](/community/forum).
