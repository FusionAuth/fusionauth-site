...
Firstly, head to your tenant configuration via the FusionAuth sidebar and edit the default tenant. Set the <InlineField>issuer</InlineField> field to your fully qualified URL (including the protocol). This will be something like `http://localhost:9011`. The issuer signs and issues tokens when users login.
