...
To get started, follow the [Docker install guide](/docs/get-started/download-and-install/docker).

If you already have Docker installed on your system, run the following commands to install and run FusionAuth.
...
The first thing you’ll need to do is set up an application. Click <Breadcrumb>Setup</Breadcrumb> under "Missing Application".
...
Lastly, before hitting "Save" on this page, ensure that "Authorization Code" and "Refresh Token" are checked in the <InlineField>Enabled grants</InlineField> section.
...
Firstly, head to your tenant configuration via the FusionAuth sidebar and edit the default tenant. Set the <InlineField>issuer</InlineField> field to your fully qualified URL (including the protocol). This will be something like <InlineFieldValue>http://localhost:9011</InlineFieldValue>. The issuer signs and issues tokens when users login.
...
Now hit save and navigate to the application configuration for our application and edit it. On the edit page, select the <Breadcrumb>JWT</Breadcrumb> tab and enable JWT by setting the toggle to <InlineFieldValue>Enabled</InlineFieldValue>.

Change the configuration under <Breadcrumb>JSON Web Token settings</Breadcrumb> to have the following values:

- <InlineField>JWT duration</InlineField>: 3600.
- <InlineField>Access Token signing key</InlineField>: Select <InlineFieldValue>Auto generate a new key on save</InlineFieldValue>.
- <InlineField>Id Token signing key</InlineField>: Select <InlineFieldValue>Auto generate a new key on save</InlineFieldValue>.
...
- Set the <InlineField>Host</InlineField> to <InlineFieldValue>smtp.sendgrid.net</InlineFieldValue>
- Set the "Port" to <InlineFieldValue>587</InlineFieldValue>
- Set the "Username" to <InlineFieldValue>apikey</InlineFieldValue> (This should be string "apikey", and not your API key)
- Select the <Breadcrumb>Change password</Breadcrumb> toggle and set the <InlineField>Password</InlineField> to the SendGrid API key that you created
- Change the <InlineField>Default from address</InlineField> to match the Sender Identity you configured
...
Once you’ve saved your configuration, you should be able to successfully send a test email by clicking the <Breadcrumb>Send test email</Breadcrumb> button. If you receive an email that looks like the below, you’re all set!
...
Navigate to your application configuration and toggle <InlineField>Passwordless login</InlineField> to <InlineFieldValue>enabled</InlineFieldValue> under the <Breadcrumb>Security</Breadcrumb> tab.
...
The above component uses the `useSession()` hook provided by NextAuth.js to dynamically render either a <Breadcrumb>Sign in</Breadcrumb> or <Breadcrumb>Sign out</Breadcrumb> button.
...
Once you’ve done this, your application should have a basic sign in and sign out flow. You can test it by navigating to `http://localhost:3000` and clicking <Breadcrumb>Sign In</Breadcrumb>.

![Next.js application with a sign in button](/img/blogs/nextjs-fusionauth-passwordless/nextjs-screen-2.png)

Clicking <Breadcrumb>Sign in</Breadcrumb> will take you to the FusionAuth sign-in form, where you’ll be able to see all the available sign-in options. Click <Breadcrumb>Login with a magic link</Breadcrumb>.
