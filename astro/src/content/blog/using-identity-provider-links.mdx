...
```bash
npm i @fastify/view @fastify/passport @fastify/secure-session @fusionauth/typescript-client ejs passport-oauth2 jose @fastify/multipart envalid twitter-api-v2 qs
...
The `utils.ts` file contains helper functions that we'll use throughout the application.
...
Create a new file in the `src/links` directory called `youtube.ts` and add the following code:
...
Create a new file in the `src/links` directory called `twitter.ts` and add the following code:
