...
* if it's the an agent response with a tool call, then the next step is to call the tools and wait for those responses
...
| Data layer | | |
|------------|-----------|-----------|
| | Primitives | The specification defines a set of JSON-RPC endpoints that can be called to perform [tool calls](https://modelcontextprotocol.io/specification/2025-06-18/server/tools) and access other features. |
...
### The stdio transport (also called "local")

The simplest transport layer to understand is [the "stdio" transport](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#stdio). It gets its name from the way it communicates with the server process. The MCP client will actually start a process on the same machine; the input and output handles from that process become the input and output for the JSON-RPC calls.

### The HTTP streaming transport (also called "remote")

This is actually [the second version](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) of an HTTP transport in the short life  of MCP – the earlier SSE transport was a little more complicated to implement.
...
1. **MCP client to MCP server**
    ...
    2. **MCP client fetches resource metadata from MCP server: **this metadata contains a pointer to the authorisation server.
2. **MCP client to authorisation server**
    3. **MCP client fetches authorisation server metadata: **this contains a lot of information about the OAuth setup, including the endpoint for Dynamic Client Registration.
    4. **MCP client registers new dynamic client: **this gives the client the credentials it can use to start the authorisation process
3. **Authorisation process**
    5. **MCP client sends user to authorisation URL: **this starts the authorisation process
    6. **User completes authorisation**
    7. **Authorisation server redirects back to client: **this is the standard OAuth completion flow
    8. **Client obtains access token**
4. **MCP client to MCP server**
    9. **Client retries the initial request that failed: **this time with the addition of the access token. Assuming everything went smoothly, this request will succeed.
