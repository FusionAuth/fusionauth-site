...
description: "Learn how a token-mediating backend (TMB) is less secure than backend-for-frontend (BFF) and when to use TMB. Part 2 of 3 in the architecture-driven auth series."
...
3. Browse to the frontend at [http://localhost:5173](http://localhost:5173).
   ...
   ![The TMB website homepage showing the logged out state.](/img/blogs/auth-architecture/loggedOut.png)

4. Click the login button at the top right and log in with the test user `user@example.com` and the password `password`.

   ![The TMB website login page with a username and password form.](/img/blogs/auth-architecture/logIn.png)
...
![The TMB website showing the user profile page when logged in.](/img/blogs/auth-architecture/loggedIn.png)
...
You can see the TMB API call logic in [`ResourceApiPage.jsx`](https://github.com/kmaida/auth-architecture/blob/main/tmb/frontend/src/pages/ResourceApiPage.jsx). The `fetchRecipe()` function passes the token to the resource server in the header of fetch calls.
```js
