...
description: "Learn how a token-mediating backend (TMB) is less secure than backend-for-frontend (BFF) and when to use a TMB. Part 2 of 3 in the architecture-driven auth series."
image: /img/blogs/bff/tmb-security-architecture-header.png
authors: Kim Maida, Nathan Contino
...
For more information about TMB, see the Internet Engineering Task Force (IETF) [OAuth 2.0 for Browser-Based Applications](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-browser-based-apps#name-token-mediating-backend) draft.
...
In a TMB frontend, the access token can't be kept in an HTTP-only cookie, because the browser needs the token for calls to resource servers. Since the access token has to be stored in the browser's local storage or application memory, it's vulnerable to exfiltration by XSS attacks, like malicious npm packages or unsanitized HTML inputs.
...
For a list of best practices mitigating the danger of client-side token storage, read the [IETF's Best Current Practice for OAuth 2.0 Security](https://datatracker.ietf.org/doc/html/rfc9700/), or this [article summarizing OAuth 2.0 Best Practices for Developers](https://maida.kim/oauth2-best-practices-for-developers).

### TMB with DPoP

One relatively new technology makes client-side tokens a lot safer — Demonstrating Proof of Possession (DPoP), available as the IETF [RFC 9449](https://datatracker.ietf.org/doc/rfc9449).
...
Your browser stores the DPoP key using [a web cryptography API that prevents exfiltration](https://developer.mozilla.org/en-US/docs/Web/API/CryptoKey/extractable). The browser can sign requests with the DPoP key, but it can't access the key itself.
...
FusionAuth added support for DPoP in [version `1.63.0`](/blog/announcing-fusionauth-1-63). For a great visual walkthrough of the protocol, see the [interactive DPoP site](https://dpop.info).
...
To follow along, you need [Node.js](https://nodejs.org), npm, and [Docker](https://docker.com) installed. While it's not explained here, you could use Deno (or Node in Docker if you know how to change your network settings) instead of Node.
...
This should download Docker images and start FusionAuth on [http://localhost:9011/admin](http://localhost:9011/admin). You can log in with the default admin credentials provided in the README: `admin@example.com` and `password`.
...
2. Open the `tmb/backend/.env` file in a text editor and update the following values with your own Client ID and secrets, found in the FusionAuth Admin UI under <Breadcrumb>Applications -> Your App -> Action -> Edit</Breadcrumb>:
...
3. Browse to the frontend at [http://localhost:5173](http://localhost:5173).
...
Remember that, unlike in the BFF architecture, a TMB frontend needs to forward the access token when calling a resource server. This allows the frontend to call both same-origin and cross-origin APIs directly. You can see this in [`ResourceApiPage.jsx`](https://github.com/kmaida/auth-architecture/blob/main/tmb/frontend/src/pages/ResourceApiPage.jsx). The `fetchRecipe()` function passes the token to the resource server in the header of fetch calls:
```js
...
And if you absolutely must use TMB, combine it with [DPoP (also supported by FusionAuth!)](/docs/lifecycle/authenticate-users/oauth/dpop) to reduce your risk profile.

{/* TODO: Uncomment when part 3 is published.
## Next in this series

The final post in this series discusses the third and least secure OAuth architecture: [Browser-Based OAuth Client (BBOC)](/blog/browser-based-oauth-client-security-architecture). Learn how it works and the rare cases when you might choose to use it.
*/}
...
{/* TODO: Uncomment when part 3 is published. - [Part 3: Browser-Based OAuth Client security architecture](/blog/browser-based-oauth-client-security-architecture) */}
...
- [Evolving Auth Standards: Security Best Practices for App Developers](/webinar/evolving-auth-standards-security-best-practices-for-app-developers)
