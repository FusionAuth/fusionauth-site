...
authors: Dan Moore
image: /img/blogs/what-where-why-mfa/the-what-why-and-when-of-multi-factor-authentication-mfa-header-image.png
...
Welcome to the world of user segmentationa and modeling. It's one of those architectural decisions that seems simple on the surface but has profound implications for your application's user experience, complexity, security, and feature set.
...
There are two main options and a derivative option to consider. The options are:
...
TODO image
![Tenant authentication flow showing user → tenant identification → login]
...
- scales well horizontally, since you can split tenants by server (though this means you have a large number of requests logins)
...
- leave data on the device after a previous, though this does not work for initial logins 
...
TODO
![Organization flow showing user → login → organization selection]
...
- there a common user identity across organizations, including profile data and credentials
...
- an authentication artifact, such as a token, which can be presented to other parts of the application without additional requests
- some standards support, if your identity provider maps applications to OIDC clients.

If you choose this, be aware that the application artifact is a cached value, with all the issues that cached values have:
...
* **Do you need to be able to dynamically add and remove organization membership without re-authentiation?** This feature is a strength of the organization model.

If you answered yes to either of these questions, the organization model is a good choice

Finally, if you want to leverage tokens for authentication without further requests, the application model excels. It also may be more less overall work than the organizations model if applications are mapped to OIDC standard clients.
...
- Reworking other features that rely on characteristics of the user segmentation model. For example, how customer service reps access specific customers' applications.

This decision isn't set in stone, but changes become more expensive as your user base grows.
