...
Single sign-on (SSO) is a session and user authentication technique that permits a user to use one set of login credentials to authenticate with multiple apps. [SSO works by establishing trust between a service provider, usually your application, and an identity provider, like FusionAuth](https://fusionauth.io/articles/authentication/how-sso-works).

{/* more */}

## Quick Overview: Next.js SSO with OIDC

### What You'll Build
In this tutorial, you'll create a Next.js application integrated with FusionAuth as the OIDC provider and NextAuth.js for session management. By the end, you'll have a fully functional SSO implementation that allows users to authenticate once and access your application seamlessly.

### How Do You Add Single Sign-On (SSO) with OpenID Connect (OIDC) in a Next.js application?

1. **Set up your IdP** (e.g., FusionAuth). → [Jump to Installing FusionAuth](#installing-fusionauth-docker)
2. **Choose a client in Next.js.** → [Jump to Create the Next.js application](#create-the-nextjs-application)
3. **Install and configure NextAuth** (recommended). → [Jump to Installing NextAuth.js](#installing-nextauthjs)
4. **Enable Authorization Code + PKCE.** → [Jump to Set up the application](#set-up-the-application)
5. **Protect pages and APIs.** → [Jump to Protecting Pages and Managing Sessions](#protecting-pages-and-managing-sessions-nextauth-sessionprovider)
6. **Test locally, then deploy.** → [Jump to Testing SSO End-to-End](#testing-sso-end-to-end)
...
SSO helps reduce password fatigue by requiring users to only remember one password and username for all the applications managed through the SSO feature. This also reduces the number of support tickets created for your IT team when a user inevitably forgets their password. In addition, SSO minimizes the number of times you have to key-in security credentials, limiting exposure to security issues like keystroke probing and exposure. Security is also enhanced by SSO because you can implement [additional functionality such as MFA](https://fusionauth.io/articles/authentication/multi-factor-authentication) or anomalous behavior detection at the identity provider without adding any complexity to your application.

In this tutorial, you'll learn how to design and implement SSO using [Next.js](https://nextjs.org/), a popular React-based framework for JavaScript and [FusionAuth as the OIDC provider](https://fusionauth.io/docs/lifecycle/authenticate-users/identity-providers/overview-oidc). Any other OIDC compatible authentication server should work as well.
...
## Implementing SSO in a Next.js web app: Prerequisites

Before you begin, you'll need the following:

* A Linux machine. The step-by-step instructions in this article are based on a [CentOS Linux machine](https://www.centos.org). If you want to work on a different OS, check out this [setup guide](/docs/quickstarts/5-minute-setup-guide) for more information. [There are detailed setup guides in the documentation.](https://fusionauth.io/docs/get-started/)
* [Docker Engine](https://docs.docker.com/engine/) and [Docker Compose](https://docs.docker.com/compose/).
* [Node.js](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm). 
* Experience with [Next.js](https://nextjs.org/) framework and application development. Follow the [Next.js Quickstart](https://fusionauth.io/docs/quickstarts/quickstart-javascript-nextjs-web) to scaffold a working app and integrate authentication step by step.
* The [Git client tool](https://git-scm.com/downloads) _(optional)_.

## Installing FusionAuth (Docker)
...
### Configure FusionAuth for OIDC and SSO

Now you need to configure FusionAuth by creating an application and registering the user.

[This setup allows users in FusionAuth to sign in to the Next.js application automatically once they are authenticated by FusionAuth.](https://fusionauth.io/docs/lifecycle/authenticate-users/single-sign-on)
...
* Add the value `http://localhost:3000/api/auth/callback/fusionauth` to the <InlineField>[Authorized redirect URLs](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/endpoints)</InlineField> field. This will be used by FusionAuth to redirect the user to your application page once the user is successfully authenticated.
* Scroll down and check the "[Require registration](https://fusionauth.io/docs/get-started/core-concepts/registrations)" checkbox. This ensures that users who haven't been registered for this application in FusionAuth aren't able to access it when using the hosted login pages.
...
Review FusionAuth [OAuth endpoints](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/endpoints) you'll use for authorize, token, and logout.
...
Now that you have the application running, let's [implement the authentication process](https://fusionauth.io/docs/lifecycle/authenticate-users/oauth/) by using [NextAuth.js](https://next-auth.js.org), a complete open-source solution.
...
Understand access, ID, and refresh tokens in FusionAuth: [Tokens & refresh](https://fusionauth.io/articles/tokens/).

### Protecting Pages and Managing Sessions (NextAuth SessionProvider)
...
Enter the correct credentials created when you set up FusionAuth and submit the form. You'll arrive back at your Next.js application home screen, [with your email address displayed and a the <InlineUIElement>Log out</InlineUIElement> button.](https://fusionauth.io/docs/lifecycle/authenticate-users/logout-session-management)

![The nextjs application after logging in.](/img/blogs/nextjs-single-sign-on/after-login.png)

## Troubleshooting NextAuth + FusionAuth

Here are common issues and solutions when integrating NextAuth with FusionAuth:

* **Signed out of NextAuth but still see FusionAuth session:** Ensure you call the FusionAuth `/oauth2/logout` and consider revoking app sessions in admin or via API. [See this forum thread for details](https://fusionauth.io/community/forum/topic/2926/nextauth-signout-and-revoking-app-sessions).
* **Callback URL mismatch errors:** Verify the redirect URL in FusionAuth matches exactly with your NextAuth configuration (including protocol and port).
* **Token expiration issues:** Configure appropriate token lifetimes in FusionAuth Application settings and implement refresh token rotation if needed.
* **Session not persisting:** Check your `NEXTAUTH_SECRET` is properly set and consistent across deployments.

## Next Steps: Theming, OAuth, and Hardening
...
* Browse all [Quickstarts](https://fusionauth.io/docs/quickstarts/) or [Download FusionAuth](https://fusionauth.io/download)
