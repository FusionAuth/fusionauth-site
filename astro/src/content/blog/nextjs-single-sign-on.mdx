...
Single sign-on (SSO) is a session and user authentication technique that permits a user to use one set of login credentials to authenticate with multiple apps. [SSO works by establishing trust between a service provider, usually your application, and an identity provider, like FusionAuth](/articles/authentication/how-sso-works).
...
* A Linux machine. The step-by-step instructions in this article are based on a [CentOS Linux machine](https://www.centos.org). If you want to work on a different OS, check out this [setup guide](/docs/quickstarts/5-minute-setup-guide) for more information. [There are detailed setup guides in the documentation.](/docs/get-started/)
* [Docker Engine](https://docs.docker.com/engine/) and [Docker Compose](https://docs.docker.com/compose/).
* [Node.js](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm). 
* Experience with [Next.js](https://nextjs.org/) framework and application development. Follow the [Next.js Quickstart](/docs/quickstarts/quickstart-javascript-nextjs-web) to scaffold a working app and integrate authentication step by step.
...
[This setup allows users in FusionAuth to sign in to the Next.js application automatically once they are authenticated by FusionAuth.](/docs/lifecycle/authenticate-users/single-sign-on)
...
* Add the value `http://localhost:3000/api/auth/callback/fusionauth` to the <InlineField>[Authorized redirect URLs](/docs/lifecycle/authenticate-users/oauth/endpoints)</InlineField> field. This will be used by FusionAuth to redirect the user to your application page once the user is successfully authenticated.
* Scroll down and check the "[Require registration](/docs/get-started/core-concepts/registrations)" checkbox. This ensures that users who haven't been registered for this application in FusionAuth aren't able to access it when using the hosted login pages.
...
Review FusionAuth [OAuth endpoints](/docs/lifecycle/authenticate-users/oauth/endpoints) you'll use for authorize, token, and logout.
...
Now that you have the application running, let's [implement the authentication process](/docs/lifecycle/authenticate-users/oauth/) by using [NextAuth.js](https://next-auth.js.org), a complete open-source solution.
...
Understand access, ID, and refresh tokens in FusionAuth: [Tokens & refresh](/articles/tokens/).
...
Enter the correct credentials created when you set up FusionAuth and submit the form. You'll arrive back at your Next.js application home screen, [with your email address displayed and a the <InlineUIElement>Log out</InlineUIElement> button.](/docs/lifecycle/authenticate-users/logout-session-management)
...
* **Signed out of NextAuth but still see FusionAuth session:** Ensure you call the FusionAuth `/oauth2/logout` and consider revoking app sessions in admin or via API. [See this forum thread for details](/community/forum/topic/2926/nextauth-signout-and-revoking-app-sessions).
...
* Browse all [Quickstarts](/docs/quickstarts/) or [Download FusionAuth](/download)
