...
Next let's set up an API Key. It's fast and only takes a moment. Navigate to <Breadcrumb>Settings</Breadcrumb> in the dashboard and then click on "API Keys". Then select the green plus sign at the top right area of the page to "Add" a new API Key.
...
Next, we'll need to add our API Key and URL for the FusionAuth server. Yours will be different. If you don't remember your API Key, navigate to <Breadcrumb>Settings</Breadcrumb> and then to "API Keys".
