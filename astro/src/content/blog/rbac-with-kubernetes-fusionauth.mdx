...
First, visit the <Breadcrumb>Tenants</Breadcrumb> page from the sidebar and edit the <Breadcrumb>Default</Breadcrumb> tenant. Change the **Issuer** field from the default value of `acme.com` to `https://fusionauth.local` and save the changes. Kubernetes will validate the Issuer field during authentication, so it's vital that this is the same as the FusionAuth domain.
...
In the <Breadcrumb>OAuth</Breadcrumb> tab, add `http://localhost:8000` and `http://localhost:18000` in the **Authorized redirect URLs** field. These URLs are used by the `oidc-login` Kubernetes plugin to redirect the user after authentication.
...
Click the <Breadcrumb>Add registration</Breadcrumb> button, select the `Kubernetes` application and the `admin` role, and save the registration.
