...
<!-- NOTICE: INSERT ARCHITECTURE / FLOW DIAGRAM HERE
     Placement: between the low/medium/high classification paragraph (above) and the Auth0 comparison paragraph (below).
     Depicts: login → 10 signals scored inside your deployment → composite LOW/MEDIUM/HIGH → policy → challenge or pass.
     Source to improve via creative design: inputs/intelligent-mfa-solution-brief-diagram/intelligent-mfa-arch-flow-diagram-solution-brief-input-enhanced.svg
     Replace this notice with the final image once design is complete, e.g.:
     ![Intelligent MFA scores each login inside your deployment and challenges only risky sessions](/img/blogs/intelligent-mfa/intelligent-mfa-arch-flow.svg)
-->
...
Still evaluating? Get a real [demo](/contact) with two logins scored by the risk engine — one clean, one suspicious — and watch exactly where it challenges and where it doesn't. That's what you bring back to your security team.
