...
<Aside type="tip">
  The easiest way to run FusionAuth is in a Docker container. Clone the [fusionauth-example-docker-compose](https://github.com/fusionauth/fusionauth-example-docker-compose) GitHub repository. Open a terminal in the `light` subdirectory, and run `docker compose up` in a terminal to start FusionAuth. Log in at http://localhost:9011 with <InlineFieldValue>admin@example.com</InlineFieldValue> and <InlineFieldValue>password</InlineFieldValue>. The example repository will use the sample [Kickstart file](https://github.com/FusionAuth/fusionauth-example-docker-compose/blob/main/light/kickstart/kickstart.json) to configure the FusionAuth instance.
</Aside>
