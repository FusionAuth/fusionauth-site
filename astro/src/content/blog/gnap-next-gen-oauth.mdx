...
In some cases, the <InlineField>key</InlineField> attribute has the value <InlineFieldValue>true</InlineFieldValue>; in this case the token is bound to the key of the client previously presented to the authorization server.

Are all GNAP tokens sender constrained? Fear not! Bearer tokens still work. The draft still has that concept, which has been so successful in the OAuth2 ecosystem. If you want a bearer token, set <InlineField>key</InlineField> to <InlineFieldValue>false</InlineFieldValue>:
