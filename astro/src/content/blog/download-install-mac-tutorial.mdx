...
htmlTitle: "Install FusionAuth on macOS: Quick Setup Guide"
description: "Get FusionAuth running on macOS in minutes. Follow step‑by‑step instructions to download, unzip, install FusionAuth App & Search, and complete setup in-browser."
authors: Brian Pontarelli
image: /img/blogs/macos-install/macos-install.jpg
categories: Tutorial
tags: macos, fusionauth, download
...
Refer to the latest documentation for [System Requirements](/docs/get-started/download-and-install/reference/system-requirements "Visit FusionAuth System Requirements") for minimum versions. If you don't yet have a database installed or have one accessible, do this first before you continue.

## Installation Options

The easiest way to get up and running on macOS will be to use the [Fast Path](/docs/get-started/download-and-install/fusionauth-app "Fast Path") installation. This method will download the zip packages and unzip them in the current working directory and have you up and running quickly.
...
Start by heading to the [Download](/docs/get-started/download-and-install/docker) page.
...
If you'd like to read additional information on the different packages found on the Download page, see the [Packages](/docs/get-started/download-and-install/reference/packages) section of the installation guide.

We will be downloading both the `fusionauth-app` and the `fusionauth-ssearch` bundles. To begin, once your on the  [Download](/docs/get-started/download-and-install/docker) page click on the file you'd like to download and select a download location. We will refer to your download location as `FUSIONAUTH_HOME`, in this example we'll use `~/fusionauth`.
...
- [Setup Wizard](/docs/get-started/download-and-install/setup)
