...
Requests made with client libraries usually require an API key as well as a URL for the FusionAuth instance which the requests will be applied to.

### Better Together?

You can use both an SDK and a client library if your use case requires the front-end SDK-based solutions for easy integration and the management access provided by a client library. They are not mutually exclusive.
...
For SPAs which must request APIs that don't live on the same domain the SPA is served from, cookies won't work. In this situation, the backend for frontend pattern is recommended. **You should not put bearer tokens into local storage or session storage.** Here's a video about the insecurity of storing tokens anyplace accessible to JavaScript running in the browser.
