...
Pretty much anything which can use OpenID Connect (OIDC), Security Assertion Markup Languae (SAML), or an HTTP based API should work. You can see our [list of quickstarts](/docs/quickstarts/) for integration examples.
...
This post will cover the nitty gritty of using these SDKS, but first, let's discuss the difference between an SDK and a [client library](/docs/v1/tech/client-libraries/).

## Client Libraries vs SDKs

At FusionAuth, a client library is different than an SDK, even though many people in the industry use the terms interchangably.

### Client Libraries

A client library is a wrapper over [FusionAuth APIs](/docs/v1/tech/apis/). Client libraries support all public FusionAuth API endpoints, and offer a convenient way to use that API in many software languages. At time of writing, there are eight supported languages, including TypeScript. There's also an OpenAPI definition, useful for viewing API changes over time or generating a client library for an unsupported language like C++ or Rust.

If you want to compare a client library to a child's toy, it is like tinker toys. Engineers build a script or program using a client library to extend or configure FusionAuth. Some examples of tasks well suited to a client library:
...
Requests made with client libraries usually require an API key as well as a URL for the FusionAuth instance to be configured.

### SDKs

An SDK, in contrast to a client library, is an opinionated collection of abstracted functionality. To continue with the toy metaphor, an SDK is a doll or a toy truck. Less assembly, but less flexibility. SDKs are designed to allow front-end engineers add authentication to their application.
...
SDKs are configured to work with a a specific FusionAuth application and therefore expect a client Id as well as a FusionAuth instance URL. The SDKs do not offer management functionality and therefore don't require an API key.

### Better Together?

If your use case requires the front-end SDK based solutions for easy integration as well as the management access provided by a client library, you can use both. They are not mutually exclusive.
...
The server side component has endpoints that map to the [hosted backend APIs](/docs/v1/tech/apis/hosted-backend). You can implement the server side component in any other language if that's preferable. The server side code sets access tokens and refresh tokens as `HTTPOnly`, `Secure` cookies. This is a great storage choice for first party API requests, because such cookies are not vulnerable to JavaScript exfiltration. The cookies are presented on every request from the SPA to an API, as long as you set the [`credentials` property correctly](https://developer.mozilla.org/en-US/docs/Web/API/Request/credentials) and the API is in the same domain that the SPA is served from. For this approach to work, APIs have to be designed to read the cookie, though they can also fall back to the `Authorization` header; you can see examples in our [API quickstarts](/docs/quickstarts/#api). 
For SPAs which make requests to APIs not on the same domain, the backend for frontend pattern is recommended. You should not put bearer tokens into local storage or session storage. Here's a video about the insecurity of storing tokens anyplace accessible to JavaScript running in the browser.

<YouTube id="2nVYLruX76M" />

You can read more about [OAuth token storage in this article](/articles/oauth/oauth-token-storage). Using a server side component gives you flexibility to store the token wherever makes sense.
...
When you use the Authorization Code grant for authentication, as well as redirects for registration and logout, you are leveraging FusionAuth provided functionality. Your application immediately supports a large number of authentication workflows. These include, but are not limited to:
...
When using a redirect, the only application which sees or holes sensitive user credentials is FusionAuth. Your application code never has access to these, which decreases the attack surface area.
...
Additional funtionality and security are great, but you typically want your login pages to look like your application. Doesn't a browser redirect make that difficult?

Well, no. FusionAuth allows complete control over the look and feel of the hosted login pages, including adding or removing HTML, CSS, images and JavaScript to the pages. Learn more about this functionality, which we call [themes](/docs/v1/tech/themes/), including an [example using Tailwind CSS](/docs/v1/tech/themes/tailwind).

One other common objection to redirection is the ability to send a successfully authenticated user right back to where they started. It's common to allow user to log in from multiple states in your SPA, after all. Luckily, you can use the `state` parameter to deep link from any page in your SPA to FusionAuth and then send the user right back to that page after a successful login. Here's a video displaying that:

<YouTube id="-vx5rdy-mvY" />

Here's code from an [example application](https://github.com/FusionAuth/fusionauth-example-node-deeplink/) which extracts a URL from a `state` parameter and returns the user to that URL after login.
...
If your application or design team cannot handle redirects, you can use the [Login API](/docs/v1/tech/apis/login) to directly pass user credentials to FusionAuth. 

You'll miss out on the functionality and security benefits above, however.

At the curent time there's no SDK support with this approach, but FusionAuth users can and have built out their own user interface using the Login API.

## In Conclusion

Using the Angular, React or Vue SDKs can accelerate your app's integration with FusionAuth.

By doing so, you can focus on the features you users actually care about while leveraging a world-class implementation of undifferentiated login workflows and security of an isolated authentication server.
