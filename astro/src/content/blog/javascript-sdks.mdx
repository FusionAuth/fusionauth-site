...
At FusionAuth, a client library is different from an SDK, even though many people in the industry use the terms interchangeably.
...
An SDK, in contrast to a client library, is an opinionated collection of abstracted functionality. To continue with the toy metaphor, an SDK is a doll or a toy truck. Less assembly, but less flexibility. SDKs are designed to allow front-end engineers to add authentication to their application.
...
* forgot password
* email verification
* [MFA](/docs/v1/tech/guides/multi-factor-authentication)
* [magic link authentication](/docs/v1/tech/passwordless/magic-links)
* password expiration
* [enterprise single sign-on (SAML, OIDC)](/docs/v1/tech/identity-providers/#enterprise-identity-providers)
* [social sign-on (Google, Facebook, etc)](http://localhost:4000/docs/v1/tech/identity-providers/#social-identity-providers)
* [profile management](/docs/v1/tech/account-management/) (with a paid license)
* [passkeys](/docs/v1/tech/admin-guide/webauthn) (with a paid license)
* and more
...
Here's code from an [example application](https://github.com/FusionAuth/fusionauth-example-node-deeplink/) which extracts a URL from a `state` parameter and returns the user to that URL after a successful exchange at the token endpoint.
...
If your application architecture, use case, or design team rules out redirects, you can use the [Login API](/docs/v1/tech/apis/login) to directly pass user credentials to FusionAuth. You'll miss out on the functionality and security benefits outlined above, however.

At the current time there's no SDK support for this approach, but FusionAuth users can and have built out their own user interface using this API.
