...
FusionAuth is an authentication gateway that can be cloud-hosted or self-hosted (where you can use a free-tier with most features websites commonly need). It uses a single tenant design.
...
In any authentication system [the bottleneck is hashing passwords](https://fusionauth.io/articles/ciam/making-sure-your-auth-system-scales), both for registration and login. Hashing is intentionally slow, in order to make attacks on a user's password take longer. Since FA is stateless, you can easily scale it horizontally by adding more instances of the Java application (usually in Docker), all accessing the same the Postgres database. Even back in 2019, [FA could support 100 million users](https://fusionauth.io/blog/got-users-100-million) on one database.




Use Ory as multi-tenant test.
FusionAuth as single.
fusionauth multi vs single tenant?
register a million users. login thousands afap. test cpu, ram, disk load, latency, total time taken. use go, multithreading.


use docker - set cpus and memory
