...
- **FusionAuth's single-tenant architecture** delivers predictable performance, high security, and full control over user data, while allowing logical multi-tenant partitioning within a single instance.  
  [Read more](/blog/single-tenant-scaling)
- **Horizontal scaling:** FusionAuth can easily scale by adding more server instances because it is stateless. Performance remains consistent even with tens of millions of users when properly resourced.  
  [More details](/blog/single-tenant-scaling)
- **Robust authentication:** CPU-intensive password hashing is a built-in security feature. Spikes in user activity can impact performance, so planning for peak loads is essential.  
  [Details](/single-tenant-scaling)
- **Multi-tenancy flexibility:** You can use multiple tenants (distinct user spaces) for different applications, environments (e.g., dev, QA, prod), or client domains, without sacrificing underlying system isolation.  
  [Architecture explanation](/blog/multi-tenancy-in-a-single-tenant-architecture)
- **Resource sharing:** Each tenant shares the infrastructure. However, total processing capacity is split between tenants, potentially decreasing speed per tenant in high-load scenarios (“noisy neighbor” effect).  
  [Learn more](/docs/extend/examples/multi-tenant)
- **Scalability guidance:** Monitor for resource limits (CPU, RAM, bandwidth), plan for spikes, and consider dedicated hardware for mission-critical applications.  
  [Best practices](/docs/extend/examples/multi-tenant)
- **Self-hosting and cost control:** FusionAuth supports cloud and self-hosted deployments with transparent pricing. Scaling up or down is straightforward, enabling cost optimization as your business needs change.  
  [Pricing discussion](/blog/single-tenant-scaling)

### Definitions

- **Single-tenancy:** Each customer or organization gets a dedicated, isolated instance of the software, improving security and control.
- **Multi-tenancy (logical):** Multiple customer spaces (“tenants”) exist within one software instance. Data and configuration are logically separated but share resources.
...
**Q: What risks are associated with shared hosting?**  
*Shared hosting is more affordable but poses risks of noisy neighbors and possible outages if other tenants overload resources. Dedicated deployments avoid these issues.*  
[Resource sharing explained](/docs/extend/examples/multi-tenant)

***

**Key links for further information:**
- [FusionAuth: Scale a Single-Tenant Deployment](o/blog/single-tenant-scaling)
