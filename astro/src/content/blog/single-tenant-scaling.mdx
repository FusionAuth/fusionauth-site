...
The accepted definition of **multi-tenancy** is when **a single instance of software runs on a single server and serves multiple tenants**. Consider a project management software website, like Trello or Redmine. The website is multi-tenant if it can serve multiple customers (companies running projects) from one server. A designer might even work on two different projects at two different companies, both using the same project management software. She would be seen as a completely different user under each company (tenant), even though she might log into both companies with the same email address.
...
To explore the benefits of single-tenant solutions yourself, use the FusionAuth [pricing calculator](/pricing) to compare how fees change at different loads and hosting types, and read about how one client saved thousands of dollars [self-hosting their own authentication ](/blog/currentdesk-saved-with-fusionauth)gateway.

## Further Reading

- [How authentication systems scale](/articles/ciam/making-sure-your-auth-system-scales)
- [FusionAuth can support 100 million users](/blog/got-users-100-million)
