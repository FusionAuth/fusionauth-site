...
title: "Custom Scopes, Third Party Applications, And FusionAuth"
description: 'How to use custom scopes in a third party application with FusionAuth.'
authors: Mark Robustelli
image: /img/blogs/custom-scopes-in-third-party-applications/custom-spcopes-blog-header.png
categories: Tutorial
tags: scopes, oauth
...
What are third party applications and custom scopes? How would I implement them using FusionAuth?

{/* more */}

In today's world of API enabled applications, it is very common to open up the APIs of your application to other applications. Not just open the APIs to other applications that you and your team have created but open the APIs to applications created by other entities. These applications that consume your APIs are known as third party applications. 

You will want to ensure the API calls to your application are secured. This is where scopes come into play. Similar to roles for a user, custom scopes can be created for applications. This ensures your APIs are only being called by applications you grant access to.
...
Change Bank has [existing APIs](https://github.com/FusionAuth/fusionauth-quickstart-ruby-on-rails-api) for their application. They provide methods for making change and alerting the authorities of an incident should it occur. The owner of the up-and-coming Budget Buddy application has reached out to the owner of Change Bank. He has asked if it would be possible to integrate with Change Bank by allowing Budget Buddy users to see their Change Bank balance in the Budget Buddy application. They agree and now Budget Buddy has become a third party application for Change Bank.
...
When the user allows the consents, Budget Buddy will be able to read the user's Change Bank balance.
...
It can be a little confusing with the screen shots all showing the local host due to the way Safari renders the URL. In the example application, there are actually two applications (Change Bank API / Budget Buddy Web App) and FusionAuth is the IdP for the Change Bank applications. In a real world example, Change Bank would likely have a web application as well. A diagram of what that may look like would be as follows: 
<LoginArchitecture />

It should be clear that Budget Buddy is its own application with its own identity provider. In the case that it requests the users Change Bank balance, the Change Bank login flow would be initiated against the Change Bank IdP. The user would then login and grant Budget Buddy the necessary consents to retrieve the balance.
