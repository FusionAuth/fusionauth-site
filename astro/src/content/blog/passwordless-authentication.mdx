...
Tech luminaries have been prophesizing the imminent death of the password for the past 20 years. Or so I've been led to believe. If you Google the phrase "the password is dead", you'll find an endless list of articles pointing to the 2004 RSA Conference, where Bill Gates apparently predicted the password's sudden demise. A death that, as Mark Twain would have put it, has been greatly exaggerated.
...
Password rules have become so comically complex that developers make [Kafkaesque games about choosing passwords](https://neal.fun/password-game/). There is an all-too-familiar feeling called [password fatigue](https://en.wikipedia.org/wiki/Password_fatigue) that has a dedicated Wikipedia page!
...
YubiKeys also play a more direct role in the transition to passwordless authentication, as they can, starting with YubiKey 5 series, create and store passkeys. There is a catch, though.
...
Furthermore, these passkeys are hardware-bound. Once a YubiKey generates a passkey, the FIDO2 credentials can neither be deleted nor copied - the space is sacrificed indefinitely.
...
The main difference between these two lies in the fact that for non-resident keys, users need to provide their username to start the authentication process, and the server then sends a challenge that matches with the stored public key. In the case of resident keys, the user doesn’t have to provide a username. Instead, the key can be identified directly by the server, which makes the whole process smoother and more simplified.
...
However, this process does raise a few questions about hardware restrictions and the capacity to store keys. As discussed earlier, current models of security keys like Yubico’s have a limitation on the number of resident keys they can store. Once maxed out, users would need a second security key for additional accounts. It's a limitation crucial to keep in mind when considering deploying passwordless authentication, but it shouldn't impact those who use it for personal accounts or even professionals handling a limited number of prioritized accounts. As technology continues to advance rapidly, it’s likely these storage limitations will expand over time as well.

Unfortunately, the decision whether to use resident versus non-resident keys isn't up to the user. Web services that offer passwordless authentication will determine the key type used based on whether they require a username to log in.
...
While the convenience of passwordless authentication methods cannot be overstated, there are scenarios where these methods might not be the ideal solution. One of the quintessential examples involves users with limited access to up-to-date technology. For individuals using dated hardware or software, the compatibility issues with newer web standards like WebAuthn can pose a significant challenge. In such cases, traditional password-based authentication remains the most accessible and reliable method.
...
While we are still a considerable distance away from passwords being completely "dead", we are certainly making promising strides towards relying less on passwords. Let's proceed with the cautious optimism that suits any emerging technology.

Until the password is in the grave, keep choosing strong passwords, think twice before entering your password after clicking a link, and make sure you set up multifactor authentication on any service that allows it. Remind others that a passwordless world is an ideal, but by no means a reality.