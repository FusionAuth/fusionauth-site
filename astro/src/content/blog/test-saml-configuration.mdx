...
You will need an instance of FusionAuth up and running. You can do so using [Docker](/docs/get-started/download-and-install/docker), using [Homebrew](/docs/get-started/download-and-install/fusionauth-app), or [a number of other methods](/docs/get-started/download-and-install/docker).

### 1. Create A New Application Named IAMShowcase Example

Log into FusionAuth and select <Breadcrumb>Applications</Breadcrumb>. Give the application a **name** such as `IAMShowcase Example`. If you have more than one tenant, select the default **Tenant**. Then click the blue disk to save the configuration.
...
If you are having trouble, set the **Debug Enabled** toggle to `true` on the <Breadcrumb>SAML</Breadcrumb> tab. This outputs more information to help you troubleshoot your configuration. You can find the logged information at <Breadcrumb>Logs -> Event Log</Breadcrumb> in the FusionAuth Admin application.
