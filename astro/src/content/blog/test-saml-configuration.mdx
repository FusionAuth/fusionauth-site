...
This is the entityID from the EntityDescriptor node in the matadata provided by IAMShowcase.
```
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" entityID="IAMShowcase" validUntil="2025-12-09T09:13:31.006Z">
```

<InlineField>Authorized Redirect URLs</InlineField> `https://sptest.iamshowcase.com/acs`
This is the Location from the AssertionConsumerService node in the matadata provided by IAMShowcase.
```
<md:AssertionConsumerService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="https://sptest.iamshowcase.com/acs" index="0" isDefault="true"/>
```
...
Select <InlineUIElement>Users</InlineUIElement>. Choose a user you wnat to test the SAML configuration with. Select <InlineUIElement>Manage</InlineUIElement>. <InlineUIElement>Add registration</InlineUIElement> for the IAMShowcase Example application for the user.
...
Click on <Breadcrumb>Applications</Breadcrumb>. Select <InlineUIElement>View</InlineUIElement> for `IAMShowcase Example`. Scroll down to the <Breadcrumb>SAML v2 integration details</Breadcrumb> section. Copy the Metadata URL and paste that into a new browser window to get the XML metadata.
...
### See The SAML Workflow In Action
