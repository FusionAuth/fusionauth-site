...
Here's how you can run the kickstart with Docker Compose.
...
1. **Database** - A PostgreSQL instance to store your data.
...
You can further customize the services by editing the `docker-compose.yml` and `.env` files. FusionAuth supports various deployment methods (e.g., Kubernetes, Helm), making it adaptable to different environments. You can learn more about working in a development environment [here](https://fusionauth.io/docs/get-started/download-and-install/development/).
