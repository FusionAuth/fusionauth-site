...
After that, initialize firebase emulators:
...
At FusionAuth, we offer a [kickstart feature](/docs/get-started/download-and-install/development/kickstart) that allows you to build a template that can replicate a development or a production environment.
...
The [Kickstart](/docs/get-started/download-and-install/development/kickstart) is a JSON file containing instructions regarding the environment setup. In the testcase we are setting up, we need to create a user. Here is how the kickstart file would look like:
...
In the code above, variables are declared, to avoid repetition. After that, an API key is defined. Then, there is a series of requests that will be executed to:
...
Edit these files to match your environment. In the `.env` file, you'll want to modify the `DATABASE_PASSWORD` and ensure the `POSTGRES_USER` and `POSTGRES_PASSWORD` are set correctly. Also add the `FUSIONAUTH_APP_KICKSTART_FILE` environment variable with the path to the kickstart file along with an entry to attach the kickstart directory as a volume on the `fusionauth` service.
...
APPLICATION_ID = "e9fdb985-9173-4e01-9d73-ac2d60d1dc8e"  # Replace with your actual FusionAuth Application ID
API_KEY = "33052c8a-c283-4e96-9d2a-eb1215c69f8f-not-for-prod" # Replace with your actual FusionAuth API_KEY ID
