...
While the commands above use the [default tenant](https://fusionauth.io/permify-docs/getting-started/quickstart#configuring-schema-via-api), `t1`, if you have a multi-tenant system, you need to set your tenant ID in every call.
...
To write the call in other languages, like Go and JavaScript, see the [API example code](https://fusionauth.io/permify-docs/api-reference/permission/bulk-check).
...
In simple cases, even when checking multiple permissions for a single user, the older API queries may be more useful than bulk permissions. For example, if you want a list of all the records Alice can view, it's not practical to list all the records in the database in your query. Rather run the appropriate [lookup call](https://fusionauth.io/permify-docs/api-reference/permission/lookup-entity):
