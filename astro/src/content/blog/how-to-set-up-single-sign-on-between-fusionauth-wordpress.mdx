...
* In the **Userinfo Endpoint URL** field, put `http://your-fusionauth-domain/oauth2/userinfo`. However since we're running in a Docker container, WordPress can't reach FusionAuth through `localhost`, so the URL should be: `http://fusionauth:9011/oauth2/userinfo`
