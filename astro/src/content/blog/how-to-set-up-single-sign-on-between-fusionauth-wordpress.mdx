...
Go to your WordPress Admin Dashboard at `localhost:8030/wp-admin` and click on the <Breadcrumb>Plugins</Breadcrumb> page from the sidebar. Click on the "Add New" button, and on the next page, search for "OpenID Connect Generic".
...
To enable user registration, click on the <Breadcrumb>Registration</Breadcrumb> tab and turn on "Self service registration." Keep the default settings unchanged.
