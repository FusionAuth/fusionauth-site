...
For FusionAuth to work in this case we have a custom MiddleWare in Traefik just to set `x-forwarded-proto` to <InlineFieldValue>https</InlineFieldValue> all the time (because there cannot be unauthenticated requests anyway).
