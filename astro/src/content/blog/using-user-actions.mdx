...
To create a user account, navigate to <Breadcrumb>Users</Breadcrumb> and click the "Add" button. Then supply an email address. You can untoggle the "Send email to set up password" switch to supply a password.
...
To do so, navigate to <Breadcrumb>Settings</Breadcrumb> then "API Keys" and click the "Add" button. Make sure `POST` is enabled for the `/api/user-action` and both `POST` and `GET` are enabled for the `/api/user/action` endpoints. You will use the former to create a User Action and the latter to execute it.
...
Navigate to the <Breadcrumb>Default</Breadcrumb> tenant and click on the "Edit" button. Scroll down and make sure the `user.action` event is enabled here too.
