...
After the AS recognizes the MCP client, the user authenticates.

The authentication process can take any form satisfies the AS: password, passkey, third-party single-sign on (SSO), multi-factor authentication (MFA).
...
As a result, an attacker can't trick an LLM into directly revealing the credential via, say, prompt injection.
...
If the AS issued a JWT as an access token, the MCP server should also follow [JWT best practices like verifying the signature](/articles/tokens/building-a-secure-jwt).
