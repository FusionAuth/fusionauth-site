...
If you want to follow along, it will be very helpful to go through the [5-Minute Setup Guide](/docs/quickstarts/5-minute-setup-guide) first, as that will set up the needed users and roles in FusionAuth.
...
Head over to the [5-Minute Setup Guide](/docs/quickstarts/5-minute-setup-guide) for FusionAuth. During setup, the application that you configure will be linked to our gateway application, so you can name it "Gateway". Note that while FusionAuth supports multi-tenant configurations, here you're setting everything up in the default tenant.
...
Using your browser (to leverage our OAuth credentials present because we signed in), go to `http://localhost:3000/branches/1/products`. This works because the user you set up in the [5-Minute Setup Guide](/docs/quickstarts/5-minute-setup-guide) has an <InlineFieldValue>admin</InlineFieldValue> role in FusionAuth, and we're passing that role as a header to the Product Inventory service.
