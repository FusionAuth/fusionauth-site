...
htmlTitle: "How to Build a Microservices Gateway with FusionAuth"
description: "Set up an API gateway for microservices using FusionAuth for auth. Learn how to centralize authentication and route requests in a secure eCommerce example."
authors: Tim Kleier
...
If you want to follow along, it will be very helpful to go through the [5-Minute Setup Guide](/docs/get-started/start-here/step-1) first, as that will set up the needed users and roles in FusionAuth.
...
In this service, we've just got one route, to get products for a specific store or branch. Notice, however, that we're allowing access (or denying it) based on the inclusion of an <InlineFieldValue>admin</InlineFieldValue> role in the `roles` header. The API gateway application will be responsible for passing this data to our Product Inventory service.

If you were to start the service (go ahead and do so with `npm start`) and send a request to `http://localhost:3002/branches/1/products`, you should receive a 403. You can simulate a successful response by adding a `roles` header with a value of <InlineFieldValue>admin</InlineFieldValue>:
...
Head over to the [5-Minute Setup Guide](/docs/get-started/start-here/step-1) for FusionAuth. During setup, the application that you configure will be linked to our gateway application, so you can name it "Gateway". Note that while FusionAuth supports multi-tenant configurations, here you're setting everything up in the default tenant.
...
![The login screen for the gateway server.](/img/blogs/node-microservices-gateway/login-screen.png)

If we've got our setup working right, you should be able to click <InlineUIElement>Login</InlineUIElement>. Login to FusionAuth with the user you set up previously. You'll see a "Hello [your name]" message.
...
Using your browser (to leverage our OAuth credentials present because we signed in), go to `http://localhost:3000/branches/1/products`. This works because the user you set up in the [5-Minute Setup Guide](/docs/get-started/start-here/step-1) has an <InlineFieldValue>admin</InlineFieldValue> role in FusionAuth, and we're passing that role as a header to the Product Inventory service.
