...
Using your browser (to leverage our OAuth credentials present because we signed in), go to `http://localhost:3000/branches/1/products`. This works because the user you set up in the [5-Minute Setup Guide](/docs/get-started/start-here/step-1) has an `admin` role in FusionAuth, and we're passing that role as a header to the Product Inventory service.
