...
```mermaid title="An API request."
sequenceDiagram

actor User as Richard
participant API as API Server
User ->> API : Get Richard's todos
API ->> API : Request validated
API ->> User : Richard's todos data
```
...
```mermaid title="A failed API request."
sequenceDiagram

actor User as Dinesh
participant API as API Server
User ->> API : Get Richard's todos
API ->> API : Request validation attempted, fails
...
```mermaid title="API request with central authority validating the key."
sequenceDiagram
...
```mermaid title="API request with central authority validating the key for multiple requests."
sequenceDiagram
...
```mermaid title="Validation using signed API keys."
sequenceDiagram

actor User as Richard
participant RS as Todos API Server
participant AS as Central Authority
RS ->> AS : Retrieve Public Keys
Note right of User : ... Time passes ...
User ->> RS : Present API Key, Request Data
RS ->> RS : Check Signature
RS ->> RS : Check Data in Key
RS ->> User : Return Todos Data
```
...
```mermaid title="Validation using signed OAuth introspection."
sequenceDiagram
...
User ->> RS : Present API Key, Request Data
RS ->> AS : API Key Valid?
AS ->> RS : API Key is Valid, Here's Data About The API Key
RS ->> RS : Perform Additional Validity Checks Against Returned Data
RS ->> User : Return Todos Data
```
...
However, when using OAuth introspection, make sure you understand what a valid response from the central authority indicates. The <InlineField>active</InlineField> attribute is the *only* required value.
...
* Richard is sent to the central authority server with a <InlineFieldValue>read</InlineFieldValue> scope request
* Richard is prompted to consent to the <InlineFieldValue>read</InlineFieldValue> scope request
* Richard consents
...
* be narrow enough to be useful to control access (avoid <InlineFieldValue>superuser</InlineFieldValue> scopes)
