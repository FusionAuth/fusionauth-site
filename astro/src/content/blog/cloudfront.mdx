...
* Go to [AWS Certificate Manager](https://aws.amazon.com/certificate-manager/) and click the <Breadcrumb>Sign In</Breadcrumb> or <Breadcrumb>Sign in to the Console</Breadcrumb> button.
* Once at the Console, enter <InlineFieldValue>Certificate Manager</InlineFieldValue> in the search bar.

<div style={{textAlign: "center"}}>
  ![AWS Certificate Manager.](/img/blogs/cloudfront/aws-cert-manager.png)
</div>
...
<div style={{textAlign: "center"}}>
  ![AWS DNS Validation.](/img/blogs/cloudfront/aws-request-dns-validation.png)
</div>
...
<div style={{textAlign: "center"}}>
  ![AWS Certificate Request.](/img/blogs/cloudfront/aws-cert-validation.png)
</div>
...
* Go to <Breadcrumb>DNS</Breadcrumb>.
* Select <Breadcrumb>Add New Record</Breadcrumb>.
* Select <InlineFieldValue>CNAME</InlineFieldValue> for <InlineField>Type</InlineField>.
* Enter <InlineFieldValue>_7f214f44b65f038ed047cf3e4885000e.auth</InlineFieldValue> for <InlineField>Name</InlineField>. Notice this is not the full value provided by the <InlineFieldValue>CNAME</InlineFieldValue> it is only the subdomain part. Each provider may be different.
* Enter <InlineFieldValue>_d000256d72e990f3dff7497b0d486f4c.xlfgrmvvlj.acm-validations.aws.</InlineFieldValue> for the <InlineField>Value</InlineField>.
* Click <Breadcrumb>Save</Breadcrumb>.

<div style={{textAlign: "center"}}>
  ![GoDaddy CNAME settings.](/img/blogs/cloudfront/godaddy-cname-settings.png)
</div>
...
<div style={{textAlign: "center"}}>
  ![Search for AWS Cloudfront.](/img/blogs/cloudfront/aws-cloudfront.png)
</div>
* Select <Breadcrumb>Create distribution</Breadcrumb>.
* Fill in the form with the necessary info.  `Origin domain` will be `codegremlins.fusionauth.io`. You will need to make a selection in the `Web Application Firewall (WAF)` section. Click <Breadcrumb>Create distribution</Breadcrumb>.
<div style={{textAlign: "center"}}>
  ![AWS Cloudfront create distribution.](/img/blogs/cloudfront/aws-cloudfront-create-distribution.png)
</div>
...
* Next, select <Breadcrumb>Edit</Breadcrumb> in the <Breadcrumb>Settings</Breadcrumb> section of the <Breadcrumb>General</Breadcrumb> tab.
<div style={{textAlign: "center"}}>
  ![AWS Cloudfront edit distribution.](/img/blogs/cloudfront/aws-cloudfront-distro-edit.png)
</div>
* Under `Alternate domain name (CNAME) - optional`, select <Breadcrumb>Add item</Breadcrumb>. Enter <InlineFieldValue>auth.codegremlins.net</InlineFieldValue>.
* Under `Custom SSL certificate - optional`, select `auth.codegremlins.net`. This is the certificate you set in the above steps.
* Select <Breadcrumb>Save Changes</Breadcrumb>.
<div style={{textAlign: "center"}}>
  ![AWS Cloudfront edit distribution values.](/img/blogs/cloudfront/aws-cname-cert.png)
</div>
...
* Next, click on the <Breadcrumb>Behaviors</Breadcrumb> tab.
* Choose `Default(*)`.
* Select <Breadcrumb>Edit</Breadcrumb>

<div style={{textAlign: "center"}}>
  ![AWS Cloudfront edit distribution behaviors.](/img/blogs/cloudfront/aws-behaviors.png)
</div>

* Scroll down to the `Cache key and origin requests` section.
* Change the <InlineField>Cache policy</InlineField> to <InlineFieldValue>CachingDisabled</InlineFieldValue>.
* Change the <InlineField>Origin request policy</InlineField> to <InlineFieldValue>AllViewerExceptHostHeader</InlineFieldValue>.
* Select <Breadcrumb>Save changes</Breadcrumb>.
...
* Go to <Breadcrumb>My Products</Breadcrumb>.
* Select <Breadcrumb>Domains</Breadcrumb>.
* Select <Breadcrumb>codegremlins.net</Breadcrumb>.
* Click on the <Breadcrumb>DNS</Breadcrumb> tab.
* Click <Breadcrumb>Add New Record</Breadcrumb>.
* Choose <InlineFieldValue>CNAME</InlineFieldValue> for the <InlineField>Type</InlineField>.
* Enter <InlineFieldValue>auth</InlineFieldValue> for the <InlineField>Name</InlineField>.
* Value is the name of the AWS Distribution domain name. In this case, <InlineField>Name</InlineField> will be <InlineFieldValue>d2gwcywxndug73.cloudfront.net</InlineFieldValue>.
* Click  <Breadcrumb>Save</Breadcrumb>.

<div style={{textAlign: "center"}}>
  ![AWS Cloudfront edit distribution behaviors settings.](/img/blogs/cloudfront/aws-behaviors-settings.png)
</div>
