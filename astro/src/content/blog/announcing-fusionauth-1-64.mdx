...
We're excited to release FusionAuth `1.64.0`. This version includes [Lambda Secrets](/docs/extend/code/lambdas#secrets), dynamic discovery for OAuth endpoints, codes for email passwordless login, and custom page titles for simple themes. As usual, we've also taken the time to quash some long(and short)standing bugs.
...
![secret shibe](/img/blogs/release-1-64/shibe.jpg "wow, not dystopic doge")

## Safely Access Secrets from Lambdas

We all have secrets. Now FusionAuth enables you to store and access them securely with [lambda secrets](/docs/extend/code/lambdas#using-secrets).
...
The email login strategy now supports the `FormField` option, in addition to magic links.
...
`1.64.0` adds the ability to set custom page titles for Simple Themes.
