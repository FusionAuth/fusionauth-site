...
Air-gapping is a security practice where software systems are isolated from unsecured networks. As mentioned above, this includes the internet. This process creates a disconnected environment for sensitive operations and data.
...
For example, if you are deplying FusionAuth to a cruise ship, you may want to allow for changes to user profiles during the cruise and syncing them up to a central server when the ship is in port.
...
You can use FusionAuth as an air-gapped auth server and easily add features to your air-gapped applications. Learn more about air-gapping FusionAuth in [the air-gapping documentation](/docs/get-started/download-and-install/air-gapping).
