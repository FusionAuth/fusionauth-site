...
You may think of an authentication server as always available on the internet. After all, web and mobile apps using an auth server are usually served over the internet.

But sometimes you just want to keep all data and applications off the internet. To do this, you need to set up your applications to run in a network disconnected from the internet or any other public network. This practice is known as air-gapping.
...
Air-gapping as a security practice where software systems are physically isolated from unsecured networks. As mentioned above, this includes the internet. This process creates a completely disconnected environment for sensitive operations and data.
...
Finally, an air-gapped environment can be due to network connectivity expense, such as on an cruise ship or mining station. In this case, it's the location driving the lack of connectivity, rather than any special security needs.
...
For example, you could use the Authorization Code grant, leverage an existing OIDC library, and configure redirect to routable domain names, and perform a token exchange to get access tokens. Those tokens could be presented to APIs to access data or functionality.
...
You can even have have disjoint network segments, where FusionAuth is on one and an API needing to validate an access token is on another. To validate the tokens were properly signed without connectivity between the API server and FusionAuth, do the following:
