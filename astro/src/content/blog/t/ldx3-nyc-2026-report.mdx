...
Team effects: what does it mean to manage a team where some fraction of the code is AI-written? What does "senior engineer" even mean if the bar for producing working code has dropped?

Tooling fatigue: a lot of people are trying to figure out what's real versus what's a fad. Reasonable position to be in right now; it's hard to know what deserves attention.

The pipeline problem isn't new, but every new AI capability sharpens it. If junior engineers used to learn by doing the unglamorous work, and AI now does the unglamorous work, how do you grow the senior engineers we need five or ten years from now? Nobody I talked to had a clean answer. I don't either.
...
A service needs to pull that credential and inject it into the request. That service can't just trust whatever token shows up; it needs to verify the token's signature against the identity provider that issued it, check that the token was issued for this purpose (the audience), and confirm which user it's tied to (the subject) before releasing anything. Skip these checks and you've built a vending machine that hands out real Notion credentials to whoever asks nicely. Get it right and this is a small, auditable piece of infrastructure.
