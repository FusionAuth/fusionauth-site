...
Sometimes, the biggest reason to mock isn't complexity: it's access. If you're integrating with a third-party identity provider like Auth0, Azure AD, Okta, or even a corporate SSO setup, you often can't configure it, seed it with test users, or force it into failure modes. You may not even be able to run it locally. You're working with a black box; in that case, mocking might be your **only option**.
...
That makes mocking your only practical option. In the end, it comes down to one question: how much access do you really have?
...
Let's suggest you use Svix, an open-source event delivery platform, to push signed and timestamped events with delivery tracking, retries, and endpoint verification. A successful webhook delivery triggers a mail sent to the user identified with the ID sent on the event. 
