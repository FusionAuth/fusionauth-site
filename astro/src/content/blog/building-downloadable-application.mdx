...
Data locality is one aspect of control that is increasingly important. How criticial it is depends on the type of data your product holds. For customer personally identifiable information (PII) in certain locations, such as Europe or California, you want to know exactly where the data is going. Not doing so can lead to harsh penalties.
...
Depending on the number of developers running against a single account, a SaaS solution can be slower and more fragile to test against. Developers are making requests over the network. It is possible for configuration may collide with other developers', unless you have many accounts. If you have many accounts, then you have to manage it.
...
* feature flagging like Launch Darkly
...
If it requires a network then, sorry, it is not longer an option. You can look to such tools for inspiration, but anything that expects to be used only in a SaaS is off-limits.
...
You can also build  automatic downgrading or rolling back of an upgrade. Plan to spend time on the tooling and infrastructure to handle such changes in self-contained way. 
...
Plan to spend some time building out appropraite communication mechanisms.
...
In such a world, how do you stand up an testbed environment to replicate an issue? Sometimes you can, but often hopping on a call with a customer to see their deployment environment is more effective. The more you can bundle, the better off you are in this respect.
...
The type of organization that can confidently self-host critical infrastructure also leveage configuration management. They want to configure of your application via an API or tooling like Terraform or Pulumi. 

Offering this functionality allows your customers manage changes to your product in the same way they manage other parts of their application infrastructure.
...
* Support needs to handle heterogenous environments and older versions.
