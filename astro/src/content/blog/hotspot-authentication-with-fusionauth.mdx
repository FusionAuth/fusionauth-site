...
Next, create a test user in FusionAuth. Navigate to <Breadcrumb>Users</Breadcrumb> then click <Breadcrumb>+</Breadcrumb> button. It is not necessary to specify an email address. Uncheck <InlineField>Send email to setup password</InlineField>. Make sure you set a <InlineField>Username</InlineField> and <InlineField>Password</InlineField>. Save the new user.
