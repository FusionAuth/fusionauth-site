...
Navigate to <Breadcrumb>Settings</Breadcrumb>, then "API Keys" and create a key. Note the value of the key, as it will also have to be set in the Python scripts.

Set the correct Timezone by going to <Breadcrumb>Settings -> System -> Reports</Breadcrumb> tab and updating the "Report timezone" value.

Next, create a test user in FusionAuth. Navigate to <Breadcrumb>Users</Breadcrumb> then click <InlineUIElement>+</InlineUIElement> button. It is not necessary to specify an email address. Uncheck "Send email to setup password". Make sure you set a "Username" and "Password". Save the new user.
