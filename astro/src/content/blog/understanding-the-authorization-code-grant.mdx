...
Start by building the [authorization code grant request](/docs/apis/oauth#authorization-code-grant-request). This URL will redirect the user to FusionAuth's login page.
...
When the user logs in successfully, FusionAuth will redirect them back to your `redirect_uri` with an [authorization code and state](/docs/apis/oauth#response). Your application needs to handle the callback and extract these parameters.
