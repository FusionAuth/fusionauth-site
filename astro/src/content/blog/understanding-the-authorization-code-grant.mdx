...
Start by building the [authorization code grant request](/docs/apis/oauth/authorize#authorization-code-grant-request). This URL will redirect the user to FusionAuth's login page.
...
When the user logs in successfully, FusionAuth will redirect them back to your `redirect_uri` with an [authorization code and state](/docs/apis/oauth/authorize#response). Your application needs to handle the callback and extract these parameters.
...
One of the most common ways to verify the access token is by requesting the [JSON Web Key Set (JWKS)](/docs/apis/oauth/jwks) public keys generated by FusionAuth. This will allow your system to cryptographically verify JWTs using the JWKS format. To find out more information you can read through our [JWT API docs](/docs/apis/jwt).
