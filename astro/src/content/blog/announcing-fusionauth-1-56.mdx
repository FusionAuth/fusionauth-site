...
We recently released FusionAuth version 1.56.0. This version focuses on API security, and it introduces some new flexibility in how you can use SSO sessions within FusionAuth.

{/* more */}

Our fearless Senior Director of Developer Relations ([welcome to the team, Kim](https://www.linkedin.com/posts/fusionauth_this-week-we-are-super-excited-to-welcome-activity-7301000700145131520-FkVA?utm_source=share&utm_medium=member_desktop&rcm=ACoAAAGscwkBMMEEWM3JSitpba873aUwI_0jz6Q)!) has dubbed this release “The SSO Snek.” Our slithering friend is here to sneak past expired SSO sessions and ensure a seamless experience for your users.
...
In version 1.56.0, we’ve improved API key security by introducing hashed storage. This builds on our [1.55.1 release](/blog/announcing-fusionauth-1-55) that added expiration times. With 1.56.0, newly-created API keys are now optionally secured before being stored, reducing risks in case of a database compromise. We want to make this transition as seamless as possible for you, so your existing keys will continue to work. You will need to create a new key if you choose to have it secured moving forward.
...
Custom domains allow you to keep your domain name front and center during the authentication process. So instead of a customer seeing piedpiper.fusionauth.io they could see auth.piedpiper.com. We’ve supported custom domains since 2021 (and unlimited domains [since 2023](/blog/introducing-unlimited-custom-domains-for-fusionauth-cloud)), but they were only available to Business hosting and above. As of 1.56.0, all new FusionAuth Cloud deployments on our next-gen platform have custom domains enabled:

- Basic Hosting: 1 Custom Domain
- Business Hosting: 5 Custom Domains
- HA Hosting: 20 Custom Domains
  - (Unlimited Custom Domains supported with an Enterprise plan)

Note: Existing customers on our legacy cloud platform will not have access to the new custom domain limits. If you would like to enable custom domains, please contact support to help you migrate to our newest implementation of FusionAuth Cloud.
