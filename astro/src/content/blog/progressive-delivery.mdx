...
* **group your users**: Dan might be a beta user who wants all new features rolled out immediately, while Adam prefers a slower, stable feature set. You can store these preferences in one or more user attributes. Then you can build your application to enable certain features for certain categories of users. The book calls this "progressive rollout".

* **allow users to opt in to certain categories**: an individual user might want access to one set of new features (e.g. a new UI on a certain page), or an organization admin could do the same for their entire team. This dispersion of authority to determine when features are enabled is called "radical delegation".
