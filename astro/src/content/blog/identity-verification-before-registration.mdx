...
* the error the user sees is stored in the [`messages.properties` file](/docs/customize/look-and-feel/advanced-theme-editor#displaying-messages) and can be customized and localized
* if the API is not available or the request times out, this code fails open and defaults to allowing registrations; you could have it deny registrations by default as well
...
You should measure the success and failure of your registration rates. The success rate can be tracked by capturing the [User Registration Create webhook](/docs/extend/events-and-webhooks/events/user-registration-create).

Tracking the failures is a bit trickier, but you can make a fetch call for every attempted registration.

Here's a sample implementation which captures the failed email address, which would be called at validation failure, when the risk threshold is reached.

<RemoteCode 
   url="https://raw.githubusercontent.com/FusionAuth/fusionauth-example-kickstart/refs/heads/main/identity-verification/lambdas/fideo.js"
   language="javascript" 
   tags="registrationfail" />
