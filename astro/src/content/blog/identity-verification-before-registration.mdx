...
<Aside type="note">
The approach used here works with any other identity verification systems that have a REST API, but you're going to use Fideo for this tutorial.
</Aside>
...
Consult [the Fideo docs](https://docs.fideo.ai/docs/verify#api-output) for details about these fields. Contact them directly to discuss what makes sense for your use case. A KYC flow for a bank would use different checks and logic than an online gaming company trying to keep bots out.
...
You can also provide a way for users to register via a chatbot or customer support ticket in case they are incorrectly flagged. Tracking this also lets you know if the identity verification logic needs to be changed.
