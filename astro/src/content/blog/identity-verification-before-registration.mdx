...
title: Identity Verification During Registration
description: Can you verify the user's identity before allowing them to create an account using self-service registration?
authors: Dan Moore
image: /img/blogs/device-grant-gaming/oauth-device-grant-game-users.png
categories: Tutorial
tags: identity verification, email verification, self-service registtration, registration, lambda, lambda http connect, api, fideo
...
Self service registration is a key component of many customer identity and access management systems. Registration lets users sign up for an account, providing credentials and other required profile attributes, and immediately gain access to the application.

{/* more */}

In this post you'll learn how to check the identity of users with a third party API before they complete registration.
...
* [Basic registration](/docs/lifecycle/register-users/basic-registration-forms) offers a single-page themeable registration flow with a limited number of user provided fields, such as first name and last name.
...
Both basic and advanced registration let you mark registration fields as required. Advanced registration offers custom validation options, including matching fields against regular expressions, and a lambda to run custom logic.
...
In contrast to email verification, which gates access after the account is created, self-service registration validation prevents creation of accounts that don't pass muster.
...
Such a lambda offers real time validation, letting you give instant feedback to users who have a typo in their email address. It also lets you block malicious users who have a fake email address or other profile data. 

When performing validation, the error is a localizable message which can be displayed to the end user.

Let's look at what such an realtime validation integration would look like.
...
* an Essentials or Enterprise license key, which you can [purchase here](/pricing); after purchase you will find it in [your Account Portal](https://account.fusionauth.io/account/plan/)
* An API key from Fideo, which you can get by [contacting them](https://www.fideo.ai/contact-us/)

<Aside type="note">
This approach will work with other identity verification systems with a REST API, but you're going to use Fideo for this tutorial.
</Aside>
...
With this API, you provide as much or as little information about the user as you have. You must have either an email address or a phone number, but you can also add the users's address or name as well.
...
Consult [the Fideo docs](https://docs.fideo.ai/docs/verify#api-output) for details about these fields. Contact them directly to discuss what makes sense for your use case. A KYC flow for a bank will use different checks and logic than an online gaming company trying to keep bots out.
...
* a sample Changebank application with a custom look and feel
...
You can log in and review this setup by [accessing the admin UI](http://localhost:9011).
...
Then try it with a real email address of yours, such as a `gmail.com` address. Since this is a real email address, it has a low risk value. The registration will succeed and you will be logged in.

TODO image

Click logout to log out from this account.

TODO image

## A Closer Look At The Validation Logic

Let's take a look at the JavaScript code for the self-service registration lambda. Here's the validation function that Kickstart installed for you:
...
* retrieves our Fideo API key from the application's configuration (where the Kickstart put it)
...
* examines the API response and compares the calculated risk to a configured threshold
  * if the value is above the threshold, the code adds an error
  * if it is below the threshold, the registration succeeds

The threshold value is defined in the lambda function as well:
...
* storing the Fideo API key on the application object makes the key easier to rotate, but it is still available to API keys and users with sufficient access; there's an open GitHub issue to add [lambda secrets management](https://github.com/FusionAuth/fusionauth-issues/issues/1629)
* the error displayed is stored in the [`messages.properties` file](/docs/customize/look-and-feel/advanced-theme-editor#displaying-messages) and can be customized and localized
* if the API is not available or the request times out, this code fails open and defaults to allowing registrations; you may want different behavior
...
Open up the [admin UI](http://localhost:9011). Use an incognito window or a different browser than you used to test the registration process.

Log in as the admin user, with an email of "<RemoteValue url="https://raw.githubusercontent.com/FusionAuth/fusionauth-example-kickstart/refs/heads/main/identity-verification/kickstart.json" selector="$.variables.adminEmail" />" and a password of "<RemoteValue url="https://raw.githubusercontent.com/FusionAuth/fusionauth-example-kickstart/refs/heads/main/identity-verification/kickstart.json" selector="$.variables.adminPassword" />".

Navigate to <Breadcrumb>Customizations -> Lambdas</Breadcrumb>. Find and edit the `Fideo Self Service Registration Validation` lambda. Change the `FAIL_THRESHOLD` value at the top of the edit screen to `0.99`. This will let almost anyone register.

TODO image

Save the new lambda code.
...
They were denied last time but now can register because of the relaxed threshold.
...
You should measure the success and failure of your registration rates. The success rate can be tracked using the [User Registration Create webhook](/docs/extend/events-and-webhooks/events/user-registration-create).

Tracking the failures is a bit trickier, but you can make another fetch call for every registration event. Here's a sample implementation which captures the failed email address and which you could call after the risk threshold is reached.

<RemoteCode 
   url="https://raw.githubusercontent.com/FusionAuth/fusionauth-example-kickstart/refs/heads/main/identity-verification/lambdas/fideo.js"
   language="javascript" 
   tags="startofregistration" />

Subtracting successful registrations from total gives the number of failed registrations.

Keep an eye on the failure rate and tweak your business logic to balance between letting users register and preventing fake registrations. The exact ratio depends on how costly each of these are.

## Next Steps

This is just a start. Other things you might want to explore using this sample app include:

* collect more on registration, such as last name, phone number or zip code, which you can then pass to the Fideo API for more precise scoring
...
* add more business logic to the lambda, looking at other aspects of the data returned by the API call, such as the email checks
* store the risk value on the user's data field and add a [JWT populate lambda](/docs/extend/code/lambdas/jwt-populate) to add it to the access token so downstream services can limit service based on the value
