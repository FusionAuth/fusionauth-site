...
The meanings of these keys are explained in detail in the [Login API docs](/docs/apis/login/authenticate), but in general, your code should extract the `loginId` and the **password** fields and authenticate the user against the legacy datastore. You'll also receive the application which the user was trying to access; you should validate that as well.
