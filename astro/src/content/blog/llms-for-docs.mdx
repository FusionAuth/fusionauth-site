...
Here at FusionAuth, we ahve also noticed a trend in the last few months. We started getting inbound sales calls from people who asked ChatGPT about us. We ask people who install our software where they heard about us from and many answered with a specific LLM name or "LLMs/GenAI".

Alloing developers to be successful by using our documentation to get to what they are trying to accomplish is something we spend a lot of time on here at FusionAuth. In this post, we'll talk about ways we have modified our documentation to make it work better with LLMs, and show you how to do so.
...
The effort of good documentation pays off in multiple ways, because in addition to helping LLMs, it also helps SEO. Google is still a massive source of traffic. And, following those guideslines above, it helps developers who are reading the docs.
...
While you can build your own, unless you are an AI focused company wanting to dogfood your solutoin, you're probably better off buying this functionality, since that is an easy way to trade money for time.
...
We don't just have our techdocs ingested in Kapa, we also ingest our [OpenAPI spec](https://github.com/FusionAuth/fusionauth-openapi), our [YouTube channel](https://www.youtube.com/c/fusionauth), our [forum posts](https://fusionauth.io/community/forum/) and a number of other sources. Kapa has answered tens of thousands of queries, both for external and internal users. It's great for discovery of documentation or other resources.
...
In addition, when you make these files available to developers browsing your site, they can retrieve it and upload it to an LLM of their choosing. This allows thme to interrogate the docs or learn more about the doc structure using their tool of choice.
...
We've explored but not implemented an MCP for FusionAuth. MCP is great for learning a tool, building protoypes, or debugging. But I'm not sure folks will be using it to replace [configuration management](/docs/operate/deploy/configuration-management) in production systems.
