...
The [UserInfo endpoint](/docs/apis/oauth/userinfo), as defined in the OIDC specification, takes an access token and returns information about the user. Per the specification:
...
* Augment the provided claims by calling other APIs using [Lambda HTTP Connect](/docs/extend/code/lambdas/#http-connect).
...
The first experience of FusionAuth can be a bit confusing, [quickstarts notwithstanding](/docs/get-started/quickstarts/). There are a lot of "nerd knobs" to tweak, as [Matt Hathcock, one of our Sales Engineers](/blog/sales-engineering), puts it.
