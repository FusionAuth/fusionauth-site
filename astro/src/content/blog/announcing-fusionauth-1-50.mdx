...
---
FusionAuth version 1.50.0 shipped on April 24, 2024. This version includes OAuth scopes support, a UserInfo lambda, an easier setup screen, and more.
...
All in all, there are 20 issues, enhancements, and bug fixes included in the 1.50.0 release. As always, please see the [release notes](/docs/release-notes/#version-1-50-0) for a full breakdown of the changes between 1.49.2 and 1.50.0, including schema changes.

Let's look at some of the more notable features in this release.

## OAuth Scopes

OAuth scopes let you create coarse grained sets of permissions that you can ask an end user to consent to. 

This isn't super useful if all of the applications delegating authentication and authorization to your identity store are owned by your organization. But if you want to build a platform or ecosystem, where third party developers and organizations access user data held by you, scopes solve that problem elegantly.
...
After that, document how developers can ask for these scopes using the `scope` parameter on their initial request. End users who want to install or use these applications will the be presented with a screen similar to this:
...
If there's a third party application, working name `LOTRUS`, which looks up a video and modifies it to turn everyone into their favorite Lord of the Rings character, this application will need `video:download`, `video:search`, and `video:upload` permissions. On the other hand, a third party application which looks at videos in aggregate and performs statistics on them only needs `video:search.

Assuming a user consents, FusionAuth would issue each of these applications a token with the appropriate scopes. When the application makes requests for data from the Pied Piper APIs, the APIs can check for the appropriate permissions before taking action.
...
Here's an example lambda which removes PII, but leaves the email domain available for any service consuming it. The `sub` claim is also untouched.
...
The initial experience of getting going with FusionAuth can be a bit confusing, [quickstarts notwithstanding](/docs/quickstarts/). There are a lot of "nerd knobs" to tweak, as [Matt Hathcock, one of our Sales Engineers](/blog/sales-engineering), puts it.

This level of customization is powerful when you are familiar with it, but it can be frustrating when first starting. In this release, there's a screen which walks new admin users through FusionAuth setup. It covers:
...
Filling out these values is enough to integrate an existing application with FusionAuth. 

The goal is to hide complexity for new users, while still allowing folks familiar with FusionAuth all the power of all the nerd knobs. Don't worry, if you are a FusionAuth veteran, you can dismiss the screen on a new install.
...
* You can now terminate TLS at FusionAuth.
