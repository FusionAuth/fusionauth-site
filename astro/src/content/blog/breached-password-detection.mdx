...
authors: Dan Moore
image: /img/blogs/breached-password-detection/how-to-enable-breached-password-detection-fusionauth.png
categories: Product, News
tags: breached password detection, user security, passwords
...
[Defense in depth](https://en.wikipedia.org/wiki/Defense_in_depth_%28computing%29) is a security concept which basically means you shouldn't rely on one single way to protect your systems. Preventing a user from using a publicly available password, one that has been compromised, will increase your system security. There are numerous publicly available databases of cracked passwords. Your auth system can check if a user's password matches anything in these datasets.
...
*Please note that this is a paid plan feature. You can [learn more about paid plans here](/pricing).*
...
Assuming you've [activated your paid plan with a license key](/docs/get-started/core-concepts/licensing), navigate to the Tenant details page, and then to the <Breadcrumb>Password</Breadcrumb> tab. Enable **Breached password detection settings** and then choose your options. If you need varied configuration for different applications, use the multi-tenant feature to create separate tenants and configure them individually.

![Enabling breached password detection.](/img/blogs/breached-password-detection/enable-breached-password-check.png)
...
* A user is created via the [User APIs](/docs/apis/users)
...
* Recording the result, which will update statistics and [fire a webhook](/docs/extend/events-and-webhooks/)
...
These settings are also available in the [Tenant API](/docs/apis/tenants), under the `tenant.passwordValidationRules.breachDetection` key. 
...
![The breached password report.](/img/blogs/breached-password-detection/breach-password-report.png)
...
![What a user sees when they are forced to change their breached password.](/img/blogs/breached-password-detection/invalid-login-message.png)
...
![The warning an administrative user sees after a compromised password is found.](/img/blogs/breached-password-detection/admin-ui-warning-of-breach.png)
...
Just a reminder, this is a paid feature. You can [learn more about paid plans here](/pricing).
