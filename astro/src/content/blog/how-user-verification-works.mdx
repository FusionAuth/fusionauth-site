...
Hopefully FusionAuth will change the deprecated `isVerified` field on the user object in the future to represent whether the user's primary identity is effectively verified. But until that happens, there is a simple algorithm to check the user identities to see if a user is effectively verified:
