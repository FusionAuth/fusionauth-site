...
The FusionAuth field `user.verified` stores whether the user has completed email verification. But as of [FusionAuth version 1.59](/docs/release-notes/#version-1-59-0) (August 2024), this field is deprecated. FusionAuth now handles verification for email and phone separately.
...
If you want to prevent a user from using your app (signing in) until they have verified their identity, follow the [gating tutorial](/docs/lifecycle/manage-users/verification/gate-accounts-until-user-email-verified).
...
This section demonstrates what email verification looks like. If you want to test it for yourself, you need an SMTP server. The simplest way to test the process is to use a free fake SMTP service, like [SMTPBucket](https://www.smtpbucket.com).

Configure SMTP and email verification in the <Breadcrumb>Email</Breadcrumb> tab of the default tenant in the FusionAuth web interface. This is also the tab where you enable <InlineUIElement>Verify email</InlineUIElement> and choose the
...
Back in the FusionAuth web interface, as the administrator, manage the new user on the <Breadcrumb>Users</Breadcrumb> page. Below, you can see the green verified tick in the header, as well as the `verified` field in the user data.
...
To understand the new FusionAuth verification model, look at the new <Breadcrumb>Identities</Breadcrumb> tab in a tenant. In the image below, you can see the old version of FusionAuth on the left and the new version on the right.
...
The <Breadcrumb>Identities</Breadcrumb> tab isn't just for email. FusionAuth now supports phone numbers as a unique identifier for users, in addition to email addresses. You can see the <Breadcrumb>Identities</Breadcrumb> tab has two columns, with <InlineUIElement>Email</InlineUIElement> on the left and identical settings for <InlineUIElement>Phone</InlineUIElement> on the right.
...
The `verified_instant` field is set only if real verification took occurs, not if it is skipped.

### How to list effectively verified users

One unintuitive implication of FusionAuth now using a verification reason, is that a user might have a `verified` value of `false` but still count as being verified (so they can log in even if your application has excluded unverified users by gating).

FusionAuth might change the deprecated `isVerified` field on the user object in the future to represent whether the user's primary identity is effectively verified. But until that happens, there is a simple algorithm to check the user identities manually yourself to see whether a user is effectively verified:

- If the user's primary identity has a `verified` value of `true`, the user is verified.
- If the `verified_reason` is not `Completed`, `Implicit`, or `Pending`, the user is verified.
- If neither of the above is true, the user is not verified.
...
- In `POST` requests (to create a user), `sendSetPasswordEmail` is deprecated. Use `sendSetPasswordIdentityType` instead. You can use the value `email` or `phone`. (The same applies to the User Registrations API.) If you don't want to waste time verifying email addresses (for example, with administrator accounts), you can use the new `skipVerification` field.
