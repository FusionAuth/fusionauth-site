...
This quote says that a user verifying that they signed up to use an application has a different purpose to a user verifying their email or phone when creating an account in FA, though both procedures are named verification and both send a link the user must click. Verifying your identity proves ownership, whereas verifying your registration avoids surprise.
...
Configure SMTP and email on the <InlineUIElement>Email</InlineUIElement> tab of the default tenant in the FA web interface. This is also the tab where you enable <InlineUIElement>Verify email</InlineUIElement> and choose the
...
<InlineUIElement>Verification template</InlineUIElement> choose `Registration Verification`. Enable self-service registration on this page too so you can register on FA as a new user.
...
Clicking the link redirects the user to FA to confirm their address:
...
Back in the FA web interface as the administrator, manage to the new user in the <Breadcrumb>Users</Breadcrumb> page. You can see below the green verified tick in the header, as well as the `verified` field in the user data.
