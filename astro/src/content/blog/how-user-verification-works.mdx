...
However, the Identities tab isn't just for email anymore (hence the new name). FA now supports phone numbers as a unique identifier for users in addition to email addresses. You can see the Identities tab has two columns, with email on the left and identical settings for phone on the right.
