...
Verification in FA prior to 1.59 is only for email addresses (and not phone numbers or other identifiers). In other words, "phone number" isn't an identity type available in the identities table.

Each user identity (email) has the fields `verified` and `verified_instant`. `verified` is false until the user clicks the verification link, when it becomes true. `verified_instant` is the time at which the user verified.
...
This quote explains that a user verifying that they signed up to use an application has a different purpose to a user verifying their email or phone, though both procedures are named verification and both send a link the user must click.
