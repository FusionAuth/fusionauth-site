...
FusionAuth supports two approaches to application session management: centralized session management and application managed sessions.

Each has their strengths and weaknesses. Let's take a look at these options.

{/* more */}

## What Is A Session

But first, let's talk about what a session is.  A user session refers to the period during which a user interacts with a web application or website. It typically begins when the user first accesses the site and ends when they leave, log out, or remain inactive for a predefined period.
...
These can all interact when your user is logging in and moving between applications.

## Your Session Management Options With FusionAuth
...
Sessions are a critical part of your application infrastructure and an important part of the authentication and logout process. FusionAuth provides flexibility to use the right session management approach.  To learn more, read the [Logout And Session Management guide](https://www.fusionauth.io/docs/lifecycle/authenticate-users/logout-session-management) which dives into these approaches in much more detail.
