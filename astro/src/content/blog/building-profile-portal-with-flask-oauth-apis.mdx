...
description: Advanced registration forms let you easily build out multi-step registration forms, but how do you let the users modify their profile data?
authors: Dan Moore
...
_Note that user profile editing functionality is now available in paid plans of FusionAuth. [Learn more](/docs/lifecycle/manage-users/account-management/)._

Previously, we built a [self service registration form](/blog/advanced-registration-form) for a real estate application. It was a two step form which captured specific information about their home buying needs. We also themed the [registration form](/blog/theme-registration-form). This tutorial builds on the previous two and will walk through building a python flask application to let a user sign in and modify the profile data they provided at registration. 
...
1. [How to use FusionAuth's advanced registration forms](/blog/advanced-registration-form)
...
![The self service profile management portal.](/img/blogs/flask-oauth-portal/final-screen.png)

> _Want to give users the power to manage their own profile data? See how FusionAuth makes it easy—[book a live demo](/contact)._
...
Next, update your application settings. Navigate to the <Breadcrumb>Applications -> OAuth</Breadcrumb> tab. Add `http://localhost:5000/callback` to the <InlineField>Authorized redirect URLs</InlineField> field. Set the logout URL to be <InlineFieldValue>http://localhost:5000</InlineFieldValue>.
...
![Configuring the FusionAuth application for the flask portal.](/img/blogs/flask-oauth-portal/oauth-tab-of-application.png)
...
Finally, while some data is available from OIDC endpoints, retrieving profile data stored in the <InlineField>registration.data</InlineField> field requires the FusionAuth client library. To install it, use this command:
...
![The initial Flask page.](/img/blogs/flask-oauth-portal/simple-app-screen.png)
...
`CLIENT_ID` and `CLIENT_SECRET` are both available in the application you created in previous tutorials. To retrieve them now, navigate to <Breadcrumb>Applications</Breadcrumb> and click the green magnifying glass to view these values:

![Finding the client id and client secret values.](/img/blogs/flask-oauth-portal/application-screen-fusionauth.png)
...
![The portal page when you authenticate with OAuth.](/img/blogs/flask-oauth-portal/oauth-portal-page.png)

## Editing profile data

You could stop here if you wanted, I suppose. You're showing the user data that is retrieved from the [`userinfo` OIDC endpoint](/docs/lifecycle/authenticate-users/oauth/endpoints#userinfo), and you could display other data like the user's name. 
...
The changes add an API key (which you added when configuring FusionAuth) and the form id for the advanced registration form. The latter was created in the [initial blog post](/blog/advanced-registration-form). To find the `FORM_ID`, navigate to <Breadcrumb>Customizations -> Forms</Breadcrumb>. You'll see the form identifier on the list of forms:

![Finding the form id for your advanced registration form.](/img/blogs/flask-oauth-portal/form-id-display.png)
...
How are the <InlineField>type</InlineField> and <InlineField>required</InlineField> attributes determined? What is this `fields` value?
...
This route examines the `user` object, which was returned from the successful authentication. It pulls off the <InlineField>sub</InlineField> attribute, which is the user identifier and looks something like <InlineFieldValue>8ffee38d-48c3-48c9-b386-9c3c114c7bc9</InlineFieldValue>. It also retrieves the <InlineField>applicationId</InlineField>. Both of these existed on the user object before, but the previous code ignored them.
...
After the object has been updated, a `PATCH` request is made. This updates only the <InlineField>data</InlineField> field.
