...
Next, update your application settings. Navigate to the <Breadcrumb>Applications -> OAuth</Breadcrumb> tab. Add `http://localhost:5000/callback` to the <InlineField>Authorized redirect URLs</InlineField> field. Set the logout URL to be `http://localhost:5000`.
...
How are the `type` and <InlineField>required</InlineField> attributes determined? What is this `fields` value?
...
This route examines the `user` object, which was returned from the successful authentication. It pulls off the <InlineField>sub</InlineField> attribute, which is the user identifier and looks something like `8ffee38d-48c3-48c9-b386-9c3c114c7bc9`. It also retrieves the `applicationId`. Both of these existed on the user object before, but the previous code ignored them.
