...
If you are on a supported deployment and want a custom domain name such as `auth.piediper.com`, it's now entirely self-service. Log in to your account portal, navigate to <Breadcrumb>Deployments</Breadcrumb> and then select <Breadcrumb>Custom URL(s)</Breadcrumb> under the <Breadcrumb>Action</Breadcrumb> dropdown. Check out the custom domains [documentation](/docs/cloud/operate/configure-custom-domain) for details about how to add the domain name and update an existing domain record.

![Managing a Custom Domain.](/img/blogs/unlimited-custom-domains/managing-domains-fusionauth-cloud.png)

Other customer identity and access management platforms typically only offer one custom domain as standard. If you need more than that, some vendors even make you pay an extra fee per domain. With FusionAuth, it's just another feature we are adding to support our [growing customer base](/blog/winning-customer-auth-market).
