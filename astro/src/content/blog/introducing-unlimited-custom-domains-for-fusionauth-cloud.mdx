...
If you are on a supported deployment and want a custom domain name such as `auth.piediper.com`, it's now entirely self-service. Log in to your account portal, navigate to <Breadcrumb>Deployments</Breadcrumb> and then select <Breadcrumb>Custom URL(s)</Breadcrumb> under the <Breadcrumb>Action</Breadcrumb> dropdown. Check out the custom domains [documentation](/docs/cloud/cloud#unlimited-custom-domains) for details about how to add the domain name and update an existing domain record.
