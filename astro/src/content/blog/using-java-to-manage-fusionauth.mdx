...
image: /img/blogs/java-client-example/tutorial-using-java-to-manage-fusionauth.png
categories: Tutorial
tags: java
...
So, if your preferred language is [Python](/blog/implementing-fusionauth-python), [Node.js](/blog/easy-integration-fusionauth-nodejs), [PHP](/blog/how-to-integrate-fusionauth-with-php), or anything else, you can use FusionAuth to take your authentication capabilities to the next level.
...
* FusionAuth (you can see the [FusionAuth system requirements here](/docs/get-started/download-and-install/reference/system-requirements))
...
FusionAuth can be deployed on nearly any platform. Depending on the platform you're using, you can get the installation instructions [from here](/docs/get-started/download-and-install/docker). 

Once FusionAuth is installed on your platform, you can follow this [simple 5-minute guide](/docs/get-started/start-here/step-1) to set it up and ensure it's running properly. That guide uses Node for the integration, but you can skip that part if you prefer to use Java.

Then, go to the FusionAuth UI and [create an API key](/docs/apis/authentication). We'll use this key to access the FusionAuth application programmatically. Make sure you scope it to only allow access to the application, user, and user/change-password APIs to follow the principle of least privilege.
...
In FusionAuth, an [application](/docs/get-started/core-concepts/applications) refers to a place where a user can log into. Before you begin integrating authentication capabilities with FusionAuth, you'll need to create at least one application. 
...
If you go to [this GitHub page](https://github.com/FusionAuth/fusionauth-java-client/blob/main/src/main/java/io/fusionauth/client/FusionAuthClient.java), you can see the various methods alongside their definitions. For example, to create an application, we'll use the `createApplication` method. If you check the method definition, you'll find that it only requires two parameters: the ID of the application and the request object that contains all the information used for creating the application. 
...
![The new application has been created..](/img/blogs/java-client-example/create-application.png)
...
![The new user has been created..](/img/blogs/java-client-example/user-created.png)
...
Let's say we want to use the FusionAuth Java client to log in the user we created. We'd do this if we wanted to log a user in from our own application, rather than using [FusionAuth's OAuth capabilities](/docs/lifecycle/authenticate-users/oauth/) and the Authorization Code grant. You might do this if you were building a command line tool to add Todos or a mobile application.
...
To change a user's password, we'll use the `changePasswordByIdentity` method. This method allows us to change a user's password using their identity -- login ID and current password. This means we can let our users change their password in our Todo application. Note that doing this will change their password across all FusionAuth applications running within [this tenant](/docs/get-started/core-concepts/tenants).
...
![The new user has been deactivated..](/img/blogs/java-client-example/user-deactivated.png)
