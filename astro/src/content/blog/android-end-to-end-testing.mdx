...
We've invested some effort implementing a fully automated end-to-end test while developing the FusionAuth Android SDK and made the tests available in the [Android Kotlin - Fusionauth SDK Quickstart](/docs/quickstarts/quickstart-android-kotlin-native) as well.
...
Once the end-to-end test suite has been thoroughly executed against various updates and releases throughout the development cycle, we can be confident to use it as well as a pass criteria for any pull request and as a test before the release workflows in the GitHub project.

Developers often reach for mocks to simulate authentication flows, but mocking critical services like FusionAuth can be dangerously misleading. As discussed in [Why Mocking Sucks](https://dzone.com/articles/why-mocking-sucks), mocks hide real-world failures, creating a false sense of stability. Similarly, [The Top 6 Indications You're Doing Mocking Wrong](https://fusionauth.io/blog/6-indicators-doing-mocking-wrong) highlights how mocking leads to brittle tests, increased maintenance, and missed integration issues especially in complex, interconnected systems.

In contrast, using a real FusionAuth instance ensures that our Android app is tested against actual login flows, user roles, and token handling — just like it would be in production. This local-first approach catches regressions early, validates the entire authentication flow, and improves confidence with every release.
...
# What Are The Specifics For The End-To-End Test Implementation
...
Starting the emulator requires the correct image for the test; we’re using the `google_apis` images for the last 5 API level.

TODO: Android Studio screenshot.
...
Although there are extensive log details available in your IDE and automated workflow to aid in debugging a failed test, it is beneficial to observe the test directly in the emulator or [record the test as a video in your workflow](https://github.com/FusionAuth/fusionauth-android-sdk/actions/runs/9051467294) for additional debugging input and context.
...
And in case of a failure, we upload the screen recording with [Upload a Build Artifact](https://github.com/marketplace/actions/upload-a-build-artifact) for [additional context during debugging](https://github.com/FusionAuth/fusionauth-android-sdk/actions/runs/9051467294).
...
Unlike mocking - which can create a false sense of stability by simulating ideal conditions, real-world tests surface unexpected regressions early as highlighted in [Why Mocking Sucks](https://dzone.com/articles/why-mocking-sucks) and [The Top 6 Indications You're Doing Mocking Wrong](https://fusionauth.io/blog/6-indicators-doing-mocking-wrong). Mocks don’t catch changes in authentication flows, browser behavior, or token lifecycles — all of which are critical in OAuth-based systems like ours.
