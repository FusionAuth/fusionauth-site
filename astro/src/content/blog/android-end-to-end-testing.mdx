...
We've invested some effort while developing the FusionAuth Android SDK and made the tests available in the [Android Kotlin - Fusionauth SDK Quickstart](/docs/quickstarts/quickstart-android-kotlin-native).
...
Implementing FusionAuth as the authentication and authorization platform enables seamless integration of various features throughout your application. This includes essential functions such as identity management, authentication and authorization, as well as advanced capabilities such as detailed user profiles, role management and custom registrations. As a result, numerous user-facing functionalities within the Android App will rely significantly on FusionAuth.
...
Once the end-to-end test suite has been thoroughly executed against various updates and releases throughout the development cycle, we can be confident to use it as well as a pass criteria for any pull request and as a test before the release workflow in the GitHub project.

# How

Initially, the test needed to operate within the IDE where development takes place. To automate the frequent manual clicks performed by an SDK developer when testing the Android app, we converted these interactions into a script. This script can be executed repeatedly, ensuring consistent and efficient testing.
...
This requires careful consideration to ensure that the tests function reliably across different Android versions. Setting up multiple emulators and testing environments repeatedly is essential to cover various scenarios effectively.
...
In the case of the Android Emulator, 10.0.2.2 serves as a special alias to your host loopback interface (127.0.0.1 on your development machine), where FusionAuth is running. Similarly, in the case of GitHub workflows, it refers to the virtual machine on which the workflow is executed.
...
Specifically, we are using API levels 29, 30, 31, 33 and 34. API level 32 is omitted because it is a special [Android 12L release](https://blog.google/products/android/12l-larger-screens/) designed for tablets and foldables with different layouts, which is not a focus for our testing.

Android Studio provides tools to configure and handle emulators directly within the IDE. Alternatively, you can also manage it in the [emulator commandline](https://developer.android.com/studio/run/emulator-commandline).
...
This setup is required only once for your emulator but becomes crucial during fully automated testing scenarios in a workflow where the emulator is set up from scratch for each test.
...
Although there are extensive log details available in your IDE and automated workflow to aid in debugging a failed test, it is beneficial to observe the test directly in the emulator or record the test as a video for additional debugging input and context.
...
For GitHub Actions, we create a Dependabot configuration to check both development and production dependencies on a weekly basis. Although currently all actions are treated as production dependencies by Dependabot, this behavior could change in the future. Therefore, we include both to ensure comprehensive coverage.
...
Although automating end-to-end testing for an Android app with OAuth can be complex, we successfully integrated it for our SDK.
