...
htmlTitle: "How to Secure a Rails API With JWTs"
description: Learn how to secure a Rails REST API using JWTs. This guide shows how to add token-based auth with FusionAuth and a Rails JWT gem.
authors: Dan Moore
...
This post is old and no longer maintained. If you'd like to protect a Ruby on Rails API with JWTs and FusionAuth, see this [tutorial on using Ruby on Rails and FusionAuth together](/docs/get-started/quickstarts/web/quickstart-ruby-rails-web).
</Aside>
...
> _Need help managing JWTs securely in production? [Let us show you](/contact) how FusionAuth does it._
...
# ...
```
We'll actually check the `iss` and `aud` claims are expected. This adds an additional layer of security. Here's another article about [securing your signed JWTs](/articles/tokens/building-a-secure-jwt).
