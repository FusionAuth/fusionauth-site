...
Save the application and then view it by clicking on the green magnifying glass. Scroll down to the <InlineUIElement>OAuth configuration</InlineUIElement> section, noting the <InlineField>Client ID</InlineField> and <InlineField>Client Secret</InlineField> values, which you'll need when configuring the web application in a bit:
...
To log in, click the <InlineUIElement>Secure</InlineUIElement> link and you'll be taken to the FusionAuth login page. These can of course be [themed](/docs/customize/look-and-feel/), but for now the default look and feel will have to do.
