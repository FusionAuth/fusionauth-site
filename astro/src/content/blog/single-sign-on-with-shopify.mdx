...
Learn how to use FusionAuth to integrate single sign-on (SSO) with a Shopify store using an OpenID Connect app. Using SSO lets your customers use one login across all your business' apps. For example, they can login to your Shopify store and ask for help using Zendesk without two separate logins.
...
Scroll down a bit, and in the <Breadcrumb>Attribute Mapping</Breadcrumb> section, enter “email” in the <InlineField>Email</InlineField> field. This tells the app that the <InlineField>Email</InlineField> field present in the FusionAuth response needs to be mapped to the <InlineField>Email</InlineField> field of the user.

![The Email field.](/img/blogs/single-sign-on-shopify/the-email-field.png)

Click <Breadcrumb>Save</Breadcrumb> once you are done. Then click on the <Breadcrumb>Setup IdP</Breadcrumb> button at the top of the page.
...
In the dashboard, click on the <Breadcrumb>Add Identity Provider</Breadcrumb> button.
...
Click on the <Breadcrumb>OAuth 2.0</Breadcrumb> tab. Choose <InlineFieldValue>Custom provider</InlineFieldValue> from the <InlineField>App Name</InlineField> dropdown, and enter an <InlineField>App Display Name</InlineField>, which will be displayed in the dashboard for you to easily identify the application. Then copy the <InlineField>OAuth Callback URL</InlineField> value. Though your value will be different, it will look something like `https://store.xecurity.com/moas/broker/login/oauth/callback/20956`. You’ll need this in the next step.
...
On the application creation page, you will need to provide a name for your application. This is so that you can distinguish your application in the dashboard. Go ahead and enter <InlineFieldValue>Shopify</InlineFieldValue>.

![Screenshot of the app creation page.](/img/blogs/single-sign-on-shopify/screenshot-of-the-app-creation-page.png)

Now, click on the <Breadcrumb>OAuth</Breadcrumb> tab. The default settings should be sufficient; the only thing you need to change is the <InlineField>Authorized redirect URLs</InlineField> field. Here, you need to put one or more URLs where the users will be redirected once they are logged in. Paste the "OAuth Callback URL" you copied in the previous step here. <InlineFieldValue>https://store.xecurity.com/moas/broker/login/oauth/callback/20956</InlineFieldValue> is the value for me, but yours will be different.
...
When you’re done, click on the blue <Breadcrumb>Save</Breadcrumb> button at the top-right corner of the page.
...
Next, navigate back to the Shopify IdP setup window from the previous section.
