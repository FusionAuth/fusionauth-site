...
description: "java-http 1.0 is a lightweight, zero-dependency Java HTTP server library that uses virtual threads to achieve 120,000+ requests per second with maintainable, synchronous-style code."
...
Building HTTP servers in Java has traditionally meant choosing between full featured complexity and simplicity you have to build. You could use heavyweight server frameworks with lots of dependencies, or you could build something from scratch using low-level APIs that were fast but make your solution difficult to maintain. java-http aims to fill this gap by providing a simple, dependency-free HTTP server library built for the JVM that's as easy to use as servers in other languages like Node.js.

{/* more */}

## What is `java-http`?

[java-http](https://github.com/FusionAuth/java-http/) is a lightweight, open-source, zero-dependency HTTP server for Java, licensed under Apache2. It's designed to feel like it could have come from the JDK itself: pure, fast, and simple. Unlike other Java web servers that pull in dependencies, java-http keeps things clean with no external requirements. Seriously. Check out [the pom](https://github.com/FusionAuth/java-http/blob/main/pom.xml). 
...
java-http inverts the traditional Java web application model. Instead of your application being deployed into a container like Tomcat, it becomes the main entry point and controls the HTTP server directly. This gives you complete control over startup and shutdown processes, environment variables, configuration files, and ports. These are all programmatically managed rather than constrained by external configuration files or what the container allows you.

## Why Does `java-http` Exist?

The Java ecosystem has lacked a simple HTTP server solution. While Apache Tomcat has been a go-to choice for Java web applications, it can be slow to adopt new standards and adds significant complexity for simple use cases. Netty is another option, but has a sprawling feature set and is overly complex.

Existing Java solutions also often come with many dependencies, pulling in libraries and functionality that most applications don't actually need. This bloat can slow down startup times, increase memory usage, and create potential security vulnerabilities through unused code paths.
...
A core principle of java-http is to minimize dependencies. The zero-dependency approach also means you won't need to worry about version conflicts or transitive dependency issues. This is a reflection of the FusionAuth team's general approach to software: fast, purpose-built tools.

## Performance: 120,000 Requests Per Second

Performance is also a core principle of java-http. In synthetic benchmarks, a server can handle around 120,000 requests per second. The goal isn't necessarily to be the fastest HTTP server in any language, but to be faster than other Java-based HTTP servers while maintaining simplicity.

That performance means the HTTP server itself probably won't be your bottleneck.
...
The biggest change in the 1.0 release of java-http is the adoption of virtual threads, made possible by Java 21's Project Loom. The first version of the library, released in 2022, used traditional NIO (non-blocking I/O), which was fast but resulted in complex, hard-to-maintain code with asynchronous operations scattered throughout. Adding new features or debugging issues required understanding complex state management and callback chains ("which socket am I thinking about now?").

Virtual threads allow the library to write code that looks and behaves like traditional threaded code while maintaining the performance benefits of asynchronous operations under the hood. The complexity of asynchronous programming is hidden from developers, making the codebase much easier to understand, debug, and extend. Developers can understand the code more quickly, bugs are easier to trace and fix, and adding new features becomes straightforward. Error handling is centralized in try-catch blocks rather than being scattered across multiple callback functions.
...
There were some performance benefits as well, especially in constrained environments. For example, FusionAuth test suites that previously took two hours to complete in thread-limited environments like GitHub Actions have been reduced to one hour thanks to the efficiency improvements that virtual threads provide.
