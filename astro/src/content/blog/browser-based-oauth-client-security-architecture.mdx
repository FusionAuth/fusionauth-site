...
authors: Nathan Contino, Kim Maida
...
The Internet Engineering Task Force (IETF), in its [OAuth 2.0 for Browser-Based Applications](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-browser-based-apps) draft, warns:
...
BBOC stores tokens in browser storage, exposing refresh tokens to JavaScript and creating vulnerabilities that no defense can fully close.
...
We discussed examples of supply chain attacks in [part one](/blog/backend-for-frontend-security-architecture) of this series. If you build your application using third-party JavaScript libraries, an attacker who compromises any of those libraries, or any of their dependencies, can execute arbitrary code in your application's context.
...
The IETF identifies this in [Section 5.1.3 (Acquisition and Extraction of New Tokens)](https://www.ietf.org/archive/id/draft-ietf-oauth-browser-based-apps-17.html#section-5.1.3). Even if the attacker cannot directly access existing tokens in your application's storage they can:
...
[RFC 9700](https://datatracker.ietf.org/doc/html/rfc9700) establishes mandatory security requirements for all OAuth implementations:
...
Implicit grants emerged when cross-origin resource sharing (CORS) adoption was limited and browsers couldn't make cross-origin token exchange requests. CORS is now universal, and the Authorization Code Flow with PKCE provides better security with equivalent functionality.
...
The [OAuth 2.0 for Browser-Based Applications](https://datatracker.ietf.org/doc/html/draft-ietf-oauth-browser-based-apps#section-6.3.4.3) specification states:
...
- **Mutual TLS ([mTLS](https://datatracker.ietf.org/doc/html/rfc8705)):** Similar to DPoP, use mTLS to bind tokens to client certificates. This is not ideal for browser implementation as it requires managed certificate distribution.
...
- **If your app handles PII, PHI, or financial data** → Use BFF. Regulatory compliance typically mandates it.
- **If your app requires compliance (SOC 2, HIPAA, PCI DSS, GDPR)** → Use BFF. Compliance frameworks increasingly view token-in-browser as a control deficiency.
- **Otherwise** → Use TMB at minimum, although BFF is best.
...
At FusionAuth, we strongly recommend migrating all applications away from BBOC to BFF. FusionAuth's [Hosted Backend](/docs/apis/hosted-backend) provides a production-ready BFF implementation with detailed documentation to get you started.
