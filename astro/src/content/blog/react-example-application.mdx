...
FusionAuth will set the <InlineField>SameSite</InlineField> attribute of the access token `app.at` cookie to <InlineFieldValue>Lax</InlineFieldValue>. This enables the cookie to be sent with requests to applications in the same subdomain. If the backend of the application runs on a different domain, the cookie will not be sent. This will result in the backend not being able to validate the token.
...
- Email: <InlineFieldValue>admin@example.com</InlineFieldValue>
- Password: `password`

This will log you in as an agent. You can also log in as a user by using the following credentials:
- Email: <InlineFieldValue>richard@example.com</InlineFieldValue>
- Password: `password`
...
This is the API to pull the details of a single ticket, using `GET` and passing the <InlineFieldValue>id</InlineFieldValue>:
