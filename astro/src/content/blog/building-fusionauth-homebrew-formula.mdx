...
htmlTitle: "How to Build a Homebrew Formula for FusionAuth"
description: Learn how to create a Homebrew formula to install FusionAuth on macOS. Follow this step-by-step guide to build, test, and publish a custom package.
authors: Tyler Scott
...
TL;DR. To just get FusionAuth up and running without learning how we did it, run the following commands:
...
How's that for fast and simple? Now you can dive in and start using the API. If this is your first time, you'll need to go through [the Setup Wizard](/blog/using-the-setup-wizard), and then you'll be good to create users, applications and roles. For specific details, read through our [detailed documentation](/docs/), and let us know if you have any questions on [StackOverflow](https://stackoverflow.com/questions/tagged/fusionauth "Jump to StackOverflow") or [GitHub](https://github.com/FusionAuth/fusionauth-issues "Jump to GitHub").  
...
Homebrew provides a bunch of variables that define where the package will be installed to. The Ruby script can use these variables to install everything into the correct location. For more information, see the [Homebrew Formula Cookbook](https://docs.brew.sh/Formula-Cookbook).
...
If you would like your application to run as a service, you will need to include a plist configuration as well. Brew automatically installs a plist file in the correct location if you define a `service` function in your formula.

Since homebrew is available on Linux and macOS, the [service](https://docs.brew.sh/Formula-Cookbook#service-files) function is a general configuration term. It is actually a way to define how brew should start your application as a service on macOS using launch daemons, or on Linux using systemd.

Here is the `service` function of our `fusionauth-app` formula that allows brew to start FusionAuth as a service:
