...
Elasticsearch is one of the two search engines available for FusionAuth. (If you aren't sure which one to choose, you can read about the [strengths and limitations of each](/docs/get-started/core-concepts/types/users#user-search).)
...
[Tenants](/docs/get-started/core-concepts/types/tenants) are useful in many situations, and FusionAuth supports unlimited tenants. You can use them for private labeling your application, for different dev/test/qa environments, or for logical separation between different applications and API keys.
