...
<Aside nodark="true" type="tip">This blog post has been [superseded by a guide which uses omniauth](/docs/get-started/quickstarts/web/quickstart-ruby-rails-web).</Aside>
...
* **Name** (Required): A display name for our application. Enter <InlineFieldValue>fusionauth-rails-app</InlineFieldValue>.
...
to the <Breadcrumb>Tenants -> Default tenant</Breadcrumb>. Set the <InlineField>Issuer</InlineField> field under the <Breadcrumb>General</Breadcrumb> tab. The value of the issuer is typically the hostname of the identity provider. Since this is a demo, we'll use the value <InlineFieldValue>fusionauth.io</InlineFieldValue>.
...
For that, we will make an additional [code exchange](/docs/lifecycle/authenticate-users/oauth/endpoints#complete-the-authorization-code-grant-request) request. 
