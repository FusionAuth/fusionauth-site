...
htmlTitle: Rails OAuth Tutorial | Secure OAuth2 Login in Ruby on Rails with FusionAuth
description: Learn how to implement OAuth2 in Ruby on Rails using the authorization code grant. Set up secure Rails OAuth authentication with FusionAuth step by step.
authors: Brett Guy
...
<Aside type="tip">This blog post has been [superseded by a guide which uses omniauth](/docs/get-started/quickstarts/web/quickstart-ruby-rails-web).</Aside>
...
In a [previous post](/blog/building-protected-api-with-rails-and-jwt), we demonstrated how to encode, decode, and verify the authenticity
of a JWT to [secure a Ruby on Rails API](/blog/building-protected-api-with-rails-and-jwt).
In this post, we will leverage those same concepts with FusionAuth being the authentication provider issuing the token.

[Secure your Rails applications with FusionAuth's OAuth integration. Book a demo to learn more.](/contact?utm_campaign=10300596-CRO_Ethos_Global_2025&utm_source=implement-oauth-rails&utm_medium=blog)
...
If you prefer to install FusionAuth locally, see the [5-Minute Setup Guide](/docs/get-started/start-here/step-1) to get up and running.
...
A [FusionAuth Application](/docs/get-started/core-concepts/applications) is simply something a user can log in to.
As such, we want to create a new application in FusionAuth for our Rails app.

### Create an application
Select <Breadcrumb>Applications</Breadcrumb> from the left nav bar. Then click the green plus sign in the upper-right hand corner to add a new application.
...
* **Authorized redirect URLs**: Found under the <Breadcrumb>OAuth</Breadcrumb> tab. This URL tells FusionAuth where to redirect to after a user successfully authenticates.
* **Logout URL**: Also found under the <Breadcrumb>OAuth</Breadcrumb> tab. This URL tells FusionAuth where to redirect to when a user logs out.

![Creating a new Application in FusionAuth..](/img/blogs/fusionauth-example-rails/add-application.png)
...
![Creating a new User in FusionAuth..](/img/blogs/fusionauth-example-rails/create-user.png)

Configure the following attributes and then save the new user:
* **Email**
* **Password and Confirm**: Toggle <Breadcrumb>Send email to setup password</Breadcrumb> to off and manually enter a password for the user.
...
Navigate to <Breadcrumb>Users -> Manage</Breadcrumb> for our newly created user.

Click on the <Breadcrumb>Add Registration</Breadcrumb> button, select the Rails application we created, and then click on the <Breadcrumb>Save</Breadcrumb> button.

![The Manage User page in FusionAuth..](/img/blogs/fusionauth-example-rails/user-registered.png)
...
verifying the access token during login. To do this, navigate to <Breadcrumb>Tenants</Breadcrumb> on the left-nav bar and then click the <Breadcrumb>Edit</Breadcrumb> button that corresponds
to the <Breadcrumb>Tenants -> Default tenant</Breadcrumb>. Set the **Issuer** field under the <Breadcrumb>General</Breadcrumb> tab. The value of the issuer is typically the hostname of the identity provider. Since this is a demo, we'll use the value `fusionauth.io`.

![The Edit Tenant page where the issuer field is set..](/img/blogs/fusionauth-example-rails/issuer.png)

The second attribute is the HMAC signing key that is used to encode the token. Navigate to <Breadcrumb>Settings -> Key Master</Breadcrumb> from the left navigation bar.
Then click on the <Breadcrumb>View</Breadcrumb> button that corresponds to the `Default Signing Key`. From here, "click to reveal" the key and copy it to a safe location.
We will need this value to successfully decode the token in our application.

![The Edit Tenant page where the issuer field is set..](/img/blogs/fusionauth-example-rails/secret.png)
...
You can recall, these can be found under the <Breadcrumb>OAuth</Breadcrumb> tab when modifying an application in FusionAuth.

![The OAuth tab in the Edit Application page of FusionAuth..](/img/blogs/fusionauth-example-rails/edit-application.png)
...
![A sequence diagram illustrating the authentication code grant flow..](/img/blogs/fusionauth-example-rails/rails-fa-sequence.png)
...
![The example Home screen for the Rails application..](/img/blogs/fusionauth-example-rails/home.png)
...
First, we will set up our client using the [oauth2](https://github.com/oauth-xx/oauth2) gem. This provides us with a [REST](https://www.codecademy.com/articles/what-is-rest-api) client wrapper for the OAuth 2.0 specification.
...
The login process begins when the user clicks on the <Breadcrumb>sign in</Breadcrumb> link we created. As seen in the sequence diagram above, we will want to direct this request to FusionAuth which is in charge of validating user credentials. In turn, the user will be presented with a default login page including username and password fields. FusionAuth provides the flexibility to customize the style of this form via [themes](/docs/customize/look-and-feel/) but for this tutorial, we will stick with the default.
...
![The FusionAuth login page..](/img/blogs/fusionauth-example-rails/fa-login.png)
...
For that, we will make an additional [code exchange](/docs/apis/oauth/token#complete-the-authorization-code-grant-request) request.
...
![The example Home screen when logged-in to the Rails application..](/img/blogs/fusionauth-example-rails/logged-in.png)
...
![The FusionAuth logging out page..](/img/blogs/fusionauth-example-rails/logging-out.png)

## Next steps
Now that we have the OAuth flow working, our foundation is set to expand as needed. Here are a few ideas to tackle next:
* Add and assign user [roles](/docs/get-started/core-concepts/roles#overview) such that functionality in our Rails application can be shown or hidden accordingly.
* Customize the FusionAuth login page with a look and feel of our application using [themes](/docs/customize/look-and-feel/).
* Use [Identity Providers](/docs/get-started/core-concepts/identity-providers#overview) to add "Login with Google" or "Login with Facebook" social login buttons.
