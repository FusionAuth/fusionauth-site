...
```xml
<samlp:LogoutRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" Destination="[https://idp.example.com/logoutService](https://idp.example.com/logoutService)" ID="_6d0a9a4dbd356e2447b25dfald52e796" IssueInstant="2022-08-26T09:16:39Z" Version="2.0">
  ...
  <saml:NameID xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">user1@yourdomain.com</saml:NameID>
...
```xml
<samlp:LogoutResponse xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ID="_6c3737282f007720e736f0f4028feed8cb9b40291c" Version="2.0" IssueInstant="2022-08-26T09:16:40Z" Destination="[https://dev.application.com/saml-lo](https://dev.application.com/saml-lo)" InResponseTo="_6d0a9a4dbd356e2447b25dfald52e796">
  <saml:Issuer>[https://idp.example.com/logoutService](https://idp.example.com/logoutService)</saml:Issuer>
  <samlp:Status>
...
Historically, many SLO implementations used front-channel bindings to log users out of multiple applications asynchronously (such as loading logout endpoints in hidden iframes). However, this technique heavily relies on third-party session cookies in the browser.

In recent years, privacy initiatives led many browsers like Safari and Firefox to block third-party cookies by default. While Google Chrome ultimately reversed its decision to completely deprecate third-party cookies in 2024—opting instead for a user-choice model—the widespread blocking of third-party cookies across other major browsers means iframe-based front-channel logout is often unreliable. 

When third-party cookies are blocked, asynchronous front-channel methods can fail to terminate sessions on external service providers. To implement reliable SLO, developers must either rely on top-level HTTP redirects (which can degrade user experience due to multiple visible redirects) or resort to back-channel logout methods, such as simple object access protocol (SOAP), which communicate directly server-to-server and do not rely on browser cookies.
