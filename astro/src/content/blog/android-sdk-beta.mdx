...
With the simplified starting point we now took care of the store, refresh and retrieve tokens in the Token Manager making sure best practices are used.

What we didn't want to compromise on is the AppAuth authentication flow. When using a browser redirect to the SSO URL, the only application that sees or holds sensitive user credentials is FusionAuth. Your application code never has access to these, which decreases the attack surface area.
