...
Scopes are also defined in more detail in the [OAuth 2.1 draft](https://www.ietf.org/archive/id/draft-ietf-oauth-v2-1-12.html). This is currently a draft but is worth reading because [updates the OAuth2 RFCs](/articles/oauth/differences-between-oauth-2-oauth-2-1) based on a decade plus of experience. The key aspects of scopes in the standards documents are:
...
Scopes relate to permissions, but so to other common authorization concepts like roles. Let's look at how scopes are different than roles.
...
* business needs
* appropriate granularity--not too narrow or too sprawling
* future-rpoofing; you want them to make sense both now and in the context of future functionality
...
Because scopes requre consent, user understanding of what they mean is critical. If a user can't understand the third-party access they are allowing based on the scope name and any additional information on the consent screen, then you have a problem and need to reconsider your scopes.

The list is good but can be improved. Let's take a look at how.
...
Scopes don't have to be tied explicitly to a resource. If you have APIs that offer functionality not directly related to a resource, such as a way to calculate a credit score, can create scopes for that functionality.
...
* are unlikley to be responsive when asked how they use data access
* may be malicious 
* are the reason you are designing and building scope functionality in the first palce

Observability is the way to understand how your APIs and scopes are being used.

### Lifetime

Because scopes are embedded in third-party software, they live for a long long time. Once someone has a working integration with your platform, they are typically loathe to invest effort in updating their solution unless there are significant benefits to them.

Such benefits could be positive, such as access to enhanced functionality or data. Or they can be negative, such as the integration continuing to work after upgrade. Don't lean too hard on negative "benefits". When you break an integratoin, developers get frustrated. When developers are frustrated, they explore alternate solutions.
...
Let's talk a bit about what the API or service that gets an access token containing a scope needs to do. APIs must check scopes correctly when responding to a request. Here's example routes from a fictitious Changebank API which check for scopes.
...
Zendesk has CRUD oriented scopes, with a focus on resources, such as tickets, users, and organizations. Scopes consist of an resource and/or an operation. Operations can be appended to a resource or standalone, in which case they apply for all resources available to that user.
...
* there is substantial non-tehcnical work required to help a platform using scopes succeed
...
TODO confusion of user w/r/t scope definition 
TODO user decision fatigue (too many scopes)

TODO refresh tokens are they tied to scopes
