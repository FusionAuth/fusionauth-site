...
Normally, the types of data accessed by an application talking to a service or API are determined by the developer of each of these. Scopes introduce a mechanism for the end user to allow or deny data access by an application. Because scopes are used by an application to request permissions, they are only applicable when:
...
Scopes are defined as set of space separated strings in a request, with each string representing a user-understandable set of permissions.
...
Scopes not only relate to permissions, but to other common authorization concepts like roles as well. Let's look at how scopes are different from roles.
