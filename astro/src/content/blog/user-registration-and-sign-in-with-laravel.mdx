...
```
<Aside type="note" nodark="true">FusionAuth supports different features when a user registers, including verification emails, required fields and custom roles. Be sure to [check out the documentation](/docs/v1/tech/apis/registrations) to make sure you include all the appropriate options.</Aside>
...
<Aside type="note" nodark="true">When users are logged in, FusionAuth also returns a JWT token. If you're building a single page application or want to perform client-side requests, [check out the documentation](/docs/v1/tech/apis/jwt) for details.</Aside>
