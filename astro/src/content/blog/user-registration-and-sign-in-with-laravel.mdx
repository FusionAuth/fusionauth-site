...
<Aside type="tip">This blog post has been [superseded by a newer Laravel tutorial](/blog/single-sign-on-laravel-fusionauth)</Aside>
...
Before we write any PHP code, FusionAuth must be downloaded and running on your local machine or server. FusionAuth is available [for all major operating systems](/docs/get-started/download-and-install/docker) or it can be [run in Docker](/docs/get-started/download-and-install/docker).

Once you have FusionAuth running, you should log into the admin panel and create a new API Key and Application. This process [is outlined here](/docs/get-started/start-here/step-1), but you can skip the OAuth settings for this tutorial.
...
<Aside type="caution">Errors in this section? Remember to run `php artisan make:controller <controller_name> --invokable` _BEFORE_ creating the corresponding route in `web.php` (see additional info below)</Aside>
...
```
<Aside type="note">FusionAuth supports different features when a user registers, including verification emails, required fields and custom roles. Be sure to [check out the documentation](/docs/apis/registrations) to make sure you include all the appropriate options.</Aside>
...
<Aside type="note">Getting errors at this point?  One common mistake is forgetting to import the FusionAuth client into your controller - `use FusionAuth\FusionAuthClient;`</Aside>
...
<Aside type="note">When users are logged in, FusionAuth also returns a JWT token. If you're building a single page application or want to perform client-side requests, [check out the documentation](/docs/apis/jwt) for details.</Aside>
