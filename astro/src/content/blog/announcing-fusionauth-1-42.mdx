...
When the MFA policy is set to <InlineFieldValue>required</InlineFieldValue>, a user with an MFA method configured, whether the TOTP method accessible to instances running on the Community plan, or any advanced MFA methods such email or SMS, will be prompted for that additional factor, based on the trust lifetime defined.
