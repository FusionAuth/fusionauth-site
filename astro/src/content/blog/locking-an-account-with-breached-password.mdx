...
Create an API key by navigating to the <Breadcrumb>Settings</Breadcrumb> tab and then to "API Keys". At a minimum configure the following permission for this key: `DELETE` on the `/api/user` endpoint. Note the API key for later use.
