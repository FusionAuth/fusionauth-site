...
<CheckList>
  <CheckItem>**LocalStorage or sessionStorage in the browser**: These are accessible by JavaScript, which means they're vulnerable to **XSS attacks**.</CheckItem>
</CheckList>

Here's where you **should** store them:

<CheckList>
  <CheckItem checked>**HttpOnly, secure cookies**: These can't be accessed by JavaScript, making them much safer. They're also sent automatically with each request — perfect for session-based auth.</CheckItem>
  <CheckItem checked>**Secure storage on mobile devices**: Use Keychain (iOS) or Keystore (Android). These are built specifically to protect sensitive data.</CheckItem>
</CheckList>
...
- [OWASP on Token Storage](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_Cheat_Sheet.html)
