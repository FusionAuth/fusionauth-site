...
Josh also dropped a link to our [expert advice on authentication flows](/articles/login-authentication-workflows). I consult these diagrams regularly when thinking about the right way to architect an application's login flows.
...
In this post, Richb201 gives feedback about the way the [SMTP configuration screen](/docs/get-started/core-concepts/types/tenants#email) is set up, including how the password field is not shown by default. We love hearing feedback from our users on how to improve FusionAuth.
