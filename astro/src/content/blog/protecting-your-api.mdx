...
First, though, what is an API?

The acronym API stands for "Application Programming Interface" and defines a method for one piece of software to communicate with another. The transport and the message format must be be defined and mutually agreed upon, whether the participants are a C program is calling into a Fortran numeric processing library or a mobile application requesting user data from a web server.
...
* an client which is the software program making the request
...
The first requirement of API authentication is allow only appropriate callers gets access to data or functionality. This requirement includes how the initial authentication is performed, properly handling unexpected requests, and determining what access is requested and provided.
...
API keys, which are high entropy random strings assigned to clients by a central authority, give you both security and performance. There are other methods which work, such as network access control lists, but for internet enabled web APIs, API keys are a great solution.
...
Typically after you authenticate a reqeust and know who the caller is, you authorize it. This lets the API server know what access the caller has or if access should be denied.
...
Let's look at the differences between centralized and decentralized API key verification. The Pied Piper Todo Management Syste allows users to create, update and delete todos and tasks. It'll have multiple client applications and an API to store data. Users should be able to create and update their own tasks, but not those of others.

We're not going to deeply discuss how the client gets the API key in this post. You could do this using the [OAuth grants](/articles/oauth/complete-list-oauth-grants) or a developer portal issuing static keys, but this section will assume the client obtained a key that needs to be valided.
...
* API keys which authenticates each client to the todo API server
...
* a database table
* a separate in-memory service or
* a standalone server
...
With introspection, every time the API server receives a request, it presents the central authority with the API key. The central authoriy returns response indicating whether the API key was valid or invalid. The format of the response is defined by the RFC.
...
With such a platform in mind, Pied Piper needs to authenticate not just the API requester, but also control access in a more sophsiticated way, based on what each service's needs. For example, the Apple watch client needs to be able to read and write todos, but the analytics service onlys need to be able to read them.
...
* Richard installs a third-party app to summarize their todos with a LLM: LLMSum
* Richard is sent to the central authority server with `read` scope request
* Richard is prompted to consent to the grant of the `read` scope
* Richard consents
* Richard is sent back to third-party app which stores the API key which contains scope information as well as other data
* LLMSum makes a request with API key to the todos API server
...
To follow the principal of least privilege and security best practices, the third-party application should ask for as limited a number scopes as is needed to perform the operations.
...
Spend some time up front thinking about what scopes make sense for your API. You want them to:
...
You should think about whether to use centralized API key verification system or decentralized verification. The centralized option requires constant communication between the consumers of API keys, the systems serving the API, and the central authority issuing API keys, all to ensure validity of the keys. 
...
You also may need set up user based coarse grained permissions for third-party API application when building a platform.
