...
authors: Dan Moore
image: /img/blogs/protecting-your-api/protecting-apis.png
categories: Education
tags: apis, authentication, api keys, oauth, tokens
excerpt_separator: "{/* more */}"
---
import APIRequestWithCentralAuthority from "../../diagrams/blog/protecting-your-api/api-request-with-api-key-and-central-authority.astro";
import APIRequestWithMultipleRequestsCentralAuthority from "../../diagrams/blog/protecting-your-api/api-request-with-api-key-and-multiple-requests-central-authority.astro";
import APIRequest from "../../diagrams/blog/protecting-your-api/api-request.astro";
import Aside from '../../components/Aside.astro';
import FailedAPIRequest from "../../diagrams/blog/protecting-your-api/failed-api-request.astro";
import SignedTokenValidation from "../../diagrams/blog/protecting-your-api/signed-token-validation.astro";
import IntrospectionValidation from "../../diagrams/blog/protecting-your-api/introspection-validation.astro";

When you are building out HTTP based JSON APIs for web applications, desktop apps or mobile applications to access functionality or data, you must authenticate these requests properly. 

You may say to yourself "of course, who wouldn't do that?". But there have been incidents in the last few years where APIs were not protected at large or prominent organizations, including [LinkedIn and Parler](https://securityboulevard.com/2021/11/biggest-api-security-attacks-of-2021-so-far/).

There's also an [OWASP API top 10 list](https://owasp.org/API-Security/editions/2023/en/0x11-t10/), and authentication is number two on it.

Let's take a look at some things to think about when you are protecting your APIs.

{/* more */}

## What Is An API?

First, though, what is an API? API stands for Application Programming Interface and in the general sense is a way for one piece of software to communicate with another. The method of transport and the format of messages need to be defined, whether a C program is calling into a numeric processing library or a mobile application is requesting user data. 
...
* No authentication
* Centralized authentication, where API key validity is determined by the central authority
* Decentralized authentication, where the API server can determine API key validity

What does API key validity mean? It means that the API key is recognized as one that was issued by the central authority. When an API key is valid, the requester is properly authenticated.
...
What does the central authority look like? It could be a database table, a separate in-memory service or a separate architectural component. Conceptually these are all the same, but of course performance and complexity are different for each of these solutions, as well as their ability to support different services. But that's one of the good things about the centralized authentication scenario. You can migrate between these implementations in a transparent manner. And having a local solution, such as a database table, makes deployment and reasoning about the system relatively easy.

There are other benefits with this approach. Revocation of an API key is easy, if a client should no longer have access. Every request is checked, so when a user should no longer be able to access their todos, the central authority updates an API key status or removes the API key from its datastore. Subsequent requests fail.

However, with the centralized authentication option, the central authority is a possible chokepoint. Each service needs to verify every request. This means the central authority needs to meet high availability and performance requirements. As you add more services and more clients, the requirements become more stringent.
...
An API key in a decentralized system is often [a JSON web token (JWT)](/articles/tokens/jwt-components-explained) which can carry more information than a simple API key in a centralized system does.
...
An API key which is a signed token can include information like:
...
But the flip side of this decentralization is that [revocation of the API key](/articles/tokens/revoking-jwts) is more complex. To minimize the impact of revocation complexity, API keys in a decentralized system have time windows, which mean that no API key in a decentralized system will be valid forever.
...
* user visits todos marketplace
* user installs third party app to summarize their todos with LLM-magic
* user is sent to the central authority server with read scope request
* user prompted for consent to the read scope
* user consents
* user sent back to third party app which stores the API key
* third party app makes request with API key to the todos API server
* todos API server validates the API key, including scopes
* todos API server returns todos data
...
You should think about whether to use a centralized system or a decentralized system. A centralized system requires constant communication between the consumers of API keys, the systems serving the API, and the central authority issuing API keys to confirm validity of the keys. 

On the other hand, a decentralized authentication system allows API key consumers to validate API keys on their own, increasing scalability while making revocation more complex.

You also need set up user based coarse grained permissions for third party API application when building a platform.
