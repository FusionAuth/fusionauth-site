...
You can store values in any of the predefined user fields such as `user.fullName`. But you can also use the <InlineField>data</InlineField> field on both the `registration` and the `user` objects to store arbitrary data.
