...
**registration.data** is the right place to store data related to a user's account that is specific to an application. As a reminder, [a registration](/docs/get-started/core-concepts/types/registrations) is a link between a user and an application.
...
Navigate to the <Breadcrumb>Registration</Breadcrumb> tab and enable <Breadcrumb>Self service registration</Breadcrumb>. Then check the **Advanced** option and select the form you created. Before you click the <Breadcrumb>Save</Breadcrumb> button, you'll see a screen like this:
