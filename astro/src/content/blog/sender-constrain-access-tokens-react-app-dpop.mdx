...
A better approach is to “sender-constrain” the tokens your application receives. Each token can only be used by the specific client it was issued by. A token exchanged between the server and a user's home computer cannot be used in any other context. This keeps the harm from potential data breaches limited.
...
When those settings are updated, restart your application and your login should now work again. This time, with sender-constrained tokens that keep your users safe from a whole host of new attack vectors.
