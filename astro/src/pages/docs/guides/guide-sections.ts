...
guides.filter(quickstart => quickstart.id.indexOf("index.md") === -1)
  .map(qs => (
    {
      href: `/docs/guides/${qs.slug}/`,
      title: qs.data.title,
      icon: qs.data.icon,
  ...
  .forEach(qs => guideSections.find(section => section.key === qs.data.section).articles.unshift(qs));

export const sections = guideSections;
