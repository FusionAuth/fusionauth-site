...
<APIBlock>
  ...
  <APIField name="fusionauth-app.startup-probe.file-path" type="String (Path)" since="1.99.9">
    The path on disk that FusionAuth will create, if the file does not exist, when database migrations are complete, regardless of whether they succeed or fail. This can be used for startup probe type functionality in Kubernetes or other container orchestrators by indicating when lengthier database migrations are done. Run more aggressive probes such as liveness or readiness probes after this file has been created.
  </APIField>
