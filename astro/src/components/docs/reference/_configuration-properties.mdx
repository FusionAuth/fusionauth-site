...
<APIBlock>
  ...
  <APIField name="database.password" type="String">
    The password that FusionAuth will use to connect to the database. If you are using [Silent Mode](/docs/get-started/download-and-install/reference/silent-mode) or [Setup Wizard](/docs/get-started/download-and-install/setup-wizard), then this password might be used when the `database.user` is automatically created for you, depending on your database server and configuration options.
  ...
  <APIField name="database.username" type="String">
    The database user that FusionAuth will use to connect to the database. If you are using [Silent Mode](/docs/get-started/download-and-install/reference/silent-mode) or [Setup Wizard](/docs/get-started/download-and-install/setup-wizard), then this user might be automatically created for you, depending on your database server and configuration options. If this user is created as part of [Silent Mode](/docs/get-started/download-and-install/reference/silent-mode) or [Setup Wizard](/docs/get-started/download-and-install/setup-wizard), it will also be the owner of the database and schema for FusionAuth.
  ...
  <APIField name="fusionauth-app.http.max-header-size" type="Integer" defaults="131072" since="1.61.0">
    The maximum size of the HTTP request header in bytes.

    The HTTP request header includes the HTTP request line, and all HTTP request headers. This includes everything until the request body if present.
  </APIField>

  <APIField name="fusionauth-app.http.max-request-size" type="Integer" defaults="'See Description'" since="1.61.0">
    A comma separated list specifying the maximum request size in bytes. This size excludes the HTTP request header.

    This configuration allows you to specify a default value, and optionally more specific sizes by `Content-Type`. You may also use wild cards to match all subtypes. For example `application/*=67108864` would set the maximum size for all `Content-Type` values beginning with `application/`.

    The following is an example configuration using the default values.

    > `*=134217728,application/x-www-form-urlencoded==67108864`

    Default values:
    - `*=134217728` - 128 Megabytes
    - `application/x-www-form-urlencoded=67108864` - 64 Megabytes
  ...
  <APIField name="fusionauth-app.silent-mode" type="Boolean" since="1.19.0">
    Determines if FusionAuth uses [Silent Mode](/docs/get-started/download-and-install/reference/silent-mode) during the startup process.

    Defaults to `true` when `database.url`, `database.username`, and `database.root.username` are set.
  </APIField>

  <APIField name="fusionauth-app.startup-probe.file-path" type="String (Path)" since="1.59.0">
The absolute path for a file that is created after a database migration is finished. The file is created whether or not the migration succeeds.

Use this file with a container management tool that checks for completion of longer startup sequences, such as a Kubernetes startup probe. Set a timeout value of at least 15 minutes when checking for the existence of the file.
  </APIField>
  ...
  <APIField name="search.type" type="String" defaults="elasticsearch" since="1.16.0">
