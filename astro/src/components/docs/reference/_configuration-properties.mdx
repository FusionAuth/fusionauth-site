...
<APIBlock>
  ...
  <APIField name="fusionauth-app.startup-probe.file-path" type="String (Path)" since="1.99.9">
    The path on disk that FusionAuth will touch when database migrations are complete (regardless of whether they succeed or fail). This can be useful for startup probe type functionality in Kubernetes or other container orchestrators. It's a good indicator that lengthier database migrations are over. After the file has been touched (indicating database migrations are completed), more aggressive probes (like liveness or readiness probes) can be run.
  </APIField>
