...
docker run --network host --name dotnet-api --rm -v "$PROJECT_DIR/complete-application":/app -w /app mcr.microsoft.com/dotnet/sdk:7.0 dotnet run &
...
until [ "$(curl -s -o /dev/null -w '%{http_code}' -X POST http://localhost:5001/Panic 2>/dev/null)" = "401" ]; do
  echo "  Waiting for .NET API app..."
  sleep 5
done
echo ".NET API app is ready."
