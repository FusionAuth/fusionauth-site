...
### Flow for Resource Owner Password Credentials 
...
The following is an example device code grant the service would receive:
...
The user-managed access (UMA) grant type is an authorization standard protocol built as an extension of OAuth 2.0. This grant flow type grants an access token to the requesting party (requesting party token, or RPT) to allow access to a resource.
...
1. The client makes a POST request to the token endpoint of the authorization server with the following parameters -
...
Additionally, client authentication is performed if it’s needed, according to the [section 2.3 of RFC 6749](https://www.rfc-editor.org/rfc/rfc6749#section-2.3).
...
You should now have a good sense of the different OAuth 2.0 authorization grant types, from their basic definition to their usage, as well as the flow used for obtaining an access token from each one. 
