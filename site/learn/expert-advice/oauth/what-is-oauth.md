...
Before OAuth was prevalent, websites would ask users to enter a username and password into a form, for example the credentials for a Gmail account, to gain access to data from Gmail. Soon, users realized they didn’t want to grant the full access provided by sharing their username and password with any old service. They perferred limited access. For example, a user might want a third party service to access their email account, but only have rights to read contacts, not send email or add contacts.
