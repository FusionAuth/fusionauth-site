...
=== Enhancements
* Better support for JSON Patch. Now supporting RFC 7386 `application/merge-patch+json` and RFC 6902 `application/json-patch+json`. Note that you may still make a request using the `PATCH` HTTP method using `application/json` and the current behavior should not be changed. All `patch` methods found in the FusionAuth client libraries will still be using `application/json` for backwards compatibility. Now that this capability exists, we will be working to build support into our client libraries.
