...
* The Two-Factor Login API now returns `409` for too many attempts, which is the same behavior as the Login API.
...
** If you are only using Authenticator/TOTP for Two-Factor, this functionaly will continue to function properly in the community version.
...
* https://github.com/FusionAuth/fusionauth-issues/issues/960[GitHub Issue #960] was delivered, and includes features such as:
