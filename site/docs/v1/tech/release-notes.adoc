...
** The reason for this potentially breaking change is due to concern of potential abuse. If you were previously using a JWT to authorize the request to the `GET` HTTP method, you will need to modify your integration to utilize an API key. See the link:/docs/v1/tech/apis/jwt#retrieve-refresh-tokens[Retrieve Refresh Tokens
