...
* Use a CSRF token with all federated login requests. This change will additional protection when using a federated login to ensure the the login is completed from the same browser that started the login workflow.
...
* Upgrade our phone number validation to include the Kosovo country code of `+383`. This upgrade will add support for various other country codes, see linked GitHub issue for more detail.
...
* Fix Tenant select control on Group index page in the admin UI when only a single tenant is configured.
...
* Enable GC logging. A new log named `fusionauth-app.gc.log` will be found in the log directory. This log will not be available for viewing in the UI or via the System Log API at this time.
