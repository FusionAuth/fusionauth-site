...
* When using FusionAuth as a SAML IdP, FusionAuth will now accept `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent` in addition to `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress`. This should allow FusionAuth to work with SAML v2 service provider that only support the persistent NameID format.
...
* The Facebook & Google Identity Providers will now default to using a redirect instead of a popup for login. All existing configurations will be migrated to use the popup dialog to remain consistent with the previous behavior. With this update you may now also use the `idp_hint` parameter to login with Facebook and Google.
...
** You may not optionally configure PKCE as required or required when not using a confidential client for better compatibility when multiple clients types within a single FusionAuth application.
...
* When using the email verification gate and self-service registration if a user requires their email to be verified and is forced through the complete registration flow they will not be correct gated.
...
* Fix a JavaScript bug that may cause some of the themed pages render correctly in the view window.
...
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1207[GitHub Issue #1208] - Steam
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1209[GitHub Issue #1209] - Epic Games
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1209[GitHub Issue #1210] - Xbox
...
:warning_message: pass:normal[Some of the features in these release are in tech-preview. Please give us feedback and let us know how they work for you!]
