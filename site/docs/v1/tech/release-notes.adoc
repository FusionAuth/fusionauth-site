...
So much cool stuff in this release, this is a brief summary, more details and items to come!
...
=== New
* FusionAuth Admin UI and FusionAuth pages are now stateless. As of this version you will no longer need to provide session pinning in a multi-node configuration.
* Multi-tenant SSO. This was a limitation prior to this released due to the way we managed the HTTP session. This limitation has been removed... and there was much rejoicing.
...
* A new API to import Refresh Tokens
* Application specific email templates for Passwordless, Email Verification, Setup Password, and Change Password.
* A new icon in cornflower blue.

=== Fixed
* When viewing a custom form field, form control is missing.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/828[GitHub Issue #828]
* When submitting forms with customized form fields, some data types were not being converted correctly resulting in error or faulty validations.
** Previously configured custom field validation errors such as `[missing]user.data.foo` may now be `[blank].user.data.foo`. Validation errors should be tested after upgrade to ensure the custom validation errors are properly resolved.
...
* The JSON Web Tokens issued by FusionAuth will not include the `jti` claim.
