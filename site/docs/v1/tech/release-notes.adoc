...
=== Security
* A change was made to the FreeMarker template engine to remove the possibility of malicious code execution through a FreeMarker template. To exploit this vulnerability a user with an API key capable of adding or editing Email or Theme templates, or a user with access to the Fusionauth Admin UI to that is able to add or edit Email or Theme templates would need to add code to the template with the intention of executing system commands.
