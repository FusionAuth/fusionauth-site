...
* Modified a core macro, `[@helpers.input]`, in the Helpers theme to accept a checkbox value. This will break anyone who tries to copy and paste the checkbox usage without modifying their `_helpers.ftl` file. Review the link:docs/v1/tech/themes/#upgrading[Upgrading] section on the link:docs/v1/tech/themes/[Themes] page for information on how to resolve potential breaking changes.
