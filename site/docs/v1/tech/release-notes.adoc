...
_Note, release notes in progress&hellip;_
...
* Modified a core macro in the theme - [@helpers.input] to accept a checkbox. This will break anyone who tries to copy paste the checkbox usage without modifying their _helpers.ftl. 
...
** This allows domains or API addresses to be safelisted to use an API key.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/933[GitHub Issue #933]
*** Rate Limiting
*** Captcha
*** IP Access Control Lists/blocking
*** Blocked domains
...
** Send two-factor
* Captcha - uses additional (re)captca to guard against brute force attacks.
** This adds (re)captcha for login pages using an email/password combination. 
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/278[GitHub Issue #278]
** Add support for Google ReCaptcha v2, Google ReCaptcha v3, HCaptcha and HCaptcha Enterprise
* IP location 
** Includes IP address in Forgot/Reset Password emails so the recipient can know where the request originated in case they did not request it. 
** Flags suspicious IP addresses and sends an email to notify user of a new login with the IP's approximate location
** With unexpected IP addresses, such as with geography, sends an email to notify the user of a new login with the approximate location of the IP address
* Lamda to limit registrations - ie limit self-service registration in a tenant or application for an email domain. This allows you to limit end users registering with your company's email domain.
* Device fingerprinting and analysis
** Include IP addresses in Forgot/Reset Password emails so the recipient can know where the request originated from in case they did not request it. 
** Limit sessions per device - ie user can only log in on one device at a time
* FusionAuth Cloud Features
** Enhanced DDoS monitoring and mitigation
** Rules based response to DDoS threats
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/905[GitHub Issue #905]
* Webhook events: 
** User Actions - when a moderator or admin takes an action on a user
...
** User Password Updated - when the user API modifies a password 
** User Password Reset Started - when the forgot password API was called
** User Password Reset Sent - when the email is sent with a reset link or code
** User Password Reset Landed - when then user arrives on the password reset page
** User Password Reset Success - when the password has been updated using the one time code
** Kickstart Success - when a kickstart file is fully executed
** See the link:/docs/v1/tech/#eventswebhooks/[Event Webhooks] documentation for additional details. 
* Issues resolved by these webhook events: 
** User Password Reset Flow - https://github.com/FusionAuth/fusionauth-issues/issues/1308[GitHub Issue #1308]
** Kickstart - https://github.com/FusionAuth/fusionauth-issues/issues/1178[GitHub Issue #1178]
...
* When using IdP linking in conjunction with the Oauth2 Device Grant, the recently completed links will be available on the Device complete themed page.
...
* Alert on unusual user behaviour around password reset
* Add an IP ACL to an API key
** Allows you to assign an IP ACL to an existing API key which limits the usage by IP address.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1325[GitHub Issue #1325]
