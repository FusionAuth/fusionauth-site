...
=== Security
* Ensure that the changePassword identifier is removed if an API is used to change a user's password after the user has initiated a change password request.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1632[GitHub Issue #1632]

=== Changed
* The JWT authorization method is no longer supported when using the `GET` HTTP method on the JWT Refresh API.
** The reason for this potentially breaking change is due to concern of potential abuse. If you were previously using a JWT to authorize the request to the `GET` HTTP method, you will need to modify your integration to utilize an API key.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1646[GitHub Issue #1646]

=== Fixed
* Entities don't support the use of `:` in the permission name
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1480[GitHub #1480], thanks to https://github.com/matthewhartstonge[@matthewhartstonge] for the help!
* An application role may not be immediately available to assign to a user after initial creation.
** This issue was due to some additional caching introduced in version `1.32.1`.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/1575[GitHub #1575]
* Handle a read-only directory inside of the configured plugin directory.
...
=== New
* Initial technology preview for SCIM Server
...
** Available with all licensed editions of FusionAuth.
...
=== Known Issues
* A potential memory leak was introduced in this version. If you are sending a lot of email through FusionAuth, this error may cause your FusionAuth service to run out of memory. This has been fixed in version `1.33.0`, if you are unable to upgrade restarting the service periodically can mitigate this potential issue.
** See https://github.com/FusionAuth/fusionauth-issues/issues/1548[GitHub Issue #1548]
