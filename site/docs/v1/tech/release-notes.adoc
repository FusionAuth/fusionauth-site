...
=== Fixed
* Static resources such as CSS and JS may be missing a `Content-Type` header which may cause a proxy using `X-Content-Type-Options: nosniff` to fail to load the resource. This was introduced in version `1.37.0`.
...
* Correct behavior during login when both self-service registration and require registration features are enabled. This appears to be a regression introduced in version `1.36.5`.
