...
=== Fixed
* Revert the garbage collection logging change introduced in version `1.47.0` for compatibility with the FusionAuth docker image.
...
* Enable garbage collection logging. A new log named `fusionauth-app.gc.log` will be found in the log directory. This log will not be available for viewing in the UI or via the System Log API at this time.
