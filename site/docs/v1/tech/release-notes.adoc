...
** To work around this issue, you may use the Application API, or if you have access to the file system where you have installed FusionAuth you may modify the following HTML template and delete lines `217 - 223`. These lines are shown here for reference.
++++
<blockquote style="margin-left: 70px;">
<span>&nbsp;/user/local/fusionauth/fusionauth-app/web/WEB-INF/templates/admin/application/_macros.ftl</span>
<pre class="pygments highlight">
<code>
217 &lt;fieldset&gt;
218   &lt;legend&gt;[@message.print key="refresh-token-settings"/]&lt;/legend&gt;
219   &lt;p&gt;&lt;em&gt;[@message.print key="{description}refresh-token-settings"/]&lt;/em&gt;&lt;/p&gt;
220   [@control.text name="application.jwtConfiguration.refreshTokenTimeToLiveInMinutes" title="${helpers.approximateFromMinutes(application.jwtConfiguration.refreshTokenTimeToLiveInMinutes)}" rightAddonText="${function.message('MINUTES')}" autocapitalize="none" autocorrect="off" required=true tooltip=function.message('{tooltip}jwtConfiguration.refreshTokenTimeToLiveInMinutes')/]
221   [@control.select name="application.jwtConfiguration.refreshTokenExpirationPolicy" items=refreshTokenExpirationPolicies required=false tooltip=function.message('{tooltip}jwtConfiguration.refreshTokenExpirationPolicy')/]
222   [@control.select name="application.jwtConfiguration.refreshTokenUsagePolicy" items=refreshTokenUsagePolicies required=false tooltip=function.message('{tooltip}jwtConfiguration.refreshTokenUsagePolicy')/]
223 &lt;/fieldset&gt;
</code>
</pre>
</blockquote>
...
** If you encounter this error, you may either upgrade to 1.18.2 or run the following SQL command:  `SELECT SETVAL('identities_id_seq', MAX(id)) FROM identities;`
...
* Initial Tech Preview of Connectors. Connectors allow you to authenticate against external systems such as LDAP. A generic connector can also be configured to authenticate against any third party system. More details and documentation coming soon.
...
* System log export API has been added for retrieving a node's system logs as a compressed zip file.  See the link:/docs/v1/tech/apis/system-logs[System Logs API] documentation for reference.
