...
** If you are only using Authenticator/TOTP for Two-Factor, this will continue to function properly in the community version.
* If you are upgrading from a version less than 1.7.0, you must do a two stage upgrade. Upgrade to a version greater than or equal to 1.7.0 but less than 1.26.0, then upgrade from that version to 1.26.0. There were internal migration changes which require this process.

[NOTE.warning]
====
The Two-Factor API changes are not backwards compatible. If you use this functionality, please test before upgrading.
====
