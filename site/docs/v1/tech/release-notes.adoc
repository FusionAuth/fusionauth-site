...
* If you make ann API request to `/api/two-factor/login` with an empty JSON body, an exception will occur instead of a validation error being returned with a `400` status code.
...
=== New
* JWT Vending machine
* IP Access Control. Requires Threat Detection feature.
...
** Send two-factor
* Captcha. Requires Threat Detection feature.
** Support for Google ReCaptcha v2, Google ReCaptcha v3, HCaptcha and HCaptcha Enterprise
* IP location. Requires Threat Detection feature.
* Impossible travel calculation during login. Requires Threat Detection feature.
* More emails
* More webhook events
