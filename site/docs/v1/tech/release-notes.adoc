...
** You may now optionally configure PKCE as required, not required, or required when not using a confidential client. This offers better compatibility when multiple clients types (a webapps and a mobile app, for example) are authenticating against a single FusionAuth application.
