...
=== Enhancement
* In version 1.17.0 a loophole was closed which allowed a reconcile lambda to set the value of `user.email` even if it had already been resolved by FusionAuth. IdP integrations that had been taking advantage of this behavior to integrate with non-compliant OpenID Connect providers will no longer operate correctly. This decision has been reversed and the behavior will be documented as an acceptable use case as long as the risks are properly understood.
