...
* WebAuthn re-authentication requires a new hidden form field named `userVerifyingPlatformAuthenticatorAvailable` on the following pages in order to detect compatible devices/browsers and prompt the user to register a passkey:
** OAuth authorize
