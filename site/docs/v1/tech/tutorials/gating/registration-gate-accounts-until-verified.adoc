...
[NOTE]
====
Make sure you select the correct gating strategy that makes the most sense for your business.  Most folks will only require an Email Verification Gate for their business needs.
====

Most folks will find email verification gating sufficient for their business needs (see adjacent tutorial link:/docs/v1/tech/tutorials/gate-accounts-until-verified[here]).  However, there are times when you want to enforce opt-in behavior to specific applications. A good real world example of this might be Google. A user might have access to YouTube and Drive which are both part of the same parent company (or tenant, to follow our metaphor within FusionAuth).  However, Google requires separate registrations to allow access to both YouTube and Drive through the same account.  Without this step, users may find it alarming their logins work for both YouTube and Drive (without their express consent, especially if they did not know about the parent relationship).
...
image::tutorials/registration-gate-accounts-until-verified/registration-verification-gate-screen-clickable-link.png[Clickable link default registratiohn verification gate screen,width=1200,role=bottom-cropped]
