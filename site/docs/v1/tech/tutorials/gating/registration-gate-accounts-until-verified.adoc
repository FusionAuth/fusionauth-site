...
Most folks will find email verification gating sufficient for their business needs (see adjacent tutorial link:/docs/v1/tech/tutorials/gate-accounts-until-user-email-verified[here]). However, there are times when you want to enforce opt-in behavior to specific applications. A good real-world example of this might be Google. A `bob@alphabet.com` might have access to YouTube and Drive which are both part of the same parent company (or tenant, to follow our metaphor within FusionAuth). However, Google requires separate registrations to allow access to both YouTube and Drive through the same account. Without this step, users may find it alarming their logins (`bob@alphabet.com`) work for both YouTube and Drive (without their express consent, especially if they did not know about the parent company relationship).

image::tutorials/registration-gate-accounts-until-verified/application-registration-google.png[Google Applications Web,width=600,role=custom]

=== To Enable

To enable registration verification gating, do the following:

* Configure the tenant with your email server information.
...
* Enable self service registration under your application.
* Configure your application with a OAuth redirect URL.
