...
[NOTE]
====
Make sure you select the correct gating strategy that makes the most sense for your business.  Most folks will only require email verification gate for their business needs.
====

Most folks will find email verification gating sufficient for their business needs (see adjacent tutorial link:/docs/v1/tech/tutorials/gate-accounts-until-verified[here]).  However, there are times when you want to enforce opt-in behavior to specific applications. A good real-world example of this might be Google. A `bob@alphabet.com` might have access to YouTube and Drive which are both part of the same parent company (or tenant, to follow our metaphor within FusionAuth).  However, Google requires separate registrations to allow access to both YouTube and Drive through the same account.  Without this step, users may find it alarming their logins (`bob@alphabet.com`) work for both YouTube and Drive (without their express consent, especially if they did not know about the parent company relationship).

image::tutorials/registration-gate-accounts-until-verified/application-registration-google.png[Google Applications Web,width=600,role=bottom-cropped custom]
...
image::tutorials/registration-gate-accounts-until-verified/verification-registration-email-mailcatcher.png[Verification email in mailcatcher,width=1200,role=bottom-cropped]
...
A point of confusion is how the user object looks before, during, and after the registration verification (and subsequent gate) is enabled.

If a user is created and registration verification is turned off, the user object will read as `verified: true` as in below:

```
//...
 "registrations" : [ {
    "applicationId" : "3c219e58-ed0e-4b18-ad48-f4f92793ae32",
    "id" : "44ae6427-60fc-4d4b-bef4-97274a6d29a6",
    "insertInstant" : 1630520102750,
    "lastLoginInstant" : 1630639038934,
    "lastUpdateInstant" : 1630520102750,
    "roles" : [ "admin" ],
...
If a user is created while registration verification is turned on, then the user object will read as `verified: false` as in below:
...
Naturally, this will turn to `verified: true` once the user verifies their application registration.

[NOTE]
====
Users imported through the import API will have whatever attributes are initially set regardless of verification settings.
====


== Common Questions

include::docs/v1/tech/tutorials/gating/_common_questions_gating.adoc[]

== Troubleshooting Email and Registration Verification

include::docs/v1/tech/tutorials/gating/_troubleshooting_gating.adoc[]
...
image::tutorials/registration-gate-accounts-until-verified/registration-verification-gate-screen-code-entry.png[Code entry default email gate screen,width=1200,role=bottom-cropped]