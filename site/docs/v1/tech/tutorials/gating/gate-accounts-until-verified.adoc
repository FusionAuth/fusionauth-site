...
redirect_from:
 - /docs/v1/tech/tutorials/gate-accounts-until-verified/
---
...
image::tutorials/email-gate-accounts-until-verified/application-enable-self-registration.png[Enabling application self registration,width=1200]
...
A point of confusion is how the user object looks before, during, and after the email verification (and subsequent gate) is enabled.

If a user is created and email verification is turned off, the user object will read as `verified: true` as in below:
...
If a user is created while email verification is turned on, then the user object will read as `verified: false` as in below:
...
Naturally, this will turn to `verified: true` once the user verifies their email.

[NOTE]
====
Users imported through the import API will have whatever attributes are initially set regardless of verification or gate settings.
====
