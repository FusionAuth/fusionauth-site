...
If a user is created and email verification is disabled, the `verified` attribute will be `true`. This is because FusionAuth treats all users as having verified emails when email verification is disabled.
