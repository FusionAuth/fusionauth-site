...
FusionAuth provides the ability to configure JWT handling. By navigating to [breadcrumb]#Settings -> Tenants# in FusionAuth and selecting the JWT tab, you will see the tenant JWT configuration settings. The following is an example screenshot of the tenant JWT configuration:

image::core-concepts/tenant-configuration-jwt.png[Tenant JWT Configuration,width=1200,role=top-cropped bottom-cropped]
...
TODO update image
image::core-concepts/tenant-configuration-jwt-refresh.png[Tenant JWT Configuration,width=1200,role=top-cropped bottom-cropped]

==== Form Fields

[api]
[field]#Refresh Token duration# [default]#defaults to `43200`#::
The length of time specified in minutes that the refresh token is valid, value must be greater than 0. See <<Refresh Tokens>> below for additional information.

[field]#Refresh Token expiration# [default]#defaults to `Fixed`#::
A fixed expiration policy means the expiration is calculated from the time the token is issued. A sliding window policy means the expiration is calculated from the last time the token was used.

[field]#Refresh Token usage#::
The refresh token usage policy. A Reusable token does not change after it was issued. A One Time Use token value will be changed each time the token is used to refresh a JWT. The client must store the new value after each usage.

[field]#Refresh Token revocation#::
The refresh token revocation policy. These are a series of events which may or may not cause a refresh token to be revoked.
...
image::core-concepts/application-jwt-enabled-configuration.png[Application JWT Configuration,width=1200,role=top-cropped bottom-cropped]
...
[WARNING.warning]
====
Refresh Tokens are sensitive information. The token must be secured on the User's device. If a Refresh Token is compromised,
