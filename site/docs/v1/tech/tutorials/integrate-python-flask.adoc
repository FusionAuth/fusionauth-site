...
Then, you can run the setup script. Please note that this setup script is designed to run on a newly installed FusionAuth instance with only one user and no tenants other than "Default". In order to follow this guide on a FusionAuth instance that does not meet this criteria, you may need to modify the above script. Refer to the link:/docs/v1/tech/client-libraries/python[Python client library] documentation for more information.
