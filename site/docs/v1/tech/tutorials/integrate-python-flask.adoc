...
image::integrations/flask-integration/admin-user-setup.png[Admin user setup in FusionAuth,width=1200]

After logging in, you’ll be redirected to the FusionAuth admin UI. This lets you configure FusionAuth manually. However, for this tutorial, you're going to create an API key and then you’ll configure FusionAuth via the API using the {language} client library.

Navigate to [breadcrumb]#Settings -> API Keys#. Click the [uielement]#+# button to add a new API Key.

image::integrations/flask-integration/api-key.png[API key setup in FusionAuth,width=1200]

Copy the value of the [field]#Key# field and then save the key. It should be a value similar to `CY1EUq2oAQrCgE7azl3A2xwG-OEwGPqLryDRBCoz-13IqyFYMn1_Udjt`.
...
Next, you need to set up FusionAuth. This can be done in different ways, but we’re going to use the {language} client library. You can use the client library with an IDE of your preference as well.
...
[source,text,title="FusionAuth setup configuration requirements file"]
----
...
Then, you can run the setup script. Please note that this setup script is designed to run on a newly installed FusionAuth instance with only one user and no tenants other than "Default". In order to follow this guide on a FusionAuth instance that does not meet this criteria, you may need to modify the above script. Refer to the https://fusionauth.io/docs/v1/tech/client-libraries/python[Python client library] documentation for more information.
...
Now, create a new `requirements.txt` file to include {technology}. Add the `flask`, `python-dotenv`, and `authlib` plugins so that your requirements file looks like this:
...
The route code is completed. Now create a new folder called `templates` and navigate into it
...
image::integrations/flask-integration/flask-homepage.png[Flask application home page,width=1200,role=bottom-cropped]

Then click [uielement]#Login#.

image::integrations/flask-integration/flask-login.png[Flask application login page,width=1200,role=bottom-cropped]

Enter in the [field]#email# and [field]#password# that you assigned to your FusionAuth user. If login is successful, you should see your OpenID profile information.

image::integrations/flask-integration/flask-profile.png[Flask application profile page,width=1200,role=bottom-cropped]

Now, click [uielement]#Logout#. If successful, you should be brought right back to the `Hello Guest` homepage.

The full code for this guide can be found link:https://github.com/ritza-co/fusionauth-flask-integration-guide[here].
