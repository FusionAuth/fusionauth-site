...
Then, you’re at the FusionAuth admin UI. This lets you configure FusionAuth manually. But for this tutorial, you're going to create an API key and then you’ll configure FusionAuth via the API using the {language} client library.
...
Then, you can run the setup script. Please note that this setup script is designed to run on a newly installed FusionAuth instance with only one user and no tenants other than "Default". To follow this guide on a FusionAuth instance that does not meet these criteria, you may need to modify the above script. Refer to the link:/docs/v1/tech/client-libraries/python[Python client library] documentation for more information.
...
Enter the [field]#email# and [field]#password# that you assigned to your FusionAuth user. If login is successful, you should see your OpenID profile information.

image::integrations/flask-integration/flask-profile.png[Flask application profile page,width=1200,role=bottom-cropped]

Now, click [uielement]#Logout#. If successful, you should be brought back to the `Hello Guest` homepage.
