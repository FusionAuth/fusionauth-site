...
// I edited the redirect and logout urls to match the Flask defaults, so I used a source block instead of a link.
[source,python]
----
...
//not sure if I should go into more detail here about the specific errors I encountered when running the script on https://fusionauth.ritza.co. Might be helpful, but seems slightly out-of-scope.
...
The full `server.py` file contents are as follows.
[source,python]
----
import json
from os import environ as env
