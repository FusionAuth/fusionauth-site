...
Now, cut and paste the following file into `Gemfile`.
...
`OP_SECRET_KEY` is the client secret, which was defined by the <<Configure FusionAuth>> step. You don't want to commit secrets like this to version contro, so use an environment variable.
