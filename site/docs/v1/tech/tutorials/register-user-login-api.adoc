...
A user can also log in via the link:/docs/v1/tech/core-concepts/integration-points/#hosted-login-pages[hosted login pages]. These use the Authorization Code grant and will redirect the user to FusionAuth.
...
In general the hosted login pages are recommended. They are link:/docs/v1/tech/themes/[customizable and localizable with themes] and when used, FusionAuth is the only server side system to see sensitive user information like credentials.
