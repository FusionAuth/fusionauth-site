...
Anyone can download and run an EAP release.
...
* After the EAP has You'll see an [breadcrumb]#Early access# tab in your account portal. Navigate to that and you'll see a list of available EAP releases with installation instructions.

You'll typically want to choose the most recent EAP release, as that will include the latest bug fixes or improvements.

If there are no EAP releases listed in that tab, that means that there are no EAP versions available. An EAP version will only be available while the GA release which contains the new features is in development. Once the GA version is released, the EAP versions leading up to that release will be removed.
