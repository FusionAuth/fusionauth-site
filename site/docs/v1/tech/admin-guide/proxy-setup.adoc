...
Here is documentation for some common proxy servers:

* https://developers.cloudflare.com/rules/transform/request-header-modification/[Cloudflare]
* http://nginx.org/en/docs/http/ngx_http_headers_module.html[NGINX]
...
This guide is all about proxying requests *to* FusionAuth, in order to add a layer of indirection between your users and FusionAuth. The benefits are listed in <<Why Use a Proxy>>.

You can also proxy requests *from* FusionAuth, such as link:/docs/v1/tech/events-webhooks/[webhooks] or link:/docs/v1/tech/connectors/[connector requests]. You can read more about that in the link:/docs/v1/tech/reference/configuration[Configuration Reference]. Look for the `proxy.*` configuration values.
