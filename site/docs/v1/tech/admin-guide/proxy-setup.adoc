...
* `X-Forwarded-Port`: The original port requested by the client. This will be the port of the proxy server. If the port is 80 or 443, it may not be present on the original request, due to how those port numbers are the defaults for HTTP and HTTPS requests, respectively.
...
* `X-Forwarded-Server`: The hostname of the proxy server. This may be be different than `X-Forwarded-Host` if there are two or more proxy servers.
