...
| `X-Forwarded-Server`
| `auth.example.com`
| The hostname of the proxy server. It should be set by every proxy server in the proxy chain. This may be different than `X-Forwarded-Host` if there are two or more proxy servers.
