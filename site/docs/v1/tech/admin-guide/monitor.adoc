...
FusionAuth is a Java application. You can use the https://github.com/open-telemetry/opentelemetry-java-instrumentation[OpenTelemetry] project to instrument it with no changes to the code. This will help you diagnose performance or other issues.

FusionAuth contains no custom spans, but it can be set up as an exporter. This can provide useful data about the types of requests, database queries, and more.

=== Set Up the Exporter

Download the https://github.com/open-telemetry/opentelemetry-java-instrumentation[Java OpenTelemetry agent]. Make it available to the FusionAuth instance. If you are using a zip/deb/rpm, put it in a directory. If using docker, attach a directory to the container.
Set an environment variable `OT_PATH` to point to the directory.

Configure FusionAuth to use the agent. 
...
Set this environment variable and make sure it is available however you start FusionAuth (in a startup script, `.bashrc`, etc):
...
Here's an example of the environment variables to set to use https://www.honeycomb.io/[HoneyComb] as your collector, but these will vary based on your chosen solution. 

[source,shell,title=Configuring OpenTelemetry collector]
----
# using the standard jar
