...
**A:** This depends on the amount of data, the schema changes required, and your upgrade option choice. If you run multiple nodes, this can be minimized.  Multi-node clusters in FusionAuth Cloud experience user-facing downtime on the order of seconds to minutes during upgrades. Testing an upgrade in a non-production environment is the surest way to understand the amount of downtime.
