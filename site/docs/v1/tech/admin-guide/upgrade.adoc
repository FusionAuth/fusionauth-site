...
* Signing up for link:/docs/v1/tech/admin-guide/releases[release notifications].
...
* Monitoring the https://github.com/FusionAuth/fusionauth-issues/issues[FusionAuth Issues GitHub repository].
...
If you want to run the migrations interactively, you can run any needed scripts out of band (meaning directly on the database yourself). The general process would be to: 
...
The timing of the database upgrade is also important to note. When an upgraded node is started and connects to the database, it will lock the database immediately if configured for link:/docs/v1/tech/guides/silent-mode[Silent Mode] and the database requires upgrading. This means that the database will be unavailable to other nodes in the cluster until the upgrade is complete. Learn more about the different runtime modes and how they affect database upgrades in the link:/#FAQs[FAQs] section below.
...
If you are using an OIDC compatible library in your application to log your users in, it is unlikely that this integration would require modification. But test it out to make sure.

=== Skipping Versions 

Sometimes, you will want to upgrade to a FusionAuth version that is a few versions newer than your current version. For example, you might want to upgrade from version 1.43.0 to 1.45.0. This is called a "version jump". Version jumps include minor versions. FusionAuth can run all the necessary database migrations to get you to the latest version, or you can apply the database migrations yourself.

In some cases, version jumps are safe to do. To determine if you can perform a version jump, you should review the link:/docs/v1/tech/release-notes[release notes] carefully. If the releases focus on bug fixes and minor improvements, you can probably perform a version jump safely. Version jumping across patch versions, from 1.43.0 to 1.43.3, for example, is usually safe. Additionally, if the only changes are to features that you are not using, a version jump will work.

The alternative to a version jump is to migrate and test at each version of FusionAuth. For example, if you were moving from 1.43.0 to 1.45.0, you'd upgrade to 1.43.1 and test, then 1.43.2 and test, then 1.43.3 and test, then 1.44.0 and test, then 1.45.0 and test. This is a good option because if there are any issues, you'll be able to know precisely which version of FusionAuth is problematic. It's a great option if you have automated tests that can exercise FusionAuth integrations.
...
- Script the changes using a client library.
- Terraform.
- Pulumi.
...
The easiest upgrade route is to simply stop all your services, upgrade each FusionAuth instance, and then bring it all back online. 

This is a good option for single-instance development or QA systems, or for systems that can tolerate longer periods of downtime. For instance, if you know you have near-zero user activity at midnight, you can upgrade then. If you have scheduled downtime for other components of your system, that is a good time to upgrade FusionAuth in this manner.
...
There is still downtime with this procedure, but it is limited to the time it takes to run the first node to run the database migration. There will be limited capacity during the upgrade while the second and third nodes are upgrading, but the system will be available for use for most of the upgrade process.
...
graph TB
    ...
    B --> |Switch traffic when ready <br/> Upgrade one node at a time| C(Node 1)
    B --> D(Node 2)
    B --> E(Node 3)
    style C fill:#C0F0C0
{% endmermaid %}
...
To perform a blue-green deployment with a three-node cluster, first create three new nodes with the new version of FusionAuth on each of them. Start up one of the new nodes. When the new node starts and connects to the database, it will lock the database and own it while it completes the database upgrade. The original nodes might give errors as they will not have access to the database. These original nodes can be removed from the load balancer. Add the new nodes to the cluster. 

With this approach, the system will be down for the duration of the database upgrade but once the database upgrade is complete, the system will be back online with the upgraded version of FusionAuth on the new nodes. 
...
graph TB
    A(Users) --> B(Load Balancer)
    subgraph green [Green Deployment - new version]
        C(Node 1)
        D(Node 2)
        E(Node 3)
        style C fill:#C0F0C0
    ...
    subgraph blue [Blue Deployment - old version]
        F(Node 1)
        G(Node 2)
        H(Node 3)
        style F fill:#add8e6
    ...
    B --> |Switch traffic when ready|green
    B --> blue
{% endmermaid %}
++++
...
In production, you may want to perform the database upgrade rather than allow FusionAuth to do so. This is also known as an out-of-band upgrade. This is useful if you want to perform the database upgrade in a way that minimizes downtime or as part of a larger automated or coordinated deployment. Upgrading the database involves running SQL scripts to update the database schema. 
...
In the event that an upgrade is not successful or causes unexpected issues, you may need to rollback your FusionAuth instance to a previous version. Below are the steps for initiating a rollback:

1.  Stop your FusionAuth instances. 
2.  Initiate the process to restore from the database backup, which you should have taken prior to starting the upgrade.
3.  Re-deploy FusionAuth but use the previous version you were using instead of the new one. How you do this will depend on your deployment method. 
...
It's import to consider the timing implications of a rollback, or any disaster recovery process. RTO, or Recovery Time Objective, is the targeted duration of time within which a service should be restored after an outage to avoid unacceptable consequences. Simply put, it is how long you can afford to be without the service or system before it greatly impacts your business. It represents the goal for the time taken to recover from failure.

RPO, or Recovery Point Objective, on the other hand, refers to the maximum tolerable amount of data loss measured in time. RPO is determined by looking at the time of the last backup you need to restore from and the restore time. 

In the context of a FusionAuth upgrade rollback, the RPO would be the time since the last backup was taken before the upgrade, and the RTO would be the time it takes to stand up the older version of the system and restore the database. 
...
**A:** You can upgrade whenever you want to. It is your dependency and we won't force you. We do recommend staying relatively close to the latest release: within the last three minor versions. In addition, we don't typically backport bug fixes. If you run into an issue with a bug, you'll need to upgrade to get the fix. We recommend you set up a regular cadence of reviewing and upgrading FusionAuth that fits your business needs, the same as you would with a framework or library your application depends on.


**Q:** How can I find out about new releases? +
**A:** We publish release notes for every release. You can find them here: link:/docs/v1/tech/release-notes[Release Notes]. There are a link:/docs/v1/tech/admin-guide/releases[variety of ways to be notified of releases, including an email list and RSS feed].

**Q:** Does FusionAuth support zero-downtime upgrades? +
**A:** At this time, it does not when there is a database change. While there are a variety of options that can minimize downtime, upgrades will cause downtime whenever a schema change is required. Please https://github.com/FusionAuth/fusionauth-issues/issues/1240[review and upvote this issue] if zero-downtime upgrades are important to you.
...
To upgrade your FusionAuth Cloud instance, see link:/docs/v1/tech/installation-guide/cloud#upgrading-a-deployment[Cloud Installation Guide].
...
FusionAuth is available in a ZIP package for macOS, Linux, and Windows. If you are using the ZIP package, please use this guide to update an existing instance of FusionAuth. You can find the ZIP packages at the FusionAuth https://fusionauth.io/download[Downloads] page.
