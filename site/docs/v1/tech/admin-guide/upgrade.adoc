...
- Running the database migration scripts
...
The timing of the database upgrade is also important to note. When an upgraded node is added to a FusionAuth cluster, it will lock the database and perform the database upgrade if configured for Silent Mode and the database requires upgrading. This means that the database will be unavailable to other nodes in the cluster until the upgrade is complete. 
...
- Upgrade FusionAuth on the first node. Once FusionAuth is updated, start it up so that it can perform the database migrations. FusionAuth will lock the database and own it while the database upgrade is completed. Note that there will be a schema mismatch between the first node and the other nodes, so the other nodes may give errors while the first node is being upgraded. The nature of these errors depend on the code paths being updated as well as how users interact with the system. They should be minimal.
...
++++
{% plantuml source: _diagrams/docs/admin-guide/rolling_upgrade.plantuml, alt: "Rolling upgrade." %}
++++
...
To perform a blue-green deployment with a three-node cluster, first create three new nodes with the new version of FusionAuth on each of them. Start up one of the new nodes. When the new node starts and connects to the database, it will lock the database and own it while it completes the database upgrade. The original nodes might give errors, as they will not have access to the database. They can be removed. The new nodes can then be added to the cluster. The system will be down for the duration of the database upgrade, but once the database upgrade is complete, the system will be back online with the upgraded version of FusionAuth on the new nodes. 

++++
{% plantuml source: _diagrams/docs/admin-guide/blue_green_upgrade.plantuml, alt: "Blue green deployment." %}
++++
 
=== Out-of-Band Database Upgrades

While FusionAuth can handle database migrations automatically using link:/docs/v1/tech/guides/silent-mode[Silent Mode]. It can also handle updates interactively on development environments using link:/docs/v1/tech/installation-guide/fusionauth-app#maintenance-mode[Maintenance Mode]. Note that Maintenance Mode should never be used in production.

In production, you may want to perform the database upgrade manually. This is useful if you want to perform the database upgrade in a way that minimizes downtime or as part of a larger automated or coordinated deployment. Upgrading the database manually involves running SQL scripts directly on your database to update the database schema. 
...
If you want to use Maintenance Mode, ensure your runtime mode is set to `development` and Silent Mode is set to `false`. Maintenance Mode should never be used in a production environment, as is only intended for use in development environments.
