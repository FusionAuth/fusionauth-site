...
The easiest upgrade route is to simply stop all your services, upgrade each FusionAuth node, and then bring it all back online. In this case, FusionAuth will be performing the database upgrade, if needed, while the servers are stopped.
...
You can also stop all nodes and run the database migration SQL statements yourself, rather than relying on FusionAuth to run them. See <<Downtime and Database Migrations>> for more.
...
2. Upgrade FusionAuth on the first node. Once FusionAuth is updated, start it up so that it can perform the database migrations. FusionAuth will lock the database and own it while the database upgrade is completed. Note that there will be a schema mismatch between the first node and the other nodes, so the other nodes may give errors while the first node is being upgraded. The nature of these errors depends on the code paths being updated as well as how users interact with the system. *For core login functionality, they should be minimal.*
3. Add the first node back to the load balancer.
4. Remove the second and third nodes from the load balancer
5. Stop the second and third nodes.
6. Upgrade the second and third nodes.
7. Start the second and third nodes.
8. Add the second and third nodes back to the load balancer.

There is still downtime with this procedure, but it is limited to the time it takes to run the first node to run the database migration. There will be limited capacity during the upgrade (only the first node) while the second and third nodes are upgrading, but the system will be available for use for most of the upgrade process.
...
2. Start up one of the new nodes. When the new node starts and connects to the database, it will lock the database and own it while it completes the database upgrade. Note that there will be a schema mismatch between the new node and the original nodes, so the original nodes may give errors while the new node  is upgrading the database. The nature of these errors depends on the code paths being updated as well as how users interact with the system. *For core login functionality, they should be minimal.*
3. Add the new nodes to the load balancer.
4. Remove the old nodes from the load balancer.
...
**A:** We recommended migrating from one minor version to the next. Read the release notes carefully and test your upgrade.
