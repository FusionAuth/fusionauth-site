...
Sometimes, you will want to upgrade to a FusionAuth version that is a few versions newer than your current version. This is called a "version jump". Version jumps include minor versions. FusionAuth migration can run all the necessary database migrations to get you to the latest version. 
...
To perform a blue-green deployment with a three-node cluster, first create three new nodes with the new version of FusionAuth on each of them. Start up one of the new nodes. When the new node starts and connects to the database, it will lock the database and own it while it completes the database upgrade. The original nodes might give errors as they will not have access to the database. These original nodes can be removed. Now add the new nodes to the cluster. The system will be down for the duration of the database upgrade but once the database upgrade is complete, the system will be back online with the upgraded version of FusionAuth on the new nodes. 
...
If you want to use Maintenance Mode, ensure your runtime mode is set to `development` and Silent Mode is set to `false`. Maintenance Mode should never be used in a production environment, as it is only intended for use in development environments.
