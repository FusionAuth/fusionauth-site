...
Here are the constraints of the EAP:

* You can only install FusionAuht via zip, deb or rpm files. These will be available on link:/direct-download[the direct download page].
...
While the FusionAuth team will make every effort to release an EAP with stable APIs, they may change. In addition, there is no supported upgrade path from an EAP instance to a released version.

=== Usage Guidelines

Here are some expected use cases where an EAP instance may be helpful.

Testing a new install with existing configuration with the EAP version, using the following steps:
...
Testing a migration from a released version to the EAP version. To do so:
