...
In order to upgrade FusionAuth to a later version, there are a number of things to consider. This document will help you understand the upgrade process, and how to plan for it. It also covers detailed instructions on how to upgrade FusionAuth on various platforms and distributions.
...
If you want to interactively (called link:/docs/v1/tech/installation-guide/fusionauth-app#maintenance-mode[Maintenance Mode]), or you can run any needed scripts out of band (meaning directly on the database yourself). 

[NOTE]
====
You should always backup your database prior to using Maintenance Mode, or performing any database migrations by any method.
====.
...
{% include _image.liquid src="/assets/img/docs/admin-guide/upgrade/blue-green.svg " alt="A representation of a Rolling upgrade." class="img-fluid" figure=true %}
...
To perform a blue/green deployment with a 3 node cluster, first create 3 entirely new nodes. Add one of these nodes to the existing cluster. At this point, the new node will lock the database, and own it while it completes the database upgrade. The original nodes might start to error, as they will not have access to the database. They can be removed. The remaining new nodes can then be added to the cluster. The system will be down for the duration of the database upgrade, but once the database upgrade is complete, the system will be back online with the upgraded version of FusionAuth on the new nodes. 

{% include _image.liquid src="/assets/img/docs/admin-guide/upgrade/rolling-upgrade.svg" alt="A representation of a Blue/Green upgrade." class="img-fluid" figure=true %}
 
=== Out of Band Database Upgrades

While FusionAuth can handle database migrations completely automatically using link:/docs/v1/tech/guides/silent-mode[Silent Mode] or interactively using Maintenance Mode, you may want to perform the database upgrade yourself. This is useful if you want to perform the database upgrade in a way that minimizes downtime, or as part of a larger automated or coordinated deployment. The scripts for the database migrations are available in the FusionAuth upgrade packages. 
