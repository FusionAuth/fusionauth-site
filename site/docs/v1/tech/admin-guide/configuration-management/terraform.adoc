...
You can't destroy the default FusionAuth Application. In FusionAuth this can't be deleted. Attempting to do so will break your Terraform state. See <<Terraform Remove>> for more details.
====
...
In this example you create an Application called forum in the newly created forum Tenant with related roles (admin and user):
...
After you're done with your configuration changes, run `terraform plan` to check the planned changes and review potential errors. After that you can go ahead with `terraform apply`.
