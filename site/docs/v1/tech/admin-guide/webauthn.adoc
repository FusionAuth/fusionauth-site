...
The bootstrap workflow can be used to authenticate using WebAuthn from a new device. It can also be used for repeated logins from the same device, but the user must choose the option rather than being prompted like the re-authentication workflow. The options in the Bootstrap settings section control the availability of the workflow and its requirements for authenticator attachment and user verification.

[.api]
[field]#Enabled#::
Enabling this toggle will enable the bootstrap workflow for applications belonging to this tenant. The availability of the bootstrap workflow can be overridden at the application level using the link:#bootstrap-settings-2[Application Bootstrap settings].

[field]#Authenticator attachment#::
This setting controls which authenticator attachments are allowed for registration ceremonies when using the bootstrap workflow. See the section link:#authenticator-attachment[below] for recommendations on configuring this option for the bootstrap workflow.

[field]#User verification#::
This setting controls whether user verification is required for registration and bootstrap ceremonies when using the bootstrap workflow. See the section link:#user-verification[below] for recommendations on configuring this option for the bootstrap workflow.
...
Because the bootstrap workflow can be used to login from a new device using WebAuthn, it is recommended to use the `cross-platform` attachment by selecting the "Cross-platform only" option if available.

:advanced_feature: WebAuthn cross-platform authenticators
...
It is _highly_ recommended to require user verification for the bootstrap workflow as it can be used to authenticate a user.
