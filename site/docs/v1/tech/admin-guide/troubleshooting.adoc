...
FusionAuth uses Elasticsearch to dynamically build the schema, so it is critical that the schema not be modified manually.
...
Whenever link:/docs/v1/tech/core-concepts/search#reindexing-elasticsearch[you reindex], this setting will be lost, so ensure you can reapply it.
