...
The additional debugging information will be written to the <<Event Log>>. You can find in the administrative user interface by navigating to [breadcrumb]#System -> Event Log#.
...
If you are receiving an invalid code for your two factor authentication and you are using a link:/docs/v1/tech/tutorials/two-factor/authenticator-app-pre-1-26[time based one time password (TOTP) application such as Google Authenticator or Authy], confirm that the system time is correct on the server.
...
If you are rate limited but need these requests to occur, please open a https://account.fusionauth.io/account/support/[support ticket] with details.
...
Suppose you had a data field called `customPermissions` which was a `string` on some users and an `object` on others. Below is a script which converts the type of `customPermissions` from `string` to `object` for all users.

[source,javascript,title="Script to convert mismatched data fields"]
----
import FusionAuthClient from "@fusionauth/typescript-client"

const client = new FusionAuthClient('bf69486b-4733-4470-a592-f1bfce7af580', 'https://local.fusionauth.io')

const response = await client.searchUsersByQuery( request: {queryString: "email:*"})
...
Whenever link:/docs/v1/tech/core-concepts/search#reindexing-elasticsearch[you reindex], this setting will be lost, so ensure you can reapply it, via a post reindex build step.
There's an https://github.com/FusionAuth/fusionauth-issues/issues/1639[open issue] to expose this configuration from within FusionAuth.
...
video::MUHcB1kdaP8[youtube,width=560,height=315]
