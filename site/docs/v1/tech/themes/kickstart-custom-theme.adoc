...
The `"requests"` section defines the API requests that perform our automations. Here, each request is encased in a JSON file, which we need to define separately. You can also have them inline, but when you are working with a lot of changes, it is easier to have each change in a separate file.
