...
Create a new theme by navigating to [breadcrumb]#Customization -> Themes# and then click the Add button. Enter a name for the theme and click the Save button.

image::themes/tailwind/create-theme.png[Create a theme,width=1200]

Create a new API key with permissions to modify the theme. You can do this by going to [breadcrumb]#Settings -> API Keys#. Then click the Add button and select `GET` and `PATCH` permissions for `/api/theme/`.
...
At this point, we have prepared the project to build the Tailwind CSS file and upload the theme to FusionAuth. To apply the changes, run the following command:
```bash
./upload.sh
```
...
To preview the changes, open FusionAuth, navigate to [breadcrumb]#Customization -> Themes#. Choose your theme, then click the preview link (the green magnifying glass).
