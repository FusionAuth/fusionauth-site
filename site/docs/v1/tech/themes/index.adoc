...
[field]#Index# [required]#Required# [since]#Since 1.26.0#::
This is the root landing page. This page is available to unauthenticated users and will be displayed whenever someone navigates to the FusionAuth host's root page.
+
[uri]#/#

[field]#Account edit# [required]#Required# [since]#Since 1.26.0#::
This page contains a form that enables authenticated users to update their profile.
+
[uri]#/account/edit#

[field]#Account index# [required]#Required# [since]#Since 1.26.0#::
...
[field]#Account two-factor disable# [required]#Required# [since]#Since 1.26.0#::
This page contains a form that accepts a verification code used to disable a multi-factor authentication method.
+
[uri]#/account/two-factor/disable#

[field]#Account two-factor enable# [required]#Required# [since]#Since 1.26.0#::
This page contains a form that accepts a verification code used to enable a multi-factor authentication method. Additionally, this page contains presentation of recovery codes when a user enables multi-factor authentication for the first time.
+
[uri]#/account/two-factor/enable#

[field]#Account two-factor index# [required]#Required# [since]#Since 1.26.0#::
...
[field]#Email verification re-sent# [required]#Required#::
...
[field]#Email verification required# [required]#Required# [since]#Since 1.27.0#::
This page is rendered when a user is required to verify their email address prior to being allowed to proceed with login. This occurs when [field]#Unverified behavior# is set to `Gated` in email verification settings on the Tenant.
+
[uri]#/email/verification-required#
...
[field]#Index# [required]#Required#  [since]#Since 1.27.0#::
This is the landing page that is rendered when the root URL `/` is accessed.
+
Prior to version `1.27.0`, navigating to this URL would redirect to `/admin` and would subsequently render the FusionAuth admin login page.
+
[uri]#/#
...
[field]#OAuth authorized not registered# [required]#Required# [since]#Since 1.28.0#::
...
+
[uri]#/oauth2/authorized-not-registered#
...
This page is used if the user initiates an OAuth logout. This page causes the user to be logged out of all associated applications via a front-channel mechanism before being redirected.
+
[uri]#/oauth2/logout#
...
[field]#OAuth two-factor methods# [required]#Required# [since]#Since 1.26.0#::
...
[field]#Verify registration #re-sent [required]#Required#::
...
+
[uri]#/registration/sent#

[field]#Verify registration required# [required]#Required#  [since]#Since 1.27.0#::
This page is rendered when a user is required to verify their registration prior to being allowed to proceed with the registration flow. This occurs when [field]#Unverified behavior# is set to `Gated` in registration verification settings on the Application.
+
[uri]#/registration/verification-required#
...
[field]#SAML logout# [required]#Required# [since]#Since 1.25.0#::
...
[.api]
[field]#fields# [type]#[Map<Integer, List<FormField>>]#::
...
The email address you are attempting to disable, this will be available when the [field]#method# is `email`.
...
The mobile phone you are attempting to disable, this will be available when the [field]#method# is `sms`.
...
[.api]
[field]#availableMethods# [type]#[List<String>]#::
...
[field]#recoveryCodes# [type]#[List<String>]#::
...
[.api]
[field]#allowEmailChange# [type]#[Boolean]#::
When `true`, an additional form is displayed to allow the user to update their email address. This is intended to assist the user if they mis-typed their email address previously. This occurs when [field]#Allow email change when gated# is enabled under email verification settings on the Tenant.

[field]#collectVerificationCode# [type]#[Boolean]#::
When `true`, a form input is displayed to allow a user to enter the verification code. This occurs when [field]#Verification strategy# is set to `FormField` under email verification settings on the Tenant.
...
[.api]
[field]#completedLinks# [type]#[List<PendingIdPLink>]# [since]#Available since 1.30.0#::
An optional parameter that may be present if the completion of this device request resulted in one or more Identity Provider links being established to this user.
+
...
[field]#method# [type]#[TwoFactorMethod]# [since]#Available since 1.26.0#::
...
[field]#resendCode# [type]#[Boolean]#  [deprecated]#Deprecated#::
...
[field]#showResendOrSelectMethod# [type]#[Boolean]#::
...
[field]#userCanReceivePush# [type]#[Boolean]# [deprecated]#Deprecated#::
...
[.api]
[field]#availableMethodsMap# [type]#[Map<String, TwoFactorMethod>]#::
A map of two-factor methods that the user has configured. The key into the map is the unique [field]#methodId#. The value is the two-factor method.

[field]#methodId# [type]#[String]#::
The method Id. This id if defined will represent the currently selected method, or last used method.

[field]#recoverCodesAvailable# [type]#[Integer]#::
