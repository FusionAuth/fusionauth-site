...
Below we are going to extend the usefulness of our account management page by adding a field to the `User Self Service Form`.

To change a form:

1. Navigate to `icon:check-square[type=fas] Forms`
2. Click the `icon:edit[type=fas ,role=ui-button blue] Edit` on `Default User Self Service provided by FusionAuth`.
...
1. Make a copy of the default theme (or create your own) by navigating to `icon:paint-brush[type=fas] Themes > icon:copy[type=fas] Duplicate` .
2. Click on the `icon:edit[type=fas, role=ui-button blue] Edit` button of the duplicated theme.
