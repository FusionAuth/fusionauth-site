...
* <<See It in Action - Forms>>
* <<Update the Account Two Factor Index Template>>
* <<See It in Action - Account Home>>
...
[NOTE]
====
Even though we are adding `birthDate` below, FusionAuth forms allow custom form fields as needed. You could for instance, add a `user.data.favColor` if needed.
====
