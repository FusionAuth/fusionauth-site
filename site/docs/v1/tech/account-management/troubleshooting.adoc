---
layout: doc
title: Account Management - Troubleshooting
description: Managing Accounts - Troubleshooting Steps
---
:page-liquid:

include::docs/v1/tech/shared/_premium-edition-blurb.adoc[]

[NOTE.since]
====
Available since 1.26.0
====

== Troubleshooting

Not able to get your account management page up and running?  Below are two common popup warnings and how to fix them.

* <<Not Registered>>
* <<Registered But Configuration Needed>>

=== Not Registered

Please note, if the user is not registered to the application, they will not be able to self serve from the account management page.

To remedy --

From the Admin UI click on `icon:users[type="fas"] Users > icon:address-card[type="fas"] Manage >  Registrations Tab > icon:plus[type="fas"] Add registration > Add User` Another option is to programmatically add the user to the application through the API.

image::account-management/account-home-not-registered.png[Login Screen, width=1200, role=bottom-cropped]


=== Registered But Configuration Needed

If you see this, you have to select a form to use.

image::account-management/configuration-incomplete-account.png[Login Screen, width=1200, role=bottom-cropped]

To set a self service form, click under `Applications > icon:edit[] Edit Application > Registration`

image::account-management/add-form-account.png[Edit Application Screen - Registrations, width=1200]

Select the form you would like to use. A default form is shipped with FusionAuth.
