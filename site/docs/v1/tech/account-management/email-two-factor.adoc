...
Email two factor is not enable by default on the `tenant` level. To enable please navigate to `Tenants > icon:edit[type=fas] Edit Tenant > Multi-Factor Tab`
...
- The `Enabled` toggle
- A Two Factor Authentication template (FusionAuth ships with a default to be customized if needed)
