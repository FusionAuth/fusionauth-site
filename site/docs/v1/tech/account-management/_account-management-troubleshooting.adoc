...
There is a discusson in https://github.com/FusionAuth/fusionauth-issues/issues/1546[GitHub Issue 1546] on modifying the implementation of the self-service account management pages to remove dependency on the SSO session, which would remove the requirement for users to check [field]#Remember me# on the login screen.
