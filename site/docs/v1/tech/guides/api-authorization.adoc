...
FusionAuth offers have three solutoins, with different strengths and weaknesses.
...
To validate this credential, the Todo API would make a call against some FusionAuth API endpoint with the key. If the call succeeds, the Todo API returns the requested task data. If, instead, FusionAuth responds with a 401 response, the Todo API denes access.
