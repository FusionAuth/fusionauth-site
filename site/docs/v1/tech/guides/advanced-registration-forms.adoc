...
* Create custom form fields (optional). 
* Assemble pre-defined and custom form fields into steps, and steps into a form.
* Configure an application to use the form for self service registration.
* Theme the form (optional, but highly recommended).

This guide will outline all of these tasks.
...
Basic registration is only one step; all the fields are on one form. With basic registration, you can mix and match the following user data fields:
...
* Break a registration process into series of less imposing steps.
...
Add a maximum price field by duplicating the `minprice` field. Use a key of `maxprice`; keys must be unique within in the data field, `registration.data` in this case. Change the name as well. All other settings can be the same as those of the `minprice` field.
...
The second screen displays the custom fields: the minimum and maximum home prices and your area of geographic interest. 
...
Click "Register" to complete your sign up. You'll be sent to the configured redirect URL value and be signed in. 

==== The Admin View

Sign into the administrative user interface and navigate to [breadcrumb]#Users# section. There is a new account:
...
The form you built has a few rough user interface elements. You can create a beter user experience by theming the form.
...
You are going to add placeholders and labels, but there's a lot more you can do; check out the [theming documentation](/docs/v1/tech/themes/) for more information.
...
These messages are displayed to the user when the the minimum or maximum prices are `invalid`. Because these fields have the `number` datatype, they are `invalid` any time the user input is not a number, including the empty string.

If any of the values added to `defaultmessages.txt` contain a double quote, escape it: `\"`. Since the file will be eventually turned into a quoted JSON attribute and send to the API, an unescaped double quote creates invalid JSON and will cause the API call to fail.
...
To associate an existing consent with a field, select a [field]#field# of `Self consent`. See the link:../consents[Consent APIs] for more information on user consents.
...
Consents are rendered as a checkbox to the user in the registration from. 
...
If your field uses a form control with a limited set of options, such as a radio button or select dropdown, the user will be forced to choose from that set of options. XXX do we validate on the server side or just in the UX
...
Every time a user authenticates, FusionAuth ensures their registation is complete.

If you add a required field to the application's registration form after users have registered, the next time one of the users authenticates using the hosted login pages, they'll be sent to the registration form to fill out the required field.  XXX Test
...
For example, if you wanted to modify the real estate search form to make the minium price a drop down instead of a numeric input field, duplicate the existing form field and modify the control. Then update the form to use the new form field. 
