...
To associate an existing consent with a field, select a [field]#field# of `Self consent`. See the link:/docs/v1/tech/apis/consents[Consent APIs] for more information on user consents. Consents are rendered as a checkbox to the user in the registration from.
...
If you need to perform more complex validation of the registration form, use this lambda link:/docs/v1/tech/lambdas/self-service-registration[Self-Service Registration Validation lambda]. If you are on a plan which includes HTTP Lambda Connect, you can also make fetch calls to external APIs if needed to validate a registration.
...
----
include::docs/src/json/users/search-registration-data-range-query.json[]
----
...
You can avoid this for users lggging in with an Identity Provider by providing all required profile fields in a lambda.
...
image::guides/advanced-registration-forms/hidden-field.png[Example of hidden form field.,width=1200,role=bottom-cropped]
...
[source,freemarker,title=Generating a hidden field]
----
...
[source,javascript,title=Setting a hidden field]
----
