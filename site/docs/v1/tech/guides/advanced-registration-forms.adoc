...
First, create the notification hour field. This is when the application will remind them of their upcoming tasks. In this ficitious app, users will only be able to select a notification between the hours of 4 and 10 in the morning.

* Set [field]#Name# to `Notification Hour`.
...
Save this. Now let's add a field to find out if the user wants a second task list reminder sent 30 minutes after.

* Set [field]#Name# to `Second Reminder`.
* Set [field]#Field# to `Custom registration data` and specify the key `registration.data.secondreminder`.
* Set [field]#Form control# to `Radio`
* Set [field]#Data type# to `Boolean`.
* Enable [field]#Required#.
* Paste the following into [field]#Options# (these are the radio button options):

[%hardbreaks]
true 
false

Save this form field. Finally, create the "How did you find us?" form field. However, this will be optional. Nice data to know, but you don't want a user to fail to sign up because of this field.

* Set [field]#Name# to `How did you find us?`.
* Set [field]#Field# to `Custom registration data` and specify the key `registration.data.signupsource`.
* Set [field]#Form control# to `Textarea`
* Set [field]#Data type# to `String`.
* Make sure [field]#Required# is not checked.

Save this field. At the end of this, you should see your three new custom form fields added:
...
* Can't have validations depend on each other (for example, requiring a field to have a value if another field has a value, but be optional otherwise)
...
Can you advanced register two times for two different apps?

what are limits on the key names (json key, I assume, or whatever is in data field).
