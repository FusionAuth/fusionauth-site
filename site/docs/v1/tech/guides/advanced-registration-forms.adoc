...
image::guides/advanced-registration-forms/minimum-price-form-field.png[Adding the minimum price field.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/geographic-area-form-field.png[Adding the geographic area field.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/new-form.png[The blank form, ready to be assembled.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/first-step-form-editor.png[Adding fields to our first step.,width=1200]
...
image::guides/advanced-registration-forms/second-step-form-editor.png[Adding fields to our second step.,width=1200]
...
image::guides/advanced-registration-forms/find-registration-url.png[Finding the registration URL.,width=1200]
...
image::guides/advanced-registration-forms/first-step-no-placeholder.png[The first page of the custom registration flow.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/new-user-data-display.png[The user data tab of the newly registered user.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/update-theme-for-tenant.png[Select your new theme for the default tenant.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/first-step-with-placeholder.png[The first page of the registration form with the correct messages added.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/first-step-with-placeholder-and-labels.png[The first registration step with labels.,width=1200,role=bottom-cropped]
...
image::guides/advanced-registration-forms/flask-app-screen-shot.png[The user profile portal.,width=1200,role=bottom-cropped]
...
Advanced user registration forms add custom data to your users' profiles. However, what happens when the user data needs to change. While you can write code to run against the APIs to modify the profile data, using a custom admin form is easier. You don't have to write any code, just configure a form.

There are two types of profile data:

* User data, which is associated with the user. This could be in standard user fields such as `mobilePhone` or custom data stored in `user.data`.
* Registration data, associated with the user's registration with an application. This could be in standard fields such as `roles` or custom data stored in `registration.data`.

Each of these types of profile data has a custom admin form associated with it.

Admin user forms are associated with the tenant and admin registration forms are associated with an application. The default forms which ship with FusionAuth are implemented as admin forms, but can easily be replaced by custom forms suited to your business needs.

=== How To Use A Custom Admin Form
 
There are four steps to using a custom admin forms.

* Determine if you are going to create a custom registration form, user form or both.
* Consider whether data should be editable by admins.
* Create any custom fields.
* Assemble the fields into a form.
* Associate the form with the tenant or application, as appropriate.

Let's walk through each of these.

==== Custom Registration Form or Custom User Form

To determine if you are going to create a custom registration form, user form or both, think about where the data is stored.

If it is useful for more than one application the user is logging into, then the data should be stored on the user, either in a custom field (`user.data.somefield`) or one of the standard user fields. 

For instance, for the real estate application built above, data such as `user.data.currentClient` would be helpful for multiple applications. For a search application, you could display additional information to current clients. For a mortgage application, you could display additional interest rate or program information. An external system could be set up to trigger a welcome email when someone became a client.

If, on the other hand, your data is really only useful to a specific application, associate it with a registration. An example of that is the `registration.data.minprice` field. The minimum and maximum price points that someone is searching for only really apply to the real estate search application. That data won't be useful for other applications.

==== Should Data Be Editable by Admins

Next, consider if the data should be editable by administrators. Some of this user profile data will be submitted by the end user and will be read-only for admin users. An example of this would be a privacy policy choice such as whether a user wants their data shared with companies affiliated with a brokerage. Depending on your business rules, you may not want to expose this data to your admins in the backend FusionAuth UI. More typically, you'll want to allow your admin users to modify most user data.

You'll also want to consider fields which contain profile data not created at user registration time. For example, a `user.data.notes` field can be used to capture information about the user relating to their real estate needs. This field can be updated by customer service reps or realtors, but it doesn't make sense to have this on the initial registration form, as new clients won't be providing this information about themselves.

==== Create Custom Fields

If you want to have any fields in your custom admin forms that are not part of a user registration form you've created, such as the `notes` field mentioned above, you'll need to create the custom field. To do so, navigate to [breadcrumb]#Customizations -> Form Fields# and add the form fields.

image::guides/advanced-registration-forms/add-custom-field.png[Adding a custom form field.,width=1200,role=bottom-cropped]

You can use any validation, form controls or data types available. Don't forget to add the field names for any new fields to your theme's messages file. Otherwise users in the administrative user interface will see a field name like `user.data.notes` instead of `User notes`.

==== Build the Forms

The next step is to build the forms. Here you can use any of the custom or standard form fields you have created.

First, let's add an admin user form. Navigate to [breadcrumb]#Customizations -> Forms# and add a new form. Make sure that the [field]#Type# is set to `Admin User`. 

Add your form fields, including any custom data you store in `user.data` and ordering them as needed. Multiple sections may help organize the data if you have a large number of fields and want them logically grouped. Here we'll just add the forms we added for the user registration as well as our new, admin only, notes form. You'll end up with a form looking similar to this:

image::guides/advanced-registration-forms/add-admin-user-form.png[Adding an admin user form.,width=1200]

Then, create a custom admin registration form for the real estate search application. To add this form, navigate to [breadcrumb]#Customizations -> Forms# and add a new form. Make sure that the [field]#Type# is set to `Admin Registration`. 

Add your form fields, ordering them as needed. You can add multiple sections if desired. This is typically a good idea if you have a large number of fields and want to logically group them. Here we'll add the three custom registration fields we added for user registration: Geographic Area, Minimum Price and Maximum Price.

At the end, you'll end up with a form looking similar to this:

image::guides/advanced-registration-forms/add-admin-registration-form.png[Adding an admin registration form.,width=1200]

Next up, you'll need to associate the form with the tenant, for the admin user form, and the application, for the admin registration form.

==== Associate the Form

If your form is an admin user form, you'll modify the tenant settings. If, on the other hand, it is a admin registration form, you'll modify the application settings. In both cases, you can use either the administrative user interface or the API to update the tenant or application. Below you'll see how to make the changes with the administrative user interface.

To change the form used to edit users, navigate to [breadcrumb]#Tenants -> Your Tenant -> General -> Form Settings#. Then chose your custom form  as the [field]#Admin user form#.

image::guides/advanced-registration-forms/associate-admin-user-form.png[Associating an admin user form with a tenant.,width=1200]

To change the admin registration form, navigate to [breadcrumb]#Applications -> Your Application -> Registration -> Form Settings#. Then chose your custom form as the [field]#Form# value.

image::guides/advanced-registration-forms/associate-admin-registration-form.png[Associating an admin registration form with an application.,width=1200]

Once you've done this, you can see how it looks for an admin user to edit a user or a registration by, well, adding editing a user or registration. These forms will be used both for editing users or registrations as well as adding new users or registrations from the FusionAuth administrative user interface.

=== Limiting User Access to the FusionAuth UI

Using custom admin forms lets users access the FusionAuth web interface to manage custom user and registration data. This functionality is available out of the box with FusionAuth. But perhaps you don't want to expose all of the FusionAuth web UI and settings to the employees who only need to be able to update user profile data? 

FusionAuth roles to the rescue! The FusionAuth application has over 25 roles that offer fine grained control over functionality. Whether you want to let people only manage themes or webhooks, consents or lambdas, roles let you lock down access. 

If you have a user account to which you want to allow only access to the user management functionality, edit the account's FusionAuth registration and give them the `user_manger` role. With this role, they'll be able to add and update users. To prevent privilege escalation, they won't be able to modify their role or anyone else's.

image::guides/advanced-registration-forms/set-user-role-user-manager.png[Granting a user a role.,width=1200]

Make sure you set the role value to `user_manager`, and save the user. When they login to the FusionAuth interface they'll see a set of tabs limited to what they permissions for:

image::guides/advanced-registration-forms/limited-menu.png[The FusionAuth administrative user interface when viewed with limited privileges.,width=1200,role=bottom-cropped]

If you edit a user, you can see the edit screen is exactly as configured above:

image::guides/advanced-registration-forms/user-edit-screen.png[Editing a user with a custom form.,width=1200,role=bottom-cropped]

And the same is true for adding or editing a registration for the application to which you've assoicated the form:

image::guides/advanced-registration-forms/registration-edit-screen.png[Editing a user application registration with a custom form.,width=1200,role=bottom-cropped]

If a user manager edits the URL and try to get to other areas, they'll see a message letting them know that access is not authorized:

image::guides/advanced-registration-forms/unauthorized-access-user-manager.png[Unauthorized access: denied!.,width=1200,role=bottom-cropped]
