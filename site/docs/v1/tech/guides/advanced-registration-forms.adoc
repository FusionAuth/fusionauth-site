...
You'll have to modify the freemarker code which generates the registration form. The `customField` macro is what generates the HTML, so you could add something similar to

[source,freemarker,title=Generating a hidden field]
----
[#if field.key == "user.data.referrer"]
  <input id="${fieldId}" type="hidden" name="${key}">
[/#if]
----

Set this form field value using JavaScript.
