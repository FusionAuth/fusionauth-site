...
You can use any validation, form controls or data types available.

==== Build the Form

The next step is to build the form. Here you can use any of the custom or standard form fields you have created.

Let's go ahead and create a custom admin registration form for the real estate search application and add a notes field for all users in our tenant.

To add the admin registration form, navigate to [breadcrumb]#Customizations -> Forms# and add a new form. Make sure that the [field]#Type# is set to `Admin Registration`.

Add your form fields, ordering them as needed. You can add multiple sections if desired. This is typically a good idea if you have a large number of fields and want to logically group them.

At the end, you'll end up with a form looking similar to this:

pic TBD

Next, let's add an admin user form. Navigate to [breadcrumb]#Customizations -> Forms# and add a new form.

Add your form fields, including any custom data you store in `user.data` and ordering them as needed. Multiple sections may help organize the data if you have a large number of fields and want them logically grouped.
...
And if they edit the URL and try to get to other areas, they'll see a message letting them know that access is not authorized.
