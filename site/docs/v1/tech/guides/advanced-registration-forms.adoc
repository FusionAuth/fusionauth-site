...
Changing datatypes for the same underlying key into `registration.data` or `user.data` is problematic if you are using Elasticsearch. This data is indexed and changing data types is bad. It is suggested that you change the key name if you must change the data type.
