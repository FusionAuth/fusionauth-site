...
An alternate option is to deny access if the user has not configured any MFA method, since a user with an MFA method enabled will have used it on log in.
If using this option, configure the `twoFactorTrustIdTimeToLiveInSeconds` setting to a small value.
