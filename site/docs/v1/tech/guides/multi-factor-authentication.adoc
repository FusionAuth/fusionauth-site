...
image::guides/multi-factor-authentication/application-settings-mfa.png[Configuring allowed MFA methods on the application.,width=1200,role=bottom-cropped]

Please see the link:/docs/v1/tech/core-concepts/applications#multi-factor[Application configuration documentation for more information].
...
Since this involves sharing secrets and verifying possession of email accounts or mobile phones, you cannot enable MFA using the FusionAuth administrative user interface. 

You have three options:

* If you have a paid edition, users may use the self service account management feature to enable MFA for their accounts. link:/docs/v1/tech/account-management/[Learn more about that option here].
...
You are `PATCH`ing the `twoFactor` array; doing so multiple times adds to the array each time (rather than setting it to exactly what you provided in the API call). https://github.com/FusionAuth/fusionauth-issues/issues/441[Read this tracking issue for more info.]
...
You can read more about updating a user in the link:/docs/v1/tech/apis/users[User API docs].
...
You'll also need to modify the link:/docs/v1/tech/email-templates/[email or message templates] if using email or SMS methods. 
...
image::guides/multi-factor-authentication/code-sent-after-user-mfa-enabled.png[The user experience with one MFA method enabled.,width=1200,role=bottom-cropped]
...
* If you have a paid edition, users may use the self service account management feature to disable MFA for their accounts. link:/docs/v1/tech/account-management/[Learn more about that option here].
...
A user may also remove an MFA method by using the self service account management (if you have a paid edition) by clicking the `-` link. link:/docs/v1/tech/account-management/[Learn more about that here.]
...
https://github.com/FusionAuth/fusionauth-issues/issues/441[Read this tracking issue for more info.]
...
This is an overview of the process. link:/docs/v1/tech/apis/two-factor[See the API docs for all parameters for these API calls].
...
If you do not have a paid FusionAuth edition, you are using the community edition. link:/pricing[Learn more about the various editions]. Due to the complexity of the new MFA implementation, SMS based MFA is no longer part of the community edition as it was before version 1.26.
...
* link:/docs/v1/tech/core-concepts/roadmap/[Check out the roadmap guidance] to see if your desired method is going to be added soon. If not, the roadmap documents various ways you can request a new feature.
...
include::docs/v1/tech/guides/_mfa-migration.adoc[]
...
Or you might want to force every user with a certain role to provide an additional factor every time they log in.
For example, super admins might need to provide a second factor every time they log in.

In FusionAuth, if a user has one or more additional factors configured for their account, they will be forced to provide that second factor on login.
Once someone has provided a second factor, they will not have to provide it again until a certain amount of time has passed.
This duration is configurable at the tenant level, and is set by the `twoFactorTrustIdTimeToLiveInSeconds` setting.

=== Forcing a User to Provide an Additional Factor Before Accessing an Application

You may want to force a user to perform MFA before they can access a sensitive application, such as the accounting application mentioned above.

One way to do that is to add link:/docs/v1/tech/guides/multi-factor-authentication#step-up-auth[step up authentication] to your application.
Each time the user accesses the application, perform a step up, which will force them to provide an additional factor.

An alternate option is to deny access in the application by augmenting the JWT based on whether the user has any MFA methods:

* If the user has not configured an MFA method, you know they haven't provided an additional factor.
* If the user has configured an MFA method, you know they have provided the additional factor when they logged in and the device was untrusted.

If using this method to enforce MFA, configure the `twoFactorTrustIdTimeToLiveInSeconds` setting to a small value, as when that duration elapses, the device will become untrusted and MFA will be required at login.

To check whether MFA is enabled, put a value in the JWT with a link:/docs/v1/tech/lambdas/jwt-populate[JWT Populate Lambda].

[source,javascript]
.Example lambda which adds MFA enabled claim
----
function populate(jwt, user, registration) {
  jwt.hasMFAEnabled = user.twoFactor && user.twoFactor.methods && user.twoFactor.methods.length > 0;
}
----

When your application receives the JWT, check the value of this claim.

If the claim is missing or `false`, the application knows that the user has not completed an additional factor, and can deny access.
Such a check can happen in the application or in a proxy.

Having FusionAuth https://github.com/FusionAuth/fusionauth-issues/issues/763[manage this check is an open feature request].
Please review the issue and comment on or upvote it if you'd like to see this handled by FusionAuth.

=== Forcing a User to Provide an Additional Factor Every Time

As mentioned above, you may want certain types of users to provide an additional factor every time they log in.

You can use the same step up authentication method as mentioned above. Using the "super admin" example, if the application sees a user who has that role, it can force a step up.

To avoid modifying your application, you can also examine the user during authentication and modify parameters sent to FusionAuth.

If you are using the hosted login pages, you can set [field]#trustComputer# to `false` every time a user visits the link:/docs/v1/tech/themes/template-variables#oauth-two-factor[`/oauth2/two-factor` page] and has a role of "superadmin".
You may also remove the theme code which sets that variable, in which case it will default to a value of `false`.

If you are using the Login API, don't send the [field]#twoFactorTrustId# value returned link:/docs/v1/tech/apis/login#complete-multi-factor-authentication[after completing a multi-factor authentication].

== Troubleshooting

// TODO break out to an include for including into troubleshooting guide

If using the email MFA method and your emails don't get sent, ensure you have configured your SMTP settings correctly. See the link:/docs/v1/tech/admin-guide/troubleshooting#troubleshooting-email[Email Troubleshooting] section for more.
