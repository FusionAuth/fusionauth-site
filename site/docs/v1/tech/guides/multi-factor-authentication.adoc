...
In the image below, all FusionAuth supported methods are enabled for this tenant: 
...
If you have a paid edition, users may also use the self service account management feature to enable MFA for their accounts. link:/docs/v1/tech/account-management/[Learn more about that option here.]

These are the steps to take to use the API to enable MFA for a single user:
...
Additionally, using this API is not required, you may optionally build your own secret. The API is provided for your convenience only.
...
Once the code has been sent or the secret shared, build a page to accept the code from the user. 
...
Unless you are using the self service account management, build this page in your application.
...
This sends an email, because that is the method corresponding to the `methodId` provided. 
...
Step up authentication allows you to be extra certain that a user is who they say they are. You can use this in your application to protect sensitive actions, such as account deletion or sending money. Because step up auth is embedded in your application, since only you know what type of actions require the additional certainty, you always need to implement it by calling the FusionAuth APIs.

To use step up auth, do the following:
...
Specify a code if you don't want FusionAuth to generate one. You can also provide a `state` object containing arbitrary JSON. This is returned to you after the step up auth process completes, and can be useful for the application when step up auth finishes. 
...
When they select one, you can send the code if they choose a message based MFA method. If you provided your own code, do not use FusionAuth to send it; FusionAuth will create a new code. Instead, send the code out of band.

=== Optionally Send the Code

This is only required if the user chooses a message based MFA method and you want to send the message with FusionAuth. Unlike with the login API, you don't have to send the code using the send API when doing step up auth. 

You only need to get the code to the user somehow. In fact, the user doesn't even need to have MFA enabled. You could, for example, build your own integration with a chat service like Slack and send the code to the user that way. Any out of band method, even https://tools.ietf.org/html/rfc1149[carrier pigeon], would work.
...
Build a screen or page to collect the code. When you have it, complete the step up by calling the Login API. This will return the user object if the provided code is valid or one of the other return codes documented in the Login API if it is not.
...
If you do not have a paid FusionAuth edition, you are using the community edition. link:/pricing/editions/[Learn more about the various editions]. Due to the complexity of the new MFA code, SMS based MFA is no longer part of the community edition as it was before version 1.26.

Therefore if you want to use message based MFA or other features such as the link:/docs/v1/tech/account-management/[self service account management], purchase a paid license. 

Google Authenticator and time based one-time password MFA will continue to work in the community edition. It has been improved and you can now add multiple authenticator devices to one user account.
...
* TOTP configuration
* Safe default MFA tenant settings

If you find your data has not migrated correctly, https://github.com/fusionauth/fusionauth-issues/issues[please file an issue] and let us know.
...
* link:https://fusionauth.io/docs/v1/tech/core-concepts/roadmap/[Check out the roadmap] to see if your desired method is going to be added soon. If not, https://github.com/fusionauth/fusionauth-issues/issues[file an issue] explaining your use case.
...
If using the email MFA method and your emails don't get sent, ensure you have configured your SMTP settings correctly.

If using the SMS MFA method and your text messages don't get sent, ensure that you have tested the SMS messenger configuration.
