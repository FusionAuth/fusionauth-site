...
There are times when you might want to force a user to MFA before they ever get access to your applciation, or force every user of a certain type to MFA.

These are two different situations.

=== Forcing a User to Before Accessing an Application

You may want to force a user to perform MFA before they can access a certain sensitive application, and to deny them access if they have not enabled MFA.
To do so with FusionAuth, the easiest way is to put a value in the JWT using a link:/docs/v1/tech/lambdas/jwt-populate[JWT Populate Lambda].
...
Then, when your application receives the JWT, it can check this claim.
If the claim is missing or `false`, the application can force the user to add an MFA method or otherwise display an error.
...
=== Forcing a User to Always MFA

Any user with an MFA method enabled will be forced to MFA if they have not before on a given device.
You can modify how long a device will be trusted by modifying the `twoFactorTrustIdTimeToLiveInSeconds` setting on the tenant.
Set this value to `1` if you'd like a user to MFA every time they log in.
