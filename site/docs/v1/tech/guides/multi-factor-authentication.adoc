...
To enable MFA for user, there are two paths. You can use the API to directly add a method or multiple methods to a user, or you can build an interface.
...
You are `PATCH`ing the `twoFactor` array; doing so multiple times adds two factor methods each time. https://github.com/FusionAuth/fusionauth-issues/issues/441[Read this tracking issue for more info.]

If adding adding a TOTP factor, make sure you share the secret so that the user may enter it into their authenticator application.
...
Finally, you can remove an MFA method using the API. Again, you can do so in two ways.

First, you can directly update the user using the User API. 


Or, you can build your own UX to send a code and disable the MFA method. For this approach, you need to:
...
To do this, you'll need to update `twoFactor` array.

[source,shell]
.Deleting MFA methods to a user directly
----
API_KEY=...
...
The reason you need to retrieve the user and modify the data, then use `PUT` is because of how `PATCH` handles arrays.
https://github.com/FusionAuth/fusionauth-issues/issues/441[Read this tracking issue for more info.]

=== Disabling MFA via the API and Your UX
