...
Finally, decide if you are going to pursue a big bang, segment by segment or migrate on authentication strategy.
...
You will also want to tweak settings to ensure an import happens as fast as possible. 
...
A big bang migration from an auth system into FusionAuth will involve migrating all your users at once. 

First, let's talk about building out the migration scripts.

You'll want to stand up a FusionAuth instance to test the migration process. To get your test FusionAuth instance into a known state, you may want to configure link:/docs/v1/tech/installation-guide/kickstart[Kickstart]. This file will also serve as a foundation for developers and CI servers in the future.
...
If you only provide a `password` field, then we will hash it for you; FusionAuth will assume the password is in plaintext. If you provide the salt, password, encryptionScheme and factor then we will assume the value in the `password` field is a hashed password, and it will not be hashed.
...
Here's an example of an import user JSON request body:
...
Once this is all tested in your testbed environment, you can prepare to go live. Make sure you inform all the internal stakeholders. Plan for downtime unless you can run your application with readonly user data. The amount of downtime is dictated by the amount of time the import took on the testbed.
