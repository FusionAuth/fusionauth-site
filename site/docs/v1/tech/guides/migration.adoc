...
image::guides/migration/add-tenant.png[Adding a tenant.,width=1200,role=bottom-cropped]
...
If the original system hashes passwords using an algorithm other than those link:/docs/v1/tech/reference/password-hashes/[schemes FusionAuth supports], write and install a link:/docs/v1/tech/plugins/custom-password-hashes/[custom password hashing plugin]. In either case, specify the scheme in the user import JSON file. (It is called an `encryptionScheme` for backwards compatibility, but is actually a hashing scheme.)
...
If you are having issues importing users, review the link:/docs/v1/tech/troubleshooting/#troubleshooting-user-import[relevant troubleshooting section].
