...
pic TBD just highlight the delete button on the connector policy
...
* Archive or delete the accounts and the corresponding data. When a user comes to your site and tries to sign in, they won't have an account and may be forced to re-register. 
* Move them to FusionAuth via a big bang migration.
* Extend the time running both systems; continue the slow migration.
