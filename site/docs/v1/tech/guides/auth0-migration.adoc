...
You may or may not want to use the `-m` switch, which takes the auth0 Id for non social users and turns that into the FusionAuth user Id. If you have external systems reliant on the Auth0 user identifier, set this. Doing so ensures imported users have the same Id as they did in Auth0. Otherwise, you can omit this switch.
