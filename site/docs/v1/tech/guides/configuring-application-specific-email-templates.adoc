...
In addition to calling the link:/docs/v1/tech/apis/users#start-forgot-password-workflow[Start Forgot Password Workflow] API, you can also use the FusionAuth hosted pages to send application-specific emails. If you choose the latter, you can add a query string parameter to the `Forgot your password?` link found on the FusionAuth link:/docs/v1/tech/core-concepts/integration-points#hosted-login-pages[hosted login screen].

To get started, go to [breadcrumb]#Applications# and click on the green magnifying glass icon for your application and copy the `Login URL`. Open a new incognito tab in your browser, and paste in the url. Next, on the login page, right-click the `Forgot your password?` link and select `Copy Link Address`. Then, append `&applicationId=YOUR_APPLICATION_ID` to the end of the link address, replacing `YOUR_APPLICATION_ID` with the Id of the application that is configured to use the custom `Forgot Password` Email Template. Paste the link with the appended `applicationId` into the browser.

To initiate the Forgot Password process, enter your user's email address on the Forgot Password page and submit the form. This action will trigger the delivery of an application-specific `Forgot Password` email to your user's inbox.