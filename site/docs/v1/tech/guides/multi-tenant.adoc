...
* There is no possibility of intermixing of user data.
...
To make things a bit more concrete, let's use an example. Suppose you wanted to build a Pied Piper Video Chat (or PPVC for short) SaaS application. Similar to Slack, but with a patented middle out compression algorithm. This application will support multiple different, entirely distinct groups of users. Each user who signs up picks their own hostname, which sets up an application that can be logged into by others. 

The PPVC will charge each instance a rate of $7/user/month. The creator of the PPVC application will be able to pick a hostname and a display background color. If someone from Hooli joins, they'll have `hooli.piedpipervideochat.com`, for example. Slack uses hostnames to distinguish between different tenants, and PPVC will as well.
...
There are a few objects which are globally available. These are typically available under the `system` path in the API namespace.
...
Data plane applications offer functionality to PPVC's users' users. So Hooli employees will log into `hooli.piedpipervideochat.com` to chat amongst themselves, and Raviga employees will do the same in the `raviga.piedpipervideochat.com` application. Within each data plane application, users can have roles and other account options such as payment or language preferences. 
...
* The hostname: `raviga.piedpipervideochat.com` points to the Raviga instance of the application. `hooli.piedpipervideochat.com` points to the Hooli instance. As mentioned above, this is similar to how Slack operates.
...
* Any needed metadata of the data plane application the end user cannot modify. For example, the PPVC application might allow users at a premium level to have ten chat rooms, whereas users at the basic level could only have two. 

Since the application is not usable for the new tenant without these configuration settings, this configuration should take place at the moment of signup, or the user should be notified both that the application is being created and when the creation is done.
...
* Configures the data plane application to deliver proper functionality. The `raviga` tenant might be on a premium plan and be allowed ten chat rooms, whereas the `hooli` tenant might be on a basic plan and have a lower limit.
* Can be used for reporting as your business grows. 

Some attributes you might need in your tenant object when implementing multi-tenancy with FusionAuth:
...
This guide will walk through pieces of creating the control plane application that a user can log in to and create or view details of their tenant:

image::guides/multi-tenant/control-plane-view.png[The control plane application interface.,width=1200,role=bottom-cropped]

The users of the multi-tenant application will then be able to register or log in at a different hostname into a dataplane application and see the chat window (or at least a note about the chat functionality that will later be built out).
...
Each of the data plane applications will be in a different FusionAuth tenant. Different users can have the same email address, as you can see in the below screenshot, where the `jian@fusionauth.io` user exists in the default tenant and the `ppvctest2` tenant:
...
This implementation maps each tenant to a single user, because `user_id` is a foreign key into the local `user` table. As mentioned above, you could choose to allow users to create multiple tenants. 

You'll need to create a user interface to manage this data as well populate the application specific fields like the background color and host. 
...
* You can use the new key for further configuration of this FusionAuth tenant. This follows the principle of least privilege and ensures you won't accidentally affect any other tenants.
...
* Allow users to sign in with an enterprise provider such as an OAuth or SAML compatible identity provider.
...
* Add a user or set of users and register them for the FusionAuth application, so they'll be able to log in.
* Set up roles.
* Set up groups.
...
Make sure you are passing the `Host` header through to the application code, since that is what is going to look up the tenant object based on the hostname.

==== Look Up the OAuth Configuration Based On the Hostname

Once the request is received by your application, you can look up the Client Id and Secret in the tenant table in your database based on the incoming request's hostname. When a request comes in (to `https://raviga.piedpipervideochat.com/login`, for example), you can map from the hostname (`raviga`) to the appropriate Client Id. Here is some sample code:
...
The final auth specific functionality for supporting multiple tenants is handling the OAuth callback. You may recall that in the <<Creating a FusionAuth Application>> section, you configured a FusionAuth application with a given `authorizedRedirectURLs` value. It was something like: `https://raviga.piedpipervideochat.com/login/callback`. And in the most recent section, you saw code that looked like `$redirectURI = '...';`, which is the same value.

You must create a part of your web application to respond to requests for the redirectURI path. The code must do the following:
...
After you've set up multiple tenants, there's still more work to build a professional grade multi-tenant SaaS application, but you have a solid foundation. Your users will be able to create their own data plane applications. And your user's users will be able to log in using FusionAuth.

Other items to consider:

=== Control Plane Codebase Vs Data Plane Codebase

Should you create the control plane application (which lets users create tenant applications) and the data plane applications (the application which actually lets users use the SaaS functionality, such as the PPVC) as two separate applications with different codebases? 

Both of these applications will be using the `tenant` object (the control plane to write it, the data plane to read it), so you'll need to either share a database or have some other way of syncing up that data. 

If you use the same web application, limit access to the controllers based on whether the control plane or a data plane application is executing. 

=== Framework Specific User Objects

Oftentimes a web framework will require a local record of the user. This will be used for permissions checks within the framework. They may even require a unique email address.

However, FusionAuth tenants have a separate userspace. In other words, I can sign up for `raviga.piedpipervideochat.com` and `hooli.piedpipervideochat.com` with the same `richard@fusionauth.io` email address. 
...
In addition, add a field in the local user object referencing the FusionAuth user Id. For FusionAuth, you may also want to add a `data.frameworkUserId` field, which will store the identifier for the local user object. This will let you map back and forth between the local user representation and the one stored in FusionAuth, as needed. 

When you need the user's email address, retrieve it from FusionAuth.

=== User Hostname Choice

If you allow users to pick their hostnames, ensure that you have a process for handling collisions.

You also should have a manual process for dealing with squatting. For example, someone may sign up with the hostname `piedpiper`, but you probably don't want `piedpiper.piedpipervideochat.com` to be in the hands of anyone except the company running PPVC.
