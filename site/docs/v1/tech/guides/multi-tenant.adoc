...
The users of the multi-tenant application will then be able to register or log in at a different hostname into a dataplane appliation and see the chat window (or at least a note about the chat functionality that will later be built out).
...
Often times a web framework will require a local record of the user. This will be used for permissions checks within the framework. They may even require a unique email address.
