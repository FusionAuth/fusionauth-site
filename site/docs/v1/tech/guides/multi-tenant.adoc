---
layout: doc
title: Multi-tenancy in FusionAuth
...
* To easily manage multiple developer environments. Having each developer use a tenant on a dedicated development server allows developers free reign to add, remove and modify configuration, while keeping them relatively insulated from each other.
...
This guide covers logical isolation of tenants. If you need physical isolation with FusionAuth, run different instances. You can also link:/learn/expert-advice/identity-basics/multi-tenancy-vs-single-tenant-idaas-solutions/[read this article for more information]. 
...
The creator of a PPVC tenant will be able to pick a hostname and a display background color. If someone from Hooli joins, they could set up `hooli.piedpipervideochat.com`, for example. As mentioned above, Slack uses hostnames to differentiate tenants, and PPVC will as well.
...
Unapologetically borrowing from https://en.wikipedia.org/wiki/Control_plane[network routing], let's use the concept of a control plane application and data plane applications. A control plane application manages user and tenant creation. This application is where you can integrate billing and general account management if charging for your application. Then there's the application that users will access to actually video chat. These will be called data plane applications. 

You need to build both of these, and some integration glue, to have a successful SaaS application. Here are common components for a multi-tenant SaaS application:
...
Let's look conceptually at each of these. 

=== The Control Plane Application

When you have a multi tenant SaaS application, you need a way to manage those tenants. For our example, PPVC must have a web or mobile application where users can sign up for Pied Piper Video Chat for their company. The users may be able to do other tasks, like set up the hostname, customize the look and feel, and set up billing. PPVC needs to charge per user per month fee, after all, otherwise you won't be able to afford a car with doors that open by going up. But this control plane application doesn't allow video chat. The control plane application is basically a tenant management portal.

For PPVC, this app will likely be a web application, but the actual implementation details don't matter much for the purposes of illustrating multi-tenancy concepts.
...
Tenant management is not part of the data plane applications. There also may be little or no overlap between the users of this application and the control plane application. 

For PPVC, the data plane application will also likely be a web application, but it could be an API used by a mobile application if that worked better for the business. The actual implementation details don't really matter for the purposes of this guide.

=== FusionAuth Tenants and Applications

For each of the web, mobile or other applications built, corresponding FusionAuth configuration objects need to be created. The control plane application will have a corresponding FusionAuth tenant where the users of the control plane application will be stored. It will also have a FusionAuth application object which contains application specific settings, such as whether a user can self register. Each data plane application will also have both a FusionAuth tenant and a FusionAuth application. 

For the PPVC control plane web application, the FusionAuth objects need to be created. Each time a user signs up and creates a new PPVC host, a new data plane tenant will also be created. Again, this is similar to how Slack works--I can create a `dan1234` Slack tenant and someone else can create a `jen1234` Slack tenant. 

If an employee at Raviga signs up for PPVC, a new FusionAuth tenant object will be created for Raviga, and within that tenant a new FusionAuth application object. This Raviga tenant is where all users of the Raviga PPVC application would be stored. 

The mapping between the data plane application configuration and the corresponding FusionAuth tenant and application objects must be stored somewhere. In this example, each data plane application has a hostname and display color, as well as FusionAuth configuration information. This additional metadata should be stored in the application database. This concept will be discussed more deeply in <<The Tenant Object>> section.

=== Data Plane Application Tenant Determination

The data plane web application needs to differentiate between different tenants. If the codebase is hosted at `piedpipervideochat.com`, there are a few ways to send incoming request to the appropriate tenant.

* The hostname: `hooli.piedpipervideochat.com` can point to the Hooli instance of the application. As mentioned above, this is similar to how Slack operates.
...
In many cases, a distinct hostname is the easiest way to differentiate tenants. It is memorable to the user, works well with internet standards such as cookies, and scales well. That's the approach this guide will take.
...
* You need to set up the hostname. When someone signs up with the `hooli` hostname, you want `hooli.piedpipervideochat.com` to point to your web application. This could be handled with DNS wildcarding or by updating DNS records automatically. 
* The FusionAuth tenant and a FusionAuth application must be created and configured. These objects allow the new data plane application to log users in.
...
* Possibly creating an initial user in the data plane application.
* Any needed metadata of the data plane application that the end user cannot modify. 

Since the application is not usable for the new tenant without these configuration settings, this configuration should take place at the moment of signup. There may be other optional configuration that can be performed asynchronously.
...
* Hostname: a hostname which users of a given organization will access to get to the PPVC chat application. Something like `hooli.piedpipervideochat.com` or `hooli` if you will always append a domain name.
* FusionAuth tenant Id: the tenant created in FusionAuth during setup.
* FusionAuth Client Id and Client Secret: this allows you to build links for users to log into the data plane application.
...
image::guides/multi-tenant/control-plane-view.png[The control plane application interface.,width=1200,role=bottom-cropped]
...
The users of data plane app can register or log in and see the chat window (or at least a note about the chat functionality that will later be built out). 

image::guides/multi-tenant/data-plane-logged-in-view.png[The data plane application interface.,width=1200,role=bottom-cropped]

Each of the data plane applications are in a different FusionAuth tenant. Different users can have the same email address, as you can see in the below screenshot, where the `jian@fusionauth.io` user exists in the default tenant and the `ppvctest2` tenant:

image::guides/multi-tenant/users-dashboard-fusionauth.png[FusionAuth user dashboard.,width=1200,role=bottom-cropped]

== Setting Up the Multi-Tenant System

Let's cover the steps to build a multi-tenant SaaS application specific to using an external identity provider like FusionAuth. FusionAuth will be used as the user database for the control plane and all data plane applications. 

Here are the tasks to set up an application skeleton, configure FusionAuth and integrate them both.

* Configure the control plane application in FusionAuth and set up your application to use FusionAuth for user sign up and login.
* Create a `tenant` table in your database, an object in your application, and CRUD methods to manage it.
...
* The callback handling for the Authorization Code grant within your application.

Let's look at each of these from the perspective of PPVC. If you'd prefer, you can download, review and run a https://github.com/FusionAuth/fusionauth-example-symfony-multitenant[fully functional multi-tenant chat application], video chat functionality not included.

=== Initial FusionAuth Configuration

The control plane application needs to be configured in FusionAuth. Create this in its own tenant to increase isolation. Configure the following on the application:

* An authorized redirect URL. 
* Enable the Authorization Code grant.
* Enable self service registration, if desired.
* Set up application specific roles, if desired.

Here's what the [breadcrumb]#OAuth# tab will look like:
...
Below, basic self service registration is enabled. Doing this will allow a user to sign up for the control plane application themselves. If you do not enable this, create the users in some other fashion using the link:/docs/v1/tech/apis/users/[User API].
...
You also need to create a Key Manager API key. Navigate to [breadcrumb]#Settings -> API Keys# and create a global API key. Make sure you enable [field]#Key manager# as this will be used to mint tenant scoped API keys.
...
The tenant object exists in your application. As outlined in <<The Tenant Object>>, this entity stores all tenant related information, whether for FusionAuth or application specific. Here's a sample `tenant` table definition:

[source,sql,title=Tenant object creation DDL]
----
CREATE TABLE `tenant` (
...
[NOTE]
====
You could use link:advanced-registration-forms/[Advanced Registration Forms], a premium feature, to capture tenant setup information, such as hostname, on user registration. Then you could use a webhook to create the FusionAuth configuration and the tenant object in your application database. This is an alternative implementation.
====

The `hostname` is an important field which will repeatedly be used in your multi-tenant application. The `hostname` is how the code differentiates between different tenants.

You may either let the user pick the application hostname (such as `hooli`) or assign it (`tenant123`). In either case, the `hostname` will typically be prepended to the domain name (in this case, `piedpipervideochat.com`) to create an address that end users of PPVC can visit to chat their hearts out. 
...
[NOTE]
====
You can use link:/docs/v1/tech/client-libraries/[any of the client libraries] to create FusionAuth configuration. This document happens to use PHP, but use whatever language your application is built in.

You can manipulate FusionAuth with client libraries by creating JSON objects as documented by the link:/docs/v1/tech/apis/[APIs] and then calling the corresponding operation.
====

How you manage FusionAuth configuration depends on your application framework and requirements. Options for calling the FusionAuth APIs to manage the new FusionAuth tenant configuration include:
...
For the example application, the last method is used. Before the tenant information is saved to the application database, the FusionAuth APIs are called and FusionAuth configuration is done. However, what makes sense depends on how many new tenants you expect to be created and how much configuration you need to do. If, for example, large numbers of PPVC users were being imported for each data plane application creation, asynchronous execution would work better. 
...
Store this key in your application's database in the `tenant` table. Both the key and the key Id must be stored. The key can be used to make other API calls. The API key Id can be used to modify the key later. 
...
At this point, you are creating the configuration for the FusionAuth application. An application is anything a user can log in to. In this case, the users logging into this application will be using the PPVC video chat. Users like Patrice or Jason Winter might be logging into the Hooli instance of the PPVC application. The hostname (`hooli`) is required for application configuration, as there are a number of URLs which must be configured.
...
There is an application base url, something like `https://hooli.piedpipervideochat.com`, which is constructed. This will serve as the foundation for any URLs or other data plane application specific configuration.

Users will be logging into this FusionAuth application using the OAuth Authorization Code grant, so the `oauthConfiguration` field needs to be set up. In that section, the code:
...
Once this application is live, the Hooli users need to be created. You have some options:
...
In this case, self service registration is enabled. Any of the above options will work when configured, though. For instance, if you wanted to allow users to sign in with Google, create an identity provider in the <<Initial FusionAuth Configuration>>, and then assign the identity provider to the application configuration for a new data plane application.

After the FusionAuth configuration is set up, store the application's Id and the application's Client Secret. These values will be needed later.
...
Now that FusionAuth tenants and applications are automatically created every time a user registers in the control plane application, the next step is to look at routing user requests to the correct tenant.
...
You need to configure your DNS and web application web server to send all requests ending in `.piedpipervideochat.com` to your web application. How to do so varies depending on your DNS provider and web application server. 
...
Make sure you are passing the `Host` header through to the application code, since that is what the applicationwill use to look up the tenant object.

==== Look Up the OAuth Configuration Based On the Hostname

Once the request is received by your application, look up the Client Id and Secret in the `tenant` table in your database based on the incoming request's hostname. 
...
The code checks to see if the host matches the control plane application. Remember, in this particular multi-tenant application, the web application responds to all requests for any users, both those creating video chat tenants and the users of the video chat application. 
...
[NOTE]
====
The sample application leverages an open source library for managing and creating OAuth URLs. This library handles setting the `state` parameter and other niceties. If your web application framework or language has such a library, using it is highly recommended.
====

The simplifed login link generation, without using a library, uses FusionAuth's well documented OAuth endpoints as well as the retrieved `client_id`:
...
Next up is logging out. In this case, you need to log the user out of FusionAuth and your application. Because the `logoutURL` setting was configured in the FusionAuth application configuration, when the logout URL is visited, FusionAuth will first log the user out of FusionAuth and then redirect the user to that location. The code at that location should ensure it logs the user out of the application (killing the session or otherwise doing so).
...
You must create a part of your web application to respond to OAuth redirection requests. The code must do the following:
...
At this point you have an access token and a user who has successfully authenticated. You can examine the token to determine if the user is authorized for this FusionAuth application. You can also examine the roles assigned to this user and expose or prohibit functionality based on them.
...
You may also create or update a local user record in your database. If there are other parts of your application which will be associated with a user, this is a common task, and many frameworks expect a local user record.

After you have set up a session in your application, redirect to the chat page for the data plane application. The control plane application should send the user to a profile page. Distinguish between these by checking the hostname.

== API Calls

If you are not using a tenant scoped API key, provide a tenant Id when you call the FusionAuth API. There are cases where the tenant can be inferred from another identifier such as when you provide an application Id. But there are other times where it cannot such as when you are creating a user. It's better to be consistent and present the tenant Id every time.
...
Doing so makes sense initially. There will be database overlap between them and it may be easier to operate a single application than to create two distinct web or mobile applications. Both of these applications will be using `tenant` data. The control plane appliation will to write and update it and the data plane application will read it. You'll need to either share a database or have some other way of syncing up that data if you create separate applications. 

If you use the same web application, limit access to functionality based on whether the host indicates the control plane or a data plane application is executing. 
...
An "attached" object, on the other hand, is linked to a different object, but if the latter is deleted, the former still exists. For instance, a signing key, created by using [breadcrumb]#Key Master#, can be associated with a tenant. When that tenant is deleted, the association is removed, but the signing key remains. 
...
You can download, review and run a https://github.com/FusionAuth/fusionauth-example-symfony-multitenant[fully functional multi-tenant application] modeled on the PPVC application. 

It is written in PHP and the Symfony framework; video chat functionality not included.
