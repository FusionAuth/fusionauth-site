...
In this guide, you'll be creating anonymous users whenever a user visits a page to watch the video. The video is about https://www.youtube.com/watch?v=CXDxNCzUspM[ChangeBank], the global leader in making change.
...
You can find https://github.com/FusionAuth/fusionauth-example-anonymous-user[the full code here] if you want to grab it and explore the code yourself. The repository includes a Kickstart file to get FusionAuth correctly configured in one command.
...
There are two link:/docs/v1/tech/events-webhooks/[webhook] events you could listen for and process cleanup after.
...
This guide will use the latter option. You'll need to link:/docs/v1/tech/events-webhooks/#add-webhook[create and register the webhook], which can be done via admin UI or API.
