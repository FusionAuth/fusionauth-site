...
If you are using FusionAuth's OAuth interface to present WebAuthn pages to users, you can customize those pages using a theme. There are several themed pages related to WebAuthn:
...
image::guides/webauthn/challenge-settings.png[The tenant settings to customize challenge lifetime.,width=1200,role=top-cropped,bottom-cropped]
...
Due to the server- and client-side components of each of the WebAuthn ceremonies, troubleshooting issues can be difficult. These tips may help you when issues come up.
