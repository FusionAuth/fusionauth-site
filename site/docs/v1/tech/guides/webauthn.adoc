...
WebAuthn passkeys allow for passwordless authentication on the sites they were registered. You can use WebAuthn to provide a streamlined authentication experience for users and improve security at the same time.

In order to use a WebAuthn passkey for authentication, you'll first need to register a passkey for a user with the link:#registration-ceremony[Registration Ceremony]. After a passkey is registered, use the link:#authentication-ceremony[Authentication Ceremony] to authenticate a user via WebAuthn. Both ceremonies involve the same basic flow:

1. Start the ceremony by sending an API request to FusionAuth to generate options for the WebAuthn JavaScript API
2. Use the options to call the WebAuthn JavaScript API on the client
3. Send an API request to FusionAuth to complete the ceremony. This request includes the response from the WebAuthn JavaScript API



== Registration Ceremony

The registration ceremony is used to register a new WebAuthn passkey on a user's device. Registration is a 3-step process:

1. Call the FusionAuth API to link:/docs/v1/tech/apis/webauthn#start-a-webauthn-passkey-registration[Start a WebAuthn Passkey Registration]
2. Call the https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API[WebAuthn JavaScript API] in the browser
3. Call the FusionAuth API to link:/docs/v1/tech/apis/webauthn#complete-a-webauthn-passkey-registration[Complete a WebAuthn Passkey Registration]

=== Start Passkey Registration

You can start the passkey registration ceremony by providing some basic information about the user and the new passkey.
