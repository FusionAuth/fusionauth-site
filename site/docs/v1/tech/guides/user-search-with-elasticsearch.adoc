...
To do so, in the administrative user interface, navigate to [breadcrumb]#System -> About# and scroll to the [breadcrumb]#System# section and you will see the type of search.
...
If the `queryString` parameter has a `:` it will be passed as-is. Otherwise, if it has no spaces and contains a `@` it is to be an email address and will be passed with a prefix: `email:`. Otherwise, if it has no spaces it will have the wildcard `*` prepended and appended.
...
The exact list of indexed fields are not documented, but if you are running FusionAuth self-hosted, you can the list by https://www.elastic.co/guide/en/elasticsearch/reference/6.3/indices-get-mapping.html[retrieving the Elasticsearch mapping].
...
If you change the value from `email:dinesh*` to `email:dinesh` this query will return 0 results, because there is user with that email and no wildcarding is done. If, instead, you change the value to `email:dinesh@fusionauth.io` this query will return the user.
...
Learn more about making https://www.elastic.co/guide/en/elasticsearch/reference/7.12/query-dsl-query-string-query.html[queryString queries in the Elasticsearch documentation].

== Searching With `query`

The `query` parameter is the most powerful way to search. The `query` parameter is a complete escaped Elasticsearch query.

Let's walk through some examples.

=== Searching On One Field

First, build the Elasticsearch query. Below is a usery to match all users with a `user.data.Company` attribute of `PiedPiper`. 
...
You can also perform advanced searches, including with multiple clauses and numeric ranges. Let's take a closer look at a more complex query.
...
* Registered for a particular application in your system.
...
TODO break out and include on the troubleshooting page

When you are troubleshooting Elasticsearch queries, running them against the Elasticsearch server is helpful. If you are running in FusionAuth Cloud, however, you cannot access Elasticsearch.
...
=== Maxium Users Returned Workarounds
...
Currently, only Elasticsearch is officially supported. Users and clients have reported success with https://opensearch.org/[OpenSearch], the fork of Elasticsearch, but it is not currently supported.
