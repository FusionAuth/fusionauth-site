...
Two factor authentication also works when <<Using the API directly>>. In that case, when you complete the passwordless authentication, instead of getting the user data, you'll get a `twoFactorId`:
...
When FusionAuth is your user datastore, adding a user means you must either provide a password or send them a link to reset their password. 

If you are only allowing passwordless authentication for your application, rather than offering it as an option, set the user's initial password to a long random string. Since they won't be using it, it doesn't matter how long it is.
