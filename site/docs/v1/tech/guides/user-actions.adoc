...
+
Unlike an instantaneous Action, a temporal Action may be cancelled or modified. An example of an instantaneous Action would be a reward, such as sending a user a discount coupon.
...
Once the Action instance expires (the `Ended` event), it can trigger a goodbye email to the user and any webhooks you configure. To prevent the user accessing your site after this date, you could either:
...
* [field]#startEmailTemplateId#, [field]#cancelEmailTemplateId#, [field]#modifyEmailTemplateId#, [field]#endEmailTemplateId# — The Id of the email templates to use when the Action starts, is cancelled, is modified, or expires. Temporal Actions have all four events, whereas instantaneous Actions have only the start event.
