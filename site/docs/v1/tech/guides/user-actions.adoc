...
* **Actioner** — The User that applies the Action. Every Action has an Actioner, even if the instance is automatically applied, in which case the Actioner is set to the administrator of the Application.
...
* Apply the Action to a User using the User Actions API, with an expiry date if appropriate. You can do this many times, to many users, if needed.
...
Actions and Action Reasons can be managed on the FusionAuth admin UI. You can apply an Action to a User using the [uielement]#Action User# option directly on the User in the FusionAuth admin UI.
...
In the `fusionauth-example-5-minute-guide` Node.js app, open `app.js`. Replace the code in `app.js` with the code below.
...
The code adds a new route that pretends to be Intercom and listens for new subscribers to start the onboarding process. In this tutorial, the API just prints the webhook to the console so that you can see what it looks like.
