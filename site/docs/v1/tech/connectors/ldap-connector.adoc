...
You can use the LDAP Connector as either a source of authentication or to migrate your users. In the former case, the backing LDAP directory remains the system of record (where the canonical user data exists). In the latter, the user is moved over once they have authenticated succesfully one time, and FusionAuth becomes the system of record.

As link:/docs/v1/tech/connectors/[documented], there are differences between these two choices. However, one difference is worth emphasizing. 
