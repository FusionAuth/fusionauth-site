...
[WARNING]
====
A valid user object must be returned with a FusionAuth compatible UUID (`user.id`) when both link:/docs/v1/tech/connectors/#required-fields[not migrating a user] and when link:/docs/v1/tech/connectors/#required-fields-2[migrating a user].
====
