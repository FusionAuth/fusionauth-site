...
* `user.registrations`: providing at least one entry in this array will associate a user with an application within FusionAuth, authorizing them to access the application.
* `user.tenantId`: the tenant that this user should be associated with. Most useful when you have multiple tenants.
...
* `user.registrations`: providing at least one entry in this array will associate a user with an application within FusionAuth, authorizing them to access the application.
* `user.data`: allows you to store arbitrary data associated with the user. This can be application or migration specific. For example, you could indicate the migration date of a user for subsequent searches.
