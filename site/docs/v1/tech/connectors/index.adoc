...
description: FusionAuth Connectors allow you to authenticate users against arbitrary data sources and optionally migrate them.
---
:page-liquid:
...
Connectors share many features with link:/docs/v1/tech/identity-providers[Identity Providers]. Both can connect FusionAuth to an external system of record for user identity.
...
* Does your external system support a standard such as OIDC or SAML? An Identity Provider is a better fit.
...
* Does your existing identity store expose no standard way to share identity data? A Connector is the better choise.
