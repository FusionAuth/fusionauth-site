...
Note that your configuration file will look slightly different from the one from this link. You only need to import the lines necessary for proper JWT authentication. For example, you do not need to bind to port 443 or set up an SSL certificate, although doing so is good practice in a production environment. More information on doing so can be found link:https://www.haproxy.com/blog/haproxy-ssl-termination/[here].
