...
First, you need to make sure the JWT issuer setting is correct. Navigate to [breadcrumb]#Tenants -> Your Tenant# and change the [field]#Issuer# field to the URL of your FusionAuth instance, for example, `\https://local.fusionauth.io`. Record this value, because we will use it later when generating the JWT.
...
You will use FusionAuth's API to generate a test JWT for your application. To do this, you need an API key with POST access to the `/api/login` endpoint. Go to [breadcrumb]#Settings -> API Keys# to add the key. Make sure the appropriate tenant is selected in the [field]#Tenant# field.
...
Here, `YOUR_FUSIONAUTH_URL` is the [field]#Issuer# name, and `API_KEY` is the key you set up on the [breadcrumb]#Settings -> API Keys# page.
...
We've shown you how to enable JWT authorization on a Kong service. In a production application, using Kong and the JWT plugin is useful to secure backend services of the application. This way, each service does not need to handle authorization or authentication. Instead, authentication and authorization are handled by FusionAuth and Kong.

We've used the FusionAuth API to create a test token on behalf of a user. For a production system, the token will generally be generated after a user signs in to your application through a frontend. Check out some of our framework integration tutorials to implement that:
