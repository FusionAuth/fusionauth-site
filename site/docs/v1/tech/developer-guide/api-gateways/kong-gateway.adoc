...
You can configure Kong Gateway to handle authorization and authentication to the resource through JSON Web Tokens (JWTs) issued on behalf of a user by an identity provider.
...
Navigate to [breadcrumb]#Applications# and then create a new Application. Fill out the [field]#Name# field, then click the [breadcrumb]#JWT# tab and toggle the [field]#Enable# switch. You can select `+Auto generate a new key on save...+` for the [field]#Access Token signing key# field. This will generate an RS256 asymmetric key pair specifically for this application.
...
image::api-gateways/kong-gateway/kong-gateway-api-key.png[FusionAuth API Key setup, width=1200]
