...
In this document, you'll learn how to set up Kong Gateway and FusionAuth as the identity provider to protect a simple HTTP service.
...
At this point, the service is inaccessible without a token. You can execute:
...
Again to confirm, this time, the command will return `HTTP/1.1 401 Unauthorized`.

== Set Up FusionAuth

Navigate to your FusionAuth instance.

First, you need to make sure the JWT issuer setting is correct. Navigate to [breadcrumb]#Tenants -> Your Tenant# and change the [field]#Issuer# to the URL of your FusionAuth instance. For example, `\https://local.fusionauth.io`. Record this value, because we will use it later when generating the JWT.

image::api-gateways/kong-gateway/kong-gateway-tenant-issuer.png[Tenant issuer.,width=1200px]

Next, you need to configure an application which will issue tokens to access the Kong Gateway service.

Navigate to [breadcrumb]#Applications# and then create a new application. Fill out the [field]#Name# field, then click the [breadcrumb]#JWT# tab and toggle the [field]#Enabled# switch. You can select `Auto generate a new key on save...` for both the [field]#Access Token signing key# and [field]#Id Token signing key# fields. This will generate an RS256 asymmetric key pair specifically for this application.
...
This creates a Kong Consumer with the username `fusionauth`. This Consumer will be linked to the authentication information that you supply to it, namely the the [field]#Public key# and [field]#Issuer# name from FusionAuth. This Consumer uses the information provided to verify any JWT used to access the service. You can enable this with the following command.
...
Here, `YOUR_FUSIONAUTH_URL` is the [field]#Issuer# name. `API_KEY` is the key you set up on the [breadcrumb]#Settings -> API Keys# page.
...
`USER_EMAIL` and `USER_PASSWORD` are the username and password of the test user that you registered to that application.
...
We've shown how to enable JWT authorization on a Kong service.  In a production application, using Kong and the JWT plugin is useful to secure backend services of the application. This way, each service does not need to handle authorization or authentication. Instead, authentication and authorization is handled by FusionAuth and Kong.

We've used the FusionAuth API to create a test token on behalf of a user. For a production system, the token will generally be generated after a user signs into you application through a front end. Check out some of our framework integration tutorials to implement that:
