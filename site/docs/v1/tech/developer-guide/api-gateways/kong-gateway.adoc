...
Navigate to [breadcrumb]#Applications# and then create a new application. Fill out the [field]#Name# field, then click the [breadcrumb]#JWT# tab and toggle the [field]#Enabled# switch. Select `Auto generate a new key on save...` for the [field]#Access Token signing key# field. This will generate an RS256 asymmetric key pair specifically for this application.
...
You will use FusionAuth's API to generate a test JWT for your application. To do this, you need an API Key with POST access to the `/api/login` endpoint. Go to [breadcrumb]#Settings -> API Keys# to add such a key. Make sure the appropriate tenant is selected in the [field]#Tenant# field.
