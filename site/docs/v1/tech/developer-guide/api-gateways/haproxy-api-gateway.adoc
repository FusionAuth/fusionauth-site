...
Finally, make sure there is at least one user registered to your application so that you can test with a JWT issued for that user. You can create a new user by navigating to [breadcrumb]#Users -> Add user#. Toggle the [field]#Send email to set up password# switch to `disabled` and manually enter a password in the [field]#Password# field.
...
* The issuer URL set in the [field]#Issuer# field from [breadcrumb]#Tenants -> Your Tenant# must exactly match the `<YOUR_FUSIONAUTH_URL>` value in the `haproxy.cfg` file. If the values do not match exactly, including any white space and slashes, the JWT token will not be accepted by HAProxy.
