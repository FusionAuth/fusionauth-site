...
* An HAProxy instance routed to an HTTP service. You can follow https://www.haproxy.com/blog/how-to-run-haproxy-with-docker/[this guide] to get a simple example running on Docker.
...
If you have trouble getting the `haproxy` container from the guide to start, make sure your `haproxy.cfg` file ends with a newline. Even a whitespace on the last line will prevent the container from running.
====
...
First, you need to make sure that the JWT issuer setting is correct. Navigate to [breadcrumb]#Tenants -> Your Tenant# and change the [field]#Issuer# field to the URL of your FusionAuth instance, for example, `\https://local.fusionauth.io`. Record this value, because you will use it later when generating the JWT. It will be referred to as `<YOUR_FUSIONAUTH_URL>`.
...
After saving the new application, find it in the list of applications and click the [uielement]#View# button next to it (look for the green magnifying glass). Here you will find the application [field]#Id#. Record this value, as you will need it further on. It will be referred to later as `<YOUR_APPLICATION_ID>`.

image::api-gateways/HAProxy-Api-Gateway/haproxy-application-id.png[Application id,width=1200px,role=bottom-cropped]

Now, navigate to [breadcrumb]#Settings -> Key Master#. You will see the access key that you've just created in this list. By default, it has the name `Access token signing key generated for application <NAME_OF_APPLICATION>`. View the key and copy the entire contents of the [field]#PEM encoded# field under the [breadcrumb]#Public Key# section into a file in the same directory as you placed the `haproxy.cfg` file whilst setting up HAProxy in the quickstart guide. Name the file `pubkey.pem`. Remember this filename, as you will supply it to the HAProxy configuration file later.
