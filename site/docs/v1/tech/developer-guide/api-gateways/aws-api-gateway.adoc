...
* A FusionAuth instance running on a publicly accessible URL. You can spin up a link:/pricing[basic FusionAuth Cloud instance] or link:/docs/v1/tech/installation-guide/[install it on any server]. If you are not able to host FusionAuth on a public server/URL, you can tunnel your local instance of FusionAuth through a tool like link:https://ngrok.com[ngrok].
...
To test, you'll need to link a FusionAuth user to your application. You can either use the existing admin user you use to log in to FusionAuth, or create a new isolated user. 
