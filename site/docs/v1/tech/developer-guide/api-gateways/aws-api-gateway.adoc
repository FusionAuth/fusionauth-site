...
* A FusionAuth instance running on a publicly accessible URL. You can spin up a link:/pricing[basic FusionAuth Cloud instance] or link:/docs/v1/tech/installation-guide/[install it on any server]. If you are not able to host FusionAuth on a public server / URL, you can proxy your local instance through a tool like link:https://ngrok.com[ngrok].
