...
To use the Login API, there are a few settings that need to be changed on FusionAuth. In the side panel in FusionAuth, select [breadcrumb]#Settings > API Keys#. Click the green `+` button at the top left to create a new key.
...
Then, under [breadcrumb]#Endpoints#, allow `POST` on the `/api/login` route. This key will only be allowed to all this endpoint with the `POST` method.
