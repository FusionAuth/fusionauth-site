...
On the [breadcrumb]#Routes# page, click `Next` without setting anything. We'll add the route via the Lambda function we'll create in a bit.

On the [breadcrumb]#Stages# page, click `Next`, leaving the defaults.

Finally, click `Create` on the [breadcrumb]#Review# page. 
...
In the main top AWS search bar, search for and select `Lambda`. 
...
Under [field]#Trigger Configuration#, choose `API Gateway` as the source. Select `Use an existing API`, and then select the API Gateway, set up earlier, in the search box, and select the `$default` stage. 

Under [breadcrumb]#Security#, choose `Create a JWT authorizor`. 

Set [field]#Identity source" to `$request.header.Authorization`.

For [field]#Issuer#, set the URL to be the same as you set in FusionAuth for the JWT issuer in the Tenant configuration. Make sure that it is exactly the same, including trailing slashes, without any whitespace etc.
...
image::api-gateways/aws-api-gateway/login-api.png[Enabling the login API,width=1200,role=bottom-cropped]
...
Now create a test user to login with on FusionAuth. Navigate to [breadcrumb]#"Users"# in the side bar, and click the green `+` button at the top right of the screen to create a new user. Choose the appropriate tenant (usually `default`), and fill in the information. You can choose to not send an email to set up the password, but rather set it directly when creating the user, by toggling the [field]#Send email to set up password# switch. Record this user email and password, as we'll use it to obtain a token. 

After saving the user, click to the [breadcrumb]#Registrations# tab. Click `Add registrations`, and select the FusionAuth application you set up earlier. 
...
Copy the `token` value out. This the the JWT, which we can use to access the API Gateway function. 
...
[source,json,title="Return token from the API call"]
----
    {"applicationId":"63b73f76-7400-487d-b122-5705b848da80","aud":"63b73f76-7400-487d-b122-5705b848da80","auth_time":"1672137288","authenticationType":"PASSWORD","email":"ehrlich@piedpiper.com","email_verified":"true","exp":"1672140888","iat":"1672137288","iss":"https://fusionauth.ritza.co","jti":"2f80b975-1870-4970-b876-90c2e7d9444b","preferred_username":"ehrlich","roles":"[]","sub":"3963000f-686d-4169-b628-9c9c4534b780","tid":"f004c1fe-484b-902c-d7ca-bdbc464da0b7"}
----
...
You can also combine the OIDC module with other security limitations, such as IP restrictions.
