...
Under [field]#Trigger Configuration#, choose `API Gateway` as the source. Select `Use an existing API`, and then select the API Gateway, set up earlier, in the search box, and select the `$default` deployment stage.
...
If you call this route, through a browser or Postman or cURL, you should receive a 401, and an `Unauthorized` message. We'll create a token next with FusionAuth to show how the route can be successfully called.
