...
* A FusionAuth instance running on a publicly accessible URL. You can spin up a link:/pricing[basic FusionAuth Cloud instance] or link:/docs/v1/tech/installation-guide/[install it on any server]. If you are not able to host FusionAuth on a public server / URL, you can proxy your local instance  of FusionAuth through a tool like link:https://ngrok.com[ngrok].
...
* If you are running a local instance of FusionAuth, Amazon Gateway will not be able to reach the keysets needed to validate the JWT. You will either need to host FusionAuth on a publicly accessible URL, or proxy your local instance through a tool like link:https://ngrok.com[ngrok].
...
* The JWT issued by the link:/docs/v1/tech/apis/login[FusionAuth Login API] has an expiry. If you wait too long before using it to call the Amazon Gateway API, the token will expire and the call will fail. 
