...
link:/docs/v1/tech/tutorials/switch-search-engines[Switch to the database search engine]. If you do so, FusionAuth won't sync user data to Elasticsearch during the import. If you require Elasticsearch for advanced searching capabilities, you can switch back to Elasticsearch after the migration is complete. Doing so is not required unless you are importing millions of users. This option is not available when using link:/docs/v1/tech/installation-guide/cloud[FusionAuth Cloud].
...
Finally, import the JSON using the `importUsers` method of a FusionAuth client library or by calling the REST API directly. This is link:/docs/v1/tech/apis/users#import-users[fully documented].
...
image::migration-guide/general/reset-user-password.png[Resetting the password of a user.,width=1200,role=bottom-cropped]
...
The message displayed in the user interface when someone enters the "forgot password" flow can be modified by changing the `forgot-password-email-sent` property in the `messages.properties` file in link:/docs/v1/tech/themes/localization#messages[your theme].
...
A more straightforward approach would be to fail the registration. You can do this with webhooks. In this case you'd use the link:/docs/v1/tech/events-webhooks/events/user-create[`user.create`] webhook, which fires every time a user is created.
...
In this scenario, you also pre-load the required certificates and notify upstream IdP administrators they'll need to update their ACS URLs eventually. You'll also configure a link:/docs/v1/tech/identity-providers/samlv2/[FusionAuth SAMLv2 Identity Provider] with the same values as your existing SAML SP.
