...
Next, consider a user's password, and related fields such as a salt or hashing scheme. Dealing with this data depends on your migration approach. For a big bang or segment by segment migration, ensure FusionAuth userstands the original system's hashing algorithm link:/docs/v1/tech/plugins/custom-password-hashing[by writing a plugin] if the password was not hashed in one of link:/docs/v1/tech/reference/password-hashes[FusionAuth's supported algorithms]. For a slow migration, the password will be available for FusionAuth to hash or re-hash. If you have the user's passwords in plain text, FusionAuth can also hash them on import.
...
In some cases, you may be able to implement a slow migration without using a Conector. If profile data in your original system of record doesn't change, or you can safely lose any changes, you can do a forked slow migration.

In this scenario, you place a custom proxy in front of both systems. This proxy is what handles the login. The proxy checks FusionAuth first using the link:/docs/v1/tech/apis/login/[Login API]. If the user does not exist, the legacy system is called, using a similar API. New users are added to FusionAuth.

All communication should be over TLS to ensure password safety

Once this proxy is up and running, you can choose when to do a big bang migration. Perform the migration, then modify the proxy to only consult FusionAuth, or remove it.

This approach has some downsides. It introduces additional complexity and also means you are tied to the FusionAuth Login API and can't take advantage of the hosted login pages. The legacy system must accept username and password. When compared to the other slow migration potion, profile data may become more fragmented. But for some circumstances, such as when you are trying to migrate an expected mass of new users but are comfortable running two systems for a while, this may be a viable alternative.
