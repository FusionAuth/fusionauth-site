...
* You can upgrade your password hash algorithms transparently without requiring anyone to reset their password. FusionAuth supports a link:/docs/v1/tech/reference/password-hashes[number of different algorithms] and you can also link:/docs/v1/tech/plugins/writing-a-plugin[bring your own] as well.
...
All entities have a link:/docs/v1/tech/reference/data-types#uuids[UUID] identifier. This Id can be specified on creation, but must be a valid UUID. If you have an identifier that is not a valid UUID, one option is to store the old Id in the `data` field of the FusionAuth configuration.

==== Evaluating FusionAuth 

If you haven't already done so, ensure FusionAuth will work with your application or applications. You can link:/docs/v1/tech/5-minute-setup-guide[install it in about five minutes] and build out a proof of concept (POC).
...
Next, consider a user's password, and related fields such as a salt or hashing scheme. Dealing with this data depends on your migration approach. For a big bang or segment by segment migration, ensure FusionAuth understands the original system's hashing algorithm link:/docs/v1/tech/plugins/custom-password-hashing[by writing a plugin] if the password was not hashed in one of link:/docs/v1/tech/reference/password-hashes[FusionAuth's supported algorithms]. For a slow migration, the password will be available for FusionAuth to hash or re-hash. If you have the user's passwords in plain text, FusionAuth can also hash them on import.
...
Please consult the link:/docs/v1/tech/apis/users[User API] for detailed field descriptions.
...
Similarly, consult the link:/docs/v1/tech/apis/users[Registration API] for full documentation of this object.
...
Create one or more tenants. Multiple tenants are useful for allowing someone to have the same email but different passwords, or allowing different settings such as the theme or password rules. Add these via the API or navigating to [breadcrumb]#Tenants# and clicking the green plus sign.

image::migration-guide/general/add-tenant.png[Adding a tenant.,width=1200,role=bottom-cropped]

Create one or many applications and add any roles needed for them. Create a FusionAuth application entity for each application whose users you are migrating. An application is anything a user can log in to, whether an API, a commercial product, or a custom web application. Add each of these via the API or navigating to [breadcrumb]#Applications# in the administrative user interface, then clicking the green plus sign to add the application. 

image::migration-guide/general/add-application.png[Adding an application.,width=1200,role=bottom-cropped]
...
All of this configuration can be done via the API or the administrative user interface. If you want to use the former, you'll have to link:/docs/v1/tech/apis/authentication#managing-api-keys[create an API key] with appropriate permissions.
...
You'll also want to link:/docs/v1/tech/apis/authentication#managing-api-keys[create an API key]. Make sure you give the key appropriate permissions. The minimum required are the `POST` method on the `/api/user/import` endpoint.

image::migration-guide/general/api-key-creation.png[Adding an application.,width=1200]

You can write the migration scripts in shell, any of the supported link:/docs/v1/tech/client-libraries/[client library languages], or against the link:/docs/v1/tech/apis/users[REST API] in any language supporting HTTP requests. Iterate over all the users in the old system or systems. Build the JSON files. Add a registration for each application to which a user should have access. 
...
If the original system hashes passwords using an algorithm other than those link:/docs/v1/tech/reference/password-hashes[schemes FusionAuth supports], write and install a link:/docs/v1/tech/plugins/custom-password-hashing[custom password hashing plugin]. In either case, specify the scheme in the user import JSON file. (It is called an `encryptionScheme` for backwards compatibility, but is actually a hashing scheme.)
...
You can assign them a high entropy password such as a UUID for the import and then use the link:/docs/v1/tech/apis/users#start-forgot-password-workflow[Forgot Password API]. This will send them an email to reset their password.

Another option is to not use the Import API. Instead, import these users one by one using the link:/docs/v1/tech/apis/users[User API], which can optionally send a setup password email. 
...
[source,json]
.Example of a Successful Login Response JSON
----
...
The generated JWT will be delivered to the client to present to any resource servers (other API servers, etc). For maximum compatibility, it should have link:/docs/v1/tech/apis/jwt[FusionAuth claim values]. 
...
image::migration-guide/general/add-connector.png[Adding a Connector.,width=1200]
...
image::migration-guide/general/add-connector-policy.png[Adding a Connector policy.,width=1200]
...
image::migration-guide/general/delete-connector-policy.png[Removing a Connector policy.,width=1200]
...
==== What About Forgot Password Functionality

What happens when a user forgets their password during a slow migration. There are two scenarios:

* The user has been migrated
* The user has not been migrated

In either case, FusionAuth will display a message saying something like "We've sent you an email containing a link that will allow you to reset your password. Once you receive the email follow the instructions to change your password." But an email will only be sent in the first case, where the user exists in FusionAuth. In the second case, the user will never receive an email.

Why, then, is this message displayed for both migrated and unmigrated users? 

The answer is that FusionAuth can't distinguish between unmigrated users and users that don't exist in your system. In order to secure your user data and prevent enumeration attacks, FusionAuth doesn't reveal whether there is a valid account for a given username or email. If FusionAuth were to display a message such as "There is no account in the system" for an unmigrated user, then attackers would be able to determine whether an accont existed, by trying different values and noting the different error messages.

This unfortunately means that the "Forgot Password" experience for migrating users is suboptimal. The current recommendation is to update the error message to say something like:

"We've sent you an email containing a link that will allow you to reset your password. Once you receive the email follow the instructions to change your password. If you don't receive an email, please check your spam folder or contact customer service." You'd need to provide a link to a form, phone number or other means of contacting customer service. Customer service representivies can then check to see whether the user is migrated or not and trigger a password reset email from the appropriate system.

The forgot password email sent message can be modified by changing the `forgot-password-email-sent` property in the `messages.properties` file in your theme.

There's an https://github.com/FusionAuth/fusionauth-issues/issues/1786[open issue discussing how to improve this experience]. Please add any comments there.
...
The proxy checks FusionAuth first using the link:/docs/v1/tech/apis/login/[Login API].
...
If you have a single users table and want to walk through how a migration would work with that, review the link:/docs/v1/tech/migration-guide/[Migrate Users tutorial].

If you are having issues importing users, review the link:/docs/v1/tech/admin-guide/troubleshooting#troubleshooting-user-import[relevant troubleshooting section].
