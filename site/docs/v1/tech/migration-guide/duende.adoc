...
Duende IdentityServer does not prescribe where you store your user data. You can store it in any database of your choice, in a file, or in a custom data store. How you store your users will determine how you export them. This guide assumes your Duende IdentityServer installation is using link:https://learn.microsoft.com/en-us/aspnet/core/security/authentication/identity?view=aspnetcore-7.0&tabs=visual-studio[ASP.NET Identity] to manage users and passwords, and that you are using link:https://www.microsoft.com/en-us/sql-server/[MSSQL Server as your Duende IdentityServer database]. If you are using another custom setup, this guide should still be helpful as a starting point, but you will need to modify the export program to match your setup.
...
* FusionAuth Tenants are a high level construct which groups entities such as users and applications together. Duende IdentityServer does not have such a concept. Each Duende IdentityServer deployment is in effect a Tenant.
...
The program also assumes that you have used ASP.NET Identity to create the user's password hashes. ASP.NET Identity stores the salt, hash and iteration in a single base64 encoded string. There are 2 versions of hashing used, as can be seen in the link:https://github.com/dotnet/aspnetcore/blob/v6.0.14/src/Identity/Extensions.Core/src/PasswordHasher.cs[ASP.NET Identity source code].
...
The ASP.NET Identity V3 hashing algorithm is directly supported by FusionAuth, and is called  link:/docs/v1/tech/reference/password-hashes#salted-pbkdf2-hmac-sha-256[salted-pbkdf2-hmac-sha256] algorithm in FusionAuth.
...
If you have not used ASP.NET Identity, or if you have created your own password hashing algorithm, you will need to update the export program to use this hashing algorithm, as well as create a custom hashing plugin for FusionAuth if necessary. FusionAuth supports link:/docs/v1/tech/reference/password-hashes[these hashing algorithms] out of the box, so you can check if your algorithm is already supported.

After you have customized the export program to your Duende IdentityServer implementation, you can run the program. The program will output a `users.json` file in the same directory as the running program, usually the `bin/Debug/net6.0` folder.
...
FusionAuth has a repository for link:https://github.com/FusionAuth/fusionauth-contrib[community contributed examples and code]. This repo contains a password hash plugin called "example-asp-identity-v2". To extend FusionAuth to support ASP.NET Identity V2 hashes, follow the instructions in the article link:/docs/v1/tech/plugins/writing-a-plugin[writing a plugin], using the plugin in the contributed examples. A quick summary of the steps:
...
After installing the plugin and restarting FusionAuth, you can verify that the plugin is installed by navigating to [breadcrumb]#Tenants -> Your Tenant#. Navigate to the [breadcrumb]#Password# tab. Under the [breadcrumb]#Cryptographic hash settings# section, you should see the new hash scheme available in the [uielement]#Scheme# dropdown. Only check that the plugin is listed - don't select it, or any new user will be created with that hash scheme. Rather than selecting the plugin here, it is specified per user in the `users.json` file.
...
The migrated user will have the Duende IdentityServer original user Id. If you have additional user attributes
