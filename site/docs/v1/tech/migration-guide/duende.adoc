...
Duende IdentityServer does not prescribe where you store your user data. You can store it in any database of your choice, in a file, or in a custom data store. How you store your users will determine how you export them. This guide assumes your IdentityServer installation is using link:https://learn.microsoft.com/en-us/aspnet/core/security/authentication/identity?view=aspnetcore-7.0&tabs=visual-studio[ASP Identity] to manage users and passwords, and that you are using link:https://www.microsoft.com/en-us/sql-server/[MSSQL Server as your IdentityServer database]. If you are using another custom setup, this guide should still be helpful as a starting point, but you will need to modify the export program to match your setup.
...
If you have an IdentityServer implementation that does not use a UUID as the user Id, you can add a new attribute under [field]#user.data# (such as [field]#user.data.originalId#) with the value of the Duende IdentityServer Id. Because everything under [field]#user.data# is indexed and available via search, you'll be able to find your users using either a new FusionAuth UUID Id, or the original Duende IdentityServer one.
...
The ASP Identity V3 hashing algorithm is directly supported by FusionAuth, and is called  link:docs/v1/tech/reference/password-hashes#salted-pbkdf2-hmac-sha-256[salted-pbkdf2-hmac-sha256] algorithm in FusionAuth.
...
Also check if there are any [field]#encryptionScheme# values that are set to  `example-asp-identity-v2`. If there are, you will need to create a custom hashing plugin for FusionAuth. This is covered in detail in the <<Add the ASP Identity V2 Hashing Plugin>> section further on in this guide.
...
If you have `bundler` installed, run `bundle install` in the `duende` directory. Otherwise install the needed gems in some other way.
