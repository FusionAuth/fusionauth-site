...
Duende IdentityServer does not prescribe where you store your user data. You can store it in a database of your choice, in a file, or in a custom data store. How you store your user information will determine how you export it. This guide assumes your Duende IdentityServer installation uses link:https://learn.microsoft.com/en-us/aspnet/core/security/authentication/identity?view=aspnetcore-7.0&tabs=visual-studio[ASP.NET Identity] to manage users and passwords stored in a link:https://www.microsoft.com/en-us/sql-server/[Microsoft SQL Server] database. If you are using another custom setup, this guide should still be helpful as a starting point, but you will need to modify the export program to match your setup.
...
Duende IdentityServer provides a package to integrate with link:https://docs.duendesoftware.com/identityserver/v6/aspnet_identity/[ASP.NET Identity], which determines the format user information is stored in.
...
We've created a sample C# export program to export users from a Duende IdentityServer database to a JSON file. This program assumes you are using link:https://learn.microsoft.com/en-us/aspnet/core/security/authentication/identity?view=aspnetcore-7.0&tabs=visual-studio[ASP.NET Identity]. You can find the folder with the source code for the export program link:https://github.com/fusionauth/fusionauth-import-scripts/tree/identityserver/duende/identity-server-export[here].
...
The program also assumes that you have used ASP.NET Identity to create the users' password hashes. ASP.NET Identity stores the salt, hash, and iteration in a single Base64-encoded string. There are two versions of hashing used, as you can see in the link:https://github.com/dotnet/aspnetcore/blob/v6.0.14/src/Identity/Extensions.Core/src/PasswordHasher.cs[ASP.NET Identity source code].
...
* Install Java JDK 8 or higher, for example, from link:https://jdk.java.net/20/[https://jdk.java.net/20/].
...
----
$ ruby ./import.rb -f http://localhost:9011 -k '...' -t '...' -u user-data.json
