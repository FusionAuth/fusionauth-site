...
If there is an attribute in your Auth0 user which cannot be directly mapped to a FusionAuth attribute, you can place it in the `user.data` field. This field can store arbitrary JSON values and will be indexed and searchable.

=== Social Logins

Auth0 also provides integrations with other social login providers such as Apple, Google or Facebook. Review the supported FusionAuth link:/docs/v1/tech/identity-providers/[Identity Providers] to ensure your social providers are supported. If not listed explicitly, they may work with an OIDC or SAML connection. Otherwise, open a https://github.com/fusionauth/fusionauth-issues/[feature request].

When migrating social logins, you may need to modify the switches of the Auth0 import script. See <<Use the Script>> for more
...
image::migration-guide/auth0/list-of-tenants-add-highlighted.png[Adding a tenant.,width=1200,role=bottom-cropped]
...
image::migration-guide/auth0/add-tenant.png[The tenant creation screen.,width=1200,role=bottom-cropped]
...
image::migration-guide/auth0/list-of-tenants.png[The tenant list.,width=1200,role=bottom-cropped]
...
image::migration-guide/auth0/add-application.png[The application creation screen.,width=1200]
...
image::migration-guide/auth0/find-login-url.png[Finding the login URL.,width=1200,role=bottom-cropped]
...
image::migration-guide/auth0/add-api-key.png[Adding an API key,width=1200]

This key needs to have the permission to run a bulk import of users. In the spirit of the principle of least privilege, give it the permission to `POST` to the `/api/user/import` endpoint. (If you are also importing social logins, you'll need more privileges: `/api/identity-provider/link` and `/api/user/search`). Record the API key string, as you'll use it below.

image::migration-guide/auth0/set-api-key-permissions.png[Setting API key permissions,width=1200]
...
image::migration-guide/auth0/list-users.png[List imported users.,width=1200,role=bottom-cropped]
...
image::migration-guide/auth0/user-login.png[The user login screen.,width=1200,role=bottom-cropped]
...
image::migration-guide/auth0/list-of-tenants-delete-highlighted.png[Deleting a tenant.,width=1200,role=bottom-cropped]
