...
You should also modify the `processUserJSON` function. This converts the JSON returned from Cognito into JSON FusionAuth expects. You can see samples of both below.

The logic of this function will depend on your UserPool custom attributes and business logic. You could, for example, give users certain FusionAuth roles, register them for more than one application or add them to a previously created FusionAuth group.

The lambda will call the https://docs.aws.amazon.com/cognito-user-identity-pools/latest/APIReference/API_GetUser.html[GetUser] API. Here is some sample data.
...
The `processUserJSON` will transfrom the above into a FusionAuth compatible format, as displayed below:
...
Once you have the lambda code customized, it is time to deploy the AWS API.
...
image::migration-guide/{migration_source_dir}/add-generic-connector.png[Adding a Generic Connector.,width=1200]

Configure the Connectory:
...
image::migration-guide/{migration_source_dir}/configured-generic-connector.png[Configuring a Generic Connector.,width=1200]
...
image::migration-guide/{migration_source_dir}/connector-policies.png[Connector policies for this Tenant.,width=1200]
...
image::migration-guide/{migration_source_dir}/connector-policy-list-after-adding.png[Cognito Connector policy added and in list.,width=1200]
...
image::migration-guide/{migration_source_dir}/user-login.png[The login page.,width=1200]

Enter the credentials that you would use to login with Cognito (the email address or username and the password).
The user should be transparently migrated over to FusionAuth.
...
Let's verify the import succeeded by viewing the user in the administrative user interface.
...
[WARNING.warning]
====
This script will send an email for every imported user. Ensure your SMTP server, configured in [breadcrumb]#Tenants -> Your Tenant -> Email#, can handle the volume. 
