...
Visit the user pool configuration containing the users you want to migrate. Navigate to [breadcrumb]#General settings -> App clients# and create a new App client. Give it a name, such as "FusionAuth migration". Disable the [field]#Generate client secret# checkbox. Uncheck all the [breadcrumb]#Auth Flows Configuration# values, but check [field]#Enable username password based authentication (ALLOW_USER_PASSWORD_AUTH)#. This is the form of authentication FusionAuth will use to migrate over your users.

At the end, you'll end up with an App client configuration similar to this:

pic TODO

Save by clicking "Create app client".
...
When you are doing this, you'll need to modify the lambda code that runs. Here's the entire AWS lambda:
...
Make sure to modify the constants at the top:
...
const fusionAuthTenantId = // the Id of the FusionAuth tenant you are importing users into
const fusionAuthApplicationId = // the Id of the FusionAuth application to which you want to register the users automatically
const authorizationHeaderValue = // a random string to ensure that only the Connector can execute this lambda
----

You should also modify the `processUserJSON` function. This function converts the JSON of the Cognito `getUser` response, shown below, to the FusionAuth User object response. The logic of this function will depend on your UserPool custom attributes and business logic.
...
Once you have the lambda code customized, you need to deploy the API.
...
----
include::https://raw.githubusercontent.com/FusionAuth/fusionauth-example-cdk-cognito-migration/cloudformation/cloudformation.yaml[]
----

You'll have to modify the lambda code to reflect changes made above. Find a JavaScript minifier online and run the customized lambda code through it. Then replace the line after `ZipFile: |` with your code.
...
Now, login with a user whose username and password you know.
...
If the authorization or credentials are incorrect, an empty body and a HTTP status code will be returned. 
...
In order to test that a migration occured correctly, you'll need to log in as a test user via the FusionAuth interface. To do so, navigate to the list of applications. View the test application by clicking the magnifying glass and scroll to the [field]#OAuth IdP login URL#. 

TODO picture

Copy and paste this URL and open it in a new incognito browser window. You should see the login screen.
...
After you are done testing, you can choose to migrate users into the default tenant or a new tenant. Whichever you choose, configure the Connector policy correctly and update the configuration of the AWS lambda to ensure that the user JSON has the correct tenant Id.

If you aren't keeping users in the test tenant, delete it.

If you need to start over because you need to tweak a setting such as the default application registration, delete the tenant you created. This will remove all the users and other configuration for this tenant, giving you a fresh start. To do so, navigate to [breadcrumb]#Tenants# and choose the red trash can icon. 
...
:extra_permissions_text: Also, give the key permissions to `POST` to the API endpoint `/api/user/change-password`.
include::docs/v1/tech/migration-guide/_create-api-key.adoc[]
...
You also may want to change the password reset token lifetime. Find that in [breadcrumb]#Tenants -> Your Tenant -> Advanced -> External identifier durations#. Modify the [field]#Change password# field value.
====
...
=== Estimate How Long to Run a Slow Migration

You can estimate how many accounts will be migrated in a given period of time.
