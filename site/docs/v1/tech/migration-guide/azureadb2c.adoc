...
----
include::https://raw.githubusercontent.com/FusionAuth/fusionauth-example-ropc-azure-function/main/RopcProxyFunction/index.js[]
----

You will need to modify the `transformToFusionUserObject` function to match the properties of the users that you want to import. This converts the JSON returned from Azure AD B2C into the JSON format FusionAuth requires. You can see samples of both below. To modify this procedure, clone or fork the https://github.com/FusionAuth/fusionauth-example-ropc-azure-function[repo for the full Function project], then make the changes in your local copy of the project.
...
Azure Functions can be coded within the Azure portal directly, or coded on your local machine and deployed as a package using https://docs.microsoft.com/en-us/azure/azure-functions/functions-develop-vs-code?tabs=csharp[Visual Studio Code, with Azure Extensions]. As the function has dependent modules, you will need to follow these instructions. Clone the https://github.com/FusionAuth/fusionauth-example-ropc-azure-function[function GitHub repo], and follow the instructions in the link to deploy to your Azure Function.
...
After enabling enhanced debug messages, try to log in again with the test user. In the administrative user interface, navigate to [breadcrumb]#System -> Event Log# and look for useful messages.
...
We've created a node console tool to authenticate with the Graph API and export the users. You can clone or fork it from https://github.com/FusionAuth/fusionauth-example-azure-ad-bulk-export[the GitHub repository]. Then update the `.env` file with your Azure AD B2C tenant and app ID and secret.
