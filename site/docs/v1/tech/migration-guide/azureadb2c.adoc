...
description: How to migrate your users to FusionAuth from Microsoft Azure AD B2C.
---
:page-liquid:

:migration_source_name: AD B2C
:migration_source_dir: azureadb2c
...
There are a number of different ways applications can be integrated with Azure AD B2C, and it would be difficult to cover them all. This guide focuses on migrating user data, including profile data and passwords. However, https://docs.microsoft.com/en-us/answers/questions/450019/azure-ad-b2c-export-user-password-hashes.html[Azure AD B2C does not allow for password hash export]. Therefore, you must perform a slow migration if you don't want to force users to reset their passwords.
...
To preserve your user's passwords, you need to perform a slow migration. Users log in to FusionAuth with their Azure AD B2C credentials, and FusionAuth transparently migrates their data. Slow migrations in FusionAuth use Connectors, a paid feature.
...
In Azure AD B2C, users are placed in https://docs.microsoft.com/en-us/azure/active-directory-b2c/technical-overview[Tenants]. FusionAuth also has the concept of link:/docs/v1/tech/core-concepts/tenants[Tenants]. Both of these include data about users, applications and other configuration.
...
In Azure AD B2C, external identity providers are managed using https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-identity-provider[Identity Providers]. With FusionAuth, these are also called link:/docs/v1/tech/identity-providers/[Identity Providers].

Review the supported FusionAuth link:/docs/v1/tech/identity-providers/[Identity Providers] to ensure any you need are supported. At this time, while there is considerable overlap between the supported identity providers, there are a number of differences.
...
* https://docs.microsoft.com/en-us/azure/active-directory-b2c/user-flow-overview[User Flows and Custom Policies] are ways for you to customize authentication or authorization workflows with Azure AD B2C. FusionAuth has a similar concept called link:/docs/v1/tech/lambdas/[Lambdas]. FusionAuth also has link:/docs/v1/tech/events-webhooks/[webhooks] fired at certain points in the user lifecycle; in certain configurations, they can also stop a particular authentication flow.
* Azure AD B2C does not have a standardised roles and permissions concept. These are usually handled with custom attributes or secondary data stores. FusionAuth has link:/docs/v1/tech/core-concepts/roles[roles] that are configured on an application by application basis and made available in a token after a successful authentication.
* Azure AD B2C has the concept of https://docs.microsoft.com/en-us/azure/active-directory-b2c/customize-ui?pivots=b2c-user-flow[User Flow Page Layouts] where users can log in or register. FusionAuth has link:/docs/v1/tech/core-concepts/integration-points#hosted-login-pages[hosted login pages] which offer a superset of the functionality of Azure AD B2C User Flow. They are link:/docs/v1/tech/themes/[customized via themes].
* In Azure AD B2C, users log into https://docs.microsoft.com/en-us/azure/active-directory-b2c/app-registrations-training-guide[App Registrations]. In FusionAuth, link:/docs/v1/tech/core-concepts/applications[Applications] are a similar construct, but users are associated with them through link:/docs/v1/tech/core-concepts/registrations[Registrations]. You can migrate both your Client Id and Client Secret from Azure AD B2C to FusionAuth.
* Azure AD B2C sends emails on your behalf, such as forgot password notifications. FusionAuth can do so too; link:/docs/v1/tech/email-templates[the templates are customizable].
* Azure AD B2C offers the Client Credentials grant. FusionAuth offers a constrained version of this using link:/docs/v1/tech/core-concepts/entity-management[Entity Management]; this is available in all paid editions.
* Refresh tokens allow JWTs to be refreshed without a user logging in. These can be migrated from Azure AD B2C using the link:/docs/v1/tech/apis/users#import-refresh-tokens[Import Refresh Tokens API].
...
* Azure AD B2C supports MFA. FusionAuth also link:/docs/v1/tech/guides/multi-factor-authentication[supports MFA], which may be enabled for a tenant and configured for a user at any time.
...
* Azure AD B2C has https://azure.microsoft.com/en-us/pricing/details/active-directory/external-identities/#pricing[a pricing model] that applies to monthly active users (MAUs). FusionAuth has no user limit. Instead, you are limited by the resources provided to a FusionAuth instance, such as memory, CPU and database capacity.
...
There are two main components you need to set up to enable this migration. The first is an Application, with an https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-ropc-policy?tabs=app-reg-ga&pivots=b2c-user-flow[ROPC flow]. The second is an https://docs.microsoft.com/en-us/azure/azure-functions/functions-overview[Azure Function] to accept login requests from FusionAuth over TLS and perform an https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-ropc-policy?tabs=app-reg-ga&pivots=b2c-user-flow[ROPC login] request on Azure AD B2C.

[WARNING.warning]
====
FusionAuth Connectors will send user credentials to Azure AD B2C for authentication. Secure both the data in transit and the Azure Function endpoint.

In this guide, all communication is over TLS and the Azure Function requires a unique query string code. You should also review the documents on https://docs.microsoft.com/en-us/azure/azure-functions/security-concepts?tabs=v4[Securing Azure Functions] to implement any additional security features you may require.
...
FusionAuth will send login requests to Azure AD B2C but needs a client Id and secret to do so. It is best practice to create a new App Registration specifically for a ROPC flow to receive these requests. This guide will walk through configuration using the Azure Portal.

[NOTE]
====
These instructions use the newer https://docs.microsoft.com/en-us/azure/active-directory-b2c/app-registrations-training-guide[App Registrations] (as opposed to the legacy Applications) experience.
====

Visit the https://aka.ms/b2cappregistrations[Azure AD B2C App Registrations] tab. Then https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-ropc-policy?tabs=app-reg-ga&pivots=b2c-user-flow#register-an-application[follow these instructions] to set up a new ROPC App Registration.

At the end, you'll end up with an app client configuration looking similar to this:

image::migration-guide/{migration_source_dir}/ropc-app-registration-configuration.png[Configuring an ROPC App Registration for migration.,width=1200,role=bottom-cropped]

Save the new configuration. On the list of app clients, you'll see your new one.

Record the Application (client) ID. This will be a GUID like `022c5902-d8ee-43b0-ac1f-c2719b799657`.

image::migration-guide/{migration_source_dir}/ropc-app-client-id.png[Finding the app client id.,width=1200,role=bottom-cropped]

Next, you'll need to set up an Azure Function to receive the login requests from FusionAuth. The Function will pass the credentials on to Azure AD B2C and, when the authentication succeeds, return a FusionAuth friendly JSON object.

==== Configuring the Azure Function

The Azure Function receives the login request from FusionAuth, attempts to log the user in via ROPC, and then returns a FusionAuth user object on success. The Function will call the https://docs.microsoft.com/en-us/graph/api/user-get?view=graph-rest-1.0&tabs=http[Users] API after successful authentication to get additional user attributes, which will then be transformed into a FusionAuth compatible format.
...
----
include::https://raw.githubusercontent.com/TheMiniDriver/fusion-ropc-azure-function/main/RopcProxyFunction/index.js[]
----

You will need to modify the `transformToFusionUserObject` function to match the properties of the users that you want to import. This converts the JSON returned from Azure AD B2C into the JSON format FusionAuth requires. You can see samples of both below. To modify this procedure, clone or fork the repo for the full Function project https://github.com/TheMiniDriver/fusion-ropc-azure-function[here] Then make the changes in your local copy of the project.

The exact implementation depends on your Directories custom attributes and business logic. You could, for example, give users certain FusionAuth roles, register them for more than one application or add them to a previously created FusionAuth group.
...
Azure Functions can be coded within the Azure portal directly, or coded on your local machine and deployed as a package using https://docs.microsoft.com/en-us/azure/azure-functions/functions-develop-vs-code?tabs=csharp[Visual Studio Code, with Azure Extensions]. As the function has dependent modules, you will need to follow these instructions. Clone the https://github.com/TheMiniDriver/fusion-ropc-azure-function[function project], and follow the instructions in the link to deploy to your Azure Function.

After deploying the Function, set the following Function environment variables, also known as https://docs.microsoft.com/en-us/azure/azure-functions/functions-how-to-use-azure-function-app-settings?tabs=portal[Application Settings], to those that you've recorded when setting up the ROPC app:

[source,bash]
.Azure Connector Function environment variables to set
----
TENANT_NAME:    // The name of your AD B2C Tenaant"fusiontut",
TENANT_ID:      // The full AD B2C Tenant Id, will end with .onmicrosoft.com
APP_NAME:       // The name of the B2C App Registration setup for ROPC flow
CLIENT_ID:      // The App Registration clientId, a.k.a Application id
...
If the authorization header or account credentials are incorrect, a 404 HTTP status code is returned, along with a message in the body. You can view the status code by running `curl` with the `-vv` switch.
...
* Set the [field]#Authentication URL# to the value of the Function Url endpoint created above.
* You don't need to set any headers, as Azure Functions have a code in the querysting.
...
Navigate to your tenant settings: [breadcrumb]#Tenants -> Azure import tenant -> Connectors#.
...
image::migration-guide/{migration_source_dir}/connector-policy-list-after-adding.png[Azure AD B2C Connector policy added and in list.,width=1200,role=bottom-cropped]
...
include::docs/v1/tech/migration-guide/_what-next-azure-ad-b2c.adoc[]
...
:extra_permissions_text: pass:normal[Also, give the key permissions to `POST` to the API endpoint `/api/user/forgot-password`.]
...
[NOTE.note]
====
The Azure AD B2C UI provides for bulk exporting of users. However, at the time of this guide, link:https://docs.microsoft.com/en-us/answers/questions/373771/azure-ad-b2c-user-export-missing-mail.html[user email addresses are not available through the bulk export]. This guide uses the link:https://docs.microsoft.com/en-us/azure/active-directory-b2c/microsoft-graph-operations[Graph API] to export user data, which does include email addresses.
====

To export the data via the graph API, you need to authenticate with the API, and then call the link:https://docs.microsoft.com/en-us/graph/api/user-list?view=graph-rest-1.0&tabs=http[List method] to return the data.

To authenticate with Azure AD B2C, you first need to register an application with Azure AD B2C, which will give you an app ID and secret. Follow link:https://docs.microsoft.com/en-us/azure/active-directory-b2c/microsoft-graph-get-started?tabs=app-reg-ga[this guide] to create an application.

We've created a node console tool to authenticate with the Graph API and export the users. You can clone or fork it from git@github.com:TheMiniDriver/fusion-azure-ad-bulk-export.git[git@github.com:TheMiniDriver/fusion-azure-ad-bulk-export.git], and configure the `.env` file with your Azure AD B2C tenant and app ID and secret.
...
----
{
  ...
  [
      {
          "id": "642caa6a-86a6-4f3c-a0c0-8fc5d817fc4a",
          "deletedDateTime": null,
          "accountEnabled": true,
          "ageGroup": null,
          "businessPhones":
          ...
          "legalAgeGroupClassification": null,
          "mail": null,
          "mailNickname": "642caa6a-86a6-4f3c-a0c0-8fc5d817fc4a",
          "mobilePhone": null,
          "onPremisesDistinguishedName": null,
          "officeLocation": null,
          "onPremisesDomainName": null,
          ...
          "surname": "Bob",
          "usageLocation": null,
          "userPrincipalName": "642caa6a-86a6-4f3c-a0c0-8fc5d817fc4a@fusiontut.onmicrosoft.com",
          "externalUserConvertedOn": null,
          "externalUserState": null,
          "externalUserStateChangeDateTime": null,
          "userType": "Member",
          ...
          [
              {
                  "signInType": "emailAddress",
                  "issuer": "fusiontut.onmicrosoft.com",
                  "issuerAssignedId": "bradley+theminidriver@indri.co.za"
              },
              {
                  "signInType": "userPrincipalName",
                  "issuer": "fusiontut.onmicrosoft.com",
                  "issuerAssignedId": "642caa6a-86a6-4f3c-a0c0-8fc5d817fc4a@fusiontut.onmicrosoft.com"
              }
          ],
          "onPremisesExtensionAttributes":
          {
