...
In this guide, all communication is over TLS and the Azure Function requires a unique query string code. You should also review the documents on https://docs.microsoft.com/en-us/azure/azure-functions/security-concepts?tabs=v4[Securing Azure Functions] to implement any additional security features.


==== Configuring the Application in Azure AD B2C

FusionAuth will send login requests to Azure AD B2C but needs a client Id to do so. You can re-use an existing application, but it is better practice to create a new one to receive these requests. This guide will walk through configuration using the Azure Console, but you can also configure this using CloudFormation, Terraform or any other IAC tool.
...
----
include::https://raw.githubusercontent.com/FusionAuth/fusionauth-example-cdk-azure-b2c-migration/main/resources/index.js[]
----

You will probably  need to modify the `transformToFusionUserObject` function to match the properties of the users that you want to import. This converts the JSON returned from Azure AD B2C into the JSON format FusionAuth requires. You can see samples of both below. 
...
Azure Functions can be coded within the Azure portal directly, or coded on your local machine and deployed as a package using https://docs.microsoft.com/en-us/azure/azure-functions/functions-develop-vs-code?tabs=csharp[Visual Studio Code, with Azure Extensions]. As the function has dependent modules, you will need to follow these instructions. Clone the https://github.com[function project], and follow the instructions in the link to deploy to your Azure Function. 
...
We've created a .NET console tool to authenticate with the Graph API and export the  users. link:TODO. Download the tool, and configure the appsettings.json file with your Azure AD B2C tenant and app ID and secret.
