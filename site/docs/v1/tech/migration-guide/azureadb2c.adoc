...
The first is an App Registration, with an https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-ropc-policy?tabs=app-reg-ga&pivots=b2c-user-flow[ROPC flow]. 

Secondly, you will need to set up a https://docs.microsoft.com/en-us/azure/active-directory-b2c/microsoft-graph-get-started?tabs=app-reg-ga[Microsoft Graph application] to retrieve the user's details, in order to import them to FusionAuth.

The third is an https://docs.microsoft.com/en-us/azure/azure-functions/functions-overview[Azure Function] to accept login requests from FusionAuth over TLS and perform an https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-ropc-policy?tabs=app-reg-ga&pivots=b2c-user-flow[ROPC login] request on Azure AD B2C, and return the relevant user data via the Azure AD B2C Graph API app.
...
FusionAuth will send login requests to Azure AD B2C to validate the users credentials, via the Azure Function. It will also retrieve the users Azure AD B2C profile from the Microsoft Graph API. It is best practice to create a 2 new App Registrations for each of the ROPC flow and the Microsoft Graph connection. This guide will walk through the configurations using the Azure Portal.
...
Visit the https://aka.ms/b2cappregistrations[Azure AD B2C App Registrations] tab. Then https://docs.microsoft.com/en-us/azure/active-directory-b2c/add-ropc-policy?tabs=app-reg-ga&pivots=b2c-user-flow#register-an-application[follow these instructions] to set up a new ROPC App Registration and Flow.
...
Now you can set up the App Registration for the Microsoft Graph API access. Follow https://docs.microsoft.com/en-us/azure/active-directory-b2c/microsoft-graph-get-started?tabs=app-reg-ga[this guide], including the "Create Client Secret" section. Record both the Client Secret, and the Application (client) ID as above.
...
APP_NAME:       // The name of the ROPC flow setup above
CLIENT_ID:      // The Microsoft Graph API App Registration clientId, a.k.a Application id
CLIENT_SECRET:  // The Microsoft Graph API App Registration client secret
----
