...
you are looking to compare FusionAuth and Firebase Authentication, link:/blog/2018/10/02/firebase-and-fusionauth-ciam-comparison[this
document may help].
...
recommended. Firebase also has a https://firebase.google.com/docs/firestore/manage-data/export-import[high level export guide worth
reviewing].
...
* FusionAuth Tenants are a high level construct which groups other entities such as
users and applications together. Firebase only has a project construct, which are independent from each other. 
* For Firebase, Roles and Permissions are generally custom code and storage, with claims added to a user token.  FusionAuth
...
Firebase has pre-built UI's for web, iOS, android etc. You can also choose to build your own login page, and call the Firebase APIs. 
