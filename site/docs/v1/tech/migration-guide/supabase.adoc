...
This guide mentions the typical parts of a bulk migration and focuses on migrating user data from a Supabase managed user database into FusionAuth.

== Planning Considerations

=== Obtaining User Data

Supabase includes a Postgres database and authentication service, storage, and Edge functions (serverless functions for backend logic). You can write a custom psql script in the Supabase SQL Editor to export user data as JSON that you can import to FusionAuth. User data is stored in `auth.users` table in Supabase and you can join the query with data from other `auth` tables to pull specific attributes.
...
The script for exporting user data is found below, it is a user defined psql function which generates a JSON array object:
...
The function `ufn_get_user_migration_data_json()` returns a JSON array with formatted user data. Copy the generated JSON and save it to a file on your machine to upload later in FusionAuth.

To export users from Supabase, you’ll perform the following steps:

1. Access the Supabase SQL Editor and create a new query.
2. Paste the `ufn_get_user_migration_data_json()` function code above, into the editor.
...
The user data to be exported will be selected from the `auth` table unless there are other tables you have which may contain other user specific fields like usernames e.t.c, in which case you can edit the script above to cater for that. The JSON output produced by the script will look similar to the example below:
