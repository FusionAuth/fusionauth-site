...
Supabase includes a Postgres database and authentication service, storage, and Edge functions (serverless functions for backend logic). You can write a custom psql script in the Supabase SQL Editor to export users in JSON format that you can import to FusionAuth. User data is stored in `auth.users` table in Supabase and can you join with other `auth` tables to pull specific attributes.
