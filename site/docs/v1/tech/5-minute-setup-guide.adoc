...
You also need to have node installed. This has been tested with node 14, but should work with other versions.
...
These steps are the standard integration with an OAuth identity provider. These also work for OpenID Connect. Since FusionAuth supports both, we'll use OpenID Connect since it is the modern standard. Let's get into the details of each step. You can also watch a video of an install:
...
This will start both the `fusionauth-search` and `fusionauth-app` components. If you are on Windows, you'll need to use the `startup.bat` file instead.
...
Here's the full Express router. You want to create a file called `routes/index.js` and put this code in it:
...
We are beginning to work on a 10 minute guide and a 42 minute guide. What topics would you want to cover next? Please choose 2 at most.
