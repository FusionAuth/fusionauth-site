---
layout: doc
title: 5-minute guide to to setup and integrate with FusionAuth
...
. Return the access and refresh tokens to the browser (or app)

These six steps are the standard integration with an OAuth identity provider. These also work for OpenID Connect. Since FusionAuth supports both, we'll use OpenID Connect since it is the modern standard. Let's get into the details of each step
...
This will start both the `fusionauth-search` and `fusionauth-app` components, which are both required to use FusionAuth.
...
image::5-minute-setup-guide/maintenance-mode-database.png[Maintenance Mode Database,width=900,role=shadowed]
...
image::5-minute-setup-guide/maintenance-mode-search.png[Maintenance Mode Search,width=900,role=shadowed bottom-cropped]
...
image::5-minute-setup-guide/interstitial.png[Interstitial,width=900,role=shadowed bottom-cropped]
...
image::5-minute-setup-guide/setup-wizard.png[Setup Wizard,width=900,role=shadowed]
...
image::5-minute-setup-guide/dashboard-applications.png[Dashboard to Applications,width=900,role=shadowed]
...
image::5-minute-setup-guide/application-listing.png[Application Listing,width=900,role=shadowed bottom-cropped]
...
image::5-minute-setup-guide/application-form.png[Application Form,width=900,role=shadowed]
...
image::5-minute-setup-guide/application-view.png[Application View Dialog,width=900,role=shadowed]
...
image::5-minute-setup-guide/user-search.png[User Search,width=900,role=shadowed bottom-cropped]
...
image::5-minute-setup-guide/registration-add-form.png[Registration Add Form,width=900,role=shadowed bottom-cropped]
...
router.get('/oauth-redirect', function (req, res, next) {
  const accessToken = client.exchangeOAuthCodeForAccessToken(req.query.code,
...
The final step is to add some code to our route that will return the `access_token` and `refresh_token` from FusionAuth as cookies to the browser and then redirect the now logged in user back to the homepage. Here is the final route for our Node.js backend:
...
router.get('/oauth-redirect', function (req, res, next) {
  const accessToken = client.exchangeOAuthCodeForAccessToken(req.query.code,
                                                             'ae1eeb2a-c693-4a29-b45b-34c3a397f7da',
                                                             '035vpBDcy0-HyB5dpZkUPw49_xzH0AhCz5kOR9EAnf0',
                                                             'http://localhost:3000/oauth-redirect');
  res.cookie('access_token', accessToken.access_token, {httpOnly: true});
  res.cookie('refresh_token', accessToken.refresh_token, {httpOnly: true});
  res.redirect(302, '/');
});
----

This code assumes that the rest of our application will use the cookie values we sent back to the browser to determine if the user is logged in and manage their account. If we need additional information about the user, we can also use the `id_token` from FusionAuth. This token is a JWT (JSON Web Token) that contains additional user information. If we use this method, then our backend must validate the JWT. This process is out of the scope of this guide, but covered in our link:/docs/v1/tech/tutorials/json-web-tokens[JWT documentation].

We can also use the `access_token` to call the link:/docs/v1/tech/apis/users#retrieve-a-user[Retrieve User API] or to call the link:/docs/v1/tech/oauth/endpoints#userinfo[OpenID Connect Userinfo API] to retrieve the user information as well.
