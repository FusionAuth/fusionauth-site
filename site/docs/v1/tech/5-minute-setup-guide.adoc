...
* link:/docs/v1/tech/getting-started/5-minute-docker[Docker], which requires docker and docker compose. This is the best choice if you have docker and docker compose installed. It's relatively quick.
* link:/docs/v1/tech/getting-started/5-minute-fastpath[FastPath], which requires a local database (PostgreSQL or MySQL). This is the best option if you have a local MySQL or PostgreSQL database installed and want to get the full installation experience.
