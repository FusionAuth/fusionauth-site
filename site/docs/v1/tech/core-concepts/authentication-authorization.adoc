...
Authentication is the process that verifies someone is who they say they are. It’s sometimes referred to as authn or AuthN. Authentication typically challenges the user to validate credentials using factors they control or can present. 

You can complete an authentication process with factors like:



* **User name and password**: 
* **Biometrics**: a user uses fingerprint or face scan (WebAuthn & Passkeys)
* **One-time passcodes**: one-time use codes for a single access session
* **Answers to security questions*
* A combination of multiple of the above factors (multi-factor authentication) 
