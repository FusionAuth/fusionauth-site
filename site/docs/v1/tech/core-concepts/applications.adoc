...
image::core-concepts/application-webauthn.png[WebAuthn configuration,width=1200,role=bottom-cropped top-cropped]
...
The bootstrap workflow is used to allow users to sign in using WebAuthn on a new device for the first time.

==== Form Fields

[.api]
[field]#Enabled# [optional]#Optional#::
When enabled, users will be able to use the WebAuthn bootstrap workflow to sign in on new devices.
