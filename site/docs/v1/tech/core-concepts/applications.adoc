...
The IP access control list that will be used to restrict or allow access to themed pages in FusionAuth. For example, it may be configured to only allow specific IP addresses to access an application's login page.
+
When configured, this value will take precedence over any IP access control list configuration on the tenant.
