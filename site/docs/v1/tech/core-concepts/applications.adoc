...
A FusionAuth Application holds configuration for how your applications interact with FusionAuth. A one to one mapping between an external web, mobile or desktop application and a FusionAuth Application is not required. You could have multiple web and mobile applications configured to use one FusionAuth Application. You could have one web application authenticate against multiple FusionAuth Applications, if different authentication scenarios were wrapped up in one application (this isn't very common, though).

The `applicationId` and `client_id` can be considered synonymous, both concepts use the same `Id` value.
