...
[field]#Default verification key# [optional]#Optional#::
The default key used to verify the signature in the following scenarios:
* When the public key cannot be determined by the `KeyInfo` element when using POST bindings.
* When using HTTP Redirect bindings.
