...
[field]#Authorized redirect URLs (ACS)# [required]#Required#::
...
Enabled to required the SAML v2 Service Provider to sign the SAML v2 authentication request. When enabled, if a signature is omitted, the request will be rejected.
...
[.api]
[field]#Form# [optional]#Optional#::
The Admin form used by FusionAuth for adding or editing a registration. FusionAuth provides a default form that will be used unless otherwise specified.
+
Updating this fields requires a paid edition of FusionAuth.
