...
+
* `Exact match` - The configured values cannot include wildcards. Values during OAuth 2.0 workflows must match a configured value exactly.
* `Allow wildcards` - The configured values may contain wildcards. Values during OAuth 2.0 workflows can be matched against wildcard patterns or exactly match a configured value.
