...
Often times you want to segment or separate your users.
...
They can have a different passwords, different identity provider links and different attributes.
...
One option is set up an administrative identity server, whether that is a second instance of FusionAuth, Google GSuite, or Azure AD.
...
Prohibit a user from logging into an application for which they are not registerred by checking the claims in the token or enabling the [field]#Require registration# setting for the application.
...
This page would be custom built and is not provided by FusionAuth.
...
The users selects an entity and then acts within the confines of that entity.
...
Each store would be an entity, and after the user has logged into an application, you'd show them all the store to which they'd have access.
