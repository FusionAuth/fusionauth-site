...
Users have sessions in FusionAuth. Sessions are equivalent to refresh tokens. Their lifetime is controlled by the Tenant or Application refresh token settings. These appear in the administrative user interface under the [breadcrumb]#Sessions# tab (you may need to scroll if your screen is small):
...
This session can only be started via a Login API call or an OAuth code flow that requests a refresh token. Any other kind of login doesn't have a session, because there is no way to get a new JWT short of the user logging in again. This session can end or or deleted via the link:/docs/v1/tech/apis/jwt/#revoke-refresh-tokens[JWT APIs]. When a session is no longer valid, the associated refresh token can't be used to create new JWTs. The JWTs themselves are valid until they expire.
...
image::user-search-database.png[User Search with Database Search Engine,width=1200,role=bottom-cropped]
...
image::user-search-query-string.png[User Search by Query String,width=1200,role=bottom-cropped]
...
image::user-search-json-query.png[User Search with JSON Query.,width=1200]
