...
This can then be read in a link:/docs/v1/tech/lambdas/jwt-populate[JWT Populate Lambda] and pushed into the tokens generated during authentication.

The downstream application can examine tokens and determine to which organizations a user has access.
...
This approach works well when you want FusionAuth to handle authentication but have all user segmentation logic in your application.
