...
FusionAuth is built from the ground up with localization in mind. You can assign a user's `preferredLanguages` via the API or link:/docs/v1/tech/guides/advanced-registration-forms/[advanced registration form]. and add that value to a JWT or retrieve it from via the link:/docs/v1/tech/apis/users[User API]. Each application registration can also have a preferred language associated with it.
...
If you set the user's preferred language, FusionAuth's administrative user interface will localized. There are no translations of the FusionAuth administration user interface, however. For instance, in the below screenshot, Richard's user has been configured to have a `preferredLanguage` of `French`. Below you can see that the user with the email dinesh@fusionauth.io was created on Oct 5, 2020, but the date is formatted as specified by the French locale. 

image::core-concepts/user-locale-fusionauth-admin-ui.png[The FusionAuth administrative interface as viewed by a user with a preferred language of French.,width=1200]

If you build your own applications, you can also localize them in the same way FusionAuth does.
