...
But what should you do with it? After all, an access token is a bearer token, which can't be revoked easily (though see link:/learn/expert-advice/tokens/revoking-jwts[here for strategies]).

This depends on what you are using the token for and your security requirements. The FusionAuth team has outlined a link:/learn/expert-advice/authentication/login-authentication-workflows[number of common flows here].

The primary storage mechanisms the FusionAuth team recommends are:

* sending the token down as a `Secure`, `HTTPOnly` cookie
* storing it server side, in a session
...
|Token can be presented to servers on domains other than that which the server side component resides in
|No
|Yes

|Accessible to anyone with physical browser access (they can look at the cookie)
|Yes
|No

|===

The proper JWT storage choice should be based on your threat modelling.

If you need to lock down access, you can implement https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html#token-sidejacking[token sidejacking protection]. An alternative is https://github.com/FusionAuth/fusionauth-issues/issues/1679[DPoP, an open feature request].
