...
These tokens can be consumed by APIs or other systems to verify that the holder of the token has been authorized by FusionAuth. link:/learn/expert-advice/tokens/[Learn more about JWTs] or link:/learn/expert-advice/dev-tools/jwt-decoder[decode a JWT].
