...
FusionAuth recommends the Authorization Code grant, where there is a server side component which exchanges the one-time use authorization code for an access token. This server side component offers a lot of flexibility around what happens with the token.

But what should you do with that token?

This of course depends on what you need, but the FusionAuth team has outlined a link:/learn/expert-advice/authentication/login-authentication-workflows[number of common flows here].

The two primary storage mechanisms the FusionAuth team recommends are:

* sending the token down as a `Secure`, `HTTPOnly` cookie
* server side, in a session

.JWT Storage Benefits
[cols="3,1,1"]
|===
...
|Accessible to anyone with physical browser access
|Yes
|No

|Transparently sent on requests
|Yes
|N/A

|Limited to cookie domain
|Yes
|No
...
JWT storage choice should be made based on your security threat modelling. In particular, if you need additional assurances, you can implement https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html#token-storage-on-client-side[JWT claim fingerprinting]. Another alternative is https://github.com/FusionAuth/fusionauth-issues/issues/1679[DPoP, an open feature request]
