...
1. The client would send a SCIM compliant request to the FusionAuth link:/docs/v1/tech/apis/scim[SCIM API endpoint].
+
[source,json]
...
----
+
2. FusionAuth will authenticate the incoming request to ensure the request is from a known SCIM client. Each SCIM client is represented as an Entity and will authenticate using the link:/learn/expert-advice/oauth/complete-list-oauth-grants#client-credentials-grant[Client Credentials Grant].
+
