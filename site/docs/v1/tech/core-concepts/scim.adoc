...
1. The client would send a SCIM compliant request to the link:/docs/v1/tech/apis/scim/SCIMUser[SCIM User endpoint] `/api/scim/resources/v2/Users`. Other link:/docs/v1/tech/apis/scim[SCIM API endpoints] are also available.
+
[source,json]
...
----
+
2. FusionAuth will authenticate the incoming request to ensure the request is from a known SCIM client. Each SCIM client and server instance is represented as an Entity and will authenticate using a link:/docs/v1/tech/apis/authentication#client-credentials[Client Credentials Grant].
