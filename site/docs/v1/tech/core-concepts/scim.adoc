...
++++
{% capture relatedTag %}topic-scim{% endcapture %}
...
* Make an API call from the https://fusionauth.io/docs/v1/tech/lambdas/scim-user-response-converter[user response complete lambda]. Ensure this idempotent as the lambda will be called any time the user is updated as well.
