...
[.api]
[field]#SCIM# [optional]#Optional#::
The SCIM server configuration if you wish for FusionAuth to handle incoming SCIM client provisioning requests
+
When [field]#SCIM# is configured on an application within the tenant, the application value will override the tenant value for that application.
