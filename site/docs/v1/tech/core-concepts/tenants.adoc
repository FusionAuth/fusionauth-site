...
When set to `Enabled` a two-factor challenge will be required during login when a user has configured one or more two-factor methods. When set to `Disalbed`, even when a user has one or more two-factor methods configured, a two-factor challenge will not be required during login.
+
