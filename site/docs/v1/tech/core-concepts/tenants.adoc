...
The status of the SCIM Server support. It must be enabled for the SCIM API Endpoints to function.

[field]#Client entity type# [required]#Required#::
The Entity Type defined for the SCIM Client.

[field]#Server entity type# [required]#Required#::
The Entity Type defined for the SCIM Server.

[field]#User request lambda# [required]#Required#::
The lambda that will be invoked on every incoming request to the SCIM User API endpoints.

[field]#User response lambda# [required]#Required#::
The lambda that will be invoked on every outgoing response from the SCIM User API endpoints.

[field]#EnterpriseUser response lambda# [required]#Required#::
The lambda that will be invoked on every outgoing response from the SCIM EnterpriseUser API endpoints.

[field]#EnterpriseUser request lambda# [required]#Required#::
The lambda that will be invoked on every incoming request to the SCIM EnterpriseUser API endpoints.

[field]#Group request lambda# [required]#Required#::
The lambda that will be invoked on every incoming request to the SCIM Group API endpoints.

[field]#Group response lambda# [required]#Required#::
The lambda that will be invoked on every outgoing response from the SCIM Group API endpoints.

[field]#Schemas# [optional]#Optional#::
The json response sent from the Schemas endpoint. This can be customized however you like, but by default it will response with the core SCIM link:https://datatracker.ietf.org/doc/html/rfc7643#section-8.7.1[schemas] for SCIM Group, SCIM User, and SCIM EnterpriseUser.
