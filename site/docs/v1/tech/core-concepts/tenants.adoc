...
Unique usernames may be enabled to allow more than one user to select the same username and allow FusionAuth to manage a unique suffix.

[.api]
[field]#tenant.usernameConfiguration.unique.enabled# [optional]#Optional# [default]#defaults to `false`# [since]#Available since 1.27.0#::
When `true`, FusionAuth will handle username collisions by generating a random suffix.
+
include::docs/v1/tech/shared/_premium-edition-blurb.adoc[]

[field]#tenant.usernameConfiguration.unique.numberOfDigits# [optional]#Optional# [default]#defaults to `5`# [since]#Available since 1.27.0#::
...
[field]#tenant.usernameConfiguration.unique.separator# [optional]#Optional# [default]#defaults to `&#35;`# [since]#Available since 1.27.0#::
...
[field]#Verification strategy# [optional]#Optional#::
The process for which the user will verify their email address.

[field]#Unverified behavior# [optional]#Optional#::
The way a user is handled during the login process when they do not have a verified email address.

[field]#Allow email change when gated# [optional]#Optional#::
When enabled, the user is allowed to change their email address when gated by the login flow.
