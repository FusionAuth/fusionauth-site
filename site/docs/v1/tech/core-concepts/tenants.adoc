...
[field]#Additional headers# [optional]#Optional#::
One or more line separated email headers to be added to each outgoing email. The header name and value should be separated by an equals sign. ( i.e. `X-SES-CONFIGURATION-SET=Value`).
...
[field]#Allow implicit verification# [optional]#Optional#::
