...
+
For example, if the login page is at `auth.example.com`, then the following are valid Relying Party Ids:
+
...
The settings in this section affect the availability and behavior of the WebAuthn bootstrap workflow. The bootstrap workflow is meant to allow users to sign in from a new device for the first time using WebAuthn.
...
+
The recommended value for the bootstrap workflow is `Cross-platform only` when the option is available.
+
