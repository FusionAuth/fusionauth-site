...
The lambda that will be invoked on every outgoing response from the SCIM EnterpriseUser API endpoints. This maps the incoming SCIM EnterpriseUser JSON to the User object.

[field]#Enterprise User request lambda# [required]#Required#::
The lambda that will be invoked on every incoming request to the SCIM EnterpriseUser API endpoints. This maps the outgoing User object to the JSON for a SCIM EnterpriseUser.
