...
The below table outlines how these roles were designed at a high level.  Of course, the risk is always relative to the information and organization.  Even a low-risk role can do significant damage in the wrong hands.
...
Below are all the roles available in FusionAuth.  Please note the additional explainer for the `user_support_manager` that follows this table.
