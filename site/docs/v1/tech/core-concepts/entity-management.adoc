...
When configuring FusionAuth to accept SCIM requests, you must create a SCIM Server Entity and a SCIM Client Entity.
These entities will complete the Client Credentials grant which will provide the access token which is used to authenticate calls to the SCIM endpoints.
These entities must be of the Entity Type configured in the Tenant SCIM configuration. 
They also must have the SCIM permissions granted to successfully call link:/docs/v1/tech/apis/scim[SCIM API endpoints] requiring authentication.

The necessary Entity Types can be created by selecting the down arrow next to the add icon icon:plus[role=ui-button green,type=fas] on the Entity Type homepage.
However, by FusionAuth ships with the two SCIM Entity Types that you can use.
...
These are required for any Entity Type you create which will be used to create Entities for SCIM authentication.

When you are creating an Entity representing a SCIM client, you must select one or more of these permissions, which will control which SCIM APIs can be called. 

* `scim:enterprise:user:create` - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#create-an-enterpriseuser[Create an EnterpriseUser]
* `scim:enterprise:user:delete` - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#delete-an-enterpriseuser[Delete an EnterpriseUser]
* `scim:enterprise:user:read` - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#retrieve-an-enterpriseuser[Retrieve an EnterpriseUser]
* `scim:enterprise:user:update` - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#update-an-enterpriseuser[Update an EnterpriseUser]
