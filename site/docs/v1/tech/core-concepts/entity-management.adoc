...
When configuring FusionAuth to accept SCIM requests, you must create a SCIM Server Entity and a SCIM Client Entity. These entities will complete the Client Credentials grant which will provide the access token which is used to authenticate calls to the SCIM endpoints. These entities must be of the Entity Type configured in the Tenant SCIM configuration. They also must have the SCIM permissions granted to successfully call link:/docs/v1/tech/apis/scim/[SCIM API endpoints] requiring authentication.
