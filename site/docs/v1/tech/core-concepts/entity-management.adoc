...
However, FusionAuth ships with two SCIM default Entity Types that you can use.
...
Each permission enables or disables access to an API endpoint. These are required for any Entity Type you create which will be used to create Entities for SCIM authentication.

When you create an Entity representing a SCIM server, you can choose which permissions it will expose by only creating those permissions that are needed by the clients. The permissions names must match the values in the table below. When you are creating an Entity representing a SCIM client, you must select one or more of the permissions available on the SCIM server entity in order for clients using those credentials to have access to the associated API. This allows you to control access to each individual API based on which permissions you decide to create in your SCIM server Entity Type, and then when you create a SCIM client Entity, you can choose which permissions to enabled for that specific Entity as well.
