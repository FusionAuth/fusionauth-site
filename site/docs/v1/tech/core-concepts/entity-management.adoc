...
When configuring FusionAuth to accept SCIM requests, you must create a SCIM Server Entity and a SCIM Client Entity. The Entity Types can be created by selecting the down arrow next to the add icon icon:plus[role=ui-button green,type=fas] on the link:#entity-types[Entity Type homepage]. However, by default FusionAuth ships with the two SCIM Entity Types provided. An external SCIM Client must be of the Entity Type SCIM Client and have the associated permissions enabled to successfully call authenticated link:/docs/v1/tech/apis/scim[SCIM API endpoints].

==== [FusionAuth Default] SCIM Client
The default SCIM Client Entity Type is the external SCIM client calling link:/docs/v1/tech/apis/scim[SCIM API endpoints].

==== [FusionAuth Default] SCIM Server
The default SCIM Server Entity Type is FusionAuth acting as the SCIM server and defines the available permissions for calling link:/docs/v1/tech/apis/scim[SCIM API endpoints].

==== [FusionAuth Default] SCIM Server Permissions
Each permission enables or disables access to an API endpoint.

* SCIMEnterpriseUserCreate - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#create-an-enterpriseuser[Create an EnterpriseUser]
* SCIMEnterpriseUserDelete - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#delete-an-enterpriseuser[Delete an EnterpriseUser]
* SCIMEnterpriseUserRead - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#retrieve-an-enterpriseuser[Retrieve an EnterpriseUser]
* SCIMEnterpriseUserUpdate - link:/docs/v1/tech/apis/scim/SCIMEnterpriseUser#update-an-enterpriseuser[Update an EnterpriseUser]
* SCIMGroupCreate - link:/docs/v1/tech/apis/scim/SCIMGroup#create-a-group[Create a Group]
* SCIMGroupDelete - link:/docs/v1/tech/apis/scim/SCIMGroup#delete-a-group[Delete a Group]
* SCIMGroupRead - link:/docs/v1/tech/apis/scim/SCIMGroup#retrieve-a-group[Retrieve a Group]
* SCIMGroupUpdate - link:/docs/v1/tech/apis/scim/SCIMGroup#update-a-group[Update a Group]
* SCIMUserCreate - link:/docs/v1/tech/apis/scim/SCIMUser#create-a-user[Create a User]
* SCIMUserDelete - link:/docs/v1/tech/apis/scim/SCIMUser#delete-a-user[Delete a User]
* SCIMUserRead - link:/docs/v1/tech/apis/scim/SCIMUser#retrieve-a-user[Retrieve a User]
* SCIMUserUpdate - link:/docs/v1/tech/apis/scim/SCIMUser#update-a-user[Update a User]
