...
It is currently not possible to have a users granted entity permissions returned via an OAuth Grant process.  A workaround would be to use the API to gather these details. +
link:https://github.com/FusionAuth/fusionauth-issues/issues/1295/[Please see active issue for additional information]