...
If you are importing password hashes, ensure you provide only the hash in the `password` import field. For some formats you'll need to separate out the hash and the salt on the link:/docs/v1/tech/apis/users/#import-users[Import API]. If you do not, the import will succeed, but the user will not be able to login.
