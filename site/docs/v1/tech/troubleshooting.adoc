...
SAML Identity Providers will not work without the proper CORS configuration. 
...
To see if CORS is the issue preventing SAML functionality from working:

* Enable debugging by navigating to [breadcrumb]#Settings -> System -> CORS# and selecting [field]#Debug enabled# and saving the settings.
