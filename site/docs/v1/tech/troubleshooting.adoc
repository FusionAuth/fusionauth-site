...
3. Run a re-install with the `-s` option enabled on FastPath.  This will reinstall Elasticsearch in additional to FusionAuth.
- `sh -c "curl -fsSL https://raw.githubusercontent.com/FusionAuth/fusionauth-install/master/install.sh | sh -s - -s"`

This should leave you with a clean install preserving:
