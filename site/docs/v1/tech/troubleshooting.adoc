...
If you are importing password hashes, ensure you provide only the hash in the `password` import field.

For some formats such as bcrypt, you'll need to separate out the hash and the salt on the Import API.

For example $2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy would be split out to the fields:

* factor: 10
* salt:  N9qo8uLOickgx2ZMRZoMye
* password: IjZAgcfl7p92ldGxad68LJZdL17lhWy

And each would be placed in the appropriate section of the import user request.
