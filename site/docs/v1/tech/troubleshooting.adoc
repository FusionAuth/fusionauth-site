...
== Troubleshooting Uninstall-Upgrade-Reinstall on RPM's
...
If you are looking to set up a clean install (scorched earth) using RPM's follow the directions below.  This section also loosely identifies how to properly clean out the FusionAuth directories for a fresh install or upgrade.

==== RPM - Remove Packages Manually
...
1. Run a re-install with the `-s` option enabled on FastPath (see warning regarding production below).  This will reinstall Elasticsearch in addition to FusionAuth.
