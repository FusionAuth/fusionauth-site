...
Many features have additional debugging that can be enabled via the administrative user interface or the API. These include, but are not limited to:

* Identity Providers
* Application SAML IdP configuration
* Connectors

If you are having issues with a particular feature, look for a [field]#Debug enabled# checkbox in the user interface. If present, enable this. 

Additional debugging information will be written to the <<Event Log>>.
...
After you have added these modules, you can configure JMX using `JDK_JAVA_OPTIONS` passed via any link:/docs/v1/tech/reference/configuration/[means of configuration].
