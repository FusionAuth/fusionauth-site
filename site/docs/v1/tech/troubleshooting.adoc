...
If you are looking to set up a clean install (scorched earth) using RPM's follow the directions below.

This section also loosely identifies how to properly clean out the FusionAuth directories for a fresh install or upgrade (but this is typically not required unless upgrade issues arise).

The guide below assumes that both FusionAuth App and FusionAuth Search will be uninstalled and reinstalled.
...
1. After removal, the following files will remain:
...
1. Run a re-install of FusionAuth App and FusionAuth Search
* Using the RPM commands
** `sudo rpm -i <file_to_be_installed>.rpm`
** link:/direct-download[Direct download packages/rpm link]
* If on a development machine, FastPath installation is also an option.
...
- Greater likelihood of a 'clean' uninstall/upgrade with a package manager (rpm)
====
