...
=== SAML and Apple Identity Provider

SAML and Apple Identity Providers will not work without the proper CORS configuration. 

If you are setting up a SAML Identity Provider in FusionAuth, link:/docs/v1/tech/identity-providers/samlv2/#cors-configuration[ensure you have configured CORS correctly].

If you are setting up an Apple Identity Provider in FusionAuth, link:/docs/v1/tech/identity-providers/apple/#cors-configuration[ensure you have configured CORS correctly].

If CORS is misconfigured, you will see a a `403` status code during the login process in the browser network console.
...
After debugging is complete, ensure you disable CORS debugging to avoid spamming the Event Log with unnecessary requests. Disable debugging by navigating to [breadcrumb]#Settings -> System -> CORS#, deselecting [field]#Debug enabled# and saving the settings.
...
If you are running FusionAuth on a server with limited memory, it may stop unexpectedly. This can also happen when many other applications are competing for memory on the server.
