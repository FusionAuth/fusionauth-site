...
The fix is to modify the problematic objects in the database and then re-index. If you have a support contract, please https://account.fusionauth.io/account/support/[open a ticket] and we'll help you out. 

If not, be aware this is a delicate operation since you are modifying a field in the database directly. Please backup your FusionAuth database and test the fix before applying it to production. 
