...
If you are having issues with certain functionality, look for a [field]#Debug enabled# checkbox in the user interface. If present, set it to true. The additional debugging information will be written to the <<Event Log>>.

=== Enabling JMX

link:https://openjdk.java.net/groups/jmx/[JMX] is an API which allows deeper insight into and monitoring of Java applications, including FusionAuth.

FusionAuth uses jdk 14, which is modular. To use JMX, you’ll need to include the correct module. The modules you need are:

* `jdk.management.agent`
* `jdk.httpserver` if you are using the JMX http adapter

These should be part of a standard Java install, but if you are using the Docker image, which is built with jlink, you'll need to build a new image with these modules. There's a link:https://github.com/FusionAuth/fusionauth-containers/issues/70[GitHub issue about adding these modules to the FusionAuth image].

After you have added the above modules, you can configure JMX by passing additional arguments to FusionAuth using any of the supported link:/docs/v1/tech/reference/configuration/[configuration options] such as adding a `fusionauth-app.additional-java-args` entry to the `fusionauth.properties` file.
