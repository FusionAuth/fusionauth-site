...
FusionAuth sends a lot of email. Troubleshooting email is difficult, as there are so many factors affecting it.
...
If you cannot, using a tool such as https://www.jetmore.org/john/code/swaks/[SWAKS] can help debug SMTP issues. Using this tool takes FusionAuth out of the equation and ensures that you can send email from the server on whic FusionAuth is installed.

If you can send the test email but aren't receiving other emails:
...
. Save the tenant
. Send an email. 
. View the system logs: [breadcrumb]#System -> Logs#. 
. Select `fusionauth-app.log` and you will see verbose SMTP output.

Note that this will log the full SMTP conversation, which can be verbose, so you should only use this for troubleshooting.
