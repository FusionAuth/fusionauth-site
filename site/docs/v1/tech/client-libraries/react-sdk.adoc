---
layout: doc
title: FusionAuth React SDK
description: The FusionAuth React SDK allows you to add login, logout and registration functionality to your React application.
navcategory: developer
---
:page-liquid:

:sectnumlevels: 0

== React SDK

The React SDK allows you to add login, logout and registration functionality to your React application.

Source Code:

* https://github.com/FusionAuth/fusionauth-react-sdk/

// this is pulled from the readme so that it is always up to date. There is a 5-10 minute lag from when a change in GitHub will be reflected here. This is pulled on site build, not when the change is made in GH.

include::https://raw.githubusercontent.com/FusionAuth/fusionauth-react-sdk/main/README.md[tags=forClientLibraryPage]

