...
Each template comes with a script to quickly set up FusionAuth with the correct settings and parameters. Find the script in the `FusionAuth-Setup` directory of a project created from a template. Once you have created a new project, view the `README.md` file in the project root directory for more information on running the setup script for that template type. Note that the setup scripts are designed to run on a newly installed FusionAuth instance with only one user and no tenants other than `Default`. To use the setup scripts on a FusionAuth instance that does not meet these criteria, you may need to modify the scripts.


Some templates will ask for a FusionAuth Client Secret when initializing a new project. Use a non-sensitive secret from a local FusionAuth installation. If you use the setup scripts to set up your FusionAuth instance, the Client Secret will be `change-this-in-production-to-be-a-real-secret`.

Do not use a production or sensitive Client Secret in the template prompts, as it is stored in the `Appsettings.json.development` file, and will be available in plain text and committed to your repo. To provide the Client Secret to your application in production, you should use one of the secure methods https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets?view=aspnetcore-7.0&tabs=windows[recommended by Microsoft]. The platform-independent hierarchical path for the Client Secret setting is `FusionAuth__ClientSecret`.
