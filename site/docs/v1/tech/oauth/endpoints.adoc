...
[field]#refresh_token_id# [type]#[UUID]#::
The unique ID of the refresh token. This ID can be used to retrieve or revoke a specific refresh token when using the link:/docs/v1/tech/apis/jwt[Refresh Token API].
+
This field will be returned in the response when `offline_access` scope was requested unless refresh tokens have been disabled for this application.
See `oauthConfiguration.generateRefreshTokens` in the link:/docs/v1/tech/apis/applications[Application API] or the `Generate refresh tokens` toggle in the FusionAuth UI when editing an application.

[field]#scope# [type]#[String]#::
The space-separated list of scopes that were granted to this access token. This will only be present for grant types of `password` and `authorization_code`.
