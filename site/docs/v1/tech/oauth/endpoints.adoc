...
+
The header value is created by taking the `clientId` and `clientSecret` properties defined in the Entity Configuration and concatenating them together using a `:` character. Here is an example of these two values concatenated.
+
[source]
----
...
[NOTE]
====
You must add an asymmetric key for any any public keys to be available at this endpoint. 

A default FusionAuth installation has no public/private keypairs. You may add them using the admin UI by navigating to [breadcrumb]#Settings -> Key Master# and generating or importing them.
====
