...
This endpoint provides a mechanism to invalidate the user's session held by FusionAuth, this effectively logs the user out of the FusionAuth SSO. Calling this endpoint does not revoke refresh tokens or JWTs. You can revoke the former by using the link:/docs/v1/tech/apis/jwt[Revoke Refresh Tokens API]. To revoke JWTs, link:/articles/tokens/revoking-jwts[implement a revocation strategy].
...
Whether a refresh token is allowed for an application can be controlled by the `oauthConfiguration.generateRefreshTokens` field in the link:/docs/v1/tech/apis/applications[Application API] or the `Generate refresh tokens` toggle in the FusionAuth admin UI.

[field]#refresh_token_id# [type]#[UUID]# [since]#Available since 1.37.0#::
The unique Id of the refresh token. This Id can be used to retrieve or revoke a specific refresh token using the link:/docs/v1/tech/apis/jwt[Refresh Token API].
+
...
[field]#scope# [type]#[String]# [since]#Available since 1.5.0#::
...
[field]#client_secret# [type]#[String]# [optional]#Optional# [since]#Available since 1.46.0#::
The client secret.
This parameter is optional when the `Authorization` header is provided.
Please see the link:#client-secret-table[the Client Secret table] for more details.
...
[field]#permissions# [type]#[Map<UUID, List<Object>]#::
...
.URI
--
[method]#GET# [uri]#/`{tenantId}`/.well-known/openid-configuration#
--

==== URL segment

[.api]
[field]#tenantId# [type]#[UUID]# [required]#Required# [since]#Available since 1.46.0#::
...
Previous to version `1.28.0`, whether the client secret was required was controlled by the [field]#oauthConfiguration.requireClientAuthentication# value of the application object.
...
For versions `1.28.0` or later, use [field]#oauthConfiguration.clientAuthenticationPolicy# to configure this functionality.
