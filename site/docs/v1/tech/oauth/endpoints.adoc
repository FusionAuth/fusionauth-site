 ...
 ** <<Approve the Device Authorization Grant Request>>
...
|The the user's preferred locale.  This parameter will be omitted if the user has not set a preferred locale.
...
|400
|The request was invalid and/or malformed. The response will contain a JSON message with the specific errors or errors on the redirect URI. The error responses
are covered in the link:./error[OAuth Error section] of the API documentation.
...
|The access token.  This request parameter will be omitted if an access token was not requested in the [field]#response_type# request parameter.
...
covered in the link:./error[OAuth Error section] of the API documentation.
...
|400
|The request was invalid and/or malformed. The response will contain a JSON message with the specific errors. The error response JSON is covered in the link:./error[OAuth Error section] of the API documentation.
...
This API has been available since 1.11.0
====
This is an optional means of the Device Activation approval, if only a refresh token is available. Otherwise you can use the <<Authorization Code Grant Request>> with the user_code.
...
.URI
--
[method]#GET# [uri]#/oauth2/device/validate``?user_code=\{user_code\}``#
--
...
----
{
  ...
  "error_description": "The user code provided is not valid.",
  "error_reason": "invalid_user_code"
}
----
