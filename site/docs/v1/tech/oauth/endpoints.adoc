...
+
The header value is created by taking the `clientId` and `clientSecret` properties defined in the Entity OAuth Configuration TBD and concatenating them together using a `:` character. Here is an example of these two values concatenated.
+
[source]
----
...
----
+
This value is then Base64 encoded. Prepend `Basic` to denote the schema type. The following is an example Authorization header using the example concatenation above.
+
[source]
----
...
+
You will almost always want to provide a scope specifying the target entity: `target-entity:92dbded-30af-4149-9c61-b578f2c72600`. You may want to provide a scope specifying the permissions: `target-entity-permission:write target-entity-permission:read`.
+
...
grant_type=client_credentials
    &scope=target-entity%3A092dbded-30af-4149-9c61-b578f2c72600%20target-entity-permission%3Awrite%20target-entity-permission%3Aread
----
