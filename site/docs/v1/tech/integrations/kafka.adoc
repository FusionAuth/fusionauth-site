...
After successfully connecting to a Kafka instance, you'll be notified for each event that happens in FusionAuth. Events will generally contain fields for 
...
As well as other fields as applicable to the event. You can find the full schema for each event in the link:/docs/v1/tech/events-webhooks/events[webhook events] documentation.
...
2. Creating a new user with email address `newuser@example.com`
