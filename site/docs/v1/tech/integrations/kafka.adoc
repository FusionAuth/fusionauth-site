...
Events can be catagorised into 2 broad types:
- System events, which include audit log events, event log data
- Tenant based events, which include detailed information about user creation/removal/changes 

By default, System events will be sent to Kafka without further configuration. Tenant events however, are dependent on a link:https://fusionauth.io/docs/v1/tech/events-webhooks/[webhook] being setup and active. The events sent to Kafka then follow the configuration of events for that webhook and tenant. If you don't already have a webhook configured, and want to use Kafka for Tenant level events, we recommend you setup a no-op webhook receiver that accepts the incoming POST request, discards it and returns a `200 OK` status. This will allow you to setup a dummy webhook configuration to control Kafka Tenant level events. To create such a receiver, you can use a low-code platform such as link:https://pipedream.com[Pipedream], or link:https://zapier.com[Zapier], or roll your own. 
