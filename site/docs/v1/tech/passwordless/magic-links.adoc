...
+
image::passwordless/magic-links/turn-on-passwordless.png[Turn on magic links passwordless login,width=1200]
...
Choose the FusionAuth hosted login pages approach if you want to use the link:/docs/v1/tech/oauth/#example-authorization-code-grant[Authorization Code grant]. Enabling magic links adds an option for a user to receive a one-time code. Because this is the Authorization Code grant, any library or framework that supports this OAuth grant will work.
...
+
image::passwordless/magic-links/login-page-with-magic-link.png[The magic links login magic link.,width=1200,role=bottom-cropped]
. Enter the user's email
+
image::passwordless/magic-links/passwordless-email-request-form.png[The magic links email request form.,width=1200,role=bottom-cropped]
...
While using the FusionAuth hosted login pages works for many, you may need more control or flexibility. You can use the passwordless API to authenticate a user with a one-time code. The link:/docs/v1/tech/apis/passwordless[API reference docs] cover each of the API calls, but this guide will walk you through an implementation.
...
image::passwordless/magic-links/duplicating-email-template.png[Duplicate the magic link login email template.,width=1200]

Then modify it with your branding and messaging.

image::passwordless/magic-links/modifying-email-template.png[Modifying the magic link login email template.,width=1200]

Configure the tenant email template settings to use your template.

image::passwordless/magic-links/update-tenant-with-new-template.png[Updating the tenant to use the new magic link email template.,width=1200]
...
image::passwordless/magic-links/localization-screen-for-email-template.png[The localization screen for your email templates.,width=1200]
...
image::passwordless/magic-links/tenant-settings.png[The tenant settings to customize code lifetime and generation strategy.,width=1200]
