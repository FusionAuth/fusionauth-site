...
Azure AD has implemented a well-known configuration endpoint, FusionAuth will be able to discover the necessary endpoints using a discovery document by entering the Microsoft Authority URL (Issuer) in the [field]#Issuer# field.

The Microsoft URL may different across national clouds, so you will need to review the Microsoft documentation to ensure you have the correct URL for your region. For the Microsoft global Azure AD service, the URLs are as follows, where `{tenantId}` is the `Directory (tenant) ID` previously noted while creating our Azure AD Application.
...
You will need to specify `openid` as a [field]#Scope#.
...
:get_started_link: link:https://docs.microsoft.com/en-us/azure/active-directory/develop/quickstart-register-app[Azure - Register An App Quickstart Guide]
:identity_provider: Azure
:linking_strategy: Link on email.  Create the user if they do not exist
