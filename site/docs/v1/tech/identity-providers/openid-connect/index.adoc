...
When configuring an OpenID Connect identity provider, you'll need to provide a redirect URL. This is the URL to which the OIDC IdP will send a successful request. From there, FusionAuth will process the request, run the link:/docs/v1/tech/lambdas/openid-connect-response-reconcile/[configured lambda], if any, and log the user into FusionAuth. From there, the redirect URL of the application will be sent the authorization code.

This means there are two redirect URLs:

* The application redirect URL, which catches the Authorization Code from FusionAuth and exchanges it for a token. This is a custom route that you write, and so you control the URL.
* The FusionAuth redirect URL, which handles the response from the OIDC IdP. This will always have the path `/oauth2/callback`. If your FusionAuth server is running at `login.pipedpiper.com`, the FusionAuth redirect URL will be `\https://login.pipedpiper.com/oauth2/callback`.

This can be somewhat confusing. Here is the request flow through an OIDC login where the application is Pied Piper, the authorization server is FusionAuth, and the remote IdP is Azure Active Directory.
