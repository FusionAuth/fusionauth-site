...
Under [breadcrumb]#Sign On#, edit the [breadcrumb]#OpenID Connect ID Token# and change the [field]#Issuer# to use the `Okta URL`. Hit [uielement]#Save# and copy this URL into a text file.
...
- In the [breadcrumb]#Applications# section, ensure that the application you are adding Okta authentication to has the [field]#Enabled# and [field]#Create Registration# switches toggled. Hit [uielement]#Save#.
