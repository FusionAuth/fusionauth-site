...
[NOTE]
====
GitHub user's must have a public email address configured to link on email.  A more cautious approach would be to link on username or create an anonymous link to ensure that this Github OIDC connection is always functional regardless of Github email status.
====
