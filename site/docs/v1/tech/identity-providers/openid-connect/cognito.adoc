...
As mentioned above, you can skip this step if you already have a user pool created and enabled the Authorization Code Grant, though you will need to make sure you add the FusionAuth [field]#Callback URL# in order for the Authorization Code grant to complete.

To create the client, in your user pool:

- Under [field]#Initial app client# on Step 5, set the app type to `confidential`.
...
- Create a user, filling out all the form fields.



=== Configure a New FusionAuth OpenID Connect Identity Provider

There is no preconfigured Identity Provider for Cognito in FusionAuth. The generic "OpenID" Identity Provider can be used though, as Cognito supports the standard OpenId Connect protocols.
