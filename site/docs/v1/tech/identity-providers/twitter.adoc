...
The API key and API secret key are typically presented at the onset of app creation. Record these values in a password vault (or similar) for safekeeping. They will be needed for later integration with the FusionAuth IdP.
...
In this example I have registered `\https://login.piedpiper.com/oauth2/callback` as the callback URL. Please note that you will need to `enable` 3-legged OAuth and complete all required fields.
...
See below for more detail. The linking strategy for Facebook. link:/docs/v1/tech/identity-providers/#linking-strategies[See Linking Strategies]
