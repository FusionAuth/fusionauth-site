...
In this example we are registering `\https://login.piedpiper.com/oauth/callback` as the callback URI. The redirect URL *must* be set to an absolute URL in the twitter console for your application. If your hostname is login.piedpiper.com, the redirect URL would be `https://login.piedpiper.com/oauth2/callback/`.
