...
The last step will be to create a LinkedIn Identity Provider in FusionAuth. Log in to the FusionAuth administrative user interface. Create an Identity Provider by navigating to [breadcrumb]#Settings -> Identity Providers# and click on the menu on the upper right and select `Add LinkedIn`.
...
image::identity-providers/identity-provider-linkedin-add.png[Add the LinkedIn Identity Provider,width=1200]
...
The linking strategy for LinkedIn. link:/docs/v1/tech/identity-providers/#linking-strategies[See Linking Strategies for more].

[field]#Reconcile lambda# [optional]#Optional#::
A lambda may be utilized to map custom claims returned from LinkedIn. See the link:/docs/v1/tech/lambdas/linkedin-reconcile[LinkedIn Reconcile lambda documentation] for more details.
+
...
:identity_provider_fragment: linkedin
:identity_provider_path: linkedin
