...
* Accidentally misconfigure SAML (incorrect IdP, verification key, etc) and SAML authentication is broken
...
This is a problem, but not an insurmountable one. Your options depend on when you discover the issue. 

If you are beginning your SAML configuration, you can avoid this scenario. Follow these steps:

* Don't enable set the value of [field]#Managed domains# before you have tested the SAML configuration. 
* Test the SAML configuration in a different or incognito browser window, ensuring that the admin application is always available to modify configuration.
* Add a user with a domain that is not in the [field]#Managed domains# setting. Ensure this user is registered with FusionAuth and has the admin role.

If you are currently locked out of your FusionAuth application, you have fewer options. Options include:
...
If you have neither an API key nor a known user, you can restore from a database backup, attempt to modify the database directly (which is not supported) or re-install FusionAuth.
