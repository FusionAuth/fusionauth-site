...
This documentation will guide you in configuring SAML v2 IdP for Okta. In this case, FusionAuth will act as Service Provider (SP) to Okta (IdP). Functionally, the result will allow you to display a [uielement]#Login with Okta# button on your FusionAuth application's login page and connect via SAML to Okta users/applications.
...
* An link:https://www.okta.com/free-trial/[Okta Workforce Identity Cloud account] connected to a business email address.
...
Click the [uielement]#Assign# button next to the user whom you will authenticate. Click [uielement]#Save and Go Back# then [uielement]#Done#.
...
Hit [uielement]#Save# and [uielement]#View# the identity provider you just created. Scroll down to the [breadcrumb]#SAML v2 Integration details# section and record these values so you can supply them to Okta.
...
Click [uielement]#Next# to get to the [breadcrumb]#Configure SAML# tab. Replace the [field]#Single sign-on URL# with the [field]#Callback URL (ACS)# integration detail from FusionAuth. Replace the [field]#Audience URI (SP Entity ID)# with the [field]#Issuer# integration detail from FusionAuth. Click [uielement]#Next# and [uielement]#Finish# to save these settings.
