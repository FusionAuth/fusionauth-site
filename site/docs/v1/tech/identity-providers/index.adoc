...
For each application, you can provide different configuration. You might do this if you had two different FusionAuth applications that were both using Apple as an identity provider, but using different Apple configuration settings.

You can override none, some or all of the configuration values.
...
Override settings are not available in the External JWT, SAMLv2, or OpenID Connect Identity Providers. You can create multiple instances of these providers, which allows for different configuration.
