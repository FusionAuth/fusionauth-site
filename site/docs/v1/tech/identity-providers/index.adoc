...
Here are some walkthroughs of linking scenarios. These are all assuming a user is logging into a remote IdP, Hooli and the FusionAuth hosted login pages are being used. Similar behavior is available via the link:/docs/v1/tech/apis/identity-providers/[Identity Provider API]. 

=== Link By Email For Existing User

The user is logging into NewApp. They don't have an account in FusionAuth.

* The user clicks on Hooli on the login screen. 
* They log in with the `richard@fusionauth.io` account. 
* They are returned to FusionAuth but see an error. TODO WHAT CAN THEY DO

=== Link By Email For Existing User, Account Exists

The user is logging into NewApp. They have an account in FusionAuth.

* The user clicks on Hooli on the login screen. 
* They log in with the `richard@fusionauth.io` account. 
* They are returned to FusionAuth and login successfully.
* The FusionAuth account with the email `richard@fusionauth.io` is linked to the Hooli `richard@fusionauth.io` account.
...
* The user clicks on Hooli on the login screen. 
* They log in with `richard@fusionauth.io`. 
* They are returned to FusionAuth.
* A new account is created in FusionAuth with the email `richard@fusionauth.io`. 
* The new FusionAuth account with the email `richard@fusionauth.io` is linked to the Hooli `richard@fusionauth.io` account.
...
* The user clicks on Hooli on the login screen. 
* They log in with the `richard` account. 
* They are returned to FusionAuth but see an error. TODO WHAT CAN THEY DO

=== Link By Username For Existing User, Account Exists
...
The user is logging into NewApp. They have an account in FusionAuth with the username `richard@fusionauth.io`.

* The user clicks on Hooli on the login screen. 
* They log in with `richard@piedpiper.com`, their account at Hooli.
* They are returned to FusionAuth.
* They are prompted to login to FusionAuth with their current NewApp username and password.
* They login with `richard@fusionauth.io`.
* The FusionAuth account with the email `richard@fusionauth.io` is linked to the Hooli `richard@piedpiper.com` account.

=== Anonymously Link

The user is logging into NewApp. They do not have an account in FusionAuth.

* The user clicks on Hooli on the login screen. 
* They log in with `richard@piedpiper.com`, their account at Hooli.
* They are returned to FusionAuth.
* There is an account created in FusionAuth with the `richard@piedpiper.com` email. However, it is not a full account.
* The user can interact with NewApp, but is not allowed to login to FusionAuth.

=== Anonymously Link, No Email Or Username Returned

The user is logging into NewApp. They do not have an account in FusionAuth. They are using the Hooli XYZ server to login, and that does not return a username or password.

* The user clicks on Hooli XYZ on the login screen. 
* They log in with `richard@hoolixyz.com`.
* They are returned to FusionAuth.
* There is an account created in FusionAuth with . However, it is not a full account.
* The user can interact with NewApp, but is not allowed to login to FusionAuth.
...
They do not have a username or email address. None of the FusionAuth hosted workflows will work for them (reset password, passwordless, etc). 
