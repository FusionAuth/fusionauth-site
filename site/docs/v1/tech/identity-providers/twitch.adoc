...
image::identity-providers/twitch-warn-in-app.png[Twitch Warning, role=bottom-cropped, width=1200]

After the user accepts the warning, they may be presented with an overview page asking if they grant permission to share their Twitch identity.

image::identity-providers/twitch-overview.png[Twitch Overview, role=bottom-cropped, width=1200]

Once the user clicks `Authorize`, Twitch will provide the user's information to FusionAuth.

Since this IdP's linking strategy defaults to `Create pending link`, FusionAuth will give you three options to link this account.
...
image::identity-providers/link-options-ui.png[Link Options, width=1200]

Once linked, the user can now log in with Twitch and you are complete with this setup!
