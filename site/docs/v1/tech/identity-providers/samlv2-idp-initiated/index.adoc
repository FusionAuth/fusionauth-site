...
Starting in version 1.43.0 if you wish instead of creating a standalone configuration for an IdP Initiated Identity Provider you may instead enable IdP Initiated logins with a link:/docs/v1/tech/identity-providers/samlv2/[standard SAMLv2 Identity Provider configuration]. This will allow users to log in with an IdP Initiated or SP Initiated request from the same configuration.
