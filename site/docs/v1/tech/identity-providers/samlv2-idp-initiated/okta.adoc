...
Navigate to [breadcrumb]#Applications -> Your Application -> OAuth#. Update the [field]#Authorized redirect URLs# field. This URL is where a user will end up after authentication and may be any value URL.

[NOTE]
====
By default, FusionAuth will use the first redirect specified in the link:/docs/v1/tech/identity-providers/samlv2-idp-initiated/okta#configure-the-fusionauth-application-redirect-url[Application OAuth Configuration]. If you would like to specify a different redirect you can do so by configuring link:/docs/v1/tech/identity-providers/samlv2-idp-initiated/okta#optional-adding-a-redirect-after-saml-login[Okta as below].
====
...
* Optionally set the value of [field]#Default RelayState# if you want to provide a specific redirect URI. If this value is omitted, the first Authorized redirect URI found in the FusionAuth Application OAuth configuration will be used. This option is supported in FusionAuth version greater than or equal to `1.41.0`. In prior versions, append `redirect_uri=` to the value specified [field]#Single sign on URL# to control the final redirect URI.  For more information link:/docs/v1/tech/identity-providers/samlv2-idp-initiated/okta#optional-adding-a-specific-redirect-after-saml-login[see section below].
...
There are a few optional workflows (adding a persistent Id, sending over an email attribute, and configuring a custom redirect) you can employ listed below in the next *two* sections.  If these are not needed, please save the Okta application by clicking [breadcrumb]#"Next"# until complete and skip down.

=== Optional: Adding a Specific Redirect After SAML login

*Scenario:* you want a user to return to a specific redirect url after SAML auth is complete, there are two options to specify this url:
...
- Example: +
`<host>/samlv2/acs/22cea679-83af-422a-806a-baae792b3ab9/85a03867-dccf-4882-adde-1a79aeec50df?redirect_uri=http://hooli.com`
...
At times it may be helpful to receive a completely unique and immutable id from Okta and tie this to the new FusionAuth user (this is the value of a persistent SAML NameID).  To do this, on the `Edit SAML Integration` Okta (current) screen please click `Add Another` under the `Attribute Statements` section.  Under the [field]#Name# field enter `userId`, for [field]#Name format# leave as `Unspecified`, and finally, for [field]#Value# enter `user.id`. With this attribute added, Okta will be sending over a persistent Id for each user.

You can also instruct Okta to send over another attribute in the AuthN Response of email (useful for when link:/docs/v1/tech/identity-providers/#linking-strategies[linking strategy] is based on email). Under the [field]#Name# field enter `email`, for [field]#Name format# leave as `Unspecified`, and finally for [field]#Value# enter `user.email`.
...
By adding these two values you will be sending an AuthN Response similar to the below:
...
// ...
    <saml2:AttributeStatement xmlns:saml2="urn:oasis:names:tc:SAML:2.0:assertion">
      <saml2:Attribute Name="email" NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:unspecified">
        <saml2:AttributeValue xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xsi:type="xs:string">joshua@fusionauth.io</saml2:AttributeValue>
      </saml2:Attribute>
