...
+
[since]#Since 1.44.0#
The `Use popup for login` method will render the button in an `iframe` because of changes to the Google JavaScript API.
+
...
+
[since]#Since 1.44.0#
The [field]#Button text# value will not be used when [field]#Login method# is set to `Use popup for login` because of changes to the Google JavaScript API.
...
+
[since]#Since 1.44.0#
The [field]#Scope# value will not be used when [field]#Login method# is set to `Use popup for login` because of changes to the Google JavaScript API. The `email`, `profile`, and `openid` scopes will always be requested.
...
[field]#One Tap# [optional]#Optional# [default]#defaults to `false`# [since]#Available since 1.44.0#::
