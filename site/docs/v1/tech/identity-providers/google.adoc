...
+
[since]#Since 1.44.0#
The `Use popup for login` and `Use vendor JavaScript` methods will render the button in an `iframe` because of changes to the Google JavaScript API.
+
...
+
[since]#Since 1.44.0#
The [field]#Button text# value will not be used when [field]#Login method# is set to `Use popup for login` or `Use vendor JavaScript` because of changes to the Google JavaScript API.
...
+
[since]#Since 1.44.0#
The [field]#Scope# value will not be used when [field]#Login method# is set to `Use popup for login` or `Use vendor JavaScript` because of changes to the Google JavaScript API. The `email`, `profile`, and `openid` scopes will always be requested.
...
The Options tab can be used to configure the Google identity provider via the https://developers.google.com/identity/gsi/web/reference/html-reference[Google Identity Services HTML API].
