...
You will be prompted to define a consent type. A more common option is `External`.
...
Once this is complete Google will direct you to complete or update scopes. It is typical to select `userinfo.email` and `userinfo.profile` as well as `openid`, but feel free to enable any you would like. Remember these same scopes should be referenced in the IdP configuration in FusionAuth (see details below)
...
If selected, the `Authorized JavaScript origins` URL *must* be allowed for `[your_host_name]` in the Google console for your application. Please note, if this is used
+
Please note if an `idp_hint` is appended to the OAuth authorize endpoint, then the login behavior will be defaulted to `use redirect`, event if popup is explicitly configured.
+
...
The linking strategy for Google. link:/docs/v1/tech/identity-providers/#linking-strategies[See Linking Strategies]
