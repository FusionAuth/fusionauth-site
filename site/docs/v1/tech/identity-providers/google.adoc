...
Adding a Login with Google button to FusionAuth is a matter of procedure only, and this guide will walk you through the steps necessary to collect the credentials from Google in order to enable this Social login.
...
You will be promoted to define a consent type.  A more common option is `External`.

image::identity-providers/google-cloud-configure-consent-internal-external.png[Configure Consent,width=1200,role=shadowed bottom-cropped]
...
Once this is complete Google will direct you to complete or update scopes.  It is typical to select userinfo.email and userinfo.profile as well as openid, but feel free to require any that you would like.  Remember these same scopes should be referenced in the IdP in FusionAuth (see details below)

image::identity-providers/google-cloud-configure-scopes.png[Configure Scopes,width=1200,role=shadowed top-cropped]

Now you may return to the Credential section, click on `Create credentials` and select `OAuth client ID`.

On this panel, select `Web application`, specify the name of the credential and fill out the `Authorized JavaScript origins` field. This value should be the origin of your application login page. In this example I have specified `\https://login.piedpiper.com` because this is the URL of FusionAuth for Pied Piper.  Additionally, you will want to set the redirect URI to `your_host_name` + `/oauth2/callback/` as shown if you set the Google Identity Provier (below) to `
...
If selected, the redirect URL *must* be set to `your_host_name` + `/oauth2/callback/` in the Google console for your application.
+
image::identity-providers/google-redirect-page.png[Google Redirect,width=375,role=custom]
+
* `User popup for login` - if selected, a popup is displayed to the user to login with google.  Once authenticated, the window is closed.
If selected, the `Authorized JavaScript origins` URL *must* be allowed for `your_host_name` in the Google console for your application.
+
image::identity-providers/google-popup-page.png[Google Redirect,width=400,role=custom]
+

[field]#Scope# [optional]#Optional#::
This optional field defines the scope you're requesting from the user during login. See the Google login documentation for further information.  Three common scopes to include are `email profile openid`.
