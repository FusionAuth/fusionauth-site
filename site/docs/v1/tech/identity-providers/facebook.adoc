...
If you are using the redirect parameter (below `Use redirect for login`), you will want to add `[host] + /oauth2/callback`  to your `Valid OAuth Redirect URIs` . If using `Use Popup for login` you will need to fill in an `App Domain`, `User Data Deletion` field, and `Privacy Policy URL`. When selecting a domain, add the top level domain of your application in the `App Domains` field (for example, in our app we added `piedpiper.com` because our application login will originate from `\https://login.piedpiper.com`).
...
Note that status of this application shows `OFF`, to make this application public you will need to change the status which will require a few other settings that Facebook will prompt you to complete. However, you may still use this application to login while in this 'development' state.
...
If selected, the redirect URL *must* be set to `[your_host_name]` + `/oauth2/callback/` in the Facebook console for your application.
+
...
+
Please note if an `idp_hint` is appended to the OAuth authorized endpoint, then the login behavior will be defaulted to `use redirect`, event if popup is explicitly configured.
+
