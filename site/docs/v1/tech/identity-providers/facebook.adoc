...
If you are using the redirect parameter (below), you will want to add `[host] + /oauth2/callback`  to your `Valid OAuth Redirect URIs` .  Otherwise, if using the popup parameter (below), you can leave the `Valid OAuth Redirect URIs` field empty.  If using `Use Popup for login` you will need to fill in an App Domain, User Data Deletion field, and Privacy Policy URL.
...
If selected, the redirect URL *must* be set to `[your_host_name]` + `/oauth2/callback/` in the Facebook vconsole for your application.
+
