...
If you are using the redirect parameter (below), you will want to add `[host] + /oauth2/callback`  to your `Valid OAuth Redirect URIs` .  Otherwise, if using the popup parameter (below), you can leave the `Valid OAuth Redirect URIs` field empty.
