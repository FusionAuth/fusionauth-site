...
Select [breadcrumb]#App IDs# and click Continue. On the next panel, select type as [breadcrumb]#App# and click Continue. On the next panel, fill in the Description field, specify the Bundle ID and select [breadcrumb]#Sign in with Apple# from the list of capabilities and then click Continue. On the next panel confirm your selection, make a note of the App ID Prefix (Team ID) and click Register.
...
The next is creating an Apple Identity Provider in FusionAuth. Switch to your FusionAuth portal, and navigate to [breadcrumb]#Settings -> Identity Providers# and click [breadcrumb]#Add Apple# from the top right selection button.
...
The optional scope value, this is a space separated list of requested user attributes. You may request the User's name, email or both. Note that you must specify at least the `email` scope for the default "Link on email" strategy (below) to work.  
+
Example: `name email`
...
When testing, if you forget to set the required scopes when adding the Apple Identity Provider in FusionAuth, you may see an error similar to the following on login:
...
To solve this, set the correct scopes (specifically `email`). Then reset access to your app on your Apple device. You can do this by  navigating to Phone Settings (or Preference on macOS) -> Apple Id -> Password & Security -> Apple ID logins -> {YOUR APP} > Stop using Apple ID.