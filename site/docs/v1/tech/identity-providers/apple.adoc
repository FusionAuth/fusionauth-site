...
Ensure that you have imported the private key provided by Apple into FusionAuth using the [field]#Key ID# specified in the Apple Developer Account dashboard. If you import with the default behavior, FusionAuth will provide an identifier. This generated identifier is unlikely to match what Apple expects.
