...
In addition if you need to support user registration when using the Safari webview on a mobile device, you will need to allow the `GET` request as the redirect flow is slightly different.
