...
Now that we have completed the Apple configuration, we can import the private key we collected in the previous step. To import the private key, navigate to [breadcrumb]#Settings -> Key Master# and select [breadcrumb]#Import Elliptic Private Key# from the actions in the top right.
