...
To support both mobile and web users, create two Applications in FusionAuth. Then enable the Apple Identity Provider for each Application, and then use the `Overrides` section to set the [field]#Service Id# to the correct value.
