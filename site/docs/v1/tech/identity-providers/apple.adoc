...
+
[since]#Since 1.43.0#
Beginning in version 1.43.0, [field]#Services Id# is no longer a required field. At least one of [field]#Bundle Id# or [field]#Services Id# is required.
...
Configure both [field]#Services Id# and [field]#Bundle Id# according to your needs. You can still override the configuration for both fields on a per-application basis.

The login API request from a native application needs to include [field]#data.isNativeApp# with a string value of `"true"`. This will use the [field]#Bundle Id# from the Apple IdP configuration rather than [field]#Services Id#.
