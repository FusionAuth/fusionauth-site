...
When an Sony Playstation Network identity provider is used to complete a federated login request FusionAuth will use the email address to reconcile user. You may optionally utilize a lambda to customize the user and user registration during this authentication event.
...
* `idToken` - the JSON payload returned by the Sony Playstation Network Token Info API
...
A default Sony Playstation Network reconcile lambda is available in FusionAuth that may be used or modified. The default Sony Playstation Network lambda function is documented below.
