...
When using Lambda HTTP Connect to make a HTTP requests, do not call a FusionAuth API which invokes the calling lambda, because it will fail. For example, in a JWT Populate lambda, do not invoke the Login API.

Requests from a lambda requires the lambda use the GraalJS engine, and HTTP requests will time out after two seconds.
