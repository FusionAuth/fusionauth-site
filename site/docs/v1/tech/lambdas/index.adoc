...
The Nashorn engine supports ECMA script version 5.1.
...
There are no limits on execution time.

=== HTTP Request Limitations

When making HTTP requests, if you make an API call in an lambda to an endpoint that triggers the same lambda, it will call itself recursively. This does not end well.
