...
Only the populate or reconcile function is allowed on the top level of a lambda.
If you want to have additional functions, create them as function objects inside the main function.
...
When making HTTP requests, if you make an API call in a lambda to an endpoint that triggers the same lambda, it will call itself recursively. This does not end well.
