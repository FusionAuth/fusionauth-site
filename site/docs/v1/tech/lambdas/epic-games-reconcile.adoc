...
When an Epic Games identity provider is used to complete a federated login request FusionAuth will use the email address to reconcile user. You may optionally utilize a lambda to customize the user and user registration during this authentication event.
// TODO: Confirm that it is email 👆
...
* `idToken` - the JSON payload returned by the Epic Token Info API
...
A default Epic Games reconcile lambda is available in FusionAuth that may be used or modified. The default Epic Games lambda function is documented below.
