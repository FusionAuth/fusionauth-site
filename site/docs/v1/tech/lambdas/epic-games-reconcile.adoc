...
There is not a default Epic Games reconcile lambda provided by FusionAuth. The following is an example lambda you can use as a starting point.

[source,javascript]
----
// Using the response from the Epic API, reconcile the User and User Registration.
function reconcile(user, registration, accessToken) {
