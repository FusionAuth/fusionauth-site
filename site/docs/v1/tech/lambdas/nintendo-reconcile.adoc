...
When an Nintendo identity provider is used to complete a federated login request FusionAuth will use the email address to reconcile user. You may optionally utilize a lambda to customize the user and user registration during this authentication event.
...
* `idToken` - the JSON payload returned by the Nintendo Token Info API
// TODO 👆 check above for accuracy

The two FusionAuth objects are well documented here in the link:/docs/v1/tech/apis/users/[User API] and link:/docs/v1/tech/apis/registrations/[Registration API] documentation. The `idToken` may contain various user claims depending upon the user's Nintendo configuration.
...
A default Nintendo reconcile lambda is available in FusionAuth that may be used or modified. The default Nintendo lambda function is documented below.
