...
When an Xbox identity provider is used to complete a federated login request FusionAuth will use the email address to reconcile user. You may optionally utilize a lambda to customize the user and user registration during this authentication event.
...
A default Xbox reconcile lambda is available in FusionAuth that may be used or modified. The default Xbox lambda function is documented below.
