...
To rehash your users passwords, do the following.

* Write a custom password hasher
* Import user passwords setting the scheme to the custom password hash
* Configure the tenant to rehash on password change with the new hash scheme you need. To do so, in the administrative user interface, navigate to [breadcrumb]#Tenants -> Your Tenant -> Password# and then to the [breadcrumb]#Cryptographic hash settings# section. Check the [field]#Re-hash on login# checkbox and save your tenant.

pic TBD

After you have configured this, when a user logs in, the password they provide will be transparently rehashed.
