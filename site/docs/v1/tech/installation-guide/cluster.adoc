...
FusionAuth is stateless and typically CPU bound. Clustering FusionAuth nodes improves performance and redundancy. You can run as many FusionAuth nodes in cluster as you'd like.

FusionAuth stores almost all state in its database. For clustered enviroments, the following interactions occur between nodes:

* Cache invalidations
* When the system log is downloaded, all nodes logs are provided
...
This infrastructure must be created and managed when operatin a FusionAuth cluster. However, this set up is beyond the scope of this document. 
...
The recommendation is to automate this process using whatever scripting or configuration managment tool you like. This will minimize the downtime. 
