...
* Set the `fusionauth-app.runtime-mode` to `production`. This setting ensures your users will never see maintenance mode.
You want to avoid that because that will write configuration information to only one node.
With a cluster, you should always be using `silent` mode with a runtime of `production`.
Ensure your database connection information is synchronized across all nodes.
With such a configuration, you will have to apply database upgrades manually.
