...
Performance bottlenecks are typically the database or the CPU nodes, depending on sizing of each of these components and primary functionality tested. (Logins, for example, use a lot of CPU due to hashing.) Make sure each of these components are monitored and sized appropriately.
