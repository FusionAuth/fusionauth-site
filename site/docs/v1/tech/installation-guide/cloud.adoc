...
It is common to have a custom domain already implemented (for illustrative purposes, let's say `auth.piedpiper.com`), and need to add second domain (currently offered only on High Availability Hosting), such as `second-auth.piedpiper.com` with minimal impact.

Before making any modifications, the use of `auth.piedpiper.com` will have created two records in your DNS provider:

1. **A Validation Record CNAME** - adding this CNAME to your DNS authorizes FusionAuth Cloud to use `auth.piedpiper.com`

    - `[random-value].auth.piedpiper.com. CNAME [random-value].acm-validations.aws.`


2. **A Vanity URL CNAME** - this CNAME will actually route traffic from `https://auth.piedpiper.com` to your FusionAuth deployment, `https://piedpiperdeployment.fusionauth.io`

    - `auth.piedpiper.com CNAME [uuid].durable.fusionauth.io` or
    - `auth.piedpiper.com CNAME piedpiperdeployment.fusionauth.io`

When updating your domain name, FusionAuth Cloud will display these same two records, as well as additional sets of two records for each custom domain requested.  Best practice is to:

1. Add the vanity URL CNAME's first
2. Wait for DNS propagation to occur,
3. Add additional validation record CNAMEs.
...
You may notice FusionAuth Cloud now supports a `[uuid].durable.fusionauth.io` CNAME. The below CNAME is functionally equivalent with one important distinction -- this durable url will not change.  This is most applicable in Disaster Recovery (DR). Using this updated CNAME will enable use of these services should you require them at a later point (you can inquire with our Sales team if DR becomes a business requirement).
...
For all deployments which include backups (HA and Business Cloud), the default retention is 3 days.
