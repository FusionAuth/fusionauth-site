...
Ensure your self-hosted instance uses the same FusionAuth and database versions as those used by your Cloud deployment. You can view your FusionAuth version in the administrative user interface. To find your deployment's database version, the best way currently is to open a support ticket. There's an https://github.com/FusionAuth/fusionauth-issues/issues/1390[open issue] to display the database version in the administrative user interface.
