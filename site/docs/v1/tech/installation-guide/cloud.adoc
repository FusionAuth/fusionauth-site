...
When performing any changes which require modifying cloud infrastructure, the cloud database is backed up, and then reconnected to the new infrastructure.  If you are not using a cloud database but rather a local database, then this data is already permanent and will be connected to the new host once available.

No data loss should occur and your configuration will remain.  Any perceived loss could indicate the need to rebuild the elastic index.  Please see link:/docs/v1/tech/core-concepts/search/#reindexing-elasticsearch[here] for additional details.
...
We offer custom domains on the High Availability plan only.  The reasons for this are twofold.

- Firstly, we use an Application Load Balancer to manage a DNS certificate for third party domains using TLS. This additional infrastructure requires an additional expense.
- Secondly, offering custom domains on only HA plans was a marketing decision around our product offerings.
