...
* The IP addresses of a FusionAuth Cloud deployment are not fixed. Whenever possible, use the domain name, which is fixed. If you need IP addresses of the deployment nodes for purposes such as a network firewall allow list, please open a https://account.fusionauth.io/account/support/[support ticket], but be aware that they may change. Please upvote or comment on this https://github.com/FusionAuth/fusionauth-issues/issues/1393[open issue about static IP addresses in FusionAuth Cloud].
