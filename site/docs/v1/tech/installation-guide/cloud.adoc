...
They are upgraded one at a time and can continue to serve traffic during the upgrade. 
...
* Test `1.x` on your staging or development environment.
...
* When the deployment is in the `Active` state in the user interface, or when the System Status API returns success, the deployment is upgraded to `1.x`.
...
If an upgrade is problematic for whatever reason, and you identify the issues within your database backup retention period, open a https://account.fusionauth.io/account/support/[support ticket].
