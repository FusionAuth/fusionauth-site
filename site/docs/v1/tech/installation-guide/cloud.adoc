...
* No access is provided to the server on which your deployment is running. This includes access to the database, Elasticsearch, or ssh. You can access your data via FusionAuth API or the administrative console.
...
* With HA and other multi-node deployments, requests are passed through a load balancer. When making requests to node specific metrics endpoints such as as `/api/status` or `/api/prometheus/metrics` each request may return different results because the response is specific to the service node responding to the request.
