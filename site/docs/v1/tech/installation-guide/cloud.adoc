...
* Each deployment's relational database can optionally be set up in a primary/secondary configuration, so if the primary becomes unavailable the secondary will "stand up" and continue to serve traffic. This is also known as "full database replication" and is a suggested configuration.
