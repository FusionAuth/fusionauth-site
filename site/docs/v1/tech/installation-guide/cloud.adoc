...
* [breadcrumb]#Users# - add and remove users from the account portal. Any user added may have either the Admin or Security Officer role. Users with the Admin role have additional privileges to create deployments and perform other administrative actions. Users with the Security Officer role are provided security notices.
...
| Medium | 1 - 10 requests per second
| Large | 10 - 50 requests per second
| X-Large | 50 - 150 requests per second
|===
...
* link:https://account.fusionauth.io/account/support/[Open a support ticket] and provide the database export file, the version of PostgreSQL you are running, and the name of the new FusionAuth deployment.
...
If you are on the HA Cloud and want a custom domain name such as `auth.example.com`, log into your portal, select the deployment menu (the three ... under "Actions") and choose the [field]#Custom URL(s)# option. 

Enter your desired custom URLs, such as `login.piedpiper.com` and follow the instructions.

image::installation-guides/cloud/custom-url-domain-entry-screen.png[The custom urls domain entry screen,width=1200]
...
You may need to know the IP addresses of the instances in your FusionAuth Cloud deployment. There are any number of reasons you might need this information, including:

* Adding the IP addresses to the allow list of an email deliverability vendor.
* Punching a hole in a corporate firewall to allow for webhook delivery.
* Adding the IP address to a network access control list to enable access to an on-premises LDAP directory with a connector.

[WARNING.warning]
====
The IP addresses of FusionAuth Cloud instances may change at any time. Prepare for this by scripting this procedure, running it regularly, and updating the allow list.

Hostnames are immutable, and if you can use them instead of IP addresses, that will require less maintenance.
====

You can find the IP addresses for your deployment, but be aware that these addresses may change at any time.

Use `dig` to find the address, based on your deployment hostname. Use the default hostname ending in `fusionauth.io`, not any custom domains you have added. Deployment nodes are named in a standardized manner.

If you registered `piedpiper.fusionauth.io` and it is an HA deployment, there are two nodes. For this deployment, the instance hostnames are `piedpiper-1.instance.fusionauth.io` and `piedpiper-2.instance.fusionauth.io`.

A single node deployment will only have the hostname `piedpiper-1.instance.fusionauth.io`.

Once you know your hostname, find the IP addresses by using the `dig` command like this:

[source,shell]
-----
dig piedpiper-1.instance.fusionauth.io
-----
...
[source,shell]
-----
; <<>> DiG 9.10.6 <<>> piedpiper-1.instance.fusionauth.io
;; global options: +cmd
;; Got answer:
;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: 34994
;; flags: qr rd ra; QUERY: 1, ANSWER: 1, AUTHORITY: 0, ADDITIONAL: 1

;; OPT PSEUDOSECTION:
; EDNS: version: 0, flags:; udp: 1232
;; QUESTION SECTION:
;piedpiper-1.instance.fusionauth.io. IN	A

;; ANSWER SECTION:
piedpiper-1.instance.fusionauth.io. 120 IN A	11.11.11.11

;; Query time: 59 msec
;; SERVER: 192.168.0.1#53(192.168.0.1)
;; WHEN: Tue Feb 01 18:05:29 MST 2022
;; MSG SIZE  rcvd: 77
-----

Look in the `ANSWER SECTION` for your IP address. Above, it is `11.11.11.11`. Repeat this for each node if needed.

There is an open issue to https://github.com/FusionAuth/fusionauth-issues/issues/1393[allow FusionAuth deployments to have static IP addresses]. If this is important to you, please upvote the issue.
