...
=== Migrations
If there were migrations required, they'll be applied automatically to your database if the installation link:/docs/v1/tech/installation-guide/fusionauth-app#runtime-modes[has a runtime mode of development]. 

==== Prior to 1.19

If the installation is in `production` mode make sure you apply the migrations out of band. 

==== Version 1.19 or later

You can use `silent` mode for db upgrades in `development` or `production` runtime modes. The runtime modes are no longer used to identify if silent mode is desired, instead set the `fusionauth-app.silent-mode` configuration value to `true`. See the link:/docs/v1/tech/reference/configuration/[configation reference] or the link:/docs/v1/tech/guides/silent-mode/[silent mode guide] for more information.

If you want to manage your own upgrades, performing the SQL migrations out of band is still a good option.
