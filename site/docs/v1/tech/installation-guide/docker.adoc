...
The default FusionAuth docker configuration sets up the network using the `bridge` configuration. This means that all the hosts defined by `docker-compose` can access each other. However, it means that any applications running on your host machine cannot be accessed by FusionAuth.

This is typically only an issue when FusionAuth is trying to access outside resources, such as sending email or requesting a webhook. For instance, if a Rails application is running locally on a Mac and you want FusionAuth, running in Docker, to send a webhook payload to it, it won't work. There is no network path from FusionAuth to the Rails application. 

In this situation, the recommended course of action is to either:

* Host your application inside the same docker network or
* Run FusionAuth on the host machine, using FastPath or another installation optin
