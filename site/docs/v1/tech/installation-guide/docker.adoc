...
Download the `docker-compose.yml` and the `.env` files and then start up the configured containers.
...
You can examine the files used with this installation method.

==== FusionAuth

The following is the `docker-compose.yml` file for FusionAuth without the Elasticsearch engine.
...
The following is the `docker-compose-override.yml`. When included in your working directory next to the above `docker-compose.yml`, your FusionAuth installation will use Elasticsearch. 
...
In the above example configurations you will find a database, search and FusionAuth service. Let's look at each of these.

=== Database Service

The database service provides a database. You will need to either set the `POSTGRES_PASSWORD` environment variable in the `db` service section, or more ideally, set the value in the host environment and leave it out of the `docker-compose.yml` file.
