...
If you don't want to provide the `database.root.user` or `database.root.password` to allow FusionAuth to set up the database itself, you can also follow the link:/docs/v1/tech/installation-guide/fusionauth-app#advanced-installation[advanced database installation guide] which lets you set up the database out of band.
