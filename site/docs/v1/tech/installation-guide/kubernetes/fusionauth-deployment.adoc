...
Open `values.yaml` with your favorite text editor and modify it as outlined below. This guide only outlines configuration values that must be set to deploy FusionAuth, but it is recommended to review the configuration in its entirety to get acclimated with available configuration options.
...
The following is an example of what you will see upon opening `values.yaml` for modification:
...
* `search.host`
* `search.protocol` - If you are using Elasticsearch service through AWS or Google, this will be `https`
* `search.port` - If you are using Elasticsearch service through AWS or Google, this will be `443`
* `search.user`
* `search.password`
...
Finally, it's time to verify the deployment via the browser. As instructed by the success message output above, create a proxy enabling you to connect to the cluster from `localhost`:
...
metadata:
  ...
  labels:
    ...
    helm.sh/chart: fusionauth-0.0.0_dev
...
A new service will be available of type `LoadBalancer` and will contain an external IP or host name corresponding to a newly provisioned Elastic Load Balancer host name. External traffic will now be directed to the EKS cluster via this ingress.
