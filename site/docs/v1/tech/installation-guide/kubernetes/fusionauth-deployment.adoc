...
If you have not setup a Postgres user for FusionAuth you may additionally need to use the root user credentials for these so FusionAuth can configure the database.
...
Additionally, you may set these values if needed to access Elasticsearch but are not required for this tutorial:
