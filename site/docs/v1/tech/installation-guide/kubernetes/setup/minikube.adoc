...
The last thing we need is the link:https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/[Ingress Controller]. For that, we will use the NGINX Ingress controller.
...
Use the minikube tunnel to use the newly created Ingress controller service:
