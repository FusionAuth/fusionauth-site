...
Create a resource group if you have not already done so.
...
Next you will need to create a virtual machine to run Elasticsearch on. You will want to supply your public SSH key using the `--ssh-key-values` argument so that you can access the VM once it has been provisioned.
...
[source,shell,title=Output of creation of an Azure VM]
----
{
  "fqdns": "",
