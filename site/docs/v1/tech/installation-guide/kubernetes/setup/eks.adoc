...
This is where `kubectl` comes in handy. Looking at the previous log, you will notice that one of the last things `etsctl` did was update our `~/.kube` file with our new cluster configuration. We can now go ahead and use `kubectl` to make requests to the Kubernetes API Server.
...
We are done! To confirm the database has been created, we can simply ask AWS using the `db-instance-identifier` we used on the creation step.
