...
Next, install the FusionAuth services, you can optionally skip the `fusionauth-search` service if you would like to use the database as the User search engine, or if you already have a compatible Elasticsearch service running. See the link:./system-requirements[System Requirements] and link:/docs/v1/tech/reference/configuration[Configuration Reference].
