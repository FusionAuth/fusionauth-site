...
If you have memory lying around, feel free to throw more at it - but in a multi-node configuration `512M` to `1GB` assigned per node should be very adequate. However, note that this is the requirement for FusionAuth alone. Other programs and the operating system take memory sa well. If you are going to run another application or a local database, that will impact FusionAuth, so scale your memory appropriately.
