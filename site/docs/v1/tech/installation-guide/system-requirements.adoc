...
[WARNING]
====
When using Advanced Threat Detection, it is recommended you allow for adequate memory headspace on your deployment.  If you FusionAuth application is taking 3GB of an in-memory heap, then you will need to allocate at least (possibly more) for the host.
====
