...
All FusionAuth nodes in a multi-node cluster must use the same runtime mode. This means that changing runtime mode of an existing deployment requires stopping all running nodes before starting new nodes with a different runtime mode. Plan for downtime during this process.
