...
Kickstart runs a set of API calls, just as if you were running them. You may want to know when Kickstart completes, especially if you are running in a CI/CD environment and want to wait until it is finished to begin your tests.

At present, the best option is to add a no-op "marker" kickstart step, such as setting the `data.kickstartComplete` field of an application to a known value, at the end of your Kickstart file. Then you can poll for that marker and begin your tests when it is set.

There's an https://github.com/FusionAuth/fusionauth-issues/issues/1178[open issue] to add a webhook to make this process simpler.
