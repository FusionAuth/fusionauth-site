...
FusionAuth enters Maintenance Mode any time the database is not accessible or is not properly set up. In this case, FusionAuth was able to connect, but the system was not configured. Here's how the Maintenance Mode screen should look:

image::5-minute-setup-guide/maintenance-mode-database.png[Maintenance Mode Database,width=1200]

Here you need to provide the super user credentials for the database along with the name of the host of the database is running on if it isn't running locally. You can also select either MySQL or PostgreSQL and change the database port if needed. For this example, you will be using PostgreSQL on the standard port with a super username of `postgres` and a password of `password`. We can also change the username and password that will be created as the primary database account that FusionAuth will access. This is the `fusionauth` user above, but can be set to any username and password you desire.
