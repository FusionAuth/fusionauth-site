...
In order to complete this process, you'll be using the FusionAuth typescript client library along with Express and Pug templates. This https://github.com/FusionAuth/fusionauth-node-example[code is available] on the FusionAuth GitHub.
...
TODO stopped here
Install these libraries running `npm install` after creating the above `package.json` file.
...
TODO need to talk to Brian about recommendation: cookies?
