...
The unique id of the SCIM resource on the SCIM provisioning server. This should be populated from the User.id property by the outgoing response lambda in order for the SCIM client to pass the correct value to the FusionAuth SCIM endpoint when querying a User by id.
