...
[source,json]
.Example JSON for the `query` parameter to search for entities with two specific permissions
----
include::../../../src/json/entities/permission-query.json[]
----

[source,json]
.Example JSON searching by `query` for for entities with two specific permissions
----
include::../../../src/json/entities/search-permission-query-request.json[]
----
