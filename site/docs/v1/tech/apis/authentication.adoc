...
* icon:shield-alt[type=fas] &nbsp;<<Basic Authentication With an API Key>>
...
Basic authentication with an API key is used by a select few FusionAuth APIs. These are typically integrated with other software packages which expect to authenticate using basic auth.
