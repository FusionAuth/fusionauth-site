...
When an API is marked with a cogs icon such as &nbsp;icon:cogs[title="Supports Client Credentials",type=fas]&nbsp;, the authorization becomes a two step process. You must:
...
* Make a request of the API with the JWT in the `Authorization` header.
