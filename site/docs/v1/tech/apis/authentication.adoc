...
When an API is marked with a cogs icon such as &nbsp;icon:cogs[title="Supports Client Credentials",type=fas]&nbsp;, it means you are required to provide a JSON Web Token (JWT) with the `client_credentials` grant type, with clients being represented by FusionAuth Entities.

Here is an example link:/docs/v1/tech/oauth/#example-client-credentials-grant[client credentials grant using Entities].

After retrieving the JWT using the client credentials grant, put it in the `Authorization` header with a prefix of `Bearer`:
