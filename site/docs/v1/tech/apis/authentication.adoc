...
Prior to version 1.26.0, FusionAuth admin user interface was the only way to create API keys, this functionality was not available through an API.

Starting from version 1.26.0 API keys can be created using API key API. Please refer to the link:/docs/v1/tech/apis/api-keys#[API Keys API].
...
[field]#Key Manager#::
Toggle to set this key is a key manager. Setting this flag to true allows this key to be used to call link:/docs/v1/tech/apis/api-keys#[API keys APIs]. An attempt to call API Keys APIs with a non-manager key (`keyManager` set to `false`) will always return a HTTP response status code 401.
