...
* icon:key[type=fas] &nbsp;<<Basic Authentication>>
...
When an API is marked with a red locked icon such as &nbsp;&nbsp;icon:lock[type=fas] it means you are required to provide an API key. 

To enable access to a secured API create one or more API keys. The API key is then supplied in the HTTP request using the Authorization header. See <<Manage API Keys>> for more information on adding additional keys.
...
The following is a cURL example using the Authorization header using the above API key to retrieve a user. The line breaks and spaces are for for readability.
[source,shell]
----
...
When an api is marked with a key, such as &nbsp;icon:key[title="Supports Basic Auth",type=fas]&nbsp; it means you call this API and authenticate using HTTP basic authentication. HTTP basic authentication is a simple, standards based, authentication method. A username and password are supplied, separated by a `:`. It must be prefaced by the string `Basic` and a space. The `username:password` string is base64 encoded.

In FusionAuth, the username must be the string `apikey` (all lowercase). The password may be any API key with the appropriate permission for the endpoint being called.
...
[field]#Tenant#::
The optional tenant to which this API key will be assigned. This value cannot be changed once the API key is created. This field is only displayed when more than one tenant exists in the FusionAuth instance. 
+
...
[field]#Key Manager#::
Enable to have this key be a key manager. When a key is a key manager, it can be used to call the link:/docs/v1/tech/apis/api-keys#[API keys APIs]. Any attempt to call the API Keys API with a non-manager key (`keyManager` set to `false`) will return a HTTP response status code `401`.
