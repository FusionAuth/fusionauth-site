...
[.example]
--
The following example demonstrates the HTTP Authorization header using the **JWT** schema. Beginning in version `1.12.0`, you may utilize the `JWT` or `Bearer` schema.
[source,properties]
----
...
[.example]
--
The following is a cURL example using the HTTP Authorization header using the **Bearer** schema with a line break and spaces for readability. Prior to version `1.12.0` you may only utilize the **JWT** schema.
[source,shell]
----
