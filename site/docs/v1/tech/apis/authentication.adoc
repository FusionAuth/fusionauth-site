...
* icon:cogs[type=fas] &nbsp;<<Client Credentials>>
...
When an API is marked with a cogs icon such as &nbsp;icon:cogs[title="Supports Client Credentials",type=fas]&nbsp;, you are required to provide a JSON Web Token (JWT) in the `Authorization` header.

This JWT is acquired by using the `client_credentials` grant.

The requesting and target entities, as well as their permissions, are all managed using link:/docs/v1/tech/core-concepts/entity-management[Entities].
...
Here's another example, using a curl script, which gets a token:
...
After retrieving the JWT using the client credentials grant, place it in the `Authorization` header with a prefix of `Bearer` and make your requests of the protected endpoint:
