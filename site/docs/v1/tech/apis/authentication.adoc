...
* icon:shield[type=fas] &nbsp;<<Basic Authentication>>
...
When an api is marked with a key such as &nbsp;icon:shield[title="Supports Basic Auth",type=fas]&nbsp; it means you call this API and authenticate using HTTP basic authentication. HTTP basic authentication is a simple, standards based, authentication method. A username and password are supplied, separated by a `:`. It must be prefaced by the string `Basic` and a space. The `username:password` string is base64 encoded.
