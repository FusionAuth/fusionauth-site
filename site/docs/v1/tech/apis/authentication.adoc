...
HTTP basic authentication is a simple mechanism to request authentication information from a client.

Basic auth requires two pieces of information, username and password. In FusionAuth, to authenticate using `Basic Auth` method, username must be `apikey` (all lowercase) and the password could be one of the available API keys.
