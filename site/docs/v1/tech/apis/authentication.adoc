...
== Client Credentials
When an API is marked with a cogs icon such as &nbsp;icon:cogs[title="Supports JWT",type=fas]&nbsp;, it means you are required to provide a JSON Web Token (JWT) with the `client_credentials` grant type.
