...
[NOTE.note]
====
The API key being copied must not be set as a key manager, if it is, this API will return an error. The resulting new key will have the same permissions as the source key.
====
