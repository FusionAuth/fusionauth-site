...
* link:#create_key_with_id[Create a API Key with a randomly generated Id]
* link:#create_key_generate_id[Create a API Key with the provided unique Id]
...
[NOTE.note]
====
The API key being copied must not be set as a key manager, if it is, this API will return an error. Also, the resulting new key will have the same permissions as the source key.
====
...
include::docs/v1/tech/apis/_apikey-create-copy-request-body.adoc[]
