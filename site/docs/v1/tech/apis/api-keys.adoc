...
This API is used to generate a new API key based on an existing API Key. The API key that is being copied must not be set an key manager, if it is, this API will return an error.

[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Generate a new Key from an Existing Key
[.endpoint]
.URI
--
