...
+
*Note:*
The value will be used exactly as provided to resolve a user by [field]#email# or the [field]#username#. If the provided value contains leading or trailing white space these characters will not be removed, and then would only match a user by [field]#email# or the [field]#username# if the values stored in FusionAuth also contain these same leading or trailing spaces.
+
