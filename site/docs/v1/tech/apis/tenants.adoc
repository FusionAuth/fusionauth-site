...
*Note:* Requests to this API must not include an `X-FusionAuth-TenantId` header, and must be authenticated by an API key which is not scoped to a specific Tenant.

=== Request

[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Search for Tenants by name
[.endpoint]
.URI
--
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Search for Tenants using the given search and sort criteria
[.endpoint]
.URI
--
