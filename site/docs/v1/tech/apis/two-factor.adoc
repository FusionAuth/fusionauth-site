---
layout: doc
title: Two Factor/Multi-Factor APIs
...
If using message based delivery, you may <<Send a Two Factor Code When Enabling MFA>> to deliver a
...
Two Factor verification code or recovery code.

If using message based delivery, you may <<Send a Two Factor Code When Disabling MFA>> to deliver a
code to the User. The User will then provide this code as input.
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Disable Two Factor Authentication With a UserId
[.endpoint]
.URI
--
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#jwt-authentication[icon:id-badge[type=fas]] Disable Two Factor Authentication Using a JWT
[.endpoint]
.URI
--
...
A Base32 encoded form of the provided secret. This is useful if you need to provide a QR code to the User to enable Two Factor authentication.
...
This operation allows you to send a message with a code to finish a Two Factor flow, and requires an existing `twoFactorId`. No API key is required. 
