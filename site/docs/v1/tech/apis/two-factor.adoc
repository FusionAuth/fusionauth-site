...
Google Authenticator and other time based one-time password algorithms are not premium features and are included in the community edition.

While there is underlying infrastructure for different code lengths, algorithms and time steps, at the current time the system only allows for the following TOTP settings. 
