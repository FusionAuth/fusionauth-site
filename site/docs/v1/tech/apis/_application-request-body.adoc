...
[field]#application.samlv2Configuration.assertionEncryptionConfiguration.keyTransportEncryptionKeyId# [type]#[UUID]# [required]#Required# [since]#Available since 1.47.0#::
...
+
This value is only used when the [field]#application.samlv2Configuration.assertionEncryptionConfiguration.keyTransportAlgorithm# is set to `RSA_OAEP`. `RSAv15` does not required a message digest function, and `RSA_OAEP_MGF1P` will always use `MGF1_SHA1` regardless of this value.
