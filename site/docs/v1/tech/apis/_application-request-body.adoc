...
You may now use URIs that do not begin with `http` to support native application redirect. Prior to this version the value will be validated to begin with `http`.
...
|[field]#application.oauthConfiguration.logoutBehavior# [type]#[String]# [optional]#Optional# [default]#Defaults to `logout_of_all_registered`# [since]#Available Since 1.11.0#
|Behavior when `oauth2/logout` is called.

Valid values:

* `redirect_only`
** end SSO session and redirect to the registered logoutURL or the passed in `post_logout_redirect_uri`
* `logout_of_all_registered`
** end SSO session and call either all logoutURLs for every application in the tenant, or all applications the user is registered for. Configurable in the themed logout page.
