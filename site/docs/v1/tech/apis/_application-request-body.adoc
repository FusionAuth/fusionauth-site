...
+
You may now use URLs that do not begin with `http` to support native application origins. Prior to this version the value will be validated to begin with `http`.
+
[since]#Available since 1.32.0#
+
You may now use URLs for authorized origins that use a single slash to denote there is no naming authority for the scheme. Prior to this version a URL such as `com.myApp:/example` would fail validation as in invalid URL.
