...
[field]#{base_field_name}.jwtConfiguration.refreshTokenExpirationPolicy# [type]#[String]# [optional]#Optional# [default]#defaults to `Fixed`#::
...
+
Required if [field]#{base_field_name}.jwtConfiguration.refreshTokenExpirationPolicy# is set to `SlidingWindowWithMaximumLifetime`.
