...
The unique Id of the form to use for Add and Edit User Registration in the FusionAuth admin UI.
+
...
[field]#application.formConfiguration.requireCurrentPasswordOnPasswordChange# [type]#[Boolean]# [optional]#Optional# [default]#Defaults to `false`# [since]#Available since 1.45.0#::
When `true`, a user is required to present their current password when changing their password on the self-service account form.
+
This setting has no effect when a self-service account form has not been set on the Application.
