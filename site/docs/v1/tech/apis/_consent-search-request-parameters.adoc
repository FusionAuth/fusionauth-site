...
The string to search for in the Consent name. This can contain wildcards using an asterisk (`*`). This search parameter is case-insensitive.
...
+
 * `id` - the UUID of the Consent
 * `insertInstant` - the link:/docs/v1/tech/reference/data-types#instants[instant] when the Consent was created
...
include::../../../src/json/consents/search-post-request.json[]
endif::[]
