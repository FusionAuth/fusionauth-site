...
When this option is set, FusionAuth will expect the NameID returned will not change and is unique. FusionAuth will default to this format if not specified.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress`
When this option is set, FusionAuth will expect an email address is returned for the `nameIdFormat`. When setting this option, ensure email addresses within this identity provider are globally unique and unchanging. Alternatively, please specify a unique id FusionAuth can link with from the external IdP via the `identityProvider.uniqueIdClaim` field.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:custom`
With this option set, you can define the `nameIdFormat` as you wish.
...
[field]#identityProvider.useNameIdForEmail# [type]#[String]# [optional]#Optional#::
The name of the claim in the SAML response that includes the username claim.
