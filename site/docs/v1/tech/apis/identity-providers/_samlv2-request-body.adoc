...
When enabled and HTTP-Redirect binding are used, FusionAuth will provide the username or email address when available to the IdP as a login hint using the configured parameter name set by the [field]#identityProvider.loginHintConfiguration.parameterName# to initiate the AuthN request.
endif::[]
