...
FusionAuth will expect an email address returned for the `NameId`. When setting this option, ensure email addresses within the external identity provider are globally unique and unchanging. Failing this requirement, please reconcile a unique id from the external IdP via the `identityProvider.uniqueIdClaim` field.
+
