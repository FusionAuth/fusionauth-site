...
[field]#identityProvider.loginHintConfiguration.enabled# [type]#[boolean]# [optional]#Optional#::
When enabled, at the onset of the AuthN request (using redirect bindings), FusionAuth will attempt to pass a custom specified login hint to the SAML IdP.

[field]#identityProvider.loginHintConfiguration.parameterName# [type]#[String]#::
When using a login hint, at the onset of the AuthN request (using redirect bindings), the name of the login hint parameter to be provided to the SAML IdP.
...
The `nameid-format` of this SAML v2 identity provider.
+
Possible values are:
+
- `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent` -
FusionAuth will expect the `NameId` returned will not change and is unique. When not specified, this is the default value.
+
...
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:custom` -
Allows customization of the `nameIdFormat` to be any value necessary. For instance, using this option, it is possible to set a nameIdFormat to `foo` per business requirements.
endif::[]
...
FusionAuth will use this claim as a unique and unchanging value from the external IdP when linking a user.
...
The name of the claim in the SAML response that includes the username.
