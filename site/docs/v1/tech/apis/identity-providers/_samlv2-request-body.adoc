...
[field]#identityProvider.loginHintConfiguration.enabled# [type]#[boolean]# [optional]#Optional# [default]#Defaults to `false`# [since]#Available since 1.43.0#::
When enabled and HTTP-Redirect binding are used, a login hint will be passed (when available) as a query parameter to the IdP using the configured parameter name set by the [field]#identityProvider.loginHintConfiguration.parameterName#.

[field]#identityProvider.loginHintConfiguration.parameterName# [type]#[String]#::
The name of the query parameter to use when sending a login hint to the IdP. This is only used when a login hint is available and [field]#identityProvider.loginHintConfiguration.enabled# is set to `true`.
...
ifndef::samlv2_idp_initiated[]
[field]#identityProvider.nameIdFormat# [type]#[String]# [required]#Required# [default]#Defaults to `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent`#::
The *NAME-ID*entifier property is used to facilitate communication about a user between a Service Provider (SP) and Identity Provider (IdP). The SP can specify the preferred format in the AuthN request regarding a user. The identity Provider will attempt to honor this format in the AuthN response.
+
- `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent` -
FusionAuth will expect the user id returned will not change and is unique.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress` -
FusionAuth will expect an email address returned for the `NameId`. When setting this option, ensure email addresses within the external identity provider are globally unique and unchanging. If the external system does not have unique email addresses, identify a unique Id from the external IdP using the the `identityProvider.uniqueIdClaim` field.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:other` -
Allows customization of the `nameIdFormat` to be any `NameId` format value.
endif::[]
...
The name of the unique `userId` claim (attribute in the `Assertion` element) in the SAML response that FusionAuth uses to uniquely link the user. If this is not set, the `emailClaim` will be used.
...
The name of the claim in the SAML response (attribute in the `Assertion` element) in the SAML response that FusionAuth uses to identity the username. If this is not set, the `NameId` value will be used to link a user. This value is required if linking on username.
