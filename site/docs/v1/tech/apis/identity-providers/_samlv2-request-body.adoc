...
The name of the email claim in the SAML response that FusionAuth uses to uniquely identity the user. If this is not set, the `useNameIdForEmail` flag must be true.
...
[field]#identityProvider.loginHintConfiguration.parameterName# [type]#[String] #Defaults to `login_hint`# [since]#Available since 1.43.0# ::
...
+
When no format is specified, this value defaults to `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent`
+
- `urn:oasis:names:tc:SAML:2.0:nameid-format:persistent` -
FusionAuth will expect the user id returned will not change and is unique.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress` -
FusionAuth will expect an email address returned for the `NameId`. When setting this option, ensure email addresses within the external identity provider are globally unique and unchanging. If the external system does not have unique email addresses, identify a unique Id from the external IdP using the `identityProvider.uniqueIdClaim` field.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:*` -
Allows customization of the `nameIdFormat` to be any `NameId` format value.
Common values might include: `unspecified`, `format:X509SubjectName`, and `format:WindowsDomainQualifiedName`.
endif::[]
