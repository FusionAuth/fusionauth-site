...
* link:/docs/v1/tech/apis/identity-providers/samlv2-idp-initiated/[SAML v2 IdP Initiated]
...
Links between an Identity Providers and a FusionAuth User are configured in the `linkingStrategy` for each Identity Provider.  To learn more about how `linkingStrategy` works click into any of the Identity Provider API docs listed above.  For working with Identity Provider Links directly there is a separate Links API which is used to create a link between a FusionAuth user and a user in a 3rd party identity provider.  To learn more about working with the Links API, go here link:/docs/v1/tech/apis/identity-providers/links/[Link APIs].
