...
This is a special type of identity provider that is only used via the link:/docs/v1/tech/apis/jwt#reconcile-a-jwt[JWT Reconcile] API. This identity provider defines the claims inside the incoming JWT and how they map to fields in the FusionAuth User object.
