...
ifeval::["{idp_type}" == "Google"]
[field]#identityProvider.applicationConfiguration``[applicationId]``.enableOneTap# [type]#[String]# [optional]#Optional# [default]#defaults to **[see description]**# [since]#Available since 1.44.0#::
Determines if Google One Tap is enabled for the Application specified by the `applicationId` key.
+
The possible values are:
+
 * `Enabled` - Google One Tap is enabled for this application
 * `Disabled` - Google One Tap is disabled for this application
+
By default, the Application will inherit the setting from [field]#identityProvider.enableOneTap#.
endif::[]
...
ifeval::["{idp_type}" == "Google"]
[field]#identityProvider.enableOneTap# [type]#[Boolean]# [optional]#Optional# [default]#defaults to `false`# [since]#Available since 1.44.0#::
