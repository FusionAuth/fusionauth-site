...
+
[since]#Since 1.44.0#
The [field]#identityProvider.applicationConfiguration``[applicationId]``.buttonText# value will not be used when [field]#identityProvider.loginMethod# is set to `UsePopup` because of changes to the Google JavaScript API.
endif::[]
...
ifeval::["{idp_type}" == "Google"]
[field]#identityProvider.applicationConfiguration``[applicationId]``.oneTap# [type]#[Object]# [optional]#Optional# [since]#Available since 1.44.0#::
The policy that governs Google One Tap availability for the Application specified by the `applicationId` key. When this parameter is omitted, the Application will use the value of [field]#identityProvider.oneTap.enabled# to determine One Tap availability.

[field]#identityProvider.applicationConfiguration``[applicationId]``.oneTap.enabled# [type]#[Boolean]# [optional]#Optional# [since]#Available since 1.44.0#::
When [field]#identityProvider.applicationConfiguration``[applicationId]``.oneTap# has been specified, the Application specified by the `applicationId` key will use this value to determine Google One Tap availability.
endif::[]
...
+
[since]#Since 1.44.0#
The [field]#identityProvider.applicationConfiguration``[applicationId]``.scope# value will not be used when [field]#identityProvider.loginMethod# is set to `UsePopup` because of changes to the Google JavaScript API. The `email`, `profile`, and `openid` scopes will always be requested.
endif::[]
...
+
[since]#Since 1.44.0#
The [field]#identityProvider.buttonText# value will not be used when [field]#identityProvider.loginMethod# is set to `UsePopup` because of changes to the Google JavaScript API.
endif::[]
...
ifeval::["{idp_type}" == "Google"]
[field]#identityProvider.oneTap.enabled# [type]#[Boolean]# [optional]#Optional# [default]#defaults to `false`# [since]#Available since 1.44.0#::
Determines if Google One Tap is enabled.
endif::[]
...
+
[since]#Since 1.44.0#
The [field]#identityProvider.scope# value will not be used when [field]#identityProvider.loginMethod# is set to `UsePopup` because of changes to the Google JavaScript API. The `email`, `profile`, and `openid` scopes will always be requested.
endif::[]
