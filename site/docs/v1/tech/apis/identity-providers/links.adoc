...
+
After the link is created, and a user first successfully logs in with the IdP they are linked to, this value will typically be set to either an email or username.
...
----
include::docs/src/json/identity-providers/links/create-link-request-with-identityProviderLink-in-body-deprecated.json[]
----
...
--
[.api]
[.deprecated]#Deprecated# [since]#Since 1.43.0#::
While this endpoint is still functional, usage of the above Link a User endpoint that takes a top level `identityProviderLink` object is preferred.
...
[.api]
[.deprecated]#Deprecated# [since]#Since 1.43.0#::
While this endpoint is still functional, usage of the above Complete a pending Link endpoint that takes a top level `identityProviderLink` object is preferred.
