...
[.api]
[field]#identityProviderLink.displayName# [type]#[String]# [optional]#Optional# [since]#Available since 1.28.1#::
A human-readable name for this link for identification and convenience. This value will generally be an email address or username. This value can be set by the API caller. So for instance, this value could be `Initial piedpiper link from user migration`. This value will be updated during the next login for the linked user based on the identity provider (`richard@piedpiper.com`, for instance). This value may not necessarily reflect the username or email you used to authenticate with the 3rd party identity provider.
...
The Id for the User that is provided by the upstream identity provider. This is the value that will allow FusionAuth to link this user on future logins. It must be immutable.
...
----
include::docs/src/json/identity-providers/links/create-link-request-with-identityProviderLink-in-body.json[]
----
...
[WARNING.deprecated]
====
While this endpoint is still functional, its usage is discouraged. Please use the link:/docs/v1/tech/apis/identity-providers/links#link-a-user[Link a User endpoint] which expects an `identityProviderLink` object if your version of FusionAuth is 1.43.0 or above.
====
== Link a User
...
A human-readable name for this link when initially created. This value should be used to make it easier to identify the user this link represents in the remote identity provider.
+
After the link is created, and a user successfully logs in for the first time with the IdP, this value will typically be set to an email or username.
...
include::docs/v1/tech/apis/identity-providers/_links-post-response-body-deprecated.adoc[]

[WARNING.deprecated]
====
While the below endpoint is still functional, its usage is discouraged. Please use the link:/docs/v1/tech/apis/identity-providers/links#link-a-user[Link a User endpoint] which expects an `identityProviderLink` object if your version of FusionAuth is 1.43.0 or above.
====
== Complete a pending Link
...
.Example Request JSON
----
include::docs/src/json/identity-providers/links/pending-link-request-deprecated.json[]
----
=== Response
