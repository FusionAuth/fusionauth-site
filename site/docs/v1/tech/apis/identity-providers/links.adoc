...
layout: doc
title: Link APIs
description: The APIs for creating, retrieving, updating and deleting Identity Provider Links
---
...
The Id or unique identifier for the User that is provided by the identity provider.
...
This API is used confirm a pending Link. If an identity provider is configured with a linking strategy of `Create a pending link`, a pending link Id will be returned from the Login API that can be used to complete the link.
...
The response for this API contains either a single Link or collection of Links. When you call this API with [field]#identityProviderId#, [field]#identityProviderUserId#, and [field]#userId#, the response will contain a single Link. When you call this API with [field]#userId# and either [field]#identityProviderId# or [field]#identityProviderUserId#, the response will contain a collection of Links. All response types are defined below along with an example JSON response.
...
This API is used to remove a Link.
