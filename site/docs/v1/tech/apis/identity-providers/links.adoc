...
This API is used to retrieve a pending IdP Link. A pending IdP Link is created after a user completes login with an Identity Provider configured with a linking strategy of [field]#Create a Pending Link#. This pending IdP link is then used to link a user in a 3rd party identity provider to a user in FusionAuth.
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Retrieve a single pending link by Id
[.endpoint]
.URI
--
[method]#GET# [uri]#/api/identity-provider/link/pending/``\{pendingLinkId\}````?userId=\{userId\}``#
--
...
[.api]
[field]#pendingLinkId# [type]#[String]# [required]#Required#::
...
include::docs/v1/tech/apis/identity-providers/_pending-link-response-body.adoc[]
