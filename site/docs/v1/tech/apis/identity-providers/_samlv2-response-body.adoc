...
[field]#identityProvider.loginHintConfiguration.enabled# [type]#[boolean]# [optional]#Optional# [default] [since]#Available since 1.43.0#::
...
FusionAuth will expect the user id returned will not change and is unique. This is the default value if not specified on the request.
+
- `urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress` -
FusionAuth will expect an email address returned used as the Name Id. When setting this option, ensure email addresses within the external identity provider are globally unique and unchanging. If the external system does not have unique email addresses, a unique Id from the external IdP using the `identityProvider.uniqueIdClaim` field must be specified.
+
