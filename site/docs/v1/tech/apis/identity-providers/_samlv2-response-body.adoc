...
FusionAuth will expect an email address returned for the `NameId`. When setting this option, ensure email addresses within the external identity provider are globally unique and unchanging. If the external system does not have unique email addresses, a unique Id from the external IdP using the `identityProvider.uniqueIdClaim` field must be specified.
+
