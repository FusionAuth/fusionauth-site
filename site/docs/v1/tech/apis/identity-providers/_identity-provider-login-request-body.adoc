...
[field]#data.isNativeApp# [type]#[String]# [optional]#Optional# [since]#Availble since 1.43.0#::
Set this parameter to `"true"` for login from a mobile app to use the [field]#bundleId# from the IdP configuration instead of the [field]#servicesId#.
...
+
[since]#Since 1.43.0#
Beginning in version 1.43.0, [field]#data.redirect_uri# is no longer a required field for mobile app logins when [field]#data.isNativeApp# is `"true"`.
endif::[]
