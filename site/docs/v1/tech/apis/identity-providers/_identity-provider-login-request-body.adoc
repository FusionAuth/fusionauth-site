...
The Facebook `code` when not using `token` returned from their login API. This token will be sent to the Facebook Graph API as the `access_token` parameter. When using `code` you must also supply `redirect_uri`. If `code` is not provided, then `token` will be required.
...
The Facebook id token when not using `code` returned from their login API. This token will be sent to the Facebook Graph API as the `access_token` parameter.  When using `token` you do not have to supply `redirect_uri`. If `token` is not provided, then `code` will be required.
endif::[]
ifeval::["{idp_type}" == "Google"]
[field]#data.code# [type]#[String]# [optional]#Optional#::
The Google `code` when not using `token` returned from their login API. This token will be sent to the Google OAuth Token Info API as the `id_token` parameter.  When using `code` you must also supply `redirect_uri`. If `code` is not provided, then `token` will be required.
...
[field]#data.token# [type]#[String]# [optional]#Optional#::
The Google id token when not using `code` returned from their login API. This token will be sent to the Google OAuth Token Info API as the `id_token` parameter.  When using `token` you do not have to supply `redirect_uri`. If `token` is not provided, then `code` will be required.
endif::[]
ifeval::["{loginProvider}" == "HYPR"]
[field]#data.code# [type]#[String]# [required]#Required#::
