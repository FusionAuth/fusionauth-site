...
[field]#identityProvider.applicationConfiguration``[applicationId]``.oneTap# [type]#[Object]# [since]#Available since 1.44.0#::
The policy that governs Google One Tap availability for the Application specified by the `applicationId` key. When this key is absent, the Application will use the value of [field]#identityProvider.oneTap.enabled# to determine One Tap availability.

[field]#identityProvider.applicationConfiguration``[applicationId]``.oneTap.enabled# [type]#[Boolean]# [since]#Available since 1.44.0#::
When [field]#identityProvider.applicationConfiguration``[applicationId]``.oneTap# is present, the Application specified by the `applicationId` key will use this value to determine Google One Tap availability.
endif::[]
...
ifeval::["{idp_type}" == "Google"]
[field]#identityProvider.oneTap.enabled# [type]#[Boolean]# [since]#Available since 1.44.0#::
