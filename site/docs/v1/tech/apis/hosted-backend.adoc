...
Be sure to review the link:https://fusionauth.io/docs/v1/tech/core-concepts/applications#oauth[Applications] section of the FusionAuth user guide to ensure proper configuration before using the hosted endpoints.
...
The URI that the browser will be redirected to at the end of the login flow. If provided this url must be defined in the list of Authorized Redirect URLs for your application. If not provided the default will be the first of the Authorized Redirect URLs defined for your application.
...
The OAuth 2 `scope` parameter to be passed to the authorize endpoint. The format is a url-encoded, space-separated list of scopes (i.e `+` or `%20`).
+
Available scopes:
+
...
link:https://fusionauth.io/docs/v1/tech/guides/basic-registration-forms[Self-service Registration] will need to be enabled otherwise this endpoint will redirect to <<Login>>.
...
To use this API correctly the browser should be redirected to this route and it will respond with a `302` redirect status code if successful.
