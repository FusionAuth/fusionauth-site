...
[cols="1,1,1,1,1,1"]
|===
| Name | Content-Type | Array Behavior | Available Since | RFC Link | Client Library Support

|FusionAuth default
|`application/json`
|Appends new values to an array. Deletion requires `GET` then `PUT`.
|1.12.0
|N/A
|Yes
...
|If an array is provided, overwrite the existing array.
|1.39.0
...
With the default method, the correct way to remove the "dev" role is to request the group JSON, find the "ceo" role from the `roleIds` array, and use `PUT` to update the group object. 

[source,shell,title=Updating an object's array with the default PATCH behavior]
----
API_KEY=...
GROUP_ID=...
APPLICATION_ID=...
group_json=`curl -H 'Authorization: '$API_KEY \
     -X GET \
     https://local.fusionauth.io/api/group/$GROUP_ID`

# construct new object without "dev" role
ceoRoleId=`echo $group_json | jq '.group.roles."'$APPLICATION_ID'"[] | select(.name == "ceo")|.id'`
name=`echo $group_json | jq '.group.name'`
data=`echo $group_json | jq '.group.data'`
new_group_json='{"group": {"name" : '$name', "data":'$data'}, "roleIds": ['$ceoRoleId']}'
...
With JSON Patch, you have a flexible set of operations that can update, remove or move fields in a JSON object. Please review https://www.rfc-editor.org/rfc/rfc6902[RFC 6902] for the full list of operations.

[source,shell,title=Updating an object's array with JSON Patch]
----
API_KEY=...
GROUP_ID=...
ROLE_ID=...
...
curl -H 'Authorization: '$API_KEY \
     ...
     https://local.fusionauth.io/api/group/$GROUP_ID \
     -d '[{ "op": "add", "path": "/roleIds", "value": [ "'$ROLE_ID'" ] }]'
----

This call sets the value of `roleIds` with the array contained in the `value` field. 

This approach is precise and can make multiple changes to a given object with one call, but has a downside of requiring you to learn a new set of operations. Additionally, the data is sent in format (an array of operations) which is only vaguely related to the structure of the object being changed.

==== JSON Merge Patch

Merge Patch is a more straightforward way to update complex JSON objects. 

[source,shell,title=Updating an object's array with JSON Merge Patch]
----
API_KEY=...
GROUP_ID=...
ROLE_ID=...
