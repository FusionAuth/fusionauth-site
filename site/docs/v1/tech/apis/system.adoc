...
|460
|FusionAuth is using Elasticsearch and the search service is reporting an unhealthy cluster status. In a cluster with 2+ nodes, this mean the cluster status is being reported as `yellow` or `red`. In a single node Elasticsearch configuration this means the cluster status is `red.[.line-through]#`#
...
The Status API is used to retrieve the current version of FusionAuth.

=== Request

[.api-authentication]
link:/docs/v1/tech/apis/authentication#no-authentication-required[icon:lock[type=fas]] Return the system version with an API key
[.endpoint]
.URI
--
