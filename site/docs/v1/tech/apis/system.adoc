...
The Logger API is used to retrieve or update the logging level for a given log name in FusionAuth.

=== Request

[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Retrieve the logging level by log name
[.endpoint]
.URI
--
...
The name of the Log to retrieve the current level for.
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Update the logging level by log name
[.endpoint]
.URI
--
[method]#POST# [uri]#/api/logger``?name=\{name\}&level=\{level\}``#
--
...
The logging level to set. Possible values are:
+
* `error`,
* `warn`,
* `info`,
