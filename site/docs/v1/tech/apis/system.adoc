...
By default, this API requires authentication. If you prefer to allow unauthenticated access to this endpoint from local scrapers, you may set `fusionauth-app.local-metrics.enabled=true` as link:/docs/v1/tech/reference/configuration#[documented here].
...
`version` will not change and will be returned as below. As a reminder, unless explicitly allowed from `localhost`, an API key is required to obtain this value.
```javascript
{
//..
    "version": "1.26.1"
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#basic-authentication-using-an-api-key[icon:shield-alt[type=fas]] link:/docs/v1/tech/apis/authentication#api-key-authentication[icon:lock[type=fas]] Retrieve FusionAuth application metrics to use with Prometheus.
[.endpoint]
.URI
--
