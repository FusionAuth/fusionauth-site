...
The maximum lifetime of a refresh token when using a [field]#refreshTokenExpirationPolicy# of `SlidingWindowWithMaximumLifetime`. Value must be greater than 0.
+
When [field]#refreshTokenExpirationPolicy# is set to `SlidingWindowWithMaximumLifetime`, this value must be greater than or equal to [field]#refreshTokenTimeToLiveInMinutes#.
