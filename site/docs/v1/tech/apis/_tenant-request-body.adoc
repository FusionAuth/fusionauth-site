...
[field]#tenant.externalIdentifierConfiguration.emailVerificationOneTimeCodeGenerator# [type]#[Integer]# [optional]#Optional# [since]#Available since 1.27.0#::
A one time code used by the user to verify their email. This is used during a gated workflow. The values generated are `randomAlphaNumeric` and are 6 digits in length.
...
[field]#tenant.externalIdentifierConfiguration.registrationVerificationOneTimeCodeGenerator# [type]#[Integer]# [optional]#Optional# [since]#Available since 1.27.0#::
A one time code used by the user to verify their registration. This is used during a gated workflow. The values generated are `randomAlphaNumeric` and are 6 digits in length.
