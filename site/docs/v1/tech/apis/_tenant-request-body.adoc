...
+
ifdef::source_tenant_id_create_request[]
If present, the only two definable fields are [field]#id# and [field]#name# . All other fields are copied from the source Tenant.
endif::[]
ifdef::source_tenant_id_update_request[]
include::../shared/_source-on-patch-usage.adoc[]
endif::[]
