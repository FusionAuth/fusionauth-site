...
The value this tenant will use for the Relying Party Id in WebAuthn ceremonies. Passkeys can only be used to authenticate on sites with the same Relying Party Id they were registered. This value must match the browser origin or be a registrable domain suffix of the browser origin. For example, if your domain is `auth.example.com`, you could use `auth.example.com` or `example.com` but not `m.auth.example.com` or `com`.
+
When this parameter is omitted, FusionAuth will use `null` for the Relying Party Id in passkey creation and request options. A `null` value in the WebAuthn client API will use the browser origin.
+
:advanced_feature: WebAuthn
include::../shared/_advanced-edition-blurb-api.adoc[]
+
