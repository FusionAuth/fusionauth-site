...
The value this tenant will use for the Relying Party Id in WebAuthn ceremonies. Passkeys can only be used to authenticate on sites with the same Relying Party Id they were registered. This value must match the browser origin or be a registrable domain suffix of the browser origin. For example, if your domain is `auth.piedpiper.com`, you could use `auth.piedpiper.com` or `piedpiper.com` but not `m.auth.piedpiper.com` or `com`.
+
