...
:default_false: pass:normal[[default]#defaults to `false`#]
...
+
* `Enabled` - Require a two-factor challenge during login.
...
+
The recommended value for the bootstrap workflow is `crossPlatform` (when supported).
+
:advanced_feature: WebAuthn
include::../shared/_advanced-edition-blurb-api.adoc[]
+
:advanced_feature!:
+
:enterprise_feature: WebAuthn roaming authenticators
include::../shared/_enterprise-edition-blurb-api.adoc[]
+
:enterprise_feature!:
...
Whether or not this tenant has the WebAuthn bootstrap workflow enabled. The bootstrap workflow is used to perform a manual login using WebAuthn and can be used on a new device.
+
:advanced_feature: WebAuthn
include::../shared/_advanced-edition-blurb-api.adoc[]
+
...
Whether or not this tenant has the WebAuthn reauthentication workflow enabled. The reauthentication workflow will automatically prompt a user to authenticate using WebAuthn on repeat logins from the same device.
+
:advanced_feature: WebAuthn
include::../shared/_advanced-edition-blurb-api.adoc[]
+
...
The value this tenant will use for the Relying Party Id in WebAuthn ceremonies. Passkeys can only be used to authenticate on sites with the same Relying Party Id they were registered. This value must match your fully qualified domain name or be a registrable suffix of your fully qualified domain. For example, if your domain is `auth.example.com`, you could use `auth.example.com` or `example.com` but not `m.auth.example.com` or `com`.
+
When this parameter is omitted, FusionAuth will use `null` for the Relying Party Id in passkey creation and request options. A `null` value in the WebAuthn client API will use the current fully qualified domain.
+
:advanced_feature: WebAuthn
include::../shared/_advanced-edition-blurb-api.adoc[]
+
...
The value this tenant will use for the Relying Party name in WebAuthn ceremonies. This value is used only for display and may be shown by browser or OS dialogs during WebAuthn ceremonies.
+
When this parameter is omitted, FusionAuth will use the [field]#tenant.issuer# value in passkey creation options.
+
:advanced_feature: WebAuthn
include::../shared/_advanced-edition-blurb-api.adoc[]
+
