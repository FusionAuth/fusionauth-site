...
* `SlidingWindowWithMaximumLifetime` - the expiration is calculated in the same manner as `SlidingWindow`, however, the token will eventually expire once the maximum lifetime is reached. &nbsp; [since]#Available since 1.46.0#
...
The absolute maximum lifetime of a refresh token, when renewing a refresh token in a Sliding Window. When this value is reached, the refresh token will be considered expired. Value must be greater than 0.
+
Required if [field]#tenant.jwtConfiguration.refreshTokenExpirationPolicy# is set to `SlidingWindowWithMaximumLifetime`, this value must be greater than or equal to the Refresh Token duration.
