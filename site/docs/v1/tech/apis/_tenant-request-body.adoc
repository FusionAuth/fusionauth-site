...
* `SlidingWindowWithMaximumLifetime` - the expiration is calculated from the last time the token was used up until an upper time limit.
...
The maximum lifetime of a Refresh Token when being renewed in a sliding window. Value must be greater than 0.
+
