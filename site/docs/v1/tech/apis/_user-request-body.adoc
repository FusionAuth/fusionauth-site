...
+
If email is not provided, then the [field]#username# will be required. If FusionAuth needs to send emails to a user with no email, for example to reset a password, it will expect a valid email address in the [field]#user.data.email# field. 
