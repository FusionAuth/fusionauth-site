...
[field]#currentPassword# [type]#[String]# [optional]#Optional# [since]#Available since 1.45.0#::
If provided, this value is compared to the user's existing password and the request fails if they do not match. This is typically used when changing a user's password, and additional verification is required.
endif::[]
