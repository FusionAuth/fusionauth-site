...
When importing an MD5 password hash, you may or may not have a salt. If you have no salt, use the empty string, `''`, as the salt and you may still use the salted MD5 plugin.

MD5 is commonly stored in hexidecimal format, such as `25d55ad283aa400af464c76d713c07ad`. FusionAuth requires imported passwords to be stored in base64. You must convert any hexidecimal values to base64 before importing, or the import will not work.

Here is an example ruby script to convert hexidecimal values to base64 encoded:

[source,ruby]
.Convert a Hexidecimal Password Hash Value to Base64
----
...
+
[deprecated]#Using this field in the URL segment is deprecated as of version 1.27.0 and may be removed in the future. This API now takes this field in the request body.#
