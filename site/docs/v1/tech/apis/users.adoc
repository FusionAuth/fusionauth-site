...
Which search query parameters are available and how they behave depends on the search engine type. Read more about link:../core-concepts/users#user-search[the different types of search engines].

=== Database Search Engine

This is a good choice for link:../core-concepts/users#database-search-engine[smaller installs, embedded scenarios, or other places where the additional capability of Elasticsearch is not required].
...
The Elasticsearch engine has link:../core-concepts/users#elasticsearch-search-engine[advanced querying capabilities and better performance].
