...
This API is used to delete a User. You must specify the Id of the User on the URI. You can also specify whether or not the User is soft or hard deleted. 

Soft deleted Users are deactivated and marked as inactive but not deleted from FusionAuth. Deactivated users' data is retained but they are unable to authenticate. Users who have been deactivated can be reactivated: <<Reactivate a User>>

Hard deleted Users' data is removed from FusionAuth. The data from an account which has been hard deleted cannot be restored via the FusionAuth API or administrative user interface. If you need to restore such data, retrieve it from a database backup.
