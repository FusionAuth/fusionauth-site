...
The standard FusionAuth password hashing schemes require the password hash to be a base64 encoded string.
If your password hashes are encoded in a different format, they will need to either be converted and imported as base64, or you can create a custom plugin using an alternative encoding.
...
For the above bcrypt hash, `10` is the factor and should be placed in the [field]#factor# field.
`N9qo8uLOickgx2ZMRZoMye` is the salt and should be placed in the [field]#salt# field.
`IjZAgcfl7p92ldGxad68LJZdL17lhWy` is the hash value and should be placed in the [field]#password# field.
...
MD5 is commonly stored in hexadecimal format, such as `25d55ad283aa400af464c76d713c07ad`. FusionAuth requires imported passwords to be stored in base64. Convert any hexadecimal values to base64 before importing, or the import will not work.
...
The Elasticsearch engine has link:/docs/v1/tech/core-concepts/users#elasticsearch-search-engine[advanced querying capabilities and better performance].
