...
Soft deleted Users are marked as inactive but not deleted from FusionAuth. Deactivated users' data is retained but they are unable to authenticate. Users who have been deactivated can be reactivated; see <<Reactivate a User>> for more.

Hard deleted Users' data is permanently removed from FusionAuth. The User's data cannot be restored via the FusionAuth API or the administrative user interface. If you need to restore the data, retrieve it from a database backup.
