...
You must specify all of the properties of the {capitalized_object_name} when calling this API with the `PUT` HTTP method.
When used with `PUT`, this API doesn't merge the existing {capitalized_object_name} and your new data. It replaces the
existing {capitalized_object_name} with your new data.
