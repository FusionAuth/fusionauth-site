...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red,type=fas]] Retrieve an Event Log by Id
[.endpoint]
.URI
--
...
[.api-authentication]
link:authentication#api-key-authentication[icon:lock[role=red,type=fas]] Searches the Event Logs using the given search criteria
[.endpoint]
.URI
--
