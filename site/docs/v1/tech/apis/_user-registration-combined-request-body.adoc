...
Whether or not the User is sent an email asking them to setup their password. This is useful if someone else is creating an account for a User with an Application. The Email Template used is the [field]#Set Password Email Template# configured in the Application settings, or if that is not configured, the value in the Tenant settings.
+
If you set this value to `true` the [field]#password# field is ignored and FusionAuth will set the initial password to a securely generated random string.
+
If you have also enabled email verification and do not select to skip verification using the [field]#skipVerification# parameter, only the setup password email will be sent to the user. Setting up the password using the email sent during this user create operation will implicitly verify the User's email if it is not already verified.
+
...
[field]#user.twoFactor.recoveryCodes# [type]#[Array<String>]# [optional]#Optional#::
