...
+
[field]#password# should contain only the password hash as a base64 encoded string. Depending on how your password database stores hashes, additional processing may be required. Please see the <<Password Hashes>> section for more information about importing particular password hashes.
