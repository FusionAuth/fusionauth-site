...
The first 22 characters after the final `$` value is the salt and the last 31 characters are the hash.
+
[field]#password# should contain only the password hash.
For the above bcrypt hash, `N9qo8uLOickgx2ZMRZoMye` is the salt and should be placed in the [field]#salt# field.
