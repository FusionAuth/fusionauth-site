...
This API is used to update an existing Key. Only the name of the Key may be changed; all other fields will remain the same. If you need to create a key with a new certificate, algorithm or other attributes, please <<Import a Key>>.

If you import a new Key and use it to sign requests or in other parts of FusionAuth, you'll need to update wherever the key is referenced as well.
