...
[field]#{base_field_name}.formConfiguration.requireCurrentPasswordOnPasswordChange# [type]#[Boolean]# [since]#Available since 1.45.0#::
Whether or not a user is required to present their current password when changing their password on the self-service account form.
+
