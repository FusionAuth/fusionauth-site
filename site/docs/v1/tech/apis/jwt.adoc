...
An set of claims to add to this JWT.  If any of the "reserved" claims, `exp` or `iat`, are specified they will be ignored. Otherwise, the claims can be any valid JSON value.
...
The length of time in seconds before the JWT expires and no longer valid. Any integer value greater than `0` is allowed. If omitted, the tenant's [field]#timeToLiveInSeconds# value will be used instead.
