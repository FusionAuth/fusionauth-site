...
[.api]
[field]#refreshToken# [type]#[String]#::
The refresh token. The value will depend on the `tenant.jwtConfiguration.refreshTokenUsagePolicy`. 
...
[field]#refresh_token# [type]#[String]#::
The encoded refresh token. This cookie is written in the response as an `HTTPOnly` cookie. The value will depend on the `tenant.jwtConfiguration.refreshTokenUsagePolicy`. 
