...
// [brettp]Note: This says "Issue an Access Token" above it say "Issue a JWT", should we be consistent?
[.api-authentication]
...
This API is used to retrieve Public Keys generated by FusionAuth, used to cryptographically verify JWT signatures signed using the corresponding RSA or ECDSA private key.
...
This API is used to create a free-form access token (JWT) with claims defined by the caller. The only "reserved" claims that cannot be specified are `exp`, `iat`. The `iat` the issued at time of the JWT and the `exp` the expiration time of the JWT as computed by adding either the user passed, or the tenant [field]#timeToLiveInSeconds# to the `iat`. If passed into the [field]#claims# object these claims will be ignored.
...
An object of claims to apply to (associate with?) this JWT.  If any of the three "reserved" claims `exp`, `iat`, or `jti` are specified they will be ignored.

[field]#keyId# [type]#[UUID]# [optional]#Optional#::
The Id of the signing key to use when signing this JWT. If this is not supplied the signing key on the tenant will be used.

[field]#timeToLiveInSeconds# [type]#[Integer]# [optional]#Optional#::
The length of time in seconds the JWT will live before it is expired and no longer valid. Value must be greater than `0`. If omitted, the [field]#timeToLiveInSeconds# value on the tenant will be used instead.
