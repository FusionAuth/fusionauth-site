...
== Reconcile a JWT
The Reconcile API is used to take a JWT issued by a third party identity provider as described by an link:identity-providers/overview[Identity Provider]
...
[.api-authentication]
link:authentication#no-authentication-required[icon:unlock-alt[role=green]] Retrieve a single Public Key for a specific Application by Application Id
[.endpoint]
.URI
--
...
[.api-authentication]
link:authentication#no-authentication-required[icon:unlock-alt[role=green]] Retrieve a single Public Key by Key Identifier
[.endpoint]
.URI
--
