...
Indicates the tenant has been configured.  It is always `true` except for under the default tenant which is false until the setup wizard has been completed.
...
[field]#{base_field_name}.emailConfiguration.unverified.allowEmailChangeWhenGated# [type]#[Boolean]# [since]#Available since 1.27.0#::
...
[field]#{base_field_name}.emailConfiguration.unverified.behavior# [type]#[String]# [since]#Available since 1.27.0#::
...
* `Gated` - verification is required before a user can complete login.

[field]#{base_field_name}.emailConfiguration.username# [type]#[String]# [since]#Available since 1.8.0#::
An optional username FusionAuth will use to authenticate with the SMTP server.
...
[field]#{base_field_name}.emailConfiguration.verificationStrategy# [type]#[String]# [since]#Available since 1.27.0#::
...
* `FormField` - send the user a short code intended to be manually entered into a form. This is only available when [field]#tenant.emailConfiguration.unverified.behavior# has the `Gated` value.

[field]#{base_field_name}.emailConfiguration.verifyEmail# [type]#[Boolean]#::
Whether the user's email addresses are verified when the user registers with your application.
