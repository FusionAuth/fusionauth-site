...
An IP ACL (Access Control List) is a list of IP ranges that are either allowed or blocked. Along with one `*` or everything range that defines the default behavior. An IP ACL may be assigned to an API Key, a Tenant, or an Application. When an IP ACL is assigned at the application level this is the only one that will be used and the tenant IP ACL will not be checked.
...
include::docs/v1/tech/apis/_ip-acl-search-response-body.adoc[]
...
This API is used to permanently delete an IP ACL. Deleting an IP ACL will remove it from any tenants and/or applications it was assigned.
