...
An IP ACL (Access Control List) is a list of IP ranges that are either allowed or blocked. Along with one entry that defines a start IP address of `*` (wild) that defines the default behavior when an IP address does not match any other range in the list. An IP ACL may be assigned to an API Key, a Tenant, or an Application. When an IP ACL is assigned at the application level this configuration will take precedence over the tenant if configured.  The IP address start and end entries for ranges currently only support IPv4.
