...
[source,json]
.Example JSON for the [field]#query# parameter to search for users with unverified registrations to a specific application
----
...
[source,json]
.Example JSON for the [field]#query# parameter to search for users with a specific role
----
include::../../../src/json/users/role-query.json[]
----

[source,json]
.Example JSON searching by [field]#query# for users with with a specific role
----
