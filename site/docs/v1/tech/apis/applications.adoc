...
.URI
--
[method]#GET# [uri]#/api/application/search``?name=\{name\}&active=\{active\}&tenantId=\{tenantId\}&orderBy={orderBy}&startRow={startRow}``#
--

==== Request Parameters

[field]#name# [type]#[String]# [optional]#Optional#::
The string to search for in the Application name. This can contain wildcards using the asterisk or percent characters (* or %). This search parameter is case-insensitive.

[field]#active# [type]#[Boolean]# [optional]#Optional#::
Filters on the Application's `active` status. Can be omitted or set to `null` to retrieve both active and inactive Applications.
...
+
For example, to order the results by the insert instant in a descending order, the value would be provided as `insertInstant DESC`. The final string is optional can be set to `ASC` or `DESC`. If omitted, the default sort order is `ASC`.
...
Filters Applications to those belonging to the specified Tenant. This parameter will be overridden if the request contains an `X-FusionAuth-TenantId` header, or if the supplied API key is scoped to a specific Tenant.
