...
The response for this API contains the WebAuthn passkey creation options that are passed to the WebAuthn API on the client. A few fields on the API response are returned as a base64-encoded string and need to be converted to an `ArrayBuffer` in JavaScript before being passed to the WebAuthn API.
