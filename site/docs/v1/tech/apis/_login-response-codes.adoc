...
|242
|The user was authenticated successfully. The user has two factor authentication enabled. Since version `1.42.0`, this status code is returned when two factor authentication is required. The response will contain the `twoFactorId` to be used
