...
The additional headers to be added to each outgoing email. Each header consists of a name and a value.
...
When set to true, this allows email to be verified as a result of completing a similar based email workflow such as change password. When set to false, the user must explicitly complete the email verification workflow even if the user has already completed a similar email workflow such as change password.
endif::[]
