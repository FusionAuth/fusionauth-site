...
[field]#apiKey.sourceKeyId# [type]#[UUID]# [optional]#Optional#::
The optional Id of an existing API key to make a copy of. All values will be copied from the source API key to the new API key.
+
ifdef::apikey_create_request[]
If present, the only two definable fields are [field]#id# and [field]#key#. All other fields are copied from the source API key.
endif::[]
