...
=== New
* https://tools.ietf.org/html/rfc8628[Device Authorization Grant]
** New themed template `Device`. This template has been added to each of your existing themes. As part of your migration please review this template to ensure it matches your intended style.
** The following theme messages where added for the Device Authorization workflow. Until these values have been translated they will be rendered in English. At your earliest convenience you will want to add these new keys to your existing themes.
++++
<blockquote>
<pre class="pygments hightlight">
<code>
device-title=
device=
...
=== Enhancement
* `/oauth2/authorize` endpoint now supports the `response_mode` parameter.
** In addition to `fragment` and `query`, `response_mode` also supports `form_post`.
** Resolves https://github.com/FusionAuth/fusionauth-issues/issues/159[GitHub Issue #159], thanks to https://github.com/bertiehub[@bertiehub] for requesting!
