...
=== New
* Actions added to [breadcrumb]#Users# icon:chevron-right[role=breadcrumb] [breadcrumb]#Manage#
** `Send password reset`
** `Resend email verification`
** [breadcrumb]#Registrations# `Resend verification`
...
=== Enhancement
* `DELETE /api/user/` with `hardDelete=true` will disassociate any UserActions created by the deleted user by removing them from the Actioning User.
* `DELETE /api/user/bulk` takes a `queryString` parameter to search by ElasticSearch query for users to delete, and a `dryRun` parameter to preview the affected users.  See the link:../apis/users#bulk-delete-users[Bulk Delete Users API documentation].
* The following apis now support `PATCH`. Addresses https://github.com/FusionAuth/fusionauth-issues/issues/121[GitHub Issue #121]
** `/api/application`
...
=== New
* link:../oauth/endpoints#device-authorize[Device Authorization Grant]
...
** See the link:../apis/passwordless[Passwordless API documentation] for additional information.
...
** A new attribute `Logout behavior` was added to the [breadcrumb]#Application# icon:chevron-right[role=breadcrumb] [breadcrumb]#Oauth# configuration
...
* The link:../apis/passwordless#send-passwordless-login[Passwordless Send API] now takes an optional `code` parameter which will be used as the Passwordless code sent in the email.  This `code` can be generated by the new Passwordless Start API.
...
** The link:../oauth/endpoints#openid-configuration[OpenId Connect discovery endpoint] now returns the following attributes:
...
* The link:../oauth/endpoints#openid-configuration[OpenId Connect discovery endpoint] now accepts optional `tenantId` request parameter.
* A link:../apis/users[User] object is returned in the `jwt.refresh-token.revoke` event JSON.
...
* A new macro was added to the `_helpers.ftl` that may be managed by your theme. If you have modified the `_helpers.ftl` template as part of your theme, you will either need to reset that template and merge your changes back in, or add the following code to your `_helpers.ftl` managed by your theme. If you encounter an issue with this, you will still likely be able to login to correct the issue, if you do get stuck you may disable your theme to login. See https://fusionauth.io/docs/v1/tech/themes/overview#handling-failures.
...
* Full theme support for login. See the link:../themes/overview[Login Theme] tutorial for additional information and examples.
