...
Detailed documentation on the configuration can be found in the link:/docs/v1/tech/connectors/[Connectors documentation].
...
Users then sign in using the normal login process, but are authenticated against the appropriate Connector.
...
* The user data is never migrated to FusionAuth. The Identity Provider always remains the system of record.
* The user must choose an Identity Provider, unless you code the login pages and use the APIs. With the hosted login pages, there will be a button indicating that the user can "Login with Google", for example.
