...
Entity Management lets you build FusionAuth to your specific business needs.  All combined, it allows for powerful modeling of your organization's real life divisions and relationships within software.  No more hijacking the `user` type (increase functionality and usability) or writing your own homegrown functionality (save money and time) to achieve complex corporate structures or model a unique permissions hierarchy, for instance.  Entity Management works for you out of the box.

* Create any `Entity Type` representing your business use case. Maybe it's a `printer` Entity Type, or a `regional office`.
* Create any `Entity` which works for your business. Maybe it's `laser-jet-01` type, or a `west-regional-division`.
* Implement and manage permissions between Entities within FusionAuth.

image::entity-home-reactor.png[Entity Home,width=1200]
image::entity-types-home-reactor.png[Entity Types Home,width=1200]

Detailed documentation on the proper configuration can be found in the link:/docs/v1/tech/entity-management/[Entity Management Overview] and link:/docs/v1/tech/apis/entity-management/[Entity Management API] documentation.
++++
