...
Set to true to enable the HTTPS server. Requires a certificate chain and private key set as either filenames or content as described below.
...
The filename containing the certificate chain of the HTTPS server, in PEM format. Ensure that this file contains the server certificate and all intermediate certificates up to, but not including, the root CA certificate.

[field]#fusionauth-app.https.private-key-file# [type]#[String]# [since]#Available since 1.43.0#::
The filename containing the private key corresponding to the HTTPS server certificate, in PKCS#8 PEM format. Keys in this format are wrapped inside of a `-----BEGIN PRIVATE KEY-----` block.

[field]#fusionauth-app.https.certificate# [type]#[String]# [since]#Available since 1.43.0#::
The raw PEM content of the certificate chain of the HTTPS server, in PEM format. Ensure that this contains the server certificate and all intermediate certificates up to, but not including, the root CA certificate.
+
