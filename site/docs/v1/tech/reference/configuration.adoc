...
Set to `true` to allow unauthenticated access to `/api/prometheus/metrics` and `/api/status` from `localhost`. This can be used to to allow the scraping of health and metrics data from local collecter agents without the need to provision an API key.
