...
This event is generated when an access token is refreshed using a refresh token.
This event is initiated from both the link:../oauth/endpoints#refresh-token-grant-request[OAuth2 Refresh Token Grant], and the link:../apis/jwt#refresh-a-jwt[Refresh a JWT API].
