...
This event is generated when a user is deleted. This is different than `user.delete` because it is sent after the tx is committed, this cannot be transactional.
...
.Example Event JSON
----
include::../../../../src/json/events/user-delete-complete.json[]
----