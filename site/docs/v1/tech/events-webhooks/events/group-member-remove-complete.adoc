...
:type: group.member.remove.complete
...
This event is generated when one or more users are removed from a group. The JSON includes the Group and an array of removed members.

This event is not generated when all users are removed from a group.
...
.Example Event JSON
----
include::docs/src/json/events/group-member-remove-complete.json[]
----
