...
[field]#Certificate key# [optional]#Optional# [since]#Available since 1.48.0#::
...
[field]#Manual certificate entry# [since]#Available since 1.48.0#::
When this toggle is enabled, a PEM-encoded [field]#Certificate# can be entered manually for this webhook rather than selecting a [field]#Certificate key# from Key Master.
+
When the form is submitted, the hidden certificate option will be cleared. For example, if the webhook had previously specified a certificate manually, the [field]#Manual certificate entry# toggle is disabled, and the form is submitted, the [field]#Certificate# value fort he webhook will be cleared.
