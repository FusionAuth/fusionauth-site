...
+
Most events are considered tenant specific and selecting a tenant will limit you to only receiving events for those tenants. The exceptions are the following instance events which are not tenant specific:
+
- `audit-log.create`
- `event-log.create`
- `kickstart.success`
