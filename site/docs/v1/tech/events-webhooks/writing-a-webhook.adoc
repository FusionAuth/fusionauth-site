...
video::peyqQggiR4o[youtube,width=560,height=315]
...
include::docs/v1/tech/events-webhooks/_application-webhooks-warning.adoc[tags=simple-webhook]
...
Here's an example (https://github.com/FusionAuth/fusionauth-example-javascript-webhooks/blob/main/simple/app.js[available on Github]) of a Webhook written in Node using Express. In this example, if the event is a ``user.delete`` event, this code deletes all of the user's Todos.
...
[source,javascript,title=Example Webhook]
----
//...
include::https://raw.githubusercontent.com/FusionAuth/fusionauth-example-javascript-webhooks/main/simple/app.js[tags=simple-webhook]
//...
----
