...
In the [breadcrumb]#Applications# page, click on the icon:search[role=ui-button green,type=fas] button next to your application to view its details. In the modal that opens, scroll down a bit until you reach the [uielement]#SAML v2 Integration details# section. Copy the address from [uielement]#Metadata URL:# and open it a new tab in your browser. If the file didn't automatically download, save it.
...
(can you put an image of the tableau screen here?)
...
(image of tableau dashboard here if possible)
