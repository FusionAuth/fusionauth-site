...
layout: passport-doc
title: Tenants
description: Learn how setup and utilize Passport tenants
---

== Overview

If you wish to utilize multiple tenants in Passport, following this tutorial to get started.

== Create a Tenant

To create a new tenant, navigate to [breadcrumb]#Settings# icon:chevron-right[role=breadcrumb] [breadcrumb]#Tenants#.

image::create_tenant.png[Create a Tenant,width=1200,role=shadowed]
...
There are two supported methods to provide the tenant Id to Passport during the API request. The first is by using
...
[source,properties]
----
X-Passport-TenantId: 24b325e1-c1b8-4cc6-a2d3-ea316cec9e2e
----
--
...
In the following example screenshot the tenant `Acme Co.` has been selected, this API key can be used to make API requests to Passport in lie
of the HTTP header. Only Users, Groups and Applications belonging to the selected tenant will be visible to this API.

image::create_tenant_api_key.png[Create a Tenant API Key,width=1200,role=shadowed]
...
----
{
  "generalErrors" : [ {
    ...
    "message" : "A Tenant Id is required to complete this request. To complete this request, you may assign a Tenant to your API key, or add the X-Passport-TenantId HTTP request header with the Tenant Id."
  } ]
}
----
