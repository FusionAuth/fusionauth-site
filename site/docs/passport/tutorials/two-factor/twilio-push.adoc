...
Ensure you have successfully configured the link:../../integrations/twilio[Twilio integration].

== Generate a secret

A secret may be generated using the link:../../apis/two-factor#generate-a-secret[Generate a Two Factor Secret] API. Using this API is not required,
...
image::../../images/example-enable-push.png[Example Push Configuration,width=1000]

In the above example, you'll notice there is a button to _Send code to mobile phone_. You'll utilize the link:../../apis/two-factor#send-a-two-factor-code[Send a Two Factor Code]
...
[source,json]
----
include::docs/passport/src/json/two-factor/send/request.json[]
----
...
In this example we'll use the link:../../apis/two-factor#enable-two-factor[Enable Two Factor] API. This API will validate that the provided
...
need to be sent to the User during the authentication process the link:../../apis/two-factor#send-a-two-factor-code[Send a Two Factor Code] API
...
request to the link:../../apis/login#complete-two-factor-authentication[Complete Two Factor Authentication] API.
