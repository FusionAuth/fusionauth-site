...
First, you'll configure the application, then register the user for the applicaiton.
...
You can leave all the other fields empty because FusionAuth will choose a default value for those fields. Go ahead and save the application in order for the Client ID details to be autogenerated.
...
* Record the the generated "Client Id" and the "Client secret" in a text file or to the clipboard. You'll use these below.
...
If users of this application have unique configuration, such as a username, timezone, or languages, which are different than the user's defauls, you could configure them here.
...
{% include _image.liquid src="/assets/img/blogs/django-single-sign-on/successful-sso-based-login.png" alt="Successful single sign-on login." class="img-fluid" figure=true %}
