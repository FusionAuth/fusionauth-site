...
Step Up Authentication with JavaScript and FusionAuth


Broken authentication is the second most prevalent security risk for web applications according to the OWASP Top 10. When it isn't implemented correctly, it leaves your app vulnerable to attacks that could leak highly sensitive information, like a user's banking information.
...
On the front-end, we do things like form validation and make sure we aren't storing any credentials or secret tokens in the code. On the back-end, we check for packages that have known vulnerabilities, do additional data validation, and handle user roles. Some packages that are widely used for security on the front-end and back-end include: validatorjs, retirejs, helmetjs, and csrf.
...
You can start by downloading the self-hosted version of FusionAuth. You will need a license key to take advantage of the MFA we'll be using to add step-up auth in just a bit. If you only want to use google authenticator MFA, you won’t need a license, but this tutorial will use email MFA. Since the purpose of this article is implementing step-up, we'll do a brief overview of how the authorization code implementation works.
...
The setup for the authorization code part of the app in this GitHub repo in the _step-up-auth_ directory. It's based on the app created in a different tutorial if you want a deeper walkthrough of the authorization code implementation.
...
![the client-side app after logging in](0_react-app-w-login)[d][e][f]
...
The majority of this file is the same as the original from the tutorial we referenced earlier. We've just added a couple of methods and elements. The `showMethods` function just shows the dropdown list. The `finStepUp` method is what we call once a user has selected an MFA option.
...
One of the biggest benefits of using step-up auth is that it makes the user experience better. Although users will have to set up MFA, which can lead to some resistance, they won’t have to go through the MFA process as often with step-up and you’ll still keep the level of security you wanted.


You might hear step-up authentication and MFA referred to interchangeably, but they aren't the same thing. MFA refers to any authentication method that involves more than one method of authentication. Typically a username and password are the first method, and MFA layers on another. It uses other methods like emailed links or biometrics. Step-up authentication uses MFA when elevated actions are requested, but the initial login might work using just a username and password.
Conclusion
...
[a]You can see other posts in markdown here: https://github.com/FusionAuth/fusionauth-site/tree/master/site/_posts 


Don't worry about taking pretty screenshots, I can redo those. But if you do take screenshots, just make it clear what you are trying to emphasize.
[b]Thanks for sending this!
[c]no problem :)
[d]Will need a zip file of all images at some point.
[e]I'll send a zip with the final markdown and all the images.
[f]heya, I didn't see the zip file, please send over when you get a chance.
