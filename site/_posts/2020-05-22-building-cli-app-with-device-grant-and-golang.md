...
<!--more-->
In this post, we'll walk through an example of how to use [FusionAuth](https://fusionauth.io) and their [Golang Client](https://github.com/FusionAuth/go-client) to authenticate a small Golang CLI app using the [OAuth2 Device Code Grant ](https://www.oauth.com/oauth2-servers/device-flow/user-flow/) ("Device Grant").

If you're a bit hazy on the Device Grant, then here is a quick refresher from the [grant's RFC (#8628)](https://tools.ietf.org/html/rfc8628):
...
Next, we need to set up FusionAuth (FA) on our local machines to properly be able to test our CLI app. If you haven't already played around with FA then I suggest checking out their [5 minute setup guide](https://fusionauth.io/docs/v1/tech/5-minute-setup-guide) and stopping after you've completed step #3. That should get you set up with a running FA server which our CLI app can authenticate with.
...
ow you're probably saying to yourself "Oooh I get it now...", but let me break it down just in case:
...
Awesome -- next up let's talk about our `login` command as that is going to cover the most important part of our example application: Authentication using the Device Grant.


## Golang `login` Walkthrough

First, let's talk about the steps we need to accomplish for our Device Grant authentication:

1. We need to reach out to the FA server to start the Device Grant and fetch a User Code for our user to input.
...
To start the Device Grant off, we need to send the ["Device Authorization Request"](https://tools.ietf.org/html/rfc8628#section-3.1). To show how we'll do that, let's start off with some abridged code and then break it down:

```go
// ...

// LoginCmd provides the command for logging into the FA server using the Device Grant.
...
1. Our `Run` func starts off by creating a `FusionAuthClient` and assigning it to `faClient`. This is using FA's [go-client](github.com/FusionAuth/go-client/pkg/fusionauth) library, which **will be our primary interface to our FA server going forward** and is going to come in handy throughout our Device Grant.
...
```go
// LoginCmd provides the subcommand for logging into the FA server using the Device Grant.
var LoginCmd = &cobra.Command{
...
Now, since this is a post about using FusionAuth and not about the Giphy API, I won't dig into the `fetch` command too much and I'll leave that to you but suffice to say it's pretty simple. Here is the whole flow in action:
