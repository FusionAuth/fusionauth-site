...
It will use this to create a database user for FusionAuth to connect to. Further down the maintenance mode page, you can specify the username and password of this user:
...
After successful setup, view the application by clicking the green magnifying glass. We will see the endpoint details:
...
The OAuth `Client ID` and `Client Secret` values can also be found on the same screen, a bit lower.
...
We are going to set up a development environment for the Go ecosystem. First, we need to download Go from its [official website](https://golang.org/).
...
Depending on your golang setup you may need to `get` the nonstandard packages (I did):
