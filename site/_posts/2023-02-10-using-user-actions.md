...
We can now create a [user action definition](https://fusionauth.io/docs/v1/tech/apis/user-actions) with the email ids and POST using `/api/user-action`. Setting the `temporal` attribute to `true` allows us to set an `expiry` time when we execute the action on a user. This means that the action will automatically be removed from the user after the time set in `expiry`. We can also set `sendEndEvent` to `true` so that we can be notified via webhook when the access period has expired.
...
Record the `id` value. Here, it is `6f4115c0-3db9-4734-aeda-b9c3f7dc4269`. You can verify that the User Action was created by going to `Settings -> User Actions` in the FusionAuth admin portal.
...
Replace `<YOUR_FUSIONAUTH_URL>` and `<USER_ID>` with the appropriate values. 
...
For more information on FusionAuth User Actions, see [https://fusionauth.io/docs/v1/tech/apis/user-actions#overview](this overview) and [https://fusionauth.io/docs/v1/tech/apis/actioning-users](this reference on actioning users.)
