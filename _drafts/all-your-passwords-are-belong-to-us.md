...
Here's the reality, billions of credentials have been leaked or stolen. Most of them are now easily downloaded online and often database of identities include passwords in plain-text. While the original database might have stored the passwords in plain-text, many times passwords are one-way hashed. However, one-way hashing is only as secure as is mathematically feasible.

<!--more-->

## Hashing

Let's take a look at one-way hashing algorithms and how computers handle them. MD5 is an algorithm that uses various bit-wise operations on any number of bytes to produce a 128-bit "hash". The algorithm was designed specifically so that going from a hash back to the original bytes is impossible. Therefore, it works well for passwords where the plain-text form of the password should not be stored. Instead, the hash is stored and when a user is authenticated, the plain-text password they typed into the login form is hashed again and this hash is then compared to the hash in the database.

While one-way hashing means we aren't storing plain-text passwords, there is still an issue with trying to determine the plain-text password by trying all combinations of bytes until you generate the same hash that is stored in the database.
...
Here's an example of a salt and the resulting combination a the and the salt which is the hashed:
...
The second method that attackers use to crack passwords is called "brute force" cracking. This means that the attack writes a small computer program that generates all possible combinations of a characters that can be used for a password and then computes the hash for each. This program can also take a salt if the password was hash with a salt. The attacker then runs the program until it generates a hash that is the same as the hash from the database. Here's a simple Java program for cracking passwords. I left out some detail, such as all the possible password characters, to keep the code short, but you get the idea.
...
This program will generate all the possible passwords with lengths between 6 and 8 and then hash each one until it finds a match. This type of brute-force hacking takes some time though because it is trying a lot of combinations.
...
This equation allows us to see that as the password length increases, the number of seconds to brute-force attack the password also increases since the computer power is a fixed divisor. The increase in password complexity (length and possible characters) is called entropy. As the entropy increases, the time required to brute-force attack a password also increases.
...
Some in the industry have argued that enough is enough with consuming massive amounts of CPU and GPU cycles simply computing hashes for passwords. By forcing users to use long passwords, we get back a ton of computing power and can reduce costs by shutting for the 42 servers we have to run to keep up with login volumes.
...
At the time of when I wrote this blog post, there are still numerous simple algorithms that have not been attacked, meaning that no one has figured out a way to reduce the need to compute every possible hash. Therefore, it is still a safe assertion that using a simple algorithm on a long password is secure. Therefore, we will assume that the only reason not to force users to use long passwords is for the first reason mentioned above, "users don't like change". However, some users will change and some will already be using long passwords. 

## Our new idea

As an the provider of an identity management solution, we understand both sides of this debate. But we also have an idea for a solution. What if we dynamically changed the algorithm that FusionAuth used to hash passwords based on the length (and possibly complexity) of the password? Could this reduce the CPU/GPU consumption on average by enough to make a difference?
...
We think this idea has merit and as more and more people start using password managers and password generators, FusionAuth could dynamically scale back its CPU/GPU utilization. If you'd like to discuss on this feature, comment on this blog post or you can visit the FusionAuth GitHub issue for it and upvote or comment.
