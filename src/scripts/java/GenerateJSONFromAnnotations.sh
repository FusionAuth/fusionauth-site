#!/usr/bin/env bash
# Run GenerateJSONFromAnnotations.java against the latest fusionauth-app release.
...
javac -cp "$CP" -d "$WORK_DIR/classes" src/scripts/java/GenerateJSONFromAnnotations.java

OUTPUT_DIR="$(pwd)/astro/src/content/json/generated"
java -cp "$CP:$WORK_DIR/classes" GenerateJSONFromAnnotations "$OUTPUT_DIR" \
