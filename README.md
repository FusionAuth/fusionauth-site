...
For guides that are self-contained (readers type code from scratch rather than cloning a repository), there is no external repository to export to. These guides store their code examples in `astro/src/code-example-internal` instead. This folder is never touched by the export workflow — it exists purely as a source for the `LocalCode` component at build time. Use this folder for local-only guides; use `code-example-repositories` for anything that has a corresponding external repository.
